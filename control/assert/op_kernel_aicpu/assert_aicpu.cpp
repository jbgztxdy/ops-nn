...
string PrintOneElement(const Eigen::half& elt) { return to_string(static_cast<float>(elt)); }
...
uint32_t AssertCpuKernel::Compute(aicpu::CpuKernelContext& ctx)
{
    ...
    KERNEL_CHECK_NULLPTR(cond, KERNEL_STATUS_PARAM_INVALID, "Get input 0 failed.")
    KERNEL_CHECK_FALSE((cond->GetTensorShape()->GetDims() == 0), KERNEL_STATUS_PARAM_INVALID,
    ...
    KERNEL_CHECK_NULLPTR(sum_attr, KERNEL_STATUS_PARAM_INVALID, "Get summarize attr failed.")
...
template <typename T>
void AssertCpuKernel::PrintOneDim(int dim_index, const std::shared_ptr<TensorShape> shape, int64_t limit,
                                  int shape_size, const T* data, int64_t* data_index, string& result)
{
    if (*data_index >= limit) {
        return;
    }
...
template <typename T>
string AssertCpuKernel::SummarizeArray(const int64_t limit, const int64_t num_elts, const Tensor& t)
{
    string ret;

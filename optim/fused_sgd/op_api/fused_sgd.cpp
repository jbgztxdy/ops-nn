...
{
    L0_DFX(FusedSgd, paramsRef, gradsRef, momentumBufferListOptionalRef, gradScaleOptional, weightDecay, momentum, lr,
           dampening, nesterov, maximize, isFirstStep);
    ...
    if (momentumBufferListOptionalRef == nullptr) {
        ...
        const aclTensor* tmpTensor = executor->AllocTensor(
            momentumBufferListOptionalRefOutShape, (*paramsRef)[0]->GetDataType(), (*paramsRef)[0]->GetStorageFormat());
        ...
        if (momentumBufferListOptionalRefOut == nullptr) {
            return std::tuple<const aclTensorList*, const aclTensorList*, const aclTensorList*>(nullptr, nullptr,
                                                                                                nullptr);
        }
    } else {
    ...
    if (momentumBufferListOptionalRef == nullptr) {
        ...
        const aclTensor* tmpTensor = executor->AllocTensor(momentumBufferListOptionalRefInputShape,
                                                           (*paramsRef)[0]->GetDataType(),
                                                           (*paramsRef)[0]->GetStorageFormat());
    ...
    auto retAicore = ADD_TO_LAUNCHER_LIST_AICORE(
        FusedSgd, OP_INPUT(paramsRef, gradsRef, momentumBufferListOptionalRefInput, gradScaleOptional),
    ...
    if (momentumBufferListOptionalRef == nullptr) {
        return std::tuple<const aclTensorList*, const aclTensorList*, const aclTensorList*>(paramsRef, gradsRef,
                                                                                            nullptr);
    } else {

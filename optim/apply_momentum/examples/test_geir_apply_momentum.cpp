...
/*!
 * \file test_geir_apply_momentum.cpp
 * \brief
 */
...
#include "../op_graph/apply_momentum_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                             \
    ...
    applyMomentum1.set_input_##intputName(placeholder##intputIndex);                                            \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) applyMomentum1.set_attr_##attrName(attrValue)
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    applyMomentum1.set_input_##intputName(placeholder##intputIndex);                                        \
    applyMomentum1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                         \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    applyMomentum1.update_output_desc_##outputName(outputName##outputIndex##_desc)
...
{
    Status ret = SUCCESS;
    auto applyMomentum1 = op::ApplyMomentum("applyMomentum1");
    ...
    ADD_INPUT(5, momentum, inDtype, std::vector<int64_t>{1});
    ADD_INPUT_ATTR(use_nesterov, false);
    ADD_INPUT_ATTR(use_locking, false);

    outputs.push_back(applyMomentum1);
    return SUCCESS;
}

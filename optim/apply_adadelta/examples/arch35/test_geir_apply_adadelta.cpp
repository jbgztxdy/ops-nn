...
/**
 * @file test_geir_apply_adadelta.cpp
 * @brief ApplyAdadelta 算子 GE IR 图模式调用示例（对齐 canndev REG_OP(ApplyAdadelta) 单输出原型）
 *
 * 构图并运行单算子子图；ApplyAdadelta 有 7 个输入 / 1 个输出（accum / accum_update 通过 inplace 输入更新）：
 *   - 输入：var, accum, accum_update, lr, rho, epsilon, grad
 *   - 输出：var（单输出；accum / accum_update 输入即输出，inplace 更新）
    ...
    vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, DataType data_type, float value)
{
    ...
    // new 后立即在 Tensor 拷贝完成后释放，避免 host 端内存泄漏（批跑场景会暴露）
...
int CreateOppInGraph(
    ...
    Graph& graph)
{
    // 自定义代码：添加 ApplyAdadelta 单算子定义到图中
    auto adadelta = op::ApplyAdadelta("apply_adadelta1");
    ...
    int idx = 0;
    // 7 个输入 —— 顺序须与 REG_OP(ApplyAdadelta) 的 .INPUT 顺序一致
    if (AddDataInput(idx++, [&](Operator& d) { adadelta.set_input_var(d); }, inDtype, tensorShape, 1.0f,
                     graph, input, inputs) != SUCCESS) return FAILED;
    if (AddDataInput(idx++, [&](Operator& d) { adadelta.set_input_accum(d); }, inDtype, tensorShape, 0.1f,
                     graph, input, inputs) != SUCCESS) return FAILED;
    if (AddDataInput(idx++, [&](Operator& d) { adadelta.set_input_accum_update(d); }, inDtype, tensorShape, 0.05f,
                     graph, input, inputs) != SUCCESS) return FAILED;
    if (AddDataInput(idx++, [&](Operator& d) { adadelta.set_input_lr(d); }, inDtype, scalarShape, 0.01f,
                     graph, input, inputs) != SUCCESS) return FAILED;
    if (AddDataInput(idx++, [&](Operator& d) { adadelta.set_input_rho(d); }, inDtype, scalarShape, 0.9f,
                     graph, input, inputs) != SUCCESS) return FAILED;
    if (AddDataInput(idx++, [&](Operator& d) { adadelta.set_input_epsilon(d); }, inDtype, scalarShape, 1e-6f,
                     graph, input, inputs) != SUCCESS) return FAILED;
    if (AddDataInput(idx++, [&](Operator& d) { adadelta.set_input_grad(d); }, inDtype, tensorShape, 0.2f,
                     graph, input, inputs) != SUCCESS) return FAILED;
...
{
    const char* graph_name = "tc_apply_adadelta_ge_irrun_test";
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./tc_apply_adadelta_ge_irrun_test_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_apply_adadelta_ge_irrun_test_npu_output_" + std::to_string(i) + ".bin";

...
- **算子功能**：执行Adadelta优化器的单步参数更新。根据当前梯度、梯度平方累积和更新量平方累积，计算参数更新量并**原地**更新权重参数（inplace语义）。对标TensorFlow `tf.raw_ops.ApplyAdadelta`接口和PyTorch `torch.optim.Adadelta`的计算语义。
...
<tbody>
  ...
  <tr>
    <td>accum</td>
    <td>输入(inplace更新)</td>
    <td>梯度平方累积，对应公式中的accum。shape/dtype必须与var一致；Kernel内显式写回输入GM地址。</td>
    <td>FLOAT16、FLOAT</td>
    <td>ND</td>
  </tr>
  <tr>
    <td>accum_update</td>
    <td>输入(inplace更新)</td>
    <td>更新量平方累积，对应公式中的accum_update。shape/dtype必须与var一致；Kernel内显式写回输入GM地址。</td>
    <td>FLOAT16、FLOAT</td>
    <td>ND</td>
  </tr>
  <tr>
...
| 调用方式 | 调用样例                                                                   | 说明                                                           |
|--------------|------------------------------------------------------------------------|--------------------------------------------------------------|
| 图模式 | [test_geir_apply_adadelta](./examples/test_geir_apply_adadelta.cpp) | 通过 [算子IR](./op_graph/apply_adadelta_proto.h)构图方式调用ApplyAdadelta算子。 |

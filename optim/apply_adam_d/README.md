...
- 算子功能：根据Adam算法更新变量"var"。此算子与ApplyAdam计算逻辑相同，但额外输出更新后的m和v。
...
| 调用方式 | 调用样例                                                                   | 说明                                                           |
|--------------|------------------------------------------------------------------------|--------------------------------------------------------------|
| 图模式调用 | [test_geir_apply_adam_d](./examples/test_geir_apply_adam_d.cpp)   | 通过[算子IR](./op_graph/apply_adam_d_proto.h)构图方式调用ApplyAdamD算子。 |
...
// 内部 C++ 命名空间/类/结构体（NsApplyAdamax / class ApplyAdamax / struct ApplyAdamaxTilingData / ApplyAdamaxCompileInfo）
// 同步保持 lowercase m 形式以保持模块内部命名一致性。
// OpType `ApplyAdaMax`（M 大写）仅作用于 REG_OP / OpDef / IMPL_OP_* / 函数符号 apply_ada_max 等"对外接口层"。
// 详见 REVIEW.md N-4 deferred 说明。

...
/**
 * @file test_geir_apply_adamax.cpp
 * @brief ApplyAdaMax 算子 GE IR 图模式调用示例（对齐 canndev REG_OP(ApplyAdaMax) 单输出原型）
 *
 * 构图并运行单算子子图；ApplyAdaMax 有 9 个输入 / 1 个输出（m / v 通过 inplace 输入更新）：
 *   - 输入：var, m, v, beta1_power, lr, beta1, beta2, epsilon, grad
 *   - 输出：var（单输出；m / v 输入即输出，inplace 更新）
 *
                     ...
                     float value)
{
    ...
    // C-2 整改：new 后立即在 Tensor 拷贝完成后释放，避免 host 端内存泄漏（批跑场景会暴露）
...
{
    // N-6/N-7 整改：fopen 用 "wb" 二进制模式（Windows 可移植性），并校验返回值
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    ...
    // shape 约定
    std::vector<int64_t> tensorShape = {2, 4, 4, 4}; // var/m/v/grad 的 shape（共 128 元素）
    std::vector<int64_t> scalarShape = {1};          // beta1_power/lr/beta1/beta2/epsilon 的 shape
    ...
    adamax.update_output_desc_var(varOutDesc);

    outputs.push_back(adamax);
    return SUCCESS;
}

int main(int argc, char* argv[])
{
    const char* graph_name = "tc_apply_adamax_ge_irrun_test";
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./tc_apply_adamax_ge_irrun_test_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_apply_adamax_ge_irrun_test_npu_output_" + std::to_string(i) + ".bin";

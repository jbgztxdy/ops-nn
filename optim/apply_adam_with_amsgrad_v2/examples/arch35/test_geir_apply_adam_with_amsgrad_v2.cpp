...
/**
 * @file test_geir_apply_adam_with_amsgrad_v2.cpp
 * @brief ApplyAdamWithAmsgradV2 算子 GE IR 图模式调用示例（对齐 REG_OP(ApplyAdamWithAmsgradV2) 11 入 / 4 出原型）
 *
 * 构图并运行单算子子图；ApplyAdamWithAmsgradV2 有 11 个输入 / 4 个输出（var/m/v/vhat 通过 inplace 输入更新）：
 *   - 输入：var, m, v, vhat, beta1_power, beta2_power, lr, beta1, beta2, epsilon, grad
 *   - 输出：var_out, m_out, v_out, vhat_out（in-place 更新 var/m/v/vhat）
 *
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    ...
    // shape 约定
    std::vector<int64_t> tensorShape = {2, 4, 4, 4}; // var / m / v / vhat / grad 的 shape（共 128 元素）
    std::vector<int64_t> scalarShape = {1};          // beta1_power / beta2_power / lr / beta1 / beta2 / epsilon
    ...
    amsgrad.update_output_desc_var_out(outDesc);
    amsgrad.update_output_desc_m_out(outDesc);
    amsgrad.update_output_desc_v_out(outDesc);
    amsgrad.update_output_desc_vhat_out(outDesc);

    outputs.push_back(amsgrad);
    return SUCCESS;
}

int main(int argc, char* argv[])
{
    const char* graph_name = "tc_apply_adam_with_amsgrad_v2_ge_irrun_test";
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./tc_apply_adam_with_amsgrad_v2_ge_irrun_test_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_apply_adam_with_amsgrad_v2_ge_irrun_test_npu_output_" + std::to_string(i) + ".bin";

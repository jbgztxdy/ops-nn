...
/**
 * @file test_geir_sparse_apply_proximal_adagrad.cpp
 * @brief SparseApplyProximalAdagrad 算子 GE IR 图模式调用示例（对齐 REG_OP(SparseApplyProximalAdagrad)）
 *
 * 构图并运行单算子子图；SparseApplyProximalAdagrad 有 7 个输入 / 2 个输出（inplace 更新）：
 *   - 输入：var, accum, lr, l1, l2, grad, indices
 *   - 输出：var, accum
...
{
    auto op0 = op::SparseApplyProximalAdagrad("sparse_apply_proximal_adagrad1");

    const int64_t rowNum = 4;
    std::vector<int64_t> tensorShape = {rowNum, 8}; // var / accum shape (N=4, inner=8)
    std::vector<int64_t> gradShape = {2, 8};        // grad shape (M=2, inner=8)
    std::vector<int64_t> indicesShape = {2};        // indices shape (M=2)
    std::vector<int64_t> scalarShape = {1};         // lr / l1 / l2

    int idx = 0;
    // 7 inputs — order must match REG_OP(SparseApplyProximalAdagrad) .INPUT order
    if (AddDataInput(
            idx++, [&](Operator& d) { op0.set_input_var(d); }, inDtype, tensorShape, 1.0f, graph, input, inputs) !=
        SUCCESS)
        return FAILED;
    if (AddDataInput(
            idx++, [&](Operator& d) { op0.set_input_accum(d); }, inDtype, tensorShape, 0.5f, graph, input, inputs) !=
        SUCCESS)
        return FAILED;
    if (AddDataInput(
...
{
    const char* graph_name = "tc_sparse_apply_proximal_adagrad_ge_irrun_test";
    ...
    for (int i = 0; i < input_num; i++) {
        string input_file = "./tc_sparse_apply_proximal_adagrad_ge_irrun_test_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        string output_file = "./tc_sparse_apply_proximal_adagrad_ge_irrun_test_npu_output_" + std::to_string(i) +
                             ".bin";

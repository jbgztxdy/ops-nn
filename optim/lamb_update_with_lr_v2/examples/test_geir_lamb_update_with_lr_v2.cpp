...
/*!
 * \file test_geir_lamb_update_with_lr_v2.cpp
 * \brief
 */
...
{
    auto op0 = op::LambUpdateWithLrV2("lamb_update_with_lr_v2_0");
    std::vector<int64_t> fullShape = {2, 16};
    std::vector<int64_t> scalarShape = {1};
    ADD_INPUT(1, x1, inDtype, scalarShape);
    ADD_INPUT(2, x2, inDtype, scalarShape);
    ADD_INPUT(3, x3, inDtype, scalarShape);
    ADD_INPUT(4, x4, inDtype, fullShape);
    ADD_INPUT(5, x5, inDtype, fullShape);
    ADD_INPUT(6, greater_y, inDtype, scalarShape);
    ADD_INPUT(7, select_e, inDtype, scalarShape);
...
{
    Graph graph("test_lamb_update_with_lr_v2");

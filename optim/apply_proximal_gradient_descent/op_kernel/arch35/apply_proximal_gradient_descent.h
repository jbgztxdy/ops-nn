    ...
    GM_ADDR alpha, GM_ADDR l1, GM_ADDR l2)
{
    ...
    alphaF = static_cast<float>(((__gm__ T*)alpha)[0]);
    l1F    = static_cast<float>(((__gm__ T*)l1)[0]);
    l2F    = static_cast<float>(((__gm__ T*)l2)[0]);
#else
    alphaF = static_cast<float>(alphaGm.GetValue(0));
    l1F    = static_cast<float>(l1Gm.GetValue(0));
    l2F    = static_cast<float>(l2Gm.GetValue(0));
#endif
    alphaS_   = alphaF;
    alphaL1_  = alphaF * l1F;
    ...
    int64_t progress, int64_t currentNum)
{
    ...
    AscendC::DataCopyPad(varLocal,   varGm[progress * ubLength_],   copyParams, padParams);
    AscendC::DataCopyPad(deltaLocal, deltaGm[progress * ubLength_], copyParams, padParams);
...
__aicore__ inline void ApplyProximalGradientDescent<T, BUFFER_MODE>::Compute(int64_t currentNum)
{
    ...
    // sign(prox)：clamp(prox * BIG, -1, 1)，零值保持为 0
    AscendC::Muls(tmpSign, tmpProx, 1e30f, currentNum);
    AscendC::Mins(tmpSign, tmpSign, 1.0f, currentNum);
    AscendC::Maxs(tmpSign, tmpSign, -1.0f, currentNum);

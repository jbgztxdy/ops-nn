...
/**
 * @file test_geir_apply_proximal_gradient_descent.cpp
 * @brief GE IR pathway verification for ApplyProximalGradientDescent operator
 *
 * Validates the GE IR (Graph Engine IR) invocation pathway:
 *   1. Build a GE graph with the ApplyProximalGradientDescent operator
...
// Use built-in IR definition (output named "var") for GE IR compatibility
...
{
    Status ret = SUCCESS;
    auto op1 = op::ApplyProximalGradientDescent("op1");

    // --- Input: var (shape {4, 8}, main tensor) ---
    std::vector<int64_t> varShape = {4, 8};
    ADD_INPUT(1, var, inDtype, varShape);

    // --- Input: alpha (scalar, shape {1}) ---
    std::vector<int64_t> alphaShape = {1};
    ADD_INPUT(2, alpha, inDtype, alphaShape);

    // --- Input: l1 (scalar, shape {1}) ---
    std::vector<int64_t> l1Shape = {1};
    ADD_INPUT(3, l1, inDtype, l1Shape);

    // --- Input: l2 (scalar, shape {1}) ---
    std::vector<int64_t> l2Shape = {1};
    ADD_INPUT(4, l2, inDtype, l2Shape);

    // --- Input: delta (same shape as var, {4, 8}) ---
    std::vector<int64_t> deltaShape = {4, 8};
    ADD_INPUT(5, delta, inDtype, deltaShape);

    // --- Output: var (same shape as input var, {4, 8}) ---
    // Note: built-in IR defines output as "var", not "var_out"
    std::vector<int64_t> outShape = {4, 8};
    ADD_OUTPUT(1, var, inDtype, outShape);
...
{
    const char* graph_name = "e2e_verify_geir_apply_proximal_gradient_descent";

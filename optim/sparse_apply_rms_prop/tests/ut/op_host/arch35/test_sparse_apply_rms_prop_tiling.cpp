...
/*!
 * \file test_sparse_apply_rms_prop_tiling.cpp
 * \brief Tiling UT for SparseApplyRMSProp operator (arch35 / ascend950).
 *
 * Schema:
 *   Inputs  (9): var, ms, mom, lr, rho, momentum, epsilon, grad, indices
 *     - var / ms / mom: dtype in {FP32, FP16, BF16}, shape [N, D...]
 *     - lr / rho / momentum / epsilon: scalar tensors (shape == {1})
 *     - grad: shape [M, D...]
 *     - indices: dtype in {INT32, INT64}, shape [M]
 *   Outputs (3): var, ms, mom
 *
 * TilingData fields (sparse_apply_rms_prop_tiling_data.h):
 *   - needCoreNum  (int32)
 *   - totalIndices (int64)
 *   - innerDim     (int64)
 *   - varFirstDim  (int64)
...
struct SparseApplyRMSPropUtCompileInfo {};

struct SparseApplyRMSPropTilingResult {
...
struct SparseApplyRMSPropTilingArgs {
...
static SparseApplyRMSPropTilingResult DoSparseApplyRMSPropTilingCase(const SparseApplyRMSPropTilingArgs& args)
{
    SparseApplyRMSPropTilingResult result{ge::GRAPH_FAILED, 0, 0, 0, 0, 0};
    ...
    std::string opType("SparseApplyRMSProp");
...
// =====================================================================
// T01 FP32 / INT32, basic shape -> key 0, fields parsed.
// var {4,8}, grad {2,8}, indices {2} -> totalIndices=2, innerDim=8, varFirstDim=4
// =====================================================================
...
// totalWork = 2048 * 1024 = 2097152, calcCoreNum = ceil/1024 = 2048,
// needCoreNum = min(2048, 64) = 64
// =====================================================================
TEST_F(TestSparseApplyRMSPropTiling, sparse_apply_rms_prop_large_multicore)
{
    ...
    EXPECT_EQ(result.innerDim, 1024);

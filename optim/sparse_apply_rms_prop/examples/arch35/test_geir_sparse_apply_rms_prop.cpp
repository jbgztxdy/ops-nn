...
/**
 * @file test_geir_sparse_apply_rms_prop.cpp
 * @brief SparseApplyRMSProp 算子 GE IR 图模式调用示例（对齐 REG_OP(SparseApplyRMSProp)）
 *
 * 构图并运行单算子子图；SparseApplyRMSProp 有 9 个输入 / 3 个输出（inplace 更新）：
 *   - 输入：var, ms, mom, lr, rho, momentum, epsilon, grad, indices
 *   - 输出：var, ms, mom
...
{
    const char* graph_name = "tc_sparse_apply_rms_prop_ge_irrun_test";
    ...
    for (int i = 0; i < input_num; i++) {
        string input_file = "./tc_sparse_apply_rms_prop_ge_irrun_test_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        string output_file = "./tc_sparse_apply_rms_prop_ge_irrun_test_npu_output_" + std::to_string(i) + ".bin";

...
ge::graphStatus AdamTiling::GetShapeInfo()
{
    ...
    if (dtype == ge::DT_FLOAT16)      dtype_size_ = 2;
    ...
    // 维测: 输入预处理结果
    OP_LOGI(ctx_->GetNodeName(), "GetShapeInfo done rank %ld dtype %ld ub %lu core %lu",
        rank_, dtype_size_, compileInfo->ubSize, compileInfo->coreNum);
...
ge::graphStatus AdamTiling::DoTilingAndSet()
{
    ...
    auto* compileInfo = static_cast<const AdamCompileInfo*>(
        ctx_->GetCompileInfo());
    ...
    for (int64_t i = 0; i < num_in; i++)
        OP_LOGI(ctx_->GetNodeName(), "TilingData input[%ld]: shape=%s stride=%s",
                i, Arr2String(tiling->input_shapes[i], R).c_str(),
                   Arr2String(tiling->input_strides[i], R).c_str());
    for (int64_t i = 0; i < num_out; i++)
        OP_LOGI(ctx_->GetNodeName(), "TilingData output[%ld]: shape=%s stride=%s",

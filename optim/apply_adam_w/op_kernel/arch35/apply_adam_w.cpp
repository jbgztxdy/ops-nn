                                        ...
                                        GM_ADDR v_out, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if constexpr (static_cast<int>(amsgrad) == 0) {
        ...
        sch.Init(var, m, v, beta1_power, beta2_power, lr, weight_decay, beta1, beta2, epsilon, grad, var_out, m_out,
                 v_out);
        sch.Process();
    } else if constexpr (static_cast<int>(amsgrad) == 1) {
        ElementwiseSch<schMode, ApplyAdamWAmsGradDAG<DTYPE_VAR, float>::OpDag> sch(&(tilingData.eleBaseTilingData),
                                                                                   &pipe);
        sch.template SetVar<float, 0>(static_cast<float>(tilingData.maximizeFactor));
        sch.Init(var, m, v, beta1_power, beta2_power, lr, weight_decay, beta1, beta2, epsilon, grad, max_grad_norm,
                 var_out, m_out, v_out);
        sch.Process();
    }
}
...
bool LambApplyOptimizerAssignTiling::IsCapable()
{
    return true;
}
...
ge::graphStatus LambApplyOptimizerAssignTiling::DoOpTiling()
{
    ...
    if (input0DType == ge::DT_FLOAT16) {
        BroadcastBaseTiling<LambApplyOptimizerAssignOp::LambApplyOptimizerAssignCompute<half, float>::OpDag> brcBaseTiling(
        ...
        BroadcastBaseTiling<LambApplyOptimizerAssignOp::LambApplyOptimizerAssignCompute<float, float>::OpDag> brcBaseTiling(
            context_, static_cast<uint32_t>(BROADCAST_KERNEL_TYPE::KERNEL_TYPE_NDDMA));
        OP_CHECK_IF(
            brcBaseTiling.DoTiling() == ge::GRAPH_FAILED,
            OP_LOGE(context_->GetNodeName(), "Do tiling failed. Please check the detailed log."),
        ...
        OP_LOGE_FOR_INVALID_DTYPE(
            context_->GetNodeName(), "grad", Ops::Base::ToString(input0DType).c_str(),
            "fp16 or fp32");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
...
ge::graphStatus LambApplyOptimizerAssignTiling::DoLibApiTiling()
{
    return ge::GRAPH_SUCCESS;
}

uint64_t LambApplyOptimizerAssignTiling::GetTilingKey() const
{
    return tilingKey;
}

ge::graphStatus LambApplyOptimizerAssignTiling::GetWorkspaceSize()
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus LambApplyOptimizerAssignTiling::PostTiling()
{
    return ge::GRAPH_SUCCESS;
}
...
REGISTER_OPS_TILING_TEMPLATE(LambApplyOptimizerAssign, LambApplyOptimizerAssignTiling, LAMB_APPLY_OPTIMIZER_ASSIGN_TILING_PRIORITY);
} // namespace optiling

...
__global__ __aicore__ void lamb_apply_optimizer_assign(GM_ADDR grad, GM_ADDR inputv, GM_ADDR inputm, GM_ADDR input3,
                                                       GM_ADDR mul0_x, GM_ADDR mul1_x, GM_ADDR mul2_x, GM_ADDR mul3_x,
                                                       GM_ADDR add2_y, GM_ADDR steps, GM_ADDR do_use_weight,
                                                       GM_ADDR weight_decay_rate, GM_ADDR output0, GM_ADDR output1,
                                                       GM_ADDR output2, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if constexpr (std::is_same<DTYPE_GRAD, half>::value) {
        ...
        sch.Process(grad, inputv, inputm, input3, mul0_x, mul1_x, mul2_x, mul3_x, add2_y, steps, do_use_weight,

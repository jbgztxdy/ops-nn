...
1. **安装社区版CANN toolkit包**
    ...
    根据实际环境，下载对应`Ascend-cann-toolkit_${cann_version}_linux-${arch}.run`包，[下载链接](https://mirrors.huaweicloud.com/artifactory/cann-run/software)。
...
2. **安装社区版CANN ops包（运行态依赖）**
    ...
    根据产品型号和环境架构，下载对应`Ascend-cann-${soc_name}-ops_${cann_version}_linux-${arch}.run`包，[下载链接](https://mirrors.huaweicloud.com/artifactory/cann-run/software)。

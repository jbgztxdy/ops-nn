...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                             \
    ...
    reluV21.set_input_##intputName(placeholder##intputIndex);                                                   \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) reluV21.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                   \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                  \
    reluV21.set_input_##intputName(placeholder##intputIndex);                                               \
    reluV21.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    reluV21.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto reluV21 = op::ReluV2("reluV21");
    std::vector<int64_t> xShape = {(26, 20, 8)};
    ADD_INPUT(1, x, inDtype, xShape);

    outputs.push_back(reluV21);
    // 添加完毕
    return SUCCESS;
}

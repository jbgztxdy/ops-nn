...
- **计算公式**：
  ...
  其中 `lambd` 为阈值参数（默认 0.5），`bias` 为偏移量参数（默认 0.0）。当 `lambd < 0` 时，内部钳位为 0。
...
<tbody>
  <tr>
    <td>input_x</td>
    <td>输入</td>
    <td>输入张量，对应公式中的 self。元素值与 lambd 比较决定归属区域。</td>
    <td>FLOAT16、FLOAT</td>
    <td>ND</td>
  </tr>
  <tr>
    <td>lambd</td>
    <td>属性</td>
    <td>阈值参数，对应公式中的 lambd。当 self_i > lambd 时进入正区域，self_i &lt; -lambd 时进入负区域。默认值为 0.5。当 lambd &lt; 0 时，内部钳位为 0。</td>
    <td>FLOAT</td>
    <td>-</td>
  </tr>
  <tr>
    <td>bias</td>
    <td>属性</td>
    <td>偏移量参数，对应公式中的 bias。在正区域减去 bias，在负区域加上 bias。默认值为 0.0。</td>
    <td>FLOAT</td>
    <td>-</td>
  </tr>
  <tr>
    <td>output_y</td>
    <td>输出</td>
    <td>输出张量，对应公式中的 out。与 input_x 的 dtype 和 shape 完全相同。</td>
    <td>FLOAT16、FLOAT</td>
    <td>ND</td>
  </tr>
</tbody></table>

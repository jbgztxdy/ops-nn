...
def gen_golden_data_simple(shape_str, dtype):
    ...
    updates = np.random.uniform(-2, 2, shape).astype(np_dtype)
    ...
    da = np.sum(updates, axis=0, keepdims=True)
    
    if x_dtype.name in ["bfloat16", "float16"]:
        da = da.astype(x_dtype, copy=False)
    
    grads.tofile("./input_grads.bin")
    features.tofile("./input_features.bin")
    weights.tofile("./input_weights.bin")
    updates.tofile("./input_updates.bin")
    da.tofile("./golden_da.bin")

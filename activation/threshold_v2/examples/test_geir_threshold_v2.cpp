...
/*!
 * \file test_geir_threshold_v2.cpp
 * \brief
 */
...
#include "../op_graph/threshold_v2_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape, useRandom, minVal, maxVal, fixedValue) \
    ...
    if (useRandom) {                                                                                            \
        ret = GenRandomDataFloat32(                                                                             \
            placeholder##intputIndex##_shape, tensor_placeholder##intputIndex,                                  \
...
#define ADD_INPUT_ATTR(attrName, attrValue) thresholdV21.set_attr_##attrName(attrValue)
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    ...
    thresholdV21.update_output_desc_##outputName(outputName##outputIndex##_desc)
...
int32_t GenRandomDataFloat32(vector<int64_t> shapes,
    Tensor& input_tensor, TensorDesc& input_tensor_desc, float minVal, float maxVal)
{
...
{
    Status ret = SUCCESS;
    auto thresholdV21 = op::ThresholdV2("thresholdV21");
    std::vector<int64_t> xShape = {8};
    std::vector<int64_t> thresholdShape = {8};
    std::vector<int64_t> valueShape = {8};
    ADD_INPUT(1, x, inDtype, xShape, true, -2.0f, 2.0f, 0.0f);
    ADD_INPUT(2, threshold, inDtype, thresholdShape, false, 0.0f, 0.0f, 0.5f);
    ADD_INPUT(3, value, inDtype, valueShape, false, 0.0f, 0.0f, 0.0f);

    outputs.push_back(thresholdV21);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        float* thresholdData = (float*)input[1].GetData();
        float* valueData = (float*)input[2].GetData();
        float* resultData = (float*)output_data_i;
        LOG_PRINT("\n========== threshold_v2 result ==========\n");
        LOG_PRINT("x            threshold    value        result\n");
        for (int64_t j = 0; j < output_shape; j++) {
            LOG_PRINT("%f    %f    %f    %f\n", xData[j], thresholdData[j], valueData[j], resultData[j]);
        }
        LOG_PRINT("==========================================\n");
    }

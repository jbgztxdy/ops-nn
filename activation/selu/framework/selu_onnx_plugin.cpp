...
static Status ParseParamsSelu(const Message* op_src, ge::Operator& op_dest)
{
    ...
    for (const auto& attr : node->attribute()) {
        if (attr.name() == "alpha" && attr.type() == ge::onnx::AttributeProto::FLOAT)
            alpha = attr.f();
        if (attr.name() == "gamma" && attr.type() == ge::onnx::AttributeProto::FLOAT)
            gamma = attr.f();
    }
...
REGISTER_CUSTOM_OP("Selu")
    .FrameworkType(ONNX)
    .OriginOpType({ge::AscendString("ai.onnx::8::Selu"), ge::AscendString("ai.onnx::9::Selu"),
                   ge::AscendString("ai.onnx::10::Selu"), ge::AscendString("ai.onnx::11::Selu"),
                   ge::AscendString("ai.onnx::12::Selu"), ge::AscendString("ai.onnx::13::Selu"),
                   ge::AscendString("ai.onnx::14::Selu"), ge::AscendString("ai.onnx::15::Selu"),
                   ge::AscendString("ai.onnx::16::Selu"), ge::AscendString("ai.onnx::17::Selu"),

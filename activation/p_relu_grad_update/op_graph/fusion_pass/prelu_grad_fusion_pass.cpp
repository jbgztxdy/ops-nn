...
std::vector<es::EsTensorHolder> CreatePatternPreluGrad(es::EsGraphBuilder& graph_builder, const es::EsTensorHolder& dy,
                                                       const es::EsTensorHolder& x, const es::EsTensorHolder& weight)
{
    ...
    OP_LOGE_IF(es::AddEdgeAndUpdatePeerDesc(*graph, *dy.GetProducer(), dy.GetProducerOutIndex(), prelu_grad, 0) !=
                   GRAPH_SUCCESS,
    ...
    OP_LOGE_IF(
        ...
        default_res, PASS_NAME.c_str(), "AddEdgeAndUpdatePeerDesc failed.");
    OP_LOGE_IF(es::AddEdgeAndUpdatePeerDesc(*graph, *weight.GetProducer(), weight.GetProducerOutIndex(), prelu_grad,
                                            SIZE_2) != GRAPH_SUCCESS,
               default_res, PASS_NAME.c_str(), "AddEdgeAndUpdatePeerDesc failed.");
...
std::vector<es::EsTensorHolder> BuildUpdateNode(es::EsGraphBuilder& graph_builder, const es::EsTensorHolder& r_dy,
                                                const es::EsTensorHolder& r_x, const es::EsTensorHolder& r_weight)
{
    ...
    es::AddEdgeAndUpdatePeerDesc(*replace_graph, *r_weight.GetProducer(), r_weight.GetProducerOutIndex(),
                                 prelu_grad_update, SIZE_2);
...
es::EsTensorHolder BuildReduceNode(es::EsGraphBuilder& graph_builder, const es::EsTensorHolder& r_dy,
                                   const es::EsTensorHolder& r_x, const es::EsTensorHolder& r_weight,
                                   const es::EsTensorHolder& update_holder)
{
    ...
    es::AddEdgeAndUpdatePeerDesc(*replace_graph, *r_weight.GetProducer(), r_weight.GetProducerOutIndex(),
                                 prelu_grad_reduce, SIZE_2);
    es::AddEdgeAndUpdatePeerDesc(*replace_graph, *update_holder.GetProducer(), update_holder.GetProducerOutIndex(),
                                 prelu_grad_reduce, SIZE_3);
...
static void SetSubNodeDesc(GNode& update_node, GNode& reduce_node, es::EsTensorHolder& dy, es::EsTensorHolder& x,
                           es::EsTensorHolder& weight)
{
    TensorDesc dy_desc;
    TensorDesc x_desc;
    TensorDesc w_desc;
...
GraphUniqPtr PReluGradFusionPass::Replacement(const std::unique_ptr<MatchResult>& match_result)
{
    ...
    auto r_dy = replace_graph_builder.CreateInput(0, "dy", input_dtypes[0], input_formats[0],
                                                  input_shapes[0].GetDims());
    auto r_x = replace_graph_builder.CreateInput(1, "x", input_dtypes[1], input_formats[1], input_shapes[1].GetDims());
    auto r_weight = replace_graph_builder.CreateInput(2, "weight", input_dtypes[2], input_formats[2],
                                                      input_shapes[2].GetDims());
    ...
    auto p_relu_grad_reduce_out = BuildReduceNode(replace_graph_builder, r_dy, r_x, r_weight,
                                                  p_relu_grad_update_out[1]);

    GraphUniqPtr res_graph = replace_graph_builder.BuildAndReset(
        {es::EsTensorHolder(p_relu_grad_update_out[0]), es::EsTensorHolder(p_relu_grad_reduce_out)});

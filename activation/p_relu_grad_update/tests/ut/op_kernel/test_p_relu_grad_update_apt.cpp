...
extern "C" __global__ __aicore__ void prelu_grad_update(GM_ADDR grads, GM_ADDR features, GM_ADDR weights, GM_ADDR dx,
                                                        GM_ADDR update, GM_ADDR workspace, GM_ADDR tiling);

class p_relu_grad_update_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "p_relu_grad_update_test SetUp\n" << endl; }
...
TEST_F(p_relu_grad_update_test, test_case_fp32_1)
{
    ...
    uint32_t blockDim = 1;
    kernel_ut::SetupTestEnvironment("activation/p_relu_grad_update/tests/ut/op_kernel/p_relu_grad_update_data",
                                    "p_relu_grad_update_data");
    ...
    auto KernelPReluGradUpdate = [](GM_ADDR grads, GM_ADDR features, GM_ADDR weights, GM_ADDR dx, GM_ADDR update,
                                    GM_ADDR workspace, GM_ADDR tiling) {
        ::prelu_grad_update<202>(grads, features, weights, dx, update, workspace, tiling);
    };
    ICPU_SET_TILING_KEY(1003);
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(KernelPReluGradUpdate, blockDim, grads, features, weights, dx, update, workspace,

                           ...
                           const aclTensor* selfOrResult, aclOpExecutor* executor)
{
    ...
    auto retAicore = ADD_TO_LAUNCHER_LIST_AICORE(EluGradV2, OP_INPUT(gradOutput, selfOrResult), OP_OUTPUT(gradIntput),
                                                 OP_ATTR(alpha, scale, inputScale, isResult));
    OP_CHECK_ADD_TO_LAUNCHER_LIST_AICORE(retAicore != ACLNN_SUCCESS, return nullptr,
                                         "EluGradV2 ADD_TO_LAUNCHER_LIST_AICORE failed.");
    return gradIntput;
}
} // namespace l0op
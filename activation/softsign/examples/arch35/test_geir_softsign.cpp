...
#include "../../op_graph/softsign_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    softsign_op.set_input_##intputName(placeholder##intputIndex);                                                   \
...
{
    Status ret = SUCCESS;
    auto softsign_op = op::Softsign("softsign");
    std::vector<int64_t> xShape = {4, 4};
    ADD_INPUT(1, x, inDtype, xShape);

    outputs.push_back(softsign_op);
    return SUCCESS;
}

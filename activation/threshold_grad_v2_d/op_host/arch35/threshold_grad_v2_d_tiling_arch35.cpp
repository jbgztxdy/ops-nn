        ...
        for (size_t i = 0; i < padded_in.size(); i++) {
            if (padded_in[i][d] != 1) {
                if (ref == -1)
                    ...
                    OP_LOGE("PadAndSqueeze", "dim %d broadcast incompatible: input[%d] size %d != %d", (int)d, (int)i,
                            (int)padded_in[i][d], (int)ref);
                    return false;
                }
            }
        }
        for (size_t i = 0; i < padded_out.size(); i++) {
            if (padded_out[i][d] != 1) {
                if (ref == -1)
                    ...
                    OP_LOGE("PadAndSqueeze", "dim %d broadcast incompatible: output[%d] size %d != %d", (int)d, (int)i,
                            (int)padded_out[i][d], (int)ref);
                    return false;
                }
            }
        }
...
public:
    ...
    ge::graphStatus RunTiling()
    {
        ...
        if (total_out == 0) {
            ctx_->SetBlockDim(1);
            ctx_->SetTilingKey(GET_TPL_TILING_KEY(THRESHOLD_GRAD_V2_D_RANK_4));
        ...
        if (rank_ <= 4) {
            ...
            ctx_->SetTilingKey(GET_TPL_TILING_KEY(THRESHOLD_GRAD_V2_D_RANK_8));
        }
        return ret;
    }
...
private:
    ge::graphStatus GetShapeInfo()
    {
        ...
        const std::set<ge::DataType> sup = {ge::DT_FLOAT16, ge::DT_FLOAT, ge::DT_BF16,
        ...
        if (attrs != nullptr) {
            const float* th = attrs->GetFloat(tgv2d::THRESHOLD_ATTR_IDX);
            if (th)
                threshold_ = *th;
        }

        OP_CHECK_IF(!tgv2d::PadAndSqueeze(raw_input_shapes_, raw_output_shapes_, max_bro_shape_, normal_input_shapes_,
    ...
    ge::graphStatus DoTilingAndSet()
    {
        ...
        for (int64_t i = 0; i < num_in; i++)
            tgv2d::PrecomputeStrides(normal_input_shapes_[i], in_strides[i]);
        for (int64_t i = 0; i < num_out; i++)
            tgv2d::PrecomputeStrides(normal_output_shapes_[i], out_strides[i]);
        ...
        OP_LOGI(
            ctx_->GetNodeName(),
            "TGV2D per_buf=%ld rank=%ld->R=%d mb=%s split(ax=%ld ai=%ld ao=%ld tail=%ld) mc(c=%ld t=%ld m=%ld ct=%ld)",
            ...
            tiling->multicore.total_tiles, tiling->multicore.tiles_main, tiling->multicore.cores_tail);
        return ge::GRAPH_SUCCESS;
    }

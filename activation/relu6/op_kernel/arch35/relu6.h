 ...
 * UB 使用: 4 个 LocalTensor（inputLocal x2 双缓冲 + tmpLocal + outputLocal）
 */
...
__aicore__ inline void Relu6<T>::Init(GM_ADDR x, GM_ADDR y, const Relu6TilingData* tilingData)
{
    ...
    // outputQueue: 1 个 buffer（输出结果）
    // tmpBuf: 1 个 buffer（Maxs 中间结果，独立管理）
...
__aicore__ inline void Relu6<T>::Compute(int64_t currentNum)
{
    ...
    // 使用独立 tmpBuf 保存 Maxs 中间结果，避免从 outputQueue 多次 AllocTensor
    AscendC::LocalTensor<T> tmpLocal = tmpBuf.template Get<T>();

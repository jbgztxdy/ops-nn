...
def gen_golden_data_simple(shape_str, dtype, negative_slope):
    ...
    features_torch = torch.from_numpy(data_features.copy())
    golden = torch.ops.aten.leaky_relu_backward(gradients_torch, features_torch, negative_slope,False)
    golden = golden.numpy()
    
    if x_dtype.name in ["bfloat16", "float16"]:
        golden = golden.astype(x_dtype, copy=False)
    
    data_gradients.tofile("./input_gradients.bin")

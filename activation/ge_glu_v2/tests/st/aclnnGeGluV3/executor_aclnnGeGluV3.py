...
@register("function_gegluv3")
class aclnnGeGluV3(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        activate_left = input_data.kwargs.get("activateLeft", False)
        ...
        def process(input_x, dim, activate_left, approximate):
            ...
            if activate_left:
                gate, x = input_x.chunk(2, dim=dim)
            else:
        ...
        def ge_glu_v2_golden(input_x, dim, approximate, activateLeft):
            """golden参考实现"""
            dtype = input_x.dtype
            ...
            try:
                if input_x.dim() == 0:
                    ...
                    output, gelu_output = process(input_x, dim, activateLeft, approximate)
        ...
        # 执行算子
        try:
            out, out_gelu = ge_glu_v2_golden(input_tensor, dim, approximate, activate_left)
        except Exception as e:
            print(f"Error executing ge_glu_v2_golden: {e}")
            print(f"Input shape: {input_tensor.shape}")
            print(f"dim: {dim}")
            print(f"approximate: {approximate}")
            print(f"activateLeft: {activate_left}")
            raise

...
__global__ __aicore__ void SwishKernel(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if constexpr (attrWork == static_cast<uint64_t>(TPL_SCALE_NEG_ONE)) {
        ElementwiseSchWithScalar<EleBaseTilingData32B, schMode, typename SwishDag::SwishNegOne<DtypeX>::OpDag> sch(
            tilingData);
        sch.Init(x, y);
        sch.Process();
    } else if constexpr (attrWork == static_cast<uint64_t>(TPL_SCALE_ZERO)) {
        ElementwiseSchWithScalar<EleBaseTilingData32B, schMode, typename SwishDag::SwishZero<DtypeX>::OpDag> sch(
            tilingData);
        sch.Init(x, y);
        sch.Process();
    } else if constexpr (attrWork == static_cast<uint64_t>(TPL_SCALE_OTHER)) {
        ElementwiseSchWithScalar<EleBaseTilingData32B, schMode, typename SwishDag::SwishOther<DtypeX>::OpDag> sch(
            tilingData);
        sch.Init(x, y);
        sch.Process();
    }

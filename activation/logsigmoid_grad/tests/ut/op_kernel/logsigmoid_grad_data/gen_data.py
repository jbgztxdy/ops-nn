...
def gen_golden_data_simple(shape_str, dtype):
    ...
    if x_dtype.name in ["bfloat16", "float16"]:
        ...
        data_grads = data_grads.astype(np.float32)
    
    features_torch = torch.from_numpy(data_features.copy()).requires_grad_(True)
    grads_torch = torch.from_numpy(data_grads.copy())
    
    output = F.logsigmoid(features_torch)
    output.backward(grads_torch)
    golden = features_torch.grad.numpy()
    
    if x_dtype.name in ["bfloat16", "float16"]:
        golden = golden.astype(x_dtype, copy=False)
    
    data_grads.tofile("./input_grads.bin")
    data_features.tofile("./input_features.bin")

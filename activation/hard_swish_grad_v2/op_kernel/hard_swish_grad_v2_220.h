...
protected:
    __aicore__ inline void CopyInAndCast(int64_t inputOffset, int64_t dataCount)
    {
        ...
        if (std::is_same_v<T, bfloat16_t> || std::is_same_v<T, half>) {
            DataCopyPad(this->gradTensor[this->elementNumEachCore], this->gradGm[inputOffset], dataCopyParams,
                        padParams);
            DataCopyPad(this->selfTensor[this->elementNumEachCore], this->selfGm[inputOffset], dataCopyParams,
                        padParams);
        } else {
    ...
    __aicore__ inline void Compute(int64_t dataCount)
    {
        ...
        this->maskLessThan = this->pingPongFlag ? this->tmpTensor[elementByte * NUM_TWO + elementByte / NUM_TWO +
                                                                  this->ubSize / NUM_TWO] :
        ...
        // step3
        Select(this->selfTensorFp32, this->maskGreater, this->selfTensorFp32, this->zero,
               SELMODE::VSEL_TENSOR_SCALAR_MODE, dataCount);
        PipeBarrier<PIPE_V>();

        Select(this->selfTensorFp32, this->maskLessThan, this->selfTensorFp32, this->one,
               SELMODE::VSEL_TENSOR_SCALAR_MODE, dataCount);
        PipeBarrier<PIPE_V>();

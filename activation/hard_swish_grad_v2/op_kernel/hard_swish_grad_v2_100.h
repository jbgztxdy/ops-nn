...
protected:
    ...
    {
        this->gradTensor = this->pingPongFlag ? this->tmpTensor[this->ubSize / NUM_TWO].template ReinterpretCast<T>() :
                                                this->tmpTensor[0].template ReinterpretCast<T>();
        this->selfTensor = this->pingPongFlag ?
                               this->tmpTensor[this->elementNumEachCore * sizeof(float) + this->ubSize / NUM_TWO]
    ...
    {
        int64_t tmpDataCount = (dataCount + ONE_REPEAT_ELE_NUM_FP32 - 1) / ONE_REPEAT_ELE_NUM_FP32 *
                               ONE_REPEAT_ELE_NUM_FP32;

        int64_t elementByte = this->elementNumEachCore * sizeof(float);
        this->maskGreater = this->pingPongFlag ? this->tmpTensor[elementByte * NUM_TWO + this->ubSize / NUM_TWO] :
                                                 this->tmpTensor[elementByte * NUM_TWO];
        ...
        negativeTensorFp32 = this->pingPongFlag ?
                                 this->tmpTensor[elementByte * NUM_FOUR + this->ubSize / NUM_TWO]
        ...
        for (int i = 0; i * ONE_REPEAT_ELE_NUM_FP32 < dataCount; i++) {
            Compare(this->maskGreater[i * ONE_REPEAT_ELE_NUM_FP32], this->selfTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32],
                    negativeTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32], CMPMODE::GT, ONE_REPEAT_ELE_NUM_FP32);
        }
        PipeBarrier<PIPE_V>();
        ...
        for (int i = 0; i * ONE_REPEAT_ELE_NUM_FP32 < dataCount; i++) {
            Compare(this->maskLessThan[i * ONE_REPEAT_ELE_NUM_FP32], this->selfTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32],
                    positiveTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32], CMPMODE::LT, ONE_REPEAT_ELE_NUM_FP32);
        }
        PipeBarrier<PIPE_V>();
        ...
        for (int i = 0; i * ONE_REPEAT_ELE_NUM_FP32 < dataCount; i++) {
            Select(this->selfTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32], this->maskGreater[i * ONE_REPEAT_ELE_NUM_FP32],
                   this->selfTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32], zeroTensorFp32, SELMODE::VSEL_CMPMASK_SPR,
                   ONE_REPEAT_ELE_NUM_FP32);
        }
        PipeBarrier<PIPE_V>();
        ...
        for (int i = 0; i * ONE_REPEAT_ELE_NUM_FP32 < dataCount; i++) {
            Select(this->selfTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32], this->maskLessThan[i * ONE_REPEAT_ELE_NUM_FP32],
                   this->selfTensorFp32[i * ONE_REPEAT_ELE_NUM_FP32], oneTensorFp32, SELMODE::VSEL_CMPMASK_SPR,
                   ONE_REPEAT_ELE_NUM_FP32);
        }
        PipeBarrier<PIPE_V>();
...
public:
    __aicore__ inline void Process()
    {
        ...
        for (int64_t i = 0; i < loopNum; i++) {
            ...
            if (remain > 0 && i == loopNum - 1 && this->blockIdx == lastCoreNum &&
                calNum * sizeof(T) % UB_ALIGN_BYTE != 0) {

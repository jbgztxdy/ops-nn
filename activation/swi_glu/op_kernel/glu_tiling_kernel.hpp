...
struct SwiGluCopyParam {
    ...
    uint16_t blockLen = 0; // 指定该指令每个连续传输数据块长度。取值范围：blockLen∈[1, 65535]。32B对齐时，单位为data
                           // block(32Bytes); 非32B对齐时，单位为Byte
    uint16_t stride =
        0; // 源操作数/目的操作数，相邻连续数据块的间隔（前面一个数据块的尾与后面数据块的头的间隔）。32B对齐时，单位为data
           // block(32Bytes); 非32B对齐时，单位为Byte
};
...
struct SwigluSingleTilingKernel {
    ...
    uint64_t combColLen = 0;    // 输入的2个input vector内存间隔（交织场景 in1
                             // in2算是一行），第1行的首地址和第2行首地址之间的跨度间隔 , Unit:element
    uint64_t colLen = 0; // 输入的2个input vector内存间隔（交织场景 in1
                         // in2算是一个行），in1首地址和in2首地址之间的跨度间隔 , Unit:element
    ...
    __aicore__ void GetTilingAndOffset(GM_ADDR tiling_gm, uint32_t inputDTypeLen)
    {
        ...
        // 申请UB的TQUE和TBUF时需要使用
        tileLength = (is32BAligned == 1) ?
                         (baseRowLen * baseColLen) :
                         baseRowLen *
                             AlignUp<uint32_t>(
                                 baseColLen,
                                 ISMAX<uint32_t>(isZero, static_cast<uint32_t>(
                                                             DEFAULT_MIN_BLOCK_SIZE /
                                                             (inputDTypeLen == 0 ? minInputDTypeLen : inputDTypeLen))));
                                                ...
                                                SwiGluSinlgeTileCopyParam& SwiGluCopyParam)
    {
        ...
        SwiGluCopyParam.splitVecCopyParam.stride = calRowLen == 1 ?
                                                       0 :
                                                       ((combColLen - calColLen) * inputDTypeLen / blockUnit);
        ...
        SwiGluCopyParam.indepVecCopyParam.stride = calRowLen == 1 ? 0 :
                                                                    ((colLen - calColLen) * inputDTypeLen / blockUnit);
    }
    ...
    __aicore__ inline void CaclTileCopyParams(uint32_t inputDTypeLen)
    {
        ...
        uint32_t minInputDTypeLen = 2;
        uint32_t isZero = 1;
        baseRowBaseColCalLen = (is32BAligned == 1) ?
                                   (baseRowLen * baseColLen) :
                                   (baseRowLen *
                                    AlignUp<uint32_t>(
                                        baseColLen,
                                        ISMAX<uint32_t>(isZero,
        ...
        // base row , tail col
        baseRowTailColCalLen = (is32BAligned == 1) ?
                                   (baseRowLen * tailColLen) :
                                   baseRowLen * AlignUp<uint32_t>(
                                                    tailColLen,
                                                    ISMAX<uint32_t>(isZero, static_cast<uint32_t>(
                                                                                DEFAULT_MIN_BLOCK_SIZE /
                                                                                (inputDTypeLen == 0 ? minInputDTypeLen :
                                                                                                      inputDTypeLen))));
        CaclOneTileCopyParam(baseRowLen, tailColLen, inputDTypeLen, baseRowTailColCopyParam);

        // tail row , base col
        tailRowBaseColCalLen = (is32BAligned == 1) ?
                                   (tailRowLen * baseColLen) :
                                   tailRowLen * AlignUp<uint32_t>(
                                                    baseColLen,
                                                    ISMAX<uint32_t>(isZero, static_cast<uint32_t>(
                                                                                DEFAULT_MIN_BLOCK_SIZE /
                                                                                (inputDTypeLen == 0 ? minInputDTypeLen :
        ...
        // tail row , tail col
        tailRowTailColCalLen = (is32BAligned == 1) ?
                                   (tailRowLen * tailColLen) :
                                   tailRowLen * AlignUp<uint32_t>(
                                                    tailColLen,
                                                    ISMAX<uint32_t>(isZero, static_cast<uint32_t>(
                                                                                DEFAULT_MIN_BLOCK_SIZE /
...
#define SWIGLU_SINGLE_PROCESS(kernelTiling)                                                                           \
    do {                                                                                                              \
        ...
        for (uint64_t tileIdx = AscendC::GetBlockIdx(); tileIdx < (kernelTiling).totalTileNum; tileIdx += blockNum) { \
            ...
            SWIGLU_SINGLE_PROCESS_TILE((kernelTiling).offsetParam, *((kernelTiling).curTileCopyParam),                \
                                       (kernelTiling).curCalLen);                                                     \
        }                                                                                                             \
    } while (0)
...
#define SWIGLU_SINGLE_PROCESS_NON32BALIGNED(kernelTiling)                                                             \
    do {                                                                                                              \
        ...
        for (uint64_t tileIdx = AscendC::GetBlockIdx(); tileIdx < (kernelTiling).totalTileNum; tileIdx += blockNum) { \
            (kernelTiling).CaclOneTileParam(tileIdx);                                                                 \
            SWIGLU_SINGLE_PROCESS_TILE_NON32BALIGNED((kernelTiling).offsetParam, *((kernelTiling).curTileCopyParam),  \
                                                     (kernelTiling).curCalLen);                                       \
        }                                                                                                             \
    } while (0)

...
/**
 * @file test_geir_fused_bias_leaky_relu_grad.cpp
 * @brief GE IR (graph mode) calling example for FusedBiasLeakyReluGrad
 *
 * Demonstrates how to call FusedBiasLeakyReluGrad via ge::Operator API
 * using the REG_OP registration from fused_bias_leaky_relu_grad_proto.h.
 *
 * Formula: x_grad = scale * y_grad * where(features > 0, 1.0, negative_slope)
 *
...
#include "../op_graph/fused_bias_leaky_relu_grad_proto.h"
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    ...
    // Create FusedBiasLeakyReluGrad operator
    auto add1 = op::FusedBiasLeakyReluGrad("fused_bias_leaky_relu_grad_0");

    // Set attributes
    add1.set_attr_negative_slope(0.2f);
    add1.set_attr_scale(1.414213562373f);

    // y_grad: (2, 4) -- upstream gradient
    std::vector<int64_t> tensorShape = {2, 4};
    ADD_INPUT(1, y_grad, inDtype, tensorShape);

    // features: (2, 4) -- forward features
    ADD_INPUT(2, features, inDtype, tensorShape);

    // x_grad output: (2, 4)
    ADD_OUTPUT(1, x_grad, inDtype, tensorShape);

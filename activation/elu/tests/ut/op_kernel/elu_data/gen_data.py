...
def gen_golden_data_simple(shape_str, alpha, scale, dtype):
    dtype_dict = {
        "float32": np.float32,
        "float16": np.float16,
    }
    ...
    golden = F.elu(tensor_x, alpha=alpha) * scale
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python gen_data.py <shape> <alpha> <scale> <dtype>")
        print("Example: python gen_data.py '(128, 64)' 1.0 1.0 float32")
        exit(1)
    gen_golden_data_simple(sys.argv[1], float(sys.argv[2]), float(sys.argv[3]), sys.argv[4])

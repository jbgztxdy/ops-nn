...
// 正常场景 self float ,out double dim负数
TEST_F(l2_glu_test, ascend910_9589_test_glu_normal_self_float_out_double) {
  auto tensor_self = TensorDesc({2, 4, 6}, ACL_FLOAT, ACL_FORMAT_ND);
  int64_t dim = -2;
  auto out_tensor_desc = TensorDesc({2, 2, 6}, ACL_DOUBLE, ACL_FORMAT_ND).Precision(0.0001, 0.0001);
  auto ut = OP_API_UT(aclnnGlu, INPUT(tensor_self, dim), OUTPUT(out_tensor_desc));
...
// 正常场景 float dim 为其他uint类型
TEST_F(l2_glu_test, test_glu_normal_self_float_dim_uint64) {
  auto tensor_self = TensorDesc({2, 4, 6}, ACL_FLOAT, ACL_FORMAT_ND);
  uint64_t dim = 1;
  auto out_tensor_desc = TensorDesc({2, 2, 6}, ACL_FLOAT, ACL_FORMAT_ND);
  auto ut = OP_API_UT(aclnnGlu, INPUT(tensor_self, dim), OUTPUT(out_tensor_desc));
...
TEST_F(l2_glu_test, ascend910B2_test_glu_self_is_bf16) {
  auto tensor_self = TensorDesc({2, 4, 6}, ACL_BF16, ACL_FORMAT_ND);
  int64_t dim = -2;
  auto out_tensor_desc = TensorDesc({2, 2, 6}, ACL_BF16, ACL_FORMAT_ND).Precision(0.01, 0.01);
  auto ut = OP_API_UT(aclnnGlu, INPUT(tensor_self, dim), OUTPUT(out_tensor_desc));
  uint64_t workspace_size = 5;
  aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size);
  EXPECT_EQ(aclRet, ACLNN_SUCCESS);

 ...
 *
 * All types (float32/float16/bfloat16) use the same formula:
 *   y = x * (sgn(x) * [-0.1444 * (clip(|0.7071*x|, max=1.769) - 1.769)^2 + 0.5] + 0.5)
 *   where sgn(x) = (x + 1e-12) / |x + 1e-12|
 * float16 and bfloat16 are promoted to float32 for computation.
 *
 * Template parameters:
...
__aicore__ inline void FastGeluV2<T, BUFFER_MODE>::Compute(int64_t currentNum)
{
    ...
    // float32 / float16 / bfloat16 path: original formula
    // For FP16/BF16: xCompute is already cast to FP32 via NEED_CAST
    ...
    // Step 2c: clamp sgn to [-1, 1] to handle inf/inf -> NaN
    AscendC::Mins(tmp1, tmp1, (CT)1.0, currentNum);
    AscendC::Maxs(tmp1, tmp1, (CT)(-1.0), currentNum);

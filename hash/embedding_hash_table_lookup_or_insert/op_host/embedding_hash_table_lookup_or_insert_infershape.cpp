...
graphStatus InferDtypeForLookupOrInsert(gert::InferDataTypeContext* context)
{
    ...
    OP_CHECK_IF(valuesDtype != DT_FLOAT,
                OP_LOGE_FOR_INVALID_DTYPE(context->GetNodeName(), "values",
                                          ge::TypeUtils::DataTypeToSerialString(valuesDtype).c_str(), "float"),
                return ge::GRAPH_FAILED);
    OP_LOGD(context->GetNodeName(), "End to do InferDtypeForLookupOrInsert");
    return GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(EmbeddingHashTableLookupOrInsert)
    .InferShape(InferShapeForLookupOrInsert)

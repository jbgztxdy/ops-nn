...
protected:
    void SetUp() override { std::cout << "TilingEmbeddingHashTableExport SetUp" << std::endl; }

    void TearDown() override { std::cout << "TilingEmbeddingHashTableExport TearDown" << std::endl; }
};
...
TEST_P(TilingEmbeddingHashTableExport, embedding_hash_table_export_tiling)
{
    ...
    auto holder = gert::TilingContextFaker()
                      ...
                      .InputShapes(
                          {&test_params.in_shape, &test_params.in_shape, &test_params.in_shape, &test_params.in_shape})
                      .OutputShapes({&test_params.out_shape, &test_params.out_shape, &test_params.out_shape,
                                     &test_params.out_shape})
                      ...
                      .NodeAttrs({{"export_mode", Ops::NN::AnyValue::CreateFrom<string>(test_params.export_mode)},
                                  {"filtered_export_flag",
                                   Ops::NN::AnyValue::CreateFrom<bool>(test_params.filtered_export_flag)}})
...
const auto EmbeddingHashTableExportTestCases = ::testing::Values(EmbeddingHashTableExportData{
    "all",
    true,                                     // attr
    {{1}, {1}},                               // input shape
...
INSTANTIATE_TEST_SUITE_P(EmbeddingHashTableExportTilingCases, TilingEmbeddingHashTableExport,
                         EmbeddingHashTableExportTestCases);
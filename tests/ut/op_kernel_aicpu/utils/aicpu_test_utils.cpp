...
uint64_t CalTotalElements(std::vector<std::vector<int64_t>>& shapes, uint32_t index)
{
    if (index < 0U) {
        return 0;
    }
...
bool CompareResult(Eigen::half output[], Eigen::half expect_output[], uint64_t num)
{
    ...
    for (uint64_t i = 0; i < num; ++i) {
        ...
        absolute_error = absolute_error >= Eigen::half(0) ? absolute_error : -absolute_error;
        ...
        if (expect_output[i] == Eigen::half(0)) {
            ...
            relative_error = relative_error >= Eigen::half(0) ? relative_error : -relative_error;
        }
        if ((absolute_error > Eigen::half(1e-3)) && (relative_error > Eigen::half(1e-3))) {
            std::cout << "output[" << i << "] = ";
            std::cout << output[i];
            std::cout << ", expect_output[" << i << "] = ";
            std::cout << expect_output[i] << std::endl;

...
bool StringToBool(const std::string& str, bool& result)
{
    ...
    try {
        ...
        if ((buff == "false") || (buff == "true")) {
            ...
            std::cout << "convert to bool failed, " << buff << " is not a bool value." << std::endl;
            return false;
        }
    } catch (std::exception& e) {
        std::cout << "convert " << str << " to bool failed, " << e.what() << std::endl;
    }
    return false;
}
...
std::string GetComplexReal(std::string tmp)
{
    ...
    std::string tmp_left = "(";
    std::string tmp_real;
    std::size_t tmp_connector_rfind = (tmp.rfind(tmp_connector) == std::string::npos) ? tmp.rfind(tmp_connector_minus) :
    ...
    tmp_real = (tmp_left_find == std::string::npos) ? tmp.substr(0, tmp_connector_rfind - 1U) :
                                                      tmp.substr(tmp_left_find + 1U, tmp_connector_rfind - 1U);
    return tmp_real;
}
...
template <>
bool ReadFile(std::string file_name, std::complex<double> output[], uint64_t size)
{
    try {
        std::ifstream input_file{file_name};
        if (!input_file.is_open()) {

...
static uint8_t* GetConstTensor(ge::Operator& op, const size_t index, int64_t shape_size)
{
    ...
    if (size > 0) {
        ...
        value_size = (const_dtype == ge::DT_INT64 || const_dtype == ge::DT_UINT64) ? size / sizeof(int64_t) :
                                                                                     size / sizeof(int32_t);
    }
...
ge::graphStatus InferShapeTest(ge::Operator& op, const Runtime2TestParam& param)
{
    ...
    if (irnum.size() > 0) {
        if (input_const.size() == 0)
            input_const.assign(irnum.size(), false);
    } else if (input_const.size() > 0) {
        if (irnum.size() == 0)
            irnum.assign(input_const.size(), 1);
    } else {
                ...
                if (type_it != kAttrTypesMap.end()) {
                    switch (type_it->second) {
                        case Ops::NN::AnyValue::ValueType::VT_BOOL: {
                            bool value;
                            if (op.GetAttr(item, value) != GRAPH_SUCCESS)
                                return GRAPH_FAILED;
                        ...
                        default:
                            std::cout << "[ERROR]" << __FILE__ << ":" << __LINE__ << "The ValueType is not supported!"
                                      << std::endl;
                    }
                }
            }

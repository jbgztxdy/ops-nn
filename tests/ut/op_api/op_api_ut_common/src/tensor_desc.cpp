...
static void AclTensorReleaseWrapper(aclTensor* p) { Release(p); }

static void AclTensorListReleaseWrapper(aclTensorList* p) { Release(p); }
...
aclTensor* DescToAclContainer(const TensorDesc& tensor_desc) { return tensor_desc.ToAclTypeRawPtr(); }

aclTensorList* DescToAclContainer(const TensorListDesc& tensor_list_desc) { return tensor_list_desc.ToAclTypeRawPtr(); }
...
void DescToJson(json& root, const TensorDesc& tensor_desc, bool is_input) { tensor_desc.ToJson(root, is_input); }
...
int SaveResultToBinaryFile(aclTensor* p, size_t index, size_t total_input, const string& file_prefix,
                           const set<short int>& inplace_output)
{
    if (index < total_input) {
        return 0;
    }
...
int SaveResultToBinaryFile(aclTensorList* p, size_t index, size_t total_input, const string& file_prefix,
                           const set<short int>& inplace_output)
{
    if (index < total_input) {
        return 0;

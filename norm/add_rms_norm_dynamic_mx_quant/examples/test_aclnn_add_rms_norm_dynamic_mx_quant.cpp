...
int main()
{
    ...
    if (ret != ACL_SUCCESS) {
        LOG_PRINT("CreateAclTensor x1 failed. ERROR: %d\n", ret);
    ...
    if (ret != ACL_SUCCESS) {
        LOG_PRINT("CreateAclTensor x2 failed. ERROR: %d\n", ret);
        aclDestroyTensor(x1);
        aclrtFree(x1_device_addr);
        aclrtDestroyStream(stream);
        aclrtResetDevice(deviceId);
        aclFinalize();
        return ret;
    ...
    if (ret != ACL_SUCCESS) {
        LOG_PRINT("CreateAclTensor x_out failed. ERROR: %d\n", ret);
        aclDestroyTensor(x1);
        aclDestroyTensor(x2);
        aclDestroyTensor(gamma);
    ...
    if (ret != ACL_SUCCESS) {
        ...
        aclDestroyTensor(gamma);
        aclDestroyTensor(y_out);
        aclDestroyTensor(x_out);
        aclrtFree(x1_device_addr);
        aclrtFree(x2_device_addr);
        aclrtFree(gamma_device_addr);
        aclrtFree(y_out_device_addr);
        aclrtFree(x_out_device_addr);
        aclrtDestroyStream(stream);
        aclrtResetDevice(deviceId);
        aclFinalize();
        return ret;
    ...
    ret = aclnnAddRmsNormDynamicMxQuant(workspace_addr, workspace_size, executor, stream);
    if (ret != ACL_SUCCESS) {
    ...
    // 5. 获取输出的值，将device侧内存上的结果拷贝至host侧，需要根据具体API的接口定义修改
    {
        ...
        ret = aclrtMemcpy(
            x_out_result.data(), x_out_result.size() * sizeof(x_out_result[0]), 
            x_out_device_addr, size * sizeof(uint16_t), ACL_MEMCPY_DEVICE_TO_HOST);
        if (ret != ACL_SUCCESS) {

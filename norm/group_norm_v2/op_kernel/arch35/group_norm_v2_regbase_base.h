...
__aicore__ inline void VFWelfordParallelFinalizeAlign(__local_mem__ float* meanLocal, __local_mem__ float* rstdLocal,
  ...
  __VEC_SCOPE__ {
    ...
    for (uint16_t i = 0; i < dichotomyAddReminderLoopCount; i++) {
      ...
      DataCopy<float, AscendC::MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(dichotomyAddLocal + i, mean, pregMerge);
    }
    ...
    for (uint16_t i = 0; i < dichotomyAddReminderLoopCount; i++) {
      ...
      DataCopy<float, AscendC::MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(dichotomyAddLocal + i, var, pregMerge);
    }
...
__aicore__ inline void VFWelfordParallelFinalizeNonAlignSituation1(
  ...
  __VEC_SCOPE__ {
    ...
    for (uint16_t i = 0; i < static_cast<uint16_t>(dichotomyAddPowerLoopCount -
                                                   dichotomyAddReminderRealLoopCount); i++) {
...
__aicore__ inline void VFWelfordParallelFinalizeNonAlignSituation3(
  ...
  __VEC_SCOPE__ {
    ...
    for (uint16_t i = 0; i < dichotomyAddPowerRemainLoopCount; i++) {
      ...
      ReduceSum(mean, dichotomyAddMeanL, pregMain);
      DataCopy<float, AscendC::MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(
          dichotomyAddLocal + dichotomyAddReminderLoopCount + welfordDiffLoopCount + welfordReminderLoopCount + i, mean,
          pregMerge);
    }
    ...
    for (uint16_t i = 0; i < dichotomyAddPowerRemainLoopCount; i++) {
      ...
      ReduceSum(var, dichotomyAddVarL, pregMain);
      DataCopy<float, AscendC::MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(
          dichotomyAddLocal + dichotomyAddReminderLoopCount + welfordDiffLoopCount + welfordReminderLoopCount + i, var,
          pregMerge);
    }
...
__aicore__ inline void VFWelfordParallelFinalize(__local_mem__ float* meanLocal, __local_mem__ float* rstdLocal,
  ...
  if (welfordAlign) {
    VFWelfordParallelFinalizeAlign(meanLocal, rstdLocal, tmpMeanLocal, tmpVarLocal, dichotomyAddLocal, reduceCount,
                                   dichotomyAddPower, dichotomyAddK, dichotomyAddLastNum, offset, reduceScale, scale,
                                   cnt, eps);
  } else {
    cnt = cnt - 1;
    VFWelfordParallelFinalizeNonAlign(meanLocal, rstdLocal, tmpMeanLocal, tmpVarLocal, dichotomyAddLocal, reduceCount,
                                      dichotomyAddPower, dichotomyAddK, dichotomyAddLastNum, offset, tailSize,
                                      reduceScale, cnt, eps);
  }
}
  ...
  __VEC_SCOPE__ {
    ...
    for (uint16_t i = 0; i < numPerCoreProcess; i++) {
      ...
      for (uint16_t j = 0; j < dichotomyAddReminderLoopCount; j++) {
        ...
        ReduceSum(mean, sumMean, pregMain);
        DataCopy<float, AscendC::MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(dichotomyAddLocal + j, mean, pregMerge);
      }

      // 整块剩余部分vcadd回刷UB
      for (uint16_t j = 0; j < static_cast<uint16_t>(dichotomyAddPowerLoopCount -
                                                     dichotomyAddReminderLoopCount); j++) {
      ...
      for (uint16_t j = 0; j < dichotomyAddReminderLoopCount; j++) {
        ...
        ReduceSum(var, sumVar, pregMain);
        DataCopy<float, AscendC::MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(dichotomyAddLocal + j, var, pregMerge);
      }
...
__aicore__ inline void CalMeanAndRstd(__local_mem__ T* xLocal, __local_mem__ float* meanLocal,
                                      ...
                                      uint32_t dichotomyAddLastNum, uint64_t reduceCount, float scale, float eps) {
  if (dichotomyAddPower >= VL_FP32) {
    CalMeanAndRstdByDichotomyAdd(xLocal, meanLocal, rstdLocal, dichotomyAddLocal, numPerCoreProcess, dichotomyAddPower,
                                 dichotomyAddK, dichotomyAddLastNum, reduceCount, scale, eps);
    return;
  }
  ...
  if constexpr (IsSameType<T1, float>::value) {
    ...
    __VEC_SCOPE__ {
      ...
      for (uint16_t i = 0; i < loopCount; i++) {
        ...
        DataCopy<T1, AscendC::MicroAPI::StoreDist::DIST_PACK_B32>(meanOutLocal + i * VL_FP32, meanOut, pregLoop);
        DataCopy<T1, AscendC::MicroAPI::StoreDist::DIST_PACK_B32>(rstdOutLocal + i * VL_FP32, rstdOut, pregLoop);
      }
    }

...
static ge::graphStatus CheckInputXShape(const gert::TilingContext* context, const gert::Shape& xShape)
{
    ...
    for (uint64_t i = 0; i < xDims; i++) {
        ...
        OP_CHECK_IF(
            (curDim <= 0),
            OP_LOGE(
                context->GetNodeName(), "The input %lu dimension must be greater than 0, currently is %ld.", i, curDim),
...
static ge::graphStatus CheckInputParams(const gert::TilingContext* context)
{
    ...
    uint64_t xDtypeSize = ge::GetSizeByDataType(xDtype);
    OP_CHECK_IF(
        (xDtypeSize <= 0), OP_LOGE(context->GetNodeName(), "xDtypeSize is invalid %lu, please check.", xDtypeSize),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(
            context->GetNodeName(),
            "Gamma dimension should be one, and the shape of gamma must be the same as  channel size of input, "
            "currently "
            "is %lu.",
            gammaSizes),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(
            context->GetNodeName(),
            "Beta dimension should be one, and the shape of beta must be the same as channel size of input, currently "
            "is "
            "%lu.",
            betaSizes),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(context->GetNodeName(), "The dtype combination of gamma, beta and inputs is invalid."),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus CheckAttrParams(const gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        (*numGroups <= 0), OP_LOGE(context->GetNodeName(), "numGroups must be bigger than 0."),
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        (channel % *numGroups != 0), OP_LOGE(context->GetNodeName(), "channel must be integer multiples of numGroups."),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

static uint64_t GetOptionalInputTensorSize(
    const gert::TilingContext* context, uint64_t index, uint64_t specifiedValue = 0)
{
...
*/
static void SetTilingKey4Ascend910D(
...
static void SetWelfordParallelN(
    const gert::TilingContext* context, uint64_t xDtypeSize, uint64_t ubRemain, GroupNormV2TilingData& tilingData)
{
    auto compileInfo = reinterpret_cast<const GroupNormV2CompileInfo*>(context->GetCompileInfo());
    uint32_t blockSize = BLOCK_SIZE;
    OP_CHECK_IF((xDtypeSize == 0), OP_LOGE(context->GetNodeName(), "XDtypeSize is zero."), return);
...
static void SetUbTiling4TwoPass(
    const gert::TilingContext* context, GroupNormV2TilingData& tilingData, uint64_t maxReduceCount, uint32_t xDtypeSize)
{
    auto compileInfo = reinterpret_cast<const GroupNormV2CompileInfo*>(context->GetCompileInfo());
    uint32_t blockSize = BLOCK_SIZE;
...
static WelfordTilingInitResult InitWelfordTilingCommon(
    const gert::TilingContext* context, GroupNormV2TilingData& tilingData, uint32_t blockSize, uint32_t xDtypeSize)
{
...
static void SetUbTiling4WelfordPerf(
    const gert::TilingContext* context, GroupNormV2TilingData& tilingData, uint64_t maxReduceCount, uint32_t ubRemain,
    uint32_t xDtypeSize)
{
    auto compileInfo = reinterpret_cast<const GroupNormV2CompileInfo*>(context->GetCompileInfo());
    uint32_t blockSize = BLOCK_SIZE;
    ...
    OP_CHECK_IF(
        (result.checkResult == false), OP_LOGE(context->GetNodeName(), "InitWelfordTilingCommon Failed."), return);
...
static void SetUbTiling4WelfordGeneralized(
    const gert::TilingContext* context, GroupNormV2TilingData& tilingData, uint32_t ubRemain, uint32_t xDtypeSize)
{
    auto compileInfo = reinterpret_cast<const GroupNormV2CompileInfo*>(context->GetCompileInfo());
    uint32_t blockSize = BLOCK_SIZE;
    WelfordTilingInitResult result = InitWelfordTilingCommon(context, tilingData, blockSize, xDtypeSize);
    ...
    uint64_t curUbSize =
        gammaRealSize + betaRealSize + count * result.hwNumAlign * xDtypeSize * BUFFER_NUM * DOUBLE_BUFFER;
...
static void SetUbTiling4Welford(
    const gert::TilingContext* context, GroupNormV2TilingData& tilingData, uint64_t maxReduceCount, uint64_t ubRemain,
    uint32_t xDtypeSize)
{
...
static void SetUbTiling4Ascend910D(
    const gert::TilingContext* context, uint64_t maxReduceCount, uint64_t ubRemain, bool isReduceFullLoad,
    GroupNormV2TilingData& tilingData)
{
    auto compileInfo = reinterpret_cast<const GroupNormV2CompileInfo*>(context->GetCompileInfo());
...
ge::graphStatus SetTilingData(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        (CheckInputParams(context) != ge::GRAPH_SUCCESS), OP_LOGE(context->GetNodeName(), "InputParams is invalid."),
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        (CheckAttrParams(context) != ge::GRAPH_SUCCESS), OP_LOGE(context->GetNodeName(), "AttrParams is invalid."),
        return ge::GRAPH_FAILED);

    GroupNormV2TilingData tilingData;
    OP_CHECK_IF(
        (SetAttrParams(context, tilingData) != ge::GRAPH_SUCCESS),
        OP_LOGE(context->GetNodeName(), "Set attrParams failed."), return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        (SetTilingParams(context, tilingData) != ge::GRAPH_SUCCESS),
    ...
    OP_CHECK_IF(
        GroupNormV2SetTilingData(context, tilingData) != ge::GRAPH_SUCCESS,
        OP_LOGE(context->GetNodeType(), "GroupNormV2SetTilingData set tiling data fail."), return ge::GRAPH_FAILED);
...
static ge::graphStatus Tiling4GroupNormV2(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        input_first == nullptr, OP_LOGE(context->GetNodeName(), "get input_first failed."), return ge::GRAPH_FAILED);
    const gert::Shape& input_shape = input_first->GetStorageShape();

    const int32_t input_dim_size = input_shape.GetDimNum();
    OP_CHECK_IF(
        input_dim_size < MIN_LEN, OP_LOGE(context->GetNodeName(), "input_dim can't be smaller than 2"),
        return ge::GRAPH_FAILED);

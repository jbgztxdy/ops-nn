...
/*!
 * \file test_geir_euclidean_norm.cpp
 * \brief EuclideanNorm 算子 GE IR 图模式调用样例。
 *
 * 样例配置：
 *   x        : DT_FLOAT,  shape = [32, 1024]，每元素填充 2.0f
 *   axes     : DT_INT64,  shape = [1]，值 = {1}（以 Const 算子注入；axes 是值依赖输入）
 *   keep_dims: false
 *   y        : DT_FLOAT,  shape = [32]
 *
 * 期望结果：y[i] = sqrt( sum_{j=0..1023} 2.0f^2 ) = sqrt(1024 * 4) = 64.0f
 */
...
#include "../../op_graph/euclidean_norm_proto.h"
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    ...
    euclideanNorm_1.update_output_desc_##outputName(outputName##outputIndex##_desc_);
    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto euclideanNorm_1 = op::EuclideanNorm("euclideanNorm_1");

    // shape 定义：x = [32, 1024], axes = [1] (1-D, value=[1]), 输出 y = [32]（keep_dims=false 时 axis=1 被消除）
    std::vector<int64_t> xShape = {32, 1024};
    std::vector<int64_t> axesShape = {1};
    std::vector<int64_t> yShape = {32};

    // 输入 x：以 op::Data placeholder 注入，dtype = inDtype (DT_FLOAT)，每元素填 2.0f
    ADD_INPUT(1, x, inDtype, xShape);

    // 输入 axes：以 op::Const 注入，dtype = DT_INT64，值 = {1}
    //   说明：axes 在 IR 中标注 ValueDepend(OPTIONAL)，infershape 与 tiling 都依赖其
    //   具体数值（用于推导输出 shape、做合轴决策），因此必须以 Const 注入，
    //   而不是普通 Data placeholder（否则编译期拿不到 axes 值会失败）。
    auto axes_const = op::Const("axes_const");
    TensorDesc axes_desc = TensorDesc(ge::Shape(axesShape), FORMAT_ND, DT_INT64);
    axes_desc.SetPlacement(ge::kPlacementHost);
    axes_desc.SetFormat(FORMAT_ND);
    int64_t axes_data[] = {1};
    Tensor axes_tensor(axes_desc, reinterpret_cast<uint8_t*>(axes_data), sizeof(axes_data));
    axes_const.SetAttr("value", axes_tensor);
    axes_const.update_output_desc_y(axes_desc);
    graph.AddOp(axes_const);
    euclideanNorm_1.set_input_axes(axes_const);
    euclideanNorm_1.update_input_desc_axes(axes_desc);
    inputs.push_back(axes_const);

    // 输出 y：dtype 与 x 一致，shape = [32]
    ADD_OUTPUT(1, y, inDtype, yShape);

    // 属性 keep_dims：默认 false，保留 reduce 维度则置为 true
    ADD_INPUT_ATTR(keep_dims, false);

    outputs.push_back(euclideanNorm_1);
    // 添加完毕
    return SUCCESS;
}

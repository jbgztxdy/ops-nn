...
static ge::graphStatus GetPlatformInfo(gert::TilingContext* context, EuclideanNormCtx& ctx)
{
    ...
    OP_CHECK_IF(ctx.blockSize == 0 || ctx.cacheLineSize == 0,
                OP_LOGE(context, "blockSize=%ld or cacheLineSize=%ld is 0",
                        ctx.blockSize, ctx.cacheLineSize),
...
static ge::graphStatus GetShapeAndDtype(gert::TilingContext* context, EuclideanNormCtx& ctx)
{
    ...
    OP_CHECK_IF(rank == 0, OP_LOGE(context, "x rank is 0; scalar input not supported"),
                return ge::GRAPH_FAILED);
    ctx.xShape.assign(rank, 0);
    for (size_t i = 0; i < rank; ++i) {
        ctx.xShape[i] = xS.GetDim(i);
        // 允许 == 0（空 tensor 模板）；负数仍非法（dynamic shape 已在更外层处理）
        OP_CHECK_IF(ctx.xShape[i] < 0,
                    OP_LOGE(context, "x dim[%zu]=%ld is negative (dynamic shape not allowed at tiling)",
                            i, ctx.xShape[i]),
                    return ge::GRAPH_FAILED);
    }
    ...
    OP_CHECK_IF(kSupported.count(ctx.xDtype) == 0,
                OP_LOGE(context, "unsupported x dtype %d (expect fp16/bf16/fp32/int32)",
...
static ge::graphStatus ParseAxesTensor(gert::TilingContext* context, EuclideanNormCtx& ctx)
{
    ...
    auto pushOne = [&](int64_t v, int64_t idx) -> ge::graphStatus {
        OP_CHECK_IF(v < -xRank || v >= xRank,
                    OP_LOGE(context, "axes[%ld]=%ld out of range [-%ld, %ld)",
                            idx, v, xRank, xRank),
                    return ge::GRAPH_FAILED);
        const int64_t norm = (v < 0) ? (v + xRank) : v;
        OP_CHECK_IF(!seen.insert(norm).second,
                    OP_LOGE(context, "duplicate axis %ld in axes (input idx=%ld)", norm, idx),
                    return ge::GRAPH_FAILED);
    ...
    if (dt == ge::DT_INT32) {
        ...
        for (int64_t i = 0; i < axesNum; ++i) {
            if (pushOne(static_cast<int64_t>(data[i]), i) != ge::GRAPH_SUCCESS) return ge::GRAPH_FAILED;
        }
    } else if (dt == ge::DT_INT64) {
        const int64_t* data = axesTensor->GetData<int64_t>();
        OP_CHECK_NULL_WITH_CONTEXT(context, data);
        for (int64_t i = 0; i < axesNum; ++i) {
            if (pushOne(data[i], i) != ge::GRAPH_SUCCESS) return ge::GRAPH_FAILED;
        }
    } else {
...
{
    bool hasR = false;
    for (bool b : ctx.isReduceAxis) { if (b) { hasR = true; break; } }
    if (!hasR) {
...
static void ClassifyEmptyTensor(EuclideanNormCtx& ctx)
{
    ...
    for (size_t i = 0; i < ctx.xShape.size(); ++i) {
        ...
        if (!isR) {
            if (sz == 0) { hasZeroA = true; }
            else         { aTotal *= sz; }
        } else {
            if (sz == 0) { hasZeroR = true; }
        }
    }
    if (hasZeroA) {
...
static ge::graphStatus PreprocessPattern(gert::TilingContext* context, EuclideanNormCtx& ctx)
{
    ...
    OP_CHECK_IF(ctx.axisNum < kMinAxisNum || ctx.axisNum > kMaxAxisNum,
                OP_LOGE(context, "axisNum=%d out of [%d, %d] after pattern preprocessing",
                        ctx.axisNum, kMinAxisNum, kMaxAxisNum),
                return ge::GRAPH_FAILED);
    ...
    for (int32_t i = 0; i < ctx.axisNum; ++i) {
        ...
        OP_CHECK_IF(ctx.isReduceAxis[static_cast<size_t>(i)] != wantR,
                    OP_LOGE(context, "axis[%d] type mismatch after preprocessing (isR=%d, want=%d)",
                            i, static_cast<int>(ctx.isReduceAxis[i]), static_cast<int>(wantR)),
                    return ge::GRAPH_FAILED);
    }
...
{
    for (int32_t i = ctx.axisNum - 1; i >= 0; --i) {
        if (!ctx.isReduceAxis[static_cast<size_t>(i)]) return i;
    }
    return -1;
}
...
static int32_t PrevSameTypeAxis(const EuclideanNormCtx& ctx, int32_t from, bool wantR)
{
    for (int32_t i = from - 1; i >= 0; --i) {
        if (ctx.isReduceAxis[static_cast<size_t>(i)] == wantR) return i;
    }
    return -1;
}
...
static void ComputeAUbFactor(EuclideanNormCtx& ctx)
{
    ...
    while (true) {
        ...
        if (aSize * ctx.innerAProdAlign >= atmp) break;
...
static int64_t ComputeRiMax(const EuclideanNormCtx& ctx)
{
    ...
    const int64_t bytesPerRElem = (kDbFactor * kNPreReuse + kNPreParallel) * ctx.dtypeSize
                                + kTmpBufCopies * kFp32Bytes;
    const int64_t outBytes      = kNOut * ctx.dtypeSize * aUnit;
    if (aUnit <= 0 || bytesPerRElem <= 0) return -1;
...
static ge::graphStatus ComputeRUbFactor(gert::TilingContext* context, EuclideanNormCtx& ctx)
{
    ...
    OP_CHECK_IF(rIMax < 1,
                OP_LOGE(context, "r_i_max < 1 (UB cannot hold one R element): aUFA=%ld iAPA=%ld",
                        ctx.aUbFactorAlign, ctx.innerAProdAlign),
                return ge::GRAPH_FAILED);
    ...
    while (true) {
        ...
        if (rSize * ctx.innerRProdAlign > rIMax) break;
    ...
    if (ctx.isTailR && ctx.rSplitAxisIdx == lastR && ctx.rUbFactor < rAxisSize) {
        ...
        OP_CHECK_IF(ctx.rUbFactor == 0,
                    OP_LOGE(context, "rUbFactor floor-aligned to 0 (block=%ld elem rIMax=%ld)",
                            bsAElem, rIMax),
                    return ge::GRAPH_FAILED);
    }
...
static bool RIsFullyLoaded(const EuclideanNormCtx& ctx)
{
    ...
    if (ctx.rUbFactor != ctx.axisShape[static_cast<size_t>(rSplit)]) return false;
    // 更外侧不再有 R 轴
    for (int32_t i = rSplit - 1; i >= 0; --i) {
        if (ctx.isReduceAxis[static_cast<size_t>(i)]) return false;
    }
    return true;
}
...
{
    if (!RIsFullyLoaded(ctx)) return ge::GRAPH_SUCCESS;
    ...
    if (atmpNew <= curATotal) return ge::GRAPH_SUCCESS;
    ...
    while (true) {
        const int64_t aSize = ctx.axisShape[static_cast<size_t>(ctx.aSplitAxisIdx)];
        if (aSize * ctx.innerAProdAlign >= atmpNew) break;
        const int32_t prev = PrevSameTypeAxis(ctx, ctx.aSplitAxisIdx, /*wantR=*/false);
        if (prev < 0) break;
        if (ctx.aSplitAxisIdx == lastA && !ctx.isTailR) {
            ctx.innerAProdAlign *= Ops::Base::CeilAlign(aSize, bsAElem);
    ...
    if (!ctx.isTailR && ctx.aSplitAxisIdx == lastAIdx) {
        ...
        OP_CHECK_IF(aFloor == 0,
                    OP_LOGE(context, "ExpandA(tail A, LastA): FloorAlign to 0 (aTotalUb=%ld bs=%ld)",
                            aTotalUb, bsAElem),
                    return ge::GRAPH_FAILED);
...
static void ComputeFusedALoopSplit(EuclideanNormCtx& ctx)
{
    ...
    ctx.usedCoreNum       = (ctx.aSmallCoreLoopCnt > 0)
                                ? static_cast<int32_t>(ctx.coreNum)
                                : ctx.aBigCoreCnt;
...
static void ComputeRLoopCnt(EuclideanNormCtx& ctx)
{
    ...
    const int64_t rChunks = Ops::Base::CeilDiv(
        ctx.axisShape[static_cast<size_t>(ctx.rSplitAxisIdx)], ctx.rUbFactor);
...
static void ComputeUbSizes(EuclideanNormCtx& ctx)
{
    ...
    const int64_t unit  = ctx.aUbFactorAlign * ctx.innerAProdAlign
                        * ctx.rUbFactorAlign * ctx.innerRProdAlign;
...
static ge::graphStatus FillAndLogTilingData(gert::TilingContext* context, const EuclideanNormCtx& ctx)
{
    ...
    OP_CHECK_IF(memset_s(td, sizeof(EuclideanNormTilingData), 0, sizeof(EuclideanNormTilingData)) != EOK,
                OP_LOGE(context, "memset tilingdata error"),
                return ge::GRAPH_FAILED);
    ...
    OP_LOGI(context,
            "EuclideanNorm tiling: dtype=%d axisNum=%d isTailR=%d usedCoreNum=%d",
            static_cast<int>(ctx.xDtype), ctx.axisNum, static_cast<int>(ctx.isTailR), ctx.usedCoreNum);
    OP_LOGI(context,
            "  axisShape=[%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld]",
            td->axisShape[0], td->axisShape[1], td->axisShape[2], td->axisShape[3], td->axisShape[4],
            td->axisShape[5], td->axisShape[6], td->axisShape[7], td->axisShape[8]);
    OP_LOGI(context,
            "  aLoopCntTotal=%ld aSplitChunkCnt=%ld | bigCore=%ld*%d smallCore=%ld*%d",
            td->aLoopCntTotal, td->aSplitChunkCnt,
            td->aBigCoreLoopCnt, td->aBigCoreCnt,
            td->aSmallCoreLoopCnt, td->usedCoreNum - td->aBigCoreCnt);
    OP_LOGI(context,
            "  aSplit=%d aUbFactor=%ld aUbFactorAlign=%ld",
            td->aSplitAxisIdx, td->aUbFactor, td->aUbFactorAlign);
    OP_LOGI(context,
            "  rSplit=%d rUbFactor=%ld rUbFactorAlign=%ld | innerAProd=%ld iAPAlign=%ld innerRProd=%ld iRPAlign=%ld",
            td->rSplitAxisIdx, td->rUbFactor, td->rUbFactorAlign,
            td->innerAProd, td->innerAProdAlign,
            td->innerRProd, td->innerRProdAlign);
    OP_LOGI(context,
            "  rLoopCntTotal=%ld | UB: preRed=%ld postRed=%ld tmp=%ld cache=%ld",
            td->rLoopCntTotal, td->preReduceUbSize, td->postReduceUbSize,
            td->tmpBufUbSize, td->cacheBufUbSize);
    return ge::GRAPH_SUCCESS;
}
...
static void ComputeEmptyRTiling(EuclideanNormCtx& ctx)
{
    ...
    const int32_t usedCoreNum = (aSmallCoreLoopCnt > 0)
                                    ? static_cast<int32_t>(ctx.coreNum)
                                    : std::max(aBigCoreCnt, 1);
...
static ge::graphStatus FillEmptyTilingData(gert::TilingContext* context, const EuclideanNormCtx& ctx)
{
    ...
    OP_LOGI(context,
            ...
            static_cast<int>(ctx.xDtype), ctx.aTotalEmpty, td->aUbFactor, td->usedCoreNum,
            td->aBigCoreLoopCnt, td->aBigCoreCnt,
            td->aSmallCoreLoopCnt, td->usedCoreNum - td->aBigCoreCnt,
            td->postReduceUbSize);
    return ge::GRAPH_SUCCESS;
...
static ge::graphStatus EuclideanNormTilingFunc(gert::TilingContext* context)
{
    ...
    // NORMAL 路径（与改动前一致；只是 BuildInitialAxisList 已在上面调过，
    //   PreprocessPattern 不再重复 BuildInitialAxisList，因此把其原首行去掉前 inline 一遍）
    ...
    // tilingkey：isEmptyTensor=0, isTailR ∈ {0,1}
    const uint64_t tilingKey = GET_TPL_TILING_KEY(static_cast<uint64_t>(0U),
                                                 static_cast<uint64_t>(ctx.isTailR ? 1U : 0U));
    context->SetTilingKey(tilingKey);
    context->SetBlockDim(static_cast<uint32_t>(ctx.usedCoreNum));

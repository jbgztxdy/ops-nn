...
static bool LoadAxesFromTensor(const gert::Tensor* axesTensor, int64_t xRank,
                                std::vector<int64_t>& axesOut, std::string& errMsg)
{
    ...
    auto pushOne = [&](int64_t v, int64_t idx) -> bool {
        if (v < -xRank || v >= xRank) {
            errMsg = "axes[" + std::to_string(idx) + "]=" + std::to_string(v) +
                     " out of range [-" + std::to_string(xRank) + ", " + std::to_string(xRank) + ")";
            return false;
        }
    ...
    if (dt == ge::DT_INT32) {
        ...
        if (data == nullptr) { errMsg = "axes data ptr is null (int32)"; return false; }
        axesOut.reserve(static_cast<size_t>(axesNum));
        for (int64_t i = 0; i < axesNum; ++i) {
            if (!pushOne(static_cast<int64_t>(data[i]), i)) return false;
        }
    } else if (dt == ge::DT_INT64) {
        const int64_t* data = axesTensor->GetData<int64_t>();
        if (data == nullptr) { errMsg = "axes data ptr is null (int64)"; return false; }
        axesOut.reserve(static_cast<size_t>(axesNum));
        for (int64_t i = 0; i < axesNum; ++i) {
            if (!pushOne(data[i], i)) return false;
        }
    } else {
...
static ge::graphStatus InferShape4EuclideanNorm(gert::InferShapeContext* context)
{
    ...
    if (axesTensor == nullptr) {
        ...
        OP_LOGW(context->GetNodeName(),
                "axes tensor not const at infer time; set output as unknown rank");
        return GRAPH_SUCCESS;
    }
    ...
    if (!LoadAxesFromTensor(axesTensor, xRank, axes, errMsg)) {
        OP_LOGE(context->GetNodeName(), "%s; xRank=%ld, axes size=%ld",
                errMsg.c_str(), xRank, axesTensor->GetShapeSize());
        return GRAPH_FAILED;
    }
    ...
    OP_LOGD(context->GetNodeName(),
            "End InferShape: keepDims=%d, xRank=%ld, axes.size=%zu, outDim=%zu, status=%d",
            static_cast<int>(keepDims), xRank, axes.size(),
            yShape->GetDimNum(), static_cast<int>(stat));
    return stat;
}

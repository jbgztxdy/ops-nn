...
protected:
    static void SetUpTestCase() { std::cout << "MaskedSoftmaxWithRelPosBiasTiling SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "MaskedSoftmaxWithRelPosBiasTiling TearDown" << std::endl; }
};
...
void testTiling(gert::StorageShape& x_shape, gert::StorageShape& atten_mask_shape, gert::StorageShape& bias_shape,
                gert::StorageShape& y_shape, float scaleValue, bool hasAtten, uint32_t key)
{
    ...
    for (uint32_t i = 0; i < sizeof(testDataType) / sizeof(ge::DataType); i++) {
        ...
        ASSERT_EQ(Ops::NN::Optiling::TilingRegistry::GetInstance().DoTilingImpl(tiling_context), ge::GRAPH_SUCCESS);
        auto tiling_key = tiling_context->GetTilingKey();
        ASSERT_EQ(tiling_key, key + 10 * i);
    }
}

void testTilingFailed(gert::StorageShape& x_shape, gert::StorageShape& atten_mask_shape, gert::StorageShape& bias_shape,
                      gert::StorageShape& y_shape, float scaleValue, bool hasAtten, ge::DataType dataType)
{
    map<string, string> socInfos;
    map<string, string> aicoreSpec;
    map<string, string> intrinsics;
...
void testTilingShapeCheckFailed(gert::StorageShape& x_shape, gert::StorageShape& atten_mask_shape,
...
void test310PTiling(gert::StorageShape& x_shape, gert::StorageShape& atten_mask_shape, gert::StorageShape& bias_shape,
                    gert::StorageShape& y_shape, float scaleValue, bool hasAtten, uint32_t key)
{

...
protected:
    __aicore__ inline void CopyIn(int64_t offset, uint32_t stackNum, uint32_t copyEleNum)
    {
        ...
        if constexpr (isAligend) {
            ...
            DataCopyParams copyParamsLast{static_cast<uint16_t>(stackNum), static_cast<uint16_t>(s2DtypeSize),
            ...
            if (tmpNS1 >= stackNum) {
                ...
                DataCopyParams bais2copyParamsLast{static_cast<uint16_t>(stackNum - tmpNS1),
                                                   static_cast<uint16_t>(s2DtypeSize), (uint16_t)(0), (uint16_t)(0)};
                                   ...
                                   uint32_t elementNum)
    {
        ...
        if constexpr (!(AscendC::IsSameType<T, float>::value)) {
            ...
            if constexpr (existMuls) {
                ...
                PipeBarrier<PIPE_V>();
                AscendC::SoftMax<float, true, false>(yFloatLocal, xFloatLocal, softmaxSharedTmpBuffer,
                                                     softmaxTilingData, softmaxShapeInfo);
                PipeBarrier<PIPE_V>();
                ...
                PipeBarrier<PIPE_V>();
                AscendC::SoftMax<float, true, false>(xFloatLocal, yFloatLocal, softmaxSharedTmpBuffer,
                                                     softmaxTilingData, softmaxShapeInfo);
                PipeBarrier<PIPE_V>();
                if constexpr (AscendC::IsSameType<T, half>::value) {
            ...
            if constexpr (existMuls) {
                ...
                PipeBarrier<PIPE_V>();
                AscendC::SoftMax<T, true, false>(yLocal, xLocal, softmaxSharedTmpBuffer, softmaxTilingData,
                                                 softmaxShapeInfo);
                PipeBarrier<PIPE_V>();
            } else {

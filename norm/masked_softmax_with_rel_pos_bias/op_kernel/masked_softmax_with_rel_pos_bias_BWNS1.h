...
private:
    ...
    __aicore__ inline void CopyIn(int64_t loopOffset, uint32_t num, uint32_t mulNumAndS2)
    {
        ...
        if (num <= tmpS1) {
            ...
            DataCopy(maskLocal[mulLoopStepSizeAndS2_ + tmpS1 * s2_], biasGm[biasGmOffset * s2_], tailNum * s2_);
        }
    ...
    __aicore__ inline void CopyInPad(int64_t loopOffset, uint32_t num)
    {
        ...
        if (num <= tmpS1) {
            ...
            int64_t biasGmOffset = localOffset % (mulNAndS1);
            DataCopyPad(maskLocal[mulLoopStepSizeAndS2AlignedSize], biasGm[biasGmOffset * s2_], copyParamsMask,
                        padParamsNormal);
        } else {
            uint32_t tailNum = num - tmpS1;
            DataCopyParams copyParamsMask{(uint16_t)(tmpS1), (uint16_t)(s2DtypeSize), (uint16_t)(0), (uint16_t)(0)};
            DataCopyParams tailCopyParamsMask{(uint16_t)(tailNum), (uint16_t)(s2DtypeSize), (uint16_t)(0),
                                              (uint16_t)(0)};
            ...
            attenMaskGmOffset = (localOffset + tmpS1) / (mulNAndS1) % w_ * s1_ + (localOffset + tmpS1) % s1_;
            DataCopyPad(maskLocal[tmpS1 * s2AlignedSize], attenMaskGm[attenMaskGmOffset * s2_], tailCopyParamsMask,
                        padParamsNormal);

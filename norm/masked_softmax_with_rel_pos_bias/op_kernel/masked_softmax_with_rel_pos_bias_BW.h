...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        if (isAligend) {
            ...
            DataCopyParams biasCopyParamsLast{ns1, s2DtypeSize, (uint16_t)(0), (uint16_t)(0)};
...
private:
    __aicore__ inline void CopyIn(int32_t b)
    {
        ...
        if (isAligend) {
            ...
            DataCopyParams copyParamsLast{(uint16_t)(ns1), s2DtypeSize, (uint16_t)(0), (uint16_t)(0)};
            DataCopyPadParams padParamsNormal{false, 0, 0, 0};
        ...
        if constexpr (existAtten) {
            ...
            if (isAligend) {
                for (uint32_t ni = 0; ni < n_; ni++) {
                    DataCopy(attenMaskLocal[ni * s1s2AlignedSize], attenMaskGm[((atten_mask_offset + b) % w_) * s1s2],
                             s1s2AlignedSize);
                }
            } else {
#if (__CCE_AICORE__ > 200)
                DataCopyParams attenMaskCopyParamsLast{(uint16_t)(s1_), s2DtypeSize, (uint16_t)(0), (uint16_t)(0)};
                DataCopyPadParams padParamsNormal{false, 0, 0, 0};
                for (uint32_t ni = 0; ni < n_; ni++) {
                    DataCopyPad(attenMaskLocal[ni * s1s2AlignedSize],
                                attenMaskGm[((atten_mask_offset + b) % w_) * s1s2], attenMaskCopyParamsLast,
                                padParamsNormal);
                }
#endif
            }
    ...
    __aicore__ inline void Compute(int32_t b, LocalTensor<T>& biasMaskLocal)
    {
        ...
        if constexpr (!(AscendC::IsSameType<T, float>::value)) {
            ...
            SoftMaxShapeInfo softmaxShapeInfo{ns1, s2AlignedSize, ns1, s2_};
            AscendC::SoftMax<T, false, false>(yLocal, yLocal, softMaxTiling, softmaxShapeInfo);
        }

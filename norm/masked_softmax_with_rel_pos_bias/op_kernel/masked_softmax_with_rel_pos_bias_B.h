    ...
    __aicore__ inline void Process()
    {
        ...
        if (tailNum != 0) {
            ...
            if constexpr (existAtten) {
                ...
                ComputeBias(offset, attenMaskLocal, tailNum, mulsNum, softmaxShapeInfo,
                            tilingData->tailSoftmaxTilingData);
            }
...
protected:
    ...
    __aicore__ inline void CopyInAttenMaskAndBias()
    {
        if constexpr (isAligend) {
            ...
            DataCopyParams attenMaskCopyParamsLast{(uint16_t)(tilingData->ws1), (uint16_t)(s2DtypeSize), (uint16_t)(0),
                                                   (uint16_t)(0)};
            ...
            DataCopyParams biasCopyParamsLast{(uint16_t)(tilingData->ns1), (uint16_t)(s2DtypeSize), (uint16_t)(0),
                                              (uint16_t)(0)};
...
#if (__CCE_AICORE__ > 200)
            LocalTensor<T> xLocal = vecInQueue.AllocTensor<T>();
            DataCopyParams copyParamsLast{(uint16_t)(stackNum * tilingData->wns1), (uint16_t)(s2DtypeSize),
...
#if (__CCE_AICORE__ > 200)
            LocalTensor<T> yLocal = vecOutQueue.DeQue<T>();
            DataCopyParams copyParamsLast{(uint16_t)(stackNum * wns1), (uint16_t)(s2DtypeSize), (uint16_t)(0),
                                          (uint16_t)(0)};
...
public:
    __aicore__ inline MaskedSoftmaxWithRelPosBiasBBf16AndHalf(
        const MaskedSoftmaxWithRelPosBiasTilingData* __restrict__ maskedSoftmaxWithRelPosBiasBTilingData)
        : MaskedSoftmaxWithRelPosBiasB<T, existAtten, existMuls, isAligend>(maskedSoftmaxWithRelPosBiasBTilingData){};
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR attenMask, GM_ADDR bias, GM_ADDR y)
    {
...
private:
    __aicore__ inline void CopyInAttenMaskAndBias()
    {
        if constexpr (isAligend) {
            ...
            LocalTensor<T> maskLocal = this->maskQueue.template AllocTensor<T>();
            DataCopyParams attenMaskCopyParamsLast(this->tilingData->ws1, this->s2DtypeSize, (uint16_t)(0),
                                                   (uint16_t)(0));
            DataCopyPadParams padParamsNormal{false, 0, 0, 0};
                                                     ...
                                                     LocalTensor<float>& biasCastTensor)
    {
        ...
        for (uint32_t wi = 0; wi < this->w_; wi++) {
            for (uint32_t ni = 0; ni < this->n_; ni++) {
                Cast(attenMaskCastTensor[attenOffset], attenMaskLocal[tempOffset], RoundMode::CAST_NONE,
                     this->s1s2Aligned);
    ...
    __aicore__ inline void ComputeAttenMaskAndBias(int64_t offset, LocalTensor<float>& xCastTensor,
                                                   LocalTensor<float>& attenMaskLocal,
                                                   LocalTensor<float>& biasCastTensor, uint32_t stackNum,
                                                   uint32_t mulsNum, const SoftMaxShapeInfo& softmaxShapeInfo,
                                                   const SoftMaxTiling& softmaxTilingData)
    {

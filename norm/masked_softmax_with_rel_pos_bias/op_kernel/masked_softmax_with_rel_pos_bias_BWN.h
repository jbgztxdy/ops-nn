...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR attenMask, GM_ADDR bias, GM_ADDR y)
    {
        ...
        if constexpr (existAtten) {
            ...
            pipe.InitBuffer(maskQueue, BUFFER_NUM,
                            stackSize * 2 * s1s2AlignedSize_ * sizeof(T)); // 2 is for mask and stten
...
private:
    __aicore__ inline void ComputeFp32(int32_t b, uint32_t stackSize1, uint32_t& tmpn,
    ...
    __aicore__ inline void CastAttenMaskForBf16AndFp16(LocalTensor<T>& attenMaskLocal,
                                                       LocalTensor<float>& attenMaskFloatLocal, uint32_t& tmpn)
    {
        if constexpr (existAtten) {
            if (stackSize <= tmpn) {
                uint32_t i = 0;
                ...
                for (uint32_t j = i2; j < stackSize; j++) {
                    Cast(attenMaskFloatLocal[attenOffset2], attenMaskLocal[tmpn * s1s2AlignedSize_],
                         RoundMode::CAST_NONE, s1s2AlignedSize_);
                    attenOffset2 += s1s2AlignedSize_;
                }
            }
        }
    }
    __aicore__ inline void ComputeBf16AndFp16(int32_t b, uint32_t stackSize1, uint32_t& tmpn,
                                              ...
                                              const SoftMaxTiling& softmaxTilingData)
    {
        ...
        LocalTensor<float> biasMaskFloatLocal =
            attenMaskFloatLocal; // 如果existAtten为false，此变量才会使用，否者会用新的biasMaskFloatLocal的值。
        if constexpr (existAtten) {
    ...
    __aicore__ inline void Compute(int32_t i, uint32_t stackSize, uint32_t& tmpn,
                                   const SoftMaxShapeInfo& softmaxShapeInfo, const SoftMaxTiling& softmaxTilingData)
    {
        if constexpr (!(AscendC::IsSameType<T, float>::value)) {
    ...
    __aicore__ inline void CopyInNonAligned(int32_t process, uint32_t stackSize, uint32_t& tmpn)
    {
        ...
        DataCopyParams copyParamsLast{(uint16_t)(s1_ * stackSize), (uint16_t)(s2_ * sizeof(T)), (uint16_t)(0),
                                      (uint16_t)(0)};
        DataCopyPadParams padParamsNormal{false, 0, 0, 0};
        DataCopyPad(xLocal, xGm[(static_cast<int64_t>(process) * this->onceBatchSize) * s1s2_], copyParamsLast,
                    padParamsNormal);
        ...
        if (stackSize <= tmpn) {
            if constexpr (existAtten) {
                uint32_t i = 0;
                DataCopyParams attenMaskCopyParamsLast{(uint16_t)(s1_), (uint16_t)(s2_ * sizeof(T)), (uint16_t)(0),
                                                       (uint16_t)(0)};
                DataCopyPad(maskLocal[i * s1s2AlignedSize_],
                            attenMaskGm[(static_cast<int64_t>(process) * this->onceBatchSize + i + atten_mask_offset) /
                                        n_ % w_ * s1s2_],
                            attenMaskCopyParamsLast, padParamsNormal);
            }
            DataCopyParams biasCopyParamsLast{(uint16_t)(s1_ * stackSize), (uint16_t)(s2_ * sizeof(T)), (uint16_t)(0),
                                              (uint16_t)(0)};
            DataCopyPad(biasLocal, biasGm[static_cast<int64_t>(localOffset % n_) * s1s2_], biasCopyParamsLast,
                        padParamsNormal);
        } else {
            if constexpr (existAtten) {
                uint32_t i = 0;
                ...
                DataCopyPad(maskLocal[i2 * s1s2AlignedSize_],
                            attenMaskGm[(static_cast<int64_t>(process) * this->onceBatchSize + i2 + atten_mask_offset) /
                                        n_ % w_ * s1s2_],
                            attenMaskCopyParamsLast, padParamsNormal);
            }
            uint32_t tailNum = stackSize - tmpn;
            DataCopyParams biasCopyParamsLast{(uint16_t)(s1_ * tmpn), (uint16_t)(s2_ * sizeof(T)), (uint16_t)(0),
                                              (uint16_t)(0)};
            DataCopyPad(biasLocal, biasGm[static_cast<int64_t>(localOffset % n_) * s1s2_], biasCopyParamsLast,
                        padParamsNormal);
            DataCopyParams biasCopyParamsLastTail{(uint16_t)(s1_ * tailNum), (uint16_t)(s2_ * sizeof(T)), (uint16_t)(0),
                                                  (uint16_t)(0)};
    ...
    __aicore__ inline void CopyOutNonAligned(int32_t process, uint32_t stackSize)
    {
        ...
        DataCopyParams copyParamsLast{(uint16_t)(stackSize * s1_), (uint16_t)(s2_ * sizeof(T)), (uint16_t)(0),
                                      (uint16_t)(0)};
    ...
    __aicore__ inline void CopyInAligned(int32_t process, uint32_t stackSize, uint32_t& tmpn)
    {
        LocalTensor<T> xLocal = vecInQueue.AllocTensor<T>();
        LocalTensor<T> maskLocal = maskQueue.AllocTensor<T>();
        DataCopy(xLocal, xGm[(static_cast<int64_t>(process) * this->onceBatchSize) * s1s2_],
                 stackSize * s1s2AlignedSize_);
        auto biasLocal = maskLocal[totalSizeAligned];
        {
            uint32_t localOffset = (process * this->onceBatchSize + relative_pos_bias_offset);
            tmpn = n_ - localOffset % n_;
            if (stackSize <= tmpn) {
                if constexpr (existAtten) {
                    uint32_t i = 0;
                    DataCopy(maskLocal[i * s1s2AlignedSize_],
                             attenMaskGm[(static_cast<int64_t>(process) * this->onceBatchSize + i + atten_mask_offset) /
                                         n_ % w_ * s1s2_],
                             s1s2AlignedSize_);
                }
                DataCopy(biasLocal, biasGm[static_cast<int64_t>(localOffset % n_) * s1s2_],
                         stackSize * s1s2AlignedSize_);
            } else {
                if constexpr (existAtten) {
                    uint32_t i = 0;
    ...
    {
        LocalTensor<T> yLocal = vecOutQueue.DeQue<T>();
        DataCopy(yGm[static_cast<int64_t>(process) * this->onceBatchSize * s1s2_], yLocal,
                 stackSize * s1s2AlignedSize_);
        vecOutQueue.FreeTensor(yLocal);
    }

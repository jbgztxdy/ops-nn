...
/*!
 * \file rms_norm_dynamic_quant_helper.h
 * \brief
 */

#ifndef RMS_NORM_DYNAMIC_QUANT_HELPER_H_
#define RMS_NORM_DYNAMIC_QUANT_HELPER_H_
...
template <typename T, template <typename U> typename R, template <typename U> typename S>
__aicore__ inline void DataCopyExStride(const R<T>& dst, const S<T>& src, const uint32_t len, const uint32_t count = 1,
                                        const uint32_t ubAligned = 0)
{
    DataCopyExtParams copyParams;
    copyParams.blockCount = count;
    copyParams.blockLen = CalculateBlockLen<T>(len);
...
#endif // RMS_NORM_DYNAMIC_QUANT_HELPER_H_

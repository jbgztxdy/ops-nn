...
REG_OP(GroupNorm)
    ...
    * @par Inputs:
    * Three inputs
    * @li x: A ND tensor of type bfloat16, float16, float32. The input feature map will be processed by group
    normalization.
    ...
    * @li eps: An optional float32, specifying the small value added to variance to avoid dividing by zero. Defaults to
    "0.00001".
    * @li data_format: An optional string, specifying the format of "x". Defaults to "NHWC". This parameter is reserved
    ...
    * @par Outputs:
    * Three outputs
    * @li y: A ND tensor of type bfloat16, float16, float32 for the normalized "x". Has the same type, format and shape
    as "x".
    ...
    */
     ...
     * "dy" supports 2-8 dimensions (N, C, *), the calculation logic only cares about the first two dimensions (N and
     C),
     * and the rest can all be combined into one dimension.
     * @li mean: A tensor. Mean of each group. Datatype support float32, float16, bfloat16. Format support ND.
     * Must be 2D (N, num_groups).
     * @li rstd: A tensor. Reciprocal standard deviation of each group. Datatype support float32, float16, bfloat16.
     Format support ND.
     * Must be 2D (N, num_groups).
     * @li x: A Tensor. Specifies the offset. Datatype support float32, float16, bfloat16. Format support ND.
     * "x" supports 2-8 dimensions (N, C, *), the calculation logic only cares about the first two dimensions (N and C),
     * and the rest can all be combined into one dimension.
     * @li gamma: A tensor. Specifies the scaling factor. Datatype support float32, float16, bfloat16. Format support
     ND.
     ...
     * @li dx: A tensor. x factor grad. Datatype is the same as the input datatype. Has the same format and shape as
     "x".
    ...
    * @brief Performs the backward operation of group normalization and swish.
     ...
     * @li rstd: A Tensor. Reciprocal standard deviation of each group. Datatype support float32, float16, bfloat16.
    Format support ND.
     * @li x: A Tensor. Specifies the offset. Datatype support float32, float16, bfloat16. Format support ND. Same shape
    as mean.
     * @li gamma: A Tensor. Specifies the scaling factor. Datatype support float32, float16, bfloat16. Format support
    ND. Same shape as dy.

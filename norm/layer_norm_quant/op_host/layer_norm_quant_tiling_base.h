...
{
    OP_CHECK_IF(tilingDataPtr == nullptr, OP_LOGE(context->GetNodeName(), "tilingDataPtr should not be empty"),
    ...
    OP_CHECK_IF(x_shape->GetStorageShape().GetDim(dimsSize - 1) <= 0 ||
                ...
                OP_LOGE(context->GetNodeName(), "numCol is invalid!"), return ge::GRAPH_FAILED);
                                                                 ...
                                                                 gert::TilingContext* context)
{
    ...
    if (totalMemNeed > sumData) {
        OP_CHECK_IF(sumData <= 0, OP_LOGE(context->GetNodeName(), "sumData is invalid!"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(tilingDataPtr->get_firstDimPerTimes() >= UINT_MAX - tilingDataPtr->get_nlFirstdimPerCore(),
                OP_LOGE(context->GetNodeName(), "row_work + row_step is invalid!"), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
} // namespace optiling

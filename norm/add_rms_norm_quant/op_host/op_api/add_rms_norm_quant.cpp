...
{
    bool isDual = Ops::NN::AclnnUtil::IsRegbase() ? ((nullptr != scales2Optional) || (nullptr != zeroPoints2Optional)) :
...
{
    L0_DFX(AddRmsNormQuant, x1, x2, gamma, scales1, scales2Optional, zeroPoints1Optional, zeroPoints2Optional,
    ...
    aclOpExecutor* executor)
{
    ...
    L0_DFX(AddRmsNormQuantV2, x1, x2, gamma, bias, scales1, scales2Optional, zeroPoints1Optional, zeroPoints2Optional,
           axis, epsilon, divMode);
    OP_LOGD("AddRmsNormQuantV2 L0_DFX.");

    auto y1Out = executor->AllocTensor(x1->GetViewShape(), op::DataType(dstType), op::Format::FORMAT_ND);

    bool outputRes = (resOut != nullptr);
    ...
    ADD_TO_LAUNCHER_LIST_AICORE(
        AddRmsNormQuantV2,
        OP_INPUT(x1, x2, gamma, scales1, scales2Optional, zeroPoints1Optional, zeroPoints2Optional, bias),
        OP_OUTPUT(y1Out, y2Out, xOut, resOut), OP_ATTR(axis, static_cast<float>(epsilon), divMode, dstType, outputRes));

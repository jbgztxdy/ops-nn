...
TEST_F(add_rms_norm_quant_test, test_case_0)
{
    ...
    size_t scalesByteSize = 256 * sizeof(float);
    size_t zeroPointsByteSize = 256 * sizeof(int32_t);
    size_t outputYByteSize = 2 * 256 * sizeof(int8_t);
    size_t outputXByteSize = 2 * 256 * sizeof(int16_t);
    ...
    tilingDatafromBin->numRow = 2;
    tilingDatafromBin->numCol = 256;
    tilingDatafromBin->blockFactor = 1;
    tilingDatafromBin->rowFactor = 64;
    tilingDatafromBin->ubFactor = 12288;

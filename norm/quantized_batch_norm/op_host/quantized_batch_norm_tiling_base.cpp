...
static inline bool IsOthersDtypeSupported(const ge::DataType dtype) { return (dtype == ge::DT_FLOAT); }

bool QuantizedBatchNormTilingBase::CheckInputDtype()
{
    OP_CHECK_IF(context_->GetInputDesc(X_INPUT_IDX) == nullptr,
                OP_LOGE(commonParams.nodeName, "Get input X desc failed"), return false);
    ...
    OP_CHECK_IF(context_->GetInputDesc(INZP_INPUT_IDX) == nullptr,
    ...
    OP_CHECK_IF(context_->GetInputDesc(OUTSCALE_INPUT_IDX) == nullptr,
    ...
    OP_CHECK_IF(context_->GetInputDesc(OUTZP_INPUT_IDX) == nullptr,
    ...
    OP_CHECK_IF(!IsDtypeSupported(xDtype),
                OP_LOGE(commonParams.nodeName, "x dtype must in [DT_INT8, DT_UINT8, DT_INT32]"), return false);

    OP_CHECK_IF((!IsOthersDtypeSupported(weightDtype)),
                OP_LOGE(commonParams.nodeName, "weight dtype must in [DT_FLOAT]"), return false);

    OP_CHECK_IF((weightDtype != biasDtype), OP_LOGE(commonParams.nodeName, "bias dtype must be same as weight dtype"),
                return false);

    OP_CHECK_IF((meanDtype != ge::DT_FLOAT), OP_LOGE(commonParams.nodeName, "running_mean dtype should be DT_FLOAT"),
                return false);

    OP_CHECK_IF((varDtype != ge::DT_FLOAT), OP_LOGE(commonParams.nodeName, "running_var dtype should be DT_FLOAT"),
                return false);

    OP_CHECK_IF((inScaleDtype != ge::DT_FLOAT), OP_LOGE(commonParams.nodeName, "input_scale dtype should be DT_FLOAT "),
                return false);

    OP_CHECK_IF((inZeroPointDtype != ge::DT_INT32),
                OP_LOGE(commonParams.nodeName, "input_zero_point dtype should be DT_INT32 "), return false);

    OP_CHECK_IF((outScaleDtype != ge::DT_FLOAT),
                OP_LOGE(commonParams.nodeName, "output_scale dtype should be DT_FLOAT"), return false);

    OP_CHECK_IF((outZeroPointDtype != ge::DT_INT32),
...
bool QuantizedBatchNormTilingBase::CheckInputShape()
{
    ...
    if (format == ge::FORMAT_NCHW) {
        OP_CHECK_IF(xStorageShape.GetDimNum() != NCHW_DIM_NUM, OP_LOGE(commonParams.nodeName, "Not supported x format"),
                    return false);
    ...
    OP_CHECK_IF(commonParams.patternR1 <= 0, OP_LOGE(commonParams.nodeName, "x shape dim 0 should be more than zero."),
                return false);
    OP_CHECK_IF(commonParams.patternA <= 0, OP_LOGE(commonParams.nodeName, "x shape dim 1 should be more than zero."),
                return false);
    OP_CHECK_IF(commonParams.patternR0 <= 0,
                OP_LOGE(commonParams.nodeName, "x shape dim_2 * dim_3 should be more than zero."), return false);
    OP_CHECK_IF(weightStorageShape.GetShapeSize() != commonParams.patternA,
                OP_LOGE(commonParams.nodeName, "weight ShapeSize: %ld should equal x shape C dim: %ld",
                        weightStorageShape.GetShapeSize(), commonParams.patternA),
                return false);
    OP_CHECK_IF(biasStorageShape.GetShapeSize() != commonParams.patternA,
                OP_LOGE(commonParams.nodeName, "bias ShapeSize: %ld should equal x shape C dim: %ld",
                        biasStorageShape.GetShapeSize(), commonParams.patternA),
                return false);
    OP_CHECK_IF(meanStorageShape.GetShapeSize() != commonParams.patternA,
                OP_LOGE(commonParams.nodeName, "mean ShapeSize: %ld should equal x shape C dim: %ld",
...
bool QuantizedBatchNormTilingBase::CheckOthersInputShape()
{
    ...
    OP_CHECK_IF(inScaleShape == nullptr, OP_LOGE(commonParams.nodeName, "Get input inScale shape failed"),
                return false);
    auto inScaleStorageShape = inScaleShape->GetStorageShape();

    auto inZeroPointShape = context_->GetInputShape(INZP_INPUT_IDX);
    OP_CHECK_IF(inZeroPointShape == nullptr, OP_LOGE(commonParams.nodeName, "Get input inZeroPoint shape failed"),
                return false);
    ...
    OP_CHECK_IF(outScaleShape == nullptr, OP_LOGE(commonParams.nodeName, "Get input outScale shape failed"),
                return false);
    auto outScaleStorageShape = outScaleShape->GetStorageShape();

    auto outZeroPointShape = context_->GetInputShape(OUTZP_INPUT_IDX);
    OP_CHECK_IF(outZeroPointShape == nullptr, OP_LOGE(commonParams.nodeName, "Get input outZeroPoint shape failed"),
                return false);
    auto outZeroPointStorageShape = outZeroPointShape->GetStorageShape();

    OP_CHECK_IF(varStorageShape.GetShapeSize() != commonParams.patternA,
                OP_LOGE(commonParams.nodeName, "var ShapeSize: %ld should equal x shape C dim: %ld",
    ...
    OP_CHECK_IF(
        ...
        return false);
    OP_CHECK_IF(inZeroPointStorageShape.GetShapeSize() != 1,
                OP_LOGE(commonParams.nodeName, "input_zero_point ShapeSize: %ld should equal 1",
                        inZeroPointStorageShape.GetShapeSize()),
                return false);
    OP_CHECK_IF(outScaleStorageShape.GetShapeSize() != 1,
                OP_LOGE(commonParams.nodeName, "output_scale ShapeSize: %ld should equal 1",
                        outScaleStorageShape.GetShapeSize()),
                return false);
    OP_CHECK_IF(outZeroPointStorageShape.GetShapeSize() != 1,
                OP_LOGE(commonParams.nodeName, "output_zero_point ShapeSize: %ld should equal 1",
...
ge::graphStatus QuantizedBatchNormTilingBase::GetPlatformInfo()
{
    ...
    OP_CHECK_IF(commonParams.coreNum == 0, OP_LOGE(commonParams.nodeName, "numBlocks should not be equal to zero."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(commonParams.ubSizePlatForm == 0, OP_LOGE(commonParams.nodeName, "ubSize should not be equal to zero."),
                return ge::GRAPH_FAILED);
...
ge::graphStatus QuantizedBatchNormTilingBase::GetShapeAttrsInfo()
{
    ...
    // check输入dtype
    OP_CHECK_IF(!QuantizedBatchNormTilingBase::CheckInputDtype(),
                OP_LOGE("GetShapeAttrsInfo", "CheckInputDtype failed."), return ge::GRAPH_FAILED);
    // check输入shape
    OP_CHECK_IF(!QuantizedBatchNormTilingBase::CheckInputShape(),
                OP_LOGE("GetShapeAttrsInfo", "CheckInputShape failed."), return ge::GRAPH_FAILED);
    OP_CHECK_IF(!QuantizedBatchNormTilingBase::CheckOthersInputShape(),
                OP_LOGE("GetShapeAttrsInfo", "CheckInputShape failed."), return ge::GRAPH_FAILED);

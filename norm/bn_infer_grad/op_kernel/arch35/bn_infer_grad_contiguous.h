...
private:
    ...
    __aicore__ inline void ExpandInvStdNCHW(int64_t tileStart, int64_t curTileLen,
                                             LocalTensor<float>& invStdLocal,
                                             LocalTensor<float>& invStdExpanded);
    __aicore__ inline void ExpandInvStdNHWC(int64_t tileStart, int64_t curTileLen,
                                             LocalTensor<float>& invStdLocal,
                                             LocalTensor<float>& invStdExpanded);
...
template <typename T>
__aicore__ inline void BNInferGradContiguous<T>::ExpandInvStdNCHW(int64_t tileStart, int64_t curTileLen,
                                                                   LocalTensor<float>& invStdLocal,
                                                                   LocalTensor<float>& invStdExpanded)
{
...
template <typename T>
__aicore__ inline void BNInferGradContiguous<T>::ExpandInvStdNHWC(int64_t tileStart, int64_t curTileLen,
                                                                   LocalTensor<float>& invStdLocal,
                                                                   LocalTensor<float>& invStdExpanded)
{
    constexpr int64_t ALIGN_ELEMS = 8;
...
__aicore__ inline void BNInferGradContiguous<T>::ExpandInvStd(int64_t tileStart, int64_t curTileLen)
{
    ...
    if (formatMode_ == 0) {
        ExpandInvStdNCHW(tileStart, curTileLen, invStdLocal, invStdExpanded);
    } else if (formatMode_ == 1) {
        ExpandInvStdNHWC(tileStart, curTileLen, invStdLocal, invStdExpanded);
    }
    PipeBarrier<PIPE_ALL>();
}

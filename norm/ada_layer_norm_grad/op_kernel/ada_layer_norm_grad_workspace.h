...
public:
    ...
    __aicore__ inline void CalculateBlockLengths(const AdaLayerNormGradTilingDataWorkspace* tilingData,
                                                 int64_t& formerBlockLength, int64_t& curRowsNum,
                                                 int64_t& curBlockBatchRowNum, int64_t& blockBatchOffset,
                                                 int64_t& blockStartBatch, int64_t& curBlockLength,
                                                 int64_t& curBlockBatchLength)
    {
    ...
    __aicore__ inline void InitGmTensors(GM_ADDR dy, GM_ADDR x, GM_ADDR rstd, GM_ADDR mean, GM_ADDR scale,
                                         ...
                                         const AdaLayerNormGradTilingDataWorkspace* tilingData,
    ...
    __aicore__ inline void InitWorkspace(const AdaLayerNormGradTilingDataWorkspace* tilingData, GM_ADDR workspace,
                                         int64_t wsLenPerBlock, int64_t curBlockBatchRowNum)
    {
        ...
        dScaleWorkspaceGM.SetGlobalBuffer((__gm__ float*)workspace + wsBase + 2 * colAlign,
                                          curBlockBatchRowNum * colAlign);
        dShiftWorkspaceGM.SetGlobalBuffer((__gm__ float*)workspace + wsBase + (curBlockBatchRowNum + 2) * colAlign,
                                          curBlockBatchRowNum * colAlign);
        mul1WorkspaceGM.SetGlobalBuffer((__gm__ float*)workspace + wsBase + (2 * curBlockBatchRowNum + 2) * colAlign,
                                        colAlign);
        mul3WorkspaceGM.SetGlobalBuffer((__gm__ float*)workspace + wsBase + (2 * curBlockBatchRowNum + 3) * colAlign,
                                        colAlign);
    ...
    __aicore__ inline void Init(GM_ADDR dy, GM_ADDR x, GM_ADDR rstd, GM_ADDR mean, GM_ADDR scale, GM_ADDR gamma,
                                ...
                                const AdaLayerNormGradTilingDataWorkspace* tilingData, TPipe& pipeIn)
    {
        // 初始化基础变量
        pipe = pipeIn;
        seq = tilingData->seq;
        ...
        // 计算块长度相关参数
        int64_t formerBlockLength, curRowsNum, curBlockBatchRowNum, blockBatchOffset, blockStartBatch, curBlockLength,
            curBlockBatchLength;
        CalculateBlockLengths(tilingData, formerBlockLength, curRowsNum, curBlockBatchRowNum, blockBatchOffset,
                              blockStartBatch, curBlockLength, curBlockBatchLength);
        ...
        workspaceGMOri.SetGlobalBuffer((__gm__ float*)workspace, wsLenPerBlock * (tilingData->blockNum - 1) +
                                                                     (tilingData->blockTail / seq) * colAlign);
        ...
        if (GetBlockIdx() < tilingData->blockNum) {
            ...
            InitGmTensors(dy, x, rstd, mean, scale, gamma, beta, pdX, pdScale, pdShift, tilingData, formerBlockLength,
                          curRowsNum, curBlockLength, blockStartBatch, curBlockBatchLength);
    ...
    __aicore__ inline void CopyInPhase0(int64_t rowIndex, int64_t ubIndex, int64_t ubFormer, int64_t calcNum)
    {
        ...
        if constexpr (IsSameType<T, float>::value) {
            ...
            DataCopyPad(buffer0.ReinterpretCast<T>()[ubFormer], dyInTensorGM[rowOffset], intriParamsT,
                        padParams); // buffer0 dy
            DataCopyPad(buffer1.ReinterpretCast<T>()[ubFormer], xInTensorGM[rowOffset], intriParamsT,
                        padParams); // buffer1 x
    ...
    __aicore__ inline void ComputePhase0Part3(int64_t ubIndex, int64_t ubFormer, int64_t calcNum)
    {
        ...
        buffer4 = queOut4.DeQue<float>();
        SetAtomicAdd<float>();
        DataCopyPad(dScaleWorkspaceGM[rowOfBatch * colAlign + ubIndex * ubFormer], buffer4,
                                         ...
                                         float reduce2, float reduce3, float rstdIn)
    {
        ...
        PipeBarrier<PIPE_V>();
        Adds(buffer1, buffer1, -reduce2,
             calcNum); // dy*gamma*(1+scale)-(x-mean)*rstd*(1/N)*∑(dy*gamma*(1+scale)*(x-mean)*rstd)
        PipeBarrier<PIPE_V>();
        Sub(buffer1, buffer1, buffer0,
            calcNum); // dy*gamma-(1/N)∑dy*gamma*(1+scale)-(x-mean)*rstd*(1/N)*∑(dy*gamma*(1+scale)*(x-mean)*rstd))
        PipeBarrier<PIPE_V>();

        if constexpr (IsSameType<T, float>::value) {
            Muls(
                buffer4, buffer1, rstdIn,
    ...
    {
        int64_t BatchRowsNum = ((GetBlockIdx() != tilingData->blockNum - 1) ? tilingData->blockFormer :

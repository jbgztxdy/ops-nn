...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用`aclnnAddRmsNormDynamicQuantV2GetWorkspaceSize`接口获取入参并根据计算流程所需workspace大小，再调用`aclnnAddRmsNormDynamicQuantV2接口执行计算。
...
int main() {
  ...
  ret = aclnnAddRmsNormDynamicQuantV2GetWorkspaceSize(x1, x2, gamma, smooth1, smooth2, beta, epsilon, nullptr, y1, y2, x, scale1, scale2, &workspaceSize, &executor);

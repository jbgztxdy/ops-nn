 ...
 */
#ifndef OP_API_INC_LEVEL2_ADD_LAYER_NORM_QUANT_V2_H_
#define OP_API_INC_LEVEL2_ADD_LAYER_NORM_QUANT_V2_H_
...
/**
 * @brief aclnnAddLayerNormQuantV2的第一段接口，根据具体的计算流程，计算workspace大小。
 ...
 * 计算公式：
 *     x = x1 + x2 + bias
 *     layernormRes = LayerNorm(x, gamma, beta)
 ...
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] layernormRes:
 * 公式中的输出`layernormRes`，数据类型支持BFLOAT16、FLOAT、FLOAT16且需要与x1一致，shape需要与x1一致。
 * 支持非连续的Tensor，数据格式支持ND。
 ...
 */
ACLNN_API aclnnStatus aclnnAddLayerNormQuantV2GetWorkspaceSize(
...
#endif // OP_API_INC_LEVEL2_ADD_LAYER_NORM_QUANT_V2_H_

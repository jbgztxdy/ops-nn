...
static const std::initializer_list<op::DataType> DTYPE_SUPPORT_LIST_DYN = {
...
static bool CheckNotNullV2(
    const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta, const char* quantMode,
    aclTensor* y1Out, aclTensor* y2Out, const aclTensor* xOut, const aclTensor* layernormRes, aclTensor* outScales1Out, aclTensor* outScales2Out)
{
...
static bool IsDynamicQuantV2(const char* quantMode)
{
...
static bool GetQuantModeV2(const char* quantMode, bool& isDynQuant)
{
    ...
    OP_CHECK(
        isValidquantMode, OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Got quantMode is not \"static\"."),
        return false);
    } else {
        isValidquantMode =
...
static bool CheckDtypeValidV2(
    ...
    const aclTensor* y2Out, const aclTensor* xOut, const aclTensor* layernormRes, const aclTensor* outScales1Out, const aclTensor* outScales2Out,
    bool isDynQuant)
{
...
static bool CheckOptCombValidV2(
    ...
    const aclTensor* zeroPoints2Optional, bool isDynQuant)
{
    ...
    OP_LOGI(
        "Got aclnnAddLayerNormQuantV2 opt input combination (%d,%d,%d,%d), isDyn=%d", s1Exist, s2Exist, z1Exist, z2Exist,
        isDynQuant);
    ...
    bool isDynQuant)
{
    ...
    bool eleShapeEqs = (x1->GetViewShape() == x2->GetViewShape()) && (x1->GetViewShape() == xOut->GetViewShape()) &&
                       (x1->GetViewShape() == y1Out->GetViewShape()) && (x1->GetViewShape() == layernormRes->GetViewShape());
...
static aclnnStatus CheckParamsV2(
    const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta,
    const aclTensor* biasOptional, const aclTensor* scales1Optional, const aclTensor* scales2Optional,
    const aclTensor* zeroPoints1Optional, const aclTensor* zeroPoints2Optional, aclTensor* y1Out, aclTensor* y2Out,
    aclTensor* xOut, aclTensor* layernormRes, aclTensor* outScales1Out, aclTensor* outScales2Out, const char* quantMode)
{
    // 1. 检查参数是否为空指针
    CHECK_RET(
    ...
    CHECK_RET(
        CheckDtypeValidV2(
            ...
            zeroPoints2Optional, y1Out, y2Out, xOut, layernormRes, outScales1Out, outScales2Out, isDynQuant),
...
aclnnStatus ComputeAddLayerNormQuantV2(const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta,
    const aclTensor* biasOptional, const aclTensor* scales1Optional, const aclTensor* scales2Optional, const aclTensor* zeroPoints1Optional, 
    const aclTensor* zeroPoints2Optional, const char* quantMode, double epsilon, bool additionalOutput, bool divMode, aclTensor* y1Out, 
    aclTensor* y2Out, aclTensor* xOut, aclTensor* layernormRes, aclTensor* outScales1Out, aclTensor* outScales2Out, aclOpExecutor* executor)
{
    ...
    if (Ops::NN::AclnnUtil::IsRegbase(GetCurrentPlatformInfo().GetCurNpuArch())) {
        auto addLayerNormQuantV2Outs = l0op::AddLayerNormQuant(x1, x2, gamma, beta, biasOptional, scales1Optional, scales2Optional,
            zeroPoints1Optional, zeroPoints2Optional, quantMode, epsilon, additionalOutput, divMode, executor);
        ...
        auto addLayerNormQuantV2Outs = l0op::AddLayerNormQuantV2(x1, x2, gamma, beta, biasOptional, scales1Optional, scales2Optional, 
            zeroPoints1Optional, zeroPoints2Optional, quantMode, epsilon, additionalOutput, divMode, executor);
        y1ComputeOut = std::get<IDX_0>(addLayerNormQuantV2Outs);
        y2ComputeOut = std::get<IDX_1>(addLayerNormQuantV2Outs);
        xComputeOut = std::get<IDX_2>(addLayerNormQuantV2Outs);
    ...
    CHECK_RET(y1ComputeOut != nullptr && y2ComputeOut != nullptr && xComputeOut != nullptr && layernormComputeOut != nullptr &&
        outScales1ComputeOut != nullptr && outScales2ComputeOut != nullptr, ACLNN_ERR_INNER_NULLPTR);
...
static aclnnStatus PrepareContiguousAndCompute(
    const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta, const aclTensor* biasOptional, 
    const aclTensor* scales1Optional, const aclTensor* scales2Optional, const aclTensor* zeroPoints1Optional, const aclTensor* zeroPoints2Optional,
    const char* quantMode, double epsilon, bool additionalOutput, bool divMode, aclTensor* y1Out, aclTensor* y2Out, aclTensor* xOut, 
    aclTensor* layernormRes, aclTensor* outScales1Out, aclTensor* outScales2Out, uint64_t* workspaceSize, aclOpExecutor* executor)
{
    bool hasEmptyTensor = x1->IsEmpty() || gamma->IsEmpty() || y2Out->IsEmpty() || outScales1Out->IsEmpty() || outScales2Out->IsEmpty();
    ...
    if (hasEmptyTensor && (!Ops::NN::AclnnUtil::IsRegbase(curArch))) {
        ret = ComputeAddLayerNormQuantV2EmptyInput(x1Cont, x2Cont, gammaCont, betaCont, biasCont, s1Cont, s2Cont, z1Cont, z2Cont,
            quantMode, epsilon, additionalOutput, divMode, outScales1Out, outScales2Out, executor);
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    } else {
        ret = ComputeAddLayerNormQuantV2(x1Cont, x2Cont, gammaCont, betaCont, biasCont, s1Cont, s2Cont, z1Cont, z2Cont, quantMode, epsilon,
            additionalOutput, divMode, y1Out, y2Out, xOut, layernormRes, outScales1Out, outScales2Out, executor);
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    }
...
aclnnStatus aclnnAddLayerNormQuantV2GetWorkspaceSize(const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta,
    const aclTensor* biasOptional, const aclTensor* scales1Optional, const aclTensor* scales2Optional, const aclTensor* zeroPoints1Optional, 
    const aclTensor* zeroPoints2Optional, const char* quantMode, double epsilon, bool additionalOutput, bool divMode, aclTensor* y1Out, aclTensor* y2Out,
    aclTensor* xOut, aclTensor* layernormRes, aclTensor* outScales1Out, aclTensor* outScales2Out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    auto ret = CheckParamsV2(x1, x2, gamma, beta, biasOptional, scales1Optional, scales2Optional, zeroPoints1Optional,
        zeroPoints2Optional, y1Out, y2Out, xOut, layernormRes, outScales1Out, outScales2Out, quantMode);
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
    ...
    ret = PrepareContiguousAndCompute(x1, x2, gamma, beta, biasOptional, scales1Optional, scales2Optional,
        ...
        y1Out, y2Out, xOut, layernormRes, outScales1Out, outScales2Out, workspaceSize, uniqueExecutor.get());
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
...
aclnnStatus aclnnAddLayerNormQuantV2(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream)
{

 ...
 *
 * @file test_AddLayerNormQuantV2_proto.cpp
 *
 * @brief
 *
 * @version 1.0
...
#include "../../../op_graph/add_layer_norm_quant_v2_proto.h"
...
{
    ASSERT_NE(gert::OpImplRegistry::GetInstance().GetOpImpl("AddLayerNormQuantV2"), nullptr);
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("AddLayerNormQuantV2")->infer_shape;
    ...
    if (infer_shape_func != nullptr) {
        ...
        EXPECT_EQ(context->GetOutputShape(4)->GetDim(0), 24);
        EXPECT_EQ(context->GetOutputShape(4)->GetDim(1), 1);

        EXPECT_EQ(context->GetOutputShape(4)->GetDimNum(), 2);
    }
}

TEST_F(AddLayerNormQuantV2, AddLayerNormQuantV2_infershape_case_unknown_rank)
{
    ASSERT_NE(gert::OpImplRegistry::GetInstance().GetOpImpl("AddLayerNormQuantV2"), nullptr);
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("AddLayerNormQuantV2")->infer_shape;
    ...
    if (infer_shape_func != nullptr) {
        ...
        EXPECT_EQ(context->GetOutputShape(5)->GetDim(0), -2);
    }
}

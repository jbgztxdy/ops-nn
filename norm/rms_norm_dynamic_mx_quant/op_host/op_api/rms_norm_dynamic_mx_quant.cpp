...
/*!
 * \file rms_norm_dynamic_mx_quant.cpp
 * \brief
 */
    ...
    aclTensor* mxscaleOut, aclTensor* rstdOut, aclOpExecutor* executor)
{
    ...
    // 调用device的RmsNormDynamicMxQuant算子
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(
        RmsNormDynamicMxQuant, OP_INPUT(x, gamma, beta), OP_OUTPUT(yOut, mxscaleOut, rstdOut),
        OP_ATTR(static_cast<float>(epsilon), scaleAlg, roundMode, dstType, outputRstd));
    if (ret != ACL_SUCCESS) {
        OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "RmsNormDynamicMxQuant ADD_TO_LAUNCHER_LIST_AICORE failed.");
        return {nullptr, nullptr, nullptr};
    }
    return {yOut, mxscaleOut, rstdOut};
}

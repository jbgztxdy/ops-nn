...
/*!
 * \file rms_norm_dynamic_mx_quant_reduce_empty.h
 * \brief When numN=0 and numM!=0, fill rstd output with NaN.
 */
#ifndef RMS_NORM_DYNAMIC_MX_QUANT_REDUCE_EMPTY_H_
#define RMS_NORM_DYNAMIC_MX_QUANT_REDUCE_EMPTY_H_
...
class RmsNormDynamicMxQuantReduceEmpty {
public:
    __aicore__ inline explicit RmsNormDynamicMxQuantReduceEmpty(
        const RmsNormDynamicMxQuantReduceEmptyTilingData* tilingDataIn)
    {
    ...
    __aicore__ inline void Init(GM_ADDR rstd)
    {
        ...
        hasOutputRstd_ = tilingData_->hasOutputRstd;
        numM_ = tilingData_->numM;
    ...
    {
        if (blockIdx_ >= usedCoreNum_ || hasOutputRstd_ == 0 || numM_ == 0) {
            return;
        }
...
private:
    ...
    TPipe pipe_;
    const RmsNormDynamicMxQuantReduceEmptyTilingData* tilingData_;
...
#endif // RMS_NORM_DYNAMIC_MX_QUANT_REDUCE_EMPTY_H_

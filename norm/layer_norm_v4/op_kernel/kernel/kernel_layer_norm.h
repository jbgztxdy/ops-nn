...
template <class BlockLayerNormCopyInX_, class BlockLayerNormCopyInGamma_, class BlockLayerNormCopyInBeta_,
          class BlockLayerNormCopyOutX_, class BlockLayerNormCopyOutMean_, class BlockLayerNormCopyOutRstd_,
          typename Tfm, typename Tweight>
...
public:
    ...
    __aicore__ inline void operator()()
    {
        ...
        for (this->curLoop_ = 0; this->curLoop_ < this->loopCount_; this->curLoop_++) {
            ...
            if (unlikely(this->curMeanIndex_ == (meanLocalSize_ - 1))) {
                ...
                blockLayerNormCopyOutMean(this->meanGm_[meanCopyOutIndex * meanLocalSize_], this->meanLocal_,
                                          meanLocalSize_);
                blockLayerNormCopyOutRstd(this->rstdGm_[meanCopyOutIndex * meanLocalSize_], this->rstdLocal_,
                                          meanLocalSize_);
                meanCopyOutIndex += 1;
    ...
    __aicore__ inline void InitInput(const Arguments& args)
    {
        ...
        uint32_t tileLengthFp16Aligned = (this->tileLength_ + BLOCK_SIZE_OF_FLOAT16 - 1) / BLOCK_SIZE_OF_FLOAT16 *
    ...
    __aicore__ inline void InitGmBuffer(const Arguments& args)
    {
        ...
        if (GetBlockIdx() < this->tailBlock_) {
            ...
            xGmOffset = this->loopCount_ * this->blockLength_ * GetBlockIdx() + this->blockLength_ * this->tailBlock_;

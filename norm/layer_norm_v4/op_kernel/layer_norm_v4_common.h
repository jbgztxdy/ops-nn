                                            ...
                                            DataCopyStruct dataCopyStruct, uint32_t tileLength)
{
    ...
    DataCopyPad(dstTensor[(sizeof(T) == FLOAT16_SIZE) * tileLength], srcTensor, dataCopyParams, padParams);
}

template <typename T>
__aicore__ inline void DataCopyOutContiguous(GlobalTensor<T> dstTensor, LocalTensor<T> srcTensor,

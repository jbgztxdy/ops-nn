                                ...
                                GM_ADDR workspace, const LayerNormV4MergeNTilingData* __restrict tilingData)
    {
        ...
        if (GetBlockIdx() < formerNum) {
            ...
            paramGmOffset = (loopCount * nRow + formerRemainNum) * formerNum +
                            (paramGmSize) * (GetBlockIdx() - formerNum);
...
private:
    __aicore__ inline void ReduceSumWithModeDifTensor(LocalTensor<float> dstTensor, LocalTensor<float> srcTensor,
                                                      uint32_t repeat, uint32_t mask, uint32_t dstRepStride,
                                                      uint32_t srcBlkStride, uint32_t srcRepStride)
    {
        ...
        for (uint32_t i = 0; i < reduceNum; ++i) {
            WholeReduceSum<float>(dstTensor[i * ALIGN_MAX_REPEAT_TIMES],
                                  srcTensor[i * ALIGN_MAX_REPEAT_TIMES * rowAlign], mask, ALIGN_MAX_REPEAT_TIMES,
                                  dstRepStride, srcBlkStride, srcRepStride);
        }
        WholeReduceSum<float>(dstTensor[reduceNum * ALIGN_MAX_REPEAT_TIMES],
                              srcTensor[reduceNum * ALIGN_MAX_REPEAT_TIMES * rowAlign], mask,
                              repeat % ALIGN_MAX_REPEAT_TIMES, dstRepStride, srcBlkStride, srcRepStride);
        PipeBarrier<PIPE_V>();
    }

    __aicore__ inline void ReduceSumWithModeSameTensor(LocalTensor<float> dstTensor, LocalTensor<float> srcTensor,
                                                       uint32_t repeat, uint32_t mask, uint32_t dstRepStride,
                                                       uint32_t srcBlkStride, uint32_t srcRepStride)
    {
        uint32_t blockNum = ((repeat * rowAlign) + NUM_SEVEN) / NUM_EIGHT;
    ...
    __aicore__ inline void MultiBrcb(LocalTensor<float> dstTensor, LocalTensor<float> srcTensor, uint32_t brcbRepeat)
    {
        ...
        for (uint32_t i = 0; i < num; ++i) {
            Brcb(dstTensor[i * MAX_REPEAT_TIMES * NUM_EIGHT * NUM_EIGHT], srcTensor[i * MAX_REPEAT_TIMES * NUM_EIGHT],
                 MAX_REPEAT_TIMES, {1, FLOAT_TO_BLOCK});
        }
        Brcb(dstTensor[num * MAX_REPEAT_TIMES * NUM_EIGHT * NUM_EIGHT], srcTensor[num * MAX_REPEAT_TIMES * NUM_EIGHT],
             brcbRepeat % MAX_REPEAT_TIMES, {1, FLOAT_TO_BLOCK});
        PipeBarrier<PIPE_V>();
    }
    ...
    __aicore__ inline void MultiSelfAdd64(LocalTensor<float> dstTensor, ComputeStruct computeStruct)
    {
        for (uint32_t i = 1; i < rowParts - 1; ++i) {
            Add(dstTensor, dstTensor[i * BLOCK_ALIGN_SIZE], dstTensor, BLOCK_ALIGN_SIZE, computeStruct.repeatTimes,
                {1, 1, 1, computeStruct.repStrid, computeStruct.repStrid, computeStruct.repStrid});
            PipeBarrier<PIPE_V>();
        }
        ...
        for (uint32_t i = 0; i < computeStruct.rowCycleNum; ++i) {
            ...
            if (rowParts > 1) {
                Add(dstTensor[dstOffset], dstTensor[rowTailOffset + dstOffset], dstTensor[dstOffset], rowTailMask,
                    MAX_REPEAT_TIMES,
                    {1, 1, 1, computeStruct.repStrid, computeStruct.repStrid, computeStruct.repStrid});
                PipeBarrier<PIPE_V>();
            }
        ...
        if (rowParts > 1) {
            Add(dstTensor[computeStruct.rowAlignOffset], dstTensor[rowTailOffset + computeStruct.rowAlignOffset],
                dstTensor[computeStruct.rowAlignOffset], rowTailMask, computeStruct.rowCycleTailNum,
                {1, 1, 1, computeStruct.repStrid, computeStruct.repStrid, computeStruct.repStrid});
            PipeBarrier<PIPE_V>();
        }
    }

    __aicore__ inline void MultiAdd(LocalTensor<float> dstTensor, LocalTensor<float> srcTensor,
                                    ComputeStruct computeStruct)
    {
        for (uint32_t i = 0; i < computeStruct.rowCycleNum; ++i) {
            ...
            for (uint32_t j = 0; j < rowParts - 1; ++j) {
                ...
                Add(dstTensor[offset + dstOffset], srcTensor[srcOffset], dstTensor[offset + dstOffset],
                    BLOCK_ALIGN_SIZE, MAX_REPEAT_TIMES, {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
            }
            Add(dstTensor[rowTailOffset + dstOffset], srcTensor[srcOffset], dstTensor[rowTailOffset + dstOffset],
                rowTailMask, MAX_REPEAT_TIMES, {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
        }

        for (uint32_t j = 0; j < rowParts - 1; ++j) {
            uint32_t offset = j * BLOCK_ALIGN_SIZE;
            Add(dstTensor[offset + computeStruct.rowAlignOffset], srcTensor[computeStruct.blockOffset],
                dstTensor[offset + computeStruct.rowAlignOffset], BLOCK_ALIGN_SIZE, computeStruct.rowCycleTailNum,
                {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
        }
        Add(dstTensor[rowTailOffset + computeStruct.rowAlignOffset], srcTensor[computeStruct.blockOffset],
            dstTensor[rowTailOffset + computeStruct.rowAlignOffset], rowTailMask, computeStruct.rowCycleTailNum,
            {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
        PipeBarrier<PIPE_V>();
    }

    __aicore__ inline void MultiMul(LocalTensor<float> dstTensor, LocalTensor<float> srcTensor,
                                    ComputeStruct computeStruct)
    {
        for (uint32_t i = 0; i < computeStruct.rowCycleNum; ++i) {
            uint32_t dstOffset = i * rowAlign * MAX_REPEAT_TIMES;
            ...
            for (uint32_t j = 0; j < rowParts - 1; ++j) {
                uint32_t offset = j * BLOCK_ALIGN_SIZE;
                Mul(dstTensor[offset + dstOffset], srcTensor[srcOffset], dstTensor[offset + dstOffset],
                    BLOCK_ALIGN_SIZE, MAX_REPEAT_TIMES, {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
            }
            Mul(dstTensor[rowTailOffset + dstOffset], srcTensor[srcOffset], dstTensor[rowTailOffset + dstOffset],
                rowTailMask, MAX_REPEAT_TIMES, {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
        }

        for (uint32_t j = 0; j < rowParts - 1; ++j) {
            uint32_t offset = j * BLOCK_ALIGN_SIZE;
            Mul(dstTensor[offset + computeStruct.rowAlignOffset], srcTensor[computeStruct.blockOffset],
                dstTensor[offset + computeStruct.rowAlignOffset], BLOCK_ALIGN_SIZE, computeStruct.rowCycleTailNum,
                {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
        }

        Mul(dstTensor[rowTailOffset + computeStruct.rowAlignOffset], srcTensor[computeStruct.blockOffset],
            dstTensor[rowTailOffset + computeStruct.rowAlignOffset], rowTailMask, computeStruct.rowCycleTailNum,
            {1, 0, 1, computeStruct.repStrid, 1, computeStruct.repStrid});
        PipeBarrier<PIPE_V>();
    }

    __aicore__ inline void MultiMulAdd(LocalTensor<float> dstTensor, LocalTensor<float> srcTensor,
                                       ComputeStruct computeStruct)
    {
        // calculate y = x * gamma
        for (uint32_t i = 0; i < computeStruct.rowCycleNum; ++i) {
            ...
            for (uint32_t j = 0; j < rowParts - 1; ++j) {
                uint32_t offset = j * BLOCK_ALIGN_SIZE;
                Mul(dstTensor[offset + dstOffset], srcTensor[offset], dstTensor[offset + dstOffset], BLOCK_ALIGN_SIZE,
                    MAX_REPEAT_TIMES, {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
                PipeBarrier<PIPE_V>();
                Add(dstTensor[offset + dstOffset], srcTensor[offset + rowAlign], dstTensor[offset + dstOffset],
                    BLOCK_ALIGN_SIZE, MAX_REPEAT_TIMES, {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
            }
            Mul(dstTensor[rowTailOffset + dstOffset], srcTensor[rowTailOffset], dstTensor[rowTailOffset + dstOffset],
                rowTailMask, MAX_REPEAT_TIMES, {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
            PipeBarrier<PIPE_V>();
            Add(dstTensor[rowTailOffset + dstOffset], srcTensor[rowTailOffset + rowAlign],
                dstTensor[rowTailOffset + dstOffset], rowTailMask, MAX_REPEAT_TIMES,
                {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
        }
        for (uint32_t i = 0; i < rowParts - 1; ++i) {
            uint32_t offset = i * BLOCK_ALIGN_SIZE;
            Mul(dstTensor[offset + computeStruct.rowAlignOffset], srcTensor[offset],
                dstTensor[offset + computeStruct.rowAlignOffset], BLOCK_ALIGN_SIZE, computeStruct.rowCycleTailNum,
                {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
            PipeBarrier<PIPE_V>();
            Add(dstTensor[offset + computeStruct.rowAlignOffset], srcTensor[offset + rowAlign],
                dstTensor[offset + computeStruct.rowAlignOffset], BLOCK_ALIGN_SIZE, computeStruct.rowCycleTailNum,
                {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
        }
        Mul(dstTensor[rowTailOffset + computeStruct.rowAlignOffset], srcTensor[rowTailOffset],
            dstTensor[rowTailOffset + computeStruct.rowAlignOffset], rowTailMask, computeStruct.rowCycleTailNum,
            {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
        PipeBarrier<PIPE_V>();
        Add(dstTensor[rowTailOffset + computeStruct.rowAlignOffset], srcTensor[rowTailOffset + rowAlign],
            dstTensor[rowTailOffset + computeStruct.rowAlignOffset], rowTailMask, computeStruct.rowCycleTailNum,
            {1, 1, 1, computeStruct.repStrid, 0, computeStruct.repStrid});
        PipeBarrier<PIPE_V>();
    }

    __aicore__ inline void DataCopyInX(LocalTensor<float>& dstTensor, GlobalTensor<Tfm> srcTensor,
    ...
    __aicore__ inline void DataCopyOutY(GlobalTensor<Tfm> dstTensor, LocalTensor<float>& srcTensor,
    ...
    __aicore__ inline void processCompute(LocalTensor<float>& xLocal, LocalTensor<float>& yLocal,
                                          LocalTensor<float>& rstdLocal, ComputeStruct computeStruct,
                                          uint32_t brcbRepeat)
    {
        ...
        int32_t srcRepStride = computeStruct
                                   .repStrid; //源操作数相邻迭代间的地址步长，即源操作数每次迭代跳过的datablock数目
    ...
    __aicore__ inline void ProcessBasicBlock(uint32_t rowNum, uint32_t currentBlockOffset, uint32_t currentParamOffset)
    {
        ...
        DataCopyStruct dataCopyStruct1{
            rowAlign == rowSize ? 1 : rowNum,
            static_cast<uint32_t>(rowAlign == rowSize ? rowNum * rowSize * sizeof(Tfm) : rowSize * sizeof(Tfm)),
            0,
            0,
            rowAlign != rowSize,
            0,
        ...
        PipeBarrier<PIPE_V>();
        ComputeStruct computeStruct{rowAlign < BLOCK_ALIGN_SIZE ? rowTailMask : BLOCK_ALIGN_SIZE,
                                    static_cast<int32_t>(rowNum),
                                    static_cast<uint8_t>(rowAlign * FLOAT32_SIZE / BLOCK_SIZE),
                                    rowNum / MAX_REPEAT_TIMES,
                                    rowNum % MAX_REPEAT_TIMES,
                                    rowNum / MAX_REPEAT_TIMES * rowAlign * MAX_REPEAT_TIMES,
                                    rowNum / MAX_REPEAT_TIMES * NUM_EIGHT * MAX_REPEAT_TIMES};
        // for每次偏移 需要偏移一个mask块，
        ...
        DataCopyStruct dataCopyStruct3{1, static_cast<uint32_t>(rowSize * sizeof(Tweight)), 0, 0, rowAlign != rowSize,
                                       0, static_cast<uint8_t>(rowAlign - rowSize)};
        ...
        DataCopyStruct dataCopyStruct5{
            rowAlign == rowSize ? 1 : rowNum,
            static_cast<uint32_t>(rowAlign == rowSize ? rowNum * rowSize * sizeof(Tfm) : rowSize * sizeof(Tfm)),
            0,
            0,

...
extern "C" __global__ __aicore__ void group_norm_swish(GM_ADDR x, GM_ADDR gamma, GM_ADDR beta, GM_ADDR y, GM_ADDR mean,
                                                       GM_ADDR rstd, GM_ADDR workspace, GM_ADDR tiling);
class group_norm_swish_test : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "group_norm_swish_test SetUp\n" << std::endl; }
    static void TearDownTestCase() { std::cout << "group_norm_swish_test TearDown\n" << std::endl; }
};
...
TEST_F(group_norm_swish_test, test_case_211)
{
    ...
    ICPU_RUN_KF(group_norm_swish, blockDim / 2, x, gamma, beta, swish, mean, rstd, workspace,

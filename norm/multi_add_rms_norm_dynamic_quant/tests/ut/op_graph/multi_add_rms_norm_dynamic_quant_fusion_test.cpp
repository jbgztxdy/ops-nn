...
/**
 * Test dual: 2 Add + AddRmsNorm + 2 DynamicQuant (with separate smooth_scales inputs)
 * Graph topology:
 *   x1_0, x1_1 -> Add1 -> Add2(with x1_2) -> AddRmsNorm(with x2, gamma) -> [y, x_out]
 *   y, smooth1 -> DynamicQuant1 -> [y1, scale1]
 *   y, smooth2 -> DynamicQuant2 -> [y2, scale2]
 */
TEST_F(multi_add_rms_norm_dynamic_quant_fusion_test, multi_add_rms_norm_dynamic_quant_fusion_test_dual)
{
    SetUpPlatform();
    ...
    auto graph_builder = es::EsGraphBuilder("multi_add_rms_norm_dq_dual");
    ...
    auto smooth1 = graph_builder.CreateInput(5, "smooth1", DT_FLOAT16, FORMAT_ND, dims_gamma);
    auto smooth2 = graph_builder.CreateInput(6, "smooth2", DT_FLOAT16, FORMAT_ND, dims_gamma);

    // Build pattern: 2 Add -> AddRmsNorm -> 2 DynamicQuant(with smooth)
    ...
    auto dq1 = BuildDynamicQuantNode(graph_builder, arnResult.y, smooth1);
    auto dq2 = BuildDynamicQuantNode(graph_builder, arnResult.y, smooth2);

    std::vector<es::EsTensorHolder> outputs;
    outputs.emplace_back(std::move(dq1.y));
    outputs.emplace_back(std::move(dq2.y));
    ...
    outputs.emplace_back(std::move(dq2.scale));
    std::shared_ptr<Graph> graph = graph_builder.Build(outputs);
    CustomPassContext pass_context;

    ops::MultiAddRmsNormDynamicQuantFusionPass pass;
    Status status = pass.Run(graph, pass_context);
    ClearPlatform();
    EXPECT_EQ(status, SUCCESS);
    EXPECT_TRUE(HasFusedOp(graph, "MultiAddRmsNormDynamicQuant"));
}

/**
 * Test dual_reshape: 2 Add + AddRmsNorm + Reshape + 2 DynamicQuant
 * Graph topology:
 *   x1_0, x1_1 -> Add1 -> Add2(with x1_2) -> AddRmsNorm(with x2, gamma) -> [y, x_out]
 *   y -> Reshape -> smooth_reshaped
 *   y, smooth_reshaped -> DynamicQuant1 -> [y1, scale1]
 *   y, smooth_reshaped -> DynamicQuant2 -> [y2, scale2]
 */
TEST_F(multi_add_rms_norm_dynamic_quant_fusion_test, multi_add_rms_norm_dynamic_quant_fusion_test_dual_reshape)
{
    SetUpPlatform();
    ...
    auto graph_builder = es::EsGraphBuilder("multi_add_rms_norm_dq_dual_reshape");
    auto x1_0 = graph_builder.CreateInput(0, "x1_0", DT_FLOAT16, FORMAT_ND, dims_x);
    auto x1_1 = graph_builder.CreateInput(1, "x1_1", DT_FLOAT16, FORMAT_ND, dims_x);
    auto x1_2 = graph_builder.CreateInput(2, "x1_2", DT_FLOAT16, FORMAT_ND, dims_x);
    auto x2 = graph_builder.CreateInput(3, "x2", DT_FLOAT16, FORMAT_ND, dims_x);
    ...
    // Build pattern: 2 Add -> AddRmsNorm -> Reshape -> 2 DynamicQuant
    auto add1 = BuildAddTensor(graph_builder, x1_0, x1_1);
    auto add2 = BuildAddTensor(graph_builder, add1, x1_2);
    auto arnResult = BuildAddRmsNormNode(graph_builder, add2, x2, gamma);
    auto smooth_reshaped = BuildReshapeNode(graph_builder, arnResult.y, BuildConstNode(graph_builder));
    auto dq1 = BuildDynamicQuantNode(graph_builder, arnResult.y, smooth_reshaped);
    auto dq2 = BuildDynamicQuantNode(graph_builder, arnResult.y, smooth_reshaped);

    std::vector<es::EsTensorHolder> outputs;
    outputs.emplace_back(std::move(dq1.y));
    outputs.emplace_back(std::move(dq2.y));
    outputs.emplace_back(std::move(arnResult.x));
    outputs.emplace_back(std::move(smooth_reshaped));
    outputs.emplace_back(std::move(dq1.scale));
    outputs.emplace_back(std::move(dq2.scale));
    std::shared_ptr<Graph> graph = graph_builder.Build(outputs);
    CustomPassContext pass_context;
...
/**
 * Test sole: 2 Add + AddRmsNorm + 1 DynamicQuant (with smooth_scales)
 * Graph topology:
 *   x1_0, x1_1 -> Add1 -> Add2(with x1_2) -> AddRmsNorm(with x2, gamma) -> [y, x_out]
 *   y, smooth -> DynamicQuant -> [y1, scale1]
 */
TEST_F(multi_add_rms_norm_dynamic_quant_fusion_test, multi_add_rms_norm_dynamic_quant_fusion_test_sole)
{
    SetUpPlatform();
    ...
    auto graph_builder = es::EsGraphBuilder("multi_add_rms_norm_dq_sole");
    auto x1_0 = graph_builder.CreateInput(0, "x1_0", DT_FLOAT16, FORMAT_ND, dims_x);
    auto x1_1 = graph_builder.CreateInput(1, "x1_1", DT_FLOAT16, FORMAT_ND, dims_x);
    auto x1_2 = graph_builder.CreateInput(2, "x1_2", DT_FLOAT16, FORMAT_ND, dims_x);
    auto x2 = graph_builder.CreateInput(3, "x2", DT_FLOAT16, FORMAT_ND, dims_x);
    ...
    // Build pattern: 2 Add -> AddRmsNorm -> 1 DynamicQuant(with smooth)
    auto add1 = BuildAddTensor(graph_builder, x1_0, x1_1);
    auto add2 = BuildAddTensor(graph_builder, add1, x1_2);
    auto arnResult = BuildAddRmsNormNode(graph_builder, add2, x2, gamma);

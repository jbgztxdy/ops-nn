...
{
    OP_CHECK_IF(context == nullptr, CUBE_INNER_ERR_REPORT("Sparse4to2QuantMatmul", "Context of infershape is null"),
    ...
    OP_CHECK_IF(xShape == nullptr || weightShape == nullptr || yShape == nullptr,
    ...
    OP_LOGD(opName, "Input xShape: %s, weightShape: %s", Ops::Base::ToString(*xShape).c_str(),
    ...
    if (xShape->GetDimNum() != ND_DIM_NUM || weightShape->GetDimNum() != ND_DIM_NUM) {
        CUBE_INNER_ERR_REPORT(opName,
                              "Input x and sparseWeight shape only support two dims shape, actually is %zu and %zu.",

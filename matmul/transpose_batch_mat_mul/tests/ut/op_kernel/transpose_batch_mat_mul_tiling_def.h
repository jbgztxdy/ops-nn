...
inline void InitMatmulTilingData(uint8_t* tiling, TBMMTilingData* const_data)
{
    memcpy(const_data, tiling, sizeof(TBMMTilingData));
}

inline void InitPpMatmulTilingData(uint8_t* tiling, PpMatmulTilingData* const_data)
{
    memcpy(const_data, tiling, sizeof(PpMatmulTilingData));
}

#define GET_TILING_DATA_WITH_STRUCT(tiling_struct, tiling_data, tiling_arg) \
    PpMatmulTilingData tiling_data;                                              \
    InitPpMatmulTilingData(tiling_arg, &tiling_data)

#define GET_TILING_DATA(tiling_data, tiling_arg)                                       \
    TBMMTilingData tiling_data;                                                 \

...
FusedMatMul CreateFusedMatmulOp(OP_TUPLE x1, OP_TUPLE x2, OP_TUPLE bias, OP_TUPLE x3, const bool& transpose_x1,
                                const bool& transpose_x2, const bool& enable_hf32, const std::string& fused_op_type)
{
    ...
    if (!shape_bias.empty()) {
        tensor_bias = create_desc_shape_range(get<0>(bias), get<1>(bias), FORMAT_ND, get<0>(bias), FORMAT_ND,
                                              get<2>(bias));
    }

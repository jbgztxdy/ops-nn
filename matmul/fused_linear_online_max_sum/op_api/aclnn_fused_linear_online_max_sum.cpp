...
inline static bool CheckAttrValid(int64_t vocabStartIndex, int64_t vocabEndIndex)
{
    ...
    if (vocabEndIndex < vocabStartIndex) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                "vocabEndIndex should be greater than or equal to vocabStartIndex, but got "
                "vocabEndIndex %ld, vocabStartIndex %ld",
                vocabEndIndex, vocabStartIndex);
        return false;
    }
    ...
    bool vocabParallelLogitsOutFlag)
{
    if (std::get<LOGITS_MAX_LOCAL_OUT_IDX>(result) == nullptr ||
        ...
        std::get<TARGET_MASK_OUT_IDX>(result) == nullptr || std::get<MASKED_TARGET_OUT_IDX>(result) == nullptr) {
        return false;
    }
...
{
    auto viewCopyResult = l0op::ViewCopy(std::get<LOGITS_MAX_LOCAL_OUT_IDX>(result), outputTensors.logitsMaxLocalOut,
                                         executor);
    CHECK_RET(viewCopyResult != nullptr, nullptr);
    viewCopyResult = l0op::ViewCopy(std::get<SUM_EXP_LOGITS_LOCAL_OUT_IDX>(result), outputTensors.sumExpLogitsLocalOut,
                                    executor);
    CHECK_RET(viewCopyResult != nullptr, nullptr);
    viewCopyResult = l0op::ViewCopy(std::get<PREDICTED_LOGITS_LOCAL_OUT_IDX>(result),
                                    outputTensors.predictedLogitsLocalOut, executor);
    CHECK_RET(viewCopyResult != nullptr, nullptr);
    viewCopyResult = l0op::ViewCopy(std::get<TARGET_MASK_OUT_IDX>(result), outputTensors.targetMaskOut, executor);
    CHECK_RET(viewCopyResult != nullptr, nullptr);
    viewCopyResult = l0op::ViewCopy(std::get<MASKED_TARGET_OUT_IDX>(result), outputTensors.maskedTargetOut, executor);
    CHECK_RET(viewCopyResult != nullptr, nullptr);
    ...
    aclTensor* vocabParallelLogitsOutOptional, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    FusedLinearOnlineMaxSumOutputTensor outputTensors = {logitsMaxLocalOut,       sumExpLogitsLocalOut,
                                                         predictedLogitsLocalOut, targetMaskOut,
                                                         maskedTargetOut,         vocabParallelLogitsOutOptional};
    ...
    std::tuple<aclTensor*, aclTensor*, aclTensor*, aclTensor*, aclTensor*, aclTensor*>
        result = l0op::FusedLinearOnlineMaxSum(inputContiguous, weightContiguous, targetContiguous, vocabStartIndex,
                                               vocabEndIndex, vocabParallelLogitsOutFlag, uniqueExecutor.get());

...
int aclnnQuantMatmulV3Test(int32_t deviceId, aclrtStream& stream)
{
    ...
    std::vector<int64_t> x2TransposedShape = {n, k};
    ...
    std::vector<int8_t> x2TransposedHostData(n * k, 1);
    std::vector<int32_t> biasHostData(n, 1);
    std::vector<float> scaleHostData(n, 1);
    std::vector<float> offsetHostData(n, 1);
    ...
    ret = CreateAclTensorX2(x2TransposedHostData, x2TransposedShape, &x2TransposedDeviceAddr, aclDataType::ACL_INT8,
                            &x2Transposed);
    ...
    workspaceSize = 0;
    ret = aclnnQuantMatmulV3GetWorkspaceSize(x1, x2Transposed, quantParam, nullptr, bias, transposeX1, transposeX2, out,
...
int main()
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnQuantMatmulV3Test failed. ERROR: %d\n", ret); return ret);

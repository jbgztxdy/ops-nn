...
static std::tuple<aclTensor*, aclTensor*> RotateQuantAICore(
...
{
    L0_DFX(
        RotateQuantAICore, x, rotation, alpha, axis, roundMode, scaleAlg, dstTypeMax, trans, dstType, yOut, scaleOut);
    auto retAicore = ADD_TO_LAUNCHER_LIST_AICORE(
        RotateQuant, OP_INPUT(x, rotation, alpha), OP_OUTPUT(yOut, scaleOut),
        OP_ATTR(dstType, axis, roundMode, scaleAlg, dstTypeMax, trans));
    OP_CHECK_ADD_TO_LAUNCHER_LIST_AICORE(
        retAicore != ACLNN_SUCCESS, return std::tuple(nullptr, nullptr),
    ...
    aclOpExecutor* executor)
{
    ...
    return RotateQuantAICore(
        x, rotation, alpha, axis, roundMode, scaleAlg, dstTypeMax, trans, dstType, yOut, scaleOut, executor);
}

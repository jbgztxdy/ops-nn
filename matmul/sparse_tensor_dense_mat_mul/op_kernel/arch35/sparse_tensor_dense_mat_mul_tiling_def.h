...
struct SparseTensorDenseMatMulTilingData {
    ...
    int64_t
        initAndOutFormerCoreElemNum; // 非尾核，1.Init阶段调用InitGlobalMemory的元素个数（即参数size）；2.Output阶段要处理的元素个数
    ...
    int64_t
        computeTotalElemNum; // 要计算的所有元素的个数，等于nnz*p，其中nnz为稀疏矩阵x1的非0元素个数，p为矩阵x2（转置后）的列数，也即输出y矩阵的列数
     ...
     */
    int64_t
        outFormerCoreUbFactor; // 非尾核，前面的一次循环要处理多少元素，即Ub一次能装下处理的元素个数，UbFactor都必须是对齐32字节后的元素个数!!
    int64_t
        outFormerCoreUbTailFactor; // 非尾核，最后一次循环要处理多少元素（<=outFormerCoreUbFactor），UbFactor都必须是对齐32字节后的元素个数!!

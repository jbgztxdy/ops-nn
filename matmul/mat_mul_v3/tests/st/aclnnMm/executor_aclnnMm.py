...
@register("executor_mm")
class mmApi(BaseApi):
    def __init__(self, task_result: TaskResult):
        super(mmApi, self).__init__(task_result)

    def __call__(self, input_data: InputDataset, with_output: bool = False):
        # 标杆计算
        mat_self = input_data.kwargs["self"]
        mat2 = input_data.kwargs["mat2"]
        output = torch.mm(mat_self, mat2)
        return output

@register("executor_aclnn_mm")
class aclnnMmApi(AclnnBaseApi):
    def __init__(self, task_result: TaskResult, backend):
        super(aclnnMmApi, self).__init__(task_result, backend)

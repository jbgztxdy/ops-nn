...
TEST_F(mat_mul_v3_test, mat_mul_v3_test_2) {
    // {{965, 30583}, {30583, 472}}
    AscendC::SetKernelMode(KernelMode::MIX_MODE);

    size_t shape_a = 965 * 30583 * sizeof(uint32_t);
    size_t shape_b = 30583 * 472 * sizeof(uint32_t);
    size_t shape_output = 965 * 472 * sizeof(uint32_t);

    size_t sysWorkspaceSize = 24 * 2048 * 2048;
    size_t usrWorkspaceSize = 0;
    size_t allWorkspaceSize = usrWorkspaceSize + sysWorkspaceSize;
    uint8_t* workspace = (uint8_t*)AscendC::GmAlloc(allWorkspaceSize);
    size_t tilingSize = sizeof(MatmulTilingData);
    ...
    tiling_data->matmulTiling.M = 965;
    tiling_data->matmulTiling.N = 472;
    tiling_data->matmulTiling.Ka = 30583;
    tiling_data->matmulTiling.Kb = 30583;
    tiling_data->matmulTiling.singleCoreM = 965;
    tiling_data->matmulTiling.singleCoreN = 384;
    tiling_data->matmulTiling.singleCoreK = 192;
    tiling_data->matmulTiling.baseM = 128;
    tiling_data->matmulTiling.baseN = 128;
    tiling_data->matmulTiling.baseK = 64;
    tiling_data->matmulTiling.depthA1 = 6;
    tiling_data->matmulTiling.depthB1 = 9;
    tiling_data->matmulTiling.stepM = 1;
    tiling_data->matmulTiling.stepN = 3;
    tiling_data->matmulTiling.stepKa = 3;
    tiling_data->matmulTiling.stepKb = 3;
    tiling_data->matmulTiling.isBias = 0;
    tiling_data->matmulTiling.transLength = 0;
    tiling_data->matmulTiling.iterateOrder = 0;
    tiling_data->matmulTiling.shareL1Size = 114688;
    ...
    tiling_data->matmulRunInfo.isNzB = 0;
    tiling_data->l2cacheUseInfo.l2CacheFlag = 0;
    tiling_data->tileL2cacheTiling.mTileCntL2 = 2;
    tiling_data->tileL2cacheTiling.nTileCntL2 = 1;
    tiling_data->tileL2cacheTiling.mTileBlock = 4;
    tiling_data->tileL2cacheTiling.nTileBlock = 3;
    tiling_data->tileL2cacheTiling.calOrder = 0;

    auto mat_mul_v3_wrapper = [](GM_ADDR x, GM_ADDR w, GM_ADDR bias, GM_ADDR contextGM, GM_ADDR y, GM_ADDR workspace,
                                 GM_ADDR tiling) {
        ::mat_mul_v3<0, 3, 2, 1, 0>(x, w, bias, contextGM, y, workspace, tiling);
    };

    ICPU_RUN_KF(mat_mul_v3_wrapper, 24, aGM, bGM, nullptr, contextGM, output, workspace, tiling);
...
TEST_F(mat_mul_v3_test, mat_mul_v3_test_3) {
    // {{965, 30583}, {30583, 472}}
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ...
    tiling_data->matmulTiling.usedCoreNum = 24;
    tiling_data->matmulTiling.M = 965;
    tiling_data->matmulTiling.N = 257;
    tiling_data->matmulTiling.Ka = 30583;
    tiling_data->matmulTiling.Kb = 30583;
    tiling_data->matmulTiling.singleCoreM = 384;
    tiling_data->matmulTiling.singleCoreN = 257;
    tiling_data->matmulTiling.singleCoreK = 192;
    tiling_data->matmulTiling.baseM = 128;
    tiling_data->matmulTiling.baseN = 128;
    tiling_data->matmulTiling.baseK = 64;
    tiling_data->matmulTiling.depthA1 = 9;
    tiling_data->matmulTiling.depthB1 = 6;
    tiling_data->matmulTiling.stepM = 3;
    tiling_data->matmulTiling.stepN = 1;
    tiling_data->matmulTiling.stepKa = 3;
    tiling_data->matmulTiling.stepKb = 3;
    tiling_data->matmulTiling.isBias = 0;
    tiling_data->matmulTiling.transLength = 0;
    tiling_data->matmulTiling.iterateOrder = 1;
    tiling_data->matmulTiling.shareL1Size = 196608;
    tiling_data->matmulTiling.shareL0CSize = 55296;
    tiling_data->matmulTiling.shareUbSize = 0;
    tiling_data->matmulTiling.batchM = 1;
    tiling_data->matmulTiling.batchN = 1;
    tiling_data->matmulTiling.singleBatchM = 1;

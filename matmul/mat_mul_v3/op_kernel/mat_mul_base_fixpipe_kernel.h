...
public:
    ...
    __aicore__ inline bool PrepareNz2NdConversion(uint8_t pingPongId, LocalTensor<C_T>& tensorNZ,
                                                  LocalTensor<C_T>& tensorND, uint64_t& ubProcessMNum,
                                                  uint64_t& srcGmOffset, uint64_t& dstGmOffset);

    // AIV 侧 NZ2ND 的公共拷入与转换流程。
    __aicore__ inline void Nz2NdCopyAndConvert(GlobalTensor<C_T> cNzGlobal, LocalTensor<C_T> tensorNZ,
                                               LocalTensor<C_T> tensorND, uint64_t ubProcessMNum, uint64_t srcGmOffset,
                                               uint8_t pingPongId, uint64_t gatherNBlocks);
...
__aicore__ inline void MatmulBaseFixPipeKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG,
                                               MM_CB>::AicLoadAndIterate(bool aicNeedWaitAiv, uint8_t pingPongId)
{
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG,
          class MM_CB>
__aicore__ inline void MatmulBaseFixPipeKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG,
                                               MM_CB>::AicProcess(GlobalTensor<typename C_TYPE::T>& cTensor,
                                                                  uint8_t enAtomic, bool aicNeedWaitAiv,
                                                                  uint8_t pingPongId)
{
    if ASCEND_IS_AIC {
...
MatmulBaseFixPipeKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG, MM_CB>::UpdateOffsetC(bool isNd)
{
...
MatmulBaseFixPipeKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG, MM_CB>::PrepareNz2NdConversion(
    uint8_t pingPongId, LocalTensor<C_T>& tensorNZ, LocalTensor<C_T>& tensorND, uint64_t& ubProcessMNum,
    uint64_t& srcGmOffset, uint64_t& dstGmOffset)
{
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG,
          class MM_CB>
__aicore__ inline void
MatmulBaseFixPipeKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG, MM_CB>::Nz2NdCopyAndConvert(
    GlobalTensor<C_T> cNzGlobal, LocalTensor<C_T> tensorNZ, LocalTensor<C_T> tensorND, uint64_t ubProcessMNum,
    uint64_t srcGmOffset, uint8_t pingPongId, uint64_t gatherNBlocks)
{
    Nz2NdCopyInWithNz<C_T>(tensorNZ, cNzGlobal[srcGmOffset], ubProcessMNum, pingPongId, this->alignedN_,
                           this->block_.matmulTilingData_->matmulTiling.baseM);
    CrossCoreSetFlag<0x2, PIPE_MTE2>(AIC_SYNC_AIV_FLAG + pingPongId);
    Nz2NdMulsAndGatherMask<C_T>(tensorND, tensorNZ, ubProcessMNum, pingPongId, this->alignedN_, this->c0Size_,
                                gatherNBlocks, this->params_);

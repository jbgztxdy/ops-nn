...
__aicore__ inline void MatrixAtoNZV2(GM_ADDR workspace, GM_ADDR src, const TCubeTiling& cfg, bool isTransposeA,
                                     ...
                                     uint32_t batch = 1, uint32_t originalBatch = 1)
{
    uint64_t c0Size = 16;
    GetSizeC0<T>(c0Size);
    ...
    if (batch > 1) {
        ...
        if ((oriN > 8192) && (oriD > 1) && (oriD != c0Size) &&
            ...
            if (originalBatch > 1 &&
                nValue * dValue * sizeof(T) * 2 > 192 * 1024) { // nd2nz cannot exceed the size of UB
                Nd2nzVnchwBMM(tempDstGlobal, tempSrcGlobal, oriN, oriD, batch, tmpBuf, usedCoreNum);
            } else {
                MatrixtoNZ(oriN, oriD, nValue, dValue, baseAN, baseAD, usedCoreNum, tempSrcGlobal, tempDstGlobal,
                           tmpBuf);
            }
        }
    }
    ...
    if (batch > 1) {
        ...
        if ((oriN > 8192) && (oriD > 1) && (oriD != c0Size) &&
            ...
            if (originalBatch > 1 &&
                ...
                MatrixtoNZ(oriN, oriD, nValue, dValue, baseBN, baseBD, usedCoreNum, tempSrcGlobal1, tempDstGlobal1,
                           tmpBuf);
            }
        }
    }

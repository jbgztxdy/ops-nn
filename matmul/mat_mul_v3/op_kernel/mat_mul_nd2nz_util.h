...
enum class Nd2NzMode : int32_t { MULTI_CORE = 0, SINGLE_CORE = 1 };
                                ...
                                uint32_t hBuffer, uint32_t wTail, bool hasFlag)
{
    ...
    for (uint32_t i = 0; i < widthBlock; i++) {
        if constexpr (sizeof(T) == sizeof(half)) {
            for (uint32_t j = 0; j < VNCHW_SIZE; j++) {
                ...
                dstLocalList[j] = (j * BLOCK_SIZE_BYTE * hBlockNum + dstLocal.GetPhyAddr() +
        ...
        if constexpr (sizeof(T) == sizeof(float)) {
            for (uint32_t j = 0; j < VNCHW_SIZE; j++) {
                ...
                srcLocalList[j] = ((2 * (j % 8) + j / 8) * BLOCK_SIZE_BYTE + midBuf.GetPhyAddr() +
                                   i * VNCHW_SIZE * BLOCK_SIZE_BYTE);
    ...
    if constexpr (sizeof(T) == sizeof(float)) {
        for (uint32_t j = 0; j < wTail; j++) {
            srcLocalList[j] = ((2 * j) * BLOCK_SIZE_BYTE + midBuf.GetPhyAddr() +
                               widthBlock * VNCHW_SIZE * BLOCK_SIZE_BYTE);
            srcLocalList[j + 8] = ((2 * j + 1) * BLOCK_SIZE_BYTE + midBuf.GetPhyAddr() +
                                   widthBlock * VNCHW_SIZE * BLOCK_SIZE_BYTE);
        }
        if (totalWidth > c0Size) {

...
__aicore__ inline void MatmulStreamKBlock::Init(const void* tilingData)
{
    ...
    // sk过程k方向切分块数
    if (params_.round >= 2) {
        aicParams_.blockBaseK = MMV3CeilAlign(matmulTilingData_->tCubeTiling.Ka / matmulTilingData_->kTailCnt,
                                              BLOCK_SIZE);
    } else {
        aicParams_.blockBaseK = matmulTilingData_->tCubeTiling.singleCoreK;
    }
    // 刷新总的使用切分块数
    aicParams_.lastLoopTotalCnt = params_.mCnt * params_.nCnt % matmulTilingData_->tCubeTiling.usedCoreNum *
                                  matmulTilingData_->kTailCnt;
    params_.totalCnt = (params_.round - 1) * matmulTilingData_->tCubeTiling.usedCoreNum + aicParams_.lastLoopTotalCnt;

    aicParams_.kBaseTail = matmulTilingData_->tCubeTiling.Ka -
                           (matmulTilingData_->kTailCnt - 1) * aicParams_.blockBaseK;
...
__aicore__ inline void MatmulStreamKBlock::CalcGMOffset(uint64_t roundIdx)
{
    if ASCEND_IS_AIC {
        ...
        aicParams_.offsetCWorkspace = ((params_.index - roundIdx * matmulTilingData_->tCubeTiling.usedCoreNum) *
                                           matmulTilingData_->kTailCnt +
...
__aicore__ inline void MatmulStreamKBlock::CalcAivBaseParams()
{
    if ASCEND_IS_AIV {
        ...
        aivParams_.mBurstBase = MMV3CeilAlign(
            MMV3DivCeil(params_.singleCoreM, matmulTilingData_->kTailCnt * GetTaskRation()),
        ...
        if (aicParams_.kCntIndex >= aivParams_.mBurstCnt) {
            ...
            aivParams_.copyGm2UbMBurstOri = (aicParams_.kCntIndex == aivParams_.mBurstCnt - 1) ? aivParams_.mBurstTail :
...
__aicore__ inline void MatmulStreamKBlock::UpdateAivParams(uint64_t index, uint64_t roundIdx)
{
    ...
    aivParams_.offsetWorkspaceSrc = (params_.index - roundIdx * matmulTilingData_->tCubeTiling.usedCoreNum) *
                                        matmulTilingData_->kTailCnt * BLOCK_BASE_M * BLOCK_BASE_N +
                                    (aicParams_.kCntIndex * aivParams_.mBurstBase +
                                     aivParams_.copyGm2UbMBurst * index) *

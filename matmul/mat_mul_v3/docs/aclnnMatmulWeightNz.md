...
- **参数说明：**
  ...
  - <term>昇腾910_95 AI处理器</term>：
    - 调用此接口之前，必须使用aclnnNpuFormatCast接口完成mat2的原始输入Format从ND到AI处理器亲和数据排布格式的转换。
...
- <term>昇腾910_95 AI处理器</term>：
  self和mat2数据类型为bfloat16，mat2为AI处理器亲和数据排布格式场景下的示例代码如下，仅供参考，具体编译和执行过程请参考[编译与运行样例](../../../docs/zh/context/编译与运行样例.md)。
  ```Cpp
    #include <iostream>
    #include <vector>
    #include "acl/acl.h"
    ...
    int main() {
      ...
      std::vector<uint16_t> dstTensorHostData(k * n, 0x3F80);

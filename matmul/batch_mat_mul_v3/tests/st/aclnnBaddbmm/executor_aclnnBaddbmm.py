...
@register("executor_baddbmm")
class baddbmmApi(BaseApi):
    def __init__(self, task_result: TaskResult):
        super(baddbmmApi, self).__init__(task_result)
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        output = torch.baddbmm(mat_input, batch1, batch2, beta=beta, alpha=alpha, out=out)
        return output

@register("executor_aclnn_baddbmm")
class aclnnBaddbmmApi(AclnnBaseApi):
    def __init__(self, task_result: TaskResult, backend):
        super(aclnnBaddbmmApi, self).__init__(task_result, backend)

...
uint64_t BatchMatMulV3IterBatchBasicApiTiling::GetTilingKey() const
{
    return BatchMatMulV3TilingKey()
        ...
        .SetBatchModel(MatMulV3BatchModel::MULTI_BATCH_MODEL)
...
bool BatchMatMulV3IterBatchBasicApiTiling::IsCapable()
{
    ...
    if ((alignMValue_ * alignKValue_ + alignKValue_ * alignNValue_) * args_.aDtypeSize * DB_SIZE >
        compileInfo_.l1Size) {
        return false;
    }
    ...
    iterBatchL1_ = ops::FloorDiv(compileInfo_.l1Size / DB_SIZE, (alignMValue_ * alignKValue_ +
                                 alignKValue_ * alignNValue_) * args_.aDtypeSize);
...
bool BatchMatMulV3IterBatchBasicApiTiling::IsMat2Transpose(
...
ge::graphStatus BatchMatMulV3IterBatchBasicApiTiling::DoOpTiling()
{
    ...
    // 特殊处理3D非连续场景
    if (context_->InputIsView(1) && IsMat2Transpose(mat2ViewShape)) {
    ...
    OP_LOGI(args_.opName, "In IterBatchBasicApi module, temp iterBatchL0A is %lu, temp iterBatchL0B is %lu, \
            ...
            runInfo_.iterBatchL1 is %lu, runInfo_.baseM is %lu, runInfo_.baseN is %lu,", iterBatchL0A_, iterBatchL0B_,
            iterBatchL0C_, iterBatchL1_, runInfo_.iterBatchL0, runInfo_.iterBatchL1, runInfo_.baseM, runInfo_.baseN);

...
protected:
    static void SetUpTestCase() { cout << "quant_batch_matmul_v4_test SetUp\n" << endl; }
    static void TearDownTestCase() { cout << "quant_batch_matmul_v4_test TearDown\n" << endl; }
};
...
TEST_F(quant_batch_matmul_v4_test, quant_batch_matmul_v4_perblock_0)
{
    ...
    auto quant_batch_matmul_v4_wrapper = [](GM_ADDR x1, GM_ADDR x2, GM_ADDR bias, GM_ADDR x1_scale, GM_ADDR x2_scale,
        ...
        ::quant_batch_matmul_v4<1, 4, 0, 0, 1>(x1, x2, bias, x1_scale, x2_scale, y_scale, x1_offset, x2_offset,
                                               y_offset, x2_table, y, workspace, tiling);
    };

    ICPU_RUN_KF(quant_batch_matmul_v4_wrapper, 6, x1GM, x2GM, biasGM, x1ScaleGM, x2ScaleGM, yScaleGM, x1OffsetGM,
                x2OffsetGM, yOffsetGM, x2TableGM, yGM, workspace, tiling);
...
TEST_F(quant_batch_matmul_v4_test, quant_batch_matmul_v4_msd_0)
{
    ...
    auto quant_batch_matmul_v4_wrapper = [](GM_ADDR x1, GM_ADDR x2, GM_ADDR bias, GM_ADDR x1_scale, GM_ADDR x2_scale,
                                            GM_ADDR y_scale, GM_ADDR x1_offset, GM_ADDR x2_offset, GM_ADDR y_offset,
                                            GM_ADDR x2_table, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling) {
        ::quant_batch_matmul_v4<0, 6, 0, 0, 0>(x1, x2, bias, x1_scale, x2_scale, y_scale, x1_offset, x2_offset,
                                               y_offset, x2_table, y, workspace, tiling);
    };

    ICPU_RUN_KF(quant_batch_matmul_v4_wrapper, 20, x1GM, x2GM, biasGM, x1ScaleGM, x2ScaleGM, yScaleGM, x1OffsetGM,
                x2OffsetGM, yOffsetGM, x2TableGM, yGM, workspace, tiling);

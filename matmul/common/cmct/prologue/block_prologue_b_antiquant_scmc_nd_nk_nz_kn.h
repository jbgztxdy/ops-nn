...
template <typename T, QuantType AntiquantType, bool trans, uint32_t MaxElement, uint32_t UbMte2InnerSize>
struct TensorTraitScaleUb<T, AntiquantType, trans, MaxElement, UbMte2InnerSize,
                          typename AscendC::Std::enable_if_t<AntiquantType == QuantType::PER_CHANNEL ||
                                                             AntiquantType == QuantType::PER_TENSOR>> {
    using Type = AscendC::TensorTrait<T, AscendC::TPosition::VECIN,
                                      AscendC::Layout<Shape<uint64_t, _1>, Stride<_1, uint64_t>>>;
};

template <typename T, QuantType AntiquantType, uint32_t MaxElement, uint32_t UbMte2InnerSize>
struct TensorTraitScaleUb<T, AntiquantType, true, MaxElement, UbMte2InnerSize,
    ...
    using Type = AscendC::TensorTrait<T, AscendC::TPosition::VECIN,
                                      AscendC::Layout<Shape<StrideN, uint64_t>, Stride<_1, StrideN>>>;
};
...
template <typename T, QuantType AntiquantType, uint32_t MaxElement, uint32_t UbMte2InnerSize>
struct TensorTraitScaleUb<T, AntiquantType, false, MaxElement, UbMte2InnerSize,
                          typename AscendC::Std::enable_if_t<AntiquantType == QuantType::MX>> {
    using Type = AscendC::TensorTrait<
        T, AscendC::TPosition::VECIN,
        AscendC::Layout<Shape<uint64_t, uint64_t>,
                        Stride<Int<CeilDiv(static_cast<uint64_t>(UbMte2InnerSize), 32UL)>, _1>>>;
};
...
struct TensorTraitBUbIn<false, TreatAsB8, SrcT, TileSize> {
    // nd nk
    using Type = AscendC::TensorTrait<AscendC::Std::conditional_t<TreatAsB8, int8_t, SrcT>, AscendC::TPosition::VECIN,
...
struct TensorTraitBUbIn<true, TreatAsB8, SrcT, TileSize> {
    ...
    using Type = AscendC::TensorTrait<
        ...
        AscendC::Layout<Shape<Shape<_16, uint64_t>, Shape<_16, Int<CeilDiv(TileSize, 16UL)>>>,
...
struct TensorTraitBUbOut<false, N, K, BufNum, Element> {
    using Type = AscendC::TensorTrait<
        ...
        AscendC::Layout<Shape<Shape<_16, Int<CeilDiv(N, 16UL)>>, Shape<_16, Int<CeilDiv(K, 16UL)>>>,
...
struct TensorTraitBUbOut<true, N, K, BufNum, Element> {
    ...
    using Type = AscendC::TensorTrait<Element, AscendC::TPosition::VECIN,
                                      AscendC::Layout<Shape<Int<N * K / UNIT>, _128>, Stride<Int<BufNum * UNIT>, _1>>>;
};

// nd-kn场景下，gm和L1的生产消费比为1:N
template <typename HighBitType, typename PrologueScaleType, bool WeightNz, uint64_t AivNum, bool HasOffset_,
          uint64_t UbInBufNum, uint64_t InnerSize, uint64_t VfN, uint64_t VfK, uint64_t UbOutBufNum, uint64_t UbInSize,
          uint64_t UbOutSize, uint64_t ScaleSize, uint64_t OffsetSize, uint64_t AntiQuantScaleAfterCastSize,
          class BType_, class ScaleType_, class TileShapeL1_>
class BlockPrologue<
    BAntiquantScmc<DAV3510, HighBitType, PrologueScaleType, true, WeightNz, AivNum, HasOffset_, UbInBufNum, InnerSize,
...
public:
    ...
    using DispatchPolicy = BAntiquantScmc<DAV3510, HighBitType, PrologueScaleType, true, WeightNz, AivNum, HasOffset_,
                                          UbInBufNum, InnerSize, VfN, VfK, UbOutBufNum, UbInSize, UbOutSize, ScaleSize,
                                          OffsetSize, AntiQuantScaleAfterCastSize>;
    ...
    static constexpr uint64_t
        UB_WEIGHT_OUTPUT_HIGH_BIT_BUFFER_NUM = DispatchPolicy::UB_WEIGHT_OUTPUT_HIGH_BIT_BUFFER_NUM;
    ...
    static constexpr uint64_t
        ANTIQUANT_SCALE_AFTER_CAST_UB_SIZE = DispatchPolicy::ANTIQUANT_SCALE_AFTER_CAST_UB_TOTAL_SIZE;
    static constexpr uint64_t
        WEIGHT_INPUT_LOW_BIT_UB_SINGLE_BUFFER_SIZE = DispatchPolicy::WEIGHT_INPUT_LOW_BIT_UB_SINGLE_BUFFER_SIZE;
    static constexpr uint64_t
        ANTIQUANT_SCALE_UB_SINGLE_BUFFER_SIZE = DispatchPolicy::ANTIQUANT_SCALE_UB_SINGLE_BUFFER_SIZE;
    static constexpr uint64_t
        ANTIQUANT_OFFSET_UB_SINGLE_BUFFER_SIZE = DispatchPolicy::ANTIQUANT_OFFSET_UB_SINGLE_BUFFER_SIZE;
    static constexpr uint64_t HIGH_BIT_DATA_UB_SINGLE_BUFFER_SIZE = DispatchPolicy::HIGH_BIT_DATA_UB_SINGLE_BUFFER_SIZE;
    static constexpr uint64_t
        ANTIQUANT_SCALE_AFTER_CAST_UB_TOTAL_SIZE = DispatchPolicy::ANTIQUANT_SCALE_AFTER_CAST_UB_TOTAL_SIZE;
    static constexpr uint64_t ANTIQUANT_SCALE_AFTER_CAST_UB_SINGLE_BUFFER_SIZE = DispatchPolicy::
        ANTIQUANT_SCALE_AFTER_CAST_UB_SINGLE_BUFFER_SIZE;
    struct Arguments {
        GM_ADDR ptrB;
        GM_ADDR ptrScale;
        GM_ADDR ptrOffset;
    ...
    __aicore__ inline BlockPrologue(const Params& params)
    {
        ...
        ubScale_ = CreateTensor<UbScaleTensorTrait>(WEIGHT_INPUT_LOW_BIT_UB_TOTAL_SIZE +
                                                    HIGH_BIT_DATA_UB_TOTAL_SIZE * sizeof(ElementOut));
        if constexpr (ANTIQUANT_SCALE_AFTER_CAST_UB_SIZE == 0) {
            ubOffset_ = CreateTensor<UbScaleTensorTrait>(WEIGHT_INPUT_LOW_BIT_UB_TOTAL_SIZE +
                                                         HIGH_BIT_DATA_UB_TOTAL_SIZE * sizeof(ElementOut) +
    ...
    __aicore__ inline void operator()(const TensorB& gB, const TensorScale& gScale, const TensorOffset& gOffset,
        ...
        for (uint64_t kMte2Offset = 0; kMte2Offset < kSize_; kMte2Offset += UB_MTE2_INNER_SIZE) {
            ...
            for (uint64_t kWeightLowBitUbOffset = 0; kWeightLowBitUbOffset < mte2RealK;
                ...
                if constexpr (ANTIQUANT_TYPE == QuantType::MX) {
                    WeightAntiQuantCompute(weightF16L1_[(cvLoopIdx_ & 1) * weightF16L1DbOffset_], ubBInB8SubL1,
                                           ubScaleAfterCast[offsetSub], ubOffset, l1RealN, l1RequireVfComputeRealK);
                } else {
                    WeightAntiQuantCompute(weightF16L1_[(cvLoopIdx_ & 1) * weightF16L1DbOffset_], ubBInB8SubL1,
                                           ubScale[offsetSub], ubOffset[offsetSub], l1RealN, l1RequireVfComputeRealK);
                }
                SetAivToAic();
            }
            SetVToMTE2();
...
private:
    ...
    __aicore__ inline void WaitAicToAiv() { AscendC::CrossCoreWaitFlag<SYNC_MODE4, PIPE_MTE3>(SYNC_AIV_AIC_FLAG); }

    __aicore__ inline void SetAivToAic() { AscendC::CrossCoreSetFlag<SYNC_MODE4, PIPE_MTE3>(SYNC_AIC_AIV_FLAG); }

    template <class TensorTraitB, class TensorScaleOffset>
    __aicore__ inline void CopyGmToUb(const AscendC::GlobalTensor<TensorTraitB>& gB, const TensorScaleOffset& gScale,
    ...
    __aicore__ inline void CopyAntiquantParamsGmToUb(const TensorScaleOffset& gScale, const TensorScaleOffset& gOffset,
                                                     uint64_t ubMte2NSize, uint64_t ubMte2KSize)
    {
        if constexpr (ANTIQUANT_TYPE == QuantType::PER_CHANNEL) {
            CopyIf<ArchTag>(ubScale_[(ubMte2LoopIdx_ % UB_MTE2_BUF_NUM) * ANTIQUANT_SCALE_UB_SINGLE_BUFFER_SIZE],
                            gScale, AscendC::MakeShape(ubMte2NSize, 1));
            if constexpr (HAS_OFFSET) {
                CopyIf<ArchTag>(ubOffset_[(ubMte2LoopIdx_ % UB_MTE2_BUF_NUM) * ANTIQUANT_OFFSET_UB_SINGLE_BUFFER_SIZE],
                                gOffset, AscendC::MakeShape(ubMte2NSize, 1));
            }
            ...
            CopyIf<ArchTag>(ubScale_[(ubMte2LoopIdx_ % UB_MTE2_BUF_NUM) * ANTIQUANT_SCALE_UB_SINGLE_BUFFER_SIZE],
                            gScale, AscendC::MakeShape(ubMte2NSize, CeilDiv(ubMte2KSize, antiQuantGroupSize_)));
            if constexpr (HAS_OFFSET) {
                CopyIf<ArchTag>(ubOffset_[(ubMte2LoopIdx_ % UB_MTE2_BUF_NUM) * ANTIQUANT_OFFSET_UB_SINGLE_BUFFER_SIZE],
                                gOffset, AscendC::MakeShape(ubMte2NSize, CeilDiv(ubMte2KSize, antiQuantGroupSize_)));
            }
            // MX场景scale需要进行数据类型转化
            if constexpr (ANTIQUANT_TYPE == QuantType::MX) {
                ...
                Prologue::Tile::TileCast<ArchTag>(
                    ubScaleAfterCast_[(ubMte2LoopIdx_ % UB_MTE2_BUF_NUM) *
                                      ANTIQUANT_SCALE_AFTER_CAST_UB_SINGLE_BUFFER_SIZE],
    ...
    __aicore__ inline void WeightAntiQuantCompute(const TensorL1& tensorL1, const TensorInB& ubBInB8,
                                                  const TensorScale& ubScale, const TensorOffset& ubOffset,
                                                  uint64_t bL1NSize, uint64_t bL1KSize)
    {
        ...
        for (uint64_t kOffset = 0; kOffset < bL1KSize; kOffset += VF_TILE_K) {
            for (uint64_t nOffset = 0; nOffset < bL1NSize; nOffset += VF_TILE_N) {
                ...
                auto regBOut = ubBOut_[WEIGHT_NZ ? (ubCalLoopId % UbOutBufNum) * VECTOR_REG_SIZE<ElementOut> :
                ...
                if constexpr (ANTIQUANT_TYPE == QuantType::PER_TENSOR) {
                    Prologue::Tile::Antiquant<ArchTag, PolicyAntiquant>(regBOut, regBIn, scaleValue_, offsetValue_,
                                                                        AscendC::MakeShape(nRealLen, kRealLen));
                } else if constexpr (ANTIQUANT_TYPE == QuantType::PER_CHANNEL) {
                    ...
                    Prologue::Tile::Antiquant<ArchTag, PolicyAntiquant>(regBOut, regBIn, regScale, regOffset,
                                                                        AscendC::MakeShape(nRealLen, kRealLen));
                    ...
                    auto regScale = ubScale[WEIGHT_NZ ? ubScale.GetLayout()(MakeCoord(nOffset, _)) :
                                                        ubScale.GetLayout()(
                                                            MakeCoord(nOffset, CeilDiv(kOffset, antiQuantGroupSize_))) *
                                                            2];
                    Prologue::Tile::Antiquant<ArchTag, PolicyAntiquant>(
                ...
                if constexpr (WEIGHT_NZ) {
                    ...
                    CopyIf<ArchTag>(
                        ...
                        AscendC::MakeShape(CeilAlign(nRealLen, BLK_ELEM<ElementOut>) *
                                               CeilAlign(kRealLen, BLK_ELEM<ElementOut>) / VECTOR_REG_SIZE<ElementOut>,
    ...
    using L1TensorTrait = AscendC::Std::conditional_t<
        ...
        AscendC::TensorTrait<ElementOut, AscendC::TPosition::B1,
                             AscendC::Layout<Shape<uint64_t, Int<VECTOR_REG_WIDTH / sizeof(ElementOut)>>,
    ...
    using UbInB8TensorTrait = typename TensorTraitBUbIn<IsZn2D<LayoutIn>::value, true, ElementIn,
                                                        UB_MTE2_INNER_SIZE>::Type;
    using UbInTensorTrait = typename TensorTraitBUbIn<IsZn2D<LayoutIn>::value, false, ElementIn,
                                                      UB_MTE2_INNER_SIZE>::Type;
    ...
    using UbOutTensorTrait = typename TensorTraitBUbOut<IsZn2D<LayoutIn>::value, VF_TILE_N, VF_TILE_K,
                                                        UB_WEIGHT_OUTPUT_HIGH_BIT_BUFFER_NUM, ElementOut>::Type;
    ...
    static constexpr uint32_t MaxUbTileN = AscendC::Std::conditional_t < WEIGHT_NZ &&
                                           ANTIQUANT_TYPE == QuantType::PER_GROUP,
                              typename AscendC::Std::tuple_element<0, typename AscendC::Std::remove_cvref_t<decltype(
                                                                          UbScaleTensorTrait{}.GetShape())>>::type,
                              _0 > ::value;
    using PolicyAntiquant = AscendC::Std::conditional_t<
        WEIGHT_NZ,
        AscendC::Std::conditional_t<
            ANTIQUANT_TYPE == QuantType::PER_GROUP,
            Prologue::Tile::AntiquantFixTilePrivate<MaxUbTileN, UB_MTE2_INNER_SIZE,
                                                    UB_WEIGHT_OUTPUT_HIGH_BIT_BUFFER_NUM, HAS_OFFSET>,
            Prologue::Tile::AntiquantFixTilePrivate<VF_TILE_N, UB_MTE2_INNER_SIZE, UB_WEIGHT_OUTPUT_HIGH_BIT_BUFFER_NUM,
                                                    HAS_OFFSET>>,

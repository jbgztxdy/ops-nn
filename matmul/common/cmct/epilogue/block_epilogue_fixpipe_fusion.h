...
public:
    ...
    __aicore__ inline void Run(BlockShape const& blockShape, int64_t dstOffset, bool splitM)
    {
        ...
        int64_t blockShapeNAlign = static_cast<int64_t>(Cmct::Gemm::Align(
            static_cast<uint64_t>(blockShapeN), static_cast<uint64_t>(AscendC::AuxGetC0Size<DataTypeOut>())));
        ...
        ASCENDC_ASSERT(stageRows > 0, {
            KERNEL_LOG(KERNEL_EORROR, "Unsupport fixpipe fusion UB size %ld, %ld, %ld!", ubTotalElems - ubBaseOffset_,
                       resultCount, blockShapeNAlign);
        });
        if (stageRows <= 0) {
            return;
        }
    ...
    __aicore__ inline void ProcessFusionStages(int64_t blockShapeM, int64_t blockShapeN, int64_t blockShapeNAlign,
                                               int64_t N, int64_t offset, int64_t resultCount, int64_t stageRows)
    {
        ...
        uint32_t x3DstStride = static_cast<uint32_t>((blockShapeNAlign - blockShapeN) * sizeof(DataTypeIn) /
                                                     UB_ALIGN_SIZE);
        uint32_t outSrcStride = static_cast<uint32_t>((blockShapeNAlign - blockShapeN) * sizeof(DataTypeOut) /
                                                      UB_ALIGN_SIZE);
        ...
        for (int64_t rowOffset = 0; rowOffset < blockShapeM; rowOffset += stageRows) {
            ...
            AscendC::DataCopyExtParams x3CopyParams{
                static_cast<uint16_t>(curRows), static_cast<uint32_t>(blockShapeN * sizeof(DataTypeIn)),
                static_cast<uint32_t>((N - blockShapeN) * sizeof(DataTypeIn)), x3DstStride, 0};
            ...
            AscendC::DataCopyExtParams outCopyParams{
                static_cast<uint16_t>(curRows), static_cast<uint32_t>(blockShapeN * sizeof(DataTypeOut)), outSrcStride,

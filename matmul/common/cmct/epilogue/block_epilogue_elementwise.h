...
public:
    ...
    __aicore__ inline void RunFromWorkspace(BlockShape const& blockShape, int64_t dstOffset,
                                            ...
                                            int64_t flagId = 5)
    {
        ...
        while (stageOffset < inputSize) {
            ...
            AscendC::DataCopyExtParams gm2UbParams{
                static_cast<uint16_t>(1), static_cast<uint32_t>(stageRows * blockShapeN * sizeof(DataTypeIn)), 0, 0, 0};
            AscendC::DataCopyPadExtParams<DataTypeIn> padParams{false, 0, 0, 0};
            AscendC::DataCopyPad<DataTypeIn>(cLocal_[stageOffset], workspaceGlobal[workspaceGmOffset], gm2UbParams,
                                             padParams);
            ...
            AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(ZERO_FLAG);
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(ZERO_FLAG);
            AscendC::DataCopyExtParams ub2GmParams{static_cast<uint16_t>(1),
                                                   static_cast<uint32_t>(stageRows * blockShapeN * sizeof(DataTypeOut)),
                                                   0, 0, 0};
            AscendC::DataCopyPad<DataTypeOut>(outputGlobal_[offset], outputLocal, ub2GmParams);
            stageOffset += stageSize;
            loop++;
        }
        AscendC::CrossCoreSetFlag<AIC_SYNC_AIV_MODE_4, PIPE_MTE3>(flagId);

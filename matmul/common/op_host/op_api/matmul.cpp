...
static const aclTensor* MatMulV3Common(const aclTensor* x1, const aclTensor* x2, const aclTensor* bias,
                                       ...
                                       const op::Format output_ori_format, const bool transposeX1,
...
static const aclTensor* MatMulCommon(const aclTensor* x1, const aclTensor* x2, const aclTensor* bias,
                                     const aclTensor* offsetW, const op::DataType output_dtype,
                            ...
                            aclOpExecutor* executor)
{
    ...
    return MatMulV3Common(x1, x2, bias, x1->GetDataType(), Format::FORMAT_ND, Format::FORMAT_ND, transposeX1,
                          transposeX2, offsetX, opImplMode, executor);
};
                                     ...
                                     const int64_t opImplMode, aclOpExecutor* executor)
{
    ...
    return MatMulV3Common(x1, x2, bias, DataType::DT_FLOAT, Format::FORMAT_ND, Format::FORMAT_ND, transposeX1,
                          transposeX2, offsetX, opImplMode, executor);
};
                          ...
                          bool transposeX2, bool enableHf32, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(GemmV3, OP_INPUT(x1, x2, c), OP_OUTPUT(gemmOut),
                                           OP_ATTR(1.0f, 1.0f, transposeX1, transposeX2, enableHf32),
                                           OP_MODE(execMode));
...
const aclTensor* GemmV3NdNzWithAlphaBeta(const aclTensor* x1, const aclTensor* x2, const aclTensor* self, float alpha,
                                         ...
                                         aclOpExecutor* executor, bool enable16In32Out)
{
    ...
    auto ret = INFER_SHAPE(GemmV3, OP_INPUT(x1, x2, self), OP_OUTPUT(out),
                           OP_ATTR(alpha, beta, transposeX1, transposeX2, enableHf32));
    if (ret != ACLNN_SUCCESS) {
        OP_LOGE(ACLNN_ERR_INNER_INFERSHAPE_ERROR, "InferShape failed");
        return nullptr;
    }
    ret = ADD_TO_LAUNCHER_LIST_AICORE(GemmV3, OP_INPUT(x1, x2, self), OP_OUTPUT(out),
                                      OP_ATTR(alpha, beta, transposeX1, transposeX2, enableHf32));
    OP_CHECK_ADD_TO_LAUNCHER_LIST_AICORE(ret != ACLNN_SUCCESS, return nullptr, "Add to launcher list aicore failed.");
    return out;
}

...
static ge::graphStatus HighPerfTilingFunc(gert::TilingContext* context)
{
    ...
    if (mm1TailM == 0)
        mm1TailM = mm1Tiling.GetBaseM();
    if (mm1TailN == 0)
        mm1TailN = mm1Tiling.GetBaseN();
    ...
    if (mm2TailM == 0)
        mm2TailM = mm2Tiling.GetBaseM();
    if (mm2TailN == 0)
        mm2TailN = mm2Tiling.GetBaseN();
    if (mm2TailK == 0)
        mm2TailK = mm2Tiling.GetBaseK();
    ...
    btLastCore = BT %
                 btPerCore; // 最后一个核心的任务数。如果BT_LAST_CORE为0，则无跑而未满核心，可作为LAST_CORE是否启动条件
    ...
    btPerEpochSingle = ((ubByteSize - TBUF_BYTE_SIZE) / BUFFER_NUM) /
                       (V64BAlignedSize * IN_BYTE_SIZE); // 每轮单核处理的行数
    if (btPerEpochSingle) {
        // full case
        ...
        // partial case
        using32BlockCnt = ((ubByteSize - TBUF_BYTE_SIZE) / BUFFER_NUM) / SIZE_32 / SIZE_2 *
                          SIZE_2;                               // 将使用的UB空间块数（偶数个32B块数）
    ...
    quebindSoftmaxByteSize = std::max(quebindSoftmaxByteSize,
                                      MAX_LEFT_BASE_BYTE_SIZE * 2); // 同时需要保证mm2左矩阵nd2nz必需的空间
    ...
    userWorkspaceSize = std::max(VOut32BAlignedSize, BT_16ALIGNED) * baseM *
                        wsNum; // 用于存放左矩阵nd2nz的结果，多块用以保证vec在处理时没有cube空闲

    // validation
    // bt
    if (BT_16ALIGNED - mm2TailK > SIZE_65535) {
        OP_LOGE(context->GetNodeName(),
                "[FusedLinearCrossEntropyLossGrad] softmax.size(0) greater than 65536 is not supported now.");
        return ge::GRAPH_FAILED;
    };
    // v
    if (V * IN_BYTE_SIZE < SIZE_512) {
        OP_LOGE(context->GetNodeName(),
                "[FusedLinearCrossEntropyLossGrad] bytesize of softmax.size(1) smaller than 512B is not supported.");
        return ge::GRAPH_FAILED;
    };
    if (V > (uint64_t)((SIZE_256 - 1) * SIZE_256)) { // VOut512BAlignedByteSize < 65536
        OP_LOGE(context->GetNodeName(),
                "[FusedLinearCrossEntropyLossGrad] softmax.size(1) greater than 65280 is not supported now.");
        return ge::GRAPH_FAILED;
    };
    // h
    if (H > SIZE_65535) {
        OP_LOGE(context->GetNodeName(),
                "[FusedLinearCrossEntropyLossGrad] input.size(1) greater than 65535 is not supported now.");
        return ge::GRAPH_FAILED;
    };
    // others
    if (btPerEpochSingle == 0U) {
        OP_LOGE(context->GetNodeName(),
                "[FusedLinearCrossEntropyLossGrad] softmax.size(1) is too large and is not supported now.");
        return ge::GRAPH_FAILED;
    }
...
static void CalcUBUsage(uint32_t ubByteSize, uint32_t H, uint32_t& mmAccumRowOnetime, uint32_t& mmAccumCyclePerRow,
                        uint32_t& mmAccumCntPerCycle, uint32_t& mmAccumCntLastCycle)
{
    ...
    mmAccumRowOnetime = static_cast<uint32_t>(std::max<uint64_t>(1ULL, ubByteSize / rowBytes));
    ...
    // 需要的循环次数（向上取整）
    mmAccumCyclePerRow = static_cast<uint32_t>((rowBytes + bytesPerCycle - 1ULL) / bytesPerCycle);
...
static ge::graphStatus MemFriendlyTilingFunc(gert::TilingContext* context)
{
    ...
    uint32_t targetMaskSize = static_cast<uint32_t>(
        context->GetInputTensor(INDEX_3)->GetShapeSize()); // target_mask的形式长度
    ...
    // V2 UB，按H大小区分情形
    uint32_t targetByteSize = ubByteSize / SIZE_32 / BUFFER_NUM / SIZE_2 *
                              SIZE_32; // 32B对齐、double buffer与两个加数空间预留
    ...
    poolByteSize = std::max(
        targetByteSize * BUFFER_NUM,
        (V1MainSize + baseV) *
            IN_BYTE_SIZE); // V1始终一次性搬运，没有db，且元素量为256倍数。V2采用了最大设计量，此处应由V2决定
    ...
    userWorkspaceByteSize += static_cast<uint64_t>(BT) * H * IN_BYTE_SIZE;
    userWorkspaceByteSize += static_cast<uint64_t>(V) * H * IN_BYTE_SIZE;
    size_t sysWorkspaceByteSize = static_cast<size_t>(ascendcPlatform.GetLibApiWorkSpaceSize());
    size_t* currentWorkspace = context->GetWorkspaceSizes(1);
    currentWorkspace[0] = userWorkspaceByteSize + sysWorkspaceByteSize;
    OP_LOGD(context->GetNodeName(), "[FusedLinearCrossEntropyLossGrad] userWorkspaceByteSize=%lld totalWorkspace=%lld.",
...
static ge::graphStatus FusedLinearCrossEntropyLossGradTilingParseFunc(

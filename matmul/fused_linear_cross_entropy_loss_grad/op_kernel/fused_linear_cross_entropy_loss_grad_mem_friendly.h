...
public:
    __aicore__ inline SlidingWindowBlockIterator() : cacheSize_(0), cacheStartIdx_(0), cacheEndIdx_(0){};

    __aicore__ inline void init(uint32_t BT, uint32_t V, uint32_t baseBT, uint32_t baseV, uint32_t baseBTPerWindow,
                                uint32_t baseVPerWindow)
    {
        BT_ = BT;
        V_ = V;
    ...
    // 判断是否还有更多分块（原有代码）
    __aicore__ inline bool hasNext() const { return !finished_; }

    // 获取当前分块计数（原有代码）
    __aicore__ inline uint32_t getCurrentTaskCount() const { return currentTaskCnt_; }
...
template <typename HalfType, typename TargetMaskType, typename MaskedTargetType, typename MM0, typename MM1,
          typename MM2>
class FusedLinearCrossEntropyLossGradMemFriendly {
public:
    __aicore__ inline FusedLinearCrossEntropyLossGradMemFriendly() {}
    __aicore__ inline void Init(GM_ADDR grad, GM_ADDR input, GM_ADDR weight, GM_ADDR target_mask, GM_ADDR masked_target,
                                GM_ADDR logits_max, GM_ADDR sum_exp_logits, GM_ADDR input_grad_out,
                                GM_ADDR weight_grad_out, GM_ADDR workspace,
                                const __gm__ FusedLinearCrossEntropyLossGradMemFriendlyTilingData* tiling_data_ptr)
    {
        tdp = tiling_data_ptr;
        if ASCEND_IS_AIV {
        ...
        mm0ResultWS[1].SetGlobalBuffer(
            (__gm__ float*)workspace + tdp->mm0ResultSizePerEpoch + aicBlockIdx * tdp->mm0ResultSize,
            tdp->mm0ResultSize);
        ...
        for (uint8_t wsIdx = 0; wsIdx < SIZE_2; wsIdx++) {
            for (uint32_t idx = 0; idx < tdp->aicNum; idx++) {
                mm1ResultWS[wsIdx][idx].SetGlobalBuffer(
                    (__gm__ float*)workspace + allocatedWSSize + idx * tdp->mm1ResultSize, tdp->mm1ResultSize);
            }
            allocatedWSSize += tdp->mm1ResultSizePerEpoch;
        }
        for (uint8_t wsIdx = 0; wsIdx < SIZE_2; wsIdx++) {
            for (uint32_t idx = 0; idx < tdp->aicNum; idx++) {
                mm2ResultWS[wsIdx][idx].SetGlobalBuffer(
                    (__gm__ float*)workspace + allocatedWSSize + idx * tdp->mm2ResultSize, tdp->mm2ResultSize);
            }
    ...
    __aicore__ inline void InitSyncCube() { evtIDFixToMTE2 = GetTPipePtr()->AllocEventID<HardEvent::FIX_MTE2>(); }

    __aicore__ inline void CloseSyncCube() { GetTPipePtr()->ReleaseEventID<HardEvent::FIX_MTE2>(evtIDFixToMTE2); }
    ...
    */
    ...
    __aicore__ inline void CopyInV1()
    {
        ...
        DataCopyExtParams copyParams{V1MainRowCnt, (uint32_t)(V1MainColCnt * IN_BYTE_SIZE), 0, V1InUBStride, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(mainUB, mm0ResultWS[WSIdx][(isVec0) ? 0 : (lastV1MainRowCnt * V1MainColCnt)], copyParams,
                    padParams);
    ...
    {
        uint32_t mainStartRow = baseTaskRowIdx * tdp->baseBT +
                                ((isVec0) ? 0 : lastV1MainRowCnt); // 当前处理数据的起始行号
    ...
    __aicore__ inline void ExpV1(LocalTensor<float>& mainUB) { Exp(mainUB, mainUB, (int32_t)tdp->V1MainSize); }
    ...
    __aicore__ inline void CopyOutV1()
    {
        ...
        auto mainHalfUB = mainUB.ReinterpretCast<HalfType>();
        DataCopyExtParams copyParams{V1MainRowCnt, (uint32_t)(V1MainColCnt * OUT_BYTE_SIZE), V1OutUBStride, 0, 0};
        DataCopyPad(mm0ResultHalfWS[WSIdx][(isVec0) ? 0 : (lastV1MainRowCnt * V1MainColCnt)], mainHalfUB, copyParams);
    ...
    */
    ...
    __aicore__ inline void ProcessV2MM1()
    {
        ...
        for (uint32_t taskIdxOffset = aicBlockIdx; taskIdxOffset < tdp->aicNum; taskIdxOffset++) {
            ...
            uint32_t currRowIdx, currColIdx;
            iterator2.getAtIndex(currIdx, currRowIdx, currColIdx);
            if (currRowIdx != baseTaskRowIdx or currIdx >= baseTaskCnt)
                continue;
    ...
    __aicore__ inline void DoV2MM1Group(uint32_t rowOffset, uint32_t rowCnt)
    {
        ...
        if (tdp->mmAccumRowOnetime >= SIZE_2 or tdp->mmAccumCyclePerRow == 1) {
            ...
            for (uint32_t cycleIdx = 0; cycleIdx < tdp->mmAccumCyclePerRow; cycleIdx++) {
                // 把目标ws的结果读取到UB
                uint32_t accumCnt = (cycleIdx == tdp->mmAccumCyclePerRow - 1) ? tdp->mmAccumCntLastCycle :
                                                                                tdp->mmAccumCntPerCycle;
    ...
    __aicore__ inline void CopyInTargetV2MM1(uint32_t rowOffset, uint32_t rowCnt)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(targetUB, mm1AccumWS[(uint64_t)baseTaskRowIdx * tdp->baseBT * tdp->H + rowOffset * tdp->H],
    ...
    {
        auto targetUB = V2TargetQ.AllocTensor<float>();
        DataCopyExtParams copyParams{1, (uint32_t)(accumCnt * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(targetUB,
                    mm1AccumWS[(uint64_t)baseTaskRowIdx * tdp->baseBT * tdp->H + rowOffset * tdp->H +
                               cycleIdx * tdp->mmAccumCntPerCycle],
                    copyParams, padParams);
        V2TargetQ.EnQue<TPosition::GM, TPosition::VECIN, float>(targetUB);
    }
    ...
    __aicore__ inline void CopyInAdditionV2MM1(uint32_t rowOffset, uint32_t rowCnt, uint32_t taskIdxOffset)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(additionUB, mm1ResultWS[WSIdx][taskIdxOffset][(uint64_t)rowOffset * tdp->H], copyParams, padParams);
        V2AdditionQ.EnQue(additionUB);
    }

    __aicore__ inline void CopyInAdditionV2MM1Large(uint32_t rowOffset, uint32_t cycleIdx, uint32_t taskIdxOffset,
                                                    uint32_t accumCnt)
    {
        auto additionUB = V2AdditionQ.AllocTensor<float>();
        DataCopyExtParams copyParams{1, (uint32_t)(accumCnt * IN_BYTE_SIZE), 0, 0, 0};
    ...
    __aicore__ inline void CopyOutV2MM1(uint32_t rowOffset, uint32_t rowCnt)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(mm1AccumWS[(uint64_t)baseTaskRowIdx * tdp->baseBT * tdp->H + rowOffset * tdp->H], targetUB,
                    copyParams);
    ...
    __aicore__ inline void CopyOutV2MM1Large(uint32_t rowOffset, uint32_t cycleIdx, uint32_t accumCnt)
    {
        auto targetUB = V2TargetQ.DeQue<TPosition::VECOUT, TPosition::GM, float>();
        DataCopyExtParams copyParams{1, (uint32_t)(accumCnt * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(mm1AccumWS[(uint64_t)baseTaskRowIdx * tdp->baseBT * tdp->H + rowOffset * tdp->H +
                               cycleIdx * tdp->mmAccumCntPerCycle],
                    targetUB, copyParams);
        V2TargetQ.FreeTensor(targetUB);
    }
    ...
    __aicore__ inline void ProcessV2MM2()
    {
        ...
        for (uint32_t taskIdxOffset = aicBlockIdx; taskIdxOffset < tdp->aicNum; taskIdxOffset++) {
            ...
            uint32_t currRowIdx, currColIdx;
            iterator2.getAtIndex(currIdx, currRowIdx, currColIdx);
            if (currColIdx != baseTaskColIdx or currIdx >= baseTaskCnt)
                continue;
            grpTaskIdxOffset[grpTaskCnt] = taskIdxOffset;
            grpTaskCnt += 1;
        }
    ...
    __aicore__ inline void CopyInTargetV2MM2(uint32_t rowOffset, uint32_t rowCnt)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(targetUB, mm2AccumWS[(uint64_t)baseTaskColIdx * tdp->baseV * tdp->H + rowOffset * tdp->H],
                    copyParams, padParams);
        V2TargetQ.EnQue<TPosition::GM, TPosition::VECIN, float>(targetUB);
    }
    ...
    __aicore__ inline void CopyInAdditionV2MM2(uint32_t rowOffset, uint32_t rowCnt, uint32_t taskIdxOffset)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(additionUB, mm2ResultWS[WSIdx][taskIdxOffset][(uint64_t)rowOffset * tdp->H], copyParams, padParams);
        V2AdditionQ.EnQue(additionUB);
    }

    __aicore__ inline void CopyInAdditionV2MM2Large(uint32_t rowOffset, uint32_t cycleIdx, uint32_t taskIdxOffset,
                                                    uint32_t accumCnt)
    {
        auto additionUB = V2AdditionQ.AllocTensor<float>();
        DataCopyExtParams copyParams{1, (uint32_t)(accumCnt * IN_BYTE_SIZE), 0, 0, 0};
    ...
    __aicore__ inline void CopyOutV2MM2(uint32_t rowOffset, uint32_t rowCnt)
    {
        auto targetUB = V2TargetQ.DeQue<TPosition::VECOUT, TPosition::GM, float>();
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(mm2AccumWS[(uint64_t)baseTaskColIdx * tdp->baseV * tdp->H + rowOffset * tdp->H], targetUB,
                    copyParams);
        V2TargetQ.FreeTensor(targetUB);
    }
    ...
    __aicore__ inline void CopyOutV2MM2Large(uint32_t rowOffset, uint32_t cycleIdx, uint32_t accumCnt)
    {
        auto targetUB = V2TargetQ.DeQue<TPosition::VECOUT, TPosition::GM, float>();
        DataCopyExtParams copyParams{1, (uint32_t)(accumCnt * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(mm2AccumWS[(uint64_t)baseTaskColIdx * tdp->baseV * tdp->H + rowOffset * tdp->H +
                               cycleIdx * tdp->mmAccumCntPerCycle],
                    targetUB, copyParams);
        V2TargetQ.FreeTensor(targetUB);
    }
        ...
        for (uint32_t rowOffset = 0; rowOffset < rowCurrVec; rowOffset += tdp->mmCastRowOnetime) {
            ...
            if (tdp->mmCastRowOnetime >= SIZE_2 or tdp->mmCastCyclePerRow == 1) {
                ...
                for (uint32_t cycleIdx = 0; cycleIdx < tdp->mmCastCyclePerRow; cycleIdx++) {
                    uint32_t castCnt = (cycleIdx == tdp->mmCastCyclePerRow - 1) ? tdp->mmCastCntLastCycle :
                                                                                  tdp->mmCastCntPerCycle;
    ...
    __aicore__ inline void CopyInFinalCastMM1(uint32_t rowIdx, uint32_t rowCnt)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(castUB, mm1AccumWS[(uint64_t)rowIdx * tdp->H], copyParams, padParams);
    ...
    {
        auto castUB = castMainQ.AllocTensor<float>();
        DataCopyExtParams copyParams{1, (uint32_t)(castCnt * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(castUB, mm1AccumWS[(uint64_t)rowIdx * tdp->H + cycleIdx * tdp->mmCastCntPerCycle], copyParams,
                    padParams);
        castMainQ.EnQue<TPosition::GM, TPosition::VECIN, float>(castUB);
    }
    ...
    __aicore__ inline void CopyOutFinalCastMM1(uint32_t rowIdx, uint32_t rowCnt)
    {
        ...
        auto castHalfUB = castUB.ReinterpretCast<HalfType>();
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * OUT_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(inputGradOutGM[(uint64_t)rowIdx * tdp->H], castHalfUB, copyParams);
    ...
    __aicore__ inline void CopyOutFinalCastMM1Large(uint32_t rowIdx, uint32_t cycleIdx, uint32_t castCnt)
    {
        auto castUB = castMainQ.DeQue<TPosition::VECOUT, TPosition::GM, float>();
        auto castHalfUB = castUB.ReinterpretCast<HalfType>();
        DataCopyExtParams copyParams{1, (uint32_t)(castCnt * OUT_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(inputGradOutGM[(uint64_t)rowIdx * tdp->H + cycleIdx * tdp->mmCastCntPerCycle], castHalfUB,
                    copyParams);
        castMainQ.FreeTensor(castUB);
    }
    ...
    __aicore__ inline void CopyInFinalCastMM2(uint32_t rowIdx, uint32_t rowCnt)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(castUB, mm2AccumWS[(uint64_t)rowIdx * tdp->H], copyParams, padParams);
        castMainQ.EnQue<TPosition::GM, TPosition::VECIN, float>(castUB);
    }
    ...
    __aicore__ inline void CopyInFinalCastMM2Large(uint32_t rowIdx, uint32_t cycleIdx, uint32_t castCnt)
    {
        ...
        DataCopyExtParams copyParams{1, (uint32_t)(castCnt * IN_BYTE_SIZE), 0, 0, 0};
        DataCopyPadExtParams<float> padParams{false, 0, 0, 0};
        DataCopyPad(castUB, mm2AccumWS[(uint64_t)rowIdx * tdp->H + cycleIdx * tdp->mmCastCntPerCycle], copyParams,
                    padParams);
        castMainQ.EnQue<TPosition::GM, TPosition::VECIN, float>(castUB);
    }
    ...
    __aicore__ inline void CopyOutFinalCastMM2(uint32_t rowIdx, uint32_t rowCnt)
    {
        ...
        auto castHalfUB = castUB.ReinterpretCast<HalfType>();
        DataCopyExtParams copyParams{1, (uint32_t)(rowCnt * tdp->H * OUT_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(weightGradOutGM[(uint64_t)rowIdx * tdp->H], castHalfUB, copyParams);
        castMainQ.FreeTensor(castUB);
    }
    ...
    __aicore__ inline void CopyOutFinalCastMM2Large(uint32_t rowIdx, uint32_t cycleIdx, uint32_t castCnt)
    {
        ...
        auto castHalfUB = castUB.ReinterpretCast<HalfType>();
        DataCopyExtParams copyParams{1, (uint32_t)(castCnt * OUT_BYTE_SIZE), 0, 0, 0};
        DataCopyPad(weightGradOutGM[(uint64_t)rowIdx * tdp->H + cycleIdx * tdp->mmCastCntPerCycle], castHalfUB,
                    copyParams);
        castMainQ.FreeTensor(castUB);
    }

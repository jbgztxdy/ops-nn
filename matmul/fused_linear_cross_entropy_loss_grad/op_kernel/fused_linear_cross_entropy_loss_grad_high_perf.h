...
public:
    __aicore__ inline FusedLinearCrossEntropyLossGradHighPerf(
        const FusedLinearCrossEntropyLossGradHighPerfTilingData& tilingData)
        : tilingData(tilingData)
    {}
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR weight, GM_ADDR softmax, GM_ADDR target_mask,
                                GM_ADDR masked_target, GM_ADDR grad_output, GM_ADDR grad_input, GM_ADDR grad_weight,
                                GM_ADDR workspace)
    {
        ...
        if ASCEND_IS_AIV {
            ...
            if (btThisCore) {
                // softmax update
                softmaxForUpdateGM.SetGlobalBuffer((__gm__ float*)softmax + tilingData.V * btOffset,
                                                   tilingData.V * btThisCore);
                targetMaskForUpdateGM.SetGlobalBuffer(
                    (__gm__ TargetMaskType*)target_mask,
                    tilingData.targetMaskSize); // false就将-1，true就pass。取全长，便于uint8时偏移计算
                maskedTarget1dForUpdateGM.SetGlobalBuffer((__gm__ MaskedTargetType*)masked_target + btOffset,
                                                          btThisCore); // 列索引
...
private:
    ...
    __aicore__ inline void ProcessUpdate()
    {
        ...
        for (uint64_t i = 0; i < btThisCore; i++) {
            ...
            if (!targetMask) {
                ...
                DataCopyExtParams copyParams{1, IN_BYTE_SIZE, 0, 0, 0};
    ...
    __aicore__ inline void LaunchMM1(uint32_t baseMIdx, uint8_t numNzGM)
    {
        ...
        auto gmC = gradInputGM[baseMIdx * tilingData.mm1Tiling.baseM * tilingData.H +
                               baseNIdx * tilingData.mm1Tiling.baseN];
        int singleCoreM = (baseMIdx == tilingData.mm1BaseMCnt - 1) ? tilingData.mm1TailM :
                                                                     tilingData.mm1Tiling.singleCoreM;
        int singleCoreN = (baseNIdx == tilingData.mm1BaseNCnt - 1) ? tilingData.mm1TailN :
                                                                     tilingData.mm1Tiling.singleCoreN;
    ...
    __aicore__ inline void WaitMM1() { mm1.End(); }
    ...
    __aicore__ inline void LaunchMM2(uint32_t baseMIdx, uint8_t numNzGM)
    {
        ...
        auto gmC = gradWeightGM[baseMIdx * tilingData.mm2Tiling.baseM * tilingData.H +
                                baseNIdx * tilingData.mm2Tiling.baseN];
        mm2.SetTensorA(gmA, true);
        mm2.SetTensorB(gmB, false);
        int singleCoreM = (baseMIdx == tilingData.mm2BaseMCnt - 1) ? tilingData.mm2TailM :
                                                                     tilingData.mm2Tiling.singleCoreM;
        int singleCoreN = (baseNIdx == tilingData.mm2BaseNCnt - 1) ? tilingData.mm2TailN :
                                                                     tilingData.mm2Tiling.singleCoreN;
    ...
    __aicore__ inline void ProcessVecFull()
    {
        ...
        if (C0ZeroSize) {
            C0TailOffset = (C0BlockCnt - 1) * tilingData.mm1Tiling.baseM * OUT_SIZE_PER_32B +
                           (OUT_SIZE_PER_32B - C0ZeroSize);
        }
        ...
        for (uint32_t baseMIdx = 0; baseMIdx < tilingData.mm1BaseMCnt; baseMIdx++) {
            ...
            for (uint32_t epoch = 0; epoch < epochCnt; epoch++) {
                uint32_t singleARowOffset = epoch * tilingData.btPerEpochTotal +
                                            epochRowOffset; // 即将处理的起始行在当前singleA内的偏移
    ...
    __aicore__ inline void CopyInVecFull(uint32_t startRow, uint16_t rowCnt)
    {
        ...
        DataCopyExtParams copyParams{rowCnt, (uint32_t)(tilingData.V * IN_BYTE_SIZE), 0, tilingData.copyInDstStride, 0};
    ...
    __aicore__ inline void MulVecFull(uint32_t startRow, uint32_t rowCnt, LocalTensor<float>& softmaxUB)
    {
        for (uint32_t i = 0; i < rowCnt; i++) {
            ...
            Muls(softmaxUB[i * tilingData.V64BAlignedSize], softmaxUB[i * tilingData.V64BAlignedSize], gradOutput,
                 tilingData.V);
        }
    }

    __aicore__ inline void CastVecFull(uint32_t rowCnt, LocalTensor<float>& softmaxUB,
                                       LocalTensor<HalfType>& softmaxHalfUB)
    {
        Cast(softmaxHalfUB, softmaxUB, RoundMode::CAST_RINT, rowCnt * tilingData.V64BAlignedSize);
    }

    __aicore__ inline void CopyOutVecFull(uint32_t startRow, uint32_t singleARowOffset, uint16_t rowCnt, uint32_t realM,
                                          uint8_t numNzGM)
    {
    ...
    __aicore__ inline void ProcessMM2Nd2nzFull(uint32_t baseMIdx, uint32_t baseKIdx, uint8_t numNzGM,
                                               uint32_t mm2BaseCurrent)
    {
        // 处理1个基本块的nd2nz
        GlobalTensor<HalfType>& A1NZGM = A1NZGMs[numNzGM];
        uint64_t gmInOffset = baseKIdx * tilingData.mm2Tiling.baseK * tilingData.VOut512BAlignedSize +
                              baseMIdx * tilingData.mm2Tiling.baseM;
        uint64_t wsOutOffset = OUT_SIZE_PER_32B * tilingData.mm2Tiling.baseK * baseKIdx;
        uint16_t realK = (baseKIdx + mm2BaseCurrent != tilingData.mm2BaseKCnt) ?
                             tilingData.mm2Tiling.baseK :
                             tilingData.mm2TailK; // 本次实际处理行数
    ...
    __aicore__ inline void CopyInMM2Nd2nzFull(uint64_t gmInOffset, uint16_t realK)
    {
        ...
        Nd2NzParams nd2NzParams{(uint16_t)1,
                                realK,
    ...
    __aicore__ inline void CopyOutMM2Nd2nzFull(uint64_t wsOutOffset, GlobalTensor<HalfType>& A1NZGM, uint16_t realK)
    {
        ...
        DataCopyParams repeatParams{C0BlockCntPerBaseM, realK, 0, (uint16_t)(tilingData.BT_16ALIGNED - realK)};

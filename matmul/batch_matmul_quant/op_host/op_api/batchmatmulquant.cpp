                                  ...
                                  const aclTensor* bias, const bool adjX1, const bool adjX2, aclOpExecutor* executor)
{
    ...
    auto ret = INFER_SHAPE(BatchMatmulFixpipe, OP_INPUT(x1, x2, quantParam, bias), OP_OUTPUT(out),
                           OP_ATTR(adjX1, adjX2));
    OP_CHECK_INFERSHAPE(ret != ACLNN_SUCCESS, return nullptr, "BatchMatmulQuant InferShape failed.");
    ret = ADD_TO_LAUNCHER_LIST_AICORE(BatchMatmulFixpipe, OP_INPUT(x1, x2, quantParam, bias), OP_OUTPUT(out),
                                      OP_ATTR(adjX1, adjX2));
    OP_CHECK_ADD_TO_LAUNCHER_LIST_AICORE(ret != ACLNN_SUCCESS, return nullptr,
                                         "BatchMatmulQuant ADD_TO_LAUNCHER_LIST_AICORE failed.");
    return out;
};
} // namespace l0op
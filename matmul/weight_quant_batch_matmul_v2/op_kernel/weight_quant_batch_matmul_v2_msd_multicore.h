...
__aicore__ inline void CheckDataCopyNd2nzParams(uint32_t nValue, uint32_t dValue, uint32_t srcDValue,
                                                uint32_t dstNzC0Stride)
{
    ...
    ascendc_assert(dstNzC0Stride <= DST_NZ_C0_STRIDE_MAX, "dstNzC0Stride should <= 16384");
}
...
__aicore__ inline void PreprocessKernel<T, precisionType>::Init(GM_ADDR x, GM_ADDR workspace,
                                                                const WeightQuantBatchMatmulV2MsdTilingData* tilingData,
                                                                TPipe* tPipe)
{
    pipe = tPipe;
    tiling_ = tilingData;
...
__aicore__ inline void WeightQuantBatchMatmulV2MsdMultiCoreKernel<
    ...
    precisionType>::Init(GM_ADDR x, GM_ADDR weight, GM_ADDR antiquantScale, GM_ADDR antiquantOffset, GM_ADDR quantScale,
...
__aicore__ inline void WeightQuantBatchMatmulV2MsdMultiCoreKernel<xType, wType, biasType, yType, aTrans, bTrans,
                                                                  ...
                                                                  weightFormat, precisionType>::BL1PreLoad(TPipe* tPipe)
{
    if ASCEND_IS_AIV {
        return;
    }
...
WeightQuantBatchMatmulV2MsdMultiCoreKernel<xType, wType, biasType, yType, aTrans, bTrans, antiQuantType,
                                           hasAntiQuantOffset, quantType, weightFormat, precisionType>::Process()
{
    if constexpr (IsSameType<wType, int4b_t>::value) {
        return;
    }

...
template <bool Trans, QuantType AntiquantType, typename Enable = void>
...
struct ScaleOffsetStride<
    false, AntiquantType,
    typename AscendC::Std::enable_if_t<AntiquantType == QuantType::PER_GROUP || AntiquantType == QuantType::MX>> {
    DEVICE decltype(auto) operator()(uint64_t n, uint64_t k, int32_t groupSize)
    {
        return AscendC::Std::make_tuple(::Act::Gemm::_1{}, n);
    }
...
struct ScaleOffsetStride<
    ...
    DEVICE decltype(auto) operator()(uint64_t n, uint64_t k, int32_t groupSize)
    {
        return AscendC::Std::make_tuple(
            groupSize == 0 ? k : CeilDiv(k, static_cast<uint64_t>(groupSize)), ::Act::Gemm::_1{});
    }
};
...
template <QuantType AntiquantType, bool InnerK, typename Enable = void>
...
template <QuantType AntiquantType, typename Enable = void>
...
template <QuantType AntiquantType, bool trans>
struct ScaleOffsetShape<
    trans, AntiquantType,
    typename AscendC::Std::enable_if_t<AntiquantType == QuantType::PER_GROUP || AntiquantType == QuantType::MX>> {
    using Type = AscendC::Std::tuple<uint64_t, uint64_t>;
...
template <
    ...
    uint32_t ubMte2InnerSize, uint32_t ubMte2BufNum, bool hasAntiquantOffset, uint64_t AIV_NUM, QuantType AntiQuantType>
...
template <
    ...
    WeightQuantBatchMatmulV2::Arch35::Act::QuantType QuantType>
struct ConfigBAntiquantScmc<
    ...
    using Type = Prologue::BAntiquantScmc<
        Arch::Ascend910_95, HighBitType, ScaleType, !bTrans, false, AIV_NUM, hasAntiquantOffset, ubMte2BufNum,
...
struct ConfigBAntiquantScmc<
    ...
    using Type = Prologue::BAntiquantScmc<
        Arch::Ascend910_95, HighBitType, ScaleType, !bTrans, true, AIV_NUM, hasAntiquantOffset, ubMte2BufNum,
        ubMte2InnerSize, VF_N, VF_K, UB_OUT_BUF_NUM, UB_IN_SIZE, UB_OUT_SIZE, SCALE_SIZE, OFFSET_SIZE,
        ANTIQUANT_SCALE_AFTER_CAST_SIZE>;
};
...
template <
    typename HighBitType, typename ScaleType, bool bTrans, typename xType, typename wType, uint32_t ubMte2InnerSize,
    uint32_t ubMte2BufNum, bool hasAntiquantOffset, uint64_t AIV_NUM, QuantType AntiQuantType>
struct ConfigBAntiquantScmc<
...
template <
    uint32_t UbMte2InnerSize, uint32_t UbMte2BufNum, bool TransA, bool TransB, QuantType AntiquantType,
    ...
    GM_ADDR bias, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    static constexpr QuantType ANTIQUANT_TYPE = static_cast<QuantType>(AntiquantType);

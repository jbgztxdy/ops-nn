...
bool ThnnFusedLstmCellGradTiling::CheckParamsShape()
{
    ...
    OP_CHECK_IF(dhyInput == nullptr, OP_LOGE(nodeName_, "input grad_hy is nullptr"), return false);
    ...
    OP_TILING_CHECK(rnnParams_.hiddenSize <= 0 || rnnParams_.batch <= 0,
                    VECTOR_INNER_ERR_REPORT_TILIING(
                        nodeName_, "ThnnFusedLstmCellGrad tensor shape not support 0 or negtive, please check."),
                    return false);
    ...
    bool ret = ValidateInputShape(INPUT_DC_INDEX, hDim) && ValidateInputShape(INPUT_CX_INDEX, hDim) &&
               ValidateInputShape(INPUT_CY_INDEX, hDim) && ValidateInputShape(INPUT_STORAGE_INDEX, gatesDim) &&
               ValidateOutputShape(OUTPUT_DGATES_INDEX, gatesDim) && ValidateOutputShape(OUTPUT_DC_PREV_INDEX, hDim);
...
void ThnnFusedLstmCellGradTiling::VectorBlockCalculate()
{
    ...
    rnnParams_.baseN = rnnParams_.singleCoreN < sizeLimit ? Ops::Base::CeilAlign(rnnParams_.singleCoreN, alignedPara_) :
                                                            sizeLimit;
...
ge::graphStatus ThnnFusedLstmCellGradTiling::Init()
{
    ...
    OP_TILING_CHECK(CheckAttr() != ge::GRAPH_SUCCESS, VECTOR_INNER_ERR_REPORT_TILIING(nodeName_, "check attr fail."),
                    return ge::GRAPH_FAILED);
    ...
    int64_t workspaceReduce = rnnParams_.batch > rnnParams_.maxReduceNumOnce ?
                                  GATES_NUM * rnnParams_.hiddenSize * FP32_BYTES *
                                      Ops::Base::CeilDiv(rnnParams_.batch, rnnParams_.maxReduceNumOnce) :
                                  0;
    int64_t workspaceDgates = (inputDSize_ != FP32_BYTES && rnnParams_.isBias == 1) ?
                                  GATES_NUM * rnnParams_.hiddenSize * FP32_BYTES * rnnParams_.batch :
                                  0;

...
bool BidirectionLSTMTiling::CheckTensorShape(gert::TilingContext* context, gert::Shape& shape, uint64_t ndim,
                                             std::vector<uint64_t> dims)
{
    OP_CHECK_IF(
        shape.GetDimNum() != ndim,
        OP_LOGE(context->GetNodeName(), "BidirectionLSTM tensor shape ndim:%lu wrong expected %lu, please check.",
                static_cast<uint64_t>(shape.GetDimNum()), ndim),
        return false);
    for (uint32_t i = 0; i < ndim; i++) {
        OP_CHECK_IF(static_cast<uint64_t>(shape.GetDim(i)) != dims[i],
                    OP_LOGE(context->GetNodeName(),
                            "BidirectionLSTM tensor shape dim[%d]:%lu wrong expected %lu, please check.", i,
...
bool BidirectionLSTMTiling::CheckInputShapes(gert::TilingContext* context)
{
    ...
    if (!_Params.packed) {
        OP_CHECK_IF(
            !CheckTensorShape(context, shape, 3, {_Params.sequenceLength, _Params.batchSize, _Params.inputSize}),
            OP_LOGE(context->GetNodeName(), "BidirectionLSTM get x shape wrong, please check."), return false);
    }
    ...
    OP_CHECK_IF(
        !CheckTensorShape(context, shape, 3,
                          {(_Params.bidirection ? 2 : 1) * _Params.numLayers, _Params.batchSize, _Params.hiddenSize}),
...
bool BidirectionLSTMTiling::CheckOutputShapes(gert::TilingContext* context)
{
    ...
    if (!_Params.packed) {
        OP_CHECK_IF(
            !CheckTensorShape(
                context, shape, 3,
                {_Params.sequenceLength, _Params.batchSize, (_Params.bidirection ? 2 : 1) * _Params.hiddenSize}),
            OP_LOGE(context->GetNodeName(), "BidirectionLSTM get output shape wrong, please check."), return false);
    } else {
        OP_CHECK_IF(!CheckTensorShape(context, shape, 2,
                                      {_Params.totalBatchSizes, (_Params.bidirection ? 2 : 1) * _Params.hiddenSize}),
                    OP_LOGE(context->GetNodeName(), "BidirectionLSTM get output shape wrong, please check."),
                    return false);
    }
    ...
    OP_CHECK_IF(
        !CheckTensorShape(context, shape, 3,
                          {(_Params.bidirection ? 2 : 1) * _Params.numLayers, _Params.batchSize, _Params.hiddenSize}),
        OP_LOGE(context->GetNodeName(), "BidirectionLSTM get output_c shape wrong, please check."), return false);
...
bool BidirectionLSTMTiling::CheckBatchSizeShape(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(shape.GetDimNum() != 1,
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM get batch_size shape ndim is not 1, please check."),
                return false);
    OP_CHECK_IF(
        shape.GetDim(0) > MAX_SEQ,
        OP_LOGE(context->GetNodeName(), "BidirectionLSTM get batch_size length %lu exceed limit %u, please check.",
                static_cast<uint64_t>(shape.GetDim(0)), MAX_SEQ),
        return false);
    _Params.isSeq = true;
...
bool BidirectionLSTMTiling::CheckXInputShape(gert::TilingContext* context)
{
    ...
    if (!_Params.packed) {
        OP_CHECK_IF(shape.GetDimNum() != NUMBER_THREE,
                    OP_LOGE(context->GetNodeName(), "BidirectionLSTM get x shape ndim is not 3, please check."),
                    return false);
        _Params.inputSize = shape.GetDim(NUMBER_TWO);
    } else {
        OP_CHECK_IF(shape.GetDimNum() != NUMBER_TWO,
                    OP_LOGE(context->GetNodeName(), "BidirectionLSTM get x shape ndim is not 2, please check."),
                    return false);
...
bool BidirectionLSTMTiling::CheckInOutShapes(gert::TilingContext* context)
{
    OP_CHECK_IF(!CheckBatchSizeShape(context),
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM CheckBatchSizeShape failed, please check."),
                return false);

    OP_CHECK_IF(!CheckXInputShape(context),
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM CheckXInputShape failed, please check."),
                return false);

    OP_CHECK_IF(!CheckInitHInputShape(context),
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM CheckInitHInputShape failed, please check."),
                return false);

    OP_CHECK_IF(!CheckInputShapes(context),
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM CheckInputShapes failed, please check."),
                return false);
    OP_CHECK_IF(!CheckOptionalInputShapes(context),
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM CheckOptionalInputShapes failed, please check."),
                return false);
    OP_CHECK_IF(!CheckOutputShapes(context),
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM CheckOutputShapes failed, please check."),
                return false);
    return true;
}
...
bool BidirectionLSTMTiling::GetCheckAttr(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(*num_layers != 1,
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM attr num_layers only support 1, please check."),
                return false);
    OP_CHECK_IF(*batch_first,
                OP_LOGE(context->GetNodeName(), "BidirectionLSTM attr batch_first only support false, please check."),
                return false);
...
bool BidirectionLSTMTiling::GetPlatformInfo(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(!val.empty(), OP_LOGE(context->GetNodeName(), "BidirectionLSTM support only ASCEND310P for now"),
                return false);
...
void BidirectionLSTMTiling::GetPaddataTilingData()
{
    ...
    _Params.padInputBaseM = _Params.padBaseN /
                            (_Params.inputSizeAligned < PAD_BASEN ? _Params.inputSizeAligned : PAD_BASEN);
    ...
    _Params.padHiddenBaseM = _Params.padBaseN /
                             (_Params.hiddenSizeAligned < PAD_BASEN ? _Params.hiddenSizeAligned : PAD_BASEN);
...
{
    int64_t total_count = (_Params.bidirection ? NUMBER_TWO : 1) * _Params.sequenceLength * _Params.batchSize *
                          _Params.hiddenSize;
    if (_Params.packed) {
...
void BidirectionLSTMTiling::GetTransdataTilingData()
{
    ...
    _Params.transInputInnerTailN = _Params.transInputInnerTailN == 0 ? _Params.transInnerBaseN :
                                                                       _Params.transInputInnerTailN;
    ...
    _Params.transHiddenInnerTailN = _Params.transHiddenInnerTailN == 0 ? _Params.transInnerBaseN :
                                                                         _Params.transHiddenInnerTailN;
...
{
    _Params.batchSizeAligned = (_Params.batchSize + NUM_PER_BLOCK_FLOAT16 - 1) / NUM_PER_BLOCK_FLOAT16 *
                               NUM_PER_BLOCK_FLOAT16;
    _Params.inputSizeAligned = (_Params.inputSize + NUM_PER_BLOCK_FLOAT16 - 1) / NUM_PER_BLOCK_FLOAT16 *
                               NUM_PER_BLOCK_FLOAT16;
    _Params.hiddenSizeAligned = (_Params.hiddenSize + NUM_PER_BLOCK_FLOAT16 - 1) / NUM_PER_BLOCK_FLOAT16 *
                                NUM_PER_BLOCK_FLOAT16;
    ...
    _Params.workspaceInOutStep = _Params.batchSizeAligned * (_Params.inputSizeAligned > _Params.hiddenSizeAligned ?
                                                                 _Params.inputSizeAligned :
                                                                 _Params.hiddenSizeAligned);
    _Params.workspaceInOutN = (_Params.sequenceLength + 1) * _Params.workspaceInOutStep;
    _Params.workspaceTmpN = _Params.workspaceWihN > _Params.workspaceWhhN ? _Params.workspaceWihN :
                                                                            _Params.workspaceWhhN;
    _Params.workspaceTmpN *= NUM_OF_GATE;
    _Params.workspaceTmpN = _Params.workspaceTmpN > _Params.workspaceInOutN ? _Params.workspaceTmpN :
                                                                              _Params.workspaceInOutN;
...
void BidirectionLSTMTiling::GetIntraCoreMMTilingData(uint32_t BaseMNFractalN, uint32_t& BaseMNum, uint32_t& BaseNNum)
{
    ...
    if (MNFractalN <= BaseMNFractalN) {
        ...
        if (_Params.IHSingleCoreM > _Params.IHSingleCoreN) {
            ...
            while (BaseNNum > 1 &&
                   BaseMFractalN_ * ((_Params.IHSingleCoreN + BaseNNum - 1 - 1) / (BaseNNum - 1)) <= BaseMNFractalN) {
                BaseNNum -= 1;
            }
            uint32_t BaseNFractalN_ = (_Params.IHSingleCoreN + BaseNNum - 1) / BaseNNum;
            while (BaseMNum > 1 &&
                   BaseNFractalN_ * ((_Params.IHSingleCoreM + BaseMNum - 1 - 1) / (BaseMNum - 1)) <= BaseMNFractalN) {
                BaseMNum -= 1;
            }
        } else {
            uint32_t BaseNFractalN_ = (_Params.IHSingleCoreN + BaseNNum - 1) / BaseNNum;
...
bool BidirectionLSTMTiling::ChooseMMStrategy(uint32_t BaseMNumA, uint32_t BaseNNumA, uint32_t BaseMNumB,
                                             uint32_t BaseNNumB)
{
...
void BidirectionLSTMTiling::GetMMTilingDataBaseK()
{
    ...
    if (MKFractalN <= _Params.BaseMKFractalN && NKFractalN <= _Params.BaseNKFractalN) {
        ...
        uint32_t baseK = static_cast<uint32_t>(
            floor(static_cast<float>(_Params.IHSingleCoreK) / (MKPieces > NKPieces ? MKPieces : NKPieces)));
    ...
    if (MKFractalN <= _Params.BaseMKFractalN && NKFractalN <= _Params.BaseNKFractalN) {
        ...
        float NKPieces = static_cast<float>(NKFractalN) / static_cast<float>(_Params.BaseNKFractalN);
        uint32_t baseK = static_cast<uint32_t>(
            floor(static_cast<float>(_Params.HHSingleCoreK) / (MKPieces > NKPieces ? MKPieces : NKPieces)));
...
{
    int64_t vecBaseRepeatMax = (UB_BUFF_SIZE / GetDtypeSize(ge::DataType::DT_FLOAT) + NUM_PER_REPEAT_FLOAT16 - 1) /
...
bool BidirectionLSTMTiling::SetLaunchInfo(gert::TilingContext* context)
{
    ...
    int64_t workspaceSize = _Params.workspaceGateN * (NUM_OF_GATE + 1) * GetDtypeSize(ge::DataType::DT_FLOAT) +
                            ...
                            (_Params.isBias ? _Params.workspaceBN * NUM_OF_GATE * GetDtypeSize(ge::DataType::DT_FLOAT) :
                                              0) +
                            _Params.workspaceInOutN * GetDtypeSize(ge::DataType::DT_FLOAT16) +
                            _Params.workspaceTmpN * GetDtypeSize(ge::DataType::DT_FLOAT16) +
                            _Params.CoreNum * BLOCK_SIZE + 2 * 1024 * 1024;
...
ge::graphStatus BidirectionLSTMTiling::runTiling(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(_Params.dataType != ge::DataType::DT_FLOAT16, OP_LOGE(context->GetNodeName(), "check dtype fail."),
                return ge::GRAPH_FAILED);
...
static ge::graphStatus TilingPrepareForBidirectionLSTM(gert::TilingParseContext* context) { return ge::GRAPH_SUCCESS; }

IMPL_OP_OPTILING(BidirectionLSTM)
    .Tiling(TilingForBidirectionLSTM)
    .TilingParse<BidirectionLSTMCompileInfo>(TilingPrepareForBidirectionLSTM);
IMPL_OP_OPTILING(BidirectionLSTMV2)
    .Tiling(TilingForBidirectionLSTM)
    .TilingParse<BidirectionLSTMCompileInfo>(TilingPrepareForBidirectionLSTM);

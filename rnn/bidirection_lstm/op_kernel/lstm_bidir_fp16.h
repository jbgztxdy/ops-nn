...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR init_h, GM_ADDR init_c, GM_ADDR w_ih, GM_ADDR w_hh, GM_ADDR b_ih,
                                GM_ADDR b_hh, GM_ADDR w_ih_reverse, GM_ADDR w_hh_reverse, GM_ADDR b_ih_reverse,
                                GM_ADDR b_hh_reverse, GM_ADDR batch_size, GM_ADDR y, GM_ADDR output_h, GM_ADDR output_c,
                                GM_ADDR usrWorkspace, const BidirectionLSTMTilingData* __restrict tilingData);
...
protected:
    ...
    __aicore__ inline void InitGlobalTensors(GM_ADDR x, GM_ADDR init_h, GM_ADDR init_c, GM_ADDR w_ih, GM_ADDR w_hh,
    ...
    __aicore__ inline void PadCopy(const GlobalTensor<half>& dstGlobal, const GlobalTensor<half>& srcGlobal,
                                   uint32_t loop, uint32_t tailM, uint32_t tailCoreNum, bool needPad, bool unpad,
                                   bool spilt_line, PadInnerParams& innerParams);
    __aicore__ inline void BatchTrans(const GlobalTensor<half>& ZNGlobal, const GlobalTensor<half>& NDGlobal,
                                      uint32_t dstStep, uint32_t srcStep, uint32_t batchTailM, uint32_t batch,
                                      uint32_t loop, uint32_t tailCoreNum, bool spilt_line,
                                      TransInnerParams& innerParams, bool isND2ZN, bool packed = false,
                                      bool isSeq = false);
    ...
    __aicore__ inline void CalCrossStartEnd(uint32_t baseOffset, uint32_t len, uint32_t& cross_start,
                                            uint32_t& cross_end);
    ...
    __aicore__ inline void IterateVectorCross(uint32_t tIdx, uint32_t offset, uint32_t len, uint32_t cross_start,
                                              uint32_t cross_end, uint64_t mask[2]);

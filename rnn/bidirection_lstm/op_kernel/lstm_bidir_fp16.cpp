...
__aicore__ inline void LstmBidirFP16::Init(GM_ADDR x, GM_ADDR init_h, GM_ADDR init_c, GM_ADDR w_ih, GM_ADDR w_hh,
                                           GM_ADDR b_ih, GM_ADDR b_hh, GM_ADDR w_ih_reverse, GM_ADDR w_hh_reverse,
                                           GM_ADDR b_ih_reverse, GM_ADDR b_hh_reverse, GM_ADDR batch_size, GM_ADDR y,
                                           GM_ADDR output_h, GM_ADDR output_c, GM_ADDR usrWorkspace,
                                           const BidirectionLSTMTilingData* __restrict lstmTiling)
{
    ...
    InitGlobalTensors(x, init_h, init_c, w_ih, w_hh, b_ih, b_hh, w_ih_reverse, w_hh_reverse, b_ih_reverse, b_hh_reverse,
                      y, output_h, output_c);
    ...
    clearInOutOffset = (tilingData->clearInOutLoop * tilingData->padBaseN + tilingData->clearInOutTailN) * block_id +
                       (block_id < tilingData->clearInOutTailCoreNum ? block_id : tilingData->clearInOutTailCoreNum) *
                           NUM_PER_BLOCK_FLOAT16;
    clearInitOffset = (tilingData->clearInitLoop * tilingData->padBaseN + tilingData->clearInitTailN) * block_id +
                      (block_id < tilingData->clearInitTailCoreNum ? block_id : tilingData->clearInitTailCoreNum) *
                          NUM_PER_BLOCK_FLOAT16;
    // vecCompute scalar
    ...
    vecTailOffset = tilingData->vecBaseN * tilingData->CoreNum * tilingData->vecLoop + tilingData->vecTailN * block_id +
                    (block_id < tilingData->vecTailCoreNum ? block_id : tilingData->vecTailCoreNum) *
                        NUM_PER_REPEAT_FLOAT16;
    ...
    if (tilingData->isSeq) {
        ...
        if (tilingData->packed) {
            ...
            assert(batchOffsets[tilingData->sequenceLength] == tilingData->totalBatchSizes,
                   "sum of batch_size[] %u not equal to x dim[0]:%u\n", batchOffsets[tilingData->sequenceLength],
                   tilingData->totalBatchSizes);
        }
    }
}
...
__aicore__ inline void LstmBidirFP16::InitTransLocalLists()
{
    ...
    for (int i = 1; i < NUM_PER_BLOCK_FLOAT16; i++) {
        ...
        NDLocalListHiddenTail[0][i] = NDLocalListHiddenTail[0][i - 1] +
                                      tilingData->transHiddenInnerTailN * sizeof(half);
        NDLocalListHiddenTail[1][i] = NDLocalListHiddenTail[1][i - 1] +
                                      tilingData->transHiddenInnerTailN * sizeof(half);
...
__aicore__ inline void LstmBidirFP16::InitTransInnerParamSingle(TransInnerParams& innerParams)
{
    ...
    innerParams.repeatParamsTransND = {
        NUM_PER_BLOCK_FLOAT16, static_cast<uint16_t>(innerParams.innerBaseN / NUM_PER_BLOCK_FLOAT16),
        static_cast<uint16_t>((innerParams.baseN - innerParams.innerBaseN) / NUM_PER_BLOCK_FLOAT16),
        DEFAULT_DATA_COPY_STRIDE};
    innerParams.repeatParamsTransZN = {1, static_cast<uint16_t>(innerParams.innerBaseN), DEFAULT_DATA_COPY_STRIDE,
                                       DEFAULT_DATA_COPY_STRIDE};
    innerParams.repeatParamsTransNDTail = {
        NUM_PER_BLOCK_FLOAT16, static_cast<uint16_t>(innerParams.innerTailN / NUM_PER_BLOCK_FLOAT16),
        static_cast<uint16_t>((innerParams.baseN - innerParams.innerTailN) / NUM_PER_BLOCK_FLOAT16),
        DEFAULT_DATA_COPY_STRIDE};
    innerParams.repeatParamsTransZNTail = {1, static_cast<uint16_t>(innerParams.innerTailN), DEFAULT_DATA_COPY_STRIDE,
                                           DEFAULT_DATA_COPY_STRIDE};
    ...
    innerParams.transDataParamsTail = {false, false,
                                       static_cast<uint8_t>(innerParams.innerTailN / NUM_PER_BLOCK_FLOAT16),
                                       NUM_PER_BLOCK_FLOAT16, DEFAULT_BLK_STRIDE};
...
__aicore__ inline void LstmBidirFP16::InitTransInnerParams()
{
    ...
    swapValue(hiddenZN2NDInnerParams.repeatParamsTransND.srcStride,
              hiddenZN2NDInnerParams.repeatParamsTransND.dstStride);
    swapValue(hiddenZN2NDInnerParams.repeatParamsTransNDTail.srcStride,
              hiddenZN2NDInnerParams.repeatParamsTransNDTail.dstStride);
    swapValue(hiddenZN2NDInnerParams.transDataParams.dstRepStride, hiddenZN2NDInnerParams.transDataParams.srcRepStride);
    swapValue(hiddenZN2NDInnerParams.transDataParamsTail.dstRepStride,
              hiddenZN2NDInnerParams.transDataParamsTail.srcRepStride);
}
...
__aicore__ inline void LstmBidirFP16::InitPadInnerParamSingle(PadInnerParams& innerParams)
{
    ...
    innerParams.repeatParamsIn = {
        static_cast<uint16_t>(innerParams.innerBaseM),
        static_cast<uint16_t>(innerParams.innerBaseN / NUM_PER_BLOCK_FLOAT16),
        static_cast<uint16_t>(innerParams.baseInN - innerParams.innerBaseN / NUM_PER_BLOCK_FLOAT16),
        DEFAULT_DATA_COPY_STRIDE};
    innerParams.repeatParamsOut = {
        static_cast<uint16_t>(innerParams.innerBaseM),
        static_cast<uint16_t>(innerParams.innerBaseN / NUM_PER_BLOCK_FLOAT16), DEFAULT_DATA_COPY_STRIDE,
        static_cast<uint16_t>(innerParams.baseOutN - innerParams.innerBaseN / NUM_PER_BLOCK_FLOAT16)};
    innerParams.repeatParamsInTail = {
        static_cast<uint16_t>(innerParams.innerBaseM),
        static_cast<uint16_t>(innerParams.innerTailN / NUM_PER_BLOCK_FLOAT16),
        static_cast<uint16_t>(innerParams.baseInN - innerParams.innerTailN / NUM_PER_BLOCK_FLOAT16),
        DEFAULT_DATA_COPY_STRIDE};
    innerParams.repeatParamsOutTail = {
        static_cast<uint16_t>(innerParams.innerBaseM),
        static_cast<uint16_t>(innerParams.innerTailN / NUM_PER_BLOCK_FLOAT16), DEFAULT_DATA_COPY_STRIDE,
        static_cast<uint16_t>(innerParams.baseOutN - innerParams.innerTailN / NUM_PER_BLOCK_FLOAT16)};
}
...
__aicore__ inline void LstmBidirFP16::InitWorkspaceTensors(GM_ADDR usrWorkspace)
{
    ...
    workspaceMMGm.SetGlobalBuffer(
        reinterpret_cast<__gm__ float*>(usrWorkspace +
                                        block_id / NUM_MM_GROUP * tilingData->workspaceGateN * sizeof(float)),
        tilingData->workspaceGateN);

    workspaceIGm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(usrWorkspace_), tilingData->workspaceGateN);
    usrWorkspace_ += tilingData->workspaceGateN * sizeof(float);
    workspaceFGm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(usrWorkspace_), tilingData->workspaceGateN);
    usrWorkspace_ += tilingData->workspaceGateN * sizeof(float);
    workspaceJGm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(usrWorkspace_), tilingData->workspaceGateN);
    usrWorkspace_ += tilingData->workspaceGateN * sizeof(float);
    workspaceOGm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(usrWorkspace_), tilingData->workspaceGateN);
    usrWorkspace_ += tilingData->workspaceGateN * sizeof(float);
    workspaceCGm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(usrWorkspace_), tilingData->workspaceGateN);
    usrWorkspace_ += tilingData->workspaceGateN * sizeof(float);

    workspaceInOutGm.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(usrWorkspace_), tilingData->workspaceInOutN);
    workspaceInGm.SetGlobalBuffer(
        reinterpret_cast<__gm__ half*>(usrWorkspace_ + tilingData->workspaceInOutStep * sizeof(half)),
        tilingData->sequenceLength * tilingData->workspaceInOutStep);
    workspacePreOutGm.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(usrWorkspace_),
                                      tilingData->sequenceLength * tilingData->workspaceInOutStep);
    workspaceInitHGm.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(usrWorkspace_), tilingData->workspaceInOutStep);
    workspaceOutputHGm.SetGlobalBuffer(
        reinterpret_cast<__gm__ half*>(usrWorkspace_ +
                                       tilingData->sequenceLength * tilingData->workspaceInOutStep * sizeof(half)),
        tilingData->workspaceInOutStep);
    usrWorkspace_ += tilingData->workspaceInOutN * sizeof(half);
    workspaceWihGM.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(usrWorkspace_),
                                   NUM_OF_GATE * tilingData->workspaceWihN);
    workspaceWihGMSelf.SetGlobalBuffer(
        reinterpret_cast<__gm__ half*>(usrWorkspace_ +
                                       block_id / NUM_MM_GROUP * tilingData->workspaceWihN * sizeof(half)),
        tilingData->workspaceWihN);
    usrWorkspace_ += NUM_OF_GATE * tilingData->workspaceWihN * sizeof(half);
    workspaceWhhGM.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(usrWorkspace_),
                                   NUM_OF_GATE * tilingData->workspaceWhhN);
    workspaceWhhGMSelf.SetGlobalBuffer(
        reinterpret_cast<__gm__ half*>(usrWorkspace_ +
                                       block_id / NUM_MM_GROUP * tilingData->workspaceWhhN * sizeof(half)),
        tilingData->workspaceWhhN);
    usrWorkspace_ += NUM_OF_GATE * tilingData->workspaceWhhN * sizeof(half);
    if (tilingData->isBias != 0) {
        workspaceBiasGM.SetGlobalBuffer(
            reinterpret_cast<__gm__ float*>(usrWorkspace_ +
                                            block_id / NUM_MM_GROUP * tilingData->workspaceBN * sizeof(float)),
            tilingData->workspaceBN);
    ...
    syncGmWsSelf.SetGlobalBuffer(
        reinterpret_cast<__gm__ int32_t*>(usrWorkspace_ + block_id * NUM_PER_BLOCK_FLOAT32 * sizeof(float)),
        NUM_PER_BLOCK_FLOAT32);
}

__aicore__ inline void LstmBidirFP16::InitGlobalTensors(GM_ADDR x, GM_ADDR init_h, GM_ADDR init_c, GM_ADDR w_ih,
                                                        GM_ADDR w_hh, GM_ADDR b_ih, GM_ADDR b_hh, GM_ADDR w_ih_reverse,
                                                        GM_ADDR w_hh_reverse, GM_ADDR b_ih_reverse,
                                                        GM_ADDR b_hh_reverse, GM_ADDR y, GM_ADDR output_h,
                                                        GM_ADDR output_c)
{
    ...
    if (tilingData->isBias != 0) {
        bIHGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ half*>(b_ih + block_id / NUM_MM_GROUP * tilingData->hiddenSize * sizeof(half)),
            tilingData->hiddenSize);
        bHHGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ half*>(b_hh + block_id / NUM_MM_GROUP * tilingData->hiddenSize * sizeof(half)),
            tilingData->hiddenSize);
    }
    ...
    if (tilingData->bidirection != 0) {
        ...
        if (tilingData->isBias != 0) {
            bIHRevGm.SetGlobalBuffer(
                reinterpret_cast<__gm__ half*>(b_ih_reverse +
                                               block_id / NUM_MM_GROUP * tilingData->hiddenSize * sizeof(half)),
                tilingData->hiddenSize);
            bHHRevGm.SetGlobalBuffer(
                reinterpret_cast<__gm__ half*>(b_hh_reverse +
                                               block_id / NUM_MM_GROUP * tilingData->hiddenSize * sizeof(half)),
                tilingData->hiddenSize);
        }
    }
...
__aicore__ inline void LstmBidirFP16::ProcessMM(uint32_t tIdx)
{
    ...
    if (tilingData->isSeq) {
        ...
        uint32_t IHSingleCoreM_ = (tilingData->IHM == tilingData->IHSingleCoreM) ?
                                      IHM_ :
                                      (IHM_ + NUM_MM_GROUP - 1) / NUM_MM_GROUP;
        matOffsetM = 0;
    ...
    for (int i = 0; i < tilingData->IHBaseNNum; i++) {
        ...
        for (int j = 0; j < IHBaseMNum_; j++) {
            ...
            for (int k = 0; k < tilingData->IHBaseKNum; k++) {
                ...
                repeatParams.dstStride = DEFAULT_DATA_COPY_STRIDE;
                DataCopy<half>(L1MatA,
                               workspaceWihGMSelf[(offsetN * tilingData->IHK + offsetIHK) * NUM_PER_FRACTAL_FLOAT16],
                               repeatParams);
                ...
                DataCopy<half>(L1MatB,
                               workspaceInGm[tIdx * tilingData->workspaceInOutStep +
                                             (offsetM * tilingData->IHK + offsetIHK) * NUM_PER_FRACTAL_FLOAT16],
                               repeatParams);
                SetFlag<HardEvent::MTE2_MTE1>(eventIdMTE2ToMTE1[1]);
                WaitFlag<HardEvent::MTE2_MTE1>(eventIdMTE2ToMTE1[1]);
                loadData3DV2.extConfig = ((uint64_t)(realIHBaseK * NUM_PER_BLOCK_FLOAT16) << NUMBER_SIXTEEN) |
                                         (uint64_t)(realBaseM * NUM_PER_BLOCK_FLOAT16);
            ...
            for (int k = 0; k < tilingData->HHBaseKNum; k++) {
                ...
                repeatParams.dstStride = DEFAULT_DATA_COPY_STRIDE;
                DataCopy<half>(L1MatA,
                               workspaceWhhGMSelf[(offsetN * tilingData->HHK + offsetHHK) * NUM_PER_FRACTAL_FLOAT16],
                               repeatParams);
                SetFlag<HardEvent::MTE2_MTE1>(eventIdMTE2ToMTE1[0]);
                WaitFlag<HardEvent::MTE2_MTE1>(eventIdMTE2ToMTE1[0]);
                ...
                repeatParams.blockCount = realBaseM;
                DataCopy<half>(L1MatB,
                               workspacePreOutGm[tIdx * tilingData->workspaceInOutStep +
                                                 (offsetM * tilingData->HHK + offsetHHK) * NUM_PER_FRACTAL_FLOAT16],
                               repeatParams);
                SetFlag<HardEvent::MTE2_MTE1>(eventIdMTE2ToMTE1[1]);
                WaitFlag<HardEvent::MTE2_MTE1>(eventIdMTE2ToMTE1[1]);
                loadData3DV2.extConfig = ((uint64_t)(realHHBaseK * NUM_PER_BLOCK_FLOAT16) << NUMBER_SIXTEEN) |
                                         (uint64_t)(realBaseM * NUM_PER_BLOCK_FLOAT16);
            ...
            if (tilingData->isBias == 0) {
                ...
                if ((j + 1) < IHBaseMNum_) {
                    ...
                    if ((i + 1) < tilingData->IHBaseNNum) {
                        ...
                        repeatParams.srcStride = (tilingData->IHN - realBaseN_) * NUM_PER_FRACTAL_FLOAT16 /
                                                 NUM_PER_BLOCK_FLOAT32;
                        repeatParams.dstStride = DEFAULT_DATA_COPY_STRIDE;
                        WaitFlag<HardEvent::V_MTE2>(eventIdVToMTE2[0]);
                        DataCopy<float>(ubMatIn, workspaceBiasGM[(offsetN + realBaseN) * NUM_PER_FRACTAL_FLOAT16],
                                        repeatParams);
                        SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
                        WaitFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
            ...
            DataCopy<float>(workspaceMMGm[(offsetM * tilingData->IHN + offsetN) * NUM_PER_FRACTAL_FLOAT16], ubMatOut,
                            repeatParams);
            if (tilingData->activeMat != 0) {
                SetFlag<HardEvent::MTE3_V>(eventIdMTE3ToV[0]);
                                                  ...
                                                  const LocalTensor<float>& buffer)
{
    ...
    Duplicate<float, false>(buffer, static_cast<float>(1.0), MASK_PLACEHOLDER, 1, DEFAULT_BLK_STRIDE,
...
__aicore__ inline void LstmBidirFP16::IterateVectorCross(uint32_t tIdx, uint32_t offset, uint32_t len,
                                                         uint32_t cross_start, uint32_t cross_end, uint64_t mask[2])
{
    SetMaskCount();
    ...
    if (tilingData->activeMat != 0) {
        ...
        if (cross_start != 0) {
            ...
            Mul<float, false>(ubCompute[NUMBER_TWO], ubCompute[NUMBER_TWO], ubCompute[NUMBER_THREE], MASK_PLACEHOLDER,
                              1, binaryParams);
        }
        if (cross_start != cross_end) {
            SetMaskNorm();
            Mul<float>(ubCompute[NUMBER_TWO][cross_start], ubCompute[NUMBER_TWO][cross_start],
                       ubCompute[NUMBER_THREE][cross_start], mask,
                       static_cast<uint8_t>((cross_end - cross_start) / NUM_PER_REPEAT_FLOAT32), binaryParams);
            SetMaskCount();
        }
        ...
        if (cross_start != 0) {
            SetVectorMask<float, MaskMode::COUNTER>(0, cross_start);
            Add<float, false>(ubCompute[NUMBER_TWO], ubCompute[NUMBER_TWO], ubCompute[0], MASK_PLACEHOLDER, 1,
                              binaryParams);
        }
        if (cross_start != cross_end) {
            SetMaskNorm();
            Add<float>(ubCompute[NUMBER_TWO][cross_start], ubCompute[NUMBER_TWO][cross_start],
                       ubCompute[0][cross_start], mask,
                       static_cast<uint8_t>((cross_end - cross_start) / NUM_PER_REPEAT_FLOAT32), binaryParams);
            SetMaskCount();
        }
        ...
        PipeBarrier<PIPE_V>();
        WaitFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
        Mul<float, false>(ubCompute[NUMBER_THREE], ubCompute[NUMBER_THREE], ubCompute[1], MASK_PLACEHOLDER, 1,
                          binaryParams);
    } else {
        DataCopy<float>(ubCompute[0], workspaceIGm[offset], repeatParamsFloat);
        SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
        ...
        PipeBarrier<PIPE_V>();
        Mul<float, false>(ubCompute[NUMBER_TWO], ubCompute[NUMBER_TWO], ubCompute[NUMBER_THREE], MASK_PLACEHOLDER, 1,
                          binaryParams);
        ...
        if (cross_start != cross_end) {
            SetMaskNorm();
            Mul<float>(ubCompute[1][cross_start], ubCompute[1][cross_start], ubCompute[NUMBER_THREE][cross_start], mask,
                       static_cast<uint8_t>((cross_end - cross_start) / NUM_PER_REPEAT_FLOAT32), binaryParams);
            SetMaskCount();
        }
        ...
        if (cross_start != cross_end) {
            SetMaskNorm();
            Add<float>(ubCompute[1][cross_start], ubCompute[1][cross_start], ubCompute[NUMBER_TWO][cross_start], mask,
                       static_cast<uint8_t>((cross_end - cross_start) / NUM_PER_REPEAT_FLOAT32), binaryParams);
            SetMaskCount();
        }
        ...
        PipeBarrier<PIPE_V>();
        Mul<float, false>(ubCompute[NUMBER_THREE], ubCompute[NUMBER_THREE], ubCompute[NUMBER_TWO], MASK_PLACEHOLDER, 1,
                          binaryParams);
    }

    if (len > cross_start) {
        repeatParamsHalf.blockLen = (len - cross_start) / NUM_PER_BLOCK_FLOAT16;
        DataCopy<half>(ubCompute_[cross_start],
                       workspacePreOutGm[tIdx * tilingData->workspaceInOutStep + offset + cross_start],
                       repeatParamsHalf);
        SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
        WaitFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
    }
    ...
    if (cross_start != 0) {
        SetVectorMask<float, MaskMode::COUNTER>(0, cross_start);
        Cast<half, float, false>(ubCompute_, ubCompute[NUMBER_THREE], RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
                                 unaryParamsF2H);
    }
    if (cross_start != cross_end) {
        SetMaskNorm();
        Cast<half, float>(ubCompute_[cross_start], ubCompute[NUMBER_THREE][cross_start], RoundMode::CAST_NONE, mask,
                          static_cast<uint8_t>((cross_end - cross_start) / NUM_PER_REPEAT_FLOAT32), unaryParamsF2H);
    }
    SetFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
    WaitFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
...
__aicore__ inline void LstmBidirFP16::CalCrossStartEnd(uint32_t baseOffset, uint32_t len, uint32_t& cross_start,
                                                       uint32_t& cross_end)
{
...
__aicore__ inline void LstmBidirFP16::ProcessVector(uint32_t tIdx)
{
    ...
    if (!tilingData->isSeq) {
        ...
        for (int i = 0; i < tilingData->vecLoop; i++) {
            IterateVectorCross(tIdx, vecOffset_, tilingData->vecBaseN, tilingData->vecBaseN, tilingData->vecBaseN,
                               mask);
        ...
        uint32_t cross_start = (batchSizes[tIdx] / NUM_PER_BLOCK_FLOAT16 * NUM_PER_BLOCK_FLOAT16) *
                               tilingData->hiddenSizeAligned;
        uint32_t cross_end = ((batchSizes[tIdx] + NUM_PER_BLOCK_FLOAT16 - 1) / NUM_PER_BLOCK_FLOAT16 *
                              NUM_PER_BLOCK_FLOAT16) *
                             tilingData->hiddenSizeAligned;
        for (int i = 0; i < tilingData->vecLoop; i++) {
...
__aicore__ inline void LstmBidirFP16::PadCopyAndCastBias(const GlobalTensor<float>& dstGlobal,
                                                         const GlobalTensor<half>& srcGlobal, uint32_t originBaseN,
                                                         uint64_t mask_, TransInnerParams& innerParams)
{
...
__aicore__ inline void LstmBidirFP16::PadCopy(const GlobalTensor<half>& dstGlobal, const GlobalTensor<half>& srcGlobal,
                                              uint32_t loop, uint32_t tailM, uint32_t tailCoreNum, bool needPad,
                                              bool unpad, bool spilt_line, PadInnerParams& innerParams)
{
    if (unpad) {
        SetAtomicAdd<half>();
        if (spilt_line) {
    ...
    for (int32_t i = 0; i <= (NUMBER_TWO * loop + 1); i++) {
        ...
        for (int j = 0; j < innerParams.innerLoop; j++) {
            ...
            if (j == (innerParams.innerLoop - 1) && needPad) {
                ...
                Duplicate<half>(ubPad[ping_pong][realN - NUM_PER_BLOCK_FLOAT16], static_cast<half>(0), innerParams.mask,
                                realM, DEFAULT_BLK_STRIDE, realN / NUM_PER_BLOCK_FLOAT16);
...
__aicore__ inline void LstmBidirFP16::BatchTrans(const GlobalTensor<half>& ZNGlobal, const GlobalTensor<half>& NDGlobal,
                                                 uint32_t dstStep, uint32_t srcStep, uint32_t batchTailM,
                                                 uint32_t batch, uint32_t loop, uint32_t tailCoreNum, bool spilt_line,
        ...
        for (int j = 0; j < innerParams.innerLoop; j++) {
            ...
            if (isND2ZN) {
                ...
                if (tailM == NUM_PER_BLOCK_FLOAT16) {
                    ...
                    Duplicate<half, false>(ubTransIn[ping_pong][tailM * realN], static_cast<half>(0), MASK_PLACEHOLDER,
                ...
                if (tailM_ == NUM_PER_BLOCK_FLOAT16) {
                    ...
                    if (tailM_ != 0) {
                        ...
                        Duplicate<half, false>(ubTransIn[ping_pong][tailM_ * realN], static_cast<half>(0),
                                               MASK_PLACEHOLDER, 1, DEFAULT_BLK_STRIDE, DEFAULT_REPEAT_STRIDE);
                    }
                }
                SetFlag<HardEvent::V_MTE3>(eventIdVToMTE3[ping_pong]);
                WaitFlag<HardEvent::V_MTE3>(eventIdVToMTE3[ping_pong]);
...
__aicore__ inline void LstmBidirFP16::BatchTransCast(const GlobalTensor<float>& ZNGlobal,
                                                     const GlobalTensor<half>& NDGlobal, uint32_t dstStep,
                                                     uint32_t srcStep, uint32_t batchTailM, uint32_t batch,
                                                     uint32_t loop, uint32_t tailCoreNum, TransInnerParams& innerParams,
                                                     bool isND2ZN)
{
    uint32_t offsetM = (loop * block_id + (block_id < tailCoreNum ? block_id : tailCoreNum));
    loop += static_cast<uint32_t>(block_id < tailCoreNum);
    ...
    for (int i = offsetM; i < (offsetM + loop); i++) {
        ...
        for (int j = 0; j < innerParams.innerLoop; j++) {
            ...
            if (isND2ZN) {
                ...
                Cast<float, half, false>(ubTransOut_, ubTransOut[0], RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
                                         unaryParamsH2F);
                SetFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
                WaitFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
                ...
                SetVectorMask<half, MaskMode::COUNTER>(0, realN * NUM_PER_BLOCK_FLOAT16);
                Cast<half, float, false>(ubTransOut[0], ubTransOut_, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
                                         unaryParamsF2H);
                PipeBarrier<PIPE_V>();
                SetFlag<HardEvent::V_MTE2>(eventIdVToMTE2[0]);
                WaitFlag<HardEvent::V_MTE2>(eventIdVToMTE2[0]);
...
{
    if (tilingData->inputSizeAligned != tilingData->inputSize) {
        PadCopy(workspaceTmpGM, xGm, tilingData->padInputLoop, tilingData->padInputTailM,
                tilingData->padInOutTailCoreNum, false, false, false, inputPadInnerParams);
        LstmSyncAll();
        BatchTrans(workspaceInGm, workspaceTmpGM, tilingData->workspaceInOutStep,
                   tilingData->batchSize * tilingData->inputSizeAligned, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
                   tilingData->transInOutLoopBatch, tilingData->transInOutLoop, tilingData->transInOutTailCoreNum,
                   false, inputND2ZNInnerParams, true, tilingData->packed, tilingData->isSeq);
        LstmSyncAll();
        PadCopy(workspaceTmpGM, wIHGm, tilingData->padWeightIHLoop, tilingData->padWeightIHTailM,
                tilingData->padWeightTailCoreNum, true, false, false, inputPadInnerParams);
        LstmSyncAll();
        BatchTrans(workspaceWihGM, workspaceTmpGM, tilingData->workspaceWihN,
                   tilingData->hiddenSize * tilingData->inputSizeAligned,
                   tilingData->hiddenSize % NUM_PER_BLOCK_FLOAT16, tilingData->transWeightLoopBatch,
                   tilingData->transWeightLoop, tilingData->transWeightTailCoreNum, false, inputND2ZNInnerParams, true);
    } else {
        BatchTrans(workspaceInGm, xGm, tilingData->workspaceInOutStep,
                   tilingData->batchSize * tilingData->inputSizeAligned, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
                   tilingData->transInOutLoopBatch, tilingData->transInOutLoop, tilingData->transInOutTailCoreNum,
                   false, inputND2ZNInnerParams, true, tilingData->packed, tilingData->isSeq);
        BatchTrans(workspaceWihGM, wIHGm, tilingData->workspaceWihN,
                   tilingData->hiddenSize * tilingData->inputSizeAligned,
                   tilingData->hiddenSize % NUM_PER_BLOCK_FLOAT16, tilingData->transWeightLoopBatch,
                   tilingData->transWeightLoop, tilingData->transWeightTailCoreNum, false, inputND2ZNInnerParams, true);
    }
    LstmSyncAll();
    if (tilingData->hiddenSizeAligned != tilingData->hiddenSize) {
        PadCopy(workspaceTmpGM, wHHGm, tilingData->padWeightHHLoop, tilingData->padWeightHHTailM,
                tilingData->padWeightTailCoreNum, true, false, false, hiddenPadInnerParams);
        LstmSyncAll();
        BatchTrans(workspaceWhhGM, workspaceTmpGM, tilingData->workspaceWhhN,
                   tilingData->hiddenSize * tilingData->hiddenSizeAligned,
                   tilingData->hiddenSize % NUM_PER_BLOCK_FLOAT16, tilingData->transWeightLoopBatch,
                   tilingData->transWeightLoop, tilingData->transWeightTailCoreNum, false, hiddenND2ZNInnerParams,
                   true);
        LstmSyncAll();
        PadCopy(workspaceTmpGM, initHGm, tilingData->padInitLoop, tilingData->padInitTailM,
                tilingData->padInitTailCoreNum, false, false, false, hiddenPadInnerParams);
        LstmSyncAll();
        BatchTrans(workspaceInitHGm, workspaceTmpGM, NONESENSE, NONESENSE,
                   tilingData->batchSize % NUM_PER_BLOCK_FLOAT16, tilingData->transInOutLoopBatch,
                   tilingData->transInitLoop, tilingData->transInitTailCoreNum, false, hiddenND2ZNInnerParams, true);
        LstmSyncAll();
        PadCopy(workspaceTmpGM, initCGm, tilingData->padInitLoop, tilingData->padInitTailM,
                tilingData->padInitTailCoreNum, true, false, false, hiddenPadInnerParams);
        LstmSyncAll();
        BatchTransCast(workspaceCGm, workspaceTmpGM, NONESENSE, NONESENSE,
                       tilingData->batchSize % NUM_PER_BLOCK_FLOAT16, tilingData->transInOutLoopBatch,
                       tilingData->transInitLoop, tilingData->transInitTailCoreNum, hiddenND2ZNInnerParams, true);
    } else {
        BatchTrans(
            workspaceWhhGM, wHHGm, tilingData->workspaceWhhN, tilingData->hiddenSize * tilingData->hiddenSizeAligned,
            tilingData->hiddenSize % NUM_PER_BLOCK_FLOAT16, tilingData->transWeightLoopBatch,
            tilingData->transWeightLoop, tilingData->transWeightTailCoreNum, false, hiddenND2ZNInnerParams, true);
        BatchTrans(workspaceInitHGm, initHGm, NONESENSE, NONESENSE, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
                   tilingData->transInOutLoopBatch, tilingData->transInitLoop, tilingData->transInitTailCoreNum, false,
                   hiddenND2ZNInnerParams, true);
        BatchTransCast(workspaceCGm, initCGm, NONESENSE, NONESENSE, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
    ...
    if (tilingData->isBias != 0 && block_id % NUM_MM_GROUP == 0) {
        PadCopyAndCastBias(workspaceBiasGM, bIHGm, tilingData->hiddenSize, tilingData->padHiddenMask,
                           hiddenND2ZNInnerParams);
        SetAtomicAdd<float>();
        PadCopyAndCastBias(workspaceBiasGM, bHHGm, tilingData->hiddenSize, tilingData->padHiddenMask,
                           hiddenND2ZNInnerParams);
        SetAtomicNone();
    }
    LstmSyncAll();
...
__aicore__ inline void LstmBidirFP16::ClearBeforeUnpadCopy(const GlobalTensor<half>& dstGlobal, uint32_t offset,
                                                           uint32_t loop, uint32_t baseN, uint32_t tailN,
                                                           uint32_t tailCoreNum)
{
    ...
    Duplicate<half, false>(ubPad[0], static_cast<half>(0), MASK_PLACEHOLDER, 1, DEFAULT_BLK_STRIDE,
                           DEFAULT_REPEAT_STRIDE);
    SetFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
    WaitFlag<HardEvent::V_MTE3>(eventIdVToMTE3[0]);
    SetMaskNorm();
...
{
    if (tilingData->hiddenSizeAligned != tilingData->hiddenSize) {
        ClearBeforeUnpadCopy(yGm, clearInOutOffset, tilingData->clearInOutLoop, tilingData->padBaseN,
                             tilingData->clearInOutTailN, tilingData->clearInOutTailCoreNum);
        ClearBeforeUnpadCopy(outHGm, clearInitOffset, tilingData->clearInitLoop, tilingData->padBaseN,
                             tilingData->clearInitTailN, tilingData->clearInitTailCoreNum);
        ClearBeforeUnpadCopy(outCGm, clearInitOffset, tilingData->clearInitLoop, tilingData->padBaseN,
                             tilingData->clearInitTailN, tilingData->clearInitTailCoreNum);
        LstmSyncAll();
    }
}
...
{
    if (tilingData->hiddenSizeAligned != tilingData->hiddenSize) {
        BatchTrans(workspaceInGm, workspaceTmpGM, tilingData->batchSize * tilingData->hiddenSizeAligned,
                   tilingData->workspaceInOutStep, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
                   tilingData->transInOutLoopBatch, tilingData->transInOutLoop, tilingData->transInOutTailCoreNum,
                   false, hiddenZN2NDInnerParams, false, tilingData->packed, tilingData->isSeq);
        LstmSyncAll();
        PadCopy(yGm, workspaceTmpGM, tilingData->padOutputLoop, tilingData->padOutputTailM,
                tilingData->padInOutTailCoreNum, true, true, tilingData->bidirection != 0, hiddenUnPadInnerParams);
        LstmSyncAll();
        BatchTrans(workspaceOutputHGm, workspaceTmpGM, NONESENSE, NONESENSE,
                   tilingData->batchSize % NUM_PER_BLOCK_FLOAT16, tilingData->transInOutLoopBatch,
                   tilingData->transInitLoop, tilingData->transInitTailCoreNum, false, hiddenZN2NDInnerParams, false);
        LstmSyncAll();
        PadCopy(outHGm, workspaceTmpGM, tilingData->padInitLoop, tilingData->padInitTailM,
                tilingData->padInitTailCoreNum, true, true, false, hiddenUnPadInnerParams);
        LstmSyncAll();
        BatchTransCast(workspaceCGm, workspaceTmpGM, NONESENSE, NONESENSE,
                       tilingData->batchSize % NUM_PER_BLOCK_FLOAT16, tilingData->transInOutLoopBatch,
                       tilingData->transInitLoop, tilingData->transInitTailCoreNum, hiddenZN2NDInnerParams, false);
        LstmSyncAll();
        PadCopy(outCGm, workspaceTmpGM, tilingData->padInitLoop, tilingData->padInitTailM,
                tilingData->padInitTailCoreNum, true, true, false, hiddenUnPadInnerParams);
    } else {
        BatchTrans(workspaceInGm, yGm, tilingData->batchSize * tilingData->hiddenSizeAligned,
                   tilingData->workspaceInOutStep, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
                   tilingData->transInOutLoopBatch, tilingData->transInOutLoop, tilingData->transInOutTailCoreNum,
                   tilingData->bidirection != 0, hiddenZN2NDInnerParams, false, tilingData->packed, tilingData->isSeq);
        BatchTrans(workspaceOutputHGm, outHGm, NONESENSE, NONESENSE, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
                   tilingData->transInOutLoopBatch, tilingData->transInitLoop, tilingData->transInitTailCoreNum, false,
                   hiddenZN2NDInnerParams, false);
        BatchTransCast(workspaceCGm, outCGm, NONESENSE, NONESENSE, tilingData->batchSize % NUM_PER_BLOCK_FLOAT16,
                       tilingData->transInOutLoopBatch, tilingData->transInitLoop, tilingData->transInitTailCoreNum,
                       hiddenZN2NDInnerParams, false);
    }
    LstmSyncAll();

...
extern "C" __global__ __aicore__ void sigmoid_cross_entropy_with_logits_grad_v2(GM_ADDR predict, GM_ADDR target,
                                                                                ...
                                                                                GM_ADDR posWeight, GM_ADDR gradient,
                                                                                GM_ADDR workspace, GM_ADDR tiling);

class sigmoid_cross_entropy_with_logits_grad_v2_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "sigmoid_cross_entropy_with_logits_grad_v2_test SetUp\n" << endl; }
    static void TearDownTestCase() { cout << "sigmoid_cross_entropy_with_logits_grad_v2_test TearDown\n" << endl; }
};
...
TEST_F(sigmoid_cross_entropy_with_logits_grad_v2_test, test_case_fp32_1)
{
    ...
    *reinterpret_cast<float*>(tilingDatafromBin->scalarData) = 0.01f;
    auto KernelSigmoidCrossEntropyWithLogitsGradV2 = [](GM_ADDR predict, GM_ADDR target, GM_ADDR dout, GM_ADDR weight,
                                                        GM_ADDR posWeight, GM_ADDR gradient, GM_ADDR workspace,
                                                        GM_ADDR tiling) {
        ::sigmoid_cross_entropy_with_logits_grad_v2<201, 1, 1, 0>(predict, target, dout, weight, posWeight, gradient,
                                                                  workspace, tiling);
    };
    ICPU_SET_TILING_KEY(196615);
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(KernelSigmoidCrossEntropyWithLogitsGradV2, blockDim, predict, target, dout, weight, posWeight, gradient,

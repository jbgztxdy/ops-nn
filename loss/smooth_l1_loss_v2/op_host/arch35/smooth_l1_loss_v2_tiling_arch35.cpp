...
ge::graphStatus SmoothL1LossV2Tiling::CheckShape()
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(tilingContext->GetNodeName(), "input predict and label dtype not same"),
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        inputPShape != inputLShape,
        OP_LOGE(tilingContext->GetNodeName(), "input predict and label shape not same"),
        return ge::GRAPH_FAILED);
...
void SmoothL1LossV2Tiling::ConvertReduceOpTilingData(Ops::Base::ReduceOpTilingData* dst, const Ops::Base::ReduceOpTilingData* src)
{
    ...
    for (int32_t i = 0; i < Ops::Base::ReduceOpTmpl::MAX_DIM; i++) {
        ...
        dst->sliceNum[i] = src->sliceNum[i];
        dst->sliceShape[i] = src->sliceShape[i];
        dst->sliceStride[i] = src->sliceStride[i];
    }
}
...
ge::graphStatus SmoothL1LossV2Tiling::TilingReduce(const SmoothL1LossV2CompileInfo* compileInfo)
{
    ...
    Ops::Base::ReduceOpTilingData optiling;
    if (this->outputDtype == ge::DT_FLOAT16 || this->outputDtype == ge::DT_BF16) {
        if (static_cast<int32_t>(this->reduction) == 1) {
            OP_CHECK_IF(
                ...
                OP_LOGE(tilingContext->GetNodeName(), "SmoothL1LossV2 Tiling failed"),
                return ge::GRAPH_FAILED);
        } else {
            OP_CHECK_IF(
...
ge::graphStatus SmoothL1LossV2Tiling::RunTiling(const SmoothL1LossV2CompileInfo* compileInfo)
{
    ...
    OP_CHECK_IF(sigma <= 0,
                    OP_LOGE(tilingContext->GetNodeName(), "the value of sigma must be positive number."),
                    return ge::GRAPH_FAILED);
    tiling->Sigma = sigma;
    ...
    OP_CHECK_IF(iter == STR_2_INT.end(),
                    OP_LOGE(tilingContext->GetNodeName(),
                                                    "reduction %s is not support, should be one of {mean, sum, none}.",
                                                    reductionStr.c_str()),
                    return ge::GRAPH_FAILED);

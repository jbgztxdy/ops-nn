...
__global__ __aicore__ void ctc_loss_v2(GM_ADDR log_probs, GM_ADDR targets, GM_ADDR input_lengths,
                                       ...
                                       GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if constexpr (isFP32 == CTC_LOSS_V2_TPL_KEY_TRUE) {
        if constexpr (threadTypeInt32 == CTC_LOSS_V2_TPL_KEY_TRUE) {
            ...
            op.Init(log_probs, targets, input_lengths, target_lengths, neg_log_likelihood, log_alpha, workspace,
                    &tilingData, &pipe);
            op.Process();
        } else {

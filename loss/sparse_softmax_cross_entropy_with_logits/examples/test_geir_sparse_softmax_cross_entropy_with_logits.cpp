...
/*!
 * \file test_geir_sparse_softmax_cross_entropy_with_logits.cpp
 * \brief geir test for sparse_softmax_cross_entropy_with_logits
 */
    ...
    vector<int64_t> shapes, Tensor& input_tensor, TensorDesc& input_tensor_desc, int32_t num_classes)
{
    ...
    for (size_t i = 0; i < size; ++i) {
        *(pData + i) = static_cast<int32_t>(i % num_classes);
    }
    input_tensor = Tensor(input_tensor_desc, reinterpret_cast<uint8_t*>(pData), data_len);
    return SUCCESS;
}
    ...
    Graph& graph)
{
    ...
    TensorDesc placeholder1_desc = TensorDesc(ge::Shape(featuresShape), FORMAT_ND, inDtype);
    placeholder1_desc.SetPlacement(ge::kPlacementHost);
    placeholder1_desc.SetFormat(FORMAT_ND);
    Tensor tensor_placeholder1;
    ret = GenFloatData(featuresShape, tensor_placeholder1, placeholder1_desc, inDtype, 0.5);
    if (ret != SUCCESS) {
    ...
    sparseSoftmaxCrossEntropyWithLogits1.set_input_features(placeholder1);
    inputs.push_back(placeholder1);

    // labels：batch_size 的向量，取值范围 [0, num_classes)
    std::vector<int64_t> labelsShape = {batchSize};
    auto placeholder2 = op::Data("placeholder2").set_attr_index(1);
    TensorDesc placeholder2_desc = TensorDesc(ge::Shape(labelsShape), FORMAT_ND, ge::DT_INT32);
    placeholder2_desc.SetPlacement(ge::kPlacementHost);
    placeholder2_desc.SetFormat(FORMAT_ND);
    Tensor tensor_placeholder2;
    ret = GenInt32Data(labelsShape, tensor_placeholder2, placeholder2_desc, static_cast<int32_t>(numClasses));
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Generate input data failed\n", GetTime().c_str());
        return FAILED;
    }
    ...
    sparseSoftmaxCrossEntropyWithLogits1.set_input_labels(placeholder2);
    inputs.push_back(placeholder2);

    // loss：batch_size 的向量；backprop：batch_size * num_classes 的矩阵
    TensorDesc loss_desc = TensorDesc(ge::Shape(labelsShape), FORMAT_ND, inDtype);
    sparseSoftmaxCrossEntropyWithLogits1.update_output_desc_loss(loss_desc);
    TensorDesc backprop_desc = TensorDesc(ge::Shape(featuresShape), FORMAT_ND, inDtype);
    sparseSoftmaxCrossEntropyWithLogits1.update_output_desc_backprop(backprop_desc);

    outputs.push_back(sparseSoftmaxCrossEntropyWithLogits1);
    return SUCCESS;
}

...
* @li y_grad: A 1D or 3D tensor dtype of float32 or bfloat16 or float16 with shape (N) or (N, H, W) if reduction is
"none". When x is 2D(shape (N, C)), y_grad should be 1D(shape(N)) or scalar, when x is 4D(shape(N, C, H, W)), y_grad
should be 3d(shape(N, H, W)). Otherwise, shape is (1).
* @li target: Indicates the real label. A 1D or 3D tensor dtype of int32, int64 or uint8 with shape (N) or (N, H, W).
When x is 2D(shape (N, C)), target should be 1D(shape(N)) or scalar, when x is 4D(shape(N, C, H, W)), target should be
3d(shape(N, H, W)).
...
* @li ignore_index: Specifies a target value that is ignored and does not affect the input gradient. An optional int.
Defaults to -100.

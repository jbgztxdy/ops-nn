...
__aicore__ inline void CrossEntropyLossGradBase<T>::InitUB()
{
    ...
    inOutQueInputSize = this->alignColLoopNum > this->colLoopNumTail ? this->alignColLoopNum :
...
__aicore__ inline void CrossEntropyLossGradBase<T>::ComputeIgnoreMask(uint64_t targetNum)
{
    ...
    AscendC::Select(ignoreSelect, mask1, ignoreSelect, static_cast<float>(1), SELMODE::VSEL_TENSOR_SCALAR_MODE,
                    targetNum);
}

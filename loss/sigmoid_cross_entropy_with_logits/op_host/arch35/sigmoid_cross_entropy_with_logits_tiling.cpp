...
ge::graphStatus SigmoidCrossEntropyWithLogitsTiling::CalcInputDtype()
{
    ...
    OP_CHECK_IF(targetDtype != this->inputDtype_,
...
ge::graphStatus SigmoidCrossEntropyWithLogitsTiling::CalcOutputDtype()
{
    ...
    OP_CHECK_IF(this->outputDtype_ != this->inputDtype_,
...
ge::graphStatus SigmoidCrossEntropyWithLogitsTiling::DoElewiseTiling()
{
    ...
    if (this->inputDtype_ == ge::DT_FLOAT16) {
        dType_ = TPL_FP16;
        ret = eleBaseTiling
                  .DoTiling<SigmoidCrossEntropyWithLogitsOp::SigmoidCrossEntropyWithLogitsDagWithCast<half>::OpDag>(
                      *tiling);
    } else if (this->inputDtype_ == ge::DT_BF16) {
        dType_ = TPL_BF16;
        ret = eleBaseTiling.DoTiling<
            SigmoidCrossEntropyWithLogitsOp::SigmoidCrossEntropyWithLogitsDagWithCast<bfloat16_t>::OpDag>(*tiling);
    } else if (this->inputDtype_ == ge::DT_FLOAT) {
        dType_ = TPL_FP32;
        ret = eleBaseTiling
                  .DoTiling<SigmoidCrossEntropyWithLogitsOp::SigmoidCrossEntropyWithLogitsDagNoCast<float>::OpDag>(
                      *tiling);
    } else {
...
ge::graphStatus SigmoidCrossEntropyWithLogitsTiling::RunTiling()
{
    ...
    OP_CHECK_IF(CalcInputDtype() != ge::GRAPH_SUCCESS, OP_LOGE(tilingContext_->GetNodeName(), "CalcInputDtype failed."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(CalcOutputDtype() != ge::GRAPH_SUCCESS,
                OP_LOGE(tilingContext_->GetNodeName(), "CalcOutputDtype failed."), return ge::GRAPH_FAILED);
    OP_CHECK_IF(CheckShape() != ge::GRAPH_SUCCESS, OP_LOGE(tilingContext_->GetNodeName(), "CheckShape failed."),
                return ge::GRAPH_FAILED);

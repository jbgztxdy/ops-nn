...
@register("ascend_method_torch_nn_smooth_l1_loss_backward")
class MethodAclnnSmoothL1LossBackwardApi(BaseApi):
    def __init__(self, task_result: TaskResult):
        super(MethodAclnnSmoothL1LossBackwardApi, self).__init__(task_result)
        OpsDataset.seed_everything()
        self.change_flag = None

    def __call__(self, input_data: InputDataset, with_output: bool = False):
        self.gradOut_ = input_data.kwargs['gradOut']
        ...
        if self.device == "cpu" and not_fp32:
            self.gradOut_ = self.gradOut_.to(torch.float32)

                        ...
                        const aclTensor* gradTarget)
{
    ...
    if (op::IsPrivateFormat(gradOutput->GetStorageFormat()) || op::IsPrivateFormat(self->GetStorageFormat()) ||
        ...
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                "Format only support ND、NCHW、NHWC、HWCN、NDHWC、NCDHW、NCL. Actual: gradOutput [%s], self [%s], "
                "target [%s], gradTarget [%s] ",
...
static const aclTensor* ComputeGradForKlDiv(const aclTensor* targetBroadcast, const aclTensor* selfCasted,
                                            const aclTensor* gradOutputCasted, bool logTarget, int64_t reduction,
...
static aclnnStatus ExecuteKlDivTargetBackward(const aclTensor* gradOutput, const aclTensor* self,
                                              ...
                                              aclTensor* gradTarget, aclOpExecutor* executor)
{
    ...
    if (!(Ops::NN::AclnnUtil::IsRegbase() && gradOutput->GetDataType() == self->GetDataType() &&
          gradOutput->GetDataType() == target->GetDataType())) {
    ...
    if (Ops::NN::AclnnUtil::IsRegbase()) {
        grad = l0op::KlDivTargetLossGrad(gradOutputCasted, selfCasted, targetBroadcast, GetReductionStr(reduction),
                                         logTarget, executor);
    } else {
...
aclnnStatus aclnnKlDivTargetBackwardGetWorkspaceSize(const aclTensor* gradOutput, const aclTensor* self,
                                                     const aclTensor* target, int64_t reduction, bool logTarget,
                                                     aclTensor* gradTarget, uint64_t* workspaceSize,
                                                     aclOpExecutor** executor)
{
    OP_CHECK_COMM_INPUT(workspaceSize, executor);

    L2_DFX_PHASE_1(aclnnKlDivTargetBackward, DFX_IN(gradOutput, self, target, reduction, logTarget),
                   DFX_OUT(gradTarget));
    // 固定写法，创建OpExecutor
...
aclnnStatus aclnnKlDivTargetBackward(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                     aclrtStream stream)
{

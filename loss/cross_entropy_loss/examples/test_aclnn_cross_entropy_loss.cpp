...
int main()
{
    ...
    std::vector<int64_t> lossOutShape = {
        1,
    };
    std::vector<int64_t> logProbOutShape = {2, 5};
    std::vector<int64_t> zlossOutShape = {
        1,
    };
    std::vector<int64_t> lseForZlossOutShape = {
        2,
    };
    ...
    // data
    std::vector<float> inputHostData = {5, 0, 3, 3, 7, 9, 3, 5, 2, 4};
    std::vector<int64_t> targetHostData = {0, 0};
    std::vector<float> lossOutHostData = {1.0937543};
    std::vector<float> logProbOutHostData = {-2.159461,  -7.159461,  -4.159461,  -4.159461,  -0.159461,
    ...
    ret = CreateAclTensor(logProbOutHostData, logProbOutShape, &logProbOutDeviceAddr, aclDataType::ACL_FLOAT,
                          &logProbOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建zloss aclTensor
    ...
    ret = CreateAclTensor(lseForZlossOutHostData, lseForZlossOutShape, &lseForZlossDeviceAddr, aclDataType::ACL_FLOAT,
                          &lseForZloss);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnCrossEntropyLossGetWorkspaceSize(input, target, weight, reduction, ignoreIndex, labelSmoothing,
                                                lseSquareScaleForZloss, returnZloss, lossOut, logProbOut, zloss,
                                                lseForZloss, &workspaceSize, &executor);

    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnCrossEntropyLossGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnCrossEntropyLoss failed. ERROR: %d\n", ret); return ret);

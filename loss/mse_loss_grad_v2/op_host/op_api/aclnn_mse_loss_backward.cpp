...
static inline bool CheckDtypeValid(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* target,
                                   const aclTensor* out)
{
    const auto& supportList = GetDtypeSupportListV2(ASCEND910B_DTYPE_DTYPE_SUPPORT_LIST,
                                                 ...
                                                 uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    // 进行计算
    auto grad = l0op::MseLossGrad(gradOutputContiguous, selfContiguous, targetContiguous, REDUCTION_STR[reduction],
                                  uniqueExecutor.get());

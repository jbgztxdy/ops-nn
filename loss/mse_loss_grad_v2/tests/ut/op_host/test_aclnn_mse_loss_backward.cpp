...
protected:
    static void SetUpTestCase() { std::cout << "mse_loss_backward_test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "mse_loss_backward_test TearDown" << std::endl; }
};
...
TEST_F(l2_mse_loss_backward_test, aclnnMseLossBackward_18_aclnnMseLossBackward_diff_input_dtype)
{
    ...
    auto ut_2 = OP_API_UT(aclnnMseLossBackward, INPUT(targetDesc, selfDesc, gradOutputDesc, reduction),
                          OUTPUT(outDesc));
    // SAMPLE: only test GetWorkspaceSize

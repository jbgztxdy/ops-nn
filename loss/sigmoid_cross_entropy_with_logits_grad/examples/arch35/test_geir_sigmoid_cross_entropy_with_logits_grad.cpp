...
/*!
 * \file test_geir_sigmoid_cross_entropy_with_logits_grad.cpp
 * \brief geir test for sigmoid_cross_entropy_with_logits_grad
 */
...
#include "../../op_graph/sigmoid_cross_entropy_with_logits_grad_proto.h"
...
{
    Status ret = SUCCESS;
    auto sigmoidCrossEntropyWithLogitsGrad1 = op::SigmoidCrossEntropyWithLogitsGrad(
        "sigmoidCrossEntropyWithLogitsGrad1");
    ...
    sigmoidCrossEntropyWithLogitsGrad1.set_input_predict(placeholder1);
    ...
    sigmoidCrossEntropyWithLogitsGrad1.set_input_target(placeholder2);
    inputs.push_back(placeholder2);

    std::vector<int64_t> doutShape = {4, 2};
    auto placeholder3 = op::Data("placeholder3").set_attr_index(2);
    TensorDesc placeholder3_desc = TensorDesc(ge::Shape(doutShape), FORMAT_ND, inDtype);
    placeholder3_desc.SetPlacement(ge::kPlacementHost);
    placeholder3_desc.SetFormat(FORMAT_ND);
    Tensor tensor_placeholder3;
    ret = GenData(doutShape, tensor_placeholder3, placeholder3_desc, inDtype, 1.0);
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Generate input data failed\n", GetTime().c_str());
        return FAILED;
    }
    ...
    sigmoidCrossEntropyWithLogitsGrad1.set_input_dout(placeholder3);
    inputs.push_back(placeholder3);

    TensorDesc gradient_desc = TensorDesc(ge::Shape(predictShape), FORMAT_ND, inDtype);
    sigmoidCrossEntropyWithLogitsGrad1.update_output_desc_gradient(gradient_desc);

    outputs.push_back(sigmoidCrossEntropyWithLogitsGrad1);
    return SUCCESS;
}

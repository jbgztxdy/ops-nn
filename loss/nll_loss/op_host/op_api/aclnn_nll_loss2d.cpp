    ...
    aclTensor* out, aclTensor* totalWeightOut, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    auto targetReshape =
        l0op::Reshape(targetContiguous, uniqueExecutor.get()->AllocIntArray(tarReshapeList, 1), uniqueExecutor.get());
    ...
    if (reduction != 0) {
        ...
        loss = l0op::Reshape(lossOut[0], uniqueExecutor.get()->AllocIntArray(outSquence, dims), uniqueExecutor.get());

...
def logit_grad(x, dy, eps):
    data_type = x.dtype
    a = x.clone().detach().requires_grad_(True)
    if data_type == torch.float16:
        a = a.float()
    b = torch.special.logit(a, eps)
    b.backward(dy)  # 直接调用，不赋值
    grad = a.grad
    if grad is None:  # 处理梯度为None的情况
        grad = torch.zeros_like(a)
    if data_type == torch.float16:
        return grad.half()
    return grad

@register("function_active_logit_grad")

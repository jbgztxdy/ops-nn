...
int main()
{
    ...
    ret = aclnnMaxPoolWithArgmaxV3GetWorkspaceSize(x, kernelSizeArray, stridesArray, padsArray, dilationsArray,
                                                   ceilMode, y, argmax, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnMaxPoolWithArgmaxV3GetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy argmax result from device to host failed. ERROR: %d\n", ret);
              return ret);

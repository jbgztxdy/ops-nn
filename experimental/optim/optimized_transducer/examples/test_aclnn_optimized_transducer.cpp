...
#define CHECK_ACL(expr)                                                                                \
    do {                                                                                               \
        aclError ret = (expr);                                                                         \
        if (ret != ACL_SUCCESS) {                                                                      \
            std::cerr << "ACL Error " << ret << " at line " << __LINE__ << ": " << #expr << std::endl; \
            return -1;                                                                                 \
        }                                                                                              \
    } while (0)
...
int main(int argc, char* argv[])
{
    ...
    std::vector<int64_t> logitsStrides = {(int64_t)maxT * maxU * V, (int64_t)maxU * V, (int64_t)V, 1};

    aclTensor* logitsTensor = aclCreateTensor(logitsShape.data(), logitsShape.size(), ACL_FLOAT, logitsStrides.data(),
                                              0, ACL_FORMAT_ND, logitsShape.data(), logitsShape.size(), logitsDev);
    ...
    aclTensor* targetsTensor = aclCreateTensor(targetsShape.data(), targetsShape.size(), ACL_INT32,
                                               targetsStrides.data(), 0, ACL_FORMAT_ND, targetsShape.data(),
                                               targetsShape.size(), targetsDev);
    ...
    aclTensor* logitLensTensor = aclCreateTensor(lengthsShape.data(), lengthsShape.size(), ACL_INT32,
                                                 lengthsStrides.data(), 0, ACL_FORMAT_ND, lengthsShape.data(),
                                                 lengthsShape.size(), logitLensDev);
    CHECK_PTR(logitLensTensor, "Failed to create logitLengths tensor");

    aclTensor* targetLensTensor = aclCreateTensor(lengthsShape.data(), lengthsShape.size(), ACL_INT32,
                                                  lengthsStrides.data(), 0, ACL_FORMAT_ND, lengthsShape.data(),
                                                  lengthsShape.size(), targetLensDev);
    ...
    aclTensor* lossTensor = aclCreateTensor(lossShape.data(), lossShape.size(), ACL_FLOAT, lossStrides.data(), 0,
                                            ACL_FORMAT_ND, lossShape.data(), lossShape.size(), lossDev);
    CHECK_PTR(lossTensor, "Failed to create loss tensor");

    aclTensor* gradsTensor = aclCreateTensor(logitsShape.data(), logitsShape.size(), ACL_FLOAT, logitsStrides.data(), 0,
                                             ACL_FORMAT_ND, logitsShape.data(), logitsShape.size(), gradsDev);
    ...
    // 第一步: 获取workspace大小
    aclError ret = aclnnOptimizedTransducerGetWorkspaceSize(logitsTensor,      // logits

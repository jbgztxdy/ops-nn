                                                 ...
                                                 GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if constexpr (schMode == static_cast<uint32_t>(OptimizedTransducerTilingKeyEnum::TILING_KEY_FLOAT)) {
        ...
        op.Init(logits, targets, logitLengths, targetLengths, loss, grad, workspace,
            tilingData.maxT, tilingData.maxU, tilingData.V, tilingData.batch_size, tilingData.bigCoreNum, tilingData.bigCoreProcessNum, tilingData.smallCoreProcessNum,
            tilingData.blank, tilingData.clamp, tilingData.fused_log_softmax, tilingData.requires_grad);
        op.Process();
    }

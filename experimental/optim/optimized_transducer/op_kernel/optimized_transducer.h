...
public:
    ...
    __aicore__ inline void Init(GM_ADDR logits, GM_ADDR targets, GM_ADDR logitLengths, GM_ADDR targetLengths, GM_ADDR loss, GM_ADDR grad, GM_ADDR workspace, uint64_t maxT, uint64_t maxU, uint64_t V, uint64_t batch_size, uint64_t bigCoreNum, uint64_t bigCoreProcessNum, uint64_t smallCoreProcessNum, int blank, float clamp, bool fused_log_softmax, bool requires_grad);
...
template <typename T>
__aicore__ inline void OptimizedTransducer<T>::Init(GM_ADDR logits, GM_ADDR targets, GM_ADDR logitLengths, GM_ADDR targetLengths, GM_ADDR loss, GM_ADDR grad, GM_ADDR workspace, uint64_t maxT, uint64_t maxU, uint64_t V, uint64_t batch_size, uint64_t bigCoreNum, uint64_t bigCoreProcessNum, uint64_t smallCoreProcessNum, int blank, float clamp, bool fused_log_softmax, bool requires_grad)
{
    ...
    if constexpr (std::is_same_v<T, float>)
    {
        for (int32_t t = 0; t != len_T; t ++)
        {
            for (int32_t u = 0; u != len_U; u ++, idx_probs += 2)
            {
                ...
                if (u < len_U - 1)
                {
                    probsGm.SetValue(offset * 2 + idx_probs + 1, logits.GetValue(targetsGm.GetValue(idx * (maxU - 1) + u)) - res);
                }
            }
        }
    }
    else
    {
        ...
        for (int32_t t = 0; t != len_T; t ++)
        {
            for (int32_t u = 0; u != len_U; u ++, idx_probs += 2)
            {
                ...
                if (u < len_U - 1)
                {
                    probsGm.SetValue(offset * 2 + idx_probs + 1, logits_float.GetValue(targetsGm.GetValue(idx * (maxU - 1) + u)) - res);
                }
            }
        }
    }
    ...
    if constexpr (std::is_same_v<T, float>)
    {
        for (int32_t t = 0; t != len_T; t ++)
        {
            for (int32_t u = 0; u != len_U; u ++, idx_probs += 2)
            {
                ...
                if (u < len_U - 1)
                {
                    probsGm.SetValue(offset * 2 + idx_probs + 1, logitsGm.GetValue(offset_logits + targetsGm.GetValue(idx * (maxU - 1) + u)));
                }
            }
        }
    }
    else
    {
        for (int32_t t = 0; t != len_T; t ++)
        {
            for (int32_t u = 0; u != len_U; u ++, idx_probs += 2)
            {
                ...
                if (u < len_U - 1)
                {
                    probsGm.SetValue(offset * 2 + idx_probs + 1, (float)logitsGm.GetValue(offset_logits + targetsGm.GetValue(idx * (maxU - 1) + u)));
                }
            }
        }
    }
...
__aicore__ inline void OptimizedTransducer<T>::ComputeAlpha(uint64_t idx)
{
    ...
    {
        alphaGm.SetValue(offset + t * len_U, alphaGm.GetValue(offset + (t - 1) * len_U) + probsGm.GetValue(offset * 2 + (t - 1) * len_U * 2));
    }
    ...
    if (len_T < len_U)
    {
        for (int32_t n = 2; n < len_T; n ++)
        {
            ...
            {
                int u = n - t;
                buf2.SetValue(t - 1, alphaGm.GetValue(offset + (t - 1) * len_U + u) + probsGm.GetValue(offset * 2 + ((t - 1) * len_U + u) * 2));
                buf3.SetValue(t - 1, alphaGm.GetValue(offset + t * len_U + u - 1) + probsGm.GetValue(offset * 2 + (t * len_U + u - 1) * 2 + 1));
            }
        ...
        for (int32_t n = len_U; n < len_T + len_U - 1; n ++)
        {
            ...
            {
                int u = n - t;
                buf2.SetValue(count, alphaGm.GetValue(offset + (t - 1) * len_U + u) + probsGm.GetValue(offset * 2 + ((t - 1) * len_U + u) * 2));
                buf3.SetValue(count, alphaGm.GetValue(offset + t * len_U + u - 1) + probsGm.GetValue(offset * 2 + (t * len_U + u - 1) * 2 + 1));
            }
    ...
    else
    {
        ...
        for (int32_t n = len_U; n < len_T; n ++)
        {
            ...
            {
                int t = n - u;
                buf2.SetValue(u - 1, alphaGm.GetValue(offset + (t - 1) * len_U + u) + probsGm.GetValue(offset * 2 + ((t - 1) * len_U + u) * 2));
                buf3.SetValue(u - 1, alphaGm.GetValue(offset + t * len_U + u - 1) + probsGm.GetValue(offset * 2 + (t * len_U + u - 1) * 2 + 1));
            }
...
__aicore__ inline void OptimizedTransducer<T>::ComputeBeta(uint64_t idx)
{
    ...
    {
        betaGm.SetValue(offset + t * len_U + len_U - 1, betaGm.GetValue(offset + (t + 1) * len_U + len_U - 1) + probsGm.GetValue(offset * 2 + (t + 1) * len_U * 2 - 2));
    }
    for (int32_t u = len_U - 2; u >= 0; u --)
    {
        betaGm.SetValue(offset + (len_T - 1) * len_U + u, betaGm.GetValue(offset + (len_T - 1) * len_U + u + 1) + probsGm.GetValue(offset * 2 + ((len_T - 1) * len_U + u) * 2 + 1));
    }
    LocalTensor<float> buf2 = B2.Get<float>();
    LocalTensor<float> buf3 = B3.Get<float>();
    if (len_T < len_U)
    {
        for (int32_t n = len_T + len_U - 4, count = 1; n >= len_U - 1; n --, count ++)
        {
            ...
            {
                int u = n - t;
                buf2.SetValue(j, betaGm.GetValue(offset + (t + 1) * len_U + u) + probsGm.GetValue(offset * 2 + (t * len_U + u) * 2));
                buf3.SetValue(j, betaGm.GetValue(offset + t * len_U + u + 1) + probsGm.GetValue(offset * 2 + (t * len_U + u) * 2 + 1));
            }
            LogAdd(buf2, buf3, count);
            for (int32_t t = len_T - 2, j = 0; j < count; j ++, t --)
            {
        ...
        for (int32_t n = len_U - 2; n >= len_T - 1; n --)
        {
            ...
            {
                int u = n - t;
                buf2.SetValue(t, betaGm.GetValue(offset + (t + 1) * len_U + u) + probsGm.GetValue(offset * 2 + (t * len_U + u) * 2));
                buf3.SetValue(t, betaGm.GetValue(offset + t * len_U + u + 1) + probsGm.GetValue(offset * 2 + (t * len_U + u) * 2 + 1));
            }
            LogAdd(buf2, buf3, len_T - 1);
            for (int32_t t = 0; t < len_T - 1; t ++)
            {
    ...
    else
    {
        ...
        for (int32_t n = len_T - 2; n >= len_U - 1; n --)
        {
            ...
            {
                int t = n - u;
                buf2.SetValue(u, betaGm.GetValue(offset + (t + 1) * len_U + u) + probsGm.GetValue(offset * 2 + (t * len_U + u) * 2));
                buf3.SetValue(u, betaGm.GetValue(offset + t * len_U + u + 1) + probsGm.GetValue(offset * 2 + (t * len_U + u) * 2 + 1));
            }
            LogAdd(buf2, buf3, len_U - 1);
            for (int32_t u = 0; u < len_U - 1; u ++)
            {
    ...
    if constexpr (std::is_same_v<T, float>)
    {
        for (int32_t t = 0; t != len_T; t ++)
        {
            for (int32_t u = 0; u != len_U; u ++)
            {
                ...
                if (t < len_T - 1)
                {
                    grad.SetValue(blank, grad.GetValue(blank) - exp(g_blank + betaGm.GetValue(offset + (t + 1) * len_U + u)));
                }
                if (u < len_U - 1)
                {
                    grad.SetValue(target_u, grad.GetValue(target_u) - exp(g_target + betaGm.GetValue(offset + t * len_U + u + 1)));
                }
                if (clamp > 0)
                {
    ...
    else
    {
        ...
        for (int32_t t = 0; t != len_T; t ++)
        {
            for (int32_t u = 0; u != len_U; u ++)
            {
                ...
                if (t < len_T - 1)
                {
                    logits_float.SetValue(blank, logits_float.GetValue(blank) - exp(g_blank + betaGm.GetValue(offset + (t + 1) * len_U + u)));
                }
                if (u < len_U - 1)
                {
                    logits_float.SetValue(target_u, logits_float.GetValue(target_u) - exp(g_target + betaGm.GetValue(offset + t * len_U + u + 1)));
                }
                if (clamp > 0)
                {
    ...
    else
    {
        for (int32_t t = 0; t != len_T; t ++)
        {
            for (int32_t u = 0; u != len_U; u ++)
            {
                ...
                if (u < len_U - 1)
                {
                    gradGm.SetValue(idx_logits + target_u, -exp(g_target + betaGm.GetValue(offset + t * len_U + u + 1)));
                }
            }
        }
    }

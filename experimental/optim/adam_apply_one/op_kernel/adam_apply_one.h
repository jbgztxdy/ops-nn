...
/*!
 * \file adam_apply_one.h
 * \brief
 * */
#ifndef ADAM_APPLY_ONE_H
#define ADAM_APPLY_ONE_H

#include "kernel_operator.h"
#include "kernel_tiling/kernel_tiling.h"
#include "adam_apply_one_tiling_key.h"
#include "adam_apply_one_tiling_data.h"
namespace NsAdamApplyOne {
...
template <typename T>
__aicore__ inline void AdamApplyOne<T>::CopyIn(int32_t progress)
{
...
template <typename T>
__aicore__ inline void AdamApplyOne<T>::CopyOut(int32_t progress)
{
...
template <typename T>
__aicore__ inline void AdamApplyOne<T>::Compute(int32_t progress)
{
    ...
    if constexpr (std::is_same_v<T, bfloat16_t>) { // bf16
        ...
        AscendC::Add(tLocal1, tLocal0, tLocal1, len); // tmp6
        AscendC::Div(tLocal1, tLocal3, tLocal1, len); // tmp7
        AscendC::Cast(tLocal2, input4Local, AscendC::RoundMode::CAST_NONE, len);
        AscendC::Mul(tLocal2, tLocal1, tLocal2, len); // tmp8
        AscendC::Cast(tLocal1, input3Local, AscendC::RoundMode::CAST_NONE, len);
        AscendC::Sub(tLocal2, tLocal1, tLocal2, len);
        ...
        AscendC::Mul(input1Local, input1Local, input4Local, len);  // tmp8
...
template <typename T>
__aicore__ inline void AdamApplyOne<T>::Process()
{

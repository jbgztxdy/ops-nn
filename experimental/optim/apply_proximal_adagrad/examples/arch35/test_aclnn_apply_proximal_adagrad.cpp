...
int main()
{
    ...
    float hostVar[ELEM_COUNT] = {
        // 覆盖 +/- / 0 / 较大值
        1.00f,  -1.00f, 0.50f,  -0.50f, 0.10f,  -0.10f, 0.00f,  2.00f,
        -2.00f, 0.05f,  -0.05f, 3.50f,  -3.50f, 0.01f,  -0.01f, 0.25f,
    };
    float hostAccum[ELEM_COUNT] = {
        // 必须非负（调用方保证）。覆盖小值 / 中等值。
        0.10f, 0.10f, 0.20f, 0.20f, 0.05f, 0.05f, 0.10f, 1.00f, 1.00f, 0.01f, 0.01f, 0.50f, 0.50f, 0.30f, 0.30f, 0.40f,
    };
    float hostGrad[ELEM_COUNT] = {
        0.10f, 0.10f, -0.20f, 0.20f, 0.30f,  -0.30f, 0.10f,  -0.50f,
        0.50f, 0.05f, -0.05f, 1.00f, -1.00f, 0.40f,  -0.40f, 0.15f,
    };
    float hostLr[1] = {0.1f};
    ...
    // =========================================================================
    {
        ...
        // =====================================================================
        tVar = aclCreateTensor(shape, ndim, ACL_FLOAT, strides, 0, ACL_FORMAT_ND, shape, ndim, devVar);
        tAccum = aclCreateTensor(shape, ndim, ACL_FLOAT, strides, 0, ACL_FORMAT_ND, shape, ndim, devAccum);
        tGrad = aclCreateTensor(shape, ndim, ACL_FLOAT, strides, 0, ACL_FORMAT_ND, shape, ndim, devGrad);
        tLr = aclCreateTensor(scalarShape, 1, ACL_FLOAT, scalarStrides, 0, ACL_FORMAT_ND, scalarShape, 1, devLr);
        tL1 = aclCreateTensor(scalarShape, 1, ACL_FLOAT, scalarStrides, 0, ACL_FORMAT_ND, scalarShape, 1, devL1);
        tL2 = aclCreateTensor(scalarShape, 1, ACL_FLOAT, scalarStrides, 0, ACL_FORMAT_ND, scalarShape, 1, devL2);
        // 占位输出复用同一 Device buffer，调用结束后通过 devVar / devAccum 直接读取
        tVarOut = aclCreateTensor(shape, ndim, ACL_FLOAT, strides, 0, ACL_FORMAT_ND, shape, ndim, devVar);
        tAccumOut = aclCreateTensor(shape, ndim, ACL_FLOAT, strides, 0, ACL_FORMAT_ND, shape, ndim, devAccum);
        ...
        CHECK_ACL(aclnnApplyProximalAdagradGetWorkspaceSize(tVar, tAccum, tLr, tL1, tL2, tGrad, tVarOut, tAccumOut,
        ...
        std::cout << "  scalars: lr=" << hostLr[0] << "  l1=" << hostL1[0] << "  l2=" << hostL2[0] << std::endl;
        ...
        for (int i = 0; i < ELEM_COUNT; ++i) {
            ...
            if (okA)
                ++passAccum;
            std::printf("  %4d | %10.5f | %10.5f | %10.5f | %10.5f | %10.5f | %s/%s\n", i, hostVar[i], hostAccum[i],
                        hostGrad[i], npuVar[i], goldVar[i], okV ? "PASS" : "FAIL", okA ? "PASS" : "FAIL");
        }
...
cleanup:
    ...
    if (tAccum)
        aclDestroyTensor(tAccum);
    if (tLr)
        aclDestroyTensor(tLr);
    if (tL1)
        aclDestroyTensor(tL1);
    if (tL2)
        aclDestroyTensor(tL2);
    if (tGrad)
        aclDestroyTensor(tGrad);
    if (tVarOut)
        aclDestroyTensor(tVarOut);
    if (tAccumOut)
        aclDestroyTensor(tAccumOut);
    if (workspace)
        aclrtFree(workspace);
    if (devVar)
        aclrtFree(devVar);

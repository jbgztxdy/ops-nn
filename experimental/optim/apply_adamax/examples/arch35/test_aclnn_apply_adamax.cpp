 ...
 *
 * @file test_aclnn_apply_adamax.cpp
 * @brief aclnnApplyAdamax 算子调用示例。
 *
 * 功能：Adamax 优化器单步参数更新（inplace）
 * 公式：
 *   m_new   = beta1 * m + (1 - beta1) * grad
 *   v_new   = max(beta2 * v, |grad|)
 *   var_new = var - lr / (1 - beta1Power) * m_new / (v_new + epsilon)
 *
 * 注意：本算子为 inplace 语义，varOut/mOut/vOut 分别与
 *      var/m/v 共享 Device 内存。
 */
#include <iostream>
#include <vector>
#include "acl/acl.h"
...
int main() {
  ...
  // 2. 构造输入与输出
  //    var/m/v/grad shape 必须完全一致，dtype 必须一致（fp16 或 fp32）。
  //    beta1Power/lr/beta1/beta2/epsilon 为 aclScalar。
  ...
  aclScalar* beta1Power = nullptr;
  aclScalar* lr = nullptr;
  aclScalar* beta1 = nullptr;
  aclScalar* beta2 = nullptr;
  ...
  float beta1PowerValue = 0.9f;
  float lrValue = 0.001f;
  float beta1Value = 0.9f;
  float beta2Value = 0.999f;
  float epsilonValue = 1e-8f;
  ...
  // 创建 Scalar 参数
  beta1Power = aclCreateScalar(&beta1PowerValue, aclDataType::ACL_FLOAT);
  lr = aclCreateScalar(&lrValue, aclDataType::ACL_FLOAT);
  beta1 = aclCreateScalar(&beta1Value, aclDataType::ACL_FLOAT);
  beta2 = aclCreateScalar(&beta2Value, aclDataType::ACL_FLOAT);
  ...
  ret = aclnnApplyAdamaxGetWorkspaceSize(var, m, v,
                                         beta1Power, lr, beta1, beta2, epsilon,
                                         grad, varOut, mOut, vOut,
                                         &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS,
            LOG_PRINT("aclnnApplyAdamaxGetWorkspaceSize failed. ERROR: %d\n", ret);
            return ret);
  ...
  ret = aclnnApplyAdamax(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnApplyAdamax failed. ERROR: %d\n", ret); return ret);
  ...
  std::vector<float> mResult(size, 0);
  std::vector<float> vResult(size, 0);
  ...
  ret = aclrtMemcpy(mResult.data(), mResult.size() * sizeof(float),
                    mDeviceAddr, size * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy m result failed. ERROR: %d\n", ret); return ret);
  ret = aclrtMemcpy(vResult.data(), vResult.size() * sizeof(float),
                    vDeviceAddr, size * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
  ...
  aclDestroyScalar(beta1Power);
  aclDestroyScalar(lr);
  aclDestroyScalar(beta1);
  aclDestroyScalar(beta2);

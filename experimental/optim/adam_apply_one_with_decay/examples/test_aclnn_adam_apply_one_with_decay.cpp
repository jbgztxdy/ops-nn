...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_adam_apply_one_with_decay.h"
...
int main()
{
    ...
    std::vector<float> mul4_xHostData = {0, 1, 2, 3, 4, 5, 6, 7};
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(mul4_xHostData, shape, &mul4_xDeviceAddr, aclDataType::ACL_FLOAT, &mul4_x);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnAdamApplyOneWithDecayGetWorkspaceSize(input0, input1, input2, input3, input4, mul0_x, mul1_x, mul2_x,
                                                     mul3_x, mul4_x, add2_y, output0, output1, output2, &workspaceSize,
                                                     &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAdamApplyOneWithDecayGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    ret = aclnnAdamApplyOneWithDecay(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAdamApplyOneWithDecay failed. ERROR: %d\n", ret); return ret);

...
aclTensor* MakeTensor(void* ptr, const std::vector<int64_t>& shape, aclDataType dt)
{
    std::vector<int64_t> strides(shape.size(), 1);
    for (int64_t i = static_cast<int64_t>(shape.size()) - 2; i >= 0; --i) {
        strides[i] = strides[i + 1] * shape[i + 1];
    }
    return aclCreateTensor(shape.empty() ? nullptr : shape.data(), shape.size(), dt,
...
Golden ComputeGolden(std::vector<float> var, std::vector<float> mg, std::vector<float> ms, std::vector<float> mom,
                     const std::vector<float>& grad, float lr, float rho, float momentum, float epsilon)
{
    size_t n = var.size();
...
bool Compare(const std::vector<float>& actual, const std::vector<float>& expect, const char* name)
{
...
int main()
{
    ...
    CHECK_ACLNN(aclnnApplyCenteredRMSPropGetWorkspaceSize(t_var, t_mg, t_ms, t_mom, t_lr, t_rho, t_momc, t_eps, t_grad,
                                                          t_var_o, t_mg_o, t_ms_o, t_mom_o, &ws_size, &executor));

    void* ws_ptr = nullptr;
    if (ws_size > 0)
        ws_ptr = DevAlloc(ws_size);
    ...
    // ---- 7. Cleanup ----
    for (aclTensor* t :
         {t_var, t_mg, t_ms, t_mom, t_lr, t_rho, t_momc, t_eps, t_grad, t_var_o, t_mg_o, t_ms_o, t_mom_o}) {
        if (t)
            aclDestroyTensor(t);
    }
    if (ws_ptr)

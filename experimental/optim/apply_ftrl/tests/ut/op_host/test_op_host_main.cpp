...
// op_host UT 全局环境：运行时加载 lib<op>_op_host_ut_lib.so（含 ApplyFtrl 的
// def/infershape/tiling 注册代码），并注册到全局 V2 SpaceRegistry，供 tiling /
// infershape executor 经 GetOpImpl("ApplyFtrl") 查找。参考 ops-math / apply_rms_prop。
...
public:
    ...
    virtual void SetUp() {
        ...
        string opHostSoPath =
            currDir.parent_path().string() + string("/libapply_ftrl_op_host_ut_lib.so");
        cout << "Loading op_host .so from: " << opHostSoPath << endl;

        gert::OppSoDesc oppSoDesc({ge::AscendString(opHostSoPath.c_str())}, "op_host_so");

        shared_ptr<gert::OpImplSpaceRegistryV2> opImplSpaceRegistryV2 =
            make_shared<gert::OpImplSpaceRegistryV2>();

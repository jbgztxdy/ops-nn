...
/*!
 * \file test_geir_apply_ftrl.cpp
 * \brief ApplyFtrl GE 图模式构图调用样例（experimental / ascend910b 适配）。
 *
 * ApplyFtrl 是 GE 图模式 / TensorFlow 算子（对标 tf.raw_ops.ApplyFtrl），无 CANN 已部署
 * aclnn 接口、无 PyTorch 等价。本样例通过 GE IR REG_OP(ApplyFtrl)（复用主线 proto
 * optim/apply_ftrl/op_graph/apply_ftrl_proto.h）构图并 RunGraph 下发。
 *
 * 与主线 optim/apply_ftrl/examples/test_geir_apply_ftrl.cpp 同构，按
 *   var → accum → linear → grad → lr → l1 → l2 → lr_power
 * 串接 8 个 Data 输入（4 张量同 shape；4 标量 shape {1}），属性 use_locking=false。
 *
 * 注（experimental 适配）：本实验算子复用同名 OpType ApplyFtrl 并补 ascend910b（DAV_2201）
 * 原生 kernel；与主线 ascend950（arch35）通过 ENABLE_EXPERIMENTAL 互斥构建。RunGraph 会下发
 * 到当前 OPP 包内已注册的 ApplyFtrl kernel——因此运行本样例前需先把目标算子编译安装进 OPP
 * （build.sh --pkg --experimental --soc=ascend910b --ops=apply_ftrl 并安装生成的 .run 包）。
 * 若需直接验证本实验 kernel 的数值正确性而不依赖 OPP 安装，请使用同目录的 kernel 直调样例
 * test_kernel_direct_apply_ftrl.asc（已在 ascend910b 真实 NPU 验证通过）。
 */
...
#include "../../../../optim/apply_ftrl/op_graph/apply_ftrl_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                             \
    ...
    applyFtrl1.set_input_##intputName(placeholder##intputIndex);                                                \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) applyFtrl1.set_attr_##attrName(attrValue)
...
{
    Status ret = SUCCESS;
    auto applyFtrl1 = op::ApplyFtrl("applyFtrl1");
    std::vector<int64_t> xShape = {2, 30};
    // 8 输入：4 张量同 shape {2,30}，4 标量 shape {1}。顺序与 proto / GE IR 严格一致。
    ADD_INPUT(1, var, inDtype, xShape);
    ADD_INPUT(2, accum, inDtype, xShape);
    ADD_INPUT(3, linear, inDtype, xShape);
    ADD_INPUT(4, grad, inDtype, xShape);
    ADD_INPUT(5, lr, inDtype, std::vector<int64_t>{1});
    ADD_INPUT(6, l1, inDtype, std::vector<int64_t>{1});
    ADD_INPUT(7, l2, inDtype, std::vector<int64_t>{1});
    ADD_INPUT(8, lr_power, inDtype, std::vector<int64_t>{1});
    ADD_INPUT_ATTR(use_locking, false);

    outputs.push_back(applyFtrl1);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    // argv[1] 可选：仅打印，dtype 默认 FLOAT16（与主线样例一致）。改 fp32/bf16 可调 inDtype。
    if (argc > 1) {

    ...
    int64_t batchDims, bool negativeIndexSupport)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(
        GatherV2, OP_INPUT(self, indices, axisTensor), OP_OUTPUT(out), OP_ATTR(batchDims, negativeIndexSupport));
    OP_CHECK(
        ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "GatherV2AiCore add to launcher failed."),
        return nullptr);
    return out;
}

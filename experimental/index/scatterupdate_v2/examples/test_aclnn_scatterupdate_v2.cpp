...
#include <vector>
#include "acl/acl.h"
#include "aclnn_scatterupdate_v2.h"
...
int main()
{
    ...
    ret = CreateAclTensor(selfXHostData, selfXShape, &selfXDeviceAddr, aclDataType::ACL_INT32, &xout);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    LOG_PRINT("aclnnScatterupdateV2GetWorkspaceSize returned: %d\n", 888);
    ...
    // 5. 调用aclnnScatterupdateV2第二段接口
    ret = aclnnScatterupdateV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnScatterupdateV2 failed. ERROR: %d\n", ret); return ret);
    ...
    std::vector<int64_t> outShape0 = {120};
    PrintOutResult(selfXShape, &selfXDeviceAddr);

...
static bool IsAiCoreSupport(const aclTensor* x)
{
    ...
    OP_CHECK(npuArch == NpuArch::DAV_3510,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "DataFormatDimMap not supported on this platform: npuArch=%d.",
                     static_cast<int>(npuArch)),
             return false);
    OP_CHECK(
        CheckType(x->GetDataType(), AICORE_DTYPE_SUPPORT_LIST),
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "DataFormatDimMap not supported: dtype=%d. Supported dtypes: INT32, INT64.",
                static_cast<int>(x->GetDataType())),
        return false);
    return true;
}

static const aclTensor* DataFormatDimMapAiCore(const aclTensor* x, const char* srcFormat, const char* dstFormat,
                                               const aclTensor* out, aclOpExecutor* executor)
{
    L0_DFX(DataFormatDimMapAiCore, x, out);

    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(DataFormatDimMap, OP_INPUT(x), OP_OUTPUT(out),
                                           OP_ATTR(srcFormat, dstFormat));
    OP_CHECK(ret == ACLNN_SUCCESS, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "DataFormatDimMapAiCore failed."), return nullptr);
    return out;
}

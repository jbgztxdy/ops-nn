...
/*!
 * \file fused_add_rms_norm_split_d.h
 * \brief add rms norm split d file
 */
#ifndef FUSED_ADD_RMS_NORM_SPLIT_D_H_
#define FUSED_ADD_RMS_NORM_SPLIT_D_H_
...
template <typename T>
class KernelFusedAddRmsNormSplitD {
public:
    __aicore__ inline KernelFusedAddRmsNormSplitD(TPipe* pipe)
    {
        Ppipe = pipe;
    }
...
private:
    ...
    {
        LocalTensor<float> x_fp32 = xFp32Buf.Get<float>();
        float rstdValue = ReadLocalFloat(rstdLocal, i_i_idx);
        PipeBarrier<PIPE_V>();
...
#endif  // _FUSED_ADD_RMS_NORM_SPLIT_D_H_

...
/*!
 * \file fused_add_rms_norm_multi_n.h
 * \brief add rms norm multi n file
 */
#ifndef FUSED_ADD_RMS_NORM_MULTI_N_H_
#define FUSED_ADD_RMS_NORM_MULTI_N_H_
...
template <typename T>
class KernelFusedAddRmsNormMultiN {
public:
    __aicore__ inline KernelFusedAddRmsNormMultiN(TPipe* pipe)
    {
        Ppipe = pipe;
    }
...
private:
    ...
    __aicore__ inline void ComputeRstd(LocalTensor<T> xLocal, LocalTensor<float> rstdLocal, uint32_t calc_row_num)
    {
        ...
        if (likely(repeatTimes > 0)) {
            Div(
                rstdLocal, reduce_buf_local, rstdLocal, NUM_PER_REP_FP32, repeatTimes,
                {1, 0, 1, DEFAULT_REPEAT_STRIDE, 0, DEFAULT_REPEAT_STRIDE});
        }
        if (unlikely(tailCount != 0)) {
            Div(
                rstdLocal[bodyCount], reduce_buf_local, rstdLocal[bodyCount], tailCount, 1,
                {1, 0, 1, DEFAULT_REPEAT_STRIDE, 0, DEFAULT_REPEAT_STRIDE});
        }
        PipeBarrier<PIPE_V>();
    }

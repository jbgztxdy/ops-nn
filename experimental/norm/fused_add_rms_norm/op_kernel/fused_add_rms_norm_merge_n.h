...
/*!
 * \file fused_add_rms_norm_merge_n.h
 * \brief add rms norm merge n file
 */
#ifndef FUSED_ADD_RMS_NORM_MERGE_N_H_
#define FUSED_ADD_RMS_NORM_MERGE_N_H_
...
template <typename T>
class KernelFusedAddRmsNormMergeN {
public:
    __aicore__ inline KernelFusedAddRmsNormMergeN(TPipe* pipe)
    {
        Ppipe = pipe;
    }
    ...
    {
        FUSED_ADD_RMS_NORM_INIT_MULTI_ROW_COMMON(tiling, x1, x2, gamma, y, rstd, x);
...
private:
    ...
    __aicore__ inline void ComputeRstd(
        LocalTensor<T> xLocal, LocalTensor<float> rstdLocal, uint32_t calc_row_num, uint32_t elementNum)
    {
    ...
    __aicore__ inline void ComputeY(
        LocalTensor<T> xLocal, LocalTensor<T> gammaLocal, LocalTensor<float> rstdLocal,
        uint32_t calc_row_num, uint32_t elementNum)
    {
        ...
        if (rowRepeatTail1 > 0) {
            Brcb(
                tmpLocal[rowRepeatLoop1 * splidRow * MOV_8], rstdLocal[rowRepeatLoop1 * splidRow],
                rowRepeatTail1, {1, 8});
            PipeBarrier<PIPE_V>();
        }
    ...
    template <typename U>
    __aicore__ inline void repeatByRow(
        const LocalTensor<U>& dstLocal, const LocalTensor<U>& src1Local, const LocalTensor<U>& src2Local,
        uint32_t calc_row_num, uint32_t type)
    {
        ...
        for (uint32_t r_i = 0; r_i < rowRepeatLoop; r_i++) {
            ...
            mulRepeat<U>(
                dstLocal[r_i * singlT * numColAlign], src1Local[r_i * singlT * numColAlign],
    ...
    template <typename U>
    __aicore__ inline void mulRepeat(
        const LocalTensor<U>& dstLocal, const LocalTensor<U>& src1Local, const LocalTensor<U>& src2Local,
        uint32_t calcRowNum, uint32_t strideParams[6])
    {
        ...
        if (src1BlkStride == 0) {
            for (uint32_t m_i = 0; m_i < mulLoop; m_i++) {
                Mul(
                    dstLocal[m_i * strideNum], src1Local[m_i * strideNum], src2Local, strideNum, calcRowNum,
                    {1, 1, src1BlkStride, dstRepStride, dstRepStride, src1RepStride});
            }
            PipeBarrier<PIPE_V>();
            if (mulTail > 0) {
                Mul(
                    dstLocal[mulLoop * strideNum], src1Local[mulLoop * strideNum], src2Local, mulTail,
            ...
            for (uint32_t m_i = 0; m_i < mulLoop; m_i++) {
                Mul(
                    dstLocal[m_i * strideNum], src1Local[m_i * strideNum],
                    src2Local[m_i * strideNum], strideNum, calcRowNum,
                    {1, 1, src1BlkStride, dstRepStride, dstRepStride, src1RepStride});
            }
            PipeBarrier<PIPE_V>();
            if (mulTail > 0) {
                Mul(
                    dstLocal[mulLoop * strideNum], src1Local[mulLoop * strideNum],
                    src2Local[mulLoop * strideNum], mulTail, calcRowNum,
                    {1, 1, src1BlkStride, dstRepStride, dstRepStride, src1RepStride});
            }
            PipeBarrier<PIPE_V>();
        }
...
#endif  // _FUSED_ADD_RMS_NORM_MERGE_N_H_

...
#ifndef FAST_HADAMARD_COMMON_INTCVT4_HPP
#define FAST_HADAMARD_COMMON_INTCVT4_HPP
                                        ...
                                        unsigned dstRepeatStride, unsigned srcRepeatStride)
{
    ...
    if (numRemainPerLine > 0) {
        ...
        for (uint32_t j = 0; j < numLoop; j++) {
            GenCastCallFp16ToInt4<TileDataD, TileDataS>(
                (__ubuf__ void*)(dstPtr + j * DS * REPEAT_MAX), srcPtr + j * SS * REPEAT_MAX, (uint8_t)REPEAT_MAX, mode,
                1, 1, (uint16_t)DS / dstNElemPerBlock, (uint16_t)SS / srcNElemPerBlock);
        }
        if (remainAfterLoop > 0) {
            GenCastCallFp16ToInt4<TileDataD, TileDataS>(
                (__ubuf__ void*)(dstPtr + numLoop * DS * REPEAT_MAX), srcPtr + numLoop * SS * REPEAT_MAX,
                (uint8_t)remainAfterLoop, mode, 1, 1, (uint16_t)DS / dstNElemPerBlock, (uint16_t)SS / srcNElemPerBlock);
        }
        set_vector_mask(-1, -1);
    }
...
PTO_INTERNAL void TCVT_FP16_TO_INT4_PACKED_IMPL(TileDataD& dst, TileDataS& src, RoundMode mode)
{
    ...
    TCvtFp16ToInt4Packed<TileDataD, TileDataS, SS, DS>(dst.data(), src.data(), mode, numRepeatPerLine, numRemainPerLine,
                                                       validRow, srcElementsPerRepeat, dstBytesPerRepeat,
...
#endif // FAST_HADAMARD_COMMON_INTCVT4_HPP

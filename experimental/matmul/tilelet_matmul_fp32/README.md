...
├── op_host
│   ├── CMakeLists.txt                    // 编译工程文件
│   ├── tilelet_matmul_fp32_def.cpp               // 算子信息库，定义算子基本信息，如名称、输入输出、数据类型等
│   ├── tilelet_matmul_fp32_infershape.cpp        // 算子形状推导，在运行时推导输出shape
│   └── tilelet_matmul_fp32_tiling.cpp            // Tiling实现，将张量划分为多个小块，区分数据类型进行并行计算
├── op_kernel
│   ├── tilelet_matmul_fp32.cpp                   // Kernel入口文件，包含主函数和调度逻辑
│   ├── tilelet_matmul_fp32_al1_fullload_kernel.h  // AL1全载模板Kernel实现文件
│   ├── tilelet_matmul_fp32_base_block.h           // 基本模板block层计算文件
│   ├── tilelet_matmul_fp32_base_kernel.h          // 基础模板Kernel实现文件
│   ├── tilelet_matmul_fp32_common.h               // Kernel模板公共头文件
│   ├── tilelet_matmul_fp32_tiling_data.h          // Tilingdata文件，存储Tiling策略相关的配置数据，如块大小等
│   └── tilelet_matmul_fp32_tiling_key.h           // Tilingkey文件，定义Tiling策略的Key,标识不同的划分方式                     
└── tests                                  // UT实现
...
- 算子规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">tilelet_matmul_fp32</td></tr>
  </table>
...
```bash
cd ${git_clone_path}/experimental/matmul/tilelet_matmul_fp32
```
...
cd ${git_clone_path}  
# 编译样例算子run包
bash build.sh --pkg  --soc=ascend910b --vendor_name=custom --ops=tilelet_matmul_fp32 --experimental 
#安装自定义算子run包
...
```bash
# 切换tilelet_matmul_fp32 aclnn执行用例目录
cd ${git_clone_path}/experimental/matmul/tilelet_matmul_fp32/examples
# 编译+执行aclnn接口+采集性能数据
bash run.sh
# 切换aclnn用例性能数据目录
cd ${git_clone_path}/experimental/matmul/tilelet_matmul_fp32/examples/output/msprof_result
```

...
/* !
 * \file tilelet_matmul_fp32_bl1_fullload_kernel.h
 * \brief
 */
#ifndef __TILELET_MATMUL_FP32_BL1_FULLLOAD_KERNEL_H__
#define __TILELET_MATMUL_FP32_BL1_FULLLOAD_KERNEL_H__
...
class TileletMatmulFp32BL1FullLoadKernel {
public:
    __aicore__ inline TileletMatmulFp32BL1FullLoadKernel(){};

    // arenaGM/peerOutGM/peerDsignalGM are accepted for signature parity with the
    // base kernel (cross-card path); the BL1 full-load path is single-card and
    // ignores them.
    __aicore__ inline void Init(GM_ADDR aGM, GM_ADDR bGM, GM_ADDR cGM, GM_ADDR biasGM, GM_ADDR workspaceGM,
                                GM_ADDR arenaGM, GM_ADDR peerOutGM, GM_ADDR peerDsignalGM, const void* tilingData,
                                TPipe* pipe);
...
private:
    __aicore__ inline void CopyInB1(const TileletMatmulFp32TilingData& matmulFp32TilingData, LocalTensor<B_T> bl1Local);
};
...
__aicore__ inline void TileletMatmulFp32BL1FullLoadKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::Init(
    ...
    GM_ADDR peerDsignalGM, const void* tilingData, TPipe* pipe)
{
    (void)arenaGM;
    (void)peerOutGM;
    (void)peerDsignalGM;
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void TileletMatmulFp32BL1FullLoadKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::Process(
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void TileletMatmulFp32BL1FullLoadKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::CopyInB1(
    const TileletMatmulFp32TilingData& matmulFp32TilingData, LocalTensor<B_T> bl1Local)
{
...
#endif // __TILELET_MATMUL_FP32_BL1_FULLLOAD_KERNEL_H__
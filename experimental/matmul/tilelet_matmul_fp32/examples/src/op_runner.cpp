...
int64_t GetEnvInt64(const char* name, int64_t defaultValue)
{
    ...
    return value == nullptr ? defaultValue : std::strtoll(value, nullptr, 10);
}
} // namespace
...
bool OpRunner::RunOp()
{
    ...
    // Single-card path: no cross-card arena/peer_out/peer_dsignal, role=0 (source), world=1.
    auto ret = aclnnTileletMatmulFp32GetWorkspaceSize(inputTensor_[0], inputTensor_[1], biasTensor,
                                               /*arena=*/nullptr, /*peer_out=*/nullptr, /*peer_dsignal=*/nullptr,
                                               transposeX1,
    ...
    INFO_LOG("Execute aclnnTileletMatmulFp32GetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnTileletMatmulFp32(workspace_, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnTileletMatmulFp32 success");

    ret = aclrtSynchronizeStreamWithTimeout(stream, 600000);
    if (ret != SUCCESS) {

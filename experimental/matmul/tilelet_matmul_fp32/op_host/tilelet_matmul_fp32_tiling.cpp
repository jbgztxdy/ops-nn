...
/*!
 * \file tilelet_matmul_fp32_tiling.cpp
 * \brief
 */
...
struct TileletMatmulFp32CompileInfo {
    uint64_t aicNum{0UL};
    uint64_t aivNum{0UL};
    uint64_t ubSize{0UL};
    uint64_t l1Size{0UL};
...
{
    auto& tiling = tilingDataPtr->tCubeTiling;
    auto& info = tilingDataPtr->tileletInfo;
    ...
    remoteTileStart = std::min(remoteTileStart, compactTileSlots);
    uint64_t remoteTileEnd = remoteTileStart;
    if (argsPtr->remoteTileCount < 0) {
        remoteTileEnd = compactTileSlots;
    } else {
        remoteTileEnd = std::min(remoteTileStart + static_cast<uint64_t>(argsPtr->remoteTileCount), compactTileSlots);
    }

    const bool serialTilelet = remoteTileEnd > remoteTileStart && argsPtr->commCoreNum < 0;
    const uint64_t computeCoreNum =
        serialTilelet ? 1UL : std::max<uint64_t>(1, std::min(compactTileSlots, compileInfoPtr->aicNum));
    ...
    const uint64_t totalCoreNum = computeCoreNum;
    const uint64_t wavefrontNStride = wavefrontN * tileN;

    info.wavefrontM = static_cast<uint32_t>(wavefrontM);
    info.wavefrontN = static_cast<uint32_t>(wavefrontN);
    info.commCoreNum = static_cast<uint32_t>(commCoreNum);
    info.computeCoreNum = static_cast<uint32_t>(computeCoreNum);
    info.totalCoreNum = static_cast<uint32_t>(totalCoreNum);
    info.mTileCnt = static_cast<uint32_t>(mTileCnt);
    info.nTileCnt = static_cast<uint32_t>(nTileCnt);
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, TileletMatmulFp32Args* argsPtr)
{
    argsPtr->opName = context->GetNodeName();
    OP_CHECK_IF(argsPtr->opName == nullptr, OP_LOGE("tilelet_matmul_fp32", "get op name invalid"), return ge::GRAPH_FAILED);
    // 输入张亮shape与描述信息
...
static ge::graphStatus InitTilingData(AscendC::tiling::TCubeTiling& tCubeTiling, TileletMatmulFp32CompileInfo* compileInfoPtr,
                                      TileletMatmulFp32Args* argsPtr)
{
...
inline void ResetBase(TileletMatmulFp32Args* argsPtr, TileletMatmulFp32RunInfo* runInfoPtr)
{
...
static void CalL1Tiling(TileletMatmulFp32CompileInfo* compileInfoPtr, TileletMatmulFp32Args* argsPtr, TileletMatmulFp32RunInfo* runInfoPtr)
{
...
static ge::graphStatus DoBL1FullLoadTiling(TileletMatmulFp32CompileInfo* compileInfoPtr, TileletMatmulFp32Args* argsPtr,
                                           TileletMatmulFp32RunInfo* runInfoPtr)
{
    ...
    FULL_LOAD_TYPE = TILELET_MATMUL_FP32_BL1_FULLLOAD;
    // fine tune tiling
    runInfoPtr->stepM = 1;
    ...
    if (totalBSizeL1 > compileInfoPtr->l1Size) {
        ...
        FULL_LOAD_TYPE = TILELET_MATMUL_FP32_NO_FULLLOAD;
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus DoOpTiling(AscendC::tiling::TCubeTiling& tCubeTiling, TileletMatmulFp32CompileInfo* compileInfoPtr,
                                  TileletMatmulFp32Args* argsPtr, TileletMatmulFp32RunInfo* runInfoPtr)
{
    // 高阶api初始化tilingdata
...
static ge::graphStatus PostTiling(gert::TilingContext* context, TileletMatmulFp32CompileInfo* compileInfoPtr,
                                  TileletMatmulFp32TilingData* tilingDataPtr, TileletMatmulFp32Args* argsPtr,
                                  TileletMatmulFp32RunInfo* runInfoPtr)
{
    // 设置tilingdata
...
static void GetWorkspaceSize(gert::TilingContext* context, TileletMatmulFp32TilingData* tilingDataPtr)
{
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(context->GetPlatformInfo());
    uint32_t sysWorkspaceSize = ascendcPlatform.GetLibApiWorkSpaceSize();
    size_t* currentWorkspace = context->GetWorkspaceSizes(1);
    currentWorkspace[0] = WS_SYS_SIZE + sysWorkspaceSize + tilingDataPtr->tileletInfo.userWorkspaceBytes;
}

// 1.Tiling需要获取的运行环境信息，包括可用核数、UB大小等，并将获取到的信息传递给'CompileInfo'
// 由于自动生成的aclnn接口实现不调用该函数，直接返回'ge::GRAPH_SUCCESS'即可
static ge::graphStatus TilingPrepareForTileletMatmulFp32([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// 2. Tiling计算主入口
static ge::graphStatus TileletMatmulFp32TilingFunc(gert::TilingContext* context)
{
    // 2.1 平台信息
    auto compileInfoPtr = std::make_unique<TileletMatmulFp32CompileInfo>();
    ...
    // 2.2 输入信息
    auto argsPtr = std::make_unique<TileletMatmulFp32Args>();
...
// 3. Tiling注册入口
IMPL_OP_OPTILING(TileletMatmulFp32)
    .Tiling(TileletMatmulFp32TilingFunc)
    .TilingParse<TileletMatmulFp32CompileInfo>(TilingPrepareForTileletMatmulFp32);
} // namespace optiling

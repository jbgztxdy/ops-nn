...
bool OpRunner::RunOp()
{
    ...
    auto ret = aclnnMatmulFp32GetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], transposeX1,
                                               transposeX2, outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Get Operator Workspace failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnMatmulFp32GetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnMatmulFp32(workspace_, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnMatmulFp32 success");

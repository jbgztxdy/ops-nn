...
int main()
{
    ...
    aclScalar* inputScaleScalar = aclCreateScalar(&inputScale, ACL_FLOAT);
    ret = aclnnEluV2GetWorkspaceSize(self, alphaScalar, scaleScalar, inputScaleScalar, out, &workspaceSize, &executor);
    // ret = aclnnEluGetWorkspaceSize(self, alpha, scale, inputScale, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnEluV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用aclnnEluV2第二段接口
    ret = aclnnEluV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnEluV2 failed. ERROR: %d\n", ret); return ret);
    ...
    for (int64_t i = 0; i < size; i++) {
        LOG_PRINT("aclnnEluV2 result[%ld] is: %f\n", i, resultData[i]);
    }

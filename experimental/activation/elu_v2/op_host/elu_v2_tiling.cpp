...
ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& inputDataNum, ge::DataType& dataType)
{
    ...
    // shape校验
    OP_CHECK_IF(inputShapeX.GetDimNum() != outShapeY.GetDimNum(),
                OP_LOGE(context, "EluV2: inputx,outputy shape dim = %zu, %zu, should be equal", inputShapeX.GetDimNum(),
                        outShapeY.GetDimNum()),
...
static ge::graphStatus EluV2TilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(GetShapeAttrsInfo(context, inputDataNum, dataType) != ge::GRAPH_SUCCESS,
    ...
    // 初始化tiling数据为0
    OP_CHECK_IF(memset_s(tiling, sizeof(EluV2TilingData), 0, sizeof(EluV2TilingData)) != EOK,

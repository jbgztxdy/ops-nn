...
| 调用方式 | 调用样例                                                                   | 说明                                                           |
|--------------|------------------------------------------------------------------------|--------------------------------------------------------------|
| aclnn 非原地 | `op_api/aclnn_elu.h`（`aclnnEluGetWorkspaceSize` / `aclnnElu`） | 与 `ops-math` 中 `aclnn_ne_tensor` 等接口拆分方式一致。 |
| aclnn 原地 | `op_api/aclnn_inplace_elu.h`（`aclnnInplaceEluGetWorkspaceSize` / `aclnnInplaceElu`） | 头文件独立，便于与 Level0 `elu.h` 分层对照。 |

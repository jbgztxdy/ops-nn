    ...
    uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    const float alphaValue = alpha->ToFloat();
    const float scaleValue = scale->ToFloat();
    const float inputScaleValue = inputScale->ToFloat();
    ...
    if (std::isinf(alphaValue) || std::isnan(alphaValue) || std::isinf(inputScaleValue) ||
        ...
        auto positiveMask = l0op::Greater(computeSelf, zeroTensor, uniqueExecutor.get());
        CHECK_RET(positiveMask != nullptr, ACLNN_ERR_INNER_NULLPTR);

        float negAlpha = alphaValue * float(-1.0);
        auto negAlphaScalar = uniqueExecutor.get()->AllocScalar(negAlpha);
        CHECK_RET(negAlphaScalar != nullptr, ACLNN_ERR_INNER_NULLPTR);
        auto negAlphaTensor = uniqueExecutor.get()->ConvertToTensor(negAlphaScalar, computeSelf->GetDataType());
        CHECK_RET(negAlphaTensor != nullptr, ACLNN_ERR_INNER_NULLPTR);

        auto selected = l0op::SelectV2(positiveMask, computeSelf, negAlphaTensor, uniqueExecutor.get());
        CHECK_RET(selected != nullptr, ACLNN_ERR_INNER_NULLPTR);

        eluOut = l0op::Muls(selected, scaleValue, uniqueExecutor.get());
    } else {
        eluOut = l0op::Elu(contiguousSelf, alphaValue, scaleValue, inputScaleValue, uniqueExecutor.get());
    }

    CHECK_RET(eluOut != nullptr, ACLNN_ERR_INNER_NULLPTR);

    // 固定写法，将计算结果转换成输出out的数据类型
    const aclTensor* castOut = eluOut;
    if (eluOut->GetDataType() != out->GetDataType()) {
        castOut = l0op::Cast(eluOut, out->GetDataType(), uniqueExecutor.get());

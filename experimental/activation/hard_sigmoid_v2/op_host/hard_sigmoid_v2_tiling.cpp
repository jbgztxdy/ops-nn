...
#include "../op_kernel/hard_sigmoid_v2_tiling_data.h"
#include "../op_kernel/hard_sigmoid_v2_tiling_key.h"
...
constexpr int64_t INT32_MIN_BYTES_PER_CORE = 64 * 1024;

struct HardSigmoidV2CompileInfo {};
...
static void FillDefaultTiling(HardSigmoidV2TilingData* tiling, ge::DataType dataType, gert::TilingContext* context)
{
    tiling->formerNum = 0;
...
static ge::graphStatus FillNormalTiling(gert::TilingContext* context, HardSigmoidV2TilingData* tiling,
                                        int64_t totalLength, ge::DataType dataType, uint64_t ubSize, int64_t coreNum)
{
    ...
    int64_t minBytesPerCore = (dataType == ge::DT_INT32) ? INT32_MIN_BYTES_PER_CORE : MIN_BYTES_PER_CORE;
    int64_t totalBytes = totalLength * dtypeSize;
    int64_t targetCoreNum = (totalBytes + minBytesPerCore - 1) / minBytesPerCore;
...
static ge::graphStatus HardSigmoidV2TilingFunc(gert::TilingContext* context)
{
    uint64_t ubSize = 0;
    int64_t coreNum = 0;
    ...
    auto* tiling = context->GetTilingData<HardSigmoidV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(HardSigmoidV2TilingData), 0, sizeof(HardSigmoidV2TilingData)) != EOK,
...
static ge::graphStatus TilingParseForHardSigmoidV2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(HardSigmoidV2)
    .Tiling(HardSigmoidV2TilingFunc)
    .TilingParse<HardSigmoidV2CompileInfo>(TilingParseForHardSigmoidV2);
} // namespace optiling

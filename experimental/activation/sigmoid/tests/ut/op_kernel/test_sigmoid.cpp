...
protected:
    ...
    static void TearDownTestCase() { std::cout << "sigmoid_test TearDown" << std::endl; }
...
TEST_F(SigmoidTest, test_case_float16_1)
{
    ...
    ICPU_RUN_KF(SigmoidleKernel, 1, x, y, workspace,
                (uint8_t*)(tilingDatafromBin)); // 根据实际设置核数，这里设置为8仅用于测试是否能测试kernel UT

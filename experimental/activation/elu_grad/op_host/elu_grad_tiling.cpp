...
#include "../op_kernel/elu_grad_tiling_key.h"

namespace optiling {

using namespace Ops::NN::OpTiling;
// SINGLE BUFFER
#define UB_NUM_FLOAT_ONE 4U
#define UB_NUM_FLOAT16_ONE 8U
#define UB_NUM_BFLOAT16_ONE 8U
// DOUBLE BUFFER
#define UB_NUM_FLOAT_TWO 7U
#define UB_NUM_FLOAT16_TWO 11U
#define UB_NUM_BFLOAT16_TWO 11U
...
static ge::graphStatus TilingParseForEluGrad([[maybe_unused]] gert::TilingParseContext* context)
{
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, uint64_t ubSize, int64_t coreNum,
                                         uint64_t& bufferOpen, uint64_t& inputNum, uint64_t& inputBytes,
                                         uint64_t& tileBlockNum, uint64_t& tileDataNum, uint64_t& inputLengthAlgin)
{
    ...
    if (context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT) {
        ...
        if (inputLengthAlgin < coreNum * (((ubSize / BLOCK_SIZE) * BLOCK_SIZE) / UB_NUM_FLOAT16_ONE)) {
            bufferOpen = 0;
            ubDataNumber = UB_NUM_FLOAT16_ONE;
        } else {
            ubDataNumber = UB_NUM_FLOAT16_TWO;
        }
    } else {
        if (inputLengthAlgin < coreNum * (((ubSize / BLOCK_SIZE) * BLOCK_SIZE) / UB_NUM_BFLOAT16_ONE)) {
            bufferOpen = 0;
            ubDataNumber = UB_NUM_BFLOAT16_ONE;
        } else {
            ubDataNumber = UB_NUM_BFLOAT16_TWO;
        }
    }
...
// tiling 分发入口
static ge::graphStatus EluGradTilingFunc(gert::TilingContext* context)
{
    // 1、获取平台运行信息
    uint64_t ubSize;
    int64_t coreNum;
    ...
    ret = GetShapeAttrsInfo(context, ubSize, coreNum, bufferOpen, inputNum, inputBytes, tileBlockNum, tileDataNum,
                            inputLengthAlgin);
    ...
    OP_CHECK_IF(GetWorkspaceSize(context) != ge::GRAPH_SUCCESS, OP_LOGE(context, "GetWorkspaceSize error"),
                return ge::GRAPH_FAILED);
    // 4、设置tiling信息
    EluGradTilingData* tiling = context->GetTilingData<EluGradTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(EluGradTilingData), 0, sizeof(EluGradTilingData)) != EOK,
...
// tiling注册入口.
IMPL_OP_OPTILING(EluGrad).Tiling(EluGradTilingFunc).TilingParse<EluGradCompileInfo>(TilingParseForEluGrad);
} // namespace optiling

...
#include "gelu_grad/op_kernel/gelu_grad_tiling_data.h"
#include "gelu_grad/op_kernel/gelu_grad_tiling_key.h"
...
static ge::graphStatus GeluGradTilingFunc(gert::TilingContext* context)
{
    ...
    GeluGradTilingData* tiling = context->GetTilingData<GeluGradTilingData>();
    ...
    tiling->versionNum = (uint32_t)versionNum;
...
static ge::graphStatus TilingParseForGeluGrad([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

// tiling注册入口.
IMPL_OP_OPTILING(GeluGrad).Tiling(GeluGradTilingFunc).TilingParse<GeluGradCompileInfo>(TilingParseForGeluGrad);
} // namespace optiling

...
int main(int argc, char** argv)
{
    ...
    ret = aclrtMemcpy(selfDeviceAddr, numElements * sizeof(float), selfData.data(), numElements * sizeof(float),
    ...
    aclTensor* self = aclCreateTensor(shape.data(), shape.size(), dataType, strides.data(), 0, ACL_FORMAT_ND,
                                      shape.data(), shape.size(), selfDeviceAddr);
    CHECK_RET(self != nullptr, FAIL("aclCreateTensor self failed"));

    aclTensor* out = aclCreateTensor(shape.data(), shape.size(), dataType, strides.data(), 0, ACL_FORMAT_ND,
    ...
    CHECK_RET(thresholdScalar != nullptr && valueScalar != nullptr, FAIL("aclCreateScalar failed"));

    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnThresholdV2GetWorkspaceSize(self, thresholdScalar, valueScalar, out, &workspaceSize, &executor);
    ...
    ret = aclrtMemcpy(outData.data(), numElements * sizeof(float), outDeviceAddr, numElements * sizeof(float),

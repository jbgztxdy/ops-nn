    ...
    __aicore__ inline void Process1(int start, int end)
    {
        ...
        for (int i = 0; i != loop; i++) {
            ...
            if (align_n == LogSoftmaxConst::ONE_TWENTY_EIGHT) {
                ...
                for (int j = 0; j != lines; j++) {
                    ...
                    half maxIndex = workLocal1.GetValue(LogSoftmaxConst::ONE +
                                                        j * LogSoftmaxConst::TWO_THOUSAND_FORTY_EIGHT);
                    uint16_t realIndex = *reinterpret_cast<uint16_t*>(&maxIndex);
            ...
            for (int k = 0; k != lines; k++) {
                ...
                WholeReduceSum<float, false>(inputLocal[align_n * k], inputLocal[align_n * k], 1, MASK_PLACEHOLDER, 1,
                                             1, LogSoftmaxConst::EIGHT);
                PipeBarrier<PIPE_V>();
            }
            SetMaskNorm();
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR out, int m, int n, int flag)
    {
        ...
        pipe.InitBuffer(inQueue, LogSoftmaxConst::TWO, LogSoftmaxConst::THIRTY_TWO_THOUSAND_SEVEN_HUNDRED_SIXTY_EIGHT);
        pipe.InitBuffer(workQueue,
                        LogSoftmaxConst::THIRTY_TWO_THOUSAND_SEVEN_HUNDRED_SIXTY_EIGHT * LogSoftmaxConst::TWO);
        pipe.InitBuffer(outQueue, LogSoftmaxConst::TWO, LogSoftmaxConst::THIRTY_TWO_THOUSAND_SEVEN_HUNDRED_SIXTY_EIGHT);
    }
        ...
        for (int i = start; i != end; i++) {
            ...
            for (int j = 0; j < m; j += LogSoftmaxConst::FOUR) {
                ...
                DataCopyExtParams copyParams{
                    static_cast<uint16_t>(blocklen), static_cast<uint32_t>(len * sizeof(float)),
    ...
    __aicore__ inline void Process4(int batch, int start, int end, int chunk)
    {
        ...
        for (int i = start; i != end; i++) {
            ...
            LocalTensor<float> inputLocal = inQueue.AllocTensor<float>();
            DataCopyExtParams copyParams{static_cast<uint16_t>(m), static_cast<uint32_t>(len * sizeof(float)),
                                         static_cast<uint32_t>((n - len) * sizeof(float)),
            ...
            outQueue.EnQue(outLocal);
            outQueue.DeQue<float>();
            DataCopyExtParams copyoutParams{static_cast<uint16_t>(m), static_cast<uint32_t>(len * sizeof(float)),
    ...
    __aicore__ inline void Process1(int start, int end)
    {
        ...
        if (coliters == 1) {
            for (int i = start; i != end; i++) {
                ...
                DataCopyExtParams copyParams{
                    static_cast<uint16_t>(coltail), static_cast<uint32_t>(len * sizeof(float)),
                    static_cast<uint32_t>((n - len) * sizeof(float)),
                    static_cast<uint32_t>(LogSoftmaxConst::TWO - (len + 7) / LogSoftmaxConst::EIGHT), 0};
                DataCopyPadExtParams<float> padParams{false, 0, 0, 0};

                DataCopyPad(inputLocal, inputGm[batch * m * n + index * LogSoftmaxConst::SIXTEEN], copyParams,
                            padParams);
                inQueue.EnQue(inputLocal);
                inputLocal = inQueue.DeQue<float>();
                ...
                Max(newmax, inputLocal, newmax, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 2, 0});

                Sub(outLocal, inputLocal, newmax, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 2, 2, 0});

                Exp(inputLocal, outLocal, coltail * LogSoftmaxConst::SIXTEEN);

                Duplicate(expsum, 0.0f, LogSoftmaxConst::SIXTEEN);
                Add(expsum, inputLocal, expsum, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 2, 0});

                Ln(expsum, expsum, LogSoftmaxConst::SIXTEEN);

                Sub(outLocal, outLocal, expsum, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 2, 2, 0});
            ...
            for (int i = start; i != end; i++) {
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    DataCopyExtParams copyParams{
                        static_cast<uint16_t>(collen), static_cast<uint32_t>(len * sizeof(float)),
                        static_cast<uint32_t>((n - len) * sizeof(float)),
                        static_cast<uint32_t>(LogSoftmaxConst::TWO - (len + 7) / LogSoftmaxConst::EIGHT), 0};
                    DataCopyPadExtParams<float> padParams{false, 0, 0, 0};

                    DataCopyPad(inputLocal,
                                inputGm[batch * m * n + index * LogSoftmaxConst::SIXTEEN +
                                        j * LogSoftmaxConst::ONE_TWENTY_EIGHT * n],
                                copyParams, padParams);
                    inQueue.EnQue(inputLocal);
                    inputLocal = inQueue.DeQue<float>();

                    Max(oldmax, inputLocal, oldmax, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 2, 0});
                    inQueue.FreeTensor(inputLocal);
                }
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    Sub(inputLocal, inputLocal, oldmax, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(collen),
                        {1, 1, 1, 2, 2, 0});
                    Exp(inputLocal, inputLocal, collen * LogSoftmaxConst::SIXTEEN);
                    Add(expsum, inputLocal, expsum, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 2, 0});
                    inQueue.FreeTensor(inputLocal);
                }
                Ln(expsum, expsum, LogSoftmaxConst::SIXTEEN);
                for (int j = 0; j != coliters; j++) {
                    ...
                    inputLocal = inQueue.DeQue<float>();
                    LocalTensor<float> outLocal = outQueue.AllocTensor<float>();
                    Sub(outLocal, inputLocal, oldmax, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(collen),
                        {1, 1, 1, 2, 2, 0});
                    inQueue.FreeTensor(inputLocal);
                    Sub(outLocal, outLocal, expsum, LogSoftmaxConst::SIXTEEN, static_cast<uint8_t>(collen),
                        {1, 1, 1, 2, 2, 0});
                    outQueue.EnQue(outLocal);
                    outQueue.DeQue<float>();
                    DataCopyExtParams copyoutParams{
                    ...
                    DataCopyPad(outGm[batch * m * n + index * LogSoftmaxConst::SIXTEEN +
                                      j * LogSoftmaxConst::ONE_TWENTY_EIGHT * n],
    ...
    __aicore__ inline void Process2(int start, int end)
    {
        ...
        if (coliters == 1) {
            for (int i = start; i != end; i++) {
                ...
                DataCopyPad(inputLocal, inputGm[batch * m * n + index * LogSoftmaxConst::ONE_TWENTY_EIGHT], copyParams,
                            padParams);
                inQueue.EnQue(inputLocal);
                inputLocal = inQueue.DeQue<float>();
                ...
                Max(newmax, inputLocal, newmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 16, 0});
                Max(newmax[LogSoftmaxConst::SIXTY_FOUR], inputLocal[LogSoftmaxConst::SIXTY_FOUR],
                    newmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 16, 0});

                Sub(outLocal, inputLocal, newmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 16, 16, 0});
                Sub(outLocal[LogSoftmaxConst::SIXTY_FOUR], inputLocal[LogSoftmaxConst::SIXTY_FOUR],
                    newmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 16, 16, 0});
                ...
                Add(expsum, inputLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 16, 0});
                Add(expsum[LogSoftmaxConst::SIXTY_FOUR], inputLocal[LogSoftmaxConst::SIXTY_FOUR],
                    expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 16, 0});

                Ln(expsum, expsum, LogSoftmaxConst::ONE_TWENTY_EIGHT);

                Sub(outLocal, outLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 16, 16, 0});
                Sub(outLocal[LogSoftmaxConst::SIXTY_FOUR], outLocal[LogSoftmaxConst::SIXTY_FOUR],
                    expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 16, 16, 0});
            ...
            for (int i = start; i != end; i++) {
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    DataCopyPad(inputLocal,
                                inputGm[batch * m * n + index * LogSoftmaxConst::ONE_TWENTY_EIGHT +
                                        j * LogSoftmaxConst::SIXTY_FOUR * n],
                                copyParams, padParams);
                    inQueue.EnQue(inputLocal);
                    inputLocal = inQueue.DeQue<float>();

                    Max(oldmax, inputLocal, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 16, 0});
                    Max(oldmax[LogSoftmaxConst::SIXTY_FOUR], inputLocal[LogSoftmaxConst::SIXTY_FOUR],
                        oldmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 16, 0});
                    inQueue.FreeTensor(inputLocal);
                }
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    Sub(inputLocal, inputLocal, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 16, 16, 0});
                    Sub(inputLocal[LogSoftmaxConst::SIXTY_FOUR], inputLocal[LogSoftmaxConst::SIXTY_FOUR],
                        oldmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 16, 16, 0});
                    Exp(inputLocal, inputLocal, collen * LogSoftmaxConst::ONE_TWENTY_EIGHT);
                    Add(expsum, inputLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 16, 0});
                    Add(expsum[LogSoftmaxConst::SIXTY_FOUR], inputLocal[LogSoftmaxConst::SIXTY_FOUR],
                        expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 16, 0});
                    inQueue.FreeTensor(inputLocal);
                }
                Ln(expsum, expsum, LogSoftmaxConst::ONE_TWENTY_EIGHT);
                for (int j = 0; j != coliters; j++) {
                    ...
                    inputLocal = inQueue.DeQue<float>();
                    LocalTensor<float> outLocal = outQueue.AllocTensor<float>();
                    Sub(outLocal, inputLocal, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 16, 16, 0});
                    Sub(outLocal[LogSoftmaxConst::SIXTY_FOUR], inputLocal[LogSoftmaxConst::SIXTY_FOUR],
                        oldmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 16, 16, 0});
                    inQueue.FreeTensor(inputLocal);
                    Sub(outLocal, outLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 16, 16, 0});
                    Sub(outLocal[LogSoftmaxConst::SIXTY_FOUR], outLocal[LogSoftmaxConst::SIXTY_FOUR],
                        expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 16, 16, 0});
                    ...
                    DataCopyPad(outGm[batch * m * n + index * LogSoftmaxConst::ONE_TWENTY_EIGHT +
                                      j * LogSoftmaxConst::SIXTY_FOUR * n],
                                outLocal, copyoutParams);
                    outQueue.FreeTensor(outLocal);
                }
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR out, int m, int n)
    {
        ...
        pipe.InitBuffer(workQueue1, LogSoftmaxConst::THIRTY_TWO_THOUSAND_SEVEN_HUNDRED_SIXTY_EIGHT +
                                        LogSoftmaxConst::TWO_FIFTY_SIX);
    ...
    __aicore__ inline void Process1(int start, int end)
    {
        ...
        for (int i = 0; i != loop; i++) {
            ...
            for (int k = 0; k != lines; k++) {
                ...
                for (int j = 0; j != iters; j++) {
                    SetVectorMask<float>(0, length[j]);
                    BlockReduceSum<float, false>(workLocal, workLocal, 1, MASK_PLACEHOLDER, 1, 1,
                                                 LogSoftmaxConst::EIGHT);
                    PipeBarrier<PIPE_V>();
                }
                SetVectorMask<float>(0, length[iters]);
...
public:
    ...
    __aicore__ inline void Process4(int batch, int start, int end, int chunk)
    {
        ...
        LocalTensor<float>
            workLocal3 = workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT + (LogSoftmaxConst::TWO + m) * chunk];
        ...
        for (int i = start; i != end; i++) {
            ...
            LocalTensor<half> inputLocal = inQueue.AllocTensor<half>();
            DataCopyExtParams copyParams{static_cast<uint16_t>(m), static_cast<uint32_t>(len * sizeof(half)),
            ...
            outQueue.EnQue(outLocal);
            outQueue.DeQue<half>();
            DataCopyExtParams copyoutParams{static_cast<uint16_t>(m), static_cast<uint32_t>(len * sizeof(half)),
    ...
    __aicore__ inline void Process3(int batch, int start, int end)
    {
        ...
        LocalTensor<float>
            workLocal3 = workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT + LogSoftmaxConst::TWO * chunksize];
        ...
        for (int i = start; i != end; i++) {
            ...
            for (int j = 0; j < m; j += LogSoftmaxConst::FOUR) {
                ...
                LocalTensor<half> inputLocal = inQueue.AllocTensor<half>();
                DataCopyExtParams copyParams{
                    static_cast<uint16_t>(blocklen), static_cast<uint32_t>(len * sizeof(half)),
                    static_cast<uint32_t>((n - len) * sizeof(half)),
    ...
    __aicore__ inline void Process1(int start, int end)
    {
        ...
        if (coliters == 1) {
            for (int i = start; i != end; i++) {
                ...
                DataCopyExtParams copyParams{static_cast<uint16_t>(coltail), static_cast<uint32_t>(len * sizeof(half)),
                                             static_cast<uint32_t>((n - len) * sizeof(half)),
                                             static_cast<uint32_t>(4 - (len + 15) / LogSoftmaxConst::SIXTEEN), 0};
                DataCopyPadExtParams<half> padParams{false, 0, 0, 0};

                DataCopyPad(inputLocal, inputGm[batch * m * n + index * LogSoftmaxConst::SIXTY_FOUR], copyParams,
                            padParams);
                inQueue.EnQue(inputLocal);
                inputLocal = inQueue.DeQue<half>();
                ...
                Max(outLocal, inputLocal, outLocal, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 8, 0});
                ...
                Sub(workLocal, workLocal, workLocal1, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 8, 8, 0});
                Exp(workLocal, workLocal, coltail * LogSoftmaxConst::SIXTY_FOUR);

                Duplicate(expsum, 0.0f, LogSoftmaxConst::SIXTY_FOUR);
                Add(expsum, workLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 8, 0});
                ...
                Sub(workLocal, workLocal, workLocal1, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 8, 8, 0});
                Sub(workLocal, workLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 8, 8, 0});
            ...
            for (int i = start; i != end; i++) {
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    DataCopyPad(inputLocal,
                                inputGm[batch * m * n + index * LogSoftmaxConst::SIXTY_FOUR +
                                        j * LogSoftmaxConst::ONE_TWENTY_EIGHT * n],
                                copyParams, padParams);
                    inQueue.EnQue(inputLocal);
                    inputLocal = inQueue.DeQue<half>();
                    ...
                    Max(oldmax, workLocal, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 8, 0});
                    inQueue.FreeTensor(inputLocal);
                }
                for (int j = 0; j != coliters; j++) {
                    ...
                    Cast(workLocal, inputLocal, RoundMode::CAST_NONE, collen * LogSoftmaxConst::SIXTY_FOUR);
                    Sub(workLocal, workLocal, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 8, 8, 0});
                    Exp(workLocal, workLocal, collen * LogSoftmaxConst::SIXTY_FOUR);
                    Add(expsum, workLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 8, 0});
                    inQueue.FreeTensor(inputLocal);
                }
                Ln(expsum, expsum, LogSoftmaxConst::SIXTY_FOUR);
                for (int j = 0; j != coliters; j++) {
                    ...
                    Sub(workLocal, workLocal, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 8, 8, 0});
                    Sub(workLocal, workLocal, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 8, 8, 0});
                    ...
                    DataCopyPad(outGm[batch * m * n + index * LogSoftmaxConst::SIXTY_FOUR +
                                      j * LogSoftmaxConst::ONE_TWENTY_EIGHT * n],
                                outLocal, copyoutParams);
                    outQueue.FreeTensor(outLocal);
                }
    ...
    __aicore__ inline void Process2(int start, int end)
    {
        ...
        if (coliters == 1) {
            for (int i = start; i != end; i++) {
                ...
                DataCopyExtParams copyParams{
                    static_cast<uint16_t>(coltail), static_cast<uint32_t>(len * sizeof(half)),
                    static_cast<uint32_t>((n - len) * sizeof(half)),
                    static_cast<uint32_t>(LogSoftmaxConst::SIXTEEN - (len + 15) / LogSoftmaxConst::SIXTEEN), 0};
                DataCopyPadExtParams<half> padParams{false, 0, 0, 0};

                DataCopyPad(inputLocal, inputGm[batch * m * n + index * LogSoftmaxConst::TWO_FIFTY_SIX], copyParams,
                            padParams);
                inQueue.EnQue(inputLocal);
                inputLocal = inQueue.DeQue<half>();

                LocalTensor<half> outLocal = outQueue.AllocTensor<half>();
                Duplicate(outLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT], LogSoftmaxConst::HALF_NEG_INF,
                          LogSoftmaxConst::TWO_FIFTY_SIX);
                Max(outLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT], inputLocal,
                    outLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT], LogSoftmaxConst::ONE_TWENTY_EIGHT,
                    static_cast<uint8_t>(coltail), {1, 1, 1, 0, 16, 0});
                Max(outLocal[LogSoftmaxConst::TWO_FIFTY_SIX], inputLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT],
                    outLocal[LogSoftmaxConst::TWO_FIFTY_SIX], LogSoftmaxConst::ONE_TWENTY_EIGHT,
                    static_cast<uint8_t>(coltail), {1, 1, 1, 0, 16, 0});

                Cast(newmax, outLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT], RoundMode::CAST_NONE,
                     LogSoftmaxConst::TWO_FIFTY_SIX);
                Cast(workLocal1, inputLocal, RoundMode::CAST_NONE, coltail * LogSoftmaxConst::TWO_FIFTY_SIX);

                Sub(workLocal1, workLocal1, newmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 32, 32, 0});
                Sub(workLocal1[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                    newmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 32, 32, 0});
                Sub(workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT], workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT],
                    newmax[LogSoftmaxConst::ONE_TWENTY_EIGHT], LogSoftmaxConst::SIXTY_FOUR,
                    static_cast<uint8_t>(coltail), {1, 1, 1, 32, 32, 0});
                Sub(workLocal1[LogSoftmaxConst::ONE_NINETY_TWO], workLocal1[LogSoftmaxConst::ONE_NINETY_TWO],
                    newmax[LogSoftmaxConst::ONE_NINETY_TWO], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 32, 32, 0});

                Exp(workLocal1, workLocal1, coltail * LogSoftmaxConst::TWO_FIFTY_SIX);

                Duplicate(expsum, 0.0f, LogSoftmaxConst::TWO_FIFTY_SIX);
                Add(expsum, workLocal1, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 32, 0});
                Add(expsum[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                    expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 32, 0});
                Add(expsum[LogSoftmaxConst::ONE_TWENTY_EIGHT], workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT],
                    expsum[LogSoftmaxConst::ONE_TWENTY_EIGHT], LogSoftmaxConst::SIXTY_FOUR,
                    static_cast<uint8_t>(coltail), {1, 1, 1, 0, 32, 0});
                Add(expsum[LogSoftmaxConst::ONE_NINETY_TWO], workLocal1[LogSoftmaxConst::ONE_NINETY_TWO],
                    expsum[LogSoftmaxConst::ONE_NINETY_TWO], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 32, 0});
                ...
                Sub(workLocal1, workLocal1, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 32, 32, 0});
                Sub(workLocal1[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                    expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 32, 32, 0});
            ...
            for (int i = start; i != end; i++) {
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    DataCopyExtParams copyParams{
                        static_cast<uint16_t>(collen), static_cast<uint32_t>(len * sizeof(half)),
                        static_cast<uint32_t>((n - len) * sizeof(half)),
                        static_cast<uint32_t>(LogSoftmaxConst::SIXTEEN - (len + 15) / LogSoftmaxConst::SIXTEEN), 0};
                    DataCopyPadExtParams<half> padParams{false, 0, 0, 0};

                    DataCopyPad(inputLocal,
                                inputGm[batch * m * n + index * LogSoftmaxConst::TWO_FIFTY_SIX +
                                        j * LogSoftmaxConst::THIRTY_TWO * n],
                                copyParams, padParams);
                    inQueue.EnQue(inputLocal);
                    inputLocal = inQueue.DeQue<half>();
                    ...
                    Max(outLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT], inputLocal,
                        outLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT], LogSoftmaxConst::ONE_TWENTY_EIGHT,
                        static_cast<uint8_t>(collen), {1, 1, 1, 0, 16, 0});
                    Max(outLocal[LogSoftmaxConst::TWO_FIFTY_SIX], inputLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT],
                        outLocal[LogSoftmaxConst::TWO_FIFTY_SIX], LogSoftmaxConst::ONE_TWENTY_EIGHT,
                        static_cast<uint8_t>(collen), {1, 1, 1, 0, 16, 0});
                    Cast(oldmax, outLocal[LogSoftmaxConst::ONE_TWENTY_EIGHT], RoundMode::CAST_NONE,
                         LogSoftmaxConst::TWO_FIFTY_SIX);
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    Sub(workLocal1, workLocal1, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 32, 32, 0});
                    Sub(workLocal1[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        oldmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 32, 32, 0});
                    Sub(workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT], workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT],
                        oldmax[LogSoftmaxConst::ONE_TWENTY_EIGHT], LogSoftmaxConst::SIXTY_FOUR,
                        static_cast<uint8_t>(collen), {1, 1, 1, 32, 32, 0});
                    Sub(workLocal1[LogSoftmaxConst::ONE_NINETY_TWO], workLocal1[LogSoftmaxConst::ONE_NINETY_TWO],
                        oldmax[LogSoftmaxConst::ONE_NINETY_TWO], LogSoftmaxConst::SIXTY_FOUR,
                        static_cast<uint8_t>(collen), {1, 1, 1, 32, 32, 0});

                    Exp(workLocal1, workLocal1, collen * LogSoftmaxConst::TWO_FIFTY_SIX);
                    Add(expsum, workLocal1, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 32, 0});
                    Add(expsum[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 32, 0});
                    Add(expsum[LogSoftmaxConst::ONE_TWENTY_EIGHT], workLocal1[LogSoftmaxConst::ONE_TWENTY_EIGHT],
                        expsum[LogSoftmaxConst::ONE_TWENTY_EIGHT], LogSoftmaxConst::SIXTY_FOUR,
                        static_cast<uint8_t>(collen), {1, 1, 1, 0, 32, 0});
                    Add(expsum[LogSoftmaxConst::ONE_NINETY_TWO], workLocal1[LogSoftmaxConst::ONE_NINETY_TWO],
                        expsum[LogSoftmaxConst::ONE_NINETY_TWO], LogSoftmaxConst::SIXTY_FOUR,
                        static_cast<uint8_t>(collen), {1, 1, 1, 0, 32, 0});
                }
                Ln(expsum, expsum, LogSoftmaxConst::TWO_FIFTY_SIX);
                for (int k = 0; k != coliters; k++) {
                    ...
                    DataCopyPad(inputLocal,
                                inputGm[batch * m * n + index * LogSoftmaxConst::TWO_FIFTY_SIX +
                                        k * LogSoftmaxConst::THIRTY_TWO * n],
                                copyParams, padParams);
                    inQueue.EnQue(inputLocal);
                    inputLocal = inQueue.DeQue<half>();
                    Cast(workLocal1, inputLocal, RoundMode::CAST_NONE, collen * LogSoftmaxConst::TWO_FIFTY_SIX);
                    ...
                    Sub(workLocal1[LogSoftmaxConst::ONE_NINETY_TWO], workLocal1[LogSoftmaxConst::ONE_NINETY_TWO],
                        oldmax[LogSoftmaxConst::ONE_NINETY_TWO], LogSoftmaxConst::SIXTY_FOUR,
                        static_cast<uint8_t>(collen), {1, 1, 1, 32, 32, 0});
                    Sub(workLocal1, workLocal1, expsum, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 32, 32, 0});
                    Sub(workLocal1[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 32, 32, 0});
                    ...
                    DataCopyPad(outGm[batch * m * n + index * LogSoftmaxConst::TWO_FIFTY_SIX +
                                      k * LogSoftmaxConst::THIRTY_TWO * n],
                                outLocal, copyoutParams);
                    outQueue.FreeTensor(outLocal);
                }
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR out, int m, int n, int flag)
    {
        ...
        pipe.InitBuffer(inQueue, LogSoftmaxConst::TWO, LogSoftmaxConst::SIXTEEN_THOUSAND_THREE_HUNDRED_EIGHTY_FOUR);
        pipe.InitBuffer(
            workQueue, LogSoftmaxConst::THIRTY_TWO_THOUSAND_SEVEN_HUNDRED_SIXTY_EIGHT + LogSoftmaxConst::TWO_FIFTY_SIX);
        pipe.InitBuffer(workQueue1, LogSoftmaxConst::THIRTY_TWO_THOUSAND_SEVEN_HUNDRED_SIXTY_EIGHT * 2 +
                                        LogSoftmaxConst::TWO_FIFTY_SIX);
        pipe.InitBuffer(outQueue, LogSoftmaxConst::TWO, LogSoftmaxConst::SIXTEEN_THOUSAND_THREE_HUNDRED_EIGHTY_FOUR);
    }
    ...
    __aicore__ inline void Process3(int start, int end)
    {
        ...
        if (coliters == 1) {
            for (int i = start; i != end; i++) {
                ...
                Cast(workLocal, inputLocal, RoundMode::CAST_NONE, coltail * LogSoftmaxConst::SIXTY_FOUR);
                Max(newmax, workLocal, newmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 8, 0});

                Sub(workLocal, workLocal, newmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 8, 8, 0});
            ...
            for (int i = start; i != end; i++) {
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    Max(oldmax, workLocal1, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 8, 0});
                    inQueue.FreeTensor(inputLocal);
                    Sub(workLocal1, workLocal1, newmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 8, 8, 0});
                }
                for (int j = 0; j != coliters; j++) {
                    LocalTensor<bfloat16_t> inputLocal = inQueue.AllocTensor<bfloat16_t>();
    ...
    __aicore__ inline void Process1(int start, int end)
    {
        ...
        if (coliters == 1) {
            for (int i = start; i != end; i++) {
                ...
                Max(newmax, workLocal1, newmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 16, 0});
                Max(newmax[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                    newmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(coltail),
                    {1, 1, 1, 0, 16, 0});
            ...
            for (int i = start; i != end; i++) {
                ...
                for (int j = 0; j != coliters; j++) {
                    ...
                    Max(oldmax, workLocal1, oldmax, LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 16, 0});
                    Max(oldmax[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        oldmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, static_cast<uint8_t>(collen),
                        {1, 1, 1, 0, 16, 0});
                }
                for (int j = 0; j != coliters; j++) {
                    ...
                    Sub(workLocal1, workLocal1, oldmax, LogSoftmaxConst::SIXTY_FOUR, uint8_t(collen),
                        {1, 1, 1, 16, 16, 0});
                    Sub(workLocal1[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        oldmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, uint8_t(collen),
                        {1, 1, 1, 16, 16, 0});
                    ...
                    Add(expsum[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, uint8_t(collen),
                        {1, 1, 1, 0, 16, 0});
                }
                Ln(expsum, expsum, LogSoftmaxConst::ONE_TWENTY_EIGHT);
                for (int j = 0; j != coliters; j++) {
                    ...
                    Sub(workLocal1[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        oldmax[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, uint8_t(collen),
                        {1, 1, 1, 16, 16, 0});
                    Sub(workLocal1, workLocal1, expsum, LogSoftmaxConst::SIXTY_FOUR, uint8_t(collen),
                        {1, 1, 1, 16, 16, 0});
                    Sub(workLocal1[LogSoftmaxConst::SIXTY_FOUR], workLocal1[LogSoftmaxConst::SIXTY_FOUR],
                        expsum[LogSoftmaxConst::SIXTY_FOUR], LogSoftmaxConst::SIXTY_FOUR, uint8_t(collen),
                        {1, 1, 1, 16, 16, 0});
    ...
    __aicore__ inline void Process2(int batch, int start, int end)
    {
        ...
        for (int i = start; i != end; i++) {
            ...
            for (int j = 0; j < m; j += LogSoftmaxConst::FOUR) {
                ...
                LocalTensor<bfloat16_t> inputLocal = inQueue.AllocTensor<bfloat16_t>();
                DataCopyExtParams copyParams{
                    static_cast<uint16_t>(blocklen), static_cast<uint32_t>(len * sizeof(bfloat16_t)),
                    static_cast<uint32_t>((n - len) * sizeof(bfloat16_t)),
                    static_cast<uint32_t>(LogSoftmaxConst::ONE_TWENTY_EIGHT - (len + 15) / LogSoftmaxConst::SIXTEEN),
                    0};
    ...
    __aicore__ inline void Process4(int batch, int start, int end, int chunk)
    {
        ...
        for (int i = start; i != end; i++) {
            ...
            LocalTensor<bfloat16_t> inputLocal = inQueue.AllocTensor<bfloat16_t>();
            DataCopyExtParams copyParams{static_cast<uint16_t>(m), static_cast<uint32_t>(len * sizeof(bfloat16_t)),
                                         static_cast<uint32_t>((n - len) * sizeof(bfloat16_t)),
                                         static_cast<uint32_t>(blocklen - (len + 15) / LogSoftmaxConst::SIXTEEN), 0};
            DataCopyPadExtParams<bfloat16_t> padParams{false, 0, 0, 0};
            DataCopyPad(inputLocal, inputGm[batch * m * n + i * chunk], copyParams, padParams);
            ...
            outQueue.EnQue(outLocal);
            outQueue.DeQue<bfloat16_t>();
            DataCopyExtParams copyoutParams{static_cast<uint16_t>(m), static_cast<uint32_t>(len * sizeof(bfloat16_t)),
                                            static_cast<uint32_t>(blocklen - (len + 15) / LogSoftmaxConst::SIXTEEN),
                                            static_cast<uint32_t>((n - len) * sizeof(bfloat16_t)), 0};
            DataCopyPad(outGm[batch * m * n + i * chunk], outLocal, copyoutParams);
            outQueue.FreeTensor(outLocal);

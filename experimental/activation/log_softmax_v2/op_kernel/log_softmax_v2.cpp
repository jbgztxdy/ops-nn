...
// ========================== 模板 Traits (类型映射) ==========================
template <typename T>
struct LogSoftmaxTraits;

template <>
struct LogSoftmaxTraits<float> {
...
template <>
struct LogSoftmaxTraits<bfloat16_t> {
...
__aicore__ inline void ProcessLogSoftmaxInternal(GM_ADDR input, GM_ADDR out, const LogSoftmaxV2TilingData& tilingData)
{
    ...
    if (dims == 2) {
        ...
        // 逻辑4: 默认场景
            ...
            if constexpr (std::is_same_v<T, bfloat16_t>) {
                if (end > start)
                    op.Process3(start, end);
            } else {
                if (end > start)
                    op.Process2(start, end);
            }
        }
    }
}

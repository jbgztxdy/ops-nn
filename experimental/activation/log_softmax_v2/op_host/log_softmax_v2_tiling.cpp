...
static ge::graphStatus LogSoftmaxV2TilingFunc(gert::TilingContext* context)
{
    ...
    for (uint32_t i = 0; i < MAX_SHAPE_DIMS; i++)
        shape[i] = INITIAL_DIM_VAL;
    ...
    if (attr0 != nullptr && attr0->GetSize() > 0) {
        ...
        for (uint32_t i = 0; i < x1_shape->GetStorageShape().GetDimNum(); i++) {
            if (static_cast<int64_t>(i) >= axisL && static_cast<int64_t>(i) <= axisR) {
                ...
                if (static_cast<int64_t>(i) == axisR) {
                    axis = index;
                    index++;
                }
            } else {
    ...
    OP_CHECK_IF(memset_s(tiling, sizeof(LogSoftmaxV2TilingData), 0, sizeof(LogSoftmaxV2TilingData)) != EOK,
...
IMPL_OP_OPTILING(LogSoftmaxV2)
    .Tiling(LogSoftmaxV2TilingFunc)
    .TilingParse<LogSoftmaxV2CompileInfo>(TilingParseForLogSoftmaxV2);
} // namespace optiling

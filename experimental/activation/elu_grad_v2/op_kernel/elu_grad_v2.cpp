...
__global__ __aicore__ void elu_grad_v2(GM_ADDR grads, GM_ADDR activations, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    AscendC::TPipe pipe;
    constexpr bool kMixedResultOutput = !NsEluGradV2::kIsSameType<DTYPE_X, DTYPE_Y> &&
                                        NsEluGradV2::kIsSameType<DTYPE_Y, float>;
    if constexpr (schMode == ELU_GRAD_V2_TPL_SCH_MODE_SAFE_RESULT) {
        ...
        if constexpr (!NsEluGradV2::kIsSameType<DTYPE_X, float> && !NsEluGradV2::kIsSameType<DTYPE_X, bfloat16_t>) {

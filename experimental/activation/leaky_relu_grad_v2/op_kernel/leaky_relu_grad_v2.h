...
template <typename T>
__aicore__ inline void LeakyReluGradV2<T>::Init(GM_ADDR dy, GM_ADDR x, GM_ADDR dx,
                                                const LeakyReluGradV2TilingData* tilingData)
{
...
__aicore__ inline void LeakyReluGradV2<T>::Compute(int32_t progress)
{
    ...
    AscendC::Select(dxLocal, temp, dyLocal, negativePart, AscendC::SELMODE::VSEL_TENSOR_TENSOR_MODE,
                    this->processDataNum);

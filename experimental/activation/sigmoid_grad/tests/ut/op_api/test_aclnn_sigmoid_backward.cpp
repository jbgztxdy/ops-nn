...
protected:
    static void SetUpTestCase() { std::cout << "sigmoid_backward_test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "sigmoid_backward_test TearDown" << std::endl; }
};

// 测试合法数据类型float
TEST_F(l2_sigmoid_backward_test, case_001_float)
{
    ...
    auto ut = OP_API_UT(aclnnSigmoidBackward, INPUT(gradOutputDesc, selfDesc), OUTPUT(gradInputDesc));
...
TEST_F(l2_sigmoid_backward_test, case_broadcast_001_float)
{
    auto gradOutputDesc = TensorDesc({2, 2, 3, 2}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-1, 1);
...
// 测试合法数据类型float16
TEST_F(l2_sigmoid_backward_test, case_002_float16)
{
...
// 测试非法数据类型double
TEST_F(l2_sigmoid_backward_test, case_003_float64)
{
...
// 测试非法数据类型int32
TEST_F(l2_sigmoid_backward_test, case_004_invalid_type_int)
{
...
// 测试隐式类型转化
TEST_F(l2_sigmoid_backward_test, case_005_promotetype)
{
...
// 测试空Tensor
TEST_F(l2_sigmoid_backward_test, case_006_empty_tensor)
{
...
// 测试非连续
TEST_F(l2_sigmoid_backward_test, case_007_not_contiguous)
{
...
// 测试空指针
TEST_F(l2_sigmoid_backward_test, case_008_nullptr)
{
    auto tensorDesc = TensorDesc({10, 5}, ACL_FLOAT, ACL_FORMAT_ND);
    int64_t reduction = 0;

    auto ut_1 = OP_API_UT(aclnnSigmoidBackward, INPUT(nullptr, tensorDesc), OUTPUT(tensorDesc));
    // SAMPLE: only test GetWorkspaceSize
    uint64_t workspace_size = 0;
    aclnnStatus aclRet = ut_1.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut_2 = OP_API_UT(aclnnSigmoidBackward, INPUT(tensorDesc, nullptr), OUTPUT(tensorDesc));
    // SAMPLE: only test GetWorkspaceSize
    aclRet = ut_2.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut_3 = OP_API_UT(aclnnSigmoidBackward, INPUT(tensorDesc, tensorDesc), OUTPUT(nullptr));
    // SAMPLE: only test GetWorkspaceSize
...
// 测试维度大于8
TEST_F(l2_sigmoid_backward_test, case_009_max_dim)
{
...
// 测试不能broadcast
TEST_F(l2_sigmoid_backward_test, case_010_cant_broadcast)
{
...
// 测试合法数据类型bfloat16
TEST_F(l2_sigmoid_backward_test, case_011_bfloat16)
{

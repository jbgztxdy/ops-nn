...
#include "../op_kernel/sigmoid_grad_tiling_data.h"
#include "../op_kernel/sigmoid_grad_tiling_key.h"

namespace optiling {

using namespace Ops::NN::OpTiling;

#define UB_NUM_FLOAT 10U
#define UB_NUM_OTHER 16U
constexpr uint32_t BUFFER_NUM = 2;
constexpr uint32_t WS_SYS_SIZE = 0;

struct SigmoidGradCompileInfo {};

static ge::graphStatus TilingParseForSigmoidGrad([[maybe_unused]] gert::TilingParseContext* context)
{
...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, uint64_t BLOCK_SIZE, uint64_t ubSize,
                                         ...
                                         uint64_t& tileDataNum, uint64_t& inputLengthAlgin)
{
    ...
    uint64_t ubDataNumber = (context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT) ? UB_NUM_FLOAT : UB_NUM_OTHER;
...
static ge::graphStatus CalculateCoreBlockNums(gert::TilingContext* context, uint64_t BLOCK_SIZE,
                                              uint64_t inputLengthAlgin, int64_t coreNum, uint64_t tileBlockNum,
...
// tiling 分发入口
static ge::graphStatus SigmoidGradTilingFunc(gert::TilingContext* context)
{
    // 1、获取平台运行信息
    ...
    ret = GetShapeAttrsInfo(context, BLOCK_SIZE, ubSize, inputNum, inputBytes, tileBlockNum, tileDataNum,
                            inputLengthAlgin);
    ...
    OP_CHECK_IF(GetWorkspaceSize(context) != ge::GRAPH_SUCCESS, OP_LOGE(context, "GetWorkspaceSize error"),
                return ge::GRAPH_FAILED);
    // 4、设置tiling信息
    SigmoidGradTilingData* tiling = context->GetTilingData<SigmoidGradTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(SigmoidGradTilingData), 0, sizeof(SigmoidGradTilingData)) != EOK,
    ...
    uint64_t smallCoreDataNum, bigCoreDataNum, smallTailDataNum, bigTailDataNum, finalSmallTileNum, finalBigTileNum,
        tailBlockNum;
    ret = CalculateCoreBlockNums(context, BLOCK_SIZE, inputLengthAlgin, coreNum, tileBlockNum, inputBytes, tileDataNum,
...
IMPL_OP_OPTILING(SigmoidGrad)
    .Tiling(SigmoidGradTilingFunc)
    .TilingParse<SigmoidGradCompileInfo>(TilingParseForSigmoidGrad);
} // namespace optiling

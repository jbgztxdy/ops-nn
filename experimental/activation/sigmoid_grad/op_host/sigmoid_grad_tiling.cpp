...
static ge::graphStatus SigmoidGradTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(
        memset_s(tiling, sizeof(SigmoidGradTilingData), 0, sizeof(SigmoidGradTilingData)) != EOK,
...
// tiling注册入口.
IMPL_OP_OPTILING(SigmoidGrad).Tiling(SigmoidGradTilingFunc).TilingParse<SigmoidGradCompileInfo>(TilingParseForSigmoidGrad);
} // namespace optiling

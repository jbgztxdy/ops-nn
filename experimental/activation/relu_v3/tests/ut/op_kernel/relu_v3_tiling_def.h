...
#ifndef __RELU_V3_HEADER__
#define __RELU_V3_HEADER__
...
#if defined(ASCENDC_CPU_DEBUG) || (defined(__DAV_CUBE__) && __NPU_ARCH__ == 2201) || (defined     (__DAV_CUBE__) && __NPU_ARCH__ == 3101) || defined(__GET_CODE_CHANNEL__)
...
for (uint64_t loop_dc=0; loop_dc < DC_PRLOAD_LOOP1; loop_dc++) {
    ...
    copy_gm_to_ubuf_align_v2((__ubuf__ uint8_t *)tilingdata_in_ub, (__gm__ uint8_t *)p_tilingdata, 0, 1, len_burst * 32, 0, 0, false, 0, 0);
...
#define GET_TILING_DATA_PTR_WITH_STRUCT(tiling_struct, dst_ptr, tiling_ptr)                                               REGISTER_TILINGDATA_SIZE(tiling_struct, __COUNTER__);                                                                 __tiling_data_ptr__ tiling_struct *dst_ptr = (__tiling_data_ptr__ tiling_struct *)tiling_ptr;

#define COPY_TILING_WITH_STRUCT(tiling_struct, src_ptr, dst_ptr)                                                              tiling_struct __ascendc_var##dst_ptr;                                                                                     copy_data_align64((uint8_t*)(&__ascendc_var##dst_ptr), (__gm__ uint8_t*)src_ptr, sizeof(tiling_struct));                  const tiling_struct* dst_ptr = &__ascendc_var##dst_ptr;

#define COPY_TILING_WITH_ARRAY(arr_type, arr_count, src_ptr, dst_ptr)                                                         arr_type __ascendc_var##dst_ptr[arr_count];                                                                               copy_data_align64((uint8_t*)(&__ascendc_var##dst_ptr), (__gm__ uint8_t*)src_ptr, sizeof(arr_type) * arr_count);           const arr_type (*dst_ptr)[arr_count] = (const arr_type(*)[arr_count])&__ascendc_var##dst_ptr;

#define TestUtDefaultTilingStruct ReluV3TilingData

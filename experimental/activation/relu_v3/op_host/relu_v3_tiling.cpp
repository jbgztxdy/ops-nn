...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(x.GetDimNum() > 0 && x[x.GetDimNum() - 1] % 8 != 0,
                OP_LOGE(context->GetNodeName(), "the last dim of `x` must be multiple of 8."), return ge::GRAPH_FAILED);
    long size = 1;
    for (auto i = 0; i < x.GetDimNum(); i++)
        size *= x[i];
    OP_CHECK_IF(size > std::numeric_limits<int>::max() / 512 * 512,
                OP_LOGE(context->GetNodeName(), "shape size is too big."), return ge::GRAPH_FAILED);
...
static ge::graphStatus TilingParse([[maybe_unused]] gert::TilingParseContext* context) { return ge::GRAPH_SUCCESS; }

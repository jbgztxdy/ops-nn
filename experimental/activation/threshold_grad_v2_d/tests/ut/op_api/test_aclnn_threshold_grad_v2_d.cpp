...
protected:
    static void SetUpTestCase() { std::cout << "l2_threshold_backward_test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "l2_threshold_backward_test TearDown" << std::endl; }
};

TEST_F(l2_threshold_backward_test, case_01_float)
{
    op::SetPlatformSocVersion(op::SocVersion::ASCEND910B);

    auto gradOutputDesc = TensorDesc({2, 2}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4});
    auto selfDesc = TensorDesc({2, 2}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4});
    auto outDesc = TensorDesc({2, 2}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4});
...
{
    auto gradOutputDesc = TensorDesc(
        {
            10,
        },
        ACL_INT32, ACL_FORMAT_ND);
    auto selfDesc = TensorDesc(
        {
            10,
        },
        ACL_INT32, ACL_FORMAT_ND);

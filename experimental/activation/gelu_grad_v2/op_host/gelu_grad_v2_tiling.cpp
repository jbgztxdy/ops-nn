...
#include "../op_kernel/gelu_grad_v2_tiling_key.h"

namespace optiling {

using namespace Ops::NN::OpTiling;
//SINGLE BUFFER
#define UB_NUM_F32_TANH_ONE 7U
#define UB_NUM_F32_ONE 5U
#define UB_NUM_F16_BF16_ONE 11U
#define UB_NUM_F16_BF16_TANH_ONE 14U
// DOUBLE BUFFER
#define UB_NUM_F32_TANH_TWO 10U
#define UB_NUM_F32_TWO 8U
#define UB_NUM_F16_BF16_TWO 14U
#define UB_NUM_F16_BF16_TANH_TWO 18U
...
static ge::graphStatus TilingParseForGeluGradV2([[maybe_unused]] gert::TilingParseContext* context)
{
...
static ge::graphStatus GetShapeAttrsInfo(
    gert::TilingContext* context, uint64_t ubSize, uint64_t coreNum, uint64_t& haveTanh, uint64_t& bufferOpen, uint64_t& inputNum, uint64_t& inputBytes,
    uint64_t& tileBlockNum, uint64_t& tileDataNum, uint64_t& inputLengthAlgin)
{
    ...
    if (attrs!=nullptr) {
        const char* attrS = attrs->GetStr(0); 
        if (attrS == nullptr || strcmp(attrS, "none") == 0) {
            haveTanh = ISNONE;
        } else if (strcmp(attrS, "tanh") == 0) {
            haveTanh = ISTANH;
        } else {
            OP_LOGE(context, "approximate is none or tanh!");
            return ge::GRAPH_FAILED;
        }
    }
    ...
    bufferOpen = 1;
    if  (context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT && haveTanh == 1) {
        if (inputLengthAlgin < coreNum * (((ubSize / BLOCK_SIZE) * BLOCK_SIZE) / UB_NUM_F32_TANH_ONE)) {
            bufferOpen = 0;
            ubDataNumber = UB_NUM_F32_TANH_ONE;
        } else {
            ubDataNumber = UB_NUM_F32_TANH_TWO;
        }
    } else if (context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT && haveTanh == 0) {
        if ((inputLengthAlgin < coreNum * (((ubSize / BLOCK_SIZE) * BLOCK_SIZE) / UB_NUM_F32_ONE))) {
            bufferOpen = 0;
        ...
        if ((inputLengthAlgin < coreNum * (((ubSize / BLOCK_SIZE) * BLOCK_SIZE) / UB_NUM_F16_BF16_ONE))) {
            bufferOpen = 0;
            ubDataNumber = UB_NUM_F16_BF16_ONE;
        } else {
            ubDataNumber = UB_NUM_F16_BF16_TWO;
        }
    } else {
        if ((inputLengthAlgin < coreNum * (((ubSize / BLOCK_SIZE) * BLOCK_SIZE) / UB_NUM_F16_BF16_ONE))) {
            bufferOpen = 0;
            ubDataNumber = UB_NUM_F16_BF16_TANH_ONE;
        } else {
            ubDataNumber = UB_NUM_F16_BF16_TANH_TWO;
        }
    }
...
// tiling 分发入口
static ge::graphStatus GeluGradV2TilingFunc(gert::TilingContext* context)
{
    // 1、获取平台运行信息
    uint64_t ubSize;
    int64_t coreNum;
    uint64_t haveTanh, bufferOpen;
    ...
    ret = GetShapeAttrsInfo(context, ubSize, coreNum, haveTanh, bufferOpen, inputNum, inputBytes, tileBlockNum, tileDataNum, inputLengthAlgin);
    ...
    OP_CHECK_IF(
        ...
        return ge::GRAPH_FAILED);
    // 4、设置tiling信息
    GeluGradV2TilingData* tiling = context->GetTilingData<GeluGradV2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(
        memset_s(tiling, sizeof(GeluGradV2TilingData), 0, sizeof(GeluGradV2TilingData)) != EOK,
...
// tiling注册入口.
IMPL_OP_OPTILING(GeluGradV2).Tiling(GeluGradV2TilingFunc).TilingParse<GeluGradV2CompileInfo>(TilingParseForGeluGradV2);
} // namespace optiling

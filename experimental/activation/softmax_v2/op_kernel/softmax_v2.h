...
template <typename T>
__aicore__ inline void SoftmaxV2<T>::Init(GM_ADDR x, GM_ADDR z, GM_ADDR workspace,
                                          const SoftmaxV2TilingData* tilingData)
{
    ...
    if (AscendC::GetBlockIdx() < tiling.big_core_num) {
        xGm.SetGlobalBuffer((__gm__ T*)x + this->blockLength * AscendC::GetBlockIdx() * lastdimnum,
                            this->blockLength * lastdimnum);
        ...
        zGm.SetGlobalBuffer((__gm__ T*)z + this->blockLength * AscendC::GetBlockIdx() * lastdimnum,
                            this->blockLength * lastdimnum);
    } else {
        xGm.SetGlobalBuffer(
            (__gm__ T*)x + (this->blockLength * AscendC::GetBlockIdx() + tiling.big_core_num) * lastdimnum,
            this->blockLength * lastdimnum);
        sGm.SetGlobalBuffer((__gm__ T*)workspace, tiling.sumspace);
        mGm.SetGlobalBuffer((__gm__ T*)workspace + tiling.sumspace, tiling.sumspace);
        zGm.SetGlobalBuffer(
            (__gm__ T*)z + (this->blockLength * AscendC::GetBlockIdx() + tiling.big_core_num) * lastdimnum,
            this->blockLength * lastdimnum);
    }
...
__aicore__ inline void SoftmaxV2<T>::CopyIn(int32_t progress, int32_t row, int32_t circle) //
{
    if (circle == 0 || circle == 1) {
        ...
        if (dim == dimNum - 1) { //求这一行的对应的行数
            ...
            AscendC::DataCopyExtParams copyParams{1, static_cast<uint32_t>(lastdimnum * sizeof(T)), 0, 0, 0};
            AscendC::DataCopyPadExtParams<T> padParams{true, 0, 0, 0};
            AscendC::DataCopyPad(mLocalin, mGm[elepartnum * lastdimnum * inparts + lastdimnum * eleinparts], copyParams,
                                 padParams);
            inQueueM.EnQue(mLocalin);
        }
    } else {
        ...
        if (dim == dimNum - 1) { //求这一行的对应的行数
            ...
            AscendC::DataCopyExtParams copyParams{1, static_cast<uint32_t>(lastdimnum * sizeof(T)), 0, 0, 0};
            AscendC::DataCopyPadExtParams<T> padParams{true, 0, 0, 0};
            AscendC::DataCopyPad(sLocalin, sGm[elepartnum * lastdimnum * inparts + lastdimnum * eleinparts], copyParams,
                                 padParams);
            AscendC::LocalTensor<T> mLocalin = inQueueM.AllocTensor<T>();
            AscendC::DataCopyPad(mLocalin, mGm[elepartnum * lastdimnum * inparts + lastdimnum * eleinparts], copyParams,
                                 padParams);
...
__aicore__ inline void SoftmaxV2<T>::CopyOut(int32_t progress, int32_t row, int32_t circle)
{
    if (circle == 0) {
        if (dim == dimNum - 1) {
            ...
            AscendC::DataCopyExtParams copyParams2{static_cast<uint16_t>(1), static_cast<uint32_t>(sizeof(T)), 0, 0,
            ...
            int eleinparts = hangshu % inparts;                 //部内偏移
            AscendC::LocalTensor<T> mLocalout = outQueueM.DeQue<T>();
            AscendC::DataCopyExtParams copyParams2{static_cast<uint16_t>(1),
                                                   static_cast<uint32_t>(lastdimnum * sizeof(T)), 0, 0,
                                                   0}; // 结构体DataCopyExtParams最后一个参数是rsv保留位
            AscendC::DataCopyPad(mGm[elepartnum * lastdimnum * inparts + lastdimnum * eleinparts], mLocalout,
                                 copyParams2);
            outQueueM.FreeTensor(mLocalout);
        }
    } else if (circle == 1) {
        if (dim == dimNum - 1) {
            ...
            AscendC::DataCopyPad(sGm[elepartnum * lastdimnum * inparts + lastdimnum * eleinparts], sLocalout,
                                 copyParams2);
        ...
        AscendC::DataCopyExtParams copyParams2{static_cast<uint16_t>(1), static_cast<uint32_t>(sizeof(T) * lastdimnum),
                                               0, 0, 0}; // 结构体DataCopyExtParams最后一个参数是rsv保留位

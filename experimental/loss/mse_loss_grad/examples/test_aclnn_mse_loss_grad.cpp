...
int main()
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建target aclTensor
    ret = CreateAclTensor(targetHostData, targetShape, &targetDeviceAddr, aclDataType::ACL_FLOAT16, &target);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建out aclTensor
    ...
    ret = aclnnMseLossGradGetWorkspaceSize(self, target, gradOutput, (char*)reduction.c_str(), out, &workspaceSize,
                                           &executor);

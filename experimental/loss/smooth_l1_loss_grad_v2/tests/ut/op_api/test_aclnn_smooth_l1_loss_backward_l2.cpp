...
{
    auto grad_output_desc = TensorDesc({1}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{0.1});
    auto self_desc = TensorDesc({1}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{0.32});
    auto target_desc = TensorDesc({1}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1});
...
TEST_F(l2_smooth_l1_loss_backward_test, case_nullptr)
{
    ...
    auto self_desc = TensorDesc({1, 0, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto target_desc = TensorDesc({1, 0, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    int64_t reduction = NONE;
    float beta = 1;
    auto grad_input_desc = TensorDesc(grad_output_desc).Precision(0.0001, 0.0001);
...
{
    auto grad_output_desc = TensorDesc({1, 4, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto self_desc = TensorDesc({1, 4, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto target_desc = TensorDesc({1, 4, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    int64_t reduction = NONE;
    float beta = 1;
    auto grad_input_desc = TensorDesc(grad_output_desc).Precision(0.0001, 0.0001);
...
{
    auto grad_output_desc = TensorDesc({1, 4, 4, 1}, ACL_FLOAT16, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto self_desc = TensorDesc({1, 4, 4, 1}, ACL_FLOAT16, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto target_desc = TensorDesc({1, 4, 4, 1}, ACL_FLOAT16, ACL_FORMAT_ND).ValueRange(-2, 2);
    int64_t reduction = NONE;
    float beta = 1;
    auto grad_input_desc = TensorDesc(grad_output_desc).Precision(0.0001, 0.0001);
...
{
    vector<aclDataType> DtypeList = {ACL_FLOAT, ACL_FLOAT, ACL_FLOAT, ACL_DOUBLE};
    for (int i = 0; i < 4; i++) {
        auto grad_output_desc = TensorDesc({1, 4, 4, 1}, DtypeList[i % 4], ACL_FORMAT_ND).ValueRange(-2, 2);
        auto self_desc = TensorDesc({1, 4, 4, 1}, DtypeList[(i + 1) % 4], ACL_FORMAT_ND).ValueRange(-2, 2);
...
{
    auto grad_output_desc = TensorDesc({1, 1, 4, 1, 1, 1, 1, 1, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto self_desc = TensorDesc({1, 1, 4, 1, 1, 1, 1, 1, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto target_desc = TensorDesc({1, 1, 4, 1, 1, 1, 1, 1, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
    int64_t reduction = NONE;
    float beta = 1;
    auto grad_input_desc = TensorDesc(grad_output_desc).Precision(0.0001, 0.0001);
...
{
    for (int i = 0; i <= 3; i++) {
        auto grad_output_desc = TensorDesc({1, 1, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
        auto self_desc = TensorDesc({1, 1, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
        auto target_desc = TensorDesc({1, 1, 4, 1}, ACL_FLOAT, ACL_FORMAT_ND).ValueRange(-2, 2);
...
{
    vector<aclFormat> ValidList = {ACL_FORMAT_NCHW,  ACL_FORMAT_NHWC,  ACL_FORMAT_ND, ACL_FORMAT_HWCN,
                                   ACL_FORMAT_NDHWC, ACL_FORMAT_NCDHW, ACL_FORMAT_NCL};
...
{
    auto grad_output_desc = TensorDesc({1, 4, 4, 1}, ACL_BF16, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto self_desc = TensorDesc({1, 4, 4, 1}, ACL_BF16, ACL_FORMAT_ND).ValueRange(-2, 2);
    auto target_desc = TensorDesc({1, 4, 4, 1}, ACL_BF16, ACL_FORMAT_ND).ValueRange(-2, 2);
    int64_t reduction = NONE;
    float beta = 1;
...
{
    for (int i = 0; i <= 3; i++) {
        auto grad_output_desc = TensorDesc({1, 1, 4, 1}, ACL_FLOAT16, ACL_FORMAT_ND).ValueRange(-2, 2);
        auto self_desc = TensorDesc({1, 1, 4, 1}, ACL_FLOAT16, ACL_FORMAT_ND).ValueRange(-2, 2);
        auto target_desc = TensorDesc({1, 1, 4, 1}, ACL_FLOAT16, ACL_FORMAT_ND).ValueRange(-2, 2);
        int64_t reduction = (enum Reduction)i;
        float beta = 1;
        auto grad_input_desc = TensorDesc(grad_output_desc).Precision(0.001, 0.001);
...
{
    for (int i = 0; i <= 3; i++) {
        auto grad_output_desc = TensorDesc({1, 1, 4, 1}, ACL_BF16, ACL_FORMAT_ND).ValueRange(-2, 2);
        auto self_desc = TensorDesc({1, 1, 4, 1}, ACL_BF16, ACL_FORMAT_ND).ValueRange(-2, 2);
        auto target_desc = TensorDesc({1, 1, 4, 1}, ACL_BF16, ACL_FORMAT_ND).ValueRange(-2, 2);
        int64_t reduction = (enum Reduction)i;
        float beta = 1;

...
protected:
    ...
    {
        std::cout << "SigmoidCrossEntropyWithLogitsGradV2TilingTest TearDown" << std::endl;
    }
};

TEST_F(SigmoidCrossEntropyWithLogitsGradV2TilingTest, test_tiling_fp32_none)
{
    std::string op_type("SigmoidCrossEntropyWithLogitsGradV2");
    ...
    gert::StorageShape predict_shape = {{32, 64}, {32, 64}};
    gert::StorageShape target_shape = {{32, 64}, {32, 64}};
    gert::StorageShape dout_shape = {{32, 64}, {32, 64}};
    gert::StorageShape grad_shape = {{32, 64}, {32, 64}};

    auto holder = gert::TilingContextFaker()
                      .SetOpType("SigmoidCrossEntropyWithLogitsGradV2")
                      .NodeIoNum(3, 1)
                      .IrInstanceNum({1, 1, 1})
                      .InputShapes({&predict_shape, &target_shape, &dout_shape})
                      .OutputShapes({&grad_shape})

...
int main()
{
    ...
    // reduction=0(none) 时输出 shape 必须与输入 shape 一致
    std::vector<int64_t> zShape = xShape;
    ...
    std::vector<float> yHostData = {0, 0.5, 2, 1};
    std::vector<float> zHostData = {0, 0, 0, 0};
    ...
    ret = aclnnSmoothL1LossGetWorkspaceSize(x, y, reduction, beta, z, &workspaceSize, &executor);
    ...
    for (int64_t i = 0; i < size; i++) {
        LOG_PRINT("aclnnSmoothL1Loss result[%ld] is: %f\n", i, resultData[i]);
    }

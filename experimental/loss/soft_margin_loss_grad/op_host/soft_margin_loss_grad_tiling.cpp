...
/*!
 * \file soft_margin_loss_grad_tiling.cpp
 * \brief
 */
...
#include "../op_kernel/soft_margin_loss_grad_tiling_key.h"
...
struct SoftMarginLossGradCompileInfo {};
...
static ge::graphStatus SoftMarginLossGradTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(GetWorkspaceSize(context) != ge::GRAPH_SUCCESS, OP_LOGE(context, "GetWorkspaceSize error"),
                return ge::GRAPH_FAILED);
    // 4、设置tiling信息
    SoftMarginLossGradTilingData* tiling = context->GetTilingData<SoftMarginLossGradTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(SoftMarginLossGradTilingData), 0, sizeof(SoftMarginLossGradTilingData)) != EOK,
    ...
    float cof = 1.0f;
    if (reduction == "mean" && inputNum > 0) {
        cof = 1.0f / static_cast<float>(inputNum);
    }
    tiling->cof = cof;

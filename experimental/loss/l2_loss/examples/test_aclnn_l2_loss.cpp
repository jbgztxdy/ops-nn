...
int main()
{
    ...
    // L2Loss 输出为标量，shape = {1}
    std::vector<int64_t> outShape = {1};
    std::vector<float> outHostData(1, 0.0);
    ...
    // L2Loss op_def 中无 attribute，接口只有 x（输入）和 out（输出）
    ret = aclnnL2LossGetWorkspaceSize(selfX, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnL2LossGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    void* workspaceAddr = nullptr;
    if (workspaceSize > static_cast<uint64_t>(0)) {
        ret = aclrtMalloc(&workspaceAddr, workspaceSize, ACL_MEM_MALLOC_HUGE_FIRST);
        CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("allocate workspace failed. ERROR: %d\n", ret); return ret);
    }
    ret = aclnnL2Loss(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnL2Loss failed. ERROR: %d\n", ret); return ret);

 ...
 */
 ...
 #include <vector>
 #include "acl/acl.h"
 #include "aclnn_binary_cross_entropy_v2.h"
 ...
 int main()
 {
     ...
     std::vector<float> selfXHostData(21, 0.3f);
     ...
     std::vector<float> selfYHostData(21, 1.0f);
     ...
     // 4. 调用aclnnBinaryCrossEntropyV2第一段接口
     ret = aclnnBinaryCrossEntropyV2GetWorkspaceSize(selfX,selfY, out, &workspaceSize, &executor);
     // CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnBinaryCrossEntropyV2GetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
     if (ret != ACL_SUCCESS) {
         const char* errMsg = aclGetRecentErrMsg();          // ← 关键一行
         LOG_PRINT("[ERROR] aclnnBinaryCrossEntropyV2GetWorkspaceSize failed, ret = %d\n", ret);
     ...
     // 5. 调用aclnnBinaryCrossEntropyV2第二段接口
     ret = aclnnBinaryCrossEntropyV2(workspaceAddr, workspaceSize, executor, stream);
     CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnBinaryCrossEntropyV2 failed. ERROR: %d\n", ret); return ret);

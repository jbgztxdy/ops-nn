...
/*!
 * \file binary_cross_entropy_v2_tiling.cpp
 * \brief
 */
 ...
 #include "../op_kernel/binary_cross_entropy_v2_tiling_data.h"
 #include "../op_kernel/binary_cross_entropy_v2_tiling_key.h"
 ...
 struct BinaryCrossEntropyV2CompileInfo {};
 ...
 static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t& totalIdx, ge::DataType& dataType)
 {
     ...
     // 校验维度数一致
     if (inputShapeX.GetDimNum() != outShapeZ.GetDimNum()|| inputShapeX.GetDimNum() != outShapeZ.GetDimNum()) {
         shapeMatch = false;
     } else {
         // 校验每个维度的大小一致
         size_t dimNum = inputShapeX.GetDimNum();
         for (size_t i = 0; i < dimNum; i++) {
             if (inputShapeX.GetDim(i) != outShapeZ.GetDim(i)|| inputShapeX.GetDim(i) != outShapeZ.GetDim(i)) {
                 shapeMatch = false;
                 break;
             }
         }
     ...
     OP_CHECK_IF(
         !shapeMatch,
         OP_LOGE(
             context, "BinaryCrossEntropyV2: inputx,inputy,outputz shape not match! dim num: x=%zu, y=%zu,  z=%zu",
 ...
 // tiling 分发入口
 // 可直接替换你的 BinaryCrossEntropyV2TilingFunc 内部实现（保留函数签名）
 static ge::graphStatus BinaryCrossEntropyV2TilingFunc(gert::TilingContext* context)
 {
     // 1. platform
     uint64_t ubSize = 0;
     int64_t coreNum = 0;
     ...
     BinaryCrossEntropyV2TilingData* tiling = context->GetTilingData<BinaryCrossEntropyV2TilingData>();
     OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
     OP_CHECK_IF(memset_s(tiling, sizeof(BinaryCrossEntropyV2TilingData), 0, sizeof(BinaryCrossEntropyV2TilingData)) != EOK,
     ...
     uint32_t ubDataNumber = (inputBytes == 1ULL) ? 6U : 4U;
 ...
 static ge::graphStatus TilingParseForBinaryCrossEntropyV2([[maybe_unused]] gert::TilingParseContext* context)
 {
     return ge::GRAPH_SUCCESS;
 }
 
 // tiling注册入口.
 IMPL_OP_OPTILING(BinaryCrossEntropyV2).Tiling(BinaryCrossEntropyV2TilingFunc).TilingParse<BinaryCrossEntropyV2CompileInfo>(TilingParseForBinaryCrossEntropyV2);
 } // namespace optiling

...
* @li var: Src tensor. An ND tensor. Must be the following type: int8, float8_e4m3fn, float8_e5m2, hifloat8. Shape
support 3-8D.
* @li indices: An ND tensor. Must be one of the following types: int32 or int64. Shape support 1D or 2D. Empty tensor
not supported.
...
* @li updates: Update tensor. An ND tensor. Must be one of the following types: bfloat16, float16. The dim num should be
same with var. Empty tensor not supported.
* updates.dim[0] = indices.dim[0], updates.dim[0] <= var.dim[0], updates.dim[axis] <= var.dim[axis], updates.dim[i] =
var.dim[i] for i != 0 && i != axis.
* @li quant_scales: Quant scale tensor. An ND tensor. Must be one of the following types: bfloat16, float32. Shape
support 1-8D. quant_scales.size() = updates.dim[quant_axis].
* @li quant_zero_points: Quant offset tensor. An ND optional tensor. Must be one of the following types: bfloat16,
int32. Shape support 1-8D.
...
* @li axis: The dim to update. An optional attribute. Defaults to -2, dtype is int64, the value range is [1,
len(updates.shape) -2] or [1 - len(updates.shape)，-2].
...
* @li round_mode: An optional string, specifying the cast type. The value range is ["rint", "round", "hybrid"]. Defaults
to "rint".
*
* @par Outputs:

 ...
 * device侧的aclTensor，数据类型支持INT8，FLOAT8_E4M3FN，FLOAT8_E5M2，HIFLOAT8。维度数量需要与updates一致。
 ...
 * @param [in] roundMode: 指定数据转换的模式，支持{"rint", "round",
 * "hybrid"}模式。selfRef的Dtype为2/35/36，对应的数据类型为INT8/FLOAT8_E4M3FN/FLOAT8_E5M2时，仅支持{"rint"}。Dtype为34，对应的数据类型为HIFLOAT8，支持{"round",
 * "hybrid"}。
...
ACLNN_API aclnnStatus aclnnInplaceQuantScatterV2GetWorkspaceSize(aclTensor* selfRef, const aclTensor* indices,
                                                                 ...
                                                                 const char* roundMode, uint64_t* workspaceSize,

...
protected:
    static void SetUpTestCase() { cout << "unique_dim_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "unique_dim_test TearDown" << endl; }
};
...
TEST_F(l2_unique_dim_test, case_016_EMPTY)
{
    ...
    int64_t dim = 0;
    auto ut = OP_API_UT(aclnnUniqueDim, INPUT(self, sorted, return_inverse, dim), OUTPUT(output, indices, counts));
...
TEST_F(l2_unique_dim_test, case_017_NULLPTR)
{
    ...
    auto ut1 = OP_API_UT(aclnnUniqueDim, INPUT(nullptr, sorted, return_inverse, dim), OUTPUT(output, indices, counts));
    aclRet = ut1.TestGetWorkspaceSize(&workspaceSize);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut2 = OP_API_UT(aclnnUniqueDim, INPUT(self, sorted, return_inverse, dim), OUTPUT(nullptr, indices, counts));
    aclRet = ut2.TestGetWorkspaceSize(&workspaceSize);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut3 = OP_API_UT(aclnnUniqueDim, INPUT(self, sorted, return_inverse, dim), OUTPUT(output, nullptr, counts));
    aclRet = ut3.TestGetWorkspaceSize(&workspaceSize);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut4 = OP_API_UT(aclnnUniqueDim, INPUT(self, sorted, return_inverse, dim), OUTPUT(output, indices, nullptr));

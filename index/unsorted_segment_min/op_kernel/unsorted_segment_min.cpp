                                                           ...
                                                           GM_ADDR output, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(TEMPLATE_SIMT_TILING_KEY)) {
        ...
        UnsortedSegment::KernelUnsortedSegment<DTYPE_X, DTYPE_SEGMENT_IDS, SimtComputeSegmentMin<DTYPE_X>,
                                               InitGmMaxValue<DTYPE_X>>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
        ...
        UnsortedSegment::KernelSimdSplitCol<DTYPE_X, DTYPE_SEGMENT_IDS, GetInitMaxValue<DTYPE_X>,
                                            ComputeMin<DTYPE_X, uint64_t>>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
        ...
        if constexpr (std::is_same<uint32_t, DTYPE_X>::value || std::is_same<uint64_t, DTYPE_X>::value ||
            ...
            UnsortedSegment::KernelSimdNonSort<DTYPE_X, DTYPE_SEGMENT_IDS, InitGmMaxValue<DTYPE_X>,
                                               SetAtomicMinOp<DTYPE_X>>
                op(&tilingData, &pipe);
            op.Init(x, segment_ids, output);
            op.Process();
        }
        ...
        if constexpr (std::is_same<uint32_t, DTYPE_X>::value || std::is_same<uint64_t, DTYPE_X>::value ||
                      std::is_same<int64_t, DTYPE_X>::value) {
            UnsortedSegment::KernelUnsortedSegmentFL<DTYPE_X, DTYPE_SEGMENT_IDS, SimtGatherMinValue<DTYPE_X>,
                                                     GetInitMaxValue<DTYPE_X>, InitGmMaxValue<DTYPE_X>,
                                                     ComputeMin<DTYPE_X, int32_t>, SetAtomicMinOp<DTYPE_X>>
                op(&pipe);
        ...
        UnsortedSegment::KernelUnsortedSegmentSortSimt<DTYPE_X, DTYPE_SEGMENT_IDS, SimtGatherMinValue<DTYPE_X>,
                                                       SimtComputeSegmentMin<DTYPE_X>, GetInitMaxValue<DTYPE_X>,
                                                       InitGmMaxValue<DTYPE_X>>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();

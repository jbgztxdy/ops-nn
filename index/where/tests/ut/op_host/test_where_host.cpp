...
#define WHERE_SUCCESS_CASE(aicpu_type, base_type)                                                    \
    ...
    {                                                                                                \
        ...
        base_type input1_tensor_buffer[] = {(base_type)0, (base_type)2, (base_type)3, (base_type)3,  \
                                            (base_type)1, (base_type)0, (base_type)0, (base_type)1}; \
        ...
        auto kernelHolder = gert::KernelRunContextFaker()                                            \
                                ...
                                .Outputs({reinterpret_cast<void*>(&output_shape),                    \
                                          reinterpret_cast<void*>(&output_tensor_data)})             \
                                .NodeIoNum(1, 1)                                                     \
                                .IrInstanceNum({1})                                                  \
...
struct WhereHostKernelTest : public testing::Test {
    WhereHostKernelTest() { whereHostKernel = aicpu::AicpuHostKernelRegister::GetInstance()->GetFunc("Where"); }
...
TEST_F(WhereHostKernelTest, test_empty_tensor)
{
    ...
    auto kernelHolder = gert::KernelRunContextFaker()
                            .KernelIONum(2, 2)
                            .Inputs(
                                {reinterpret_cast<void*>(&input1_shape), reinterpret_cast<void*>(&input1_tensor_data)})
                            .Outputs(
                                {reinterpret_cast<void*>(&output_shape), reinterpret_cast<void*>(&output_tensor_data)})
                            .NodeIoNum(1, 1)
                            .IrInstanceNum({1})

...
public:
    ...
    __aicore__ inline void ProcessSegment(int64_t& segmentOffset, int64_t segmentIdsLength);
...
template <typename T1>
__aicore__ inline void SparseSegmentMeanMultiCoreKernel<T1>::Init(GM_ADDR y, GM_ADDR workspace, AscendC::TPipe& pipeIn,
                                                                  const SparseSegmentMeanSimdTilingData* tilingData)
{
    ...
    numResultWorkspace_.SetGlobalBuffer((__gm__ int64_t*)workspace +
                                        ...
                                        GM_NUM_OFFSET_BYTE / sizeof(int64_t)); // 局部
    segmentIdWorkspace_.SetGlobalBuffer(
        (__gm__ int64_t*)workspace + tilingData_->indicesOuter * DOUBLE * tilingData_->innerSize / DOUBLE +
        GM_NUM_OFFSET_BYTE / sizeof(int64_t) +
        DOUBLE * tilingData_->indicesOuter * (GM_NUM_OFFSET_BYTE / sizeof(int64_t))); // 局部
                                                                            ...
                                                                            int64_t segmentIdsLength)
{
    ...
    perInnerNum = tilingData_->outBufferSize / sizeof(float) < perInnerNum ?
                      tilingData_->outBufferSize / sizeof(float) :
                      perInnerNum;
...
template <typename T1>
__aicore__ inline void SparseSegmentMeanMultiCoreKernel<T1>::CopyIn(LocalTensor<float>& xLocal, int32_t burstLen,
...
__aicore__ inline void SparseSegmentMeanMultiCoreKernel<T1>::CopyOut(int64_t count, int64_t yGmOffset)
{
    ...
    DataCopyExtParams copyOutParamT1 = {static_cast<uint16_t>(1), static_cast<uint32_t>(count * sizeof(T1)),

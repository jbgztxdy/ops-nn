    ...
    GM_ADDR x, GM_ADDR y, const DataFormatDimMapTilingData* tilingData)
{
    ...
    // 向上取整覆盖 ubLength_ 非 8 倍数的尾位，再按 32 字节对齐，避免 Compares 写 mask 越界
    uint32_t cmpBytes = (((ubLength_ + 7) / 8 + 31) / 32) * 32;
...
__aicore__ inline void DataFormatDimMap<T>::Compute(int64_t currentNum)
{
    ...
    // INT64 输入通过 int32 中转再到 float。算子语义为维度索引，取值范围 [-formatLen, formatLen)，
    // formatLen <= 5，恒不溢出 int32，因此该中转无精度损失。

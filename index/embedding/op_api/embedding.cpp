...
const aclTensor* EmbeddingAiCore(const aclTensor* self, const aclTensor* indices, aclTensor* embeddingOut,
                                 aclOpExecutor* executor)
{
    L0_DFX(EmbeddingAiCore, self, indices);

    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(Embedding, OP_INPUT(self, indices), OP_OUTPUT(embeddingOut));
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "EmbeddingAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."), return nullptr);
    return embeddingOut;
}

...
__global__ __aicore__ void reverse_sequence(GM_ADDR x, GM_ADDR seq_lengths, GM_ADDR y, GM_ADDR workspace,
                                            GM_ADDR tiling)
{
    AscendC::TPipe pipeBase;
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY);
    REGISTER_TILING_DEFAULT(ReverseSequenceTilingData);
    if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_SIMT && DYTPE_MODE == TPL_MODE_DTYPE_B8 &&
        ...
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_SIMT && DYTPE_MODE == TPL_MODE_DTYPE_B8 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceSimtTilingData4RegBase, tilingData, tiling);
        ReverseSequenceSimt<int8_t, DTYPE_SEQ_LENGTHS, uint64_t> op(&pipeBase, &tilingData);
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_SIMT && DYTPE_MODE == TPL_MODE_DTYPE_B16 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceSimtTilingData4RegBase, tilingData, tiling);
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BSA && DYTPE_MODE == TPL_MODE_DTYPE_B64 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BSA && DYTPE_MODE == TPL_MODE_DTYPE_B32 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBSATilingData, tilingData, tiling);
        ReverseSequenceBSA<int32_t, DTYPE_SEQ_LENGTHS> op(&pipeBase, &tilingData);
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BSA && DYTPE_MODE == TPL_MODE_DTYPE_B16 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBSATilingData, tilingData, tiling);
        ReverseSequenceBSA<int16_t, DTYPE_SEQ_LENGTHS> op(&pipeBase, &tilingData);
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BSA && DYTPE_MODE == TPL_MODE_DTYPE_B8 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBSATilingData, tilingData, tiling);
        ReverseSequenceBSA<int8_t, DTYPE_SEQ_LENGTHS> op(&pipeBase, &tilingData);
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BS && DYTPE_MODE == TPL_MODE_DTYPE_B8 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BS && DYTPE_MODE == TPL_MODE_DTYPE_B16 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBSTilingData, tilingData, tiling);
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BS && DYTPE_MODE == TPL_MODE_DTYPE_B32 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBSTilingData, tilingData, tiling);
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BS && DYTPE_MODE == TPL_MODE_DTYPE_B64 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBSTilingData, tilingData, tiling);
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_SBA && DYTPE_MODE == TPL_MODE_DTYPE_B8 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_SBA && DYTPE_MODE == TPL_MODE_DTYPE_B16 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceA1SBATilingData, tilingData, tiling);
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_SBA && DYTPE_MODE == TPL_MODE_DTYPE_B32 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceA1SBATilingData, tilingData, tiling);
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_SBA && DYTPE_MODE == TPL_MODE_DTYPE_B64 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT32) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceA1SBATilingData, tilingData, tiling);
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BAS && DYTPE_MODE == TPL_MODE_DTYPE_B64 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        ...
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BAS && DYTPE_MODE == TPL_MODE_DTYPE_B32 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBASTilingData, tilingData, tiling);
        ReverseSequenceBAS<int32_t, DTYPE_SEQ_LENGTHS> op(&pipeBase, &tilingData);
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BAS && DYTPE_MODE == TPL_MODE_DTYPE_B16 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBASTilingData, tilingData, tiling);
        ReverseSequenceBAS<int16_t, DTYPE_SEQ_LENGTHS> op(&pipeBase, &tilingData);
        op.Init(x, seq_lengths, y);
        op.Process();
    } else if constexpr (TEMPLATE_MODE == TPL_MODE_TEMPLATE_BAS && DYTPE_MODE == TPL_MODE_DTYPE_B8 &&
                         ADDR_MODE == TPL_MODE_ADDR_INT64) {
        GET_TILING_DATA_WITH_STRUCT(ReverseSequenceBASTilingData, tilingData, tiling);

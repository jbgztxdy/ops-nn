...
/*!
 * \file unsorted_segment_prod_infershape.cpp
 * \brief
 */
...
static graphStatus InferShape4UnsortedSegmentProd(gert::InferShapeContext* context)
{
    const char* opName_ = "UnsortedSegmentProd";
    OP_LOGD(context->GetNodeName(), "Begin to do UnsortedSegmentProd InferShape");
    ...
    if (num_segments_tensor->GetShapeSize() > 1) {
        OP_LOGE_FOR_INVALID_SHAPESIZE(opName_, "num_segments",
...
IMPL_OP_INFERSHAPE(UnsortedSegmentProd)
    .InferShape(InferShape4UnsortedSegmentProd)

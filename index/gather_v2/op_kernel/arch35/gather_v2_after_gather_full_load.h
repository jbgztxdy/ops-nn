...
private:
    ...
    static __simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_BOUND) inline void GatherSimt3D(
        const U yIndexBase, U currentCoreElements, U m0, U shift0, U m1, U shift1, int64_t pOffset, U gatherSize,
        U innerSize, U gatherDimSize, __ubuf__ T* srcTensor, __gm__ INDICES_T* indices, __gm__ volatile T* y);

    template <const bool NIS>
    static __simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_BOUND) inline void GatherSimt2D(
        const U yIndexBase, U currentCoreElements, U m0, U shift0, U innerSize, U gatherDimSize, __ubuf__ T* srcTensor,
...
template <typename T, typename INDICES_T, typename U>
template <const bool NIS>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_BOUND) inline void Gatherv2FullLoad<T, INDICES_T, U>::GatherSimt3D(
    const U yIndexBase, U currentCoreElements, U m0, U shift0, U m1, U shift1, int64_t pOffset, U gatherSize,
    U innerSize, U gatherDimSize, __ubuf__ T* srcTensor, __gm__ INDICES_T* indices, __gm__ volatile T* y)
{
    for (U index = static_cast<U>(threadIdx.x); index < currentCoreElements; index += static_cast<U>(blockDim.x)) {
...
template <typename T, typename INDICES_T, typename U>
template <const bool NIS>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_BOUND) inline void Gatherv2FullLoad<T, INDICES_T, U>::GatherSimt2D(
    const U yIndexBase, U currentCoreElements, U m0, U shift0, U innerSize, U gatherDimSize, __ubuf__ T* srcTensor,
    __gm__ INDICES_T* indices, __gm__ volatile T* y)
{
    for (U index = static_cast<U>(threadIdx.x); index < currentCoreElements; index += static_cast<U>(blockDim.x)) {
...
template <typename T, typename INDICES_T, typename U>
__aicore__ inline void Gatherv2FullLoad<T, INDICES_T, U>::CopyIn(LocalTensor<T> xLocal, GlobalTensor<T> xGm,
                                                                 int64_t offset, uint32_t nBurst, uint64_t copyLen)
{
...
__aicore__ inline void Gatherv2FullLoad<T, INDICES_T, U>::LoopColProcess()
{
    ...
    int64_t curCoreData = (blockIdx_ == needCoreNum_ - 1) ? tilingData_->lastCoreElements :
                                                            tilingData_->perCoreElements;
    ...
    for (int64_t loop = 0; loop < loopNum_; ++loop) {
        ...
        if (unlikely(negativeIndexSupport_)) {
            asc_vf_call<GatherSimt3D<true>>(
                dim3(threadNum_), yIndexBase, currentCoreElements_, m0, shift0, m1, shift1, pOffset, gatherSize_,
            ...
            asc_vf_call<GatherSimt3D<false>>(
                dim3(threadNum_), yIndexBase, currentCoreElements_, m0, shift0, m1, shift1, pOffset, gatherSize_,
                innerSize_, gatherDimSize_, (__ubuf__ T*)(gaBuffer.GetPhyAddr()),
                (__gm__ INDICES_T*)(indicesGm_.GetPhyAddr()), (__gm__ volatile T*)(yGm_.GetPhyAddr()));
        }
...
__aicore__ inline void Gatherv2FullLoad<T, INDICES_T, U>::NoLoopProcess()
{
    ...
    U yIndexBase = blockIdx_ * tilingData_->perCoreElements * innerSize_;
    if (unlikely(negativeIndexSupport_)) {
        asc_vf_call<GatherSimt2D<true>>(dim3(threadNum_), yIndexBase, currentCoreElements_, m0, shift0, innerSize_,
                                        gatherDimSize_, (__ubuf__ T*)(gaBuffer.GetPhyAddr()),
                                        (__gm__ INDICES_T*)(indicesGm_.GetPhyAddr()),
                                        (__gm__ volatile T*)(yGm_.GetPhyAddr()));
    } else {
        asc_vf_call<GatherSimt2D<false>>(dim3(threadNum_), yIndexBase, currentCoreElements_, m0, shift0, innerSize_,
                                         gatherDimSize_, (__ubuf__ T*)(gaBuffer.GetPhyAddr()),
                                         (__gm__ INDICES_T*)(indicesGm_.GetPhyAddr()),
                                         (__gm__ volatile T*)(yGm_.GetPhyAddr()));
    }
...
template <typename T, typename INDICES_T, typename U>
__aicore__ inline void Gatherv2FullLoad<T, INDICES_T, U>::Init(GM_ADDR x, GM_ADDR indices, GM_ADDR axis, GM_ADDR y,
                                                               const GatherV2TilingData* tilingData)
{

...
static const std::initializer_list<DataType> DTYPE_SUPPORT_LIST = {
    DataType::DT_FLOAT,  DataType::DT_FLOAT16,   DataType::DT_UINT64,     DataType::DT_INT64, DataType::DT_UINT32,
    DataType::DT_INT32,  DataType::DT_UINT16,    DataType::DT_INT16,      DataType::DT_UINT8, DataType::DT_INT8,
    DataType::DT_DOUBLE, DataType::DT_COMPLEX64, DataType::DT_COMPLEX128, DataType::DT_BOOL,  DataType::DT_BF16};
...
static bool CheckDtypeValid(const aclTensor* self, const aclTensor* index, const aclTensor* out)
{
    ...
    if (self->GetViewShape().GetDimNum() > MAX_INPUT_DIM_NUM) {
        OP_LOGW("self dim num is %zu, greater than %lu, may cause performance issue.", self->GetViewShape().GetDimNum(),
                MAX_INPUT_DIM_NUM);
    }
    ...
    if (self->GetDataType() == DataType::DT_BF16) {
        if (GetCurrentPlatformInfo().GetCurNpuArch() != NpuArch::DAV_2201 && !Ops::NN::AclnnUtil::IsRegbase()) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "take operator doesn't support [%s], except on ASCEND910B or ASCEND950 chip",

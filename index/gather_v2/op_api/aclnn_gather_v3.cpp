...
static inline bool CheckShape(const aclTensor *self, int64_t dim, const aclTensor *index, int64_t batchDims, const aclTensor *out) {
  ...
  if (batchDims != 0) {
    ...
    for (int64_t i = 0; i < batchDims; i++) {
      if (self->GetViewShape().GetDim(i) != index->GetViewShape().GetDim(i)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "aclnnGatherV3, self.shape[:batch_dims] should be equal to indices.shape[:batch_dims].");
        return false;
      }
    }
  }

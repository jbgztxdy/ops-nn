...
public:
    ...
    __aicore__ inline void Init(GM_ADDR indices, GM_ADDR outputShape, GM_ADDR values, GM_ADDR defaultValue, GM_ADDR y);
    __aicore__ inline void Process();

private:
    __aicore__ inline void SetDefaultValue();
    static __simt_vf__ __aicore__ inline void SparseToDenseSimtCompute(COMP_T numDims, COMP_T sparseBlockOffset,
                                                                       COMP_T sparseDataNum, __gm__ IDX_T* indices,
                                                                       __gm__ IDX_T* outputShape, __gm__ Y_T* values,
                                                                       __gm__ Y_T* y);
...
template <typename IDX_T, typename Y_T, typename COMP_T, bool isScalar>
__aicore__ inline void SparseToDenseSimt<IDX_T, Y_T, COMP_T, isScalar>::Init(GM_ADDR indices, GM_ADDR outputShape,
                                                                             GM_ADDR values, GM_ADDR defaultValue,
                                                                             GM_ADDR y)
{
...
__aicore__ inline void SparseToDenseSimt<IDX_T, Y_T, COMP_T, isScalar>::Process()
{
    ...
    asc_vf_call<SparseToDenseSimt<IDX_T, Y_T, COMP_T, isScalar>::SparseToDenseSimtCompute>(
        dim3(USED_THREAD), numDims, sparseBlockOffset, sparseDataNum, (__gm__ IDX_T*)(indices_.GetPhyAddr()),
        (__gm__ IDX_T*)(outputShape_.GetPhyAddr()), (__gm__ Y_T*)(values_.GetPhyAddr()),
        (__gm__ Y_T*)(y_.GetPhyAddr()));
}
...
LAUNCH_BOUND(USED_THREAD) inline void SparseToDenseSimt<IDX_T, Y_T, COMP_T, isScalar>::SparseToDenseSimtCompute(
    COMP_T numDims, COMP_T sparseBlockOffset, COMP_T sparseDataNum, __gm__ IDX_T* indices, __gm__ IDX_T* outputShape,
    __gm__ Y_T* values, __gm__ Y_T* y)
{
    COMP_T sparseBlockMax = sparseBlockOffset + sparseDataNum;
    for (COMP_T idx = sparseBlockOffset + threadIdx.x; idx < sparseBlockMax; idx += blockDim.x) {

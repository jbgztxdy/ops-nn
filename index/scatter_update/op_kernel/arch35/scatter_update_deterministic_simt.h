...
class ScatterUpdateDeterministicSimt
...
template <typename T, typename U, typename MASK_T, bool splitCol, typename CAST_T, uint32_t castType>
__aicore__ inline void ScatterUpdateDeterministicSimt<T, U, MASK_T, splitCol, CAST_T, castType>::Init(GM_ADDR var,
                                                                                                      GM_ADDR indices,
                                                                                                      GM_ADDR updates,
                                                                                                      GM_ADDR workspace)
{
...
__aicore__ inline void ScatterUpdateDeterministicSimt<T, U, MASK_T, splitCol, CAST_T, castType>::ProcessSplitRow()
{
    ...
    uint32_t totalCol = static_cast<uint32_t>(
        this->tilingData_.varShape[1]); // 尾轴较大时走到切分列的分支，此处只处理尾轴较小场景。所以使用uint32即可
    uint64_t varStride = this->tilingData_.varStride;
    uint64_t indicesBlockCount = this->blockIdx_ == this->tilingData_.usedCoreNum - 1 ?
                                     this->tilingData_.tailBlockIndices :
    ...
    asc_vf_call<ScatterUpdateSimtCompute<T, U, MASK_T>>(
        dim3(THREAD_NUM_DETERMINISTIC), varFirstDimSize, updatesBlockCount, indicesBlockOffset, updatesBlockOffset,
        varStride, totalCol, magic, shift, (__gm__ MASK_T*)(this->workspaceMask_.GetPhyAddr()),
        (__gm__ U*)(this->indicesGm_.GetPhyAddr()), (__gm__ T*)(this->varGm_.GetPhyAddr()),
        (__gm__ T*)(this->updatesGm_.GetPhyAddr()));
}

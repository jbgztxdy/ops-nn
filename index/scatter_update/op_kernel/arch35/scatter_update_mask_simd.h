...
private:
    ...
    __aicore__ inline void ProcessSplitColForBatchIndices(int64_t start, int64_t end, LocalTensor<U>& indicesLocal,
...
template <typename T, typename U>
__aicore__ inline void ScatterUpdateMaskSIMDImpl<T, U>::Init(GM_ADDR var, GM_ADDR indices, GM_ADDR updates,
                                                             GM_ADDR workspace)
{
    ...
    if (this->tilingData_.normNeedSplitRow == 1) {
        this->pipe_.InitBuffer(this->indicesInQueue_, 1,
                               ops::CeilAlign(this->tilingData_.indicesBatchCopySizeAlign,
                                              static_cast<uint64_t>(platform::GetUbBlockSize())));
    } else {
        this->pipe_.InitBuffer(this->indicesInQueue_, DB_BUFFER,
                               ops::CeilAlign(this->tilingData_.processRowPerUb * sizeof(U),
                                              static_cast<uint64_t>(platform::GetUbBlockSize())));
    }
...
template <typename T, typename U>
__aicore__ inline void ScatterUpdateMaskSIMDImpl<T, U>::ProcessNotSplitUpdates(uint32_t rowLen, uint32_t alignedLength,
...
template <typename T, typename U>
__aicore__ inline void ScatterUpdateMaskSIMDImpl<T, U>::ProcessSplitColForBatchIndices(int64_t start, int64_t end,
                                                                                       ...
                                                                                       LocalTensor<uint8_t>& maskTensor)
{
    ...
    for (int64_t i = start; i < end; i++) {
        ...
        for (int64_t j = 0; j < this->tilingData_.colLoopByUb - 1; j++) {
            this->CopyInUpdates(updatesOffset + j * this->tilingData_.processColPerUb, INDICES_ONE,
                                this->tilingData_.processColPerUb);
        ...
        this->CopyInUpdates(updatesOffset + (this->tilingData_.colLoopByUb - 1) * this->tilingData_.processColPerUb,
                            INDICES_ONE, this->tilingData_.processColTail);
        ProcessSplitUpdates(indicesValue, this->tilingData_.processColTail,
...
template <typename T, typename U>
__aicore__ inline void ScatterUpdateMaskSIMDImpl<T, U>::ProcessSplitUpdates(U indicesValue, uint32_t outLen,

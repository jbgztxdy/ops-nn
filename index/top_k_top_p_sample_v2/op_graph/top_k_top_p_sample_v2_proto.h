...
namespace ge {
/**
 * @brief Perform topk-topp-sample sampling calculations based on input word frequency logits, top-k/top-p/min-p
 ...
 * @li logits: A Required tensor in [batch, voc_size] on default. Acts as the input word frequencies to be sampled. The
 ...
 * @li top_k: A Required tensor in [batch, ] on default. Acts as the sampled k-value for each batch. The supported types
 * are: int32, Supports non-contiguous tensors, and must be a non-null pointer.
 * @li top_p: A Required tensor in [batch, ] on default. Acts as the p-value sampled for each batch. The supported types
 * are: float16, bfloat16, float32, Supports non-contiguous tensors, and must be a non-null pointer.
 * @li q: An optional tensor in [batch, voc_size], Defaults to "None". Acts as the sampled q-value for each batch. The
 * supported types are: float32, Supports non-contiguous tensors.
 * @li min_ps: An optional tensor in [batch, ], Defaults to "None". Acts as the minp-value sampled for each batch. The
 * supported types are: float16, bfloat16, float32, Supports non-contiguous tensors.\n
 * @par Outputs:
 * @li logits_select_idx: A 2D tensor which is the sequence of numbers, format supports ND, and must be a non-null
 * pointer. The supported types are: int64. \n
 * @li logits_top_kp_select: An optional 2D tensor. Acts as the logits remaining in the logits file after undergoing the
 ...
 * float32.
 * @li logits_idx: An optional 2D tensor, When `is_need_sample_result=False`, it is disabled and outputs a null pointer;
 ...
 * @li logits_sort_masked: An optional 2D tensor. Acts as the intermediate result of the logits remaining after the
 ...
 *
 * @par Attributes:
 * @li eps: An optional attribute of type float. Prevent division by zero in softmax and weight sampling; default value
 * 1e-8.
 * @li is_need_logits: An optional attribute of type bool. Control the output condition of logits_top_kp_select,
 ...
 * @li top_k_guess: An optional attribute of type int32. Indicates the size of candidate logits for each batch when
 ...
 * @li input_is_logits: An optional attribute of type bool. Indicates whether the input logits are not normalized,
 * defaulting to True. When input_is_logits is True, logits will be normalized; when input_is_logits is False, logits
 * will not be normalized.
 * @li is_need_sample_result: An optional attribute of type bool. Indicates whether to output the intermediate results,

...
public:
    ...
    __aicore__ inline void CumsumAndOut(
        TOPKPParams& params, LocalTensor<float>& bufLocal, uint32_t& totalMrgCnt, uint32_t& partTotalMrg, float* sumVal,
        float topp, bool& ifRet, const GlobalTensor<float>& sortValeGM, const GlobalTensor<uint32_t>& sortIndexGM)
    {
        ...
        const GlobalTensor<float>& sortValeGM, const GlobalTensor<uint32_t>& sortIndexGM)
    {
        ...
        while (queNumOri > 1) {
            ...
            if (ifCumsum) {
                CumsumAndOut(params, bufLocal, totalMrgCnt, partTotalMrg, &sumVal, params.topp, ifRet, sortValeGM, sortIndexGM);
                if (!ifRet) {
                    params.toppNum = totalMrgCnt;
                    break;
                }

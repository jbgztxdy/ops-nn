...
protected:
    static void SetUpTestCase() { std::cout << "l2InplaceScatterUpdateTest SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "l2InplaceScatterUpdateTest TearDown" << std::endl; }
};
...
TEST_F(l2InplaceScatterUpdateTest, l2_inplace_scatter_update_test_err_self_null)
{
    ...
    auto ut = OP_API_UT(aclnnInplaceScatterUpdate, INPUT((aclTensor*)nullptr, indicesDesc, updateDesc, axis), OUTPUT());
...
TEST_F(l2InplaceScatterUpdateTest, l2_inplace_scatter_update_test_err_update_null)
{
    ...
    auto ut = OP_API_UT(aclnnInplaceScatterUpdate, INPUT(selfDesc, indicesDesc, (aclTensor*)nullptr, axis), OUTPUT());
...
TEST_F(l2InplaceScatterUpdateTest, l2_inplace_scatter_update_test_err_indices_null)
{
    ...
    auto ut = OP_API_UT(aclnnInplaceScatterUpdate, INPUT(selfDesc, (aclTensor*)nullptr, updateDesc, axis), OUTPUT());
...
// 正常场景：self为4维ACL_FLOAT类型，format为ACL_FORMAT_HWCN,
// non-continuous、axis为-2，tensor值未更新，返回ACLNN_SUCCESS，精度校验通过

...
static bool CheckCalParams(const gert::TilingContext* context, const CalParams& cal_params)
{
    OP_CHECK_IF(cal_params.indices_size == 0, OP_LOGE(context->GetNodeName(), "indices_size = 0 is not support"),
                return false);
    OP_CHECK_IF(cal_params.core_num == 0, OP_LOGE(context->GetNodeName(), "core_num = 0 is not support"), return false);
    OP_CHECK_IF(cal_params.var_size == 0, OP_LOGE(context->GetNodeName(), "var_size = 0 is not support"), return false);
    OP_CHECK_IF(cal_params.var_data_each_block == 0,
                OP_LOGE(context->GetNodeName(), "var_data_each_block = 0 is not support"), return false);
                             ...
                             CalParams& cal_params)
{
    ...
    OP_CHECK_IF(half_ub_size == 0, OP_LOGE(context->GetNodeName(), "half_ub_size = 0 is not support"), return true);
    OP_CHECK_IF(!CheckCalParams(context, cal_params), OP_LOGE(context->GetNodeName(), "calculation parameters valid!"),
                return false);
    OP_CHECK_IF(tiling_data_ptr->indices_last_dim == 0,
                OP_LOGE(context->GetNodeName(), "tiling_data_ptr->indices_last_dim = 0 is not support"), return true);
    OP_CHECK_IF(tiling_data_ptr->indices_last_dim == 0,
                OP_LOGE(context->GetNodeName(), "tiling_data_ptr->indices_last_dim = 0 is not support"), return true);
    int64_t half_ub_indices_num = half_ub_size / cal_params.indices_size;
    OP_CHECK_IF(half_ub_indices_num == 0, OP_LOGE(context->GetNodeName(), "half_ub_indices_num = 0 is not support"),
                return true);
    ...
    tiling_data_ptr->indices_last_num = cal_params.indices_num -
    ...
    if (cal_params.add_data_num < cal_params.var_data_each_block) {
        ...
        tiling_data_ptr->indice_step = ceil(float(tiling_data_ptr->indice_step) / cal_params.var_data_each_block) *
                                       cal_params.var_data_each_block;
    } else {
        tiling_data_ptr->indice_step = ceil(float(cal_params.max_indice) / cal_params.core_num);
        tiling_data_ptr->indice_step = ceil(float(tiling_data_ptr->indice_step) / cal_params.var_data_each_block) *
                                       cal_params.var_data_each_block;
                                             ...
                                             int64_t need_cast)
{
    ...
    OP_CHECK_IF(alloc_indice_ubsize == 0, OP_LOGE(context->GetNodeName(), "alloc_indice_ubsize = 0 is not support"),
                return false);
    OP_CHECK_IF(!CheckCalParams(context, cal_params), OP_LOGE(context->GetNodeName(), "calculation parameters valid!"),
                return false);
    ...
    tiling_data_ptr->indices_each_core_data = (tiling_data_ptr->indices_each_core_data +
                                               tiling_data_ptr->indices_last_dim - 1) /
                                              tiling_data_ptr->indices_last_dim * tiling_data_ptr->indices_last_dim;
                                   ...
                                   CalParams& cal_params, const CalcShapeInfo& calc_shape_info)
{
    ...
    OP_CHECK_IF(!CalRunningParams(context, tiling_data_ptr, cal_params),
                OP_LOGE(context->GetNodeName(), "calculation running parameters failed!"), return false);
                                 ...
                                 CalParams& cal_params, const CalcShapeInfo& calc_shape_info)
{
    ...
    OP_CHECK_IF(!CalScatterNdHighPerfBranchParams(context, tiling_data_ptr, cal_params, 0),
                OP_LOGE(context->GetNodeName(), "calculate scatter nd high performance branch parameters failed!"),
                return false);
                            ...
                            const ScatterCompileInfo* compile_info, const CalcShapeInfo& calc_shape_info)
{
    ...
    cal_params.indices_last_dim = (calc_shape_info.indices_shape.GetDimNum() > 0) ?
                                      calc_shape_info.indices_shape.GetDim(calc_shape_info.indices_shape.GetDimNum() -
                                                                           1) :
                                      0;

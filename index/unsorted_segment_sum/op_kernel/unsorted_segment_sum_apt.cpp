    ...
    GM_ADDR output, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(TEMPLATE_SIMT_TILING_KEY)) {
        ...
        if constexpr (
            is_same<uint32_t, DTYPE_X>::value || is_same<uint64_t, DTYPE_X>::value ||
            is_same<int64_t, DTYPE_X>::value) {
            return;
        } else {
        ...
        if constexpr (
            ...
            UnsortedSegmentSum::USSKernelSimdDynSort<DTYPE_X, DTYPE_SEGMENT_IDS> op(&tilingData, &pipe);
            op.Init(x, segment_ids, output);
            op.Process();
        }
        ...
        KernelUSSDeterministic<float, int32_t> op(&pipe);
        op.Init(x, segment_ids, output, workspace, &tilingData);
        op.Process();
    } else if (TILING_KEY_IS(FP16_INT32_TILING_KEY)) {
        GET_TILING_DATA_WITH_STRUCT(UnsortedSegmentSumDetermTilingData, tilingData, tiling);
        KernelUSSDeterministic<half, int32_t> op(&pipe);
        op.Init(x, segment_ids, output, workspace, &tilingData);
        op.Process();
    } else if (TILING_KEY_IS(BF16_INT32_TILING_KEY)) {
        GET_TILING_DATA_WITH_STRUCT(UnsortedSegmentSumDetermTilingData, tilingData, tiling);
        KernelUSSDeterministic<bfloat16_t, int32_t> op(&pipe);
        op.Init(x, segment_ids, output, workspace, &tilingData);
        op.Process();
    } else if (TILING_KEY_IS(FP32_INT64_TILING_KEY)) {
        GET_TILING_DATA_WITH_STRUCT(UnsortedSegmentSumDetermTilingData, tilingData, tiling);
        KernelUSSDeterministic<float, int64_t> op(&pipe);
        op.Init(x, segment_ids, output, workspace, &tilingData);
        op.Process();
    } else if (TILING_KEY_IS(FP16_INT64_TILING_KEY)) {
        GET_TILING_DATA_WITH_STRUCT(UnsortedSegmentSumDetermTilingData, tilingData, tiling);
        KernelUSSDeterministic<half, int64_t> op(&pipe);
        op.Init(x, segment_ids, output, workspace, &tilingData);
        op.Process();
    } else if (TILING_KEY_IS(BF16_INT64_TILING_KEY)) {
        GET_TILING_DATA_WITH_STRUCT(UnsortedSegmentSumDetermTilingData, tilingData, tiling);
        KernelUSSDeterministic<bfloat16_t, int64_t> op(&pipe);
        op.Init(x, segment_ids, output, workspace, &tilingData);
        op.Process();
        ...
        UnsortedSegmentSum::KernelUnsortedSegmentSortSimt<DTYPE_X, DTYPE_SEGMENT_IDS> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    }
}
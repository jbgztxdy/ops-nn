...
/*!
 * \file uss_simd_split_col.h
 * \brief uss_simd_split_col
 */
#ifndef ASCENDC_USS_SIMD_SPLIT_COL_H_
#define ASCENDC_USS_SIMD_SPLIT_COL_H_
...
private:
    ...
    TPipe* pipe_ = nullptr;
    const UnsortedSegmentSumSimdSplitColTilingData* td_;
};

template <typename X_T, typename IDS_T>
__aicore__ inline void USSKernelSimdSplitCol<X_T, IDS_T>::Init(GM_ADDR x, GM_ADDR segmentIds, GM_ADDR output)
{
...
template <typename X_T, typename IDS_T>
__aicore__ inline void USSKernelSimdSplitCol<X_T, IDS_T>::Process()
{
    ...
    for (uint64_t aLoop = 0; aLoop < aLoopNum; aLoop++) {
        ...
        for (uint64_t sLoop = 0; sLoop < sLoopNum; sLoop++) {
            ...
            for (uint64_t i = 0; i < rows; i++) {
                ...
                Add(yLocal[dstOffset], yLocal[dstOffset], xLocal[i * colsAlign], cols);
            }

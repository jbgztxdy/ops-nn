...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR indices, GM_ADDR y, const GatherNdTilingData* tilingData,
                                TPipe* pipe);
...
template <typename INDICES_T>
__aicore__ inline void GatherNdSimd<INDICES_T>::Init(GM_ADDR x, GM_ADDR indices, GM_ADDR y,
                                                     const GatherNdTilingData* tilingData, TPipe* pipe)
{
    pipe_ = pipe;
...
template <typename INDICES_T>
template <typename T>
__aicore__ inline void GatherNdSimd<INDICES_T>::CopyIn(LocalTensor<T> xLocal, GlobalTensor<T> xGm, int64_t offset,
...
__aicore__ inline void GatherNdSimd<INDICES_T>::SplitColProcess(int64_t colsAlign)
{
    ...
    for (int64_t i = 0; i < currentCoreElements_; i++) {
        ...
        for (int64_t j = 0; j < loopSize; j++) {
            int64_t cols = (j == loopSize - 1) ? (tilingData_->gatherSize - j * tilingData_->maxElement) :
...
__aicore__ inline void GatherNdSimd<INDICES_T>::Process()
{
    ...
    int64_t colsAlign = (tilingData_->gatherSize * tilingData_->dtypeSize + ubBlockSize - 1) / ubBlockSize *
                        ubBlockSize;

...
protected:
    static void SetUpTestCase() { std::cout << "LinearIndexV2Tiling SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "LinearIndexV2Tiling TearDown" << std::endl; }
};
...
TEST_F(LinearIndexV2Tiling, test_linear_index_int64)
{
    std::string op_type("LinearIndexV2");
    ASSERT_NE(gert::OpImplRegistry::GetInstance().GetOpImpl(op_type.c_str()), nullptr);
    auto tiling_func = gert::OpImplRegistry::GetInstance().GetOpImpl(op_type.c_str())->tiling;
...
TEST_F(LinearIndexV2Tiling, test_linear_index_invalid_dtype)
{
    std::string op_type("LinearIndexV2");
    ASSERT_NE(gert::OpImplRegistry::GetInstance().GetOpImpl(op_type.c_str()), nullptr);
    auto tiling_func = gert::OpImplRegistry::GetInstance().GetOpImpl(op_type.c_str())->tiling;
...
TEST_F(LinearIndexV2Tiling, test_linear_index_zero_indices)
{
    std::string op_type("LinearIndexV2");
    ASSERT_NE(gert::OpImplRegistry::GetInstance().GetOpImpl(op_type.c_str()), nullptr);
    auto tiling_func = gert::OpImplRegistry::GetInstance().GetOpImpl(op_type.c_str())->tiling;
    ...
    auto holder = gert::TilingContextFaker()
                      ...
                      .NodeIoNum(2, 1)
                      .IrInstanceNum({0, 1, 1})
                      .InputShapes({&stride_shape, &value_shape})
                      .OutputShapes({&output_shape})
                      .CompileInfo(&compile_info)
                      .PlatformInfo(reinterpret_cast<char*>(&platform_info))
                      .NodeInputTd(0, ge::DT_INT32, ge::FORMAT_ND, ge::FORMAT_ND)

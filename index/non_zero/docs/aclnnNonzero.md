...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnNonzeroGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnNonzero”接口执行计算。
...
- **参数说明：**
  ...
  * self（aclTensor*, 计算输入）：表示待计算的目标张量，Device侧的aclTensor。shape支持0-8维，支持[非连续的Tensor](../../../docs/context/非连续的Tensor.md)，[数据格式](../../../docs/context/数据格式.md)支持ND。
  ...
  * out（aclTensor*, 计算输出）：Device侧的aclTensor，数据类型支持INT64。shape支持2维，支持[非连续的Tensor](../../../docs/context/非连续的Tensor.md)，[数据格式](../../../docs/context/数据格式.md)支持ND。

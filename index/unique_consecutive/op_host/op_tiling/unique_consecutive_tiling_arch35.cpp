...
bool UniqueConsecutiveTilingHelper::GetAttrs()
{
    ...
    OP_CHECK_IF(axis != DEFAULT_AXIS,
                OP_LOGE_FOR_INVALID_VALUE(context_->GetNodeName(), "axis", std::to_string(axis).c_str(),
                                          std::to_string(DEFAULT_AXIS).c_str()),
                return false);
    ...
    OP_CHECK_IF(returnInvIdx, OP_LOGE_FOR_INVALID_VALUE(context_->GetNodeName(), "return_idx", "True", "False"),
                return false);
    OP_CHECK_IF(
        (ge::DataType::DT_INT32 != outIdxDtype_) && (ge::DataType::DT_INT64 != outIdxDtype_),
        OP_LOGE_FOR_INVALID_VALUE(context_->GetNodeName(), "out_idx", ToString(outIdxDtype_).c_str(), "INT32 or INT64"),
        return false);
...
bool UniqueConsecutiveTilingHelper::CheckTensorAndAttr() { return true; }
...
bool UniqueConsecutiveTilingHelper::DoUbTiling()
{
    ...
    this->tempUbSize_ = this->collectingCntBufSize_ + this->offsetCntBufSize_ + this->prevIdxBufSize_ +
                        this->shapeBufSize_;
    if (this->outIdxDtype_ == ge::DataType::DT_INT64) {
        this->byteSize_ = this->dtSizeX_ + this->dtSizeX_ + static_cast<int64_t>(sizeof(int64_t)) +
                          static_cast<int64_t>(sizeof(int64_t));
    } else {
        this->byteSize_ = this->dtSizeX_ + this->dtSizeX_ + static_cast<int64_t>(sizeof(int32_t)) +
                          static_cast<int64_t>(sizeof(int32_t));
    }
    this->tileLength_ = (static_cast<int64_t>(this->ubSize_) - this->tempUbSize_) / this->byteSize_;
    this->valueQueueSize_ = (this->tileLength_ * this->dtSizeX_) / static_cast<int64_t>(this->blockSize_) *
                            static_cast<int64_t>(this->blockSize_);
    ...
    if (outIdxDtype_ == ge::DataType::DT_INT64) {
        this->countQueueSize_ = (this->tileLength_ * static_cast<int64_t>(sizeof(int64_t))) /
                                static_cast<int64_t>(this->blockSize_) * static_cast<int64_t>(this->blockSize_);
...
void UniqueConsecutiveTilingHelper::SetTilingDataAndTilingKeyAndWorkSpace(UniqueConsecutiveTilingData* tiling)
{
    ...
    currentWorkspace[0] = (this->useCoreNums_ == 1) ? this->sysWorkspaceSize_ + 0 :
                                                      this->sysWorkspaceSize_ + this->coreWorkSpace_;
...
{
    OP_CHECK_IF(nullptr == context, OP_LOGE("Tiling4UniqueConsecutive", "TilingContext is NULL, failed"),

...
template <typename T, bool T0>
__aicore__ inline void ExpandIntoJaggedPermute<T, T0>::Init(
    GM_ADDR permute, GM_ADDR input_offsets, GM_ADDR output_offsets, GM_ADDR output_permute,
    const ExpandIntoJaggedPermuteTilingData* __restrict tilingData)
{
    ...
    coreLen = (blockIdx == this->realCoreNum - 1) ? (coreTaskNum - 1) * oneTaskLen + lastTaskLen :
                                                    coreTaskNum * oneTaskLen;
...
template <typename T, bool T0>
__aicore__ inline void ExpandIntoJaggedPermute<T, T0>::CopyOut(const int64_t outoffsetStart, const int64_t nowlen)
{
...
__aicore__ inline void ExpandIntoJaggedPermute<T, T0>::Process()
{
    ...
    for (int32_t tokensId = 0; tokensId < this->coreTaskNum; tokensId++) {
        int64_t copyOffsetLen = (inputLen - nowoutputOffsets) < oneTaskOffsetLen ? (inputLen - nowoutputOffsets) :
                                                                                   oneTaskOffsetLen;

...
int main()
{
    ...
    ret = CreateAclTensor(permuteHostData, permuteShape, &permuteDeviceAddr, aclDataType::ACL_INT32, &permute);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(inputOffsetsHostData, inputOffsetsShape, &inputOffsetsDeviceAddr, aclDataType::ACL_INT32,
                          &inputOffsets);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(outputOffsetsHostData, outputOffsetsShape, &outputOffsetsDeviceAddr, aclDataType::ACL_INT32,
                          &outputOffsets);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    ret = CreateAclTensor(outputPermuteHostData, outputPermuteShape, &outputPermuteDeviceAddr, aclDataType::ACL_INT32,
                          &outputPermute);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnExpandIntoJaggedPermuteGetWorkspaceSize(permute, inputOffsets, outputOffsets, outputSize, outputPermute,
                                                       &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnExpandIntoJaggedPermuteGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnExpandIntoJaggedPermute failed. ERROR: %d\n", ret); return ret);

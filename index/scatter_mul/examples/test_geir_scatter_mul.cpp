...
static string GetTime()
{
    ...
    char tmp[64]; strftime(tmp, sizeof(tmp), "%Y-%m-%d %H:%M:%S,000", localtime(&timep));
    return tmp;
}
...
static int32_t GenInt32Idx(vector<int64_t> shape, Tensor& t, TensorDesc& desc, int32_t mod)
{
    ...
    for (size_t i = 0; i < n; ++i) p[i] = (int32_t)(i % mod);   // valid indices in [0, mod)
...
int CreateOppInGraph(vector<Tensor>& input, vector<Operator>& inputs, vector<Operator>& outputs, Graph& graph)
{
    ...
    Tensor tVar; GenFloatOnes(varShape, tVar, descVar, 1.0f);
    dVar.update_input_desc_x(descVar); dVar.update_output_desc_y(descVar);
    input.push_back(tVar); graph.AddOp(dVar); inputs.push_back(dVar);
    ...
    Tensor tIdx; GenInt32Idx(idxShape, tIdx, descIdx, 16);
    dIdx.update_input_desc_x(descIdx); dIdx.update_output_desc_y(descIdx);
    input.push_back(tIdx); graph.AddOp(dIdx); inputs.push_back(dIdx);
    ...
    Tensor tUpd; GenFloatOnes(updShape, tUpd, descUpd, 2.0f);
    dUpd.update_input_desc_x(descUpd); dUpd.update_output_desc_y(descUpd);
    input.push_back(tUpd); graph.AddOp(dUpd); inputs.push_back(dUpd);
...
int main(int argc, char* argv[])
{
    ...
    if (CreateOppInGraph(input, inputs, outputs, graph) != SUCCESS) { printf("create graph failed\n"); return FAILED; }
    ...
    if (session->RunGraph(graph_id, input, output) != SUCCESS) {
        ...
        delete session; GEFinalize(); return FAILED;
    }

...
public:
        ...
        if (corePNums > 0) {
            ...
            for (uint64_t i = 0; i <= pBlocks; i++) {
                ...
                uint64_t gmStartInt8 = (corePStart * this->N * this->Q + i * this->ubPBlockLength * this->N * this->Q) *
                                       sizeof(T);
                uint64_t ubStartInt8 = this->AlignedToTarget(this->ubPBlockLength * this->N * this->Q,
                                                             INT8_ALIGNED_NUM);
                ...
                Select(this->xLocal, this->compareMaskLocal, this->xLocal, this->value,
                       SELMODE::VSEL_TENSOR_SCALAR_MODE, pNums * this->N * this->Q);
                this->PIPE_V_S();
                // 搬出xLocal
    ...
    __aicore__ inline void InitSelect()
    {
        ...
        uint64_t repeatMarkerLocalStart = this->AlignedToTarget(this->ubPBlockLength * this->N * this->Q,
                                                                COMPARE_ALIGNED);
        ...
        uint64_t markerLocalStart = repeatMarkerLocalStart +
                                    this->AlignedToTarget(this->ubPBlockLength * this->N * this->Q * sizeof(half),
                                                          INT8_ALIGNED_NUM);
        ...
        DataCopyExtParams copyParamsPNQ{
            1, static_cast<uint32_t>(this->ubPBlockLength * this->N * this->Q * sizeof(half)), 0, 0, 0};

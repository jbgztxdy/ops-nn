...
public:
    ...
    __aicore__ inline void UnSortedProcess(uint32_t startIndex, uint32_t indexNums, uint32_t startPosIdx)
    {
        if (indexNums) {
            this->CopyInIndices(startIndex, indexNums);
            this->PIPE_MTE2_S();
            uint32_t notSortIndexNumsAligned = ((indexNums + ASCENDC_SORT_NUMS - 1) / ASCENDC_SORT_NUMS) *
                                               ASCENDC_SORT_NUMS;
    ...
    __aicore__ inline void BatchProcess(uint32_t updatesUbStart, uint32_t leftUbSize, uint32_t indexNums,
                                        uint32_t indexNumsAligned, uint32_t batchSize)
    {
    ...
    __aicore__ inline void LargeTailProcess(uint32_t updatesUbStart, uint32_t leftUbSize, uint32_t indexNums,
                                            uint32_t indexNumsAligned)
    {
        ...
        for (uint32_t i = 0; i < indexNums; i++) {
            ...
            for (uint32_t j = 0; j < mteTimes; j++) {
                ...
                DataCopyPad(updatesLocal, this->updatesGm[posIdxValue * this->N + j * leftUbSizeAligned], copyParams,
                            padParams);
                this->PIPE_MTE2_S();
                SetAtomicAdd<VT>();
            ...
            if (mteLeft > 0) {
                ...
                DataCopyPadExtParams<VT> padParams{true, 0, 0, 0};
                DataCopyPad(updatesLocal, this->updatesGm[posIdxValue * this->N + mteTimes * leftUbSizeAligned],
    ...
    __aicore__ inline void SameIndexAdd(uint32_t start, uint32_t end, uint32_t dst)
    {
        if (end > start) {
            ...
            while (true) {
                if (end - start == 1) {
                    PipeBarrier<PIPE_V>();
                    Add(updatesLocal[start * this->NAligned], updatesLocal[start * this->NAligned],
                        updatesLocal[end * this->NAligned], this->NAligned);
                    break;
                } else {
                    uint32_t mid = start + (end - start) / DOUBLE + 1;
                    PipeBarrier<PIPE_V>();
                    Add(updatesLocal[start * this->NAligned], updatesLocal[start * this->NAligned],
                        updatesLocal[mid * this->NAligned], (end - mid + 1) * this->NAligned);
                    end = mid - 1;
                }
            }
        }
    ...
    __aicore__ inline void CopyInUpdates(uint32_t indexNums, uint32_t startIndex, uint32_t mte2Nums, uint32_t dst)
    {
        ...
        for (uint32_t i = 0; i < mte2Nums; i++) {
            ...
            DataCopyPad(updatesLocal[i * this->NAligned], this->updatesGm[posIdxValue * this->N], copyParams,
                        padParams);
        }
    }

    __aicore__ inline void GetMaxSortNums() { this->maxSortNums = BIG_TAIL_OP_MAX_SORT_NUMS; }
};
} // namespace InplaceScatterAddNS
#endif
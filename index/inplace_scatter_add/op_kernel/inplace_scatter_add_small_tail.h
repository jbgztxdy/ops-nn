...
public:
    ...
    __aicore__ inline void ComputeAndOutUpdates(uint32_t indexNums, uint32_t mte3Nums, uint32_t dst)
    {
        ...
        for (uint32_t i = 1; i < mte3Nums; i++) {
            ...
            if (preIndexValue == iIndexValue) {
                Add(updatesLocal[prePosIdxValue * this->NAligned], updatesLocal[prePosIdxValue * this->NAligned],
                    updatesLocal[iPosIdxValue * this->NAligned], this->NAligned);
                this->PIPE_V_S(); // this is high performance
            } else {
                DataCopyPad(this->varGm[preIndexValue * this->N], updatesLocal[prePosIdxValue * this->NAligned],
                            copyParams);
                pre = i;
        ...
        if (lastIndexValue == preIndexValue) {
            ...
            DataCopyPad(this->varGm[lastIndexValue * this->N], updatesLocal[lastPosIdxValue * this->NAligned],
                        copyParams);
        }
        SetAtomicNone();
    }
    ...
    __aicore__ inline void CopyInUpdates(uint32_t gmStartIndex, uint32_t mte2Nums, uint32_t dst)
    {
        if (this->N == this->NAligned) {
            DataCopyExtParams copyParams{static_cast<uint16_t>(1),
                                         static_cast<uint32_t>(mte2Nums * this->N * sizeof(VT)), 0, 0, 0};
            ...
            DataCopyExtParams copyParams{static_cast<uint16_t>(mte2Nums), static_cast<uint32_t>(this->N * sizeof(VT)),
                                         0, 0, 0};
            DataCopyPadExtParams<VT> padParams{true, 0, 0, 0};
            auto updatesLocal = this->allUbLocal[dst];
            DataCopyPad(updatesLocal, this->updatesGm[gmStartIndex * this->N], copyParams, padParams);
    ...
    __aicore__ inline void GetMaxSortNums()
    {
        ...
        this->maxSortNums = maxSortNumsTemp > SMALL_TAIL_OP_MAX_SORT_NUMS ? SMALL_TAIL_OP_MAX_SORT_NUMS :
                                                                            maxSortNumsTemp;
    }
};
} // namespace InplaceScatterAddNS

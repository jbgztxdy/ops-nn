...
public:
    ...
    __aicore__ inline void Init(GM_ADDR var, GM_ADDR indices, GM_ADDR updates,
                                const InplaceScatterAddTilingData* tilingData, TPipe* tPipe)
    {
        ...
        if (GetBlockIdx() < this->frontCoreNum) {
            ...
            this->coreStartIndex = this->frontCoreNum * this->frontCoreIndicesNum +
                                   (GetBlockIdx() - this->frontCoreNum) * this->tailCoreIndicesNum;
        }
    ...
    __aicore__ inline void SortIndices(uint32_t startIndex, uint32_t sortIndexNums)
    {
        ...
        Sort<float, true>(sortDstLocal, sortScoreLocal, sortIndexLocal, sortTempLocal,
                          static_cast<int32_t>(repeatTimes));
        PIPE_V_S();

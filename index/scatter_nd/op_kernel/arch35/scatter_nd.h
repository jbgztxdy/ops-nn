...
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_LAUNCH_BOUND) inline void SimtCompute(
    const TYPE_T loopNum, __ubuf__ INDICES_T* idxLocalAddr, __ubuf__ PARAMS_T* xLocalAddr,
    __gm__ PARAMS_T* outputGmAddr, const __ubuf__ TYPE_T* strideListAddr, const __ubuf__ TYPE_T* outputShapeAddr,
    ...
    const TYPE_T indiceOffSet, const TYPE_T magic, const TYPE_T shift)
{
    for (uint32_t index = threadIdx.x; index < currUbTilingSize; index += blockDim.x) {
        ...
        for (TYPE_T dim = 0; dim < rankSize; ++dim) {
            ...
            outOfBound |= (static_cast<TYPE_T>(indiceVal) < 0 ||
...
template <typename INDICES_T, typename PARAMS_T, typename TYPE_T>
__aicore__ inline void ScatterNd<INDICES_T, PARAMS_T, TYPE_T>::Init(GM_ADDR indices, GM_ADDR x, GM_ADDR shape,
                                                                    GM_ADDR y,
    ...
    if (blockIdx < blockNum) {
        ...
        for (auto idx = 0; idx < this->ubLoopCnt; idx++) {
            ...
            asc_vf_call<SimtCompute<INDICES_T, PARAMS_T, TYPE_T>>(
                ...
                (__ubuf__ TYPE_T*)strideList.GetPhyAddr(), (__ubuf__ TYPE_T*)outputShape.GetPhyAddr(), currUbTilingSize,
                xOffSet, sliceSize, rankSize, indiceOffSet, magic, shift);

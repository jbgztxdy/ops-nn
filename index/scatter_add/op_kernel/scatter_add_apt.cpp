...
__aicore__ inline void ScatterAddUnsortSimtAddr32Scalar(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
...
__aicore__ inline void ScatterAddUnsortSimtAddr32Tensor(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
                                                        GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    GET_TILING_DATA(tilingData, tiling);
...
__aicore__ inline void ScatterAddUnsortSimtAddr64Scalar(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
                                                        GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    GET_TILING_DATA(tilingData, tiling);
    if constexpr (is_same<uint8_t, DTYPE_VAR>::value) {
...
__aicore__ inline void ScatterAddUnsortSimtAddr64Tensor(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
                                                        GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    GET_TILING_DATA(tilingData, tiling);
    if (tilingData.isDeterminTemplate) {
...
__aicore__ inline void ScatterAddSortSimtAddr32Scalar(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
                                                      GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    GET_TILING_DATA(tilingData, tiling);
    if constexpr (is_same<int8_t, DTYPE_VAR>::value || is_same<uint8_t, DTYPE_VAR>::value) {
        ...
        if (tilingData.indicesCastMode == CAST_0) {
            ScatterAddSimtSort<DTYPE_INDICES, DTYPE_VAR, DTYPE_INDICES, uint32_t, true, CAST_0, ADD> op(tilingData,
                                                                                                        pipe);
            op.Init(var, indices, updates, userWs);
            op.Process();
...
__aicore__ inline void ScatterAddSortSimtAddr32Tensor(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
                                                      GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    GET_TILING_DATA(tilingData, tiling);
    if constexpr (is_same<int8_t, DTYPE_VAR>::value || is_same<uint8_t, DTYPE_VAR>::value) {
        ...
        if (tilingData.indicesCastMode == CAST_0) {
            ScatterAddSimtSort<DTYPE_INDICES, DTYPE_VAR, DTYPE_INDICES, uint32_t, false, CAST_0, ADD> op(tilingData,
                                                                                                         pipe);
            op.Init(var, indices, updates, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_1) {
...
__aicore__ inline void ScatterAddSortSimtAddr64Scalar(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
                                                      GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    GET_TILING_DATA(tilingData, tiling);
    if constexpr (is_same<int8_t, DTYPE_VAR>::value || is_same<uint8_t, DTYPE_VAR>::value) {
        ...
        if (tilingData.indicesCastMode == CAST_0) {
            ScatterAddSimtSort<DTYPE_INDICES, DTYPE_VAR, DTYPE_INDICES, uint64_t, true, CAST_0, ADD> op(tilingData,
                                                                                                        pipe);
            op.Init(var, indices, updates, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_1) {
...
__aicore__ inline void ScatterAddSortSimtAddr64Tensor(GM_ADDR var, GM_ADDR indices, GM_ADDR updates, GM_ADDR y,
                                                      GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    GET_TILING_DATA(tilingData, tiling);
    if constexpr (is_same<int8_t, DTYPE_VAR>::value || is_same<uint8_t, DTYPE_VAR>::value) {
        ...
        if (tilingData.indicesCastMode == CAST_0) {
            ScatterAddSimtSort<DTYPE_INDICES, DTYPE_VAR, DTYPE_INDICES, uint64_t, false, CAST_0, ADD> op(tilingData,
                                                                                                         pipe);
            op.Init(var, indices, updates, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_1) {
                                                ...
                                                GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    ...
    if constexpr (platform::IsSupportAtomicAddTypeSIMD<DTYPE_VAR>()) {
        if (tilingData.indicesCastMode == CAST_0) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, DTYPE_INDICES, true, CAST_0, ADD> op(
                tilingData, pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_1) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, int16_t, true, CAST_1, ADD> op(tilingData,
                                                                                                        pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_2) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, int32_t, true, CAST_2, ADD> op(tilingData,
                                                                                                        pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_3) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, int16_t, true, CAST_3, ADD> op(tilingData,
                                                                                                        pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_4) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, uint8_t, true, CAST_4, ADD> op(tilingData,
                                                                                                        pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_5) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, uint8_t, true, CAST_5, ADD> op(tilingData,
                                                                                                        pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        }
                                                ...
                                                GM_ADDR userWs, GM_ADDR tiling, TPipe& pipe)
{
    ...
    if constexpr (platform::IsSupportAtomicAddTypeSIMD<DTYPE_VAR>()) {
        if (tilingData.indicesCastMode == CAST_0) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, DTYPE_INDICES, false, CAST_0, ADD> op(
                tilingData, pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_1) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, int16_t, false, CAST_1, ADD> op(tilingData,
                                                                                                         pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_2) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, int32_t, false, CAST_2, ADD> op(tilingData,
                                                                                                         pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_3) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, int16_t, false, CAST_3, ADD> op(tilingData,
                                                                                                         pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_4) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, uint8_t, false, CAST_4, ADD> op(tilingData,
                                                                                                         pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        } else if (tilingData.indicesCastMode == CAST_5) {
            ScatterAddSIMDSortSupportAtomicAdd<DTYPE_VAR, DTYPE_INDICES, uint8_t, false, CAST_5, ADD> op(tilingData,
                                                                                                         pipe);
            op.Init(var, indices, updates, y, userWs);
            op.Process();
        }

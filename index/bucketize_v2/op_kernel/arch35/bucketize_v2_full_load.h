...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR boundaries, GM_ADDR y,
                                const BucketizeV2FullLoadTilingData* tilingData);
...
__aicore__ inline void BucketizeV2FullLoad<X_T, B_T, Y_T, RIGHT>::Init(GM_ADDR x, GM_ADDR boundaries, GM_ADDR y,
                                                                       const BucketizeV2FullLoadTilingData* tilingData)
{
    ...
    int64_t curBlockFactor = (GetBlockIdx() < tilingData_->usedCoreNum - 1) ? tilingData_->blockFactor :
...
template <typename X_T, typename B_T, typename Y_T, bool RIGHT>
__aicore__ inline void BucketizeV2FullLoad<X_T, B_T, Y_T, RIGHT>::CopyInX(int64_t offset, uint32_t copyLen)
{
...
template <typename X_T, typename B_T, typename Y_T, bool RIGHT>
__aicore__ inline void BucketizeV2FullLoad<X_T, B_T, Y_T, RIGHT>::Compute(LocalTensor<B_T>& boundLocal,
                                                                          uint32_t dataLen)
{
    LocalTensor<X_T> xLocal = xQueue_.DeQue<X_T>();
    LocalTensor<Y_T> yLocal = yQueue_.AllocTensor<Y_T>();
    BinaryQuery<X_T, B_T, Y_T, RIGHT>(xLocal, boundLocal, yLocal, dataLen, tilingData_->maxIter,
                                      tilingData_->boundSize);

...
static const std::initializer_list<op::DataType> ASCEND950_AICORE_DTYPE_SUPPORT_LIST = {
    ...
    op::DataType::DT_BF16,  op::DataType::DT_INT8,    op::DataType::DT_UINT8, op::DataType::DT_INT64};
...
static bool CheckDtypeValid(const aclTensor* self, const aclTensor* boundaries, const aclTensor* out,
                            const bool outInt32)
{
    // 获取芯片支持的数据类型
...
static aclnnStatus CheckParams(const aclTensor* self, const aclTensor* boundaries, const bool outInt32,
                               const aclTensor* out)
{
                                           ...
                                           aclOpExecutor** executor)
{
    ...
    if (boundaries->Numel() == 0) {
        ...
        aclIntArray* outputShapeArray = uniqueExecutor.get()->AllocIntArray(outputFillShape.data(),
                                                                            outputFillShape.size());
        CHECK_RET(outputShapeArray != nullptr, ACLNN_ERR_INNER_NULLPTR);
        const aclTensor* outputDims = uniqueExecutor.get()->ConvertToTensor(
            outputFillShape.data(), outputFillShape.size(), op::DataType::DT_INT64);
    ...
    auto bucketizeOut = l0op::BucketizeV2(selfCasted, boundariesCasted, outInt32, right, uniqueExecutor.get());
    CHECK_RET(bucketizeOut != nullptr, ACLNN_ERR_INNER_NULLPTR);
...
aclnnStatus aclnnBucketize(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream)
{
    // 固定写法，调用框架能力，完成计算

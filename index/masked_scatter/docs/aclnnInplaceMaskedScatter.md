...
算子功能：根据掩码(mask)张量中元素为True的位置，复制(source)中的元素到(selfRef)对应的位置上。
...
- **参数说明：**
  ...
  - selfRef（aclTensor\*, 计算输入/输出）：Device侧的aclTensor。[数据格式](../../../docs/zh/context/数据格式.md)支持ND。支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)。
    - <term>Atlas A2 训练系列产品/Atlas 800I A2 推理产品/A200I A2 Box 异构组件</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT、FLOAT16、DOUBLE、INT8、INT16、INT32、INT64、UINT8、BOOL、BFLOAT16。
  - mask（aclTensor\*, 计算输入）：Device侧的aclTensor，数据类型支持BOOL、UINT8。shape不能大于selfRef，且需要和selfRef满足[broadcast关系](../../../docs/zh/context/broadcast关系.md)。[数据格式](../../../docs/zh/context/数据格式.md)支持ND。
  - source（aclTensor\*, 计算输入）: Device侧的aclTensor，[数据格式](../../../docs/zh/context/数据格式.md)支持ND。数据类型需要与selfRef相同，其元素数量需要大于等于mask中元素为true的数量。
    - <term>Atlas A2 训练系列产品/Atlas 800I A2 推理产品/A200I A2 Box 异构组件</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT、FLOAT16、DOUBLE、INT8、INT16、INT32、INT64、UINT8、BOOL、BFLOAT16。
...
- **返回值：**
  ...
  返回161001 (ACLNN_ERR_PARAM_NULLPTR): 1. 传入的selfRef、mask、source是空指针。
  返回161002 (ACLNN_ERR_PARAM_INVALID): 1. selfRef和mask的数据类型不在支持的范围之内。
                                        2. selfRef和mask的shape无法做broadcast。
                                        3. mask的shape维度大于selfRef。
                                        4. source的数据类型和selfRef的数据类型不同。
  ````
...
- **参数说明：**
  ...
  - workspaceSize（uint64_t, 入参）：在Device侧申请的workspace大小，由第一段接口aclnnInplaceMaskedScatterGetWorkspaceSize获取。

...
#include "level2/aclnn_max_unpool3d_backward.h"
...
TEST_F(l2_max_unpool3d_backward_test, case_double) {
  auto gradOutput_desc = TensorDesc({1, 1, 4, 4}, ACL_DOUBLE, ACL_FORMAT_ND);
  auto self_desc = TensorDesc({1, 1, 2, 2}, ACL_DOUBLE, ACL_FORMAT_ND);
  auto indices_desc = TensorDesc({1, 1, 2, 2}, ACL_INT64, ACL_FORMAT_ND).ValueRange(0, 8);
  auto output_szie = IntArrayDesc(vector<int64_t>{1, 4, 4});
  auto stride = IntArrayDesc(vector<int64_t>{1, 2, 3});
  auto padding = IntArrayDesc(vector<int64_t>{1, 2, 3});
  auto out_desc = TensorDesc({1, 1, 2, 2}, ACL_DOUBLE, ACL_FORMAT_ND);

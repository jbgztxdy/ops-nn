...
#include "level2/aclnn_unique2.h"
...
// 测试合法数据类型：Float
TEST_F(l2_unique2_test, case_001_FLOAT)
{
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 6);
}

// 测试合法数据类型：Float16
TEST_F(l2_unique2_test, case_002_FLOAT16)
{
   test_run(ACL_FLOAT16, ACL_FORMAT_NCHW, 4);
}

// 测试合法数据类型：Float64
TEST_F(l2_unique2_test, case_003_DOUBLE)
{
   test_run(ACL_DOUBLE, ACL_FORMAT_ND, 6);
}

// 测试合法数据类型: Int8
TEST_F(l2_unique2_test, case_004_INT8)
{
    test_run(ACL_INT8, ACL_FORMAT_NHWC, 4);
}

// 测试合法数据类型：Int16
TEST_F(l2_unique2_test, case_005_INT16)
{
    test_run(ACL_INT16, ACL_FORMAT_HWCN, 4);
}

// 测试合法数据类型：Int32
TEST_F(l2_unique2_test, case_006_INT32)
{
    test_run(ACL_INT32, ACL_FORMAT_NDHWC, 5);
}

// 测试合法数据类型：Int64
TEST_F(l2_unique2_test, case_007_INT64)
{
    test_run(ACL_INT64, ACL_FORMAT_NCDHW, 5);
}

// 测试合法数据类型：UInt8
TEST_F(l2_unique2_test, case_008_UINT8)
{
    test_run(ACL_UINT8, ACL_FORMAT_NHWC, 4);
}

// 测试合法数据类型：UInt16
TEST_F(l2_unique2_test, case_009_UINT16)
{
    test_run(ACL_UINT16, ACL_FORMAT_NHWC, 4);
}

// 测试合法数据类型：UInt32
TEST_F(l2_unique2_test, case_010_UINT32)
{
    test_run(ACL_UINT32, ACL_FORMAT_ND, 6);
}

// 测试合法数据类型：UInt8
TEST_F(l2_unique2_test, case_011_UINT64)
{
    test_run(ACL_UINT64, ACL_FORMAT_ND, 6);
}

// 测试合法数据类型：Bool
TEST_F(l2_unique2_test, case_012_BOOL)
{
    test_run(ACL_BOOL, ACL_FORMAT_ND, 6);
}

// 测试不合法数据类型：BF16
TEST_F(l2_unique2_test, case_013_BF16)
{
...
// sorted参数
TEST_F(l2_unique2_test, case_022_SORTED)
{
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 6, true, true, true);  // sorted为true
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 6, false, true, true); // sorted为false
}

// returnInverse参数
TEST_F(l2_unique2_test, case_023_RETURN_INVERSE)
{
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 6, false, true, true);  // returnInverse为true
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 6, false, false, true); // returnInverse为false
}

// returnCounts参数
TEST_F(l2_unique2_test, case_024_RETURN_COUNTS)
{
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 6, false, true, true);  // returnCounts为true
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 6, false, true, false); // returnCounts为false
}

TEST_F(l2_unique2_test, ascend910_9589_case_001)
{
    test_run(ACL_FLOAT, ACL_FORMAT_ND, 1, true, true, true);  // sorted为true
}
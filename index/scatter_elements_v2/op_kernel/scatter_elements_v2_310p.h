...
public:
    ...
    __aicore__ inline void CopyOut(GlobalTensor<int8_t> gmTensor, LocalTensor<int8_t> ubTensor, uint64_t mte3Start,
                                   uint64_t mteNum)
    {
        DataCopy(gmTensor[mte3Start], ubTensor, mteNum);
    }

    __aicore__ inline void CopyIn(GlobalTensor<int8_t> gmTensor, LocalTensor<int8_t> ubTensor, uint64_t mte2Start,
                                  uint64_t mteNum)
    {
...
public:
                                              ...
                                              uint64_t validIndicesStart, uint64_t validIndicesEnd, uint64_t varBak = 0)
    {
        ...
        uint64_t mte2Start = rowId * this->indicesN * sizeof(U) + indicesBlocks * INDICES_UB_NUMS * sizeof(U) -
                             alignedBakIndices * sizeof(U);
        ...
        mte2Start = rowId * this->updatesN * sizeof(MT) + indicesBlocks * INDICES_UB_NUMS * sizeof(MT) -
                    alignedBakUpdates * sizeof(MT);
        if (mte2Start < 0) {
...
public:
                                ...
                                const ScatterElementsV2TilingData* tilingData, TPipe* tPipe)
    {
        ...
        if (GetBlockIdx() == this->coreNum - 1) {
            this->coreStartM = (this->frontCoreNum * this->frontCoreData + this->tailCoreNum * this->tailCoreData) *
                               VAR_ROWS;
    ...
    __aicore__ inline void ProcessBigIndices(uint64_t rows, uint64_t startRow)
    {
        ...
        for (uint64_t i = 0; i < rows; i++) {
            for (uint64_t j = 0; j <= indicesBlocks; j++) {
                ...
                uint64_t mte2Start = (startRow + i) * this->indicesN * sizeof(U) + j * TENSOR_UB_LENGTH * sizeof(U) -
                                     alignedBakIndices * sizeof(U);
                if (mte2Start < 0) {
                    alignedBakIndices = alignedBakIndices + mte2Start / sizeof(U);
                    mte2Start = 0;
                ...
                mte2Start = (startRow + i) * this->updatesN * sizeof(MT) + j * TENSOR_UB_LENGTH * sizeof(MT) -
                            alignedBakUpdates * sizeof(MT);
                if (mte2Start < 0) {
                    alignedBakUpdates = alignedBakUpdates + mte2Start / sizeof(MT);
                    mte2Start = 0;

...
public:
        ...
        if (indicesDim <= BLOCK_SIZE) {
            ...
            if constexpr (!IsScalar) {
                if (indicesDim != updatesDim) {
                    ...
                    offsetGmTensor1.SetGlobalBuffer(
                        reinterpret_cast<__gm__ int32_t*>(this->workspace + AGG_INDICES_NUM * sizeof(int32_t)));
                    ...
                    initGatherOffset1.SetCastFloat(this->mode >= SCATTER_MODE_REDUCTION_BEGIN &&
                                                   this->mode <= SCATTER_MODE_REDUCTION_END);
    ...
    __aicore__ inline void ProcessMiddleDimBatchSizeMore()
    {
        ...
        using V = typename std::conditional<std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value ||
        ...
        this->SetTransposeOffsets(transposeVarBatchForward, transposeIndicesBatchForward, transposeUpdatesBatchForward,
                                  transposeDataBatchBackward, maxBatch);
        SyncAll();
        ...
        for (uint32_t i = 0; i <= targetParts; i++) {
            ...
            SyncAll();
            this->ProcessScatter(
                scatterElements, xDstGm, indicesDstGm, updatesDstGm,
                ScatterParams{tileBatchSize * this->xDim1, this->xDim0, tileBatchSize * this->indicesDim1,
                              this->indicesDim0, tileBatchSize * this->updatesDim1, this->updatesDim0});
            SyncAll();
            coreId = 0;
    ...
    __aicore__ inline void ProcessMiddleDimBatchSizeOneSpecial()
    {
        ...
        this->InitTileOffset<V>(transposeVarTileForward, this->xDim0, maxSize, offsetIndex++,
                                OffsetType::TRANSPOSE_FORWARD);
        offsetIndex++;
        if constexpr (!IsScalar) {
            offsetIndex++;
        }
        this->InitTileOffset<V>(transposeDataTileBackward, this->xDim0, maxSize, offsetIndex++,
                                OffsetType::TRANSPOSE_BACKWARD);
        SyncAll();
        for (uint32_t i = 0; i <= targetParts; i++) {
            ...
            // 转置var
            this->ProcessTileTranspose(transposeVarTileForward, this->xGm, xDstGm, workCoreId,
                                       TileTransposeParams{this->xDim0, this->xDim1, i * partSize, tileSize, true});
            // 转置indices
            ...
            this->ProcessScatter(
                scatterElements, xDstGm, indicesDstGm, updatesDstGm,
                ScatterParams{tileSize, this->xDim0, tileSize, this->indicesDim0, tileSize, this->updatesDim0});
            SyncAll();
            // 转置回来
            workCoreId = 0;
            this->ProcessTileTranspose(transposeDataTileBackward, xDstGm, this->xGm, workCoreId,
                                       TileTransposeParams{tileSize, this->xDim0, i * partSize, this->xDim1, false});
            SyncAll();
        }
    }
    ...
    __aicore__ inline void ProcessMiddleDimBatchSizeOne()
    {
        ...
        uint32_t offsetIndex = 0;
        this->InitTileOffset<V>(transposeVarTileForward, this->xDim0, maxSize, offsetIndex++, OffsetType::PAD_UNPAD);
        this->InitTileOffset<int32_t>(transposeIndicesTileForward, this->indicesDim0, maxSize, offsetIndex++,
                                      OffsetType::PAD_UNPAD);
        if constexpr (!IsScalar) {
            this->InitTileOffset<V>(transposeUpdatesTileForward, this->updatesDim0, maxSize, offsetIndex++,
                                    OffsetType::PAD_UNPAD);
        }
        ...
        for (uint32_t i = 0; i <= targetParts; i++) {
            ...
            uint32_t coreId = 0;
            // 转置var
            this->ProcessTileTranspose(transposeVarTileForward, this->xGm, xDstGm, coreId,
                                       TileTransposeParams{this->xDim0, this->xDim1, i * partSize, tileSize, true});
            // 转置indices
            this->ProcessTileTranspose(
                transposeIndicesTileForward, this->indicesGm, indicesDstGm, coreId,
                TileTransposeParams{this->indicesDim0, this->indicesDim1, i * partSize, tileSize, true});

            // 转置updates
            if constexpr (!IsScalar) {
                this->ProcessTileTranspose(
                    transposeUpdatesTileForward, this->updatesGm, updatesDstGm, coreId,
                    TileTransposeParams{this->updatesDim0, this->updatesDim1, i * partSize, tileSize, true});
            } else {
                updatesDstGm = this->updatesGm;
            }
            SyncAll();
            ...
            // 转置回来
            coreId = 0;
            this->ProcessTileTranspose(transposeDataTileBackward, xDstGm, this->xGm, coreId,
                                       TileTransposeParams{tileSize, this->xDim0, i * partSize, this->xDim1, false});
            SyncAll();
        }
    }
    ...
    __aicore__ inline GlobalTensor<int32_t> InitTransposeOffset(uint32_t dim0, uint32_t dim1, uint32_t offsetIndex,
                                                                bool isForward, uint32_t partSize)
    {
        GlobalTensor<int32_t> offsetGmTensor;
        uint32_t dimValue = 0;
                                               ...
                                               uint32_t partSize)
    {
        ...
        offsetGmTensor = this->InitTransposeOffset<int32_t>(this->indicesDim0, this->indicesDim1, offsetIndex++, true,
                                                            partSize);
        if (offsetGmTensor.GetSize() > 0) {
        ...
        if constexpr (!IsScalar) {
            offsetGmTensor = this->InitTransposeOffset<V>(this->updatesDim0, this->updatesDim1, offsetIndex++, true,
                                                          partSize);
            if (offsetGmTensor.GetSize() > 0) {
    ...
    __aicore__ inline void ProcessBatchTranspose(TransposeType& transpose, GlobalTensor<SrcType>& srcGm,
                                                 GlobalTensor<DstType>& dstGm, uint32_t& coreId,
    ...
    __aicore__ inline void CalcPartSplitParams(uint64_t totalSize, uint32_t& targetParts, uint32_t& partSize,
                                               uint32_t& extraSize)
    {
    ...
    __aicore__ inline void CalcBatchSplitParams(uint64_t batchSize, uint32_t& targetParts, uint32_t& partSize,
                                                uint32_t& extraBatch, uint32_t& maxBatch)
    {
    ...
    __aicore__ inline void SetSrcGlobalBuffers(uint32_t tileStart, GlobalTensor<T>& xSrcGm,
                                               GlobalTensor<U>& indicesSrcGm, GlobalTensor<T>& updatesSrcGm)
    {
        xSrcGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(this->x) + tileStart * this->xDim1 * this->xDim0);
        indicesSrcGm.SetGlobalBuffer(reinterpret_cast<__gm__ U*>(this->indices) +
                                     tileStart * this->indicesDim1 * this->indicesDim0);
        updatesSrcGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(this->updates) +
                                     tileStart * this->updatesDim1 * this->updatesDim0);
    }

    __aicore__ inline void SetDstGlobalBuffersForBatch(uint32_t maxBatch, GlobalTensor<T>& xDstGm,
                                                       GlobalTensor<int32_t>& indicesDstGm,
                                                       GlobalTensor<T>& updatesDstGm)
    {
        auto offsetGmStart = 1024 * 2 * sizeof(int32_t);
        xDstGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(this->workspace + offsetGmStart));
        indicesDstGm.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t*>(
            this->workspace + offsetGmStart + maxBatch * this->xDim1 * this->xDim0 * sizeof(T)));
        updatesDstGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ T*>(this->workspace + offsetGmStart +
                                        maxBatch * (this->xDim1 * this->xDim0 * sizeof(T) +
                                                    this->indicesDim1 * this->indicesDim0 * sizeof(int32_t))));
    }

    __aicore__ inline void SetDstGlobalBuffersForTile(uint32_t extraSize, GlobalTensor<T>& xDstGm,
                                                      GlobalTensor<int32_t>& indicesDstGm,
                                                      GlobalTensor<T>& updatesDstGm)
    {
        auto offsetGmStart = AGG_INDICES_NUM * 2 * sizeof(int32_t); // 每个core需要2个offset
        xDstGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(this->workspace + offsetGmStart));
        indicesDstGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ int32_t*>(this->workspace + offsetGmStart + extraSize * this->xDim0 * sizeof(T)));
        updatesDstGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(this->workspace + offsetGmStart +
                                                                 extraSize * this->xDim0 * sizeof(T) +
    ...
    __aicore__ inline void ProcessScatter(ScatterType& scatter, GlobalTensor<XType>& xGm,
                                          GlobalTensor<IndicesType>& indicesGm, GlobalTensor<UpdatesType>& updatesGm,
    ...
    template <typename TransposeType, typename SrcType, typename DstType>
    __aicore__ inline void ProcessTileTranspose(TransposeType& transpose, GlobalTensor<SrcType>& srcGm,
                                                GlobalTensor<DstType>& dstGm, uint32_t& coreId,
    ...
    __aicore__ inline void InitTileOffset(TransposeType& transpose, uint64_t dimValue, uint32_t extraSize,
                                          uint32_t offsetIndex, OffsetType offsetType)
    {
        if (dimValue > BASE_TILE_SIZE)
            return;
        ...
        // offsetIndex 4-7: BLOCK_SIZE * OFFSET_TABLE_SIZE * sizeof(uint32_t) * 4 + OFFSET_TABLE_SIZE *
        // OFFSET_TABLE_SIZE * sizeof(uint32_t) * (offsetIndex - 4)
        if (offsetIndex < 4) {

...
/*!
 * \file aclnn_scatter_reduce.h
 * \brief
 */

#ifndef OP_API_INC_SCATTER_REDUCE_H_
#define OP_API_INC_SCATTER_REDUCE_H_
...
/**
 * @brief aclnnScatterReduce的第一段接口，根据具体的计算流程，计算workspace大小。
 ...
 * 具体操作含义如下：
 * 0：表示替换操作，将src中的对应位置的值按照index写入out中的对应位置
 * 1：表示累加操作，将src中的对应位置的值按照index累加到out中的对应位置
 * 2：表示累乘操作，将src中的对应位置的值按照index累乘到out中的对应位置
 * 3：表示取最大值操作，将src中的对应位置的值按照index与out中的对应位置取最大值
 * 4：表示取最小值操作，将src中的对应位置的值按照index与out中的对应位置取最小值
 * 5：表示取平均值操作，将src中的对应位置的值按照index与out中的对应位置求平均值
 * @param [in] includeSelf: 规约计算时是否将self中的原始值参与计算，数据类型为BOOL。
 * true：参与规约；
 * false：不参与规约。
 * @param [in] out: npu
 * device侧的aclTensor，数据类型支持BFLOAT16、FLOAT16、FLOAT32、DOUBLE、INT64、INT32、INT16、INT8、UINT8、BOOL、COMPLEX64、COMPLEX128,
 ...
 * @param [in] workspaceSize: 在npu device侧申请的workspace大小，由第一段接口aclnnScatterReduceGetWorkspaceSize获取。
 ...
 * (none, 0), (add, 1), (mul, 2), (max, 3), (min, 4), (mean, 5)。
 * 具体操作含义如下：
 * 0：表示替换操作，将src中的对应位置的值按照index写入selfRef中的对应位置
 * 1：表示累加操作，将src中的对应位置的值按照index累加到selfRef中的对应位置
 * 2：表示累乘操作，将src中的对应位置的值按照index累乘到selfRef中的对应位置
 * 3：表示取最大值操作，将src中的对应位置的值按照index与selfRef中的对应位置取最大值
 * 4：表示取最小值操作，将src中的对应位置的值按照index与selfRef中的对应位置取最小值
 * 5：表示取平均值操作，将src中的对应位置的值按照index与selfRef中的对应位置求平均值
 * @param [in] includeSelf: 规约计算时是否将selfRef中的原始值参与计算，数据类型为BOOL。
 * true：参与规约；
 * false：不参与规约。
 * @param [out] workspaceSize: 返回用户需要在npu device侧申请的workspace大小。
 * @param [out] executor: 返回op执行器，包含算子计算流程。
...
#endif // OP_API_INC_SCATTER_REDUCE_H_

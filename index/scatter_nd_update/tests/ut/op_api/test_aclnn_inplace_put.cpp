...
protected:
    static void SetUpTestCase() { std::cout << "inplace_put_test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "inplace_put_test TearDown" << std::endl; }
};
...
TEST_F(l2_inplace_put_test, aclnnInplacePut_input_nullptr)
{
    ...
    auto ut_self_nullptr = OP_API_UT(aclnnInplacePut, INPUT((aclTensor*)nullptr, tensor_desc, tensor_desc, false),
                                     OUTPUT());
    ...
    auto ut_index_nullptr = OP_API_UT(aclnnInplacePut, INPUT(tensor_desc, (aclTensor*)nullptr, tensor_desc, false),
                                      OUTPUT());

    aclRet = ut_index_nullptr.TestGetWorkspaceSize(&workspaceSize);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);

    auto ut_source_nullptr = OP_API_UT(aclnnInplacePut, INPUT(tensor_desc, tensor_desc, (aclTensor*)nullptr, false),
                                       OUTPUT());

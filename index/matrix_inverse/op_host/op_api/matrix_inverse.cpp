...
static const std::initializer_list<DataType> DTYPE_SUPPORT_LIST = {DataType::DT_FLOAT, DataType::DT_DOUBLE,
...
static inline const aclTensor* MatrixInverseAicpu(const aclTensor* self, aclTensor* out, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICPU(MatrixInverse, OP_ATTR_NAMES({"adjoint"}), OP_INPUT(self), OP_OUTPUT(out),
                                          OP_ATTR(false));

    ...
    TPipe &pipe)
{
    ...
    if (cast_mode == CAST_INT32_TO_INT16) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMinValue<X_T>, InitGmMinValue<X_T>, ComputeMax<X_T, uint32_t>, SetAtomicMaxOp<X_T>, CAST_INT32_TO_INT16> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if (cast_mode == CAST_INT64_TO_INT32) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMinValue<X_T>, InitGmMinValue<X_T>, ComputeMax<X_T, uint32_t>, SetAtomicMaxOp<X_T>, CAST_INT64_TO_INT32> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if (cast_mode == CAST_INT64_TO_INT16) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMinValue<X_T>, InitGmMinValue<X_T>, ComputeMax<X_T, uint32_t>, SetAtomicMaxOp<X_T>, CAST_INT64_TO_INT16> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if (cast_mode == CAST_INT32_TO_UINT8) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMinValue<X_T>, InitGmMinValue<X_T>, ComputeMax<X_T, uint32_t>, SetAtomicMaxOp<X_T>, CAST_INT32_TO_UINT8> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if (cast_mode == CAST_INT64_TO_UINT8) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMinValue<X_T>, InitGmMinValue<X_T>, ComputeMax<X_T, uint32_t>, SetAtomicMaxOp<X_T>, CAST_INT64_TO_UINT8> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMinValue<X_T>, InitGmMinValue<X_T>, ComputeMax<X_T, uint32_t>, SetAtomicMaxOp<X_T>, CAST_NONE> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    }
}
...
__aicore__ inline void KernelSimtDispatch(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR tiling, TPipe &pipe)
{
    ...
    UnsortedSegment::KernelUnsortedSegment<X_T, SEGMENT_IDS_T, SimtComputeSegmentMax<X_T>, InitGmMinValue<X_T>> op(&tilingData, &pipe);
    op.Init(x, segment_ids, output);
    op.Process();
}

template <typename X_T, typename SEGMENT_IDS_T>
__aicore__ inline void KernelSplitColDispatch(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR tiling, TPipe &pipe)
{
    ...
    UnsortedSegment::KernelSimdSplitCol<X_T, SEGMENT_IDS_T, GetInitMinValue<X_T>, ComputeMax<X_T, uint64_t>> op(&tilingData, &pipe);
    op.Init(x, segment_ids, output);
    op.Process();
}

template <typename X_T, typename SEGMENT_IDS_T>
__aicore__ inline void KernelNonSortDispatch(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR tiling, TPipe &pipe)
{
    if constexpr (std::is_same<uint32_t, X_T>::value || std::is_same<uint64_t, X_T>::value || std::is_same<int64_t, X_T>::value) {
        return;
    } else {
        ...
        UnsortedSegment::KernelSimdNonSort<X_T, SEGMENT_IDS_T, InitGmMinValue<X_T>, SetAtomicMaxOp<X_T>> op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    }
}
...
__aicore__ inline void KernelOutFlDispatch(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR tiling, TPipe &pipe)
{
    ...
    if constexpr (std::is_same<uint32_t, X_T>::value || std::is_same<uint64_t, X_T>::value || std::is_same<int64_t, X_T>::value) {
        UnsortedSegment::KernelUnsortedSegmentFL<X_T, SEGMENT_IDS_T, SimtGatherMaxValue<X_T>, GetInitMinValue<X_T>, InitGmMinValue<X_T>, ComputeMax<X_T, int32_t>, SetAtomicMaxOp<X_T>> op(&pipe);
...
template <typename X_T, typename SEGMENT_IDS_T>
__aicore__ inline void KernelSortSimtDispatch(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR tiling, TPipe &pipe)
{
    ...
    UnsortedSegment::KernelUnsortedSegmentSortSimt<X_T, SEGMENT_IDS_T, SimtGatherMaxValue<X_T>, SimtComputeSegmentMax<X_T>, GetInitMinValue<X_T>, InitGmMinValue<X_T>> op(&tilingData, &pipe);
    op.Init(x, segment_ids, output);
    op.Process();
}
    ...
    GM_ADDR output, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(TEMPLATE_SIMT_TILING_KEY)) {
        ...
        if constexpr (std::is_same<uint32_t, DTYPE_X>::value || std::is_same<uint64_t, DTYPE_X>::value || std::is_same<int64_t, DTYPE_X>::value) {
            return;
        } else {

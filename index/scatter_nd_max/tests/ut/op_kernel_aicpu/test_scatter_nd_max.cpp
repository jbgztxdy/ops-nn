...
class TEST_SCATTER_ND_MAX_UT : public testing::Test {};

auto CreateScatterNdMaxNodeDef(const vector<vector<int64_t>>& shapes, const vector<DataType>& data_types,
                               const vector<void*>& datas) -> decltype(CpuKernelUtils::CreateNodeDef())
{
    auto node_def = CpuKernelUtils::CreateNodeDef();
    NodeDefBuilder(node_def.get(), "ScatterNdMax", "ScatterNdMax")
...
void RunScatterNdMaxKernel(const vector<vector<int64_t>>& shapes, const vector<DataType>& data_types, const T* input_x,
                           const Index* input_indices, const T* input_updates, const T* expect_output)
{
    ...
    auto node_def = CreateScatterNdMaxNodeDef(shapes, data_types, datas);
    RUN_KERNEL(node_def, HOST, KERNEL_STATUS_OK);

    EXPECT_TRUE(CompareResult(output_data.get(), expect.get(), output_size));
}

TEST_F(TEST_SCATTER_ND_MAX_UT, DATA_TYPE_DT_FLOAT_INT32_SUCC)
{
    ...
    RunScatterNdMaxKernel(shapes, data_types, input_x, input_indices, input_updates, expect_output);
}

TEST_F(TEST_SCATTER_ND_MAX_UT, DATA_TYPE_DT_INT32_INT64_SUCC)
{
    vector<DataType> data_types = {DT_INT32, DT_INT64, DT_INT32, DT_INT32};
    vector<vector<int64_t>> shapes = {{2, 3}, {2, 1}, {2, 3}, {2, 3}};
    int32_t input_x[6] = {1, 7, 3, 4, 5, 6};
    int64_t input_indices[2] = {0, 1};
    int32_t input_updates[6] = {10, 2, 11, 1, 2, 9};
    int32_t expect_output[6] = {10, 7, 11, 4, 5, 9};
    RunScatterNdMaxKernel(shapes, data_types, input_x, input_indices, input_updates, expect_output);
}

TEST_F(TEST_SCATTER_ND_MAX_UT, FAILED_INDICES_TYPE)
{
...
TEST_F(TEST_SCATTER_ND_MAX_UT, FAILED_OUT_OF_BOUNDS_INDEX)
{

...
static inline EsCTensorHolder* EsIndexPutV3(EsCTensorHolder* selfRef, EsCTensorHolder* values, EsCTensorHolder* masks,
                                            EsCTensorHolder** indices, int64_t indices_num, bool accumulate,
                                            bool deterministic)
{
    ...
    auto node = ge::es::CompliantNodeBuilder(ge_graph)
                    ...
                    .IrDefAttrs({
                        {"accumulate", ge::es::CompliantNodeBuilder::kEsAttrOptional, "Bool",
                         ge::es::CreateFrom(accumulate)},
                        {"deterministic", ge::es::CompliantNodeBuilder::kEsAttrOptional, "Bool",
                         ge::es::CreateFrom(deterministic)},
                    })
                    .Build();

    ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, selfRef->GetProducer(), selfRef->GetOutIndex(),
                                                             node, kPortSelfRef));
    ES_ASSERT_GRAPH_SUCCESS(
        ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, values->GetProducer(), values->GetOutIndex(), node, kPortValues));
    ES_ASSERT_GRAPH_SUCCESS(
        ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, masks->GetProducer(), masks->GetOutIndex(), node, kPortMasks));

    for (int64_t i = 0; i < indices_num; ++i) {
        ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, indices[i]->GetProducer(),
                                                                 indices[i]->GetOutIndex(), node,
                                                                 static_cast<int32_t>(kPortIndicesBase + i)));
    }

    return builder.GetTensorHolderFromNode(std::move(node), 0);
}

static inline EsCTensorHolder* EsIndexPutWithSortV2(EsCTensorHolder* self, EsCTensorHolder* linear_index,
                                                    EsCTensorHolder* pos_idx, EsCTensorHolder* values,
                                                    const int64_t* indexed_sizes, int64_t indexed_sizes_num,
                                                    bool accumulate)
{
    ...
    auto node = ge::es::CompliantNodeBuilder(ge_graph)
                    ...
                    .IrDefAttrs({
                        {"indexed_sizes", ge::es::CompliantNodeBuilder::kEsAttrRequired, "ListInt",
    ...
    ES_ASSERT_GRAPH_SUCCESS(
        ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, self->GetProducer(), self->GetOutIndex(), node, kSortPortSelf));
    ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, linear_index->GetProducer(),
                                                             linear_index->GetOutIndex(), node, kSortPortLinearIndex));
    ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, pos_idx->GetProducer(), pos_idx->GetOutIndex(),
                                                             node, kSortPortPosIdx));
    ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, values->GetProducer(), values->GetOutIndex(),
                                                             node, kSortPortValues));

    // 4. Return TensorHolder from node output
    return builder.GetTensorHolderFromNode(std::move(node), 0);
}

static inline EsCTensorHolder* EsLinearIndexV2(EsCTensorHolder** indices_list, int64_t indices_list_num,
                                               EsCTensorHolder* stride, EsCTensorHolder* value_size)
{
    ...
    for (int64_t i = 0; i < indices_list_num; ++i) {
        ...
        if (one_indices != nullptr) {
            ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(
                *ge_graph, one_indices->GetProducer(), one_indices->GetOutIndex(), node, static_cast<int32_t>(i)));
        }
    }

    // Connect stride and value_size after indices_list
    ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, stride->GetProducer(), stride->GetOutIndex(),
                                                             node, static_cast<int32_t>(indices_list_num)));
    ES_ASSERT_GRAPH_SUCCESS(ge::es::AddEdgeAndUpdatePeerDesc(*ge_graph, value_size->GetProducer(),
                                                             value_size->GetOutIndex(), node,
                                                             static_cast<int32_t>(indices_list_num + 1)));

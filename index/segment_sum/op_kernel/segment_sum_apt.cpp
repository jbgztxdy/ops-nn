...
template <typename T>
__aicore__ inline void invokeTemplateAllClear(GM_ADDR output, const SegmentSumSimdTilingData* tilingData,
...
__aicore__ inline void invokeTemplateSimdDeterm(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR y, GM_ADDR workspace,
                                                const SegmentSumSimdTilingData* tilingData, AscendC::TPipe& pipeIn)
{
...
extern "C" __global__ __aicore__ void segment_sum(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR workspace,
                                                  GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(TEMPLATE_SIMT_TILING_KEY)) {
        ...
        if constexpr (!(std::is_same_v<DTYPE_X, uint32_t> || std::is_same_v<DTYPE_X, uint64_t> ||
                        std::is_same_v<DTYPE_X, int64_t>)) {

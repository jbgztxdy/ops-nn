...
__aicore__ inline void SegmentSumSimd<T1, T2>::ComputeSumAndCopyOut(LocalTensor<T1>& yLocal, int32_t curLoopOutters, int32_t curLoopInners, int32_t curLoopInnersAlign, int64_t colOffset, T2& curId)
{
    ...
    for (int32_t i = 0; i < curLoopOutters; i++) {
        ...
        if (curId == preId_) {
            ...
            DataCopy(yLocal, xLocal[i * curLoopInnersAlign], curLoopInnersAlign); // 偏移使用 xLocal[i * curLoopInners]也行，DataCopy搬运数量必须向上对齐
            preId_ = curId;
            // Add(yLocal, yLocal, xLocal[i * curLoopInnersAlign], curLoopInners); // 也可以yBuf初始清零然后Add
        } else { // curId != preId_
            ...
            preId_ = curId;
            DataCopy(yLocal, xLocal[i * curLoopInnersAlign], curLoopInnersAlign); // 上对齐32B
        }
    }
...
__aicore__ inline void SegmentSumSimd<T1, T2>::Process()
{
    ...
    // Duplicate<T1>(yLocal, 0, normalLoopInners_);
    ...
    for (int64_t col = 0; col < colUbLoop_; col++) {
        ...
        int64_t colOffset = col * normalLoopInners_;
        for (int64_t row = 0; row < rowUbLoop_; row++) {
            int64_t curLoopOutters = row == rowUbLoop_ - 1 ? tailLoopOutters_ : normalLoopOutters_;
            curLoopInners = col == colUbLoop_ - 1 ? tailLoopInners_ : normalLoopInners_;
            int64_t curLoopInnersAlign = (curLoopInners + blockNumT1_ - 1) / blockNumT1_ * blockNumT1_;

            int64_t xGmOffset = row * normalLoopOutters_ * tilingData_->innerDim + col * normalLoopInners_;
            int64_t segmentIdsGmOffset = row * normalLoopOutters_;

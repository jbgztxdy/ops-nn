...
ge::graphStatus GroupedQuantMaxRegbase::DoTiling()
{
    OP_CHECK_IF((GetPlatform() != ge::GRAPH_SUCCESS),
        OP_LOGE(context_->GetNodeName(), "DoGroupedQuantMaxTiling GetPlatform Failed."), return ge::GRAPH_FAILED);
    OP_CHECK_IF((GetOpParam() != ge::GRAPH_SUCCESS),
        OP_LOGE(context_->GetNodeName(), "DoGroupedQuantMaxTiling GetOpParam Failed."), return ge::GRAPH_FAILED);
...
ge::graphStatus GroupedQuantMaxRegbase::CheckDtype() 
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
            context_->GetNodeName(), "scale", ge::TypeUtils::DataTypeToSerialString(scaleDtype_).c_str(),
    ...
    OP_CHECK_IF(
        groupListDtype_ != ge::DT_INT64,
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
            context_->GetNodeName(), "groupList", ge::TypeUtils::DataTypeToSerialString(groupListDtype_).c_str(),
    ...
    OP_CHECK_IF(
        amaxDtype_ != xDtype_,
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
            context_->GetNodeName(), "amax", Ops::Base::ToString(amaxDtype_).c_str(),
...
ge::graphStatus GroupedQuantMaxRegbase::CheckAttrs()
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            context_->GetNodeName(), "roundMode", roundMode, errorMsg_.c_str()),
        return ge::GRAPH_FAILED);
...
ge::graphStatus GroupedQuantMaxRegbase::CheckShape(
    ...
    const gert::Shape& amaxShape) const
{
    ...
    OP_CHECK_IF(
        groupListDimNum != 1,
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
            context_->GetNodeName(), "groupList", std::to_string(groupListDimNum).c_str(),
            "The shape dim of groupList must be 1D"),
        return ge::GRAPH_FAILED);

    OP_CHECK_IF(
        !IsShapeEqualsNumGroups(scaleShape, groupListShape.GetDim(0)),
    ...
    OP_CHECK_IF(
        !IsShapeEqualsNumGroups(amaxShape, groupListShape.GetDim(0)),
        OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
    ...
    OP_CHECK_IF(
        num_groups < 1 || num_groups > 384,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            context_->GetNodeName(), "groupList.dim0", std::to_string(num_groups).c_str(),
...
ge::graphStatus GroupedQuantMaxRegbase::GetOpParam()
{
    ...
    if (xSizeNum == 0ULL) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            context_->GetNodeName(), "x", "empty", "grouped_quant_max does not support empty tensor");
        return ge::GRAPH_FAILED;
    }
    ...
    OP_CHECK_IF((
        CheckShape(xInputShape, scaleInputShape, groupListInputShape, yOutputShape, amaxOutputShape) != ge::GRAPH_SUCCESS),
...
ge::graphStatus GroupedQuantMaxRegbase::CalcUbFactor()
{
    ...
    int64_t groupListSize = ((num_groups * BYTES_OF_INPUT_GROUP_LIST_TYPE + ALIGN_BYTES - 1) / ALIGN_BYTES) * ALIGN_BYTES;
    int64_t maxSize = ((num_groups * BYTES_OF_TMP_AMAX_TYPE + ALIGN_BYTES - 1) / ALIGN_BYTES) * ALIGN_BYTES;
    int64_t groupMaxSize = ((coreNum_ * num_groups * BYTES_OF_TMP_AMAX_TYPE + ALIGN_BYTES - 1) / ALIGN_BYTES) * ALIGN_BYTES;
...
ge::graphStatus GroupedQuantMaxRegbase::CalcTiling()
{
    ...
    OP_CHECK_IF(
        (CalcUbFactor() != ge::GRAPH_SUCCESS),
        OP_LOGE(context_->GetNodeName(), "CalcUBFactor failed."), return ge::GRAPH_FAILED);
...
ge::graphStatus GroupedQuantMaxRegbase::WriteTilingData()
{
    ...
    OP_LOGD(
        context_->GetNodeName(), "totalCoreNum:%ld, actualCoreNum:%ld, blockTailCoreNum_:%ld, blockFactor:%ld, blockTailFactor:%ld, ubFactor:%ld, dim1:%ld, numGroups:%ld", coreNum_, actualCoreNum_,
        blockTailCoreNum_, blockFactor_, blockTailFactor_, ubFactor_, dim1_, numGroups_);
...
static ge::graphStatus TilingPrepareForGroupedQuantMax(gert::TilingParseContext* context)
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(
            context->GetNodeName(), "GroupedQuantMax GetHardwareInfo Failed, vectorCoreNum:%d, ubSize:%lu.",
...
IMPL_OP_OPTILING(GroupedQuantMax).Tiling(TilingForGroupedQuantMax)

    ...
    uint32_t* blockSize, uint32_t* vectorLength)
{
    ...
    *coreNum = ascendcPlatform.GetCoreNumAiv();
    ...
    OP_CHECK_IF((*ubSize <= RESERVED_UB),
              ...
              "Get ub size failed, ub size: %u", static_cast<uint32_t>(*ubSize)),
              return ge::GRAPH_FAILED);
    *blockSize = Ops::Base::GetUbBlockSize(context);
    OP_CHECK_IF((*blockSize <= 0),
              OP_LOGE(context->GetNodeName(),
              "Get block Size failed, block size: %u", static_cast<uint32_t>(*blockSize)),
              return ge::GRAPH_FAILED);

    *vectorLength = Ops::Base::GetVRegSize(context);
    OP_CHECK_IF((*vectorLength <= 0),
              OP_LOGE(context->GetNodeName(),
              "Get vector Length failed, vector Length: %u", static_cast<uint32_t>(*vectorLength)),
              return ge::GRAPH_FAILED);
...
static void CalcUbCut(DequantBiasArch35TilingData* td, int64_t N, int64_t ubSize, bool hasActivate,
    bool hasBias, ge::DataType biasDtype, int64_t outputDtype, uint32_t blockSize, uint32_t vectorLength)
{
    ...
    if (hasActivate) bytesPerElem += SIZE_FLOAT;
    if (hasBias) {
    ...
    if (baseLen < align_elems) baseLen = align_elems;
    if (baseLen > N) baseLen = N;
...
{
    if (!hasBias) { return TPL_BIAS_FLOAT; }
    if (biasDtype == ge::DT_FLOAT)   { return TPL_BIAS_FLOAT; }
    if (biasDtype == ge::DT_FLOAT16) { return TPL_BIAS_HALF; }
    if (biasDtype == ge::DT_BF16)    { return TPL_BIAS_BFLOAT; }
    if (biasDtype == ge::DT_INT32)   { return TPL_BIAS_INT; }
...
static ge::graphStatus DequantBiasTilingFunc(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(memset_s(td, sizeof(*td), 0, sizeof(*td)) != EOK,
                OP_LOGE(context, "memset tiling failed"), return ge::GRAPH_FAILED);

    CalcBlockSplit(td, M, N, coreNum);
    CalcUbCut(td, N, static_cast<int64_t>(ubSize), hasActivate, hasBias, biasDtype, outputDtype,
              blockSize, vectorLength);
    ...
    ASCENDC_TPL_SEL_PARAM(context,
        static_cast<uint32_t>(hasBias ? TPL_HAS_BIAS : TPL_NO_BIAS),

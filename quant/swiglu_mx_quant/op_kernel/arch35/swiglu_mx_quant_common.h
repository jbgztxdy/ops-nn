...
template <typename T> __aicore__ inline void PadZeroM(__ubuf__ T *swigluOutAddr, uint32_t num)
{
    ...
    __ubuf__ uint16_t *mxScaleReciprocalAddr, uint16_t dtypeYMaxExp)
{
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < blockCount; i++) {
            MicroAPI::DataCopy<T, MicroAPI::PostLiteral::POST_MODE_UPDATE, MicroAPI::LoadDist::DIST_DINTLV_B16>(x0, x1,
                xAddr, 256);
...
__aicore__ inline void ComputeScaleCuBLASNotLastAxis(uint16_t blockCount, __ubuf__ T *xAddr,
    __ubuf__ uint8_t *mxScaleAddr, __ubuf__ uint16_t *mxScaleReciprocalAddr, uint32_t invDtypeMax)
{
    __VEC_SCOPE__
    {
        MicroAPI::RegTensor<T> x0, x1;
        ...
        MicroAPI::Cast<float, T, CAST_ONE>((MicroAPI::RegTensor<float> &)maxFP32_1,
            (MicroAPI::RegTensor<T> &)absMax1Dim2, maskAll);
        ...
        MicroAPI::DataCopy<uint16_t, MicroAPI::StoreDist::DIST_INTLV_B16>(mxScaleReciprocalAddr, absMax0,
            absMax1, maskAll);
    }
}
    ...
    __ubuf__ uint16_t *mxScale2ReciprocalAddr, __ubuf__ uint8_t *y2Addr)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < blockCount; i++) {
            ...
            MicroAPI::DataCopy<uint8_t, MicroAPI::StoreDist::DIST_PACK4_B32>(y2Addr + (i * dataLen / 2),
                (MicroAPI::RegTensor<uint8_t> &)dim0x0FP4, dataMaskB8);
            MicroAPI::DataCopy<uint8_t, MicroAPI::StoreDist::DIST_PACK4_B32>(y2Addr + 64 + (i * dataLen / 2),
                (MicroAPI::RegTensor<uint8_t> &)dim0x1FP4, dataMaskB8);
        }
    }
}
...
__aicore__ inline void ComputeScaleLast(uint16_t fEmax, __ubuf__ uint16_t *maxExpAddr,
    __ubuf__ uint16_t *mxScaleLocalAddr, __ubuf__ uint16_t *halfScaleLocalAddr, int64_t dim0OnceSize,
    int64_t alignDim1Size)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < loopNumScale; i++) {
            ...
            AscendC::MicroAPI::DataCopy<uint16_t, AscendC::MicroAPI::PostLiteral::POST_MODE_UPDATE,
                AscendC::MicroAPI::StoreDist::DIST_PACK_B16>(mxScaleLocalAddr, scaleValue, onceNumMxScale,
                preMaskScale);
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < loopNum; i++) {
            ...
            if constexpr (IsSame<T, half>::value && roundMode == RoundMode::CAST_FLOOR) {
                ...
                MicroAPI::Cast<float, bfloat16_t, CAST_ZERO>(halfScaleForMulFP32,
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < loopNum; i++) {
            ...
            if constexpr (IsSame<T, half>::value) {
                ...
                AscendC::MicroAPI::Cast<float, bfloat16_t, CAST_ZERO>(floatScaleForMul,
                    (AscendC::MicroAPI::RegTensor<bfloat16_t> &)halfScaleForMul, maskAll);
            ...
            AscendC::MicroAPI::DataCopy<int8_t, AscendC::MicroAPI::PostLiteral::POST_MODE_UPDATE,
                AscendC::MicroAPI::StoreDist::DIST_NORM_B8>(outLocalAddr,
                (AscendC::MicroAPI::RegTensor<int8_t> &)vdExp0FP8Zero, onceXNum, maskAllB8);
        }
    }
}

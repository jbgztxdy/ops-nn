...
ge::graphStatus GroupQuantTiling::GetShapeAttrsInfo() {
  ...
  OP_CHECK_IF((xDtype != ge::DT_FLOAT && xDtype != ge::DT_FLOAT16 && xDtype != ge::DT_BF16),
      OP_LOGE(context_->GetNodeName(),
                                      "x datatype only support FLOAT32, FLOAT16 or BFLOAT16, but is %s",
  ...
  OP_CHECK_IF((scaleDtype != ge::DT_FLOAT && scaleDtype != ge::DT_FLOAT16 && scaleDtype != ge::DT_BF16),
      OP_LOGE(context_->GetNodeName(),
                                      "scale datatype only support FLOAT32, FLOAT16 or BFLOAT16, but is %s",
  ...
  OP_CHECK_IF(dimE < 1,
      OP_LOGE(context_->GetNodeName(),
                                      "no support for the first dim of scale shape(%s) is less than 1",
                                      Ops::Base::ToString(scaleShape).c_str()),
      return ge::GRAPH_FAILED);
  ...
  OP_CHECK_IF((grpIdxDtype != ge::DT_INT32 && grpIdxDtype != ge::DT_INT64),
      OP_LOGE(context_->GetNodeName(),
                                      "group_index datatype only support INT32 or INT64, but is %s",
  ...
  if (inputOffset == nullptr) {
    ...
    OP_CHECK_IF(offsetDtype != scaleDtype,
        OP_LOGE(context_->GetNodeName(),
                                        "offset datatype(%s) should be same as scale datatype(%s)",
    ...
    OP_CHECK_IF(offsetShape.GetDimNum() != DIM_NUM_OF_OFFSET || offsetShape.GetDim(DIM_INDEX_0) != 1,
        OP_LOGE(context_->GetNodeName(),
                                        "offset should be scalar or vector with shape is [1, ], but shape is %s",
  ...
  if (yDtype == ge::DT_INT4) {
    OP_CHECK_IF(dimH % EVEN_FACTOR != 0,
        OP_LOGE(context_->GetNodeName(),
                                        "y datatype is int4, the second dim of shape(%s) should be an even number",
...
ge::graphStatus GroupQuantTiling::DoOpTiling() {
  ...
  OP_CHECK_IF(needCoreNum < 1,
      OP_LOGE(context_->GetNodeName(), "need core num should be greater than 0"),
      return ge::GRAPH_FAILED);
...
ge::graphStatus GroupQuantTiling::PostTiling() {
  ...
  OP_CHECK_IF(rawTilingData == nullptr,OP_LOGE(context_->GetNodeType(), "GetRawTilingData failed."), ge::GRAPH_FAILED);
  OP_CHECK_IF(

...
{
    L0_DFX(FakeQuantAffineCachemask, self, scale, zeroPoint, axis, quantMin, quantMax);
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(FakeQuantAffineCachemask, OP_INPUT(self, scale, zeroPoint),
                                           OP_OUTPUT(fakeOut, maskOut), OP_ATTR(axis, quantMin, quantMax));
    if (ret != ACL_SUCCESS) {

...
private:
    ...
    __aicore__ inline void ComputeMulti(int64_t nRow, int64_t segLen,
    ...
    const FakeQuantAffineCachemaskTilingDataArch35* td)
{
    ...
    bool isTail = (blockUnion_ > 1)
                      ? (innerForTail == blockUnion_ - 1)
                      : (blockIdx_ == numCore_ - 1);
...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPH<T, ZpT, HAS_ZP>::CopyInXMulti(
    int64_t gmXOff, int64_t nRow, int64_t segLen)
{
...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPH<T, ZpT, HAS_ZP>::CopyOutYMulti(
    int64_t gmYOff, int64_t nRow, int64_t segLen)
{
...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPH<T, ZpT, HAS_ZP>::CopyOutMaskMulti(
    int64_t gmMOff, int64_t nRow, int64_t segLen)
{
    ...
    if (dstStrideBytes == 0) {
        AscendC::DataCopyExtParams paramsOne{
            1,
            static_cast<uint32_t>(nRow * segLen * sizeof(uint8_t)),
            0, 0, 0};
...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPH<T, ZpT, HAS_ZP>::FillScaleZpNSeg(
    int64_t gmSOff, int64_t segN)
{
    ...
    AscendC::DataCopyExtParams sp{
        1, static_cast<uint32_t>(scaleNum * sizeof(T)), 0, 0, 0};
    ...
    if constexpr (HAS_ZP == 1) {
        ...
        AscendC::DataCopyExtParams zp{
            1, static_cast<uint32_t>(scaleNum * sizeof(ZpT)), 0, 0, 0};
...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPH<T, ZpT, HAS_ZP>::ComputeMulti(
    int64_t nRow, int64_t segLen,
    int64_t scaleBaseOff, int64_t zpBaseOff)
{
            ...
            for (uint16_t i = 0; i < vfLoopNum; ++i) {
                ...
                if constexpr (std::is_same<T, float>::value) {
                    Reg::DataCopy<float, Reg::LoadDist::DIST_NORM>(
                        vregTmp, xPtr + rowOffElems + i * VL_FP32);
                } else {
                    Reg::DataCopy<T, Reg::LoadDist::DIST_UNPACK_B16>(
                        vregX, xPtr + rowOffElems + i * VL_FP32);
                ...
                Reg::DataCopy<float, Reg::StoreDist::DIST_NORM_B32>(
                    m1Ptr + rowOffElems + i * VL_FP32, vregM1, mask);
    ...
    AscendC::Cast<half, float>(hBuf, m1Buf,
        AscendC::RoundMode::CAST_NONE, static_cast<int32_t>(totalElem));
...
__aicore__ inline void FakeQuantAffineCachemaskPH<T, ZpT, HAS_ZP>::Process()
{
    ...
    if (baseLen_ <= 0 || baseN_ <= 0)            return;
    if (blockS_ <= 0 || blockN_ <= 0 || blockLen_ <= 0) return;
    ...
    for (int64_t s = 0; s < blockS_; ++s) {
        ...
        for (int64_t nb = 0; nb < nLoop; ++nb) {
            ...
            if (dim1_ > 0) gmSOff = gmSOff % dim1_;

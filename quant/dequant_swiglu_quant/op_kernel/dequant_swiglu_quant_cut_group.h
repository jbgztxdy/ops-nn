...
public:
    __aicore__ inline DequantSwigluQuantGroup(TPipe* pipe)
        : DequantSwigluQuantOps::DequantSwigluQuantBase<TEMPLATE_DSQ_ARGS>(pipe)
    {
        this->pipe_ = pipe;
    };
...
__aicore__ inline void DequantSwigluQuantGroup<TEMPLATE_DSQ_ARGS>::Process()
{
    ...
    for (int32_t groupIdx = 0; groupIdx < this->tl_->inGroupNum; ++groupIdx) {
        int64_t realGroupIdx = this->tl_->speGroupType == 0 ?
                                   static_cast<int64_t>(groupIdx) :
                                   static_cast<int64_t>(this->groupIndexGm_(groupIdx * GROUPINDEX_STRIDE));
        this->realDimx_ = this->tl_->speGroupType == 0 ?
                              static_cast<int64_t>(this->groupIndexGm_(groupIdx)) :

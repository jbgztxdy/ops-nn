...
const uint32_t MAX_BLOCK_LEN = 65535U * 32U; // datacopy指令每个连续传输数据块的最长长度为65535，单位为32bytes
...
template<typename T>
inline T AlignUp(T num, T rnd) -> T
{
    return (((rnd) == 0) ? 0 : (((num) + (rnd) - 1) / (rnd) * (rnd)));
}
// align num to multiples of rnd, round down
template<typename T>
inline T AlignDown(T num, T rnd) -> T
{
    return ((((rnd) == 0) || ((num) < (rnd))) ? 0 : ((num) / (rnd) * (rnd)));
}
template<typename T>
inline T DivCeil(T num, T div) -> T
{
...
inline bool GetLengthByType(int32_t dtype, uint32_t& dsize) {
    ...
    {
        return Glu_Flag == GLU_FLAG::SWIGLU_GRAD_SINGLE ? 1U : 0U;
    }
    ...
    bool GluSingleTilingCalculator::GetBufferNumAndDataLenPerUB(uint64_t ubSize, int32_t dtype, uint64_t& dataLenPerUB)
    {
        ...
        case GLU_FLAG::SWIGLU_SINGLE:
            if (dtype == ge::DT_FLOAT16) {
                singleDataSize = static_cast<uint32_t>(bufferNum * XXGLU_TQUE_NUM * sizeof(int16_t) + SWIGLU_TBUF_NUM_HALF * sizeof(int32_t));
            } else if (dtype == ge::DT_BF16) {
                singleDataSize = static_cast<uint32_t>(bufferNum * XXGLU_TQUE_NUM * sizeof(int16_t) + SWIGLU_TBUF_NUM_BF16 * sizeof(int32_t));
            } else {
                singleDataSize = static_cast<uint32_t>(bufferNum * XXGLU_TQUE_NUM * sizeof(int32_t) + SWIGLU_TBUF_NUM_FLOAT * sizeof(int32_t));
            }
...
inline bool GluSingleTilingCalculator::CalcUbMaxTileLen(uint64_t ubSize, int32_t dtype, GluSingleTilingOptParam& optTiling)
{
    ...
    OP_LOGI((Glu_Flag == SWIGLU_SINGLE ? "SwiGlu" : "SwiGluGrad"), "CalcTiling ubSize:%lu, bufferNum:%d, maxTileLenPerUB:%u", ubSize, bufferNum, optTiling.maxTileLen);
    return true;
}
...
inline void GluSingleTilingCalculator::SaveOptBaseShape(uint32_t baseRowLen_, uint32_t baseColLen_, GluSingleTilingOptParam& optTiling)
{
    ...
    if ((optTiling.optTotalTileNum == 0UL) ||
...
{
    return ((baseRowlen_ < 1U) || (baseRowlen_ > 1U && MustBeSingleBaseRowLen(baseColLen_)));
}
...
inline bool GluSingleTilingCalculator::CalcOptBaseShape(GluSingleTilingOptParam& optTiling)
{
    ...
    OP_LOGI((Glu_Flag == SWIGLU_SINGLE ? "SwiGlu" : "SwiGluGrad"), "CalcOptBaseShape init baseColLen : %u", baseColLen_);
    if (MustBeSingleBaseRowLen(baseColLen_)) {
        SaveOptBaseShape(1U, baseColLen_, optTiling);
        return true;
    }
    ...
    while(true) {
        ...
        if (isInvalidBaseShape(baseRowlen_, baseColLen_)) {
            OP_LOGI((Glu_Flag == SWIGLU_SINGLE ? "SwiGlu" : "SwiGluGrad"), "CalcOptBaseShape baseRowln:%u or baseColLen:%u is invalid. optTotalTileNum:%lu end",
        ...
        if (baseColLen_ % alignPackLen == 0U) {
...
bool GluSingleTilingCalculator::CalcTiling(uint32_t totalCore, uint64_t ubSize, int32_t dtype,  platform_ascendc::SocVersion socVersion_)
{
    ...
    if (!GetLengthByType(dtype, inputDTypeLen)) {
        OP_LOGI((Glu_Flag == SWIGLU_SINGLE ? "SwiGlu" : "SwiGluGrad"), "CalcTiling Unsupported input data type %d", dtype);
        return false;
    }
    ...
    OP_LOGI((Glu_Flag == SWIGLU_SINGLE ? "SwiGlu" : "SwiGluGrad"), "CalcTiling GetLengthByType:%u ubMinBlockLen:%u cacheLineLen:%u alignPackLen:%u", inputDTypeLen, ubMinBlockLen, cacheLineLen, alignPackLen);
    ...
    if (optTilingDb.tileNumPerCore == 2UL) {
        GluSingleTilingOptParam optTilingNoDb;
        if (CalcOptTiling<Glu_Flag, 1>(ubSize, dtype, optTilingNoDb) &&
            (optTilingNoDb.tileNumPerCore == 1UL) && (optTilingNoDb.totalUsedCoreNum >= optTilingDb.totalUsedCoreNum)) {
    ...
    totalUsedCoreNum = optTiling->totalUsedCoreNum;
    OP_LOGI((Glu_Flag == SWIGLU_SINGLE ? "SwiGlu" : "SwiGluGrad"), "CalcTilingRES baseRowLen:%u baseColLen:%u", optTiling->optBaseRowLen, optTiling->optBaseColLen);
    return true;
}
...
ge::graphStatus processEmptyTensor(gert::TilingContext* context, const uint32_t& totalCore)
{
    SwiGluTilingData tilingData;
    ...
    OP_CHECK_IF(
        yTotalLength != 0,
        OP_LOGE(
            context, "y must be an empty tensor when x is empty, but total num of y is %ld.",
            yTotalLength),
...
ge::graphStatus Tiling4SwiGlu(gert::TilingContext* context) {
    OP_LOGD(context, "Tiling4SwiGlu enter.");
    ...
    if ((curShortSocName_ == platform_ascendc::SocVersion::ASCEND910_95 ||
        curShortSocName_ == platform_ascendc::SocVersion::ASCEND910_55) && GetSelfIdx<Glu_Flag>()) {
        OP_LOGD(context, "Tiling SwiGluGrad RegBase start");
        GluBaseTiling4RegBase tilingObj(context);
        return tilingObj.DoTiling();
    }
    uint64_t ubSize = 0;
    platformInfo.GetCoreMemSize(platform_ascendc::CoreMemType::UB, ubSize);
    OP_CHECK_IF((totalCore < 0 || ubSize <= UB_RESERVED_BUFF),
        OP_LOGE(context, "Compile Info is invalid, coreNum:%u, ubSize:%lu", totalCore, ubSize),
    ...
    OP_LOGE_IF(attrs == nullptr, false, context, "Get op attrs failed.");

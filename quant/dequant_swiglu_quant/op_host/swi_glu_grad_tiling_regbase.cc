...
namespace optiling {
    ...
    bool GluBaseTiling4RegBase::CheckShapeValid(const gert::Shape& gradYShape, const gert::Shape& xShape, const int64_t dim)
    {
        ...
        for (int64_t i = 0; i < dimXNum; i++) {
            ...
            if (i != splitDim) {
                if (xDim != yDim) {
                    OP_LOGE(opName_, "[cur_dim(%ld) split_dim(%ld)]: y_grad number(%ld) is not equal to x(%ld).", i, splitDim, yDim, xDim);
                    return false;
                }
            } else {
                if (xDim != yDim * DOUBLE) {
                    OP_LOGE(opName_, "[cur_dim(%ld) split_dim(%ld)]: y_grad number(%ld) is not equal to x(%ld) / 2.", i, splitDim, yDim, xDim);
                    return false;
                }
            }
        }
    ...
    ge::graphStatus GluBaseTiling4RegBase::DoOpTiling()
    {
        ...
        rowNormalNum_ = ops::FloorDiv(rowTotalNum_, static_cast<int64_t>(rowTileNum_));
        colNormalNum_ = ops::FloorDiv(colTotalNum_, static_cast<int64_t>(colTileNum_));
    ...
    uint64_t GluBaseTiling4RegBase::GetTilingKey() const
    {
        ...
        if (colTotalNum_ * dataSize_ < MOVE_ALIGN_LIMIT_BYTE) {
            moveAlign = 0U;
        }
    ...
    ge::graphStatus GluBaseTiling4RegBase::PostTiling()
    {
        ...
        workspaces[0] = WORK_SPACE_SIZE + usedCoreNum_ * platform::GetUbBlockSize(context_);
        return ge::GRAPH_SUCCESS;
    }
    ...
    void GluBaseTiling4RegBase::AutoTiling()
    {
        ...
        int64_t base = BASE_BLOCK_COPY_ALIGN / static_cast<uint32_t>(dataSize_);
        ...
        */
        ...
        usedCoreNum_ = usedCoreNum_ == 0 ? 1UL : usedCoreNum_;
        ...
        */
        ...
        for (int64_t m : cutSet) {
            ...
            int64_t rowNormalBlock = ops::CeilDiv(rowTotalNum_, m);
            // 列方向按照512基本块切分
            int64_t colNormalBlock = ops::CeilDiv(colNumAlign, n);
    ...
    std::set<int64_t> GluBaseTiling4RegBase::FindUniqueCut()
    {

...
ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::GetAttr()
{
    ...
    OP_CHECK_IF(tilingParams_.scaleAlg != 1,
    ...
    OP_CHECK_IF(tilingParams_.activateLeft != 1,
        OP_LOGE_WITH_INVALID_ATTR(context_->GetNodeName(), "activate_left", std::to_string(tilingParams_.activateLeft).c_str(),
...
ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::CheckDtype()
{
    ...
    OP_CHECK_IF(Y_SUPPORT_DTYPE_SET.count(y1Dtype) == 0 || y2Dtype != y1Dtype,
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "y1_out and y2_out",
        yDtypeMsg.c_str(),
...
ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::CheckOutputScaleDtype()
{
    ...
    OP_CHECK_IF(OUTPUT_SCALE_DTYPE_SET.count(scale1Dtype) == 0 || OUTPUT_SCALE_DTYPE_SET.count(scale2Dtype) == 0,
        ...
        scaleDtypeMsg.c_str(), "mxscale1_out and mxscale2_out dtype must be FLOAT8_E8M0"),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
    ...
    const gert::Shape &mxScale2Shape, const gert::Shape &xShape)
{
    ...
    OP_CHECK_IF(mxScale1Shape.GetDimNum() != expectScaleDimNum || mxScale2Shape.GetDimNum() != expectScaleDimNum,
        OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(context_->GetNodeName(), "mxscale1_out and mxscale2_out",
            dimMsg.c_str(), ("mxscale1_out and mxscale2_out shape dims must be "
            + std::to_string(expectScaleDimNum)).c_str()),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF((mxScale1Shape.GetDim(scaleLastIdx) != DIGIT_TWO) || (mxScale2Shape.GetDim(scaleLastIdx) != DIGIT_TWO),
        OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context_->GetNodeName(), "mxscale1_out and mxscale2_out",
            dimMsgScale.c_str(), "mxscale1_out and mxscale2_out shape last dim must be 2"),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(CheckMxScale2Shape(mxScale2Shape, xShape, dimGradX, batchDimCount, expectScaleDimNum)
        != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "CheckMxScale2Shape failed"), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::CheckMxScale1Shape(const gert::Shape &mxScale1Shape,
    const gert::Shape &xShape, int64_t dimGradX, int64_t batchDimCount)
{
    if (batchDimCount > 0) {
        for (int64_t i = 0; i < batchDimCount; i++) {
            std::string reasonMsgBatch = "x shape is " + Ops::Base::ToString(xShape) +
                ", the " + std::to_string(i) + "th dimension of mxscale1_out must match x";
            OP_CHECK_IF(mxScale1Shape.GetDim(i) != xShape.GetDim(i),
                ...
                Ops::Base::ToString(mxScale1Shape).c_str(), reasonMsgBatch.c_str()),
                return ge::GRAPH_FAILED);
        }
    }
    ...
    OP_CHECK_IF(mxScale1Shape.GetDim(mxScale1CeilDimIdx) != mxScaleShapeDim1,
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "mxscale1_out",
...
ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::CheckMxScale2Shape(const gert::Shape &mxScale2Shape,
    const gert::Shape &xShape, int64_t dimGradX, int64_t batchDimCount, int64_t expectScaleDimNum)
{
    if (batchDimCount > 0) {
        for (int64_t i = 0; i < batchDimCount; i++) {
            std::string reasonMsgBatch = "x shape is " + Ops::Base::ToString(xShape) +
                ", the " + std::to_string(i) + "th dimension of mxscale2_out must match x";
            OP_CHECK_IF(mxScale2Shape.GetDim(i) != xShape.GetDim(i),
                ...
                Ops::Base::ToString(mxScale2Shape).c_str(), reasonMsgBatch.c_str()),
                return ge::GRAPH_FAILED);
        }
    }
    int64_t mxScale2NDimIdx = expectScaleDimNum - 2;
    std::string reasonScale2MsgDim1 =
        "x shape is " + Ops::Base::ToString(xShape) + ", grad_x column count = " + std::to_string(dimGradX) +
        ", mxscale2_out shape[" + std::to_string(mxScale2NDimIdx) + "] must be equal to grad_x column count";
    OP_CHECK_IF(mxScale2Shape.GetDim(mxScale2NDimIdx) != dimGradX,
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "mxscale2_out",
    ...
    std::string reasonScale2MsgDim0 = "x shape is " + Ops::Base::ToString(xShape) + " isGroupIdx is " +
        ...
        std::to_string(tilingParams_.numGroups) + " dimM is " + std::to_string(tilingParams_.dimM) +
        ", mxscale2_out shape ceil-dim must be " +
        (tilingParams_.isGroupIdx == 1 ? "FloorDiv(dimM, 64) + numGroups" : "CeilDiv(dimM, 64)");
    OP_CHECK_IF(mxScale2Shape.GetDim(mxScale2CeilDimIdx) != mxScaleShapeDim2,
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "mxscale2_out",
...
ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::CheckYShape(const gert::Shape &xShape,
    const gert::Shape &y1Shape, const gert::Shape &y2Shape)
{
    int64_t xDimNum = xShape.GetDimNum();
    ...
    OP_CHECK_IF(xDimNum != y1DimNum,
        ...
        (std::to_string(xDimNum) + " vs " + std::to_string(y1DimNum)).c_str(), "same dim num"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(lastDim % SPLIT_BLOCK_H != 0,
            ...
            "input x's last dim (2N) must be a multiple of 64"), return ge::GRAPH_FAILED);
    ...
    for (int64_t i = 0; i < xDimNum - 2; i++) {
        std::string reasonBatch = "x shape is " + Ops::Base::ToString(xShape) + ", y1_out shape is "
            + Ops::Base::ToString(y1Shape) + ", the " + std::to_string(i) + "th dimension must match";
        OP_CHECK_IF(xShape.GetDim(i) != y1Shape.GetDim(i),
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "y1_out",
            Ops::Base::ToString(y1Shape).c_str(), reasonBatch.c_str()), return ge::GRAPH_FAILED);
    }

    // Check m dimension: y1_out's second-to-last dim must equal x's second-to-last dim (dimM)
    std::string reasonMsgM = "x shape is " + Ops::Base::ToString(xShape) +
        ", y1_out shape is " + Ops::Base::ToString(y1Shape) +
        ", the m-dimension (second-to-last) of y1_out must match x";
    OP_CHECK_IF(y1Shape.GetDim(y1DimNum - 2) != tilingParams_.dimM,
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "y1_out",
        Ops::Base::ToString(y1Shape).c_str(), reasonMsgM.c_str()), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        y1Shape.GetDim(y1DimNum - 1) != dimGradX,
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "y1_out", Ops::Base::ToString(y1Shape).c_str(),
        reasonMsg.c_str()), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(y1Shape != y2Shape,
        ...
        "y1_out shape must be same as y2_out shape"), return ge::GRAPH_FAILED);
...
ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::CheckShape()
{
    ...
    OP_CHECK_IF(xSize <= 0 || y1Size <= 0,
        ...
        "x and y1_out shape size must > 0"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(mxScale1Size <= 0 || mxScale2Size <= 0,
        ...
        sizeMxscaleMsg.c_str(), "mxscale1_out and mxscale2_out shape size must > 0"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(xDimNum < DIGIT_TWO || xDimNum > 6 || y1DimNum != xDimNum,
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "x and y1_out", shapeDimMsg.c_str(), "2D to 6D, same dim"),
        return ge::GRAPH_FAILED);

    // x and y1_out must have matching shape
    std::string shapeMsg =
        "x shape is " + Ops::Base::ToString(xShape) + " y1_out shape is " + Ops::Base::ToString(y1Shape);
    OP_CHECK_IF(xShape != y1Shape,
        OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context_->GetNodeName(), "x and y1_out", shapeMsg.c_str(),
        "x shape and y1_out shape must be identical."), return ge::GRAPH_FAILED);
...
void SwigluBackwardMxQuantWithDualAxisTiling::PrintTilingData()
{
    OP_LOGI(context_->GetNodeName(),
        ...
        tilingData_->splitBlockH, tilingData_->dimM, tilingData_->dimN, tilingData_->numGroups, tilingData_->dimBatch,
...
ge::graphStatus SwigluBackwardMxQuantWithDualAxisTiling::CheckYGradShape(const gert::Shape &xShape, int64_t xDimNum)
{
    ...
    OP_CHECK_IF(yGradDimNum != xDimNum,
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "y_grad",
        std::to_string(yGradDimNum).c_str(), (std::to_string(xDimNum) + "D").c_str()),
        return ge::GRAPH_FAILED);

    std::string yGradShapeMsg = "x shape is " + Ops::Base::ToString(xShape) +
        " y_grad shape is " + Ops::Base::ToString(yGradShape);
    for (int64_t i = 0; i < xDimNum - 2; i++) {
    ...
    OP_CHECK_IF(yGradShape.GetDim(yGradDimNum - 2) != xShape.GetDim(xDimNum - 2),
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "y_grad", yGradShapeMsg.c_str(),
        "The m-dimension (second-to-last) of y_grad must match x"), return ge::GRAPH_FAILED);

    std::string yGradDim1Msg = "x shape is " + Ops::Base::ToString(xShape) +
        " y_grad shape is " + Ops::Base::ToString(yGradShape) +
        ", dimN = x last dim / 2 = " + std::to_string(tilingParams_.dimN);
    OP_CHECK_IF(yGradShape.GetDim(yGradDimNum - 1) != tilingParams_.dimN,
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "y_grad", yGradDim1Msg.c_str(),

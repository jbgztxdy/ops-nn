...
/*!
 * \file test_geir_trans_quant_param_v2.cpp
 * \brief
 */
...
{
    Status ret = SUCCESS;
    auto add1 = op::TransQuantParamV2("add1");
    std::vector<int64_t> scaleShape = {3};
    std::vector<int64_t> offsetShape = {3};
    std::vector<int64_t> yShape = {3};
    ADD_INPUT(1, scale, inDtype, scaleShape);
    ADD_INPUT(2, offset, inDtype, offsetShape);
    ADD_INPUT_ATTR(round_mode, 0);
    ADD_OUTPUT(0, y, DT_UINT64, yShape);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        uint64_t* resultData = (uint64_t*)output_data_i;

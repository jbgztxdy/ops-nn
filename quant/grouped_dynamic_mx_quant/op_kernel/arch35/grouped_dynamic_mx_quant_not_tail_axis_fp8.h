...
template <typename T, typename U>
__aicore__ inline void GroupedDynamicMxQuantBaseFP8<T, U>::ParseTilingData(
    const GroupedDynamicMxQuantTilingData& tilingData)
{
                                                                   ...
                                                                   int64_t blockCount, int64_t dataLen)
{
    DataCopyExtParams outCopyParams_ = {
        ...
        static_cast<uint32_t>((postAxisSize_ - dataLen) * sizeof(uint8_t)), static_cast<uint32_t>(0)};
...
template <typename T, typename U>
__aicore__ inline void GroupedDynamicMxQuantBaseFP8<T, U>::Init(GM_ADDR x, GM_ADDR groupIndex, GM_ADDR y,
                                                                GM_ADDR mxScale,
                                                                const GroupedDynamicMxQuantTilingData& tilingData)
{
...
__aicore__ inline void GroupedDynamicMxQuantBaseFP8<T, U>::ComputeOCP(int64_t dataLen, int64_t blockCount,
                                                                      __ubuf__ T* xAddr, __ubuf__ uint8_t* mxScaleAddr,
                                                                      __ubuf__ uint8_t* yAddr)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < regLoop; i++) {
            ...
            Reg::Select<uint16_t>(reversedShareExpRegTensor, specialExpRegTensor, reversedShareExpRegTensor,
                                  specialDataMask);
            ...
            for (uint16_t j = 0; j < static_cast<uint16_t>(blockCount); j++) {
                ...
                if constexpr (IsSame<T, half>::value) {
                    ...
                    Reg::Mul(valueRegTensor, xRegTensor, (Reg::RegTensor<bfloat16_t>&)reversedShareExpRegTensor, p0);
        ...
        if (tailVfLen != 0) {
            ...
            for (uint16_t k = 0; k <= static_cast<uint16_t>(blockCountLoop); k++) {
                ...
                Reg::LoadAlign(xRegTensor1,
                               xAddr + (static_cast<uint16_t>(blockCount) - k - 1) * dataLen + regLoop * vfLen);
                if constexpr (IsSame<T, half>::value) {
            ...
            for (uint16_t j = 0; j < static_cast<uint16_t>(blockCount); j++) {
                ...
                if constexpr (IsSame<T, half>::value) {
                    ...
                    Reg::Mul(valueRegTensor, xRegTensor, (Reg::RegTensor<bfloat16_t>&)reversedShareExpRegTensor, p1);
                    Reg::Cast<float, bfloat16_t, castTraitZero>(yZero, valueRegTensor, maskAll);
                    Reg::Cast<float, bfloat16_t, castTraitOne>(yOne, valueRegTensor, maskAll);
                }
                Reg::Interleave(yZero, yOne, yZero, yOne);
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < regLoop; i++) {
            ...
            for (uint16_t j = 0; j <= static_cast<uint16_t>(blockCountLoop); j++) {
                ...
                Reg::LoadAlign(xRegTensor1,
                               xAddr + (static_cast<uint16_t>(blockCount) - j - 1) * dataLen + i * vfNum32);
            ...
            // 2. BF16→FP32
            Reg::Interleave(expMaxreShareExpFP16YOneRegTensor, fullZeroReg, expMaxreShareExpFP16YOneRegTensor,
                            fullZeroReg);
            Reg::Cast<float, T, castTraitHalf2Float>((Reg::RegTensor<float>&)expMaxAndAddOneFP32RegTensor,
                                                     ...
                                                     preMaskScale);
            // 3. Inf/Zero校验
            Reg::CompareScalar<uint32_t, CMPMODE::LT>(infMask, expMaxAndAddOneFP32RegTensor, MAX_EXP_FOR_FP32,
                                                      preMaskScale);
            ...
            // 5. 提取 Exp 和 Man
            Reg::ShiftRights(expAndreShareExpFP32RegTensor, expMaxAndAddOneFP32RegTensor, SHR_NUM_FOR_FP32,
                             preMaskScale);
            Reg::And(manAndmxScaleFP32RegTensor, expMaxAndAddOneFP32RegTensor, manMaskFP32Reg, preMaskScale);
            // 6. 条件向上取整
            Reg::CompareScalar<uint32_t, CMPMODE::GT>(p1, expAndreShareExpFP32RegTensor, NUMBER_ZERO, preMaskScale);
            Reg::CompareScalar<uint32_t, CMPMODE::LT>(p2, expAndreShareExpFP32RegTensor, NUMBER_TWO_FIVE_FOUR,
                                                      preMaskScale);
            Reg::CompareScalar<uint32_t, CMPMODE::GT>(p3, manAndmxScaleFP32RegTensor, NUMBER_ZERO, preMaskScale);
            Reg::MaskAnd(p1, p1, p2, preMaskScale);
            Reg::MaskAnd(p1, p1, p3, preMaskScale);
            Reg::CompareScalar<uint32_t, CMPMODE::EQ>(p2, expAndreShareExpFP32RegTensor, NUMBER_ZERO, preMaskScale);
            Reg::CompareScalar<uint32_t, CMPMODE::GT>(p3, manAndmxScaleFP32RegTensor, NUMBER_HALF, preMaskScale);
            ...
            // 7. 异常值处理
            Reg::Select<uint32_t>(manAndmxScaleFP32RegTensor, manAndmxScaleFP32RegTensor, fp8NanRegTensor32, infMask);
            ...
            // 9. 求 1/scale
            Reg::ShiftLefts(manAndmxScaleFP32RegTensor, manAndmxScaleFP32RegTensor, SHR_NUM_FOR_BF16, preMaskScale);
            Reg::Sub(expAndreShareExpFP32RegTensor, biasRegTensor32, manAndmxScaleFP32RegTensor, preMaskScale);
            Reg::Select<uint32_t>(expAndreShareExpFP32RegTensor, expAndreShareExpFP32RegTensor, nanRegTensor32,
                                  infMask);
            ...
            if constexpr (IsSame<T, half>::value) {
                Reg::Cast<float, bfloat16_t, castTraitZero>(
                    reversedShareExpRegTensorFP32Zero, (Reg::RegTensor<bfloat16_t>&)expMaxreShareExpFP16YOneRegTensor,
                    p0);
                Reg::Cast<float, bfloat16_t, castTraitOne>(
                    reversedShareExpRegTensorFP32One, (Reg::RegTensor<bfloat16_t>&)expMaxreShareExpFP16YOneRegTensor,
                    p0);
            }
            for (uint16_t j = 0; j < static_cast<uint16_t>(blockCount); j++) {
                ...
                if constexpr (IsSame<T, half>::value) {
                    ...
                    Reg::Mul(valueRegTensor, xRegTensor, (Reg::RegTensor<bfloat16_t>&)expMaxreShareExpFP16YOneRegTensor,
                             p0);
                    Reg::Cast<float, bfloat16_t, castTraitZero>(yZero, valueRegTensor, maskAll);
                    Reg::Cast<float, bfloat16_t, castTraitOne>(yOne, valueRegTensor, maskAll);
                }
        ...
        if (tailVfLen != 0) {
            ...
            for (uint16_t k = 0; k <= static_cast<uint16_t>(blockCountLoop); k++) {
                ...
                Reg::LoadAlign(xRegTensor1,
                               xAddr + (static_cast<uint16_t>(blockCount) - k - 1) * dataLen + regLoop * vfNum32);
                Reg::And(expMaxRegTensor0, (Reg::RegTensor<uint16_t>&)xRegTensor0, absMaskBF16Reg, p0);
                Reg::And(expMaxRegTensor1, (Reg::RegTensor<uint16_t>&)xRegTensor1, absMaskBF16Reg, p0);
                Reg::Max(expMaxRegTensor, expMaxRegTensor0, expMaxRegTensor1, p0);
                Reg::Max(expMaxreShareExpFP16YOneRegTensor, expMaxRegTensor, expMaxreShareExpFP16YOneRegTensor, p0);

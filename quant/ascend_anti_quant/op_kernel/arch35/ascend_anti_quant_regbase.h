...
public:
    __aicore__ inline AscendAntiQuantRegbase(const AscendAntiQuantTilingData* td,
                                             float scaleV, float offsetV)
...
private:
    ...
    __aicore__ inline void Compute(LocalTensor<uint8_t>& xLocal,
                                   LocalTensor<U>& outLocal,
                                   int64_t count);
...
template <typename T, typename U, bool SqrtMode>
__aicore__ inline void AscendAntiQuantRegbase<T, U, SqrtMode>::ProcessChunk(
...
template <typename T, typename U, bool SqrtMode>
__aicore__ inline void AscendAntiQuantRegbase<T, U, SqrtMode>::Compute(
    LocalTensor<uint8_t>& xLocal, LocalTensor<U>& outLocal, int64_t count)
{
        ...
        for (uint16_t i = 0; i < vfLoopNum; ++i) {
            ...
            if constexpr (IsSameType<T, hifloat8_t>::value) {
                AscendC::Reg::DataCopy<T, AscendC::Reg::LoadDist::DIST_UNPACK4_B8>(
                    vregX, xAddr + i * VL);
                AscendC::Reg::Cast<float, T, AAQ_CT_HIFP8_TO_FP32>(
                    vregFloatX, vregX, mask);
            } else if constexpr (IsSameType<T, fp8_e5m2_t>::value) {
                AscendC::Reg::DataCopy<T, AscendC::Reg::LoadDist::DIST_UNPACK4_B8>(
                    vregX, xAddr + i * VL);
                AscendC::Reg::Cast<float, T, AAQ_CT_FP8E5M2_TO_FP32>(
                    vregFloatX, vregX, mask);
            } else if constexpr (IsSameType<T, fp8_e4m3fn_t>::value) {
                AscendC::Reg::DataCopy<T, AscendC::Reg::LoadDist::DIST_UNPACK4_B8>(
                    vregX, xAddr + i * VL);
                AscendC::Reg::Cast<float, T, AAQ_CT_FP8E4M3_TO_FP32>(
                    vregFloatX, vregX, mask);
            } else if constexpr (IsSameType<T, int8_t>::value) {
                AscendC::Reg::DataCopy<T, AscendC::Reg::LoadDist::DIST_UNPACK4_B8>(
                    vregX, xAddr + i * VL);
                AscendC::Reg::Cast<half, T, AAQ_CT_INT8_TO_HALF>(
                    vregHalfX, vregX, mask);
                AscendC::Reg::Cast<float, half, AAQ_CT_HALF_TO_FP32>(
            ...
            if constexpr (IsSameType<U, half>::value) {
                AscendC::Reg::Cast<half, float, AAQ_CT_FP32_TO_HALF>(
                    vregY, vregFloatY, mask);
                AscendC::Reg::DataCopy<U, AscendC::Reg::StoreDist::DIST_PACK_B32>(
                    outAddr + i * VL, vregY, mask);
            } else {  // float
                AscendC::Reg::DataCopy<U, AscendC::Reg::StoreDist::DIST_NORM>(
                    outAddr + i * VL, vregFloatY, mask);
            }
        }
    }
}

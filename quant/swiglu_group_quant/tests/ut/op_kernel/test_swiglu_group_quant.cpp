...
extern "C" __global__ __aicore__ void swiglu_group_quant(
    GM_ADDR x, GM_ADDR weight, GM_ADDR groupIndex, GM_ADDR y, GM_ADDR scaleOut, GM_ADDR yOrigin, GM_ADDR workspace,
    GM_ADDR tiling);
...
void RunKernelWithTilingKey(uint64_t tilingKey, bool outputOrigin)
{
    ...
    uint8_t* scaleOut = reinterpret_cast<uint8_t*>(AscendC::GmAlloc(outputScaleSize));
    ...
    auto swigluGroupQuantKernel = [](GM_ADDR x, GM_ADDR weight, GM_ADDR groupIndex, GM_ADDR y, GM_ADDR scaleOut,
                                      GM_ADDR yOrigin, GM_ADDR workspace, GM_ADDR tiling) {
        ::swiglu_group_quant(x, weight, groupIndex, y, scaleOut, yOrigin, workspace, tiling);
    };
    ICPU_RUN_KF(swigluGroupQuantKernel, blockDim, x, nullptr, nullptr, y, scaleOut, yOrigin, workspace, tiling);

    AscendC::GmFree(x);
    AscendC::GmFree(y);
    AscendC::GmFree(scaleOut);

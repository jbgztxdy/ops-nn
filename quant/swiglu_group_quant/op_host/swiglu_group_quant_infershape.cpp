...
static const std::initializer_list<ge::DataType> Y_SUPPORT_DTYPE_SET = {
    ge::DT_FLOAT8_E4M3FN, ge::DT_FLOAT8_E5M2, ge::DT_FLOAT4_E2M1, ge::DT_FLOAT4_E1M2};
} // namespace
...
graphStatus InferShape4SwigluGroupQuant(gert::InferShapeContext* context)
{
    ...
    OP_CHECK_NULL_WITH_CONTEXT(context, scaleOutShape);
    ...
    if (Ops::Base::IsUnknownRank(*xShape)) {
        ...
        Ops::Base::SetUnknownRank(*scaleOutShape);
    ...
    if (xShape->GetDim(splitDim) == UNKNOWN_DIM_VALUE) {
        *yShape = *xShape;
        *yOriginShape = *xShape;
        scaleOutShape->SetDimNum(0);
        for (int64_t i = 0; i < xRank - 1; ++i) {
            scaleOutShape->AppendDim(xShape->GetDim(i));
        }
        scaleOutShape->AppendDim(UNKNOWN_DIM_VALUE);
        return ge::GRAPH_SUCCESS;
    }
...
graphStatus InferDtype4SwigluGroupQuant(gert::InferDataTypeContext* context)
{
    ...
    OP_CHECK_IF(std::find(Y_SUPPORT_DTYPE_SET.begin(), Y_SUPPORT_DTYPE_SET.end(), dstType) ==
        ...
        OP_LOGE(context->GetNodeName(),
                "dst_type is illegal, only supports FLOAT8_E4M3FN, FLOAT8_E5M2, FLOAT4_E2M1 or FLOAT4_E1M2, "
                "but got %d(%s).",
    ...
    const int64_t* quantModeAttr = attrsPtr->GetAttrPointer<int64_t>(ATTR_INDEX_QUANT_MODE);
    bool isMxQuant = quantModeAttr != nullptr && *quantModeAttr == MX_QUANT_MODE;
    context->SetOutputDataType(OUTPUT_IDX_SCALE_OUT, isMxQuant ? ge::DT_FLOAT8_E8M0 : ge::DT_FLOAT);

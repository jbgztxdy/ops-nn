...
/**
* @brief Performs SwiGLU activation followed by Block FP8, MX FP8, or MX FP4 quantization.
*
* @par Inputs:
* @li x: Required tensor of type float16 or bfloat16. The last dimension is split into two equal parts
* for SwiGLU and must be divisible by 256.
...
* FLOAT4_E2M1 and FLOAT4_E1M2. Defaults to FLOAT8_E4M3FN.
* @li quant_mode: Optional int. 0 means Block FP8 quantization, 1 means MX quantization. Defaults to 0.
...
* @li scale_out: Scale tensor. float32 for Block FP8 and float8_e8m0 for MX.
...
REG_OP(SwigluGroupQuant)
    ...
    .OUTPUT(scale_out, TensorType({DT_FLOAT, DT_FLOAT8_E8M0}))

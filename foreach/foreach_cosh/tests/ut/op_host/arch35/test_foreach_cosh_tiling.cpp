...
protected:
    ...
    {
        std::cout << "ForeachCoshTilingTest SetUp" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "ForeachCoshTilingTest TearDown" << std::endl;
    }
};
...
static void DoCase(std::initializer_list<int64_t> xShape, ge::DataType dt, uint64_t expectKey)
{
    ...
    struct ForeachCoshCompileInfo {} ci;
...
TEST_F(ForeachCoshTilingTest, foreach_cosh_float32)
{
    DoCase({4, 4}, ge::DT_FLOAT,
           GET_TPL_TILING_KEY(FOREACH_COSH_TPL_SCH_MODE_FLOAT));
}

TEST_F(ForeachCoshTilingTest, foreach_cosh_float16)
{
    DoCase({4, 4}, ge::DT_FLOAT16,
           GET_TPL_TILING_KEY(FOREACH_COSH_TPL_SCH_MODE_FLOAT16));
}

TEST_F(ForeachCoshTilingTest, foreach_cosh_bfloat16)
{
    DoCase({4, 4}, ge::DT_BF16,
           GET_TPL_TILING_KEY(FOREACH_COSH_TPL_SCH_MODE_BF16));
}

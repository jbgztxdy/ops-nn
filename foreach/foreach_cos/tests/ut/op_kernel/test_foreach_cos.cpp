...
extern "C" __global__ __aicore__ void foreach_cos(GM_ADDR inputs, GM_ADDR outputs, GM_ADDR workspace, GM_ADDR tiling);

class foreach_cos_test : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "foreach_cos_test SetUp\n" << std::endl; }
    static void TearDownTestCase() { std::cout << "foreach_cos_test TearDown\n" << std::endl; }
};

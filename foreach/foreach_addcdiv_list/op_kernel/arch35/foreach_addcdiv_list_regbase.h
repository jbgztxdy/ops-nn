...
/*!
 * \file foreach_addcdiv_list_regbase.h
 * \brief Reg-VF kernel for foreach_addcdiv_list: y[t][i] = x1[t][i] + scalars[t] * (x2[t][i] / x3[t][i]).
 *        Thin alias over the shared addc kernel with the divide combine op.
 */
#ifndef FOREACH_ADDCDIV_LIST_REGBASE_H
#define FOREACH_ADDCDIV_LIST_REGBASE_H

 ...
 * \brief arch35 regbase entry for foreach_addcdiv_list (per-tensor scalar list).
 */
#include "foreach_addcdiv_list_regbase.h"
    ...
    GM_ADDR tiling, TPipe* tPipe)
{
    ...
    ForeachAddcdivListRegbase<T, ScalarT, ForeachSoloTilingDataRegbase> op;
    op.Init(inputs, tensor1, tensor2, scalars, outputs, workspace, tilingData, tPipe);
    op.Process();
}

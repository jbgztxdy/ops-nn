...
@register("function_aclnnForeachPowList")
class aclnnForeachPowListExecutor(BaseApi):

    def __call__(self, input_data: InputDataset, with_output: bool = False):
        x1 = input_data.kwargs["x1"]
        x2 = input_data.kwargs["x2"]

        output = torch._foreach_pow(x1, x2)

        return output

@register("aclnnfunction_aclnnForeachPowList")

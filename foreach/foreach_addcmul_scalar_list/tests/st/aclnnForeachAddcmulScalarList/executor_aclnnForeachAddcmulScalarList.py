...
@register("function_aclnnForeachAddcmulScalarList")
class aclnnForeachAddcmulScalarListExecutor(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        scalars = input_data.kwargs["scalars"]
        
        output = None
        if self.device == 'cpu':
            output = self.converter(torch._foreach_addcmul, x1, x2, x3, scalars)
        else:
            output = torch._foreach_addcmul(x1, x2, x3, scalars.cpu())

        return output

@register("aclnnfunction_aclnnForeachAddcmulScalarList")

...
int CreateOppInGraph(vector<Tensor>& input, vector<Operator>& inputs, vector<Operator>& outputs, Graph& graph)
{
    ...
    for(int i=0;i<N;++i){ auto d=MkF32("x3_"+std::to_string(i),idx++,tShape,input); graph.AddOp(d); inputs.push_back(d); op1.set_dynamic_input_x3(i,d); }
    auto dS=MkF32("scalars",idx++,{N},input); graph.AddOp(dS); inputs.push_back(dS); op1.set_input_scalars(dS);

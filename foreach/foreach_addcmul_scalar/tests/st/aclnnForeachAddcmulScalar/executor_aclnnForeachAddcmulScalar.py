...
@register("function_aclnnForeachAddcmulScalar")
class aclnnForeachAddcmulScalarExecutor(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        scalar = input_data.kwargs["scalar"][0].item()
        
        output = None
        if self.device == 'cpu':
            output = self.converter(torch._foreach_addcmul, x1, x2, x3, scalar)
        else:
            output = torch._foreach_addcmul(x1, x2, x3, scalar)

        return output

@register("aclnnfunction_aclnnForeachAddcmulScalar")

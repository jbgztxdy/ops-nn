...
/*!
 * \file foreach_maximum_list_simt.h
 * \brief SIMT kernel implementation for foreach_maximum_list
 */

#ifndef FOREACH_MAXIMUM_LIST_SIMT_H
#define FOREACH_MAXIMUM_LIST_SIMT_H

#include "kernel_operator.h"
#include "kernel_tiling/kernel_tiling.h"
#include "foreach_maximum_list_tiling_data.h"
#include "foreach_maximum_list_tiling_key.h"
...
namespace NsForeachMaximumList {

using namespace AscendC;

constexpr uint32_t THREAD_NUM = 512;

// ===== float32 VF kernel (NaN-propagating maximum) =====
template <typename T>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM) inline void ForeachMaximumListSimtFp32(int64_t count, __gm__ T* x1,
                                                                                       __gm__ T* x2, __gm__ T* y)
{
    for (int64_t idx = static_cast<int64_t>(AscendC::Simt::GetThreadIdx()); idx < count;
        ...
        T a = x1[idx];
        T b = x2[idx];
        // NaN propagation: (isnan(a) || a > b) ? a : b
        y[idx] = (isnan(static_cast<float>(a)) || a > b) ? a : b;
    }
}

// ===== float16 VF kernel (NaN-propagating maximum) =====
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM) inline void ForeachMaximumListSimtFp16(int64_t count, __gm__ half* x1,
                                                                                       __gm__ half* x2, __gm__ half* y)
{
    for (int64_t idx = static_cast<int64_t>(AscendC::Simt::GetThreadIdx()); idx < count;
        ...
        half a = x1[idx];
        half b = x2[idx];
        // NaN propagation: (__hisnan(a) || a > b) ? a : b
        y[idx] = (__hisnan(a) || a > b) ? a : b;
    }
}

// ===== bfloat16 VF kernel (NaN-propagating maximum) =====
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM) inline void ForeachMaximumListSimtBf16(int64_t count,
...
// ===== int32 VF kernel (no NaN, simple max) =====
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM) inline void ForeachMaximumListSimtInt32(int64_t count,
                                                                                        __gm__ int32_t* x1,
                                                                                        __gm__ int32_t* x2,
                                                                                        __gm__ int32_t* y)
{
...
__aicore__ inline void Process(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, const ForeachMaximumListTilingData* tilingData)
{
    ...
    for (int32_t t = 0; t < tilingData->tensorNum; t++) {
        ...
        if (oS < oE) {
            ...
            if constexpr (schMode == 0) {
                AscendC::Simt::VF_CALL<ForeachMaximumListSimtFp32<T>>(AscendC::Simt::Dim3(THREAD_NUM), cnt, x1P, x2P,
                                                                      yP);
            } else if constexpr (schMode == 1) {
                AscendC::Simt::VF_CALL<ForeachMaximumListSimtFp16>(
                ...
                AscendC::Simt::VF_CALL<ForeachMaximumListSimtBf16>(
                ...
                AscendC::Simt::VF_CALL<ForeachMaximumListSimtInt32>(
...
#endif // FOREACH_MAXIMUM_LIST_SIMT_H

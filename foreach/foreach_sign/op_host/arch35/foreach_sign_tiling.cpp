...
/*!
 * \file foreach_sign_tiling.cpp
 * \brief Tiling implementation for foreach_sign (arch35 / ascend950)
 */
...
static ge::graphStatus ForeachSignTilingFunc(gert::TilingContext* context)
{
    ...
    ForeachSignTilingData* tiling = context->GetTilingData<ForeachSignTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ForeachSignTilingData), 0,
        sizeof(ForeachSignTilingData)) != EOK,
    ...
    if (xDtype == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_FLOAT);
    } else if (xDtype == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_FLOAT16);
    } else if (xDtype == ge::DT_INT8) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_INT8);
    } else if (xDtype == ge::DT_INT32) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_INT32);
    } else if (xDtype == ge::DT_INT64) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_INT64);
    } else if (xDtype == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_BF16);
    } else {
    ...
    OP_LOGD(context->GetNodeName(), "ForeachSignTilingFunc end. tensorNum=%d, "
        "needCoreNum=%d, tilingKey=%lu, maxTensorElements=%ld",
        tensorNum, needCoreNum, tilingKey, maxTensorElements);
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus TilingParseForForeachSign([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachSign)
    .Tiling(ForeachSignTilingFunc)
    .TilingParse<ForeachSignCompileInfo>(TilingParseForForeachSign);

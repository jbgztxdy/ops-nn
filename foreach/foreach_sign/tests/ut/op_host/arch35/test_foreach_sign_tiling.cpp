...
protected:
    ...
    {
        std::cout << "ForeachSignTilingTest SetUp" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "ForeachSignTilingTest TearDown" << std::endl;
    }
};
...
static void DoCase(std::initializer_list<int64_t> xShape, ge::DataType dt, uint64_t expectKey)
{
    ...
    struct ForeachSignCompileInfo {} ci;
...
TEST_F(ForeachSignTilingTest, foreach_sign_float32)
{
    DoCase({4, 4}, ge::DT_FLOAT,
           GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_FLOAT));
}

TEST_F(ForeachSignTilingTest, foreach_sign_float16)
{
    DoCase({4, 4}, ge::DT_FLOAT16,
           GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_FLOAT16));
}

TEST_F(ForeachSignTilingTest, foreach_sign_bfloat16)
{
    DoCase({4, 4}, ge::DT_BF16,
           GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_BF16));
}

TEST_F(ForeachSignTilingTest, foreach_sign_int8)
{
    DoCase({4, 4}, ge::DT_INT8,
           GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_INT8));
}

TEST_F(ForeachSignTilingTest, foreach_sign_int32)
{
    DoCase({4, 4}, ge::DT_INT32,
           GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_INT32));
}

TEST_F(ForeachSignTilingTest, foreach_sign_int64)
{
    DoCase({4, 4}, ge::DT_INT64,
           GET_TPL_TILING_KEY(FOREACH_SIGN_SCH_MODE_INT64));
}

...
@register("function_aclnnForeachSubScalarV2")
class aclnnForeachSubScalarV2Executor(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        output = None
        if self.device == 'cpu':
            output = self.converter(torch._foreach_sub, x1, scalar)
        else:
            output = torch._foreach_sub(x1, scalar)

        return output

@register("aclnnfunction_aclnnForeachSubScalarV2")

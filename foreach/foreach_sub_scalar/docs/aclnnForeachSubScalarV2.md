...
- 算子功能：对输入张量列表的每个张量与标量scalar执行相减运算。本接口相较于[aclnnForeachSubScalar](aclnnForeachSubScalar.md)，修改入参scalar的结构类型aclTensor为aclScalar，请根据实际情况选择合适的接口。
- 计算公式：
...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnForeachSubScalarV2GetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachSubScalarV2”接口执行计算。

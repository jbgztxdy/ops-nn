...
- 算子功能：
  返回一个和输入张量列表同样形状大小的新张量列表，对张量列表和标量值scalar执行逐元素比较，返回最大值的张量。
  本接口相较于[aclnnForeachMaximumScalar](aclnnForeachMaximumScalar.md)，修改入参scalar的结构类型aclTensor为aclScalar，请根据实际情况选择合适的接口。
- 计算公式：
...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnForeachMaximumScalarV2GetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachMaximumScalarV2”接口执行计算。
...
- **参数说明**：
  <table style="undefined;table-layout: fixed; width: 1503px"><colgroup>
    ...
    <tr>
      ...
      <td><ul><li>与入参`x`的数据类型具有一定对应关系，且数值不超过入参`x`对应数据类型的数值范围:</li>

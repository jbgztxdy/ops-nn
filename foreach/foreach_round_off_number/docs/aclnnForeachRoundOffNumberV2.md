...
- 算子功能：
  ...
  本接口相较于[aclnnForeachRoundOffNumber](aclnnForeachRoundOffNumber.md)，修改入参roundMode的结构类型aclTensor为aclScalar，请根据实际情况选择合适的接口。
- 计算公式：
...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnForeachRoundOffNumberV2GetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachRoundOffNumberV2”接口执行计算。

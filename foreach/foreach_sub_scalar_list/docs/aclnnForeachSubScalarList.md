...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnForeachSubScalarListGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachSubScalarList”接口执行计算。
...
- **参数说明**：
  ...
  <table style="undefined;table-layout: fixed; width: 1503px"><colgroup>
    ...
    <tr>
      ...
      <td><ul><li>数据类型与入参`x`的数据类型具有一定对应关系：<ul><li>当入参`x`的数据类型为FLOAT32、FLOAT16、BFLOAT16时，`scalars`的数据类型仅支持FLOAT32。</li><li>当入参`x`的数据类型为INT32时，`scalars`的数据类型仅支持INT64。</li></ul></td>
      <td>FLOAT32、INT64</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>

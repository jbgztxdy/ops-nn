...
- 算子功能：将指定的张量值加到张量列表中的每个张量中，并返回更新后的张量列表。
...
每个算子分为[两段式接口](../../../docs/context/两段式接口.md)，必须先调用“aclnnForeachAddScalarGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnForeachAddScalar”接口执行计算。

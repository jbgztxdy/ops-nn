...
/*!
 * \file foreach_tan_tiling_arch35.cpp
 * \brief tiling implementation for foreach_tan operator (arch35/ascend950)
 */
...
#include "foreach/foreach_tan/op_kernel/arch35/foreach_tan_tiling_data.h"
#include "foreach/foreach_tan/op_kernel/arch35/foreach_tan_tiling_key.h"

namespace optiling {

constexpr uint32_t DCACHE_SIZE = 128 * 1024;
constexpr int64_t ASCENDC_TOOLS_WORKSPACE = 16 * 1024 * 1024; // 16MB
constexpr int64_t MAX_TENSOR_COUNT = 256;
constexpr int64_t ALIGN_SIZE = 32;
constexpr int64_t MIN_PER_CORE_ELEMENTS = 1024;

struct ForeachTanCompileInfo {};
...
static ge::graphStatus ForeachTanTilingFunc(gert::TilingContext* context)
{
    OP_LOGD(context->GetNodeName(), "ForeachTanTilingFunc begin.");
    ...
    // 4. Calculate core count based on max tensor
    ...
    // 6. Set tiling key based on x dtype
    uint64_t tilingKey = 0;
    if (xDtype == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_TAN_SCH_MODE_FLOAT);
    } else if (xDtype == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_TAN_SCH_MODE_FLOAT16);
    } else if (xDtype == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_TAN_SCH_MODE_BF16);
    } else {

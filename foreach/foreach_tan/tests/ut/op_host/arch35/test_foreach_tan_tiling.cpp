...
#include "../../../../op_kernel/arch35/foreach_tan_tiling_data.h"
#include "../../../../op_kernel/arch35/foreach_tan_tiling_key.h"

using namespace ut_util;

class ForeachTanTilingArch35 : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "ForeachTanTilingArch35 SetUp" << std::endl; }
    static void TearDownTestCase() { std::cout << "ForeachTanTilingArch35 TearDown" << std::endl; }
};
...
static void DoCase(std::initializer_list<int64_t> xShape, ge::DataType dt, uint64_t expectKey)
{
    ...
    GetPlatFormInfos(R"({"hardware_info":{"UB_SIZE":196608,"CORE_NUM":20}})", soc, ai, intr);
    pf.Init();

    struct ForeachTanCompileInfo {
    } ci;
    auto opType = std::string("ForeachTan");
...
TEST_F(ForeachTanTilingArch35, foreach_tan_arch35_float32)
{
    DoCase({8, 4}, ge::DT_FLOAT, GET_TPL_TILING_KEY(FOREACH_TAN_SCH_MODE_FLOAT));
}

TEST_F(ForeachTanTilingArch35, foreach_tan_arch35_float16)
{
    DoCase({8, 4}, ge::DT_FLOAT16, GET_TPL_TILING_KEY(FOREACH_TAN_SCH_MODE_FLOAT16));
}

TEST_F(ForeachTanTilingArch35, foreach_tan_arch35_bfloat16)
{
    DoCase({8, 4}, ge::DT_BF16, GET_TPL_TILING_KEY(FOREACH_TAN_SCH_MODE_BF16));
}

...
/*!
 * \file foreach_sigmoid_tiling.cpp
 * \brief Tiling implementation for foreach_sigmoid (arch35/SIMT)
 */
...
#include "foreach/foreach_sigmoid/op_kernel/arch35/foreach_sigmoid_tiling_data.h"
#include "foreach/foreach_sigmoid/op_kernel/arch35/foreach_sigmoid_tiling_key.h"
...
struct ForeachSigmoidCompileInfo {};
...
static ge::graphStatus ForeachSigmoidTilingFunc(gert::TilingContext* context)
{
    ...
    if (xDtype == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGMOID_SCH_MODE_FLOAT16);
    } else if (xDtype == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGMOID_SCH_MODE_FLOAT);
    } else if (xDtype == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIGMOID_SCH_MODE_BF16);
    } else {
...
IMPL_OP_OPTILING(ForeachSigmoid)
    .Tiling(ForeachSigmoidTilingFunc)
    .TilingParse<ForeachSigmoidCompileInfo>(TilingParseForForeachSigmoid);

...
extern "C" __global__ __aicore__ void foreach_asin(GM_ADDR inputs_1, GM_ADDR outputs, GM_ADDR workspace,
                                                   GM_ADDR tiling);

class foreach_asin_test : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "foreach_asin_test SetUp\n" << std::endl; }
    static void TearDownTestCase() { std::cout << "foreach_asin_test TearDown\n" << std::endl; }
};

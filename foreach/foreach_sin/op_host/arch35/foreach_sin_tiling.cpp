...
/*!
 * \file foreach_sin_tiling.cpp
 * \brief Tiling implementation for foreach_sin (arch35 / ascend950)
 */
...
#include "../../op_kernel/arch35/foreach_sin_tiling_data.h"
#include "../../op_kernel/arch35/foreach_sin_tiling_key.h"
...
static ge::graphStatus ForeachSinTilingFunc(gert::TilingContext* context)
{
    OP_LOGD(context->GetNodeName(), "ForeachSinTilingFunc begin.");
    ...
    ForeachSinTilingData* tiling = context->GetTilingData<ForeachSinTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ForeachSinTilingData), 0, sizeof(ForeachSinTilingData)) != EOK,
    ...
    if (xDtype == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIN_SCH_MODE_FLOAT16);
    } else if (xDtype == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIN_SCH_MODE_FLOAT);
    } else if (xDtype == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SIN_SCH_MODE_BF16);
    } else {
    ...
    OP_LOGD(context->GetNodeName(),
            "ForeachSinTilingFunc end. tensorNum=%d, "
            "needCoreNum=%d, tilingKey=%lu, maxTensorElements=%ld",
            tensorNum, needCoreNum, tilingKey, maxTensorElements);
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus TilingParseForForeachSin([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachSin).Tiling(ForeachSinTilingFunc).TilingParse<ForeachSinCompileInfo>(TilingParseForForeachSin);

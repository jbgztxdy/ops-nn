...
@register("function_aclnnForeachAddcdivScalarList")
class aclnnForeachAddcdivScalarListExecutor(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        output = None
        if self.device == 'cpu':
            output = self.converter(torch._foreach_addcdiv, x1, x2, x3, scalars)
        else:
            output = torch._foreach_addcdiv(x1, x2, x3, scalars.cpu())

        return output

@register("aclnnfunction_aclnnForeachAddcdivScalarList")

...
class ForeachMulListTilingTest : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "ForeachMulListTilingTest SetUp" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "ForeachMulListTilingTest TearDown" << std::endl;
    }
};

static void DoCase(std::initializer_list<int64_t> xShape,
                   ge::DataType dt, uint64_t expectKey)
{
    fe::PlatFormInfos pf;
    ...
    struct ForeachMulListCompileInfo {} ci;
    auto opType = std::string("ForeachMulList");
    ...
    gert::StorageShape xS1 = {xShape, xShape};
    gert::StorageShape xS2 = {xShape, xShape};
    ...
    auto th = gert::TilingContextFaker().NodeIoNum(2, 1).IrInstanceNum({1, 1}, {1})
        .InputShapes({&xS1, &xS2}).OutputShapes({&yS})
...
TEST_F(ForeachMulListTilingTest, foreach_mul_list_float32)
{
    DoCase({4, 4}, ge::DT_FLOAT,
           GET_TPL_TILING_KEY(FOREACH_MUL_LIST_TPL_SCH_MODE_FLOAT));
}

TEST_F(ForeachMulListTilingTest, foreach_mul_list_float16)
{
    DoCase({4, 4}, ge::DT_FLOAT16,
           GET_TPL_TILING_KEY(FOREACH_MUL_LIST_TPL_SCH_MODE_FLOAT16));
}

TEST_F(ForeachMulListTilingTest, foreach_mul_list_bf16)
{
    DoCase({4, 4}, ge::DT_BF16,
           GET_TPL_TILING_KEY(FOREACH_MUL_LIST_TPL_SCH_MODE_BF16));
}

TEST_F(ForeachMulListTilingTest, foreach_mul_list_int32)
{
    DoCase({4, 4}, ge::DT_INT32,
           GET_TPL_TILING_KEY(FOREACH_MUL_LIST_TPL_SCH_MODE_INT32));
}

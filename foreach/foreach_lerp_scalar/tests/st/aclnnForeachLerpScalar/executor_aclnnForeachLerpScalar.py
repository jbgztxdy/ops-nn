...
@register("function_aclnnForeachLerpScalar")
class aclnnForeachLerpScalarExecutor(BaseApi):

    def __call__(self, input_data: InputDataset, with_output: bool = False):
        x1 = input_data.kwargs["x1"]
        x2 = input_data.kwargs["x2"]
        weight = input_data.kwargs["weight"]
        output = torch._foreach_lerp(x1, x2, weight)
        
        return output

@register("aclnnfunction_aclnnForeachLerpScalar")

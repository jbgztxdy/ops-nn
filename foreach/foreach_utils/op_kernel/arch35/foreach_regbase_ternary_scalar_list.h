...
/*!
 * \file foreach_regbase_ternary_scalar_list.h
 * \brief Regbase (Reg-VF) base for foreach ternary ops with a per-tensor scalar list:
 *        y[i] = f(x1[i], x2[i], x3[i], scalars[i]). 3 input tensor lists batched in/out;
 *        the per-tensor tensorIndex is threaded to the Predicate so it reads scalars[tensorIndex].
 *        Derived supplies Compute(inLocal, t1Local, t2Local, outLocal, tensorIndex, dataCount).
 */
#ifndef FOREACH_REGBASE_TERNARY_SCALAR_LIST_H
#define FOREACH_REGBASE_TERNARY_SCALAR_LIST_H
...
public:
    ...
    __aicore__ inline void Compute(int64_t tensorIndex, int64_t dataCount)
    {
        ...
        pred_.Compute(inLocal, inLocalTensorOne, inLocalTensorTwo, outLocal, tensorIndex, dataCount);
...
#endif // FOREACH_REGBASE_TERNARY_SCALAR_LIST_H

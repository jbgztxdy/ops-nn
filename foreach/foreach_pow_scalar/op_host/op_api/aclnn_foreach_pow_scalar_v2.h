 ...
 */
ACLNN_API aclnnStatus aclnnForeachPowScalarV2GetWorkspaceSize(const aclTensorList* x, const aclScalar* scalar,
 ...
 * device侧申请的workspace大小，由第一段接口aclnnForeachPowScalarV2GetWorkspaceSize获取。 param [in] stream: acl
 * stream流。 param [in] executor: op执行器，包含了算子计算流程。 return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnForeachPowScalarV2(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                              aclrtStream stream);

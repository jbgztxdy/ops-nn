...
protected:
    ...
    {
        std::cout << "ForeachPowScalarTilingTest SetUp" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "ForeachPowScalarTilingTest TearDown" << std::endl;
    }
};

static void DoCase(std::initializer_list<int64_t> xShape,
                   ge::DataType inputDt, uint64_t expectKey)
{
    fe::PlatFormInfos pf;
    ...
    struct ForeachPowScalarCompileInfo {} ci;
    ...
    auto th = gert::TilingContextFaker().NodeIoNum(2, 1).IrInstanceNum({1, 1}, {1})
        .InputShapes({&xS, &sS}).OutputShapes({&yS})
...
TEST_F(ForeachPowScalarTilingTest, foreach_pow_scalar_float32)
{
    DoCase({4, 4}, ge::DT_FLOAT,
           GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_FLOAT));
}

TEST_F(ForeachPowScalarTilingTest, foreach_pow_scalar_float16)
{
    DoCase({4, 4}, ge::DT_FLOAT16,
           GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_FLOAT16));
}

TEST_F(ForeachPowScalarTilingTest, foreach_pow_scalar_int32)
{
    DoCase({4, 4}, ge::DT_INT32,
           GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_INT32));
}

TEST_F(ForeachPowScalarTilingTest, foreach_pow_scalar_bfloat16)
{
    DoCase({4, 4}, ge::DT_BF16,
           GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_BF16));
}

...
protected:
    ...
    {
        std::cout << "ForeachLogTilingTest SetUp" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "ForeachLogTilingTest TearDown" << std::endl;
    }
};
...
static void DoCase(std::initializer_list<int64_t> xShape, ge::DataType dt, uint64_t expectKey)
{
    ...
    struct ForeachLogCompileInfo {} ci;
...
TEST_F(ForeachLogTilingTest, foreach_log_float32)
{
    DoCase({4, 4}, ge::DT_FLOAT,
           GET_TPL_TILING_KEY(FOREACH_LOG_TPL_SCH_MODE_FLOAT));
}

TEST_F(ForeachLogTilingTest, foreach_log_float16)
{
    DoCase({4, 4}, ge::DT_FLOAT16,
           GET_TPL_TILING_KEY(FOREACH_LOG_TPL_SCH_MODE_FLOAT16));
}

TEST_F(ForeachLogTilingTest, foreach_log_bfloat16)
{
    DoCase({4, 4}, ge::DT_BF16,
           GET_TPL_TILING_KEY(FOREACH_LOG_TPL_SCH_MODE_BF16));
}

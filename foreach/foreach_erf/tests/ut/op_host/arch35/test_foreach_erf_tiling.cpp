...
protected:
    ...
    {
        std::cout << "ForeachErfTilingTest SetUp" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "ForeachErfTilingTest TearDown" << std::endl;
    }
};
...
static void DoCase(std::initializer_list<int64_t> xShape, ge::DataType dt, uint64_t expectKey)
{
    ...
    struct ForeachErfCompileInfo {} ci;
...
TEST_F(ForeachErfTilingTest, foreach_erf_float32)
{
    DoCase({4, 4}, ge::DT_FLOAT,
           GET_TPL_TILING_KEY(FOREACH_ERF_TPL_SCH_MODE_FLOAT));
}

TEST_F(ForeachErfTilingTest, foreach_erf_float16)
{
    DoCase({4, 4}, ge::DT_FLOAT16,
           GET_TPL_TILING_KEY(FOREACH_ERF_TPL_SCH_MODE_FLOAT16));
}

TEST_F(ForeachErfTilingTest, foreach_erf_bfloat16)
{
    DoCase({4, 4}, ge::DT_BF16,
           GET_TPL_TILING_KEY(FOREACH_ERF_TPL_SCH_MODE_BF16));
}

...
__aicore__ void L0PingPongAllClose(__gm__ uint8_t* input, __gm__ uint8_t* weight, __gm__ uint8_t* bias,
                                   __gm__ uint8_t* output, const Conv3DV2TilingData& tilingData)
{
    // BL1 bypass关闭 + M模式输出
    if (tilingData.conv3dApiTiling.bl1BypassFlag == static_cast<uint8_t>(ConvBL1ByPass::BYPASS_OFF) &&
        ...
        DispatchConv3DV2Kernel<InputType, WeightType, OutputType, BiasType, ConvL0PingPong::ALL_CLOSE,
                               ConvBL1ByPass::BYPASS_OFF, GroupConvType::NoGroup_Conv, OutputOrder::M_Mode,
                               QuantType::NO_QUANT>(input, weight, bias, nullptr, nullptr, output, nullptr,
                                                    &tilingData);
        return;
    }
    // BL1 bypass关闭 + HW模式输出
    if (tilingData.conv3dApiTiling.bl1BypassFlag == static_cast<uint8_t>(ConvBL1ByPass::BYPASS_OFF) &&
        ...
        DispatchConv3DV2Kernel<InputType, WeightType, OutputType, BiasType, ConvL0PingPong::ALL_CLOSE,
                               ConvBL1ByPass::BYPASS_OFF, GroupConvType::NoGroup_Conv, OutputOrder::HW_Mode,
                               QuantType::NO_QUANT>(input, weight, bias, nullptr, nullptr, output, nullptr,
                                                    &tilingData);
        return;
    }
    ...
    if (tilingData.conv3dApiTiling.bl1BypassFlag == static_cast<uint8_t>(ConvBL1ByPass::BYPASS_ON) &&
        tilingData.conv3dApiTiling.outputOrder == static_cast<int8_t>(OutputOrder::M_Mode)) {
        DispatchConv3DV2Kernel<InputType, WeightType, OutputType, BiasType, ConvL0PingPong::ALL_CLOSE,
                               ConvBL1ByPass::BYPASS_ON, GroupConvType::NoGroup_Conv, OutputOrder::M_Mode,
                               QuantType::NO_QUANT>(input, weight, bias, nullptr, nullptr, output, nullptr,
                                                    &tilingData);
        return;
    }
    ...
    if (tilingData.conv3dApiTiling.bl1BypassFlag == static_cast<uint8_t>(ConvBL1ByPass::BYPASS_ON) &&
        tilingData.conv3dApiTiling.outputOrder == static_cast<int8_t>(OutputOrder::HW_Mode)) {
        DispatchConv3DV2Kernel<InputType, WeightType, OutputType, BiasType, ConvL0PingPong::ALL_CLOSE,
                               ConvBL1ByPass::BYPASS_ON, GroupConvType::NoGroup_Conv, OutputOrder::HW_Mode,
                               QuantType::NO_QUANT>(input, weight, bias, nullptr, nullptr, output, nullptr,
                                                    &tilingData);
        return;
    }
 ...
 */
template <typename InputType, typename WeightType, typename OutputType, typename BiasType>
__aicore__ void L0BPingPongOpen(__gm__ uint8_t* input, __gm__ uint8_t* weight, __gm__ uint8_t* bias,
                                __gm__ uint8_t* output, const Conv3DV2TilingData& tilingData)
{
    // BL1 bypass关闭 + M模式输出
    if (tilingData.conv3dApiTiling.bl1BypassFlag == static_cast<uint8_t>(ConvBL1ByPass::BYPASS_OFF) &&
        tilingData.conv3dApiTiling.outputOrder == static_cast<int8_t>(OutputOrder::M_Mode)) {
        DispatchConv3DV2Kernel<InputType, WeightType, OutputType, BiasType, ConvL0PingPong::L0B_OPEN,
                               ConvBL1ByPass::BYPASS_OFF, GroupConvType::NoGroup_Conv, OutputOrder::M_Mode,
                               QuantType::NO_QUANT>(input, weight, bias, nullptr, nullptr, output, nullptr,
                                                    &tilingData);
        return;
 ...
 */
template <typename InputType, typename WeightType, typename OutputType, typename BiasType>
__aicore__ void L0APingPongOpen(__gm__ uint8_t* input, __gm__ uint8_t* weight, __gm__ uint8_t* bias,
                                __gm__ uint8_t* output, const Conv3DV2TilingData& tilingData)
{
    // BL1 bypass关闭 + M模式输出
    if (tilingData.conv3dApiTiling.bl1BypassFlag == static_cast<uint8_t>(ConvBL1ByPass::BYPASS_OFF) &&
        tilingData.conv3dApiTiling.outputOrder == static_cast<int8_t>(OutputOrder::M_Mode)) {
        DispatchConv3DV2Kernel<InputType, WeightType, OutputType, BiasType, ConvL0PingPong::L0A_OPEN,
                               ConvBL1ByPass::BYPASS_OFF, GroupConvType::NoGroup_Conv, OutputOrder::M_Mode,
                               QuantType::NO_QUANT>(input, weight, bias, nullptr, nullptr, output, nullptr,
                                                    &tilingData);
        return;
 ...
 */
template <typename InputType, typename WeightType, typename OutputType, typename BiasType>
__aicore__ void L0PingPongAllOpen(__gm__ uint8_t* input, __gm__ uint8_t* weight, __gm__ uint8_t* bias,
                                  __gm__ uint8_t* output, const Conv3DV2TilingData& tilingData)
{
    // BL1 bypass关闭 + M模式输出
    if (tilingData.conv3dApiTiling.bl1BypassFlag == static_cast<uint8_t>(ConvBL1ByPass::BYPASS_OFF) &&
        tilingData.conv3dApiTiling.outputOrder == static_cast<int8_t>(OutputOrder::M_Mode)) {
        DispatchConv3DV2Kernel<InputType, WeightType, OutputType, BiasType, ConvL0PingPong::ALL_OPEN,
                               ConvBL1ByPass::BYPASS_OFF, GroupConvType::NoGroup_Conv, OutputOrder::M_Mode,
                               QuantType::NO_QUANT>(input, weight, bias, nullptr, nullptr, output, nullptr,
                                                    &tilingData);
        return;
 ...
 */
template <typename T>
__global__ __aicore__ void Conv3dCustomKernel(__gm__ uint8_t* input, __gm__ uint8_t* weight, __gm__ uint8_t* bias,
                                              __gm__ uint8_t* output, const Conv3DV2TilingData tilingData)
{
 ...
 */
template <typename T>
void Conv3dCustomApi(aclrtStream stream, const at::Tensor& input, const at::Tensor& weight, const torch::Tensor& output,
                     const vector<int64_t> strideList, const vector<int64_t> paddingList,
...
{
    // 获取输出张量的原始形状（NCDHW格式）
    vector<int64_t> oriOutputShapeList = {output.size(N_DIM_IDX_NDC1HWC0), oriWeightShapeList[N_DIM_IDX_NCDHW],
                                          output.size(D_DIM_IDX_NDC1HWC0), output.size(H_DIM_IDX_NDC1HWC0),
                                          output.size(W_DIM_IDX_NDC1HWC0)};
    ...
    if constexpr (std::is_same_v<T, c10::Half>) {
        conv3dTilingEngine.SetDataType(Conv3dApiTiling::ConvDtype::FLOAT16, Conv3dApiTiling::ConvDtype::FLOAT16,
                                       Conv3dApiTiling::ConvDtype::FLOAT16);
    } else if constexpr (std::is_same_v<T, c10::BFloat16>) {
        conv3dTilingEngine.SetDataType(Conv3dApiTiling::ConvDtype::BF16, Conv3dApiTiling::ConvDtype::BF16,
                                       Conv3dApiTiling::ConvDtype::BF16);
    } else {
        conv3dTilingEngine.SetDataType(Conv3dApiTiling::ConvDtype::FLOAT32, Conv3dApiTiling::ConvDtype::FLOAT32,
    ...
    if (bias.has_value()) {
        ...
        TORCH_CHECK(biasTensor.numel() == oriWeightShapeList[N_DIM_IDX_NCDHW], "Bias numel must equal Cout (",
                    oriWeightShapeList[N_DIM_IDX_NCDHW], "), but got ", biasTensor.numel());
 ...
 */
template <>
void Conv3dCustomApi<double>(aclrtStream stream, const at::Tensor& input, const at::Tensor& weight,
                             const torch::Tensor& output, const vector<int64_t> strideList,
                             const vector<int64_t> paddingList, const vector<int64_t> dilationList,
                                 ...
                                 const c10::IntArrayRef& pads, const c10::IntArrayRef& dilations)
{
    ...
    vector<int64_t> kernelSize = {oriWeightShape[D_DIM_IDX_NCDHW], oriWeightShape[H_DIM_IDX_NCDHW],
                                  oriWeightShape[W_DIM_IDX_NCDHW]};
 ...
 * @return 输出张量（NDC1HWC0格式）
 */
torch::Tensor Conv3dCustomNpu(const torch::Tensor& input, const torch::Tensor& weight, const c10::IntArrayRef& strides,
                              const c10::IntArrayRef& pads, const c10::IntArrayRef& dilations,
                              const c10::IntArrayRef& oriInputShape, const c10::IntArrayRef& oriWeightShape,
                              const c10::optional<torch::Tensor>& bias, bool enableHF32)
{
    ...
    if (bias.has_value()) {
        ...
        if (input.scalar_type() == at::kBFloat16) {
            TORCH_CHECK(bias.value().scalar_type() == at::kFloat,
                        "Bias type must be float when input dtype is bfloat16");
        }
    }
    ...
    auto acl_call = [=]() -> int {
        AT_DISPATCH_FLOATING_TYPES_AND2(at::kHalf, at::kBFloat16, input.scalar_type(), "Conv3dCustomNpu", [&] {
            Conv3dCustomApi<scalar_t>(stream, input, weight, output, strideList, paddingList, dilationList,
                                      oriInputShapeList, oriWeightShapeList, bias, enableHF32);
        });
        return 0;
    };
...
{
    m.def("conv3d_custom(Tensor input, Tensor weight, int[3] stride, int[3] padding, int[3] dilation, int[5] "
          "oriInputShape, int[5] oriWeightShape, Tensor? bias, bool enable_hf32 = False) -> Tensor");
}

// 注册NPU实现，将conv3d_custom算子映射到Conv3dCustomNpu函数
TORCH_LIBRARY_IMPL(EXTENSION_MODULE_NAME, PrivateUse1, m) { m.impl("conv3d_custom", Conv3dCustomNpu); }

...
// 注册Meta实现，用于形状推导
TORCH_LIBRARY_IMPL(EXTENSION_MODULE_NAME, Meta, m) { m.impl("add", add_meta); }
...
torch::Tensor add_npu(const torch::Tensor& x, const torch::Tensor& y)
{
    ...
    auto acl_call = [=]() -> int {
        // 根据输入张量的数据类型进行分发
        AT_DISPATCH_SWITCH(x.scalar_type(), "add_npu",
                           // Float32类型处理
                           AT_DISPATCH_CASE(torch::kFloat32,
                                            [&] {
                                                using scalar_t = float;
                                                add_kernel<scalar_t><<<numBlocks, nullptr, stream>>>(
...
// 注册NPU实现
TORCH_LIBRARY_IMPL(EXTENSION_MODULE_NAME, PrivateUse1, m) { m.impl("add", add_npu); }

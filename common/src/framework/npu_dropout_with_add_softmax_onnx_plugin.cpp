...
static Status ParseOpToGraphNpuDropoutWithAddSoftmax(const ge::Operator& op, ge::Graph& graph)
{
    ...
    auto oneslike_cast_op = ge::op::Cast((ori_name + "_Cast_oneslike").c_str())
                                .set_input_x(oneslike_op)
                                .set_attr_dst_type(4);

    // opType[AxpyWithSoftmaxAndDropOutDoMask] only supports fp16.
    auto data0_cast_op = ge::op::Cast((ori_name + "_Cast_data0").c_str())
                             .set_input_x(identity_data0_op)
                             .set_attr_dst_type(1);
    auto data1_cast_op = ge::op::Cast((ori_name + "_Cast_data1").c_str())
                             .set_input_x(identity_data1_op)
                             .set_attr_dst_type(1);

    // retain = 1 is applied for inferencing.
    float retain = 1;
    auto AxpyWithSoftmaxAndDropOutDoMask = ge::op::AxpyWithSoftmaxAndDropOutDoMask(
                                               (ori_name + "_AxpyWithSoftmaxAndDropOutDoMask").c_str())

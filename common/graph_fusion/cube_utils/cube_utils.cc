...
void PostCubeUtils::ResolveUnitDependencies()
{
    for (auto& unit : m_idxtonodetypes_) {
        ...
        for (auto& unitname : depend_unitsname) {
            OPS_LOG_D("PostCube", "match unit name = %s index = [%u] GetName() = %s ", unit.GetName().c_str(),
                      unitmapindex_[unitname], unitname.c_str());
...
bool PostCubeUtils::JudgeCachePass(const PostCubeNodeInfo& node, std::stack<uint32_t>& index, uint32_t& ret_index) const
{
    ...
    if (index.empty() && node.GetIsHeadNode() &&
        ...
        ret_index = 0;
        OPS_LOG_D("PostCube", "JudgeCachePass is headcubenode node = %s type = %s",
    ...
    OPS_LOG_D("PostCube", "JudgeCachePass node can post_cube name = %s type = %s",
              GNodeGetName(node.GetNode()).GetString(), GNodeGetType(node.GetNode()).GetString());
...
bool PostCubeUtils::GetNodeIndex(const PostCubeNodeInfo& node, const uint32_t& index) const
{
    for (auto& nodetype : m_idxtonodetypes_[index].GetNode()) {
        ...
        if (nodetype.first == node_type) {
            OPS_LOG_D("PostCube", "GetNodeIndex node is name = %s type = %s index= %u",
                      GNodeGetName(node.GetNode()).GetString(), GNodeGetType(node.GetNode()).GetString(), index);
            return true;
        }
    }
    return false;
...
bool PostCubeUtils::PreCachePass(const PostCubePassInfo& cur_pass, const PostCubeNodeInfo& node) const
{
    ...
    if (ret != ge::GRAPH_SUCCESS) {
        OPS_LOG_D("PostCube", "PreCachePass post relu+quant node can't be post_cube name = %s type = %s",
                  GNodeGetName(node.GetNode()).GetString(), GNodeGetType(node.GetNode()).GetString());
        return false;
    }
    bool find_flag = false;
...
bool PostCubeUtils::PreMatchAcorrdingToPass(const PostCubePassInfo& cur_pass, const PostCubeNodeInfo& node) const
{
    ...
    OPS_LOG_D("PostCube", "node isn' name = %s type = %s", GNodeGetName(node.GetNode()).GetString(),
              GNodeGetType(node.GetNode()).GetString());
    return true;
}
...
bool PostCubeUtils::NeedToCutPass(PostCubePassInfo& m_pass) const
{
    ...
    if (cube_type == kConv2DTransposeD && m_pass.m_opnodes[0].GetNode()->GetOutputsSize() > 1) {
        if (GNodeGetType(m_pass.m_opnodes[1].GetNode()) == kAscendQuantAsc ||
            ...
            OPS_LOG_D(
                "PostCube",
                "Headnode is kConv2DTransposeD and second node is kAscendQuant or kAscendDequant, dont need to cut");
            m_pass.m_flag = 2;
            return false;
        }
        if (GNodeGetType(m_pass.m_opnodes[1].GetNode()) != kAscendQuantAsc &&
            ...
            OPS_LOG_D(
                "PostCube",
                "Headnode is kConv2DTransposeD but second node is not kAscendQuant or kAscendDequant, need to cut");
            m_pass.m_flag = 1;
            return true;
        }
    }
    ...
    for (auto& unit : m_idxtonodetypes_) {
        for (auto& nodes : unit.GetNode()) {
            ...
            if (nodes.first == node_type) {
                OPS_LOG_D("PostCube", "Node isinwhitelist name = %s type = %s",
                          GNodeGetName(node.GetNode()).GetString(), node_type.c_str());
                return true;
            }
        }
    }
...
ge::graphStatus PostCubeUtils::FiltrNodeStrategyForQuant(const PostCubeNodeInfo& cur_node,
                                                         const PostCubeNodeInfo& prenode) const
{
...
ge::graphStatus PostCubeUtils::CheckEltWiseShapeIsSame(const PostCubeNodeInfo& node, const ge::TensorDesc& input_desc0,
                                                       const ge::TensorDesc& input_desc1) const
{
    ...
    OPS_LOG_D("PostCube", "node %s support_post_cube_ability is 0x%lx", GNodeGetName(node.GetNode()).GetString(),
              post_cube_support_attr);
    if (PostCubeComm::CheckPostCubeAbilityAttr(node.GetNode(), PostCubeAbilityType::SupportPostEltwiseBroadcast)) {
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus PostCubeUtils::JudgeIsMatch(const PostCubeNodeInfo& node, std::stack<uint32_t>& cur_index,
                                            uint32_t& ret_index) const
{
    if (!IsInWhitelist(node)) {
        OPS_LOG_D("PostCube", "geIsMatch node isn't inwhitelist name = %s type = %s",
                  GNodeGetName(node.GetNode()).GetString(), GNodeGetType(node.GetNode()).GetString());
        return ge::GRAPH_FAILED;
    }
    if (!node.GetIsHeadNode() && PostCubeComm::GetPostCubeCubeType(node.GetNode()) != PostCubeCubeType::NotCube) {
        OPS_LOG_D("PostCube", "JudgeIsMatch node iscube but node headnode name = %s type = %s",
                  GNodeGetName(node.GetNode()).GetString(), GNodeGetType(node.GetNode()).GetString());
        return ge::GRAPH_FAILED;
    }
...
void PostCubeUtils::ProcessOutputNodesForPass(const PostCubeNodeInfo& node, PostCubeMatchParams& fp_mch_params)
{
    ...
    for (size_t idx = 0; idx < post_cube_node->GetOutputsSize(); ++idx) {
        ...
        for (const auto& out_node : outputNodesPairs) {
            OPS_LOG_D("PostCube", " GenerateMatchedPassesImpl node outnode post_cube name = %s type = %s",
                      GNodeGetName(out_node.first).GetString(), GNodeGetType(out_node.first).GetString());
...
void PostCubeUtils::GenerateMatchedPassesImpl(PostCubeNodeInfo& node, PostCubeMatchParams& fp_mch_params)
{
    ...
    const auto& post_cube_node = node.GetNode();
    OPS_LOG_D("PostCube", "GenerateMatchedPassesImpl start node name = %s type = %s",
    ...
    if (ret != ge::GRAPH_SUCCESS) {
        OPS_LOG_D("PostCube", "GenerateMatchedPassesImpl node can't be post_cube name =%s type = %s",
                  GNodeGetName(post_cube_node).GetString(), GNodeGetType(post_cube_node).GetString());
        return;
    }
    if (fp_mch_params.cur_index.size() > 1) {
        ...
        if (ret != ge::GRAPH_SUCCESS) {
            OPS_LOG_D("PostCube",
                      "GenerateMatchedPassesImpl post relu+quant node can't be post_cube name = %s type = %s",
                      GNodeGetName(post_cube_node).GetString(), GNodeGetType(post_cube_node).GetString());
            return;
        }
    }
    ...
    OPS_LOG_D("PostCube",
              "GenerateMatchedPassesImpl node can post_cube name = %s type = %s belong_unitname = %s index = %d",
              GNodeGetName(post_cube_node).GetString(), GNodeGetType(post_cube_node).GetString(),
              node.GetBelongUnitType().c_str(), tmp_cur_index);
    ...
    if (post_cube_node->GetOutputsSize() > 1) {
        if (!PostCubeComm::CheckPostCubeAbilityAttr(node.GetCubeNode(), PostCubeAbilityType::SupportMultipleOutput)) {
            OPS_LOG_D("PostCube",
                      "GenerateMatchedPassesImpl node does not support multiple output post_cube name = %s type = %s",
                      GNodeGetName(post_cube_node).GetString(), GNodeGetType(post_cube_node).GetString());
...
void PostCubeUtils::GenerateMatchedPassesFromStack(const std::stack<PostCubeNodeInfo>& cur_pass,
...
void PostCubeUtils::ExtendPassWithPostCubeNodes(std::vector<ge::GNodePtr>& pass, const ge::GNodePtr& start_node,
                                                const std::unordered_set<ge::GNodePtr>& post_cube_nodes) const
{
...
void PostCubeUtils::ChangeOrInsertPass(PostCubePassInfo& tmp_pass)
{
    ...
    for (auto& m_already_pass : m_matchpasses_) {
        ...
        for (uint32_t index = 0; index < static_cast<uint32_t>(m_already_pass.m_opnodes.size()); index++) {
            if (GNodeGetName(m_already_pass.m_opnodes[index].GetNode()) !=
                GNodeGetName(tmp_pass.m_opnodes[index].GetNode())) {
                m_find_flag = false;
...
ge::graphStatus PostCubeUtils::RelinkHeadEdges(ge::Graph& graph, PostCubeNodeInfo& head_node,
                                               PostCubeNodeInfo& post_cubeenhancenode) const
{
    ...
    if (graph.AddDataEdge(*head_node.GetNode(), 0, *post_cubeenhancenode.GetNode(), 0) != ge::GRAPH_SUCCESS) {
        REPORT_OPS_ERROR(
            "[GraphOpt][PostCubePss][RelinkHeadEdges] Fail to add edge between src node [%s] and dst node[%s].",
...
ge::graphStatus PostCubeUtils::RelinkOutputEdges(ge::Graph& graph, const PostCubePassInfo& match_pass,
                                                 PostCubeNodeInfo& post_cubeenhancenode) const
{
    ...
    for (const auto& outchild_node : outputNodesPairs) {
        ...
        if (graph.RemoveEdge(*last_tofuzednode.GetNode(), 0, *outchild_node.first, outchild_node.second) !=
            ge::GRAPH_SUCCESS) {
            return ge::GRAPH_FAILED;
        }
        if (graph.AddDataEdge(*post_cubeenhancenode.GetNode(), 0, *outchild_node.first, outchild_node.second) !=
            ge::GRAPH_SUCCESS) {
            return ge::GRAPH_FAILED;
        }
...
ge::graphStatus PostCubeUtils::RelinkAntiEltWiseEdges(ge::Graph& graph, const ge::GNodePtr& inputfather_node,
                                                      ...
                                                      PostCubeNodeInfo& post_cubeenhancenode)
{
    ...
    if (inputfather_node->GetInputsSize() != 0) {
        ...
        if (graph.AddDataEdge(*input_node.first, input_node.second, *post_cubeenhancenode.GetNode(), 1) !=
            ge::GRAPH_SUCCESS) {
            REPORT_OPS_ERROR("[GraphOpt][PostCubePss][RelinkAntiEltWiseEdges]  Fail to add  edge between src node [%s] "
                             "and dst node[%s].",
                             GNodeGetName(inputfather_node).GetString(),
...
ge::graphStatus PostCubeUtils::RelinkOtherEltWiseEdges(ge::Graph& graph, const int32_t& src_port,
                                                       const PostCubeNodeInfo& tofuzednode,
                                                       PostCubeNodeInfo& post_cubeenhancenode,
                                                       const ge::GNodePtr& inputfather_node) const
{
    // todo
    if (inputfather_node != nullptr) {
        if (graph.AddDataEdge(*inputfather_node, src_port, *post_cubeenhancenode.GetNode(), 1) != ge::GRAPH_SUCCESS) {
            REPORT_OPS_ERROR("[GraphOpt][PostCubePss][RelinkOtherEltWiseEdges] Fail to add  edge between src node [%s] "
                             "and dst node[%s].",
                             GNodeGetName(inputfather_node).GetString(),
                             GNodeGetName(post_cubeenhancenode.GetNode()).GetString());
            return ge::GRAPH_FAILED;
        ...
        if (!post_cubeenhancenode.GetNode()->GetInControlNodes().empty() &&
            ...
            if (graph.AddControlEdge(*inputfather_node, *post_cubeenhancenode.GetNode()) != ge::GRAPH_SUCCESS) {
                REPORT_OPS_ERROR("[GraphOpt][PostCubePss][RelinkOtherEltWiseEdges] Fail to add  edge between src node "
                                 "[%s] and dst node[%s].",
                                 GNodeGetName(inputfather_node).GetString(),
                                 GNodeGetName(post_cubeenhancenode.GetNode()).GetString());
                return ge::GRAPH_FAILED;
...
ge::graphStatus PostCubeUtils::RelinkEltWiseEdges(ge::Graph& graph, const PostCubePassInfo& match_pass,
                                                  PostCubeNodeInfo& post_cubeenhancenode)
{
    std::vector<PostCubeNodeInfo> fixednodeids(match_pass.m_opnodes.begin() + 1, match_pass.m_opnodes.end());
...
ge::graphStatus PostCubeUtils::RelinkEltWiseEdgesImpl(ge::Graph& graph, const PostCubePassInfo& match_pass,
                                                      PostCubeNodeInfo& tofuzednode,
                                                      PostCubeNodeInfo& post_cubeenhancenode)
{
    ...
    for (size_t idx = 0; idx < tofuzednode.GetNode()->GetInputsSize(); ++idx) {
        ...
        if (IsNodeInPass(match_pass.m_opnodes, in_node.first)) {
            OPS_LOG_D("PostCube", "inputfathernode is inpost_cubenode name = %s",
                      GNodeGetName(in_node.first).GetString());
            continue;
        }
                                             ...
                                             const PostCubePassInfo& match_pass, PostCubeNodeInfo& post_cubeenhancenode)
{
    if (RelinkHeadEdges(graph, head_node, post_cubeenhancenode) != ge::GRAPH_SUCCESS) {
        REPORT_OPS_ERROR(
            "[GraphOpt][PostCubePss][RelinkOpEdges]  Fail to add edge between head node [%s] and dst node[%s].",
            GNodeGetName(head_node.GetNode()).GetString(), GNodeGetName(post_cubeenhancenode.GetNode()).GetString());
        return ge::GRAPH_FAILED;
    }
...
bool PostCubeUtils::IsNodeInPass(const std::vector<PostCubeNodeInfo>& fixednodeids,
                                 const ge::GNodePtr& input_node) const
{
                                         ...
                                         const ge::GNodePtr& post_cubenode, std::vector<ge::GNodePtr>& new_nodes)
{
    for (uint32_t i = POST_CUBE_INPUT_2_INDEX; i < 10; i++) {
        ...
        OPS_MAKE_SHARED(funtcparam = std::make_shared<PostCubeFunctionParam>("Dummy", post_cubenode, i),
                        return ge::GRAPH_FAILED);
                                            ...
                                            ge::AscendString& eltwise_mode_asc) const
{
    ...
    if (!fixednodeids.empty() && GNodeGetType(fixednodeids[0].GetNode()) == kAscendDequantAsc &&
                                               ...
                                               ge::Graph& graph) const
{
    ...
    std::string op_name = std::string(GNodeGetName(head_node.GetNode()).GetString()) + "_pass." +
                          std::to_string(match_pass.pass_index);
                                                     ...
                                                     std::set<ge::GNodePtr>& todeletenode) const
{
    ...
    for (auto& tofuzednode : fixednodeids) {
        ...
        OPS_LOG_D("PostCube", "DeleteToFusedNodeEdge 2 name = %s type = %s",
                  GNodeGetName(tofuzednode.GetNode()).GetString(), GNodeGetType(tofuzednode.GetNode()).GetString());
                                          ...
                                          std::vector<ge::GNodePtr>& new_nodes)
{
    ...
    for (auto& pass : m_matchpasses_) {
        ...
        for (auto& node : pass.m_opnodes) {
            ...
            bool is_cube_op = std::find(kSupportPostCubeCubeTypeVec.begin(), kSupportPostCubeCubeTypeVec.end(),
                                        op_type) != kSupportPostCubeCubeTypeVec.end();
...
ge::graphStatus PostCubeUtils::InitInput2()
{
    if (isa_arch_version_ != ISAArchVersion::EN_ISA_ARCH_V350) {
        ...
        OPS_MAKE_SHARED(
            strategy21ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy21>()),
            return ge::GRAPH_FAILED);
        ...
        OPS_MAKE_SHARED(
            strategy22ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy22>()),
            return ge::GRAPH_FAILED);
        ...
        OPS_MAKE_SHARED(
            strategy23ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy23>()),
            return ge::GRAPH_FAILED);
        ...
        OPS_MAKE_SHARED(
            strategy24ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy24>()),
            return ge::GRAPH_FAILED);
        m_opmaps_.emplace(make_pair("quant_scale_0:" + kAscendDequant + "_" + kLeakyRelu, strategy24ptr));
        OPS_LOG_D("PostCube", "Add strategy24 name = %s",
                  ("quant_scale_0:" + kAscendDequant + "_" + kLeakyRelu).c_str());

        PostCubeAddInputPtr strategy25ptr = nullptr;
        OPS_MAKE_SHARED(
            strategy25ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy25>()),
            return ge::GRAPH_FAILED);
        ...
        OPS_MAKE_SHARED(pattern_dequant_lut_ptr = std::shared_ptr<PostCubeAddInputBase>(
                            std::make_shared<AddInputStrategyDequntLut>()),
                        return ge::GRAPH_FAILED);
        m_opmaps_.emplace(make_pair("quant_scale_0:" + kAscendDequant + "_" + kTanh, pattern_dequant_lut_ptr));
        OPS_LOG_D("PostCube", "Add strategy for pattern [%s]",
                  ("quant_scale_0:" + kAscendDequant + "_" + kTanh).c_str());

        m_opmaps_.emplace(make_pair("quant_scale_0:" + kAscendDequant + "_" + kElu, pattern_dequant_lut_ptr));
        OPS_LOG_D("PostCube", "Add strategy for pattern [%s]",
                  ("quant_scale_0:" + kAscendDequant + "_" + kElu).c_str());

        m_opmaps_.emplace(make_pair("quant_scale_0:" + kAscendDequant + "_" + kSigmoid, pattern_dequant_lut_ptr));
        OPS_LOG_D("PostCube", "Add strategy for pattern [%s]",
                  ("quant_scale_0:" + kAscendDequant + "_" + kSigmoid).c_str());
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus PostCubeUtils::InitInput3()
{
    ...
    OPS_MAKE_SHARED(
        strategy31ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy31>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy32ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy32>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy33ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy33>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy34ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy34>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy35ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy35>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy36ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy36>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy37ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy37>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy38ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy38>()),
        return ge::GRAPH_FAILED);
...
ge::graphStatus PostCubeUtils::InitInput4()
{
    ...
    OPS_MAKE_SHARED(
        strategy41ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy41>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy42ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy42>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy43ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy43>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy44ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy44>()),
        return ge::GRAPH_FAILED);
...
ge::graphStatus PostCubeUtils::InitInput5()
{
    ...
    if (isa_arch_version_ != ISAArchVersion::EN_ISA_ARCH_V350) {
        OPS_MAKE_SHARED(
            strategy5xptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy51>()),
            return ge::GRAPH_FAILED);
    } else {
        OPS_MAKE_SHARED(
            strategy5xptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy52>()),
            return ge::GRAPH_FAILED);
    }
...
ge::graphStatus PostCubeUtils::InitInput6()
{
    ...
    OPS_MAKE_SHARED(
        strategy61ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy61>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy62ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy62>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy63ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy63>()),
        return ge::GRAPH_FAILED);
    ...
    OPS_MAKE_SHARED(
        strategy64ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy64>()),
        return ge::GRAPH_FAILED);
...
ge::graphStatus PostCubeUtils::InitInput7()
{
    ...
    OPS_MAKE_SHARED(
        strategy71ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy71>()),
        return ge::GRAPH_FAILED);
    PostCubeAddInputPtr strategy72ptr = nullptr;
    OPS_MAKE_SHARED(
        strategy72ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy72>()),
        return ge::GRAPH_FAILED);
...
ge::graphStatus PostCubeUtils::InitInput8()
{
    ...
    OPS_MAKE_SHARED(
        strategy81ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy81>()),
        return ge::GRAPH_FAILED);
...
ge::graphStatus PostCubeUtils::InitInput9()
{
    ...
    OPS_MAKE_SHARED(
        strategy91ptr = std::shared_ptr<PostCubeAddInputBase>(std::make_shared<PostCubeAddInputStrategy91>()),
        return ge::GRAPH_FAILED);
...
ge::graphStatus PostCubeUtils::GetPostCubeNodeList(ge::GNodePtr conv_node, const ge::CustomPassContext& context)
{
    ...
    if (GNodeGetType(conv_node) == kAscendMergeAsc) {
        OPS_LOG_D("PostCube", "head node name = %s type = %s is merge not do with", GNodeGetName(conv_node).GetString(),
    ...
    OPS_LOG_D("PostCube", "convnode_name = %s type = %s", GNodeGetName(conv_node).GetString(),
              GNodeGetType(conv_node).GetString());
    ge::GNodePtr merge_node = PostCubeComm::GetMergeNodeByCube(conv_node);
    if (merge_node != nullptr) {
        conv_node = merge_node;
        OPS_LOG_D("PostCube", "head node has replaced with merge node name = %s type = %s",
                                                              ...
                                                              const std::string& first_unitname)
{
    ...
    for (size_t idx = 1; idx < match_pass.m_opnodes.size(); idx++) {
        ...
        if (tofuzednode.GetBelongUnitType() == first_unitname) {
            ...
            OPS_LOG_D("PostCube", "first_index = %d firstname = %s second_index = %d secondname = %s",
                      funtcparam->GetFirstIndex(),
            ...
            strategy_key = funtcparam->GetInputName() + ":" +
                           std::string(GNodeGetType(tofuzednode.GetNode()).GetString());
            break;
        }
    }
    ...
    for (size_t idx = 1; idx < match_pass.m_opnodes.size(); idx++) {
        ...
        for (size_t indata_idx = 0; indata_idx < tofuzednode.GetNode()->GetInputsSize(); ++indata_idx) {
            ...
            if (!GNodeGetAttr(inputfather_node, ATTR_SCALE, scale_tmp)) {
                OPS_LOG_W("PostCube", "Get scale attr of quant node[%s] failed!",
                          GNodeGetName(inputfather_node).GetString());
                return m_opmaps_["DEFAULT"];
            }
            float offset_a = 0.0f;
            ...
            return m_opmaps_[funtcparam->GetInputName() + ":" +
                             std::string(GNodeGetType(inputfather_node).GetString())];
        }
    }
...
{
    OPS_LOG_D("PostCube",
              "AddInputTwoUnitStrategy input name = %s input indexdex = %d first_unitname = %s second_unitname = %s",
              funtcparam->GetInputName().c_str(), funtcparam->GetParaIndex(), first_unitname.c_str(),
              second_unitname.c_str());
    ...
    if (has_second_unit) {
        if (has_first_unit) {
            OPS_LOG_D("PostCube", "1.first_index = %d second_index = %d firstname = %ssecondname = %s",
                      funtcparam->GetFirstIndex(), funtcparam->GetSecondIndex(),
                      GNodeGetType(match_pass.m_opnodes[funtcparam->GetFirstIndex()].GetNode()).GetString(),
                      GNodeGetType(match_pass.m_opnodes[funtcparam->GetSecondIndex()].GetNode()).GetString());
            strategy_key = funtcparam->GetInputName() + ":" +
                           std::string(

...
template <typename T>
ge::graphStatus PostCubeAddInputBase::UpdateSalarInput(ge::TensorDesc tensor_desc, T value, ge::Tensor tensornode,
                                                              ...
                                                              const PostCubeFunctionParamPtr& functpara) const
{
    ...
    if (graph.AddDataEdge(newdatanode, 0, *post_cubenode, input_index) != ge::GRAPH_SUCCESS) {
        REPORT_OPS_ERROR(
            "[GraphOpt][PostCubePss][RelkDataEdge] Fail to add edge between src node[%s] and dst node[%s].",
                                                                       ...
                                                                       ge::TensorDesc& out_tensor_desc) const
{
    ...
    if (canContinueMuls == kCanUseBrocastMuls) {
        ...
        OPS_LOG_D("PostCube", "UpdateVectorMulsOutputTensorDesc can use brocast:%s",
                  PostCubeComm::ShapeToString(broadcastshape).c_str());
                                                                     ...
                                                                     std::vector<ge::GNodePtr>& new_nodes) const
{
    ...
    std::string vector_muls_name = std::string(GNodeGetName(post_cubenode).GetString()) + "VectorMuls" +
                                   functpara->GetInputName();
    ge::GNode newopnode = ge::es::CompliantNodeBuilder(&graph)
    ...
    if (UpdateVectorMulsOutputTensorDesc(tufuzenode_inputdesc, postnode_inputdesc, out_tensor_desc) !=
        ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus PostCubeAddInputBase::CreateAndUpdateVectorMulScalarInput(ge::Graph& graph,
                                                                          const PostCubeFunctionParamPtr& functpara,
                                                                          const PostCubeNodeInfo& prefuzednode,
                                                                          const T& value,
                                                                          std::vector<ge::GNodePtr>& new_nodes) const
{
    (void)new_nodes;
    ...
    int input_index = functpara->GetParaIndex();
    const ge::GNodePtr& post_cubenode = functpara->GetPostCubeNode();
    std::string vector_mul_scalar_name = std::string(GNodeGetName(post_cubenode).GetString()) + kVectorMulScalar +
                                         functpara->GetInputName();
    ge::GNode newopnode = ge::es::CompliantNodeBuilder(&graph)
                                                       ...
                                                       const ge::GNodePtr& post_op_desc) const
{
    ...
    newopnode.UpdateInputDesc(1, postnode_inputdesc);
    newopnode.UpdateOutputDesc(0, out_tensor_desc);
    OPS_LOG_D("PostCube", "Host op[%s, %s] has been created.", GNodeGetName(newopnode).GetString(),
              GNodeGetType(newopnode).GetString());
    return newopnode;
}
                                                            ...
                                                            const ge::DataType& data_type)
{
    ...
    OPS_LOG_D("PostCube", "Post fuzed node is [%s, %s], negative_slope[%f].", GNodeGetName(post_op_desc).GetString(),
              GNodeGetType(post_op_desc).GetString(), negative_slope);
...
ge::graphStatus PostCubeAddInputBase::CreateAndRelinkCastNode(ge::Graph& graph, const ge::GNodePtr& inputnode,
                                                              const ge::GNodePtr& outputnode, const int& input_index,
                                                              std::vector<ge::GNodePtr>& new_nodes) const
{
    (void)new_nodes;
    ...
    std::string cast_op_name = std::string(GNodeGetName(outputnode).GetString()) + "INSERTCAST" +
...
ge::graphStatus PostCubeAddInputBase::CloneVectorInput(ge::Graph& graph, const PostCubeNodeInfo& tofuzednode,
                                                       const PostCubeFunctionParamPtr& functpara,
                                                       std::vector<ge::GNodePtr>& new_nodes) const
{
    const ge::GNodePtr& post_cubenode = functpara->GetPostCubeNode();
    ...
    if (functpara->GetDataType() != ge::DT_UNDEFINED && cur_tensor_desc.GetDataType() != functpara->GetDataType()) {
...
template <typename T>
ge::graphStatus PostCubeAddInputBase::CreateAndUpdateScalarInput(ge::Graph& graph,
                                                                 const PostCubeFunctionParamPtr& functpara, T value,
                                                                 const ge::DataType& data_type,
                                                                 std::vector<ge::GNodePtr>& new_nodes) const
{
    (void)new_nodes;
...
ge::graphStatus PostCubeAddInputBase::CreateScalarInputNode(ge::Graph& graph, const PostCubeFunctionParamPtr& functpara,
                                                            const ge::GNodePtr& first_node,
                                                            const int64_t input_strategy) const
{
    ...
    OPS_LOG_D("PostCube", "Offset and scale of node[%s, %s] is [%f] and [%f].", GNodeGetName(first_node).GetString(),
              GNodeGetType(first_node).GetString(), offset, scale);
    ...
    std::string set_data_op_name = std::string(GNodeGetName(post_cube_node).GetString()) + kSetQuantOffsetScale +
                                   functpara->GetInputName();
    ...
    ge::GNode set_quant_data_node = ge::es::CompliantNodeBuilder(&graph)
                                        ...
                                        .IrDefInputs(
                                            {{ATTR_OFFSET, ge::es::CompliantNodeBuilder::kEsIrInputRequired, ""},
    ...
    OPS_LOG_D("PostCube", "Output data type of first node[%s, %s] is [%s].", GNodeGetName(first_node).GetString(),
    ...
    OPS_LOG_D("PostCube", "Set input strategy[%ld] for op[%s, %s].", input_strategy, set_data_op_name.c_str(),
              kSetQuantOffsetScale.c_str());
...
ge::GNodePtr PostCubeAddInputBase::GetQuantScaleOffset(const PostCubePassInfo& match_pass, const uint32_t& index,
...
ge::GNodePtr PostCubeAddInputBase::GetNodeOutputDescByIndex(const std::vector<PostCubeNodeInfo>& opnodes,
                                                            uint32_t index, ge::TensorDesc& output_desc) const
{
                                                       ...
                                                       const ge::GNodePtr& input2_op_desc)
{
    ...
    if (post_op_desc != nullptr) {
        ...
        OPS_LOG_D("PostCube", "Set attr[%s] value[%s] for op[%s]", kAttrReluOPType, tmp_str.GetString(),
                  op_name.c_str());
    }
    OPS_LOG_D("PostCube", "Host op[%s, %s] has been created.", GNodeGetName(newopnode).GetString(),
              GNodeGetType(newopnode).GetString());
...
bool PostCubeAddInputBase::CanShapeBroadcast(const ge::Shape& shape1, const ge::Shape& shape2)
{
    OPS_LOG_D("PostCube", "Begin to check can two shape broadcast, shape1 dims = %zu %s, shape2 dims = %zu %s",
              shape1.GetDimNum(), PostCubeComm::ShapeToString(shape1).c_str(), shape2.GetDimNum(),
              PostCubeComm::ShapeToString(shape2).c_str());
...
bool PostCubeAddInputBase::CanBroadcast(const ge::TensorDesc& tensor_desc1, const ge::TensorDesc& tensor_desc2)
{
...
void PostCubeAddInputBase::GetShapeAfterExpandDims(const ge::GNode& node, const ge::TensorDesc& tensor_desc,
                                                   ge::Shape& shape)
{
    ...
    if (node.GetInputAttr(kAscendReshapeTypeMaskAsc, 1, attr_value) == ge::GRAPH_SUCCESS &&
        ...
        OPS_LOG_D("PostCube", "Begin to expand dims, reshape type[%ld], shape[%s].", reshape_type_mask,
                  PostCubeComm::ShapeToString(shape).c_str());
        ...
        transformer::ShapeTransferAccordingToFormatExt::TransferShape(
            tensor_desc.GetOriginFormat(), tensor_desc.GetFormat(), tensor_desc.GetDataType(), inner_shape, op_value);
    }
...
bool PostCubeAddInputBase::GetShapeByFormat(const ge::Format old_format, const ge::Format new_format,
                                            const ge::DataType data_type, const ge::Shape& old_shape,
                                            ge::Shape& new_shape)
{
    new_shape = old_shape;
    ...
    bool ret = transformer::ShapeTransferAccordingToFormatExt::TransferShape(
        old_format, new_format, data_type, inner_old_shape, inner_new_shape, ext_axis);
...
uint32_t PostCubeAddInputBase::IsSameShape(const ge::TensorDesc& tensor_desc1, const ge::TensorDesc& tensor_desc2)
{
    if (PostCubeComm::IsShapeEqual(tensor_desc1.GetShape(), tensor_desc2.GetShape())) {
        OPS_LOG_D("PostCube", "two tensor is same shape %s",
                  PostCubeComm::ShapeToString(tensor_desc1.GetShape()).c_str());
        return kIsSameShape;
                                                       ...
                                                       std::vector<ge::GNodePtr>& new_nodes) const
{
    ...
    std::string set_m1_dequant_name = std::string(GNodeGetName(post_cubenode).GetString()) + "SetM1Dequant" +
                                      functpara->GetInputName();
    ge::GNode newopnode = ge::es::CompliantNodeBuilder(&graph)
                                                       ...
                                                       std::vector<ge::GNodePtr>& new_nodes) const
{
    ...
    OPS_LOG_D("PostCube", "Pre fuzed node is [%s, %s]", GNodeGetName(pre_fuze_node).GetString(),
              GNodeGetType(pre_fuze_node).GetString());
    if (PostCubeComm::CheckPeerOutNode(pre_fuze_node, 1) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    const ge::GNodePtr& post_cube_node = functpara->GetPostCubeNode();
    OPS_CHECK_NOTNULL(post_cube_node);
    OPS_LOG_D("PostCube", "PostCube node is [%s, %s]", GNodeGetName(post_cube_node).GetString(),
    ...
    // create set quant scale host op
    std::string quant_scale_op_name = std::string(GNodeGetName(post_cube_node).GetString()) + kSetQuantScale +
                                      functpara->GetInputName();
    ge::GNode quant_scale_node = CreateQuantScaleOpDesc(graph, quant_scale_op_name, pre_fuze_node, post_fuze_node,
                                                        nullptr);
                                                       ...
                                                       std::vector<ge::GNodePtr>& new_nodes) const
{
    ...
    OPS_LOG_D("PostCube", "Pre fuzed node is [%s, %s]", GNodeGetName(pre_fuze_node).GetString(),
              GNodeGetType(pre_fuze_node).GetString());
    if (PostCubeComm::CheckPeerOutNode(pre_fuze_node, 1) != ge::GRAPH_SUCCESS) {
        OPS_LOG_D("PostCube", "Node[%s, %s] does not have input1.", GNodeGetName(pre_fuze_node).GetString(),
                  GNodeGetType(pre_fuze_node).GetString());
        return ge::GRAPH_FAILED;
    }
    ...
    std::string vec_mul_scalar_op_name = std::string(GNodeGetName(post_cube_node).GetString()) + kVectorMulScalar +
                                         functpara->GetInputName();
    ...
    OPS_LOG_D("PostCube", "Node[%s, %s] has been add to graph.", GNodeGetName(vec_mul_scalar_node).GetString(),
              GNodeGetType(vec_mul_scalar_node).GetString());
    ...
    OPS_MAKE_SHARED(vec_mul_scalar_node_ptr = std::make_shared<ge::GNode>(vec_mul_scalar_node),
                    return ge::GRAPH_FAILED);
    ge::GNode quant_scale_node = CreateQuantScaleOpDesc(graph, quant_scale_op_name, pre_fuze_node, post_fuze_node,
...
{
    std::string vector_muls_op_name = std::string(GNodeGetName(post_cube_node).GetString()) + "VectorMuls" +
                                      functpara->GetInputName();
    ge::GNode vector_muls_node = CreateVectorMulsOpDesc(graph, vector_muls_op_name, pre_fuze_node, post_fuze_node);
    OPS_LOG_D("PostCube", "Node[%s, %s] has been add to graph.", GNodeGetName(vector_muls_node).GetString(),
              GNodeGetType(vector_muls_node).GetString());
                                                       ...
                                                       std::vector<ge::GNodePtr>& new_nodes) const
{
    ...
    const ge::GNodePtr& post_fuze_node = match_pass.m_opnodes[functpara->GetSecondIndex()].GetNode();
    OPS_CHECK_NOTNULL(post_fuze_node);
    OPS_LOG_D("PostCube", "Post fuzed node is [%s, %s]", GNodeGetName(post_fuze_node).GetString(),
              GNodeGetType(post_fuze_node).GetString());
    if (PostCubeComm::CheckPeerOutNode(post_fuze_node, 1) != ge::GRAPH_SUCCESS) {
        OPS_LOG_D("PostCube", "Node[%s, %s] does not have input1.", GNodeGetName(post_fuze_node).GetString(),
                  GNodeGetType(post_fuze_node).GetString());
        return ge::GRAPH_FAILED;
    }
...
void PostCubeAddInputStrategy25::UpdateQuantScaleNodeShape(const ge::GNode& quant_scale_node)
{
    ...
    if (PostCubeComm::IsShapeEqual(quant_scale_input0_shape, quant_scale_input1_shape)) {
        OPS_LOG_D("PostCube", "Node[%s, %s] x1 shape is same with x2, no need to broadcast.",
    ...
    if (!PostCubeComm::IsShapeEqual(quant_scale_input0_shape, broadcast_shape)) {
        OPS_LOG_D("PostCube", "Broadcast node[%s, %s] output shape from %s to new shape %s.",
                  GNodeGetName(quant_scale_node).GetString(), GNodeGetType(quant_scale_node).GetString(),
                  PostCubeComm::ShapeToString(quant_scale_input0_shape).c_str(),
...
void PostCubeAddInputStrategy25::UpdatePostNodeShape(const ge::GNode& pre_node, const ge::GNode& post_node)
{
    ...
    ge::Shape new_shape;
    if (!GetShapeByFormat(post_tensor_desc.GetOriginFormat(), pre_tensor_desc.GetFormat(),
                          pre_tensor_desc.GetDataType(), post_shape, new_shape)) {
        OPS_LOG_D("PostCube", "Can not get new shape for node[%s, %s].", GNodeGetName(post_node).GetString(),
                  GNodeGetType(post_node).GetString());
        return;
    }
    OPS_LOG_D("PostCube", "New shape for node[%s, %s]'s second input is [%s].", GNodeGetName(post_node).GetString(),
              GNodeGetType(post_node).GetString(), PostCubeComm::ShapeToString(new_shape).c_str());
    ...
    OPS_LOG_D("PostCube", "Update shape of peer node[%s, %s]'s output[%d] to [%s].",
              GNodeGetName(peer_node).GetString(), GNodeGetType(peer_node).GetString(), peer_port,
              PostCubeComm::ShapeToString(new_shape).c_str());
                                                       ...
                                                       std::vector<ge::GNodePtr>& new_nodes) const
{
    ...
    auto second_node = GetNodeOutputDescByIndex(match_pass.m_opnodes, functpara->GetSecondIndex(),
                                                secondnode_outputdesc);
    if (second_node == nullptr) {
        return ge::GRAPH_FAILED;
    }
    ge::DataType dst_datatype = ge::DT_FLOAT16;
                                                       ...
                                                       std::vector<ge::GNodePtr>& new_nodes) const
{
    ...
    if (functpara->GetDataType() == ge::DT_INT8 || functpara->GetDataType() == ge::DT_INT4) {

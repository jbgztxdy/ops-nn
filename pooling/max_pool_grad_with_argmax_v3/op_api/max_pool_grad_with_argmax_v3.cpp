...
const inline aclTensor* MaxPoolGradWithArgmaxV3AiCore(const aclTensor* gradOutput, const aclTensor* self,
                                                      ...
                                                      const aclIntArray* stride, const aclIntArray* padding, int dtype,
                                                      const aclIntArray* dilation, bool ceilMode,
                                                      std::string& dataFormat, aclTensor* gradInput,
                                                      aclOpExecutor* executor)
{
    L0_DFX(MaxPoolGradWithArgmaxV3AiCore, gradOutput, self, indices, kernelSize, stride, padding, dtype, dilation,
           ceilMode, dataFormat, gradInput);
    ...
    ADD_TO_LAUNCHER_LIST_AICORE(MaxPoolGradWithArgmaxV3, OP_INPUT(self, gradOutput, indices), OP_OUTPUT(gradInput),
...
const aclTensor* MaxPoolGradWithArgmaxV3(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* indices,
                                         ...
                                         const aclIntArray* padding, const op::DataType dtype,
                                         const aclIntArray* dilation, bool ceilMode, std::string& dataFormat,
                                         aclOpExecutor* executor)
{
    auto gradInput = executor->AllocTensor(self->GetStorageShape(), self->GetOriginalShape(), self->GetDataType(),
                                           self->GetStorageFormat(), self->GetOriginalFormat());

    if (IsAiCoreSupport(self)) {
        // 调用算子计算
        return MaxPoolGradWithArgmaxV3AiCore(gradOutput, self, indices, kernelSize, stride, padding, dtype, dilation,
                                             ceilMode, dataFormat, gradInput, executor);
    } else {
        // 当前没有匹配的aicpu算子
        return nullptr;
    }

...
protected:
    static void SetUpTestCase() { cout << "l2_adaptive_avg_pool2d_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "l2_adaptive_avg_pool2d_test TearDown" << endl; }
};
...
TEST_F(l2_adaptive_avg_pool2d_test, adaptive_avg_pool2d_outputsize_common)
{
    ...
    for (auto dtype : dtypes) {
        auto input_tensor = TensorDesc({2, 2, 3, 3}, dtype, ACL_FORMAT_NCHW)
...
TEST_F(l2_adaptive_avg_pool2d_test, adaptive_avg_pool2d_outputsize_input_discontinuous)
{
    ...
    for (auto dtype : dtypes) {
        auto input_tensor = TensorDesc({2, 2, 4, 3}, dtype, ACL_FORMAT_NCHW, {64, 32, 8, 1}, 0, {2, 2, 4, 8})
                                .ValueRange(40, 50);
        auto int_array_desc = IntArrayDesc(output_size);
...
TEST_F(l2_adaptive_avg_pool2d_test, adaptive_avg_pool2d_outputsize_common_HW_not_same)
{
    ...
    vector<int64_t> output_size = {3, 2};
    for (auto dtype : dtypes) {
        auto input_tensor = TensorDesc({2, 2, 4, 3}, dtype, ACL_FORMAT_NCHW)
...
TEST_F(l2_adaptive_avg_pool2d_test, adaptive_avg_pool2d_dimnum_invalid)
{
    ...
    vector<int64_t> output_size = {3, 2};
    for (auto dtype : dtypes) {
        auto input_tensor = TensorDesc({2, 2, 2, 2, 3}, dtype, ACL_FORMAT_NCHW)
                                .Value(vector<float>{0.5,  1.5,  2.5,  3.5,  4.5,  5.5,  6.5,  7.5,  8.5,  9.5,
                                                     10.5, 11.5, 12.5, 13.5, 14.5, 15.5, 16.5, 17.5, 18.5, 19.5,
                                                     20.5, 21.5, 22.5, 23.5, 24.5, 25.5, 26.5, 27.5, 28.5, 29.5,
                                                     30.5, 31.5, 32.5, 33.5, 34.5, 35.5, 36.5, 37.5, 38.5, 39.5,

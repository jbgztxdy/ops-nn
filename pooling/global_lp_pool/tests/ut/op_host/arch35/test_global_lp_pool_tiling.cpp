/**
 * Copyright (c) 2026 Huawei Technologies Co., Ltd.
 * This program is free software, you can redistribute it and/or modify it under the terms and conditions of
 * CANN Open Software License Agreement Version 2.0 (the "License").
 * Please refer to the License for details. You may not use this file except in compliance with the License.
 * THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT, MERCHANTABILITY, OR FITNESS FOR A PARTICULAR PURPOSE.
 * See LICENSE in the root of the software repository for the full text of the License.
 */

#include <iostream>
#include <vector>
#include <gtest/gtest.h>
#include "ut_op_common.h"
#include "ut_op_util.h"
#include "exe_graph/runtime/storage_format.h"
#include "exe_graph/runtime/storage_shape.h"
#include "register/op_impl_registry.h"
#include "kernel_run_context_facker.h"
#include "platform/platform_infos_def.h"
#include "test_cube_util.h"

using namespace std;
using namespace ge;
using namespace gert;

// GlobalLpPool CompileInfo is an empty struct (tiling_parse is a no-op).
// We define a minimal TilingParseInfo for the test framework.
struct GlobalLpPoolTilingParseInfo {};

struct GlobalLpPoolTilingTestParam {
    string caseName;
    string opType;
    string compileInfo;

    // input of op tiling
    initializer_list<int64_t> xOriginShape;
    Format xOriginFormat;
    initializer_list<int64_t> xStorageShape;
    Format xStorageFormat;
    initializer_list<int64_t> yOriginShape;
    Format yOriginFormat;
    initializer_list<int64_t> yStorageShape;
    Format yStorageFormat;

    float p;

    // expected output
    uint64_t tilingKey = 0;
    bool result;
};

class GlobalLpPoolTilingRunTime : public testing::TestWithParam<GlobalLpPoolTilingTestParam> {
protected:
    static void SetUpTestCase() { std::cout << "GlobalLpPoolTilingRunTime SetUp" << std::endl; }
    static void TearDownTestCase() { std::cout << "GlobalLpPoolTilingRunTime TearDown" << std::endl; }
};

static string TilingData2Str(const gert::TilingData* tilingData)
{
    auto data = tilingData->GetData();
    string result;
    for (size_t i = 0; i < tilingData->GetDataSize(); i += sizeof(int32_t)) {
        result += std::to_string((reinterpret_cast<const int32_t*>(tilingData->GetData())[i / sizeof(int32_t)]));
        result += " ";
    }
    return result;
}

TEST_P(GlobalLpPoolTilingRunTime, general_cases)
{
    GlobalLpPoolTilingTestParam param = GetParam();
    cout << "run case " << param.caseName << endl;

    // platform setup
    string platform_info_str = R"({"hardware_info": {"BT_SIZE": 0, "load3d_constraints": "1",
        "Intrinsic_fix_pipe_l0c2out": false,
        "Intrinsic_data_move_l12ub": true,
        "Intrinsic_data_move_l0c2ub": true,
        "Intrinsic_data_move_out2l1_nd2nz": false,
        "UB_SIZE": 245760, "L2_SIZE": 33554432, "L1_SIZE": 524288,
        "L0A_SIZE": 65536, "L0B_SIZE": 65536, "L0C_SIZE": 131072,
        "CORE_NUM": 64}})";
    map<string, string> soc_infos;
    map<string, string> aicore_spec;
    map<string, string> intrinsics;
    map<string, string> soc_version_infos = {{"Short_SoC_version", "Ascend950"}, {"NpuArch", "3510"}};
    GetPlatFormInfos(platform_info_str.c_str(), soc_infos, aicore_spec, intrinsics);
    fe::PlatFormInfos platform_info;
    platform_info.Init();

    // get function
    ASSERT_NE(OpImplRegistry::GetInstance().GetOpImpl(param.opType.c_str()), nullptr);
    auto tilingFunc = gert::OpImplRegistry::GetInstance().GetOpImpl(param.opType.c_str())->tiling;
    auto tilingParseFunc = gert::OpImplRegistry::GetInstance().GetOpImpl(param.opType.c_str())->tiling_parse;

    GlobalLpPoolTilingParseInfo opInfo;
    auto kernelHolder = gert::KernelRunContextFaker()
                            .KernelIONum(1, 1)
                            .Inputs({const_cast<char*>(param.compileInfo.c_str())})
                            .Outputs({&opInfo})
                            .Build();
    ASSERT_EQ(tilingParseFunc(kernelHolder.GetContext<gert::KernelContext>()), GRAPH_SUCCESS);

    StorageShape xShape = {param.xOriginShape, param.xStorageShape};
    StorageShape yShape = {param.yOriginShape, param.yStorageShape};
    vector<pair<string, Ops::NN::AnyValue>> attrsPairs = {
        make_pair("p", Ops::NN::AnyValue::CreateFrom<float>(param.p)),
    };
    auto tilingData = gert::TilingData::CreateCap(2048);
    auto holder = gert::TilingContextFaker()
                      .SetOpType(param.opType)
                      .NodeIoNum(1, 1)
                      .IrInstanceNum({1})
                      .InputShapes({&xShape})
                      .OutputShapes({&yShape})
                      .NodeAttrs(attrsPairs)
                      .NodeInputTd(0, DT_FLOAT16, param.xOriginFormat, param.xStorageFormat)
                      .NodeOutputTd(0, DT_FLOAT16, param.yOriginFormat, param.yStorageFormat)
                      .CompileInfo(&opInfo)
                      .PlatformInfo(reinterpret_cast<char*>(&platform_info))
                      .TilingData(tilingData.get())
                      .Build();

    TilingContext* tilingContext = holder.GetContext<gert::TilingContext>();
    ASSERT_NE(tilingContext->GetPlatformInfo(), nullptr);
    holder.GetContext<gert::TilingContext>()->GetPlatformInfo()->SetPlatformRes("SoCInfo", soc_infos);
    holder.GetContext<gert::TilingContext>()->GetPlatformInfo()->SetPlatformRes("AICoreSpec", aicore_spec);
    holder.GetContext<gert::TilingContext>()->GetPlatformInfo()->SetCoreNumByCoreType("AICore");
    holder.GetContext<gert::TilingContext>()->GetPlatformInfo()->SetPlatformRes("AICoreintrinsicDtypeMap", intrinsics);
    tilingContext->GetPlatformInfo()->SetPlatformRes("version", soc_version_infos);

    if (param.result) {
        ASSERT_EQ(tilingFunc(tilingContext), ge::GRAPH_SUCCESS);
    } else {
        ASSERT_EQ(tilingFunc(tilingContext), ge::GRAPH_FAILED);
        return;
    }

    auto tilingKey = tilingContext->GetTilingKey();
    auto blockDim = tilingContext->GetBlockDim();
    string outputTilingData = TilingData2Str(tilingContext->GetRawTilingData());
    cout << "tilingKey=" << tilingKey << " blockDim=" << blockDim << " tilingData(len)=" << outputTilingData.length()
         << endl;

    // Verify tilingKey matches expected
    EXPECT_EQ(tilingKey, param.tilingKey);

    // Verify blockDim is valid (>0 for normal, >=0 for empty)
    if (param.tilingKey == 2) {
        // Normal path: blockDim must be > 0 and <= coreNum
        EXPECT_GT(blockDim, 0u);
        EXPECT_LE(blockDim, 64u);
    }

    // Verify tilingData is non-empty for valid cases
    if (tilingContext->GetRawTilingData()->GetDataSize() > 0) {
        EXPECT_GT(tilingContext->GetRawTilingData()->GetDataSize(), 0u);
    }
}

static GlobalLpPoolTilingTestParam general_cases_params[] = {
    // ─── Normal cases (tilingKey=2: isEmptyTensor=0, isTailR=1) ───
    {"GlobalLpPool_basic_4d_fp16_p2",
     "GlobalLpPool",
     "{}",
     {1, 64, 32, 32},
     Format::FORMAT_NCHW,
     {1, 64, 32, 32},
     Format::FORMAT_NCHW,
     {1, 64, 1, 1},
     Format::FORMAT_NCHW,
     {1, 64, 1, 1},
     Format::FORMAT_NCHW,
     2.0f,
     2,
     true},
    {"GlobalLpPool_small_spatial_4d_fp16_p1",
     "GlobalLpPool",
     "{}",
     {2, 32, 4, 4},
     Format::FORMAT_NCHW,
     {2, 32, 4, 4},
     Format::FORMAT_NCHW,
     {2, 32, 1, 1},
     Format::FORMAT_NCHW,
     {2, 32, 1, 1},
     Format::FORMAT_NCHW,
     1.0f,
     2,
     true},
    {"GlobalLpPool_large_channel_4d_fp16_p3",
     "GlobalLpPool",
     "{}",
     {1, 256, 16, 16},
     Format::FORMAT_NCHW,
     {1, 256, 16, 16},
     Format::FORMAT_NCHW,
     {1, 256, 1, 1},
     Format::FORMAT_NCHW,
     {1, 256, 1, 1},
     Format::FORMAT_NCHW,
     3.0f,
     2,
     true},
    {"GlobalLpPool_basic_5d_fp16_p2",
     "GlobalLpPool",
     "{}",
     {1, 32, 4, 8, 8},
     Format::FORMAT_ND,
     {1, 32, 4, 8, 8},
     Format::FORMAT_ND,
     {1, 32, 1, 1, 1},
     Format::FORMAT_ND,
     {1, 32, 1, 1, 1},
     Format::FORMAT_ND,
     2.0f,
     2,
     true},

    // ─── Empty tensor (tilingKey=1: isEmptyTensor=1) ───
    {"GlobalLpPool_empty_a_axis",
     "GlobalLpPool",
     "{}",
     {0, 64, 32, 32},
     Format::FORMAT_NCHW,
     {0, 64, 32, 32},
     Format::FORMAT_NCHW,
     {0, 64, 1, 1},
     Format::FORMAT_NCHW,
     {0, 64, 1, 1},
     Format::FORMAT_NCHW,
     2.0f,
     1,
     true},

};

INSTANTIATE_TEST_CASE_P(GlobalLpPool, GlobalLpPoolTilingRunTime, testing::ValuesIn(general_cases_params));

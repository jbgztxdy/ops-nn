...
template <typename T>
__simd_callee__ inline void CustomDuplicate(__local_mem__ T* dstAddr, uint32_t calNum, uint16_t loop)
{
...
template <typename T>
__simd_callee__ inline void CustomCopy(const __local_mem__ T* dstAddr, const __local_mem__ T* srcAddr,
...
__simd_callee__ inline void CustomCopyByScatterSingleRow(const __local_mem__ T* dstAddr, const __local_mem__ T* srcAddr,
...
template <typename T, typename U>
__simd_callee__ inline void CustomCopyByScatterMultiRows(const __local_mem__ T* dstAddr, const __local_mem__ T* srcAddr,
...
__simd_callee__ inline void AvgPoolB32Impl(RegDstT& res, __local_mem__ T* srcAddr, MicroAPI::RegTensor<U>& index, uint16_t kH,
...
template <typename T, typename U, bool NO_DIV, typename RegDstT>
__simd_callee__ inline void AvgPoolB16Impl(RegDstT& res, __local_mem__ T* srcAddr, MicroAPI::RegTensor<U>& index, uint16_t kH,
                                   uint16_t kW, U rowStrideInub, float32_t divisor, MicroAPI::MaskReg& pMask, uint16_t channels = 1)
{
    MicroAPI::RegTensor<T> vd1;
...
__simd_callee__ inline void AvgPoolSingleChannelB32(__local_mem__ M* dstLocalAddr, __local_mem__ M* srcLocalAddr,
...
template <typename M, typename U>
__simd_callee__ inline void AvgPoolSingleChannelB16(__local_mem__ M* dstLocalAddr, __local_mem__ M* srcLocalAddr,
                                            uint16_t kH, uint16_t kW, U rowStrideInUb, 
                                            uint16_t alignChannels, uint16_t repeatElms, float32_t divisor)
{
    MicroAPI::RegTensor<M> res;
...
template <typename M, typename U>
__simd_callee__ inline void AvgPoolSingleChannel(__local_mem__ M* dstLocalAddr, __local_mem__ M* srcLocalAddr,
                                           uint16_t kH, uint16_t kW, U rowStrideInUb, 
                                           uint16_t alignChannels, uint16_t repeatElms, float32_t divisor)
{
...
template <typename T, typename U, bool NO_DIV, typename RegDstT>
__simd_callee__ inline void AvgPoolImpl(RegDstT& res, __local_mem__ T* srcAddr, MicroAPI::RegTensor<U>& index, uint16_t kH,
                                   uint16_t kW, U rowStrideInub, float32_t divisor, MicroAPI::MaskReg& pMask, uint16_t channels = 1) {
...
__simd_callee__ inline void AvgPoolSplitW(__local_mem__ Z* dstLocalAddr, __local_mem__ T* srcAddr,
...
template <typename T, typename U, typename Z, bool NO_DIV=false>
__simd_callee__ inline void AvgPoolSplitH(__local_mem__ Z* dstLocalAddr, __local_mem__ T* srcAddr,
...
template <typename T, typename U, typename Z, bool NO_DIV=false>
__simd_callee__ inline void AvgPoolSplitBatch(__local_mem__ Z* dstLocalAddr, __local_mem__ T* srcAddr,
                                         MicroAPI::RegTensor<U>& index, uint16_t kH, uint16_t kW, uint16_t loopN,
...
__simd_vf__ inline void GenScatterIndexVf(__ubuf__ U* dstAddr, uint32_t wIn, uint32_t wInDst)
{
    using regType = typename VciTypeGet<U>::type;
    MicroAPI::RegTensor<U> v0;
    MicroAPI::RegTensor<U> v1;
...
{
    GenScatterIndexVf<U, SingleRow>((__ubuf__ U*)indexLocal.GetPhyAddr(), wIn, wInDst);
}

template <typename U, bool SingleRow>
__simd_vf__ inline void NHWCGenScatterIndexVf(__ubuf__ U* dstAddr, uint32_t wIn, uint32_t wInDstElms, uint32_t channels)
{
    using regType = typename VciTypeGet<U>::type;
    MicroAPI::RegTensor<U> v0;
    MicroAPI::RegTensor<U> v1;
...
{
    NHWCGenScatterIndexVf<U, SingleRow>((__ubuf__ U*)indexLocal.GetPhyAddr(), wIn, wInDstElms, channels);
}

template <typename U>
__simd_vf__ inline void NHWCGenGatherIndexSingleRowVf(__ubuf__ U* dstAddr, uint32_t wStride, uint32_t channels)
{
    using regType = typename VciTypeGet<U>::type;
...
{
    NHWCGenGatherIndexSingleRowVf<U>((__ubuf__ U*)indexLocal.GetPhyAddr(), wStride, channels);
}

template <typename U>
__simd_vf__ inline void NHWCGenGatherIndexMultiRowVf(__ubuf__ U* dstAddr, uint32_t wFactorOut, uint32_t wInElms,
...
{
    NHWCGenGatherIndexMultiRowVf<U>((__ubuf__ U*)indexLocal.GetPhyAddr(), wFactorOut, wInElms, hStride, wStride, channels);
}

template <typename U>
__simd_vf__ inline void NHWCGenGatherIndexMultiBatchVf(__ubuf__ U* dstAddr, uint32_t hFactorOut, uint32_t wFactorOut,
                                                       uint32_t hIn, uint32_t wInElms, uint32_t hStride, uint32_t wStride,
                                                       uint32_t channels, U batchElemtsIn, U batchElemtsOut)
{
    using regType = typename VciTypeGet<U>::type;
    MicroAPI::RegTensor<U> v0;
    MicroAPI::RegTensor<U> v1;
                                                ...
                                                uint32_t hStride, uint32_t wStride, uint32_t channels, LocalTensor<U>& indexLocal)
{
    ...
    NHWCGenGatherIndexMultiBatchVf<U>((__ubuf__ U*)indexLocal.GetPhyAddr(), hFactorOut, wFactorOut, hIn, wInElms,
        hStride, wStride, channels, batchElemtsIn, batchElemtsOut);
}

__simd_callee__ inline void FastDivImpl(MicroAPI::RegTensor<uint32_t>& res, MicroAPI::RegTensor<uint32_t>& src, MicroAPI::RegTensor<uint32_t>& magic, int16_t shift, MicroAPI::MaskReg &mask) 
{
...
__simd_callee__ inline void CalcWindowSize(MicroAPI::RegTensor<float>& res, RegT& src, T kD, T sD, T negFrontPad, T dIn, T dInAndBackendPad, MicroAPI::MaskReg &mask) 
{
    RegT tmp1, tmp2;
...
__simd_vf__ inline void ComputeDivisorImplB32Vf(__ubuf__ float* dstAddr, int32_t outW, int32_t outH,
    uint32_t m0, uint32_t m1, uint32_t shift0, uint32_t shift1, uint16_t loopNum, int32_t oneRegLength,
    int32_t start, int32_t totalNum, int32_t oneBatchOut,
    int32_t kH, int32_t kW, int32_t sH, int32_t sW, int32_t negTopPad, int32_t hInAndBottomPad,
    int32_t negLeftPad, int32_t wInAndRightPad, int32_t hIn, int32_t wIn)
{
    ...
    for (uint16_t i = 0; i < loopNum; i++)
    {
        ...
        FastDivImpl((MicroAPI::RegTensor<uint32_t> &)vd1, (MicroAPI::RegTensor<uint32_t> &)v0, magic0, shift0, p0);
...
__aicore__ inline void ComputeDivisorImplB32(__local_mem__ float* divAddr, const CalcDivisorParam &param, int32_t start, int32_t total)   {
    ...
    ComputeDivisorImplB32Vf<countIncludePad, PAD_MULTI_BATCH>(dstAddr, outW, outH,
        m0, m1, shift0, shift1, loopNum, oneRegLength, start, totalNum, oneBatchOut,
        kH, kW, sH, sW, negTopPad, hInAndBottomPad, negLeftPad, wInAndRightPad, hIn, wIn);
}


template <bool countIncludePad, bool PAD_MULTI_BATCH>
__simd_vf__ inline void ComputeDivisorImplB64MainVf(__ubuf__ float* dstAddr, int64_t outW, int64_t outH,
    uint16_t loopNum, int64_t oneRegLength, int32_t start, int32_t oneBatchOut,
    int64_t kH, int64_t kW, int64_t sH, int64_t sW, int64_t negTopPad, int64_t hInAndBottomPad,
    int64_t negLeftPad, int64_t wInAndRightPad, int64_t hIn, int64_t wIn)
{
...
template <bool countIncludePad, bool PAD_MULTI_BATCH>
__simd_vf__ inline void ComputeDivisorImplB64PadSmallVf(__ubuf__ float* writeAddr, __ubuf__ float* startAddr,
    uint16_t loopNum, uint32_t repeatElm)
{
...
template <bool countIncludePad, bool PAD_MULTI_BATCH>
__simd_vf__ inline void ComputeDivisorImplB64PadLargeVf(__ubuf__ float* writeAddr, __ubuf__ float* startAddr,
    uint16_t loopNum, uint16_t loopInner, uint16_t tailInner, uint32_t repeatElm)
{
    auto curDstAddr = writeAddr;
...
__aicore__ inline void ComputeDivisorImplB64(__local_mem__ float* divAddr, const CalcDivisorParam &param, int32_t start, int32_t total)   {
    ...
    ComputeDivisorImplB64MainVf<countIncludePad, PAD_MULTI_BATCH>(dstAddr, outW, outH,
        loopNum, oneRegLength, start, oneBatchOut, kH, kW, sH, sW, negTopPad,
        hInAndBottomPad, negLeftPad, wInAndRightPad, hIn, wIn);
    ...
    if  (PAD_MULTI_BATCH && (oneBatchOut < total)) {
        ...
        if (oneBatchOut < oneRegLength) {
            ...
            ComputeDivisorImplB64PadSmallVf<countIncludePad, PAD_MULTI_BATCH>(writeAddr, startAddr, loopNum, repeatElm);
        } else {
            ...
            ComputeDivisorImplB64PadLargeVf<countIncludePad, PAD_MULTI_BATCH>(writeAddr, startAddr, loopNum, loopInner, tailInner, repeatElm);            
        }
    }
}

template <typename T>
__simd_vf__ inline void AvgPoolDivNormChannelVf(__ubuf__ T* dstAddr, __ubuf__ float32_t* srcAddr,
    __ubuf__ float32_t* divAddr, uint16_t loopNum, uint16_t tailNum, uint32_t oneRegNum, uint32_t oneRegChannel, uint32_t channel)
{
...
__aicore__ inline void AvgPoolDivNormChannel(__local_mem__ T* dstAddr, __local_mem__ float32_t* srcAddr, __local_mem__ float32_t* divAddr, uint32_t num, uint32_t channel=1)
{
    ...
    AvgPoolDivNormChannelVf<T>((__ubuf__ T*)dstAddr, (__ubuf__ float32_t*)srcAddr,
        (__ubuf__ float32_t*)divAddr, loopNum, tailNum, oneRegNum, oneRegChannel, channel);
}

template <typename T, bool CHANNEL_BROADACAST=false>
__simd_vf__ inline void AvgPoolDivNormVf(__ubuf__ T* dstAddr, __ubuf__ float32_t* srcAddr,
    __ubuf__ float32_t* divAddr, uint16_t loopNum, uint16_t oneRegNum, uint32_t num)
{
    MicroAPI::RegTensor<float32_t> src;
    MicroAPI::RegTensor<float32_t> div;
    MicroAPI::RegTensor<float32_t> tmp;
...
template <typename T, bool CHANNEL_BROADACAST>
__aicore__ inline void AvgPoolDivNorm(__local_mem__ T* dstAddr, __local_mem__ float32_t* srcAddr, __local_mem__ float32_t* divAddr, uint32_t num, uint32_t channel=1)
{
    ...
    AvgPoolDivNormVf<T, CHANNEL_BROADACAST>((__ubuf__ T*)dstAddr, (__ubuf__ float32_t*)srcAddr,
        (__ubuf__ float32_t*)divAddr, loopNum, oneRegNum, num);
}

template <typename T, bool CHANNEL_BROADACAST=false>
__simd_vf__ inline void AvgPoolDivBatchV1Vf(__ubuf__ T* dstAddr, __ubuf__ float32_t* srcAddr,
    __ubuf__ float32_t* divAddr, uint16_t oneRegNum, uint16_t loopNum, uint16_t tailNum,
    uint16_t loopBatch, uint32_t oneRegChannel, uint32_t channel, uint32_t batchElement)
{
    MicroAPI::RegTensor<float32_t> src;
    MicroAPI::RegTensor<float32_t> div;
    MicroAPI::RegTensor<float32_t> tmp;
...
__aicore__ inline void AvgPoolDivBatchV1(__local_mem__ T* dstAddr, __local_mem__ float32_t* srcAddr, __local_mem__ float32_t* divAddr, uint32_t batchNum, uint32_t batchElement, uint32_t channel=1)
{
    ...
    AvgPoolDivBatchV1Vf<T, CHANNEL_BROADACAST>((__ubuf__ T*)dstAddr, (__ubuf__ float32_t*)srcAddr,
        (__ubuf__ float32_t*)divAddr, oneRegNum, loopNum, tailNum, loopBatch, oneRegChannel, channel, batchElement);
}

template <typename T, bool CHANNEL_BROADACAST=false>
__simd_vf__ inline void AvgPoolDivBatchV2Vf(__ubuf__ T* dstAddr, __ubuf__ float32_t* srcAddr,
    __ubuf__ float32_t* divAddr, uint16_t loopNum, uint16_t onceRepeatNum, uint16_t tailRepeatNum,
    uint32_t channel, uint32_t batchElement)
{
    MicroAPI::RegTensor<float32_t> src;
    MicroAPI::RegTensor<float32_t> div;
    MicroAPI::RegTensor<float32_t> tmp;
...
__aicore__ inline void AvgPoolDivBatchV2(__local_mem__ T* dstAddr, __local_mem__ float32_t* srcAddr, __local_mem__ float32_t* divAddr, uint32_t batchNum, uint32_t batchElement, uint32_t channel=1)
{
    ...
    AvgPoolDivBatchV2Vf<T, CHANNEL_BROADACAST>((__ubuf__ T*)dstAddr, (__ubuf__ float32_t*)srcAddr,
        (__ubuf__ float32_t*)divAddr, loopNum, onceRepeatNum, tailRepeatNum, channel, batchElement);
}
...
template <typename T>
__simd_callee__ inline void DuplicateReg(MicroAPI::RegTensor<T>& reg, MicroAPI::MaskReg mask)
{
    MicroAPI::Duplicate(reg, 0, mask);
}

template <typename T>
__simd_callee__ inline void DuplicateValue(const __local_mem__ void* dstAddr, uint32_t calNum, uint32_t offset)
{
    uint32_t num = calNum;
    MicroAPI::RegTensor<T> v0;
...
template <typename T>
__simd_callee__ inline void MergeAvgParaRes(MicroAPI::RegTensor<T>& res, __local_mem__ T* dstLocalAddr, int32_t num)
{
...
__simd_callee__ inline void LoadOneElement(const __local_mem__ void* input, RegDstT& dst, uint32_t offset)
{
    MicroAPI::UnalignReg u0;
...
template <typename T, typename RegDstT>
__simd_callee__ inline void MergeSumRes(RegDstT& res, const __local_mem__ T* dstLocalAddr, int32_t offset)
{
    // merge cur result with pre result
...
__simd_callee__ inline void SumWithGather(MicroAPI::RegTensor<T>& res, __local_mem__ T* srcAddr,
...
template <typename U>
__simd_vf__ inline void GenGatherIndexMultiBatchVf(__ubuf__ U* dstAddr, uint32_t hFactorOut, uint32_t wFactorOut,
                                                   uint32_t batchElemtsIn, uint32_t wIn, uint32_t hStride, uint32_t wStride,
                                                   U batchElemtsOut)
{
    using regType = typename VciTypeGet<U>::type;
    MicroAPI::RegTensor<U> v0;
                                                ...
                                                uint32_t hStride, uint32_t wStride, LocalTensor<U>& indexLocal)
{
    ...
    GenGatherIndexMultiBatchVf<U>((__ubuf__ U*)indexLocal.GetPhyAddr(), hFactorOut, wFactorOut, batchElemtsIn, wIn, hStride,
        wStride, batchElemtsOut);
}

template <typename U>
__simd_vf__ inline void GenGatherIndexMultiRowVf(__ubuf__ U* dstAddr, uint32_t wFactorOut, uint32_t wIn,
...
{
    GenGatherIndexMultiRowVf<U>((__ubuf__ U*)indexLocal.GetPhyAddr(), wFactorOut, wIn, hStride, wStride);
}

template <typename U>
__simd_vf__ inline void GenGatherIndexSingleRowVf(__ubuf__ U* dstAddr, uint32_t wStride)
{
    using regType = typename VciTypeGet<U>::type;
    MicroAPI::RegTensor<U> v0;
    MicroAPI::RegTensor<U> v1;
...
{
    GenGatherIndexSingleRowVf<U>((__ubuf__ U*)indexLocal.GetPhyAddr(), wStride);
}

template <typename U>
__simd_vf__ inline void GenGatherIndexSingleKernelVf(__ubuf__ U* dstAddr, uint32_t wIn, uint32_t kW, uint32_t kH)
{
    using regType = typename VciTypeGet<U>::type;
...
__aicore__ inline void GenGatherIndexSingleKernel(uint32_t wIn, uint32_t kW, uint32_t kH,
                                              LocalTensor<U>& indexLocal)
{
    GenGatherIndexSingleKernelVf<U>((__ubuf__ U*)indexLocal.GetPhyAddr(), wIn, kW, kH);
}

template <uint16_t REG_NUM, uint16_t IDX, typename U>
__simd_callee__ inline void LoadIndex(__local_mem__ U* indexAddr, MicroAPI::RegTensor<U>& index)
{
...
__simd_callee__ inline void DivCompute(MicroAPI::RegTensor<T>& res, MicroAPI::RegTensor<Z>& sum, float32_t divisor)
{
...
__simd_callee__ inline void ReduceSumWithGatherOne(MicroAPI::RegTensor<Z>& res, __local_mem__ T* srcAddr,
    MicroAPI::RegTensor<U>& index, MicroAPI::MaskReg& mask)
{
    MicroAPI::RegTensor<T> vd1;
    if constexpr (sizeof(T) == TWO) {
...
template <typename T, typename U, typename Z>
__simd_callee__ inline void ReduceSumWithGather(MicroAPI::RegTensor<Z>& res, __local_mem__ T* srcAddr,
    MicroAPI::RegTensor<U>& index, MicroAPI::MaskReg& mask)
{
    MicroAPI::RegTensor<T> vd1;
    if constexpr (sizeof(T) == TWO) {
...
__simd_callee__ inline void ComputeReduceSumWithGather(MicroAPI::RegTensor<Z>&res, __local_mem__ T* srcAddr, MicroAPI::RegTensor<U>& index,
...
__simd_vf__ inline void AvgPoolSingleKernelCommonVf(__ubuf__ T* dstLocalAddr, __ubuf__ T* xLocalAddr,
                                     ...
                                     uint16_t tailLoopElements, float32_t divisor)
{
    ...
    AvgPoolSingleKernelCommonVf<T, U, REG_NUM>((__ubuf__ T*)dstLocalAddr, (__ubuf__ T*)xLocalAddr,
        (__ubuf__ U*)indexAddr, loopN, loopH, loopW, oneChannelElements,
        oneLoopStrideH, oneLoopStrideW, tailLoopElements, divisor);
}

template <typename T, typename U>
__simd_vf__ inline void AvgPoolSingleKernelDefaultVf(__ubuf__ T* dstLocalAddr, __ubuf__ T* xLocalAddr,
    __ubuf__ U* indexAddr, uint16_t loopN, uint16_t loopH, uint16_t loopW,
    U oneChannelElements, U oneLoopStrideH, U oneLoopStrideW,
...
__aicore__ inline void AvgPoolSingleKernelDefault(__local_mem__ T* dstLocalAddr, __local_mem__ T* xLocalAddr,
                                     ...
                                     float32_t divisor, uint16_t regNum, uint16_t kernelSize)
{
    AvgPoolSingleKernelDefaultVf<T, U>((__ubuf__ T*)dstLocalAddr, (__ubuf__ T*)xLocalAddr,
        (__ubuf__ U*)indexAddr, loopN, loopH, loopW, oneChannelElements,
        oneLoopStrideH, oneLoopStrideW, divisor, regNum, kernelSize);
}

...
bool MaxPool3DWithArgmaxV2TilingSIMT::IsCapable() { return true; }
...
ge::graphStatus MaxPool3DWithArgmaxV2TilingSIMT::GetShapeAttrsInfo()
{
    ...
    inputData.inputShape = array<uint64_t, NCDHW_DIMS>{
    ...
    inputData.outShape = array<uint64_t, NCDHW_DIMS>{
    ...
    inputData.dilation = array<uint64_t, DHW_DIMS>{dilationD, dilationH, dilationW};
...
ge::graphStatus MaxPool3DWithArgmaxV2TilingSIMT::DoOpTiling()
{
    ...
    outputDataCount = tilingData_->nDim * tilingData_->cDim * tilingData_->dOutDim * tilingData_->hOutDim *
                      tilingData_->wOutDim;
    int64_t threads = std::min(outputDataCount, DEFAULT_THREAD_NUM);
    isThreadNum512 = tilingData_->kSizeD * tilingData_->kSizeH * tilingData_->kSizeW >= KERNEL_SIZE_THRESHOLD &&
                     outputDataCount > MAX_THREAD_NUM * coreNum;
...
uint64_t MaxPool3DWithArgmaxV2TilingSIMT::GetTilingKey() const
{
    ...
    int idx = (isNcdhw ? 0 : 1) * ONE_TEMPLATE_NUM + (isInt32 ? 0 : 1) * DATA_TYPE_NUM + (isThreadNum256 ? 0 : 1);
...
ge::graphStatus MaxPool3DWithArgmaxV2TilingSIMT::PostTiling() { return ge::GRAPH_SUCCESS; }

...
static bool CheckNotNullPtr(const aclTensor* self, const aclIntArray* kernelSize, const aclIntArray* stride,
                            const aclIntArray* padding, const aclIntArray* dilation, aclTensor* out, aclTensor* indices)
{
...
static bool CheckDtypeValid(const aclTensor* self, const aclTensor* out, const aclTensor* indices, bool isMask,
                            bool isMask2ND)
{
    // 检查self的数据类型是否在支持列表内
...
static bool CheckFormat(const aclTensor* self, const aclTensor* out)
{
    ...
    OP_CHECK(!op::IsPrivateFormat(self->GetStorageFormat()),
...
static inline int64_t PoolingOutShape(int64_t inputSize, int64_t kernelSize, int64_t pad_l, int64_t pad_r,
...
static bool CheckOutAndIndicesShape(const aclTensor* self, int64_t kH, int64_t kW, int64_t padH, int64_t padW,
                                    int64_t dH, int64_t dW, int64_t dilationH, int64_t dilationW, bool ceilMode,
                                    bool isMask, bool isMask2ND, const aclTensor* out, const aclTensor* indices)
{
...
static bool CheckShape(const aclTensor* self, const aclIntArray* kernelSize, const aclIntArray* stride,
                       const aclIntArray* padding, const aclIntArray* dilation, bool ceilMode, aclTensor* out,
                       aclTensor* indices, bool isMask, bool isMask2ND)
{
    OP_CHECK(
    ...
    if (padH < 0 || padW < 0) {
        OP_LOGW("Negative pad inputs are not supported, and the output precision is not guaranteed when the pad is "
                "negative.");
    }
    ...
    // 检查out和indices的shape
    const bool ret = CheckOutAndIndicesShape(self, kH, kW, padH, padW, dH, dW, dilationH, dilationW, ceilMode, isMask,
                                             isMask2ND, out, indices);
    CHECK_RET(ret, false);
                               ...
                               aclTensor* indices, bool isMask, bool isMask2ND)
{
    ...
    // 检查数据维度是否合法
    CHECK_RET(CheckShape(self, kernelSize, stride, padding, dilation, ceilMode, out, indices, isMask, isMask2ND),
...
static const aclTensor* SelfPreProcess(const aclTensor* self, bool isMask, bool isMask2ND, aclOpExecutor* executor)
{
    ...
    if (isMask) {
        ...
        OP_CHECK(selfTrans != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The TransDataSpecial return nullptr."),
                 return nullptr);
        return selfTrans;
    } else {
        return castSelf;
...
static TupleArrayOut OutputProcess(TupleArrayOut output, const aclTensor* self, const aclTensor* indicesInput,
                                   bool isMask, aclOpExecutor* executor)
{
    ...
    if (isMask) {
        ...
        OP_CHECK(outTrans != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The out TransDataSpecial return nullptr."),
                 return nullptrArray);

        castOut = l0op::Cast(outTrans, self->GetDataType(), executor);
        OP_CHECK(castOut != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "The out Cast return nullptr."),
                 return nullptrArray);
    } else {
...
static aclnnStatus ExecMaxPool2dWithIndicesGetWorkspaceSize(const aclTensor* self, const aclIntArray* kernelSize,
                                                            ...
                                                            uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    auto maxPoolResult = l0op::MaxPoolWithArgmaxV1(selfPreResult, kernelSize, stride, padding, dilation, ceilMode,
                                                   uniqueExecutor.get());
    ...
    uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    auto selfUnsqueezed = isSelf3D ? View3Das5D(selfContiguous, uniqueExecutor.get()) :
                                     View4Das5D(selfContiguous, uniqueExecutor.get());
...
static aclnnStatus ExecMaxPool3dWithIndicesGetWorkspaceSize(const aclTensor* self, const aclIntArray* kernelSize,
                                                            const aclIntArray* stride, const aclIntArray* padding,
                                                            const aclIntArray* dilation, bool ceilMode, aclTensor* out,
...
static aclnnStatus ExecMaxPool2dWithIndicesGetWorkspaceSizeV3(const aclTensor* self, const aclIntArray* kernelSize,
                                                              const aclIntArray* stride, const aclIntArray* padding,
                                                              const aclIntArray* dilation, const bool ceilMode,
                                                              ...
                                                              uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    // 返回tuple，包含out和indices
    auto maxPoolResult = l0op::MaxPoolWithArgmaxV3(selfUnsqueezed, kernelSize, stride, padding, indices->GetDataType(),
    ...
    auto indicesSqueezed = isSelf3D ? View4Das3D(maxPoolIndicesResult, dstFormat, uniqueExecutor.get()) :
                                      maxPoolIndicesResult;
...
aclnnStatus aclnnMaxPool2dWithMaskGetWorkspaceSize(const aclTensor* self, const aclIntArray* kernelSize,
                                                   const aclIntArray* stride, const aclIntArray* padding,
                                                   const aclIntArray* dilation, bool ceilMode, aclTensor* out,
                                                   aclTensor* indices, uint64_t* workspaceSize,
                                                   aclOpExecutor** executor)
{
    ...
    const int64_t kH = kernelRef[0];
    const int64_t kW = (kernelRef.Size() == 1) ? kH : kernelRef[1];
    L2_DFX_PHASE_1(aclnnMaxPool2dWithMask, DFX_IN(self, kernelSize, stride, padding, dilation, ceilMode),
                   DFX_OUT(out, indices));
    if (!(kH == 1 && kW == 1) && (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910B ||
                                  GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910_93)) {
        return ExecMaxPool2dWithIndicesReshape3DGetWorkspaceSize(self, kernelSize, stride, padding, dilation, ceilMode,
                                                                 out, indices, false, workspaceSize, executor);
    } else {
        return ExecMaxPool2dWithIndicesGetWorkspaceSize(self, kernelSize, stride, padding, dilation, ceilMode, out,
                                                        indices, true, false, workspaceSize, executor);
    }
}
...
aclnnStatus aclnnMaxPool2dWithIndicesGetWorkspaceSize(const aclTensor* self, const aclIntArray* kernelSize,
                                                      const aclIntArray* stride, const aclIntArray* padding,
                                                      const aclIntArray* dilation, bool ceilMode, aclTensor* out,
                                                      aclTensor* indices, uint64_t* workspaceSize,
                                                      aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnMaxPool2dWithIndices, DFX_IN(self, kernelSize, stride, padding, dilation, ceilMode),
                   DFX_OUT(out, indices));
    if (Ops::NN::AclnnUtil::IsRegbase()) {
        return ExecMaxPool2dWithIndicesGetWorkspaceSizeV3(self, kernelSize, stride, padding, dilation, ceilMode, out,
                                                          indices, workspaceSize, executor);
    } else {
        return ExecMaxPool3dWithIndicesGetWorkspaceSize(self, kernelSize, stride, padding, dilation, ceilMode, out,
                                                        indices, workspaceSize, executor);
    }
}

aclnnStatus aclnnMaxPool2dWithIndices(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                      aclrtStream stream)
{

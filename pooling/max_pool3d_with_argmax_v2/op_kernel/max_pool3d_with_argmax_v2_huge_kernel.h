...
private:
    __aicore__ inline void DoMaxPool(uint32_t partNC, uint32_t partD, uint32_t partH, uint32_t partW,
                                     uint32_t inputOffset, uint32_t wsOffset, uint32_t padDL, uint32_t padDR,
    ...
    __aicore__ inline void PrepareScalars()
    {
        ...
        this->transDataParams.repeatTimes = this->RoundUpBlock(this->partD * this->partH * this->roundW,
                                                               MIN_TRANSPOSE_ROWS) /
                                            NCHW_CONV_ADDR_LIST_SIZE;
        this->transDataParams.dstRepStride = (this->transDataParams.repeatTimes == 1) ? 0 : NCHW_CONV_ADDR_LIST_SIZE;
        this->transDataParams.srcRepStride = (this->transDataParams.repeatTimes == 1) ?
                                                 0 :
                                                 NCHW_CONV_ADDR_LIST_SIZE / this->blockLength;
    }
    ...
    __aicore__ inline void CreateKernelIndexes(uint32_t indStart)
    {
        ...
        Brcb(this->kernelIndexes, this->transDataTensorUb1,
             this->RoundUpBlock(this->partW, BLOCKS_IN_REP) / BLOCKS_IN_REP, {1, BLOCKS_IN_REP});
        PipeBarrier<PIPE_V>();

        for (uint32_t i = 1; i < this->partH; i++) {
            Adds(this->kernelIndexes[this->partW * this->blockLength * i], this->kernelIndexes, (T)(1.f * i * this->kW),
                 this->partW * this->blockLength);
        }
        PipeBarrier<PIPE_V>();

        for (uint32_t i = 1; i < this->partD; i++) {
            Adds(this->kernelIndexes[this->partH * this->partW * this->blockLength * i], this->kernelIndexes,
    ...
    __aicore__ inline void CastBF16(LocalTensor<V> src, LocalTensor<float> dst, uint32_t partNC)
    {
        if (sizeof(V) != sizeof(float)) {
            ...
            for (uint32_t i = 0; i < wholeRepeatTimes; i++) {
                Copy(dst[i * MAX_REPEAT_TIMES * this->mask],
                     this->transDataTensorUb1[i * MAX_REPEAT_TIMES * this->mask], this->mask, MAX_REPEAT_TIMES,
                     {1, 1, BLOCKS_IN_REP, BLOCKS_IN_REP});
            }
            Copy(dst[wholeOffset], this->transDataTensorUb1[wholeOffset], this->mask, tailRepeatTimes,
                 {1, 1, BLOCKS_IN_REP, BLOCKS_IN_REP});
            Copy(dst[tailOffset], this->transDataTensorUb1[tailOffset], tailElemsPerRepeat, 1,
                 {1, 1, BLOCKS_IN_REP, BLOCKS_IN_REP});
            PipeBarrier<PIPE_V>();
        }
    }
    ...
    __aicore__ void Img2ColC0(const uint32_t partNC, uint32_t partD, uint32_t partH, uint32_t partW,
    ...
    __aicore__ inline void CopyIn(GlobalTensor<S> srcGm, const uint32_t& partNC, uint32_t partD, uint32_t partH,
                                  uint32_t partW, const uint32_t padDL, const uint32_t padDR, const uint32_t padHL,
                                  const uint32_t padHR, const uint32_t padWL, const uint32_t padWR);
    __aicore__ inline void CopyOutVals(const uint32_t& dstOffset, const uint32_t& partNC);

    __aicore__ void Padding(LocalTensor<T>& srcUb, LocalTensor<T>& dstUb, const uint32_t& partD, const uint32_t& partH,
                            const uint32_t& partW, const uint32_t& partDWoPad, const uint32_t& partHWoPad,
                            const uint32_t& partWWoPad, const uint32_t pDL, const uint32_t pDR, const uint32_t pHL,
                            const uint32_t pHR, const uint32_t pWL, const uint32_t pWR);
                ...
                for (w = 0; w < this->roundWidthInput; w += offW, wOut += 1) {
                    ...
                    for (uint32_t dKer = 0; dKer < dCycleMax; dKer++) {
                        ...
                        for (uint32_t hKer = 0; (hKer < hCycleMax) && (partDWoPad > 0); hKer++) {
                            ...
                            uint32_t curPartH = hKer != (hCycleMax - 1) ? this->partH :
                                                                          ((this->kH - 1) * this->dH + 1 -
                                                                           hKer * (this->partH + this->dH - 1));
                            ...
                            for (uint32_t wKer = 0; (wKer < wCycleMax) && (partHWoPad > 0); wKer++) {
                                ...
                                uint32_t curPartW = wKer != (wCycleMax - 1) ? this->partW :
                                                                              ((this->kW - 1) * this->dW + 1 -
                                                                               wKer * (this->partW + this->dW - 1));
                                ...
                                if (partWWoPad > 0) {
                                    DoMaxPool(batchLen, curPartD, curPartH, curPartW, inputOffset, outInKer, padDL,
                                              padDR, padHL, padHR, padWL, padWR);
                                }
                            }
                        }
                    }
                    FinalMax(outputOffset, batchLen, dOut * this->sD - this->pD, hOut * this->sH - this->pH,
                                                                      ...
                                                                      int32_t offD, int32_t offH, int32_t offW)
{
    ...
    Copy(this->transDataTensorUb1, reduceMaxResult, (this->blockLengthS / this->blockLength) * this->blockLength,
         this->blockLength, {1, 0, static_cast<uint16_t>(this->blockLengthS / this->blockLength), 1});
    PipeBarrier<PIPE_V>();
    Copy(reduceMaxResult, this->transDataTensorUb1, this->blockLength * BLOCKS_IN_REP,
    ...
    Select<T>(transDataTensorUb2, maskIndex, transDataTensorUb2[indexStart], static_cast<T>(1.f + this->kernelDHW),
    ...
    Select<T>(transDataTensorUb2[indexStart], maskIndex, transDataTensorUb2[indexStart], static_cast<T>(-1.f),
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DWithArgmaxV2HugeKernel<T, S>::RemovingGaps(const LocalTensor<T>& dst, uint32_t partD,
                                                                          uint32_t partH, uint32_t partW)
{
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DWithArgmaxV2HugeKernel<T, S>::Img2ColC0(const uint32_t partNC, uint32_t partD,
                                                                       uint32_t partH, uint32_t partW,
                                                                       ...
                                                                       const uint32_t pWR)
{
    ...
    LocalTensor<uint8_t>
        nanMask = maskIndex[this->RoundUpBlock((curKernelDHW * this->blockLength) / BITS_UINT8, BLOCK_LEN_UINT8)];
    ...
    if (pDL != 0 || pDR != 0 || pHL != 0 || pHR != 0 || pWL != 0 || pWR != 0) {
        ...
        Padding(this->transDataTensorUb1, transDataTensorUb2, partD, partH, partW, partDWoPad, partHWoPad, partWWoPad,
    ...
    SetMaskCount();
    SetVectorMask<T, MaskMode::COUNTER>(this->mask);
    Compare<T, uint8_t, false>(maskIndex, reduceMaxResult, transDataTensorUb2, CMPMODE::EQ, MASK_PLACEHOLDER,
                               wholeRow / this->mask, {1, 0, 1, BLOCKS_IN_REP, 0, BLOCKS_IN_REP});
    Compare<T, uint8_t, false>(nanMask, transDataTensorUb2, transDataTensorUb2, CMPMODE::EQ, MASK_PLACEHOLDER,
                               wholeRow / this->mask, {1, 1, 1, BLOCKS_IN_REP, BLOCKS_IN_REP, BLOCKS_IN_REP});
    PipeBarrier<PIPE_V>();
    event0 = this->pipe->FetchEventID(HardEvent::V_S);
    SetFlag<HardEvent::V_S>(event0);
    WaitFlag<HardEvent::V_S>(event0);
    ...
    Select<T>(transDataTensorUb2, maskIndex, this->kernelIndexes, static_cast<T>(1.f + this->kernelDHW),
    ...
    Select<T>(transDataTensorUb2, nanMask, transDataTensorUb2, this->kernelIndexes, SELMODE::VSEL_TENSOR_TENSOR_MODE,
              wholeRow);
    PipeBarrier<PIPE_V>();
    event0 = this->pipe->FetchEventID(HardEvent::V_S);
    SetFlag<HardEvent::V_S>(event0);
    ...
    const uint32_t padDR, const uint32_t padHL, const uint32_t padHR, const uint32_t padWL, const uint32_t padWR)
{
    ...
    if (this->widthInput % this->blockLengthS == 0 && downPartW == partWWoPad) {
        if (((this->widthInput - downPartW) / this->blockLengthS) < MAX_UINT16) {
            DataCopyParams params{static_cast<uint16_t>(partHWoPad),
                                  static_cast<uint16_t>(downPartW / this->blockLengthS),
            ...
            DataCopyParams params{static_cast<uint16_t>(1), static_cast<uint16_t>(downPartW / this->blockLengthS), 0,
                                  0};
            for (uint32_t nc = 0; nc < partNC; nc++) {
                for (uint32_t d = 0, dstOff = 0; d < tmpDInSize; d += this->hwInputSize, dstOff += upPartW * partH) {
        ...
        DataCopyExtParams copyPadParamsInput{static_cast<uint16_t>(partHWoPad),
                                             static_cast<uint32_t>(partWWoPad * sizeof(S)),
...
template <typename T, typename S>
__aicore__ inline void KernelMaxPool3DWithArgmaxV2HugeKernel<T, S>::CopyOutVals(const uint32_t& dstOffset,
    ...
    const uint32_t pDR, const uint32_t pHL, const uint32_t pHR, const uint32_t pWL, const uint32_t pWR)
{
    ...
    if (pDR != 0) {
        Duplicate<T>(srcUb[(partDWoPad + pDL) * partHwInp * this->blockLength], this->minFloat,

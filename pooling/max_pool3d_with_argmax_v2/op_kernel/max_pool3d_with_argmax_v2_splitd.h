...
public:
    __aicore__ KernelMaxPool3DWithArgmaxV2SplitD(const MaxPool3DWithArgmaxV2SplitDTilingData* __restrict__ tilingData_)
    ...
    const uint32_t pWL, const uint32_t pWR)
{
    ...
    if (!onlyRow) {
        ...
        Select<T>(idxsT, maskIndex, idxsT, (T)-1.f, SELMODE::VSEL_TENSOR_SCALAR_MODE, alignedRowlen);
        PipeBarrier<PIPE_V>();

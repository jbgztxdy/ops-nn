...
protected:
        ...
        uint32_t rowLen, uint32_t dhVal)
    {
        ...
        for (uint32_t curWidthOut = 0, srcOff = 0; curWidthOut < wholeRow;
            ...
            Select<T>(
                srcDst[curWidthOut], masks[srcOff], kernelIndexes, (T)(1.f * kernelDHW), SELMODE::VSEL_TENSOR_SCALAR_MODE,
                blockKernelDHW);
        }
        PipeBarrier<PIPE_V>();
        ...
        uint32_t rowLen, uint32_t dhVal)
    {
        ...
        for (maxRep = 0; maxRep < rowLen / MAX_REPEAT_TIMES; maxRep++) {
            Select<T>(
                ...
                (T)(1.f * kernelDHW), SELMODE::VSEL_TENSOR_SCALAR_MODE, blockKernelDHW, MAX_REPEAT_TIMES,
                {1, 1, 1, static_cast<uint8_t>(kernelDHW), 0, 0});
        }
        Select<T>(
            srcDst[MAX_REPEAT_TIMES * maxRep * blockKernelDHW], masks[maskOff * maxRep], kernelIndexes, (T)(1.f * kernelDHW),

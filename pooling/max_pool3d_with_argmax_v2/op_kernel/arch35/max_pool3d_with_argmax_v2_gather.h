...
template <
    typename T1, typename T2, const uint32_t IS_PAD = 1, typename UB_INDEXT = int16_t, typename CAL_INDEXT = int32_t>
...
private:
    ...
    __aicore__ inline void CopyToCalcBuffer(
        ...
        uint32_t srcRowStride, uint32_t dstBatchStride, uint32_t dstDepStride, uint32_t dstRowStride,
    ...
    __aicore__ inline void ComputeBaseIndex(
    ...
    __aicore__ inline void ConvertBaseIndexAndWriteArgmax(
    ...
    __aicore__ inline void MultiRowGather(
        __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr);
    __aicore__ inline void MultiDepGather(
        __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr);
    __aicore__ inline void MultiNcGather(
        __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr);

    __aicore__ inline void SingleRowGatherForInt64(
        __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr);
    __aicore__ inline void MultiRowGatherForInt64(
        __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr);
    __aicore__ inline void MultiDepGatherForInt64(
        __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr);
    __aicore__ inline void MultiNcGatherForInt64(
        __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr);
...
__aicore__ inline void MaxPool3DWithArgmaxV2GatherKernel<T1, T2, IS_PAD, UB_INDEXT, CAL_INDEXT>::Init(
...
__aicore__ inline void MaxPool3DWithArgmaxV2GatherKernel<T1, T2, IS_PAD, UB_INDEXT, CAL_INDEXT>::CopyIn()
{
    ...
    if constexpr (IS_PAD == 1) {
        ...
        loopModeParamsT1.loop1DstStride =
            dInputActualNoPad_ * hInputActualNoPad_ * wInputActualAlignedNoPad * sizeof(T1);
    ...
    CAL_INDEXT dIndexBase, __local_mem__ T2*& argmaxAddrLocal, const uint16_t* v2Part, uint16_t totalElem)
{
    if constexpr (sizeof(UB_INDEXT) == sizeof(CAL_INDEXT)) {
        ...
        ComputeBaseIndex(
            maxIndexReg, wInputActualAlignedPad, hInputActualPad, ncInputOffset, t1Reg, divResultReg, wIndexRegUnpack);
        ConvertIndexWithoutPadAlign(
            t1Reg, divResultReg, wIndexRegUnpack, left, wInput, hIndexBase, hInput, dIndexBase, maxIndexConvertReg);
        if constexpr (sizeof(CAL_INDEXT) == sizeof(T2)) {
            ...
            AscendC::MicroAPI::UnPack<T2, CAL_INDEXT, AscendC::MicroAPI::HighLowPart::LOWEST>(
                maxIndexConvertRegT2, maxIndexConvertReg);
            MicroAPI::DataCopyUnAlign(argmaxAddrLocal, maxIndexConvertRegT2, u1, v2Part[0]);
            MicroAPI::DataCopyUnAlignPost(argmaxAddrLocal, u1, 0);
            AscendC::MicroAPI::UnPack<T2, CAL_INDEXT, AscendC::MicroAPI::HighLowPart::HIGHEST>(
                maxIndexConvertRegT2, maxIndexConvertReg);
            ...
            AscendC::MicroAPI::UnPack<T2, int32_t, AscendC::MicroAPI::HighLowPart::LOWEST>(
                maxIndexConvertRegT2, maxIndexConvertRegTep);
            MicroAPI::DataCopyUnAlign(argmaxAddrLocal, maxIndexConvertRegT2, u1, v2Part[0]);
            MicroAPI::DataCopyUnAlignPost(argmaxAddrLocal, u1, 0);
            AscendC::MicroAPI::UnPack<T2, int32_t, AscendC::MicroAPI::HighLowPart::HIGHEST>(
                maxIndexConvertRegT2, maxIndexConvertRegTep);
            MicroAPI::DataCopyUnAlign(argmaxAddrLocal, maxIndexConvertRegT2, u1, v2Part[1]);
            MicroAPI::DataCopyUnAlignPost(argmaxAddrLocal, u1, 0);
        ...
        AscendC::MicroAPI::UnPack<CAL_INDEXT, UB_INDEXT, AscendC::MicroAPI::HighLowPart::LOWEST>(
            divResultRegCal, divResultReg);
        AscendC::MicroAPI::UnPack<CAL_INDEXT, UB_INDEXT, AscendC::MicroAPI::HighLowPart::LOWEST>(
            wIndexRegUnpackCal, wIndexRegUnpack);
        ConvertIndexWithoutPadAlign(
            t1RegCal, divResultRegCal, wIndexRegUnpackCal, left, wInput, hIndexBase, hInput, dIndexBase,
            maxIndexConvertReg);
        if constexpr (sizeof(CAL_INDEXT) == sizeof(T2)) {
        ...
        AscendC::MicroAPI::UnPack<CAL_INDEXT, UB_INDEXT, AscendC::MicroAPI::HighLowPart::HIGHEST>(
            divResultRegCal, divResultReg);
        AscendC::MicroAPI::UnPack<CAL_INDEXT, UB_INDEXT, AscendC::MicroAPI::HighLowPart::HIGHEST>(
            wIndexRegUnpackCal, wIndexRegUnpack);
        ConvertIndexWithoutPadAlign(
        ...
        AscendC::MicroAPI::UnPack<int32_t, UB_INDEXT, AscendC::MicroAPI::HighLowPart::LOWEST>(
            divResultRegTep, divResultReg);
        AscendC::MicroAPI::UnPack<int32_t, UB_INDEXT, AscendC::MicroAPI::HighLowPart::LOWEST>(
            wIndexRegUnpackTep, wIndexRegUnpack);

        AscendC::MicroAPI::UnPack<CAL_INDEXT, int32_t, AscendC::MicroAPI::HighLowPart::LOWEST>(t1RegCal, t1RegTep);
        AscendC::MicroAPI::UnPack<CAL_INDEXT, int32_t, AscendC::MicroAPI::HighLowPart::LOWEST>(
            divResultRegCal, divResultRegTep);
        AscendC::MicroAPI::UnPack<CAL_INDEXT, int32_t, AscendC::MicroAPI::HighLowPart::LOWEST>(
            wIndexRegUnpackCal, wIndexRegUnpackTep);
        ConvertIndexWithoutPadAlign(
            t1RegCal, divResultRegCal, wIndexRegUnpackCal, left, wInput, hIndexBase, hInput, dIndexBase,
            maxIndexConvertReg);
        ...
        AscendC::MicroAPI::UnPack<CAL_INDEXT, int32_t, AscendC::MicroAPI::HighLowPart::HIGHEST>(
            divResultRegCal, divResultRegTep);
        AscendC::MicroAPI::UnPack<CAL_INDEXT, int32_t, AscendC::MicroAPI::HighLowPart::HIGHEST>(
            wIndexRegUnpackCal, wIndexRegUnpackTep);
        ConvertIndexWithoutPadAlign(
        ...
        AscendC::MicroAPI::UnPack<int32_t, UB_INDEXT, AscendC::MicroAPI::HighLowPart::HIGHEST>(
            divResultRegTep, divResultReg);
        AscendC::MicroAPI::UnPack<int32_t, UB_INDEXT, AscendC::MicroAPI::HighLowPart::HIGHEST>(
            wIndexRegUnpackTep, wIndexRegUnpack);
    ...
    __local_mem__ T2*& argmaxAddrLocal, const uint16_t* v2Part, uint16_t totalElem)
{
    ...
    if constexpr (sizeof(UB_INDEXT) == sizeof(CAL_INDEXT)) {
        ...
        ConvertIndexWithoutPadAlign(
            t1Reg, divResultReg, wIndexRegUnpack, left, wInput, hIndexBase, hInput, dIndexBase, maxIndexCalReg);
        if constexpr (sizeof(CAL_INDEXT) == sizeof(T2)) {
            MicroAPI::DataCopyUnAlign(argmaxAddrLocal, maxIndexCalReg, u1, totalElem);
            MicroAPI::DataCopyUnAlignPost(argmaxAddrLocal, u1, 0);
        } else if constexpr (sizeof(CAL_INDEXT) * 2 == sizeof(T2)) {
            AscendC::MicroAPI::UnPack<T2, CAL_INDEXT, AscendC::MicroAPI::HighLowPart::LOWEST>(
                maxIndexConvertRegT2, maxIndexCalReg);
            MicroAPI::DataCopyUnAlign(argmaxAddrLocal, maxIndexConvertRegT2, u1, v2Part[0]);
            MicroAPI::DataCopyUnAlignPost(argmaxAddrLocal, u1, 0);
            AscendC::MicroAPI::UnPack<T2, CAL_INDEXT, AscendC::MicroAPI::HighLowPart::HIGHEST>(
                maxIndexConvertRegT2, maxIndexCalReg);
    ...
    for (uint16_t d = 0; d < dKernel; d++) {
        for (uint16_t i = 0; i < hKernel; i++) {
            for (uint16_t j = 0; j < wKernel; j++) {
                UB_INDEXT relIndex =
                    d * hInputActualPad * wStrideOffset * dDilation + i * wStrideOffset * hDilation + j * wDilation;
                ...
                if constexpr (std::is_same<T1, bfloat16_t>::value || std::is_same<T1, float16_t>::value) {
                    MicroAPI::DataCopyGather(
                        calcReg, computeAddr, (MicroAPI::RegTensor<uint16_t>&)indexWithOffset, gatherMask);
                } else {
                    MicroAPI::DataCopyGather(
            ...
            for (uint16_t hLoop = 0; hLoop < static_cast<uint16_t>(hOutputActual); hLoop++) {
                ...
                __VEC_SCOPE__
                {
                    ...
                    for (uint16_t wLoop = 0; wLoop < loopW; wLoop++) {
                        ...
                        ProcessW(
                            computeAddr, maxValueAddr, wOffset, wInputActualAlignedPad_, hInputActualPad, indexReg,
                            dKernel, hKernel, wKernel, repeatsElem, wOutOffset, maxIndexReg, dDilation, hDilation,
                            wDilation);

                        ConvertBaseIndexAndWriteArgmax(
                            maxIndexReg, wInputActualAlignedPad_, hInputActualPad, ncInputOffset, left, wInput,
                            hIndexBase, hInput, dIndexBase, argmaxAddrLocal, v2Part, repeatsElem);
                    }
                    ...
                    ProcessW(
                        computeAddr, maxValueAddr, wOffsetTail, wInputActualAlignedPad_, hInputActualPad, indexReg,
                        dKernel, hKernel, wKernel, tailRepeatsElem, wOutOffsetTail, maxIndexReg, dDilation, hDilation,
                        wDilation);

                    ConvertBaseIndexAndWriteArgmax(
                        maxIndexReg, wInputActualAlignedPad_, hInputActualPad, ncInputOffset, left, wInput, hIndexBase,
                        hInput, dIndexBase, argmaxAddrLocal, v2PartTail, tailRepeatsElem);
                }
            }
        }
    }
    ...
    __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr)
{
    ...
    uint32_t outputPlaneStride = dOutputActual * hOutputActual * wOutputActual;
    uint32_t dBatchInputStride =
        static_cast<uint32_t>(dBatchCount) * dStride * hInputActualPad * wInputActualAlignedPad;
    ...
    for (uint16_t nc = 0; nc < static_cast<uint16_t>(highAxisActual); nc++) {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t dLoop = 0; dLoop < static_cast<uint16_t>(dLoopTimes); dLoop++) {
                ...
                ProcessW(
                    computeAddr, maxValueAddr, wOffset, wInputActualAlignedPad, hInputActualPad, indexReg, dKernel,
                    hKernel, wKernel, repeatsElem, wOutputOffset, maxIndexReg, dDilation, hDilation, wDilation);
                ConvertBaseIndexAndWriteArgmax(
                    maxIndexReg, wInputActualAlignedPad, hInputActualPad, ncInputOffset, left, wInput, hIndexBase,
                    hInput, dIndexBase, argmaxAddrLocal, v2Part, repeatsElem);
            }
            ...
            ProcessW(
                computeAddr, maxValueAddr, wOffsetTail, wInputActualAlignedPad, hInputActualPad, indexReg, dKernel,
                hKernel, wKernel, tailRepeatsElem, wOutputOffsetTail, maxIndexReg, dDilation, hDilation, wDilation);
            ConvertBaseIndexAndWriteArgmax(
                maxIndexReg, wInputActualAlignedPad, hInputActualPad, ncInputOffset, left, wInput, hIndexBase, hInput,
                dIndexBase, argmaxAddrLocal, v2PartTail, tailRepeatsElem);
        }
    }
    return;
}
    ...
    __local_mem__ T1* computeAddr, __local_mem__ T1* maxValueAddr, __local_mem__ T2* argmaxAddr)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t ncLoop = 0; ncLoop < ncLoopTimes; ncLoop++) {
            ...
            ProcessW(
                computeAddr, maxValueAddr, wOffset, wInputActualAlignedPad, hInputActualPad, indexReg, dKernel, hKernel,
                wKernel, repeatsElem, wOutputOffset, maxIndexReg, dDilation, hDilation, wDilation);
            ConvertNcAndWriteArgmax(
                maxIndexReg, maxIndexConvertRegTep, maxIndexConvertRegT2, u1, wInputActualAlignedPad, hInputActualPad,
                left, wInput, hIndexBase, hInput, dIndexBase, wOffset, num3D, rate4D, argmaxAddrLocal, v2Part,
                repeatsElem);
        }
        ...
        ProcessW(
            computeAddr, maxValueAddr, wOffsetTail, wInputActualAlignedPad, hInputActualPad, indexReg, dKernel, hKernel,
            wKernel, tailRepeatsElem, wOutputOffsetTail, maxIndexReg, dDilation, hDilation, wDilation);
        ConvertNcAndWriteArgmax(
            maxIndexReg, maxIndexConvertRegTep, maxIndexConvertRegT2, u1, wInputActualAlignedPad, hInputActualPad, left,
            wInput, hIndexBase, hInput, dIndexBase, wOffsetTail, num3D, rate4D, argmaxAddrLocal, v2PartTail,
            tailRepeatsElem);
    }
    return;
}
        ...
        for (uint16_t dLoop = 0; dLoop < static_cast<uint16_t>(dOutputActual); dLoop++) {
            ...
            for (uint16_t hLoop = 0; hLoop < static_cast<uint16_t>(hOutputActual); hLoop++) {
                ...
                for (uint16_t wLoop = 0; wLoop < loopW; wLoop++) {
                    ...
                    __VEC_SCOPE__
                    {
                        ...
                        __local_mem__ T2* argmaxAddrLocal =
                            argmaxAddr + static_cast<T2>(outputBase) + static_cast<T2>(wLoop * repeatsElem);
                        MicroAPI::Arange(indexReg, static_cast<UB_INDEXT>(0));
                        MicroAPI::MaskReg preg = MicroAPI::CreateMask<UB_INDEXT, MicroAPI::MaskPattern::ALL>();
                        MicroAPI::Muls(indexReg, indexReg, static_cast<UB_INDEXT>(wStride), preg);
                        ProcessW(
                            computeAddr, maxValueAddr, wOffset, wInputActualAlignedPad, hInputActualPad, indexReg,
                            dKernel, hKernel, wKernel, repeatsElem, wOutOffset, maxIndexReg, dDilation, hDilation,
                            wDilation);

                        ConvertBaseIndexAndWriteArgmax(
                            maxIndexReg, wInputActualAlignedPad, hInputActualPad, ncInputOffset, left, wInput,
                            hIndexBase, hInput, dIndexBase, argmaxAddrLocal, v2Part, repeatsElem);
                    }
                }
                __VEC_SCOPE__
                {
                    ...
                    UB_INDEXT ncInputOffset = static_cast<UB_INDEXT>(ncInputBase);
                    __local_mem__ T2* argmaxAddrLocal =
                        argmaxAddr + static_cast<T2>(outputBase) + static_cast<T2>(loopW * repeatsElem);
                    MicroAPI::Arange(indexReg, static_cast<UB_INDEXT>(0));
                    MicroAPI::MaskReg preg = MicroAPI::CreateMask<UB_INDEXT, MicroAPI::MaskPattern::ALL>();
                    MicroAPI::Muls(indexReg, indexReg, static_cast<UB_INDEXT>(wStride), preg);
                    ...
                    UB_INDEXT wOutOffsetTail = static_cast<UB_INDEXT>(outputBase + tailOutputOffset);
                    ProcessW(
                        computeAddr, maxValueAddr, wOffsetTail, wInputActualAlignedPad, hInputActualPad, indexReg,
                        dKernel, hKernel, wKernel, tailRepeatsElem, wOutOffsetTail, maxIndexReg, dDilation, hDilation,
                        wDilation);
        ...
        for (uint16_t dLoop = 0; dLoop < static_cast<uint16_t>(dOutputActual); dLoop++) {
            ...
            for (uint16_t hLoop = 0; hLoop < hLoopTimes; hLoop++) {
                ...
                __VEC_SCOPE__
                {
                    ...
                    UB_INDEXT ncInputOffset = static_cast<UB_INDEXT>(ncInputBase);
                    __local_mem__ T2* argmaxAddrLocal =
                        argmaxAddr + static_cast<T2>(ncDOutBase) + static_cast<T2>(hLoop * hOutputStride);
                    CalGatterIndex2D<UB_INDEXT>(indexReg, rate2D, wOutputActual, wStride);
                    ProcessW(
                        computeAddr, maxValueAddr, wOffset, wInputActualAlignedPad, hInputActualPad, indexReg, dKernel,
                        hKernel, wKernel, repeatsElem, wOutputOffset, maxIndexReg, dDilation, hDilation, wDilation);
            ...
            __VEC_SCOPE__
            {
                ...
                UB_INDEXT ncInputOffset = static_cast<UB_INDEXT>(ncInputBase);
                __local_mem__ T2* argmaxAddrLocal =
                    argmaxAddr + static_cast<T2>(ncDOutBase) + static_cast<T2>(hLoopTimes * hOutputStride);
                CalGatterIndex2D<UB_INDEXT>(indexReg, rate2D, wOutputActual, wStride);
    ...
    for (uint16_t nc = 0; nc < static_cast<uint16_t>(highAxisActual); nc++) {
        ...
        for (uint16_t dLoop = 0; dLoop < static_cast<uint16_t>(dLoopTimes); dLoop++) {
            ...
            __VEC_SCOPE__
            {
                ...
                UB_INDEXT ncInputOffset = static_cast<UB_INDEXT>(ncInputBase);
                __local_mem__ T2* argmaxAddrLocal =
                    argmaxAddr + static_cast<T2>(ncOutBase) + static_cast<T2>(dLoop * dBatchOutputStride);
                CalGatterIndex3D<UB_INDEXT>(indexReg, rate3D, num2D, rate2D, wOutputActual, wStride);
                ProcessW(
                    computeAddr, maxValueAddr, wOffset, wInputActualAlignedPad, hInputActualPad, indexReg, dKernel,
                    hKernel, wKernel, repeatsElem, wOutputOffset, maxIndexReg, dDilation, hDilation, wDilation);
        ...
        __VEC_SCOPE__
        {
            ...
            UB_INDEXT ncInputOffset = static_cast<UB_INDEXT>(ncInputBase);
            __local_mem__ T2* argmaxAddrLocal =
                argmaxAddr + static_cast<T2>(ncOutBase) + static_cast<T2>(dLoopTimes * dBatchOutputStride);
            CalGatterIndex3D<UB_INDEXT>(indexReg, rate3D, num2D, rate2D, wOutputActual, wStride);
            UB_INDEXT wOffsetTail = static_cast<UB_INDEXT>(ncInputBase + tailDInputOffset);
            UB_INDEXT wOutputOffsetTail = static_cast<UB_INDEXT>(ncOutBase + tailDOutputOffset);
            ProcessW(

...
static inline int64_t PoolingOutShape(const int64_t inputSize, const int64_t kernelSize, const int64_t padL,
                                      ...
                                      const bool ceilMode)
{
    int64_t outputSize = divRtn(inputSize + padL + padR - dilation * (kernelSize - 1) - 1 + (ceilMode ? stride - 1 : 0),
                                stride) +
                         1;
    ...
    if (dilationsRef.Size() == SIZE_1) {
        ...
        OP_CHECK(((dilationsHL == 1) && (dilationsHR == 1) && (dilationsWL == 1) && (dilationsWR == 1)),
                 OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                         "dilation value along each spatial axis only support 1, but got %ld, %ld, %ld, %ld",
                         dilationsHL, dilationsHR, dilationsWL, dilationsWR),
                 return ACLNN_ERR_PARAM_INVALID);
    }
                                    ...
                                    const aclIntArray* pads, const aclIntArray* dilations)
{
    OP_CHECK(CheckAttrSize1Or2(kernelShape),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The size of kernelShape must be one or two, but got %zu.",
                     kernelShape->Size()),
             return ACLNN_ERR_PARAM_INVALID);
    OP_CHECK(
        CheckAttrSize0Or1Or2(strides),
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The size of strides must be zero, one or two, but got %zu.", strides->Size()),
        return ACLNN_ERR_PARAM_INVALID);
    OP_CHECK(
        CheckAttrSize0Or1Or2Or4(pads),
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The size of pads must be zero, one, two or four, but got %zu.", pads->Size()),
        return ACLNN_ERR_PARAM_INVALID);
    OP_CHECK(CheckAttrSize0Or1Or2Or4(dilations),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The size of dilations must be zero, one, two or four, but got %zu.",
                     dilations->Size()),
             return ACLNN_ERR_PARAM_INVALID);
    ...
    if (autoPad != autoPadDefault) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "value of autoPad must be zero representing NOTSET, but got autoPad:%ld",
                autoPad);
        return ACLNN_ERR_PARAM_INVALID;
    }
                       ...
                       const aclIntArray* pads, const int64_t ceilMode, aclTensor* out)
{
    ...
    const int64_t dW = (stridesRef.Size() == 0) ? 1 : ((stridesRef.Size() == SIZE_1) ? dH : stridesRef[INDEX_1]);
    OP_CHECK(((dH > 0) && (dW > 0)),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The size of strides should be greater than 0, but got dh:%ld, dw:%ld",
                     dH, dW),
             return false);
    ...
    OP_CHECK(
        ...
        return false);
    // 检查out的shape是否符合预期输出
    const bool ret = CheckOutputShape(self, kH, kW, padHL, padHR, padWL, padWR, dH, dW, DILATIONS_HW_DEFAULT, ceilMode,
                                      out);
    CHECK_RET(ret, false);
                               ...
                               const int64_t ceilMode, aclTensor* out)
{
    ...
    // 检查参数列表长度和数值是否满足要求
    CHECK_RET(CheckParamsLogic(kernelShape, strides, autoPad, pads, dilations) == ACLNN_SUCCESS,
...
static const aclTensor* ExecMaxPoolV3(const aclTensor* self, const aclTensor* selfContiguous,
                                      const aclIntArray* kernelShape, const aclIntArray* strides,
                                      const aclIntArray* pads, const int64_t ceilMode, aclTensor* out,
                                      aclOpExecutor* executor)
{
    ...
    auto maxPoolOutResult = l0op::MaxPoolV3(selfProcsResult, kernelShape4, strides4, pads4, selfUnsqueFormat, ceilMode,
                                            executor);
...
static const aclTensor* ExecMaxPool3DWithArgmaxV2(const aclTensor* self, const aclTensor* selfContiguous,
                                                  const aclIntArray* kernelShape, const aclIntArray* strides,
                                                  const aclIntArray* pads, const int64_t ceilMode, aclTensor* out,
                                                  aclOpExecutor* executor)
{
    ...
    auto [outResult, indicesResult] = l0op::MaxPool3DWithArgmaxV2Ncdhw(selfUnsqueezed, kernel3D, stride3D, padding3D,
                                                                       dilation3D, ceilMode, dataFormat, executor);
                                         ...
                                         uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    if (ExecMaxPool3DCapable(kernelShape, pads)) {
        outSqueezed = ExecMaxPool3DWithArgmaxV2(self, selfContiguous, kernelShape, strides, pads, ceilMode, out,
                                                uniqueExecutor.get());
    } else {
        outSqueezed = ExecMaxPoolV3(self, selfContiguous, kernelShape, strides, pads, ceilMode, out,
                                    uniqueExecutor.get());
    }

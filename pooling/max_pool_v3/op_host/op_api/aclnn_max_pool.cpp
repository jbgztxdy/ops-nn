...
static inline const int64_t PoolingOutShape(const int64_t inputSize, const int64_t kernelSize, const int64_t padL,
...
static const bool CheckOutputShape(const aclTensor* self, const int64_t kH, const int64_t kW, const int64_t padHL,
...
aclnnStatus aclnnMaxPoolGetWorkspaceSize(const aclTensor* self, const aclIntArray* kernelShape,
  ...
  if (!isDavid) {
    ...
    selfProcsResult = Selfto5HDProcess(selfUnsqueezed, uniqueExecutor.get());
    CHECK_RET(selfProcsResult != nullptr, ACLNN_ERR_INNER_NULLPTR);
  }
  ...
  auto kernelShape4 = kernelShapeOps(kernelShape, uniqueExecutor.get());
  auto strides4 = stridesOps(strides, uniqueExecutor.get());
  auto pads4 = padsOps(pads, uniqueExecutor.get());
  ...
  auto maxPoolOutResult =
      l0op::MaxPoolV3(selfProcsResult, kernelShape4, strides4, pads4, selfUnsqueFormat, ceilMode, uniqueExecutor.get());
  CHECK_RET(maxPoolOutResult != nullptr, ACLNN_ERR_INNER_NULLPTR);

  const aclTensor* outProcsResult = maxPoolOutResult;
  if (!isDavid) {
    // 对返回结果进行处理，进行5HD格式转换
    outProcsResult = OuttoNDProcess(maxPoolOutResult, uniqueExecutor.get());
    CHECK_RET(outProcsResult != nullptr, ACLNN_ERR_INNER_NULLPTR);
  }

  const op::Format& dstFormat = out->GetStorageFormat();
  auto outSqueezed = isSelf3D ? View4Das3D(outProcsResult, dstFormat, uniqueExecutor.get()) : outProcsResult;

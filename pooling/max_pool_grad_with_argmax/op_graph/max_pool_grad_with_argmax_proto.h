...
* @li include_batch_in_index: A boolean attribute indicating whether the batch dimension is included when computing
argmax indices.
*     If false (default), the argmax index is computed within each batch independently, and the index only reflects the
spatial and channel position.
*     If true, the batch dimension is included in the flattened index computation, so the argmax index spans across the
batch dimension.
*     Default value: false.

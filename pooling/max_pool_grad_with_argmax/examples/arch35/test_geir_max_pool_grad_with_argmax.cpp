...
{
    Status ret = SUCCESS;
    auto add1 = op::MaxPoolGradWithArgmax("max_pool_grad_with_argmax");
    vector<vector<int64_t>> shapes = {
        {4, 1, 4, 4}, // x shape (input)
        {4, 1, 2, 2}, // grad shape (gradient input)
        {4, 1, 2, 2}  // argmax shape (indices from forward)
    };
    ...
    ADD_OUTPUT(4, y, inDtype, shapes[0]);
...
int main(int argc, char* argv[])
{
    ...
    Graph graph("max_pool_grad_with_argmax_graph");
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./max_pool_grad_with_argmax_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./max_pool_grad_with_argmax_npu_output_" + std::to_string(i) + ".bin";

...
#include "nn_other.h"
#include "../../op_graph/avg_pool_v2_proto.h"
...
{
    Status ret = SUCCESS;
    auto add1 = op::AvgPoolV2("avg_pool_v2");

    vector<vector<int64_t>> shapes = {
        {4, 1, 8, 8} // x shape (input)
    };
    vector<vector<int64_t>> attrs = {
        {1, 1, 3, 3}, // ksize
        {1, 1, 2, 2}, // strides
        {1, 1, 1, 1}  // pads: {top, bottom, left, right}
    };

    ADD_INPUT(1, x, inDtype, shapes[0]);

    vector<int64_t> output_shape = {4, 1, 4, 4};
    ADD_OUTPUT(2, y, inDtype, output_shape);

    ADD_INPUT_ATTR(ksize, attrs[0]);
    ADD_INPUT_ATTR(strides, attrs[1]);
    ADD_INPUT_ATTR(padding_mode, "CALCULATED");
    ADD_INPUT_ATTR(pads, attrs[2]);
    ADD_INPUT_ATTR(data_format, "NCHW");
    ADD_INPUT_ATTR(global_pooling, false);
    ADD_INPUT_ATTR(ceil_mode, false);
    ADD_INPUT_ATTR(exclusive, true);
    ADD_INPUT_ATTR(divisor_override, 0);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./avg_pool_v2_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./avg_pool_v2_npu_output_" + std::to_string(i) + ".bin";
        ...
        std::cout << "output y (avg_pool_v2 result):" << std::endl;

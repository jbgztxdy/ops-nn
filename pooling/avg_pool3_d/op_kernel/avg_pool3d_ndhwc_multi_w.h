...
private:
    ...
    __aicore__ inline void ReduceSumMultiWindow(const Index& startIndex, const Index& endIndex,
                                                LocalTensor<float>& sumBufLocal, int64_t outputPointIdx,
                                                int64_t nOffset, int64_t windowNum);
    __aicore__ inline void ReduceSumRow(const Index& startIndex, LocalTensor<float>& sumBufLocal,
                                        LocalTensor<T>& inputLocal, int64_t outputPointIdx, int64_t windowNum);
    __aicore__ inline void ReduceSumRowRepeat(const Index& startIndex, LocalTensor<float>& sumBufLocal,
                                              LocalTensor<T>& inputLocal, int64_t windowNum);
...
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::InitTiling(const AvgPool3DTilingData* __restrict__ tiling)
{
    ...
    poolMem.poolParam = PoolParameter(tiling->kD, tiling->kH, tiling->kW, tiling->dD, tiling->dH, tiling->dW,
                                      tiling->pD, tiling->pH, tiling->pW, tiling->divisorOverride,
                                      tiling->countIncludePad);
    ...
    poolMem.atomicAddNum = poolMem.outputPointNum < tiling->atomicAddNum ? poolMem.outputPointNum :
                                                                           tiling->atomicAddNum;
    ...
    isSamePoolSize = poolMem.poolParam.divisorOverride ||
                     ((poolMem.poolParam.countIncludePad || poolMem.poolParam.padW == 0) && !tiling->ceilMode);
}

template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::CopyIn(int64_t offset, uint16_t blockCount,
...
template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::DataCopyOutNonPad(LocalTensor<T>& outputLocal,
    ...
    for (int i = 0; i < blockCount; i++, curPointIdx++) {
        ...
        if ((validDataLen < poolMem.numPerBlock) && (curPointIdx >= poolMem.lastPointOffset - poolMem.atomicAddNum)) {
            ...
            if (curPointIdx > poolMem.lastPointOffset - poolMem.atomicAddNum) {
                SetAtomicAdd<T>();
                DataCopy(poolMem.outputGlobal[curPointIdx * validDataLen], outputLocal[i * poolMem.alignC],
                         poolMem.alignC);
                SetAtomicNone();
            ...
            DataCopy(poolMem.outputGlobal[curPointIdx * validDataLen], outputLocal[i * poolMem.alignC],
                     poolMem.alignC - poolMem.numPerBlock);
            ...
            DataCopy(poolMem.outputGlobal[(curPointIdx + 1) * validDataLen - poolMem.numPerBlock],
                     outputLocal[gatherOffset], poolMem.numPerBlock);
        } else {
...
template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::CopyOut(int64_t offset, uint16_t blockCount,
                                                                      uint32_t blockLen)
{
...
template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::ReduceSumRow(const Index& startIndex,
...
template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::ReduceSumRowRepeat(const Index& startIndex,
                                                                                 LocalTensor<float>& sumBufLocal,
                                                                                 LocalTensor<T>& inputLocal,
                                                                                 int64_t windowNum)
{
    ...
    for (int64_t i = 0; i < poolSize; ++i) {
        for (int64_t j = 0; j < loop; ++j) {
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                Add(sumBufLocal[offset], sumBufLocal[offset], inputLocal.template ReinterpretCast<float>()[src1Offset],
                    mask, windowNum, repeatParams);
            }
        }
    }
}
...
template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::ReduceSumMultiWindow(const Index& startIndex,
                                                                                   ...
                                                                                   int64_t nOffset, int64_t windowNum)
{
    ...
    for (int64_t id = dstart; id < dend; ++id) {
        ...
        for (int64_t ih = hstart; ih < hend; ++ih) {
            ...
            CopyIn(nOffset * poolMem.inputShape.strideN + dOffset + hOffset + wStartOffset, blockCount, poolMem.inC,
                   rightPadding);
...
template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::ReduceMeanMultiWindow(int64_t outputPointIdx,
                                                                                    int64_t windowNum)
{
    Index startIndex;
...
template <typename T, int32_t QUEUE_DEPTH>
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::Init(GM_ADDR x, GM_ADDR y, GM_ADDR workspace,
...
__aicore__ inline void KernelAvgPool3dMultiW<T, QUEUE_DEPTH>::Process()
{
    ...
    for (int64_t outputPointIdx = poolMem.outputPointOffset, count = 0;
         outputPointIdx < poolMem.outputPointOffset + poolMem.outputPointNum;

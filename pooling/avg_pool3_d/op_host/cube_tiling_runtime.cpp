...
uint64_t CubeCompileInfo::CheckTilingInRepo(const char* op_name, const int64_t* shape, size_t dim_num, bool conv) const
{
...
bool Conv3DCompileInfo::AnalyzeExtendInfo(const json& compile_info)
{
...
bool Conv3DBackPropInputCompileInfo::AnalyzeExtendInfo(const json& compile_info)
{
...
std::string DebugTilingData(gert::TilingContext* context)
{
    ...
    auto data = static_cast<const int32_t*>(tiling_data->GetData());
...
static size_t InitVarsValuesForConv3DBpInput(
...
static size_t InitVarsValuesForConv3D(
    uint32_t var_bit_flags, const gert::Shape& in_shape, const gert::Shape& out_shape, gert::Shape& var_value,
    int64_t* shape_for_range_match)
{
    ...
    bool check_c1)
{
    ...
    size_t dim_num = InitVarsValuesForConv3D(
...
ge::graphStatus TilingForConv3DBpInput(gert::TilingContext* context, size_t dedy_idx, bool check_c1)
{
        ...
        if (compile_info->dedy_c1 != -1) {
            OP_CHECK_IF(
                ...
                CUBE_INNER_ERR_REPORT(
                    op_name, "unsupported input, input out_backprop channel [%ld] should be equal to filter [%ld]",
                    dedy_channel_dim, compile_info->dedy_c1),
                return ge::GRAPH_FAILED);
        } else {
    ...
    gert::Shape var_value;
    int64_t shape_for_range_match[4]; // 4: ndhw
    size_t dim_num = InitVarsValuesForConv3DBpInput(

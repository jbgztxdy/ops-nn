...
public:
    __aicore__ inline AdaptiveMaxPool3dGradSimt(
        TPipe *pipe, const AdaptiveMaxPool3dGradTilingDataV35 *__restrict__ tilingData)
...
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void InitOutputZeroSimt(
...
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void AdaptiveMaxPool3dGradKernelSimt(
    const __gm__ VALUE_T* gradData,
    const __gm__ OFFSET_T* argmaxData,
    ...
    __gm__ VALUE_T* xGradData)
{
    ...
    const INDEX_T dhOutPlaneSize =
        static_cast<INDEX_T>(inD) * static_cast<INDEX_T>(inH) * static_cast<INDEX_T>(inW);
    const INDEX_T inPlaneSize =
        static_cast<INDEX_T>(outD) * static_cast<INDEX_T>(outH) * static_cast<INDEX_T>(outW);
    ...
    if constexpr (IsDeterministic) {
        for (INDEX_T ncIdx = threadStart; ncIdx < totalNC; ncIdx += threadStride) {
            ...
            const INDEX_T ncXGradOffset = static_cast<INDEX_T>(n) * cStride +
                                           static_cast<INDEX_T>(c) * inPlaneSize;
        ...
        for (INDEX_T index = threadStart; index < outCount; index += threadStride) {
            ...
            INDEX_T inputIdx = static_cast<INDEX_T>(n) * cStride +
                               static_cast<INDEX_T>(c) * inPlaneSize +
...
template <typename VALUE_T, typename OFFSET_T>
__aicore__ inline void AdaptiveMaxPool3dGradSimt<VALUE_T, OFFSET_T>::Init(
    GM_ADDR x, GM_ADDR grad, GM_ADDR argmax, GM_ADDR y, GM_ADDR workspace)
{
...
__aicore__ inline void AdaptiveMaxPool3dGradSimt<VALUE_T, OFFSET_T>::Process()
{
    ...
    int64_t outputSize = static_cast<int64_t>(nDims) * static_cast<int64_t>(cDims) *
                         static_cast<int64_t>(outD) * static_cast<int64_t>(outH) * static_cast<int64_t>(outW);
    ...
    if (deterministicFlag) {
        Simt::VF_CALL<AdaptiveMaxPool3dGradKernelSimt<VALUE_T, OFFSET_T, true>>(
            Simt::Dim3(THREAD_DIM), gradData, argmaxData,
            nDims, cDims, inD, inH, inW, outD, outH, outW, isOverLap, xGradData);
    } else {

...
__simt_callee__ __aicore__ inline static FORMAT_TYPE PStart(int64_t size, int64_t pad, int64_t kernel,
                                                            FORMAT_TYPE magicStride, FORMAT_TYPE shiftStride)
{
    if (size + pad < kernel) {
        return 0;
    } else {
...
template <typename FORMAT_TYPE>
__simt_callee__ __aicore__ inline static FORMAT_TYPE PEnd(int64_t size, int64_t pad, int64_t poolSize,
                                                          FORMAT_TYPE magicStride, FORMAT_TYPE shiftStride)
{
    using DIV_T = typename std::conditional<std::is_same<FORMAT_TYPE, int32_t>::value, uint32_t, uint64_t>::type;
...
__simt_vf__ __aicore__ LAUNCH_BOUND(SimtProc::THREAD_DIM) inline void MaxPoolGradWithArgmaxNchw(
    __ubuf__ FORMAT_TYPE* SimtParam, const __gm__ VALUE_T* bottomData, const int64_t nD, const int64_t cD, const int hD,
    const int wD, const int hOutDim, const int wOutDim, const int kernelH, const int kernelW, const int strideH,
    const int strideW, const int padH, const int padW, __gm__ VALUE_T* topData, __gm__ INDICES_T* topMask);

template <typename VALUE_T, typename INDICES_T, typename FORMAT_TYPE>
__simt_vf__ __aicore__ LAUNCH_BOUND(SimtProc::THREAD_DIM) inline void MaxPoolGradWithArgmaxNhwc(
    __ubuf__ FORMAT_TYPE* SimtParam, const __gm__ VALUE_T* bottomData, const int64_t nD, const int64_t cD,
    const int hDim, const int wDim, const int hOutDim, const int wOutDim, const int kernelH, const int kernelW,
    const int strideH, const int strideW, const int padH, const int padW, __gm__ VALUE_T* topData,
...
public:
    __aicore__ inline MaxPoolGradWithArgmaxSimt(
        TPipe* pipe,
...
template <typename VALUE_T, typename INDICES_T, int Format_T, typename FORMAT_TYPE>
__aicore__ inline void MaxPoolGradWithArgmaxSimt<VALUE_T, INDICES_T, Format_T, FORMAT_TYPE>::Init(GM_ADDR x,
                                                                                                  GM_ADDR grad,
                                                                                                  GM_ADDR argmax,
                                                                                                  GM_ADDR y)
{
...
__aicore__ inline void MaxPoolGradWithArgmaxSimt<VALUE_T, INDICES_T, Format_T, FORMAT_TYPE>::Process()
{
    ...
    if constexpr (Format_T == NCHW_FORMAT) {
        asc_vf_call<MaxPoolGradWithArgmaxNchw<VALUE_T, INDICES_T, FORMAT_TYPE>>(
            dim3(SimtProc::THREAD_DIM), (__ubuf__ FORMAT_TYPE*)SimtParam.GetPhyAddr(), gradData, SimtTilingData(0),
...
template <typename VALUE_T, typename INDICES_T, typename FORMAT_TYPE>
__simt_vf__ __aicore__ LAUNCH_BOUND(SimtProc::THREAD_DIM) inline void MaxPoolGradWithArgmaxNchw(
    __ubuf__ FORMAT_TYPE* SimtParam, const __gm__ VALUE_T* bottomData, const int64_t nDims, const int64_t cDims,
    const int hDims, const int wDims, const int hOutDim, const int wOutDim, const int kernelH, const int kernelW,
    const int strideH, const int strideW, const int padH, const int padW, __gm__ VALUE_T* topData,
    __gm__ INDICES_T* topMask)
{
...
template <typename VALUE_T, typename INDICES_T, typename FORMAT_TYPE>
__simt_vf__ __aicore__ LAUNCH_BOUND(SimtProc::THREAD_DIM) inline void MaxPoolGradWithArgmaxNhwc(
    __ubuf__ FORMAT_TYPE* SimtParam, const __gm__ VALUE_T* bottomData, const int64_t nDim, const int64_t cDim,
    const int hDim, const int wDim, const int hOutDim, const int wOutDim, const int kernelH, const int kernelW,
    const int strideH, const int strideW, const int padH, const int padW, __gm__ VALUE_T* topData,
    __gm__ INDICES_T* topMask)
{

...
std::pair<es::EsTensorHolder, es::EsTensorHolder> CreatePoolNode(
    es::EsGraphBuilder& graphBuilder, const char* opType, const es::EsTensorHolder& x,
    const std::vector<int64_t>& ksize, const std::vector<int64_t>& strides, const std::vector<int64_t>& pads,
    const std::vector<int64_t>& dilation, const bool setInvalidDilation = false, const char* v0Padding = "VALID")
{
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v1_to_v3_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV1", x, {1, 2, 2, 1}, {1, 2, 2, 1}, {1, 1, 1, 1},
                                  {1, 1, 1, 1});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v2_nhwc_to_v3_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV2", x, {1, 2, 2, 1}, {1, 2, 2, 1}, {1, 1, 1, 1},
                                  {1, 1, 1, 1});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v2_nchw_to_v3_success)
{
    ...
    auto x = graphBuilder.CreateInput(0, "x", DT_FLOAT, FORMAT_NCHW, {64, 32, 32, 32});
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV2", x, {1, 1, 2, 3}, {1, 1, 2, 3}, {0, 2, 1, 0},
                                  {1, 1, 2, 1});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v1_bf16_to_v3_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV1", x, {1, 3, 4, 1}, {1, 2, 5, 1}, {0, 2, 1, 0},
                                  {1, 1, 1, 1});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v2_float_to_v3_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV2", x, {1, 5, 3, 1}, {1, 4, 2, 1}, {0, 3, 2, 0},
                                  {1, 1, 2, 3});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v2_bf16_nhwc_to_v3_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV2", x, {1, 4, 2, 1}, {1, 3, 2, 1}, {0, 0, 1, 0},
                                  {1, 1, 1, 1});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v1_zero_pad_to_v3_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV1", x, {1, 2, 3, 1}, {1, 2, 3, 1}, {0, 0, 0, 0},
                                  {1, 1, 1, 1});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v2_nchw_identity_dilation_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV2", x, {1, 1, 3, 3}, {1, 1, 2, 2}, {0, 1, 1, 0},
                                  {1, 1, 1, 1});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v1_large_window_success)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV1", x, {1, 7, 5, 1}, {1, 3, 2, 1}, {0, 4, 2, 0},
                                  {1, 1, 3, 2});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});
...
TEST_F(MaxPoolWithArgmaxV3FusionPassTest, maxpoolwithargmax_v2_unsupported_soc_950_fused)
{
    ...
    auto outputs = CreatePoolNode(graphBuilder, "MaxPoolWithArgmaxV2", x, {1, 3, 3, 1}, {1, 2, 2, 1}, {0, 1, 1, 0},
                                  {1, 1, 1, 1});
    std::shared_ptr<Graph> graph = graphBuilder.BuildAndReset({outputs.first, outputs.second});

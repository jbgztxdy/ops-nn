...
{
    L0_DFX(MaxPoolGradWithArgmaxV1AiCore, gradOutput, self, indices, kernelSize, stride, padding, dilation, ceilMode,
           gradInput);
    ...
    ADD_TO_LAUNCHER_LIST_AICORE(MaxPoolGradWithArgmaxV1, OP_INPUT(self, gradOutput, indices), OP_OUTPUT(gradInput),
                                         ...
                                         aclOpExecutor* executor)
{
    ...
    auto ret = INFER_SHAPE(MaxPoolGradWithArgmaxV1, OP_INPUT(self, gradOutput, indices), OP_OUTPUT(gradInput),
    ...
    if (IsAiCoreSupport(self)) {
        // 调用算子计算
        return MaxPoolGradWithArgmaxV1AiCore(gradOutput, self, indices, kernelSize, stride, padding, dilation, ceilMode,
                                             gradInput, executor);
    } else {
        // 当前没有匹配的aicpu算子
        return nullptr;

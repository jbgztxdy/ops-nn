...
private:
    ...
    uint64_t tailH;
    uint64_t tailOutH;
    const uint64_t workspaceSizePerCore = (this->depthOut + this->pD + this->ceilD) *
                                          (this->heightOut + this->pH + this->ceilH) * this->partOutW *
                                          this->blockLength;

    TEventID eventMTE3_MTE2_workspace = this->pipe.template AllocEventID<HardEvent::MTE3_MTE2>();

public:
    __aicore__ KernelMaxPool3DGradWithArgmaxSplitH(
        const MaxPool3DGradWithArgmaxSplitHTilingData* __restrict__ tilingData_)
        : KernelMaxPool3DGradWithArgmaxBase<T, S>(
              ...
              tilingData_->partD, tilingData_->partH, tilingData_->partW, tilingData_->partOutD, tilingData_->partOutH,
              tilingData_->partOutW, tilingData_->batchesPerCore, tilingData_->leftOverBatches, tilingData_->ceilD,
              tilingData_->ceilH, tilingData_->ceilW, tilingData_->sizeUb1, tilingData_->sizeUb2,
              tilingData_->sizeValues)
    {}
        ...
        if (sizeof(T) == sizeof(S)) {
            ...
            if (this->dAdd || this->hAdd) {
                this->workspaceGm.SetGlobalBuffer(
                    reinterpret_cast<__gm__ float*>(workspace) + this->coreIdx * workspaceSizePerCore);
            } else {
                this->pipe.template ReleaseEventID<HardEvent::MTE3_MTE2>(eventMTE3_MTE2_workspace);
            }
        }
...
private:
    ...
    __aicore__ void CopyOut(
        GlobalTensor<S> dstGm, const uint64_t& partNC, const uint64_t& realPartOutD, const uint64_t& realPartOutH);
    __aicore__ void ProcessBatches(
        const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, const uint64_t& offH);
    __aicore__ void Im2ColCall(
        const LocalTensor<T>& Im2ColTensor, const uint64_t& part, const uint64_t& curDepthOff_, bool& addMulti,
        bool& addTail, bool& onlyTail, bool& overUint8);
    __aicore__ void Compute();
    __aicore__ void ComputeIteration(
        const int64_t& dOff, const int64_t& hOff, const uint64_t& partNC, const uint64_t& partOut, const uint64_t& part,
        const uint64_t& inputOffset, const uint64_t& outputOffset, const uint64_t& padDL, const uint64_t& padDR,
        const uint64_t& padHL, const uint64_t& padHR, const uint64_t& padWL, const uint64_t& padWR, bool& addMulti,
        bool& addTail, bool& onlyTail, bool computeTail);

    __aicore__ void ComputeAll(
        const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, const uint64_t& offH, bool& addMulti,
        bool& addTail, bool& onlyTail);
};
...
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitH<T, S>::CreateGeneralOffsets()
{
    ...
    if ((this->partW % this->blockLength == 0) && (sizeof(T) == BLOCK_SIZE / this->blockLength)) {
        ...
        PipeBarrier<PIPE_V>();
        i += this->partW;
        for (int64_t curH = offH + (int)this->sH; curH < offH + (int)this->partH * (int)this->sH;
             curH += (int)this->sH) {
...
template <typename T, typename S>
template <typename V>
__aicore__ inline void KernelMaxPool3DGradWithArgmaxSplitH<T, S>::TransposeBack(
    const LocalTensor<float>& dstTensor, const LocalTensor<float>& srcTensor, const uint64_t& partNC,
    const uint64_t& realPartOutD, const uint64_t& realPartOutH, const uint64_t& dataLen)
{
    ...
    for (uint64_t j = 0; j < realPartOutD; ++j) {
        for (uint64_t i = 0; i < this->blockLength; ++i) {
            srcLocalList[NCHW_CONV_ADDR_LIST_SIZE / this->blockLength * i] =
                (uint64_t)srcTensor[NCHW_CONV_ADDR_LIST_SIZE * i + j * curPartHwOut * this->blockLength].GetPhyAddr();
            srcLocalList[NCHW_CONV_ADDR_LIST_SIZE / this->blockLength * i + 1] =
                (uint64_t)tmp[NCHW_CONV_ADDR_LIST_SIZE * i + j * curPartHwOut * this->blockLength].GetPhyAddr();
            dstLocalList[NCHW_CONV_ADDR_LIST_SIZE / this->blockLength * i] =
                (uint64_t)
                    dstTensor[i * realPartOutD * this->RoundUpBlock(alPartHwOut, MIN_TRANSPOSE_ROWS) + j * alPartHwOut]
                        .GetPhyAddr();
            dstLocalList[NCHW_CONV_ADDR_LIST_SIZE / this->blockLength * i + 1] =
                (uint64_t)tmp2[i * realPartOutD * this->RoundUpBlock(alPartHwOut, MIN_TRANSPOSE_ROWS) + j * alPartHwOut]
                    .GetPhyAddr();
        }
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitH<T, S>::CopyOut(
    GlobalTensor<S> dstGm, const uint64_t& partNC, const uint64_t& realPartOutD, const uint64_t& realPartOutH)
{
    ...
    if (this->hwOutputSize % this->blockLengthS == 0 && hwOut == downSize) {
        if ((this->hwOutputSize - hwOut) / this->blockLengthS < MAX_UINT16) {
            DataCopyParams copyParams{
                static_cast<uint16_t>(realPartOutD), static_cast<uint16_t>(hwOut / this->blockLengthS),
                static_cast<uint16_t>((roundSize - hwOut) / this->blockLengthS),
                static_cast<uint16_t>((this->hwOutputSize - hwOut) / this->blockLengthS)};
            for (uint64_t nc = 0; nc < partNC; ++nc) {
                DataCopy(dstGm[nc * this->outSize], Im2ColTensor[nc * transAlPartDhwOut], copyParams);
            }
        } else {
            DataCopyParams copyParams{
                static_cast<uint16_t>(1), static_cast<uint16_t>(hwOut / this->blockLengthS), 0, 0};
            for (uint64_t nc = 0; nc < partNC; ++nc) {
        ...
        DataCopyExtParams copyPadParams{
            static_cast<uint16_t>(realPartOutD), static_cast<uint32_t>(hwOut * sizeof(S)),
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitH<T, S>::Im2ColCall(
    const LocalTensor<T>& Im2ColTensor, const uint64_t& part, const uint64_t& curDepthOff_, bool& addMulti,
    bool& addTail, bool& onlyTail, bool& overUint8)
{
    ...
    if constexpr (sizeof(T) == sizeof(S)) { // fp32 | fp16 case
        if (!this->hAdd && !this->wAdd) {
            for (; curHeightOut < part; dstOffset += this->gmHOff, ++curHeightOut) {
                this->template Im2Col<false>(
                    dstOffset, sliceByWidthOffset, Im2ColTensor, curDepthOff_, this->partOutW, this->partW, addMulti,
                    addTail, onlyTail, overUint8);
            }
        } else {
            if (this->hAdd && !this->wAdd) {
                this->template Im2Col<false>(
                ...
                for (; curHeightOut < part; dstOffset += this->gmHOff, ++curHeightOut) {
                    this->template Im2Col<true>(
                        dstOffset, sliceByWidthOffset, Im2ColTensor, curDepthOff_, this->partOutW, this->partW,
                        addMulti, addTail, onlyTail, overUint8);
                }
            } else {
                for (; curHeightOut < part; dstOffset += this->gmHOff, ++curHeightOut) {
                    this->template Im2Col<true>(
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitH<T, S>::ComputeIteration(
    const int64_t& dOff, const int64_t& hOff, const uint64_t& partNC, const uint64_t& partOut, const uint64_t& part,
    const uint64_t& inputOffset, const uint64_t& outputOffset, const uint64_t& padDL, const uint64_t& padDR,
    const uint64_t& padHL, const uint64_t& padHR, const uint64_t& padWL, const uint64_t& padWR, bool& addMulti,
    bool& addTail, bool& onlyTail, bool computeTail)
{
    ...
    this->transDataParamsReverse.dstRepStride =
        (this->transDataParamsReverse.repeatTimes == 1) ? 0 : NCHW_CONV_ADDR_LIST_SIZE / this->blockLength;
    this->transDataParamsReverse.srcRepStride =
        (this->transDataParamsReverse.repeatTimes == 1) ? 0 : NCHW_CONV_ADDR_LIST_SIZE;
    ...
    if constexpr (sizeof(T) == sizeof(S)) {
        ...
        Cast(
            transDataTensorUb2, curInVals, RoundMode::CAST_NONE,
            partNC * this->RoundUpBlock(partFull, this->blockLengthS));
        PipeBarrier<PIPE_V>();
    }
    ...
    if constexpr (sizeof(T) != sizeof(S)) {
        if (this->dAdd || this->hAdd) {
            ...
            for (uint64_t curD = 0; curD < this->partOutD; ++curD) {
                DataCopy(
                    this->tmpBuf[curD * partOut * this->partOutW * this->blockLength],
                    this->workspaceGm
                        [curWorkspaceOffset +
                         curD * (this->heightOut + this->pH + this->ceilH) * this->partOutW * this->blockLength],
                    partOut * this->partOutW * this->blockLength);
            }
    ...
    if (padHL || padWL || padHR || padWR) {
        tmpBufOffset = 0;
        bool smallSingleMask =
            ((this->stripPadRepeatTimes == 0) || (this->stripPadRepeatTimes == 1 && this->stripPadTail == 0)) ? true :
                                                                                                                false;
        bool doTail =
            ((this->stripPadRepeatTimes == 1 && this->stripPadTail == 0) || (this->stripPadTail == 0)) ? false : true;

        this->StripPad(
            padDL, padHL, padWL, this->partOutD - padDL - padDR, partOut - padHL - padHR, this->widthOut,
            this->partOutD, partOut, this->partOutW, smallSingleMask, doTail);
        PipeBarrier<PIPE_V>();
    }

    TransposeBack<S>(
        Im2ColTensor, this->tmpBuf[tmpBufOffset], partNC, this->partOutD - padDL - padDR, partOut - padHL - padHR,
        realTransAlignRoundPartOutSize);
    PipeBarrier<PIPE_V>();

    if (computeTail) {
        this->gmDOff =
            (this->sD * this->partOutH * this->partOutW - this->partH * this->sH * this->partOutW) * this->blockLength;

        this->transDataParams.repeatTimes =
            this->RoundUpBlock(this->partRoundDhwInp, MIN_TRANSPOSE_ROWS) / NCHW_CONV_ADDR_LIST_SIZE;
        this->transDataParams.dstRepStride = (this->transDataParams.repeatTimes == 1) ? 0 : NCHW_CONV_ADDR_LIST_SIZE;
        this->transDataParams.srcRepStride =
            (this->transDataParams.repeatTimes == 1) ? 0 : NCHW_CONV_ADDR_LIST_SIZE / this->blockLength;
    }
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitH<T, S>::ComputeAll(
    const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, const uint64_t& offH, bool& addMulti,
    bool& addTail, bool& onlyTail)
{
    uint64_t d = 0, h = 0;
    ...
    for (dOut = 0; dOut < this->roundDepthOut; dOut += offD, d += this->partD) {
        ...
        uint64_t padDR = (((dOut + this->partOutD) > (this->depthOut + this->pD)) && (this->ceilD != 0)) ?
                             (dOut + this->partOutD) - (this->depthOut + this->pD) :
                             0;
        ...
        for (hOut = 0; hOut < this->roundHeightOut; hOut += offH, h += this->partH) {
            ...
            uint64_t padHR = (((hOut + this->partOutH) > (this->heightOut + this->pH)) && (this->ceilH != 0)) ?
                                 (hOut + this->partOutH) - (this->heightOut + this->pH) :
                                 0;
            ...
            uint64_t outputOffset = b * this->outSize + (dOut >= this->pD ? dOut - this->pD : 0) * this->hwOutputSize +
                                    (hOut >= this->pH ? hOut - this->pH : 0) * this->widthOut;

            ComputeIteration(
                d * this->sD - this->pD, h * this->sH - this->pH, batchesPerIter, this->partOutH, this->partH,
                inputOffset, outputOffset, padDL, padDR, padHL, padHR, this->pW, this->ceilW, addMulti, addTail,
                onlyTail, false);
            curWorkspaceOffset += this->sH * this->partH * this->partOutW * this->blockLength;
        }
        if (tailH >= 1) {
            uint64_t padHL = ((int(this->pH - hOut) > 0) && (this->pH != 0)) ? this->pH - hOut : 0;
            uint64_t padHR = (((hOut + this->tailOutH) > (this->heightOut + this->pH)) && (this->ceilH != 0)) ?
                                 (hOut + this->tailOutH) - (this->heightOut + this->pH) :
                                 0;
            uint64_t inputOffset = b * this->inputSize + d * this->hwInputSize + h * this->widthInput;
            uint64_t outputOffset = b * this->outSize + (dOut >= this->pD ? dOut - this->pD : 0) * this->hwOutputSize +
                                    (hOut >= this->pH ? hOut - this->pH : 0) * this->widthOut;
            ...
            ComputeIteration(
                d * this->sD - this->pD, h * this->sH - this->pH, batchesPerIter, tailOutH, tailH, inputOffset,
                outputOffset, padDL, padDR, padHL, padHR, this->pW, this->ceilW, addMulti, addTail, onlyTail, true);
        }
        curWorkspaceOffset = prevWorkspaceOffset +
                             this->sD * (this->heightOut + this->pH + this->ceilH) * this->partOutW * this->blockLength;
    }
}

template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitH<T, S>::ProcessBatches(
    const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, const uint64_t& offH)
{

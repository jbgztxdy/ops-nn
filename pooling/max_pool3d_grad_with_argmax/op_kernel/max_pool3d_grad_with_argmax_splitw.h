...
private:
    ...
    const uint64_t workspaceSizePerCore = (this->depthOut + this->pD + this->ceilD) *
                                          (this->heightOut + this->pH + this->ceilH) *
                                          (this->widthOut + this->pW + this->ceilW) * this->blockLength;
...
private:
    ...
    __aicore__ void ComputeIteration(
        const int64_t& dOff, const int64_t& hOff, const int64_t& wOff, const uint64_t& partNC, const uint64_t& partOut,
        const uint64_t& part, const uint64_t& inputOffset, const uint64_t& outputOffset, const uint64_t& padDL,
        const uint64_t& padDR, const uint64_t& padHL, const uint64_t& padHR, const uint64_t& padWL,
        const uint64_t& padWR, bool& addMulti, bool& addTail, bool& onlyTail, bool computeTail);

    __aicore__ void ComputeAll(
        const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, const uint64_t& offH,
        const uint64_t& offW, bool& addMulti, bool& addTail, bool& onlyTail);
};
...
template <typename T, typename S>
template <typename V>
__aicore__ inline void KernelMaxPool3DGradWithArgmaxSplitW<T, S>::TransposeBack(
    const LocalTensor<float>& dstTensor, const LocalTensor<float>& srcTensor, const uint64_t& partNC,
        ...
        for (uint64_t j = 0; j < realPartOutH; ++j) {
            for (uint64_t i = 0; i < this->blockLength; ++i) {
                srcLocalList[NCHW_CONV_ADDR_LIST_SIZE / this->blockLength * i] =
                    (uint64_t)srcTensor
                        [NCHW_CONV_ADDR_LIST_SIZE * i + j * realPartOutW * this->blockLength +
                         k * hwPartOut * this->blockLength]
                            .GetPhyAddr();
                srcLocalList[NCHW_CONV_ADDR_LIST_SIZE / this->blockLength * i + 1] =
                    (uint64_t)
                        tmp[NCHW_CONV_ADDR_LIST_SIZE * i + j * realPartOutW * this->blockLength +
                            k * hwPartOut * this->blockLength]
                            .GetPhyAddr();
                dstLocalList[NCHW_CONV_ADDR_LIST_SIZE / this->blockLength * i] =
                    (uint64_t)dstTensor
                        [i * realPartOutD * realPartOutH * this->RoundUpBlock(alWOut, MIN_TRANSPOSE_ROWS) + j * alWOut +
                         k * alWOut * realPartOutH]
                            .GetPhyAddr();
    ...
    for (uint64_t k = 0; k < realPartOutD; ++k) {
        for (uint64_t j = 0; j < realPartOutH; ++j) {
            for (uint64_t i = 0; i < NCHW_CONV_ADDR_LIST_SIZE; ++i) {
                srcLocalList[i] = (uint64_t)srcTensor
                                      [NCHW_CONV_ADDR_LIST_SIZE * i + j * realPartOutW * this->blockLength +
                                       k * hwPartOut * this->blockLength]
                                          .GetPhyAddr();
                dstLocalList[i] =
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitW<T, S>::CopyOut(
    GlobalTensor<S> dstGm, const uint64_t& partNC, const uint64_t& realPartOutD, const uint64_t& realPartOutH,
    const uint64_t& realPartOutW)
{
    ...
    if (this->widthOut % this->blockLengthS == 0 && downPartOutW == realPartOutW) {
        if ((this->widthOut - realPartOutW) / this->blockLengthS < MAX_UINT16) {
            DataCopyParams copyParams{
                static_cast<uint16_t>(realPartOutH), static_cast<uint16_t>(realPartOutW / this->blockLengthS),
            ...
            DataCopyParams copyParams{
                static_cast<uint16_t>(1), static_cast<uint16_t>(realPartOutW / this->blockLengthS), 0, 0};
            for (uint64_t nc = 0; nc < partNC; ++nc) {
                auto tmpSrc = Im2ColTensor[nc * transAlPartDhwOut];
                auto tmpDst = dstGm[nc * this->outSize];
                for (uint64_t dOut = 0; dOut < realPartOutD; ++dOut) {
        ...
        DataCopyExtParams copyPadParams{
            static_cast<uint16_t>(realPartOutH), static_cast<uint32_t>(realPartOutW * sizeof(S)),
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitW<T, S>::ComputeIteration(
    const int64_t& dOff, const int64_t& hOff, const int64_t& wOff, const uint64_t& partNC, const uint64_t& partOut,
    const uint64_t& part, const uint64_t& inputOffset, const uint64_t& outputOffset, const uint64_t& padDL,
    const uint64_t& padDR, const uint64_t& padHL, const uint64_t& padHR, const uint64_t& padWL, const uint64_t& padWR,
    bool& addMulti, bool& addTail, bool& onlyTail, bool computeTail)
{
    ...
    if constexpr (sizeof(T) == sizeof(S)) {
        ...
        Cast(
            transDataTensorUb2, curInVals, RoundMode::CAST_NONE, partNC * this->RoundUpBlock(part, this->blockLengthS));
        PipeBarrier<PIPE_V>();
    }
        ...
        if (this->dAdd || this->hAdd || this->wAdd) {
            ...
            for (uint64_t curD = 0; curD < this->partOutD; ++curD) {
                for (uint64_t curH = 0; curH < this->partOutH; ++curH) {
                    DataCopy(
                        this->tmpBuf
                            [curD * this->partOutH * partOut * this->blockLength + curH * partOut * this->blockLength],
                        this->workspaceGm
                            [curWorkspaceOffset + (curD * (this->heightOut + this->pH + this->ceilH) + curH) *
                                                      (this->widthOut + this->pW + this->ceilW) * this->blockLength],
                        partOut * this->blockLength);
                }
            }
    ...
    if constexpr (sizeof(T) == sizeof(S)) {
        if (!this->wAdd) {
            this->template Im2Col<false>(
                dstOffset, sliceByWidthOffset, Im2ColTensor, curDepthOff_, partOut, part, addMulti, addTail, onlyTail,
                this->addParamsOverUint8);
        } else {
    ...
    if (padHL || padWL || padHR || padWR) {
        ...
        if (this->stripPadRepeatTimes == 0 || (this->stripPadRepeatTimes == 1 && this->stripPadTail == 0)) {
            this->PadStripParams.dstRepeatSize =
                this->stripPadRepeatTimes == 0 ? (this->stripPadTail / this->blockLength) : this->vecProcBlocks;
        ...
        this->StripPad(
            padDL, padHL, padWL, this->partOutD - padDL - padDR, this->partOutH - padHL - padHR,
            partOut - padWL - padWR, this->partOutD, this->partOutH, partOut, smallSingleMask, doTail);
        PipeBarrier<PIPE_V>();
    }

    TransposeBack<S>(
        Im2ColTensor, this->tmpBuf[tmpBufOffset], partNC, this->partOutD - padDL - padDR,
        this->partOutH - padHL - padHR, partOut - padWL - padWR, realTransAlignRoundPartOutSize);
    PipeBarrier<PIPE_V>();
    ...
    if constexpr (sizeof(T) == sizeof(S)) { // fp32/fp16 case
        if (this->dAdd || this->hAdd || this->wAdd) {
            SetAtomicAdd<S>();
            CopyOut(
                this->gradInputGm[outputOffset], partNC, this->partOutD - padDL - padDR, this->partOutH - padHL - padHR,
                partOut - padWL - padWR);
            SetAtomicNone();
        } else {
            CopyOut(
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitW<T, S>::ComputeAll(
    const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, const uint64_t& offH, const uint64_t& offW,
    ...
    for (dOut = 0; dOut < this->roundDepthOut; dOut += offD, d += this->partD) {
        ...
        for (hOut = 0; hOut < this->roundHeightOut; hOut += offH, h += this->partH) {
            ...
            for (wOut = 0; wOut < this->roundWidthOut; wOut += offW, w += this->partW) {
                ...
                uint64_t padWR = (((wOut + this->partOutW) > (this->widthOut + this->pW)) && (this->ceilW != 0)) ?
                                     (wOut + this->partOutW) - (this->widthOut + this->pW) :
                                     0;
                uint64_t inputOffset = b * this->inputSize + d * this->hwInputSize + h * this->widthInput + w;
                uint64_t outputOffset = b * this->outSize +
                                        (dOut >= this->pD ? dOut - this->pD : 0) * this->hwOutputSize +
                                        (hOut >= this->pH ? hOut - this->pH : 0) * this->widthOut +
                                        (wOut >= this->pW ? wOut - this->pW : 0);

                ComputeIteration(
                    d * this->sD - this->pD, h * this->sH - this->pH, w * this->sW - this->pW, batchesPerIter,
                    this->partOutW, this->partW, inputOffset, outputOffset, padDL, padDR, padHL, padHR, padWL, padWR,
                    addMulti, addTail, onlyTail, false);
                curWorkspaceOffset += this->sW * this->partW * this->blockLength;
            }
            if (tailW >= 1) {
                uint64_t padWL = ((int(this->pW - wOut) > 0) && (this->pW != 0)) ? this->pW - wOut : 0;
                uint64_t padWR = (((wOut + this->tailOutW) > (this->widthOut + this->pW)) && (this->ceilW != 0)) ?
                                     (wOut + this->tailOutW) - (this->widthOut + this->pW) :
                                     0;
                uint64_t inputOffset = b * this->inputSize + d * this->hwInputSize + h * this->widthInput + w;
                uint64_t outputOffset = b * this->outSize +
                                        (dOut >= this->pD ? dOut - this->pD : 0) * this->hwOutputSize +
                ...
                ComputeIteration(
                    d * this->sD - this->pD, h * this->sH - this->pH, w * this->sW - this->pW, batchesPerIter, tailOutW,
                    tailW, inputOffset, outputOffset, padDL, padDR, padHL, padHR, padWL, padWR, addMulti, addTail,
                    onlyTail, true);
            }
            curWorkspaceOffset = prevWorkspaceOffsetH +
                                 this->sH * this->partH * (this->widthOut + this->pW + this->ceilW) * this->blockLength;
        }
        curWorkspaceOffset = prevWorkspaceOffsetD + this->sD * this->partD *
                                                        (this->heightOut + this->pH + this->ceilH) *
                                                        (this->widthOut + this->pW + this->ceilW) * this->blockLength;
    }
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitW<T, S>::ProcessBatches(
    const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, const uint64_t& offH, const uint64_t& offW)
{

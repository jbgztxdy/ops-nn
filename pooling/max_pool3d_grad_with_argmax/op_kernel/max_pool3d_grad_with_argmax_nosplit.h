...
private:
        ...
        if constexpr (indices && (sizeof(T) == sizeof(S)) && (sizeof(T) == sizeof(half))) {
            if (this->partRoundDhwInp % this->blockLength == 0) {
                ...
                DataCopyExtParams copyPadParamsInput{
                    static_cast<uint16_t>(partNC), static_cast<uint32_t>((this->partRoundDhwInp) * sizeof(V)), 0,
                    (this->partRoundDhwInp % this->blockLength) <= curBlockLen, 0};
            ...
            if (this->partRoundDhwInp % curBlockLen == 0) {
                ...
                DataCopyExtParams copyPadParamsInput{
                    static_cast<uint16_t>(partNC), static_cast<uint32_t>((this->partRoundDhwInp) * sizeof(V)), 0, 0, 0};
                DataCopyPadExtParams<V> paddingParamsInput{false, 0, 0, 0};
                DataCopyPad(dstUb, srcGm, copyPadParamsInput, paddingParamsInput);
            }
        }
    ...
    __aicore__ void Im2ColCall(
        const LocalTensor<T>& Im2ColTensor, bool& addMulti, bool& addTail, bool& onlyTail, bool& overUint8);
    __aicore__ void Compute();
    __aicore__ void ComputeIteration(
        const uint64_t& partNC, const uint64_t& inputOffset, const uint64_t& outputOffset, const uint64_t& padDL,
        const uint64_t& padDR, const uint64_t& padHL, const uint64_t& padHR, const uint64_t& padWL,
        const uint64_t& padWR, bool& addMulti, bool& addTail, bool& onlyTail);
...
__aicore__ void KernelMaxPool3DGradWithArgmaxNoSplit<T, S>::CopyOut(GlobalTensor<S> dstGm, const uint64_t& partNC)
{
    ...
    if (this->transAlignRoundPartOutSize == this->outSize) {
        ...
        DataCopyParams params{
            static_cast<uint16_t>(partNC), static_cast<uint16_t>(this->outSize * sizeof(S)),
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxNoSplit<T, S>::Im2ColCall(
    const LocalTensor<T>& Im2ColTensor, bool& addMulti, bool& addTail, bool& onlyTail, bool& overUint8)
{
    ...
    if (!this->dAdd && !this->hAdd && !this->wAdd) {
        ...
        if (this->dAdd && !this->hAdd && !this->wAdd) {
            for (; curHeightOut < this->partH;
                 ...
                 ++curHeightOut) { // there is a chance that sH >= kH and sW >= kW which means that the first slicing
                                   // window may be coppied, not added
                this->template Im2Col<false>(
                    dstOffset, sliceByWidthOffset, Im2ColTensor, this->curDepthOff, this->partOutW, this->partW,
                    addMulti, addTail, onlyTail, overUint8);
            }
            ...
            this->template Im2Col<false>(
                dstOffset, sliceByWidthOffset, Im2ColTensor, this->curDepthOff, this->partOutW, this->partW, addMulti,
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxNoSplit<T, S>::ComputeIteration(
    const uint64_t& partNC, const uint64_t& inputOffset, const uint64_t& outputOffset, const uint64_t& padDL,
    ...
    bool& addMulti, bool& addTail, bool& onlyTail)
{
    ...
    if constexpr (sizeof(T) == sizeof(S)) {
        ...
        Cast(
            ...
            partNC * this->RoundUpBlock(this->partRoundDhwInp, this->blockLengthS));
        PipeBarrier<PIPE_V>();
    }
    ...
    if (padHL || padWL || padHR || padWR) {
        ...
        this->StripPad(
            padDL, padHL, padWL, this->depthOut, this->heightOut, this->widthOut, this->partOutD, this->partOutH,
...
__aicore__ void KernelMaxPool3DGradWithArgmaxNoSplit<T, S>::Compute()
{
    ...
    for (int64_t b = 0; b < this->roundBatches; b += this->blockLength) {
        ComputeIteration(
            this->blockLength, b * this->inputSize, b * this->outSize, this->pD, this->ceilD, this->pH, this->ceilH,
            this->pW, this->ceilW, multipleAddOrCopy, doTail, onlyTail);
    }

    if (this->partNC != 0) {
        ComputeIteration(
            this->partNC, this->roundBatches * this->inputSize, this->roundBatches * this->outSize, this->pD,
            this->ceilD, this->pH, this->ceilH, this->pW, this->ceilW, multipleAddOrCopy, doTail, onlyTail);
    }
}

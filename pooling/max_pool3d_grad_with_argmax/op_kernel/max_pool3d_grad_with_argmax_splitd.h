...
private:
    uint64_t tailD;
    uint64_t tailOutD;
    const uint64_t overlapPart =
        (this->kD + ((this->kD - 1) * (this->dD - 1)) - this->sD) * this->partOutH * this->partOutW * this->blockLength;
...
public:
    __aicore__ KernelMaxPool3DGradWithArgmaxSplitD(
        const MaxPool3DGradWithArgmaxSplitDTilingData* __restrict__ tilingData_)
...
private:
    ...
    __aicore__ void Im2ColCall(
        const LocalTensor<T>& Im2ColTensor, const uint64_t& part, bool& addMulti, bool& addTail, bool& onlyTail,
        bool& overUint8);
    __aicore__ void Compute();
    __aicore__ void ComputeIteration(
        const int64_t& dOff, const uint64_t& partNC, const uint64_t& partOut, const uint64_t& part,
        ...
        bool& addTail, bool& onlyTail, bool firstIter, bool computeTail);

    __aicore__ void ComputeAll(
        const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, bool& addMulti, bool& addTail,
        bool& onlyTail);
};
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitD<T, S>::CopyOut(
    GlobalTensor<S> dstGm, const uint64_t& partNC, const uint64_t& realPartFull)
{
    ...
    if (this->outSize % this->blockLengthS == 0 && realPartFull == downSize) {
        if (((this->outSize - realPartFull) / this->blockLengthS) < MAX_UINT16) {
            DataCopyParams params{
                static_cast<uint16_t>(partNC), static_cast<uint16_t>(realPartFull / this->blockLengthS),
            ...
            DataCopyParams params{
                static_cast<uint16_t>(1), static_cast<uint16_t>(realPartFull / this->blockLengthS), 0, 0};
        ...
        DataCopyExtParams params{
            static_cast<uint16_t>(partNC), static_cast<uint32_t>(realPartFull * sizeof(S)),
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitD<T, S>::Im2ColCall(
    const LocalTensor<T>& Im2ColTensor, const uint64_t& part, bool& addMulti, bool& addTail, bool& onlyTail,
    bool& overUint8)
{
    ...
    if constexpr (sizeof(T) == sizeof(S)) { // fp32 | fp16 case
        if (!this->dAdd && !this->hAdd && !this->wAdd) {
            for (; curD < part; dstOffset += this->gmDOff, ++curD) {
                for (; curHeightOut < this->partH; dstOffset += this->gmHOff, ++curHeightOut) {
                    this->template Im2Col<false>(
                        dstOffset, sliceByWidthOffset, Im2ColTensor, this->curDepthOff, this->partOutW, this->partW,
            ...
            if (this->dAdd && !this->hAdd && !this->wAdd) {
                for (; curHeightOut < this->partH;
                     dstOffset += this->gmHOff,
                     ++curHeightOut) { // there is a chance that sH >= kH and sW >= kW which means that the first
                                       // slicing window may be coppied, not added
                    this->template Im2Col<false>(
                        dstOffset, sliceByWidthOffset, Im2ColTensor, this->curDepthOff, this->partOutW, this->partW,
                        addMulti, addTail, onlyTail, overUint8);
                }
        ...
        if (!this->dAdd) {
            ...
            } else if (this->hAdd && !this->wAdd) { // overlap only by H, which means that every first H slice may be
                                                    // coppied, not added
                for (; curD < part; dstOffset += this->gmDOff, ++curD) {
                    this->template Im2Col<false>(
                        dstOffset, sliceByWidthOffset, Im2ColTensor, this->curDepthOff, this->partOutW, this->partW,
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitD<T, S>::ComputeIteration(
    const int64_t& dOff, const uint64_t& partNC, const uint64_t& partOut, const uint64_t& part,
    const uint64_t& inputOffset, const uint64_t& outputOffset, const uint64_t& padDL, const uint64_t& padDR,
    const uint64_t& padHL, const uint64_t& padHR, const uint64_t& padWL, const uint64_t& padWR, bool& addMulti,
    bool& addTail, bool& onlyTail, bool firstIter, bool computeTail)
{
    ...
    if constexpr (sizeof(T) != sizeof(S)) { // bfloat16 case
        if (this->dAdd) {
            ...
            DataCopy(
                this->workspaceGm, this->tmpBuf[partOut * this->partHwOut * this->blockLength - overlapPart],
                overlapPart);
    ...
    if (padHL || padWL || padHR || padWR) {
        ...
        this->StripPad(
            padDL, padHL, padWL, partOut - padDL - padDR, this->heightOut, this->widthOut, partOut, this->partOutH,
            this->partOutW, smallSingleMask, doTail);
        PipeBarrier<PIPE_V>();
    }
...
template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitD<T, S>::ComputeAll(
    const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD, bool& addMulti, bool& addTail,
    bool& onlyTail)
{
    uint64_t d = 0;
    ...
    ComputeIteration(
        d * this->sD - this->pD, batchesPerIter, this->partOutD, this->partD, inputOffset, outputOffset, padDL, padDR,
        this->pH, this->ceilH, this->pW, this->ceilW, addMulti, addTail, onlyTail, true, false);
    dOut += offD;
    d += this->partD;

    for (; dOut < this->roundDepthOut; dOut += offD, d += this->partD) {
        padDL = ((int(this->pD - dOut) > 0) && (this->pD != 0)) ? this->pD - dOut : 0;
        padDR = (((dOut + this->partOutD) > (this->depthOut + this->pD)) && (this->ceilD != 0)) ?
                    (dOut + this->partOutD) - (this->depthOut + this->pD) :
                    0;
        ...
        ComputeIteration(
            d * this->sD - this->pD, batchesPerIter, this->partOutD, this->partD, inputOffset, outputOffset, padDL,
            padDR, this->pH, this->ceilH, this->pW, this->ceilW, addMulti, addTail, onlyTail, false, false);
    }
    if (tailD >= 1) {
        padDL = ((int(this->pD - dOut) > 0) && (this->pD != 0)) ? this->pD - dOut : 0;
        padDR = (((dOut + this->tailOutD) > (this->depthOut + this->pD)) && (this->ceilD != 0)) ?
                    (dOut + this->tailOutD) - (this->depthOut + this->pD) :
                    0;
        inputOffset = b * this->inputSize + d * this->hwInputSize;
        outputOffset = b * this->outSize + (dOut >= this->pD ? dOut - this->pD : 0) * this->hwOutputSize;

        ComputeIteration(
            d * this->sD - this->pD, batchesPerIter, tailOutD, tailD, inputOffset, outputOffset, padDL, padDR, this->pH,
            this->ceilH, this->pW, this->ceilW, addMulti, addTail, onlyTail, false, true);
    }
}

template <typename T, typename S>
__aicore__ void KernelMaxPool3DGradWithArgmaxSplitD<T, S>::ProcessBatches(
    const uint64_t& b, const uint64_t& batchesPerIter, const uint64_t& offD)
{

    ...
    Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::MaxPoolWithArgmax("max_pool_with_argmax");
    vector<vector<int64_t>> shapes = {
        {4, 1, 4, 4}, // x shape
        {4, 1, 2, 2}, // y shape (output)
        {4, 1, 2, 2}  // argmax shape
    };
    ...
    ADD_OUTPUT(3, argmax, DT_INT32, shapes[2]);
    ADD_INPUT_ATTR(ksize, attrs[0]);
    ADD_INPUT_ATTR(strides, attrs[1]);
    ADD_INPUT_ATTR(padding, "VALID");
    ADD_INPUT_ATTR(Targmax, 3); // int32
    ADD_INPUT_ATTR(include_batch_in_index, false);
    ADD_INPUT_ATTR(data_format, "NCHW");
    ADD_INPUT_ATTR(nan_prop, false);
...
int main(int argc, char* argv[])
{
    ...
    Graph graph("max_pool_with_argmax_graph");
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./max_pool_with_argmax_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./max_pool_with_argmax_npu_output_" + std::to_string(i) + ".bin";
        ...
        if (i == 0) {
            std::cout << "output y (pooling result):" << std::endl;
            ...
            std::cout << "output argmax (indices):" << std::endl;
            int32_t* data = reinterpret_cast<int32_t*>(output_data_i);
            for (int j = 0; j < std::min(output_shape, (int64_t)16); j++) {
                printf("  argmax[%d] = %d\n", j, data[j]);
            }
        }

...
ge::graphStatus CheckKernelAndStrides(gert::InferShapeContext* context, const std::string& dataFormatStr)
{
    ...
    if (dataFormatStr == "NCHW") {
        if (ksize_data[IDX_ZERO] != ONE) {
            OP_LOGE_FOR_INVALID_VALUE(context->GetNodeName(), "ksize[0]",
                std::to_string(ksize_data[IDX_ZERO]).c_str(), "1");
            return GRAPH_FAILED;
        }
        if (ksize_data[IDX_ONE] != ONE) {
            OP_LOGE_FOR_INVALID_VALUE(context->GetNodeName(), "ksize[1]",
                std::to_string(ksize_data[IDX_ONE]).c_str(), "1");
            return GRAPH_FAILED;
        }
        ...
        if (ksize_data[IDX_THREE] != ONE) {
            OP_LOGE_FOR_INVALID_VALUE(context->GetNodeName(), "ksize[3]",
                std::to_string(ksize_data[IDX_THREE]).c_str(), "1");
            return GRAPH_FAILED;
        }
    }
...
ge::graphStatus InferShape4AvgPoolGrad(gert::InferShapeContext* context)
{
    ...
    if (inputDimNum != CHW_DIMS && inputDimNum != NCHW_DIMS) {
        OP_LOGE_FOR_INVALID_SHAPESIZE(context->GetNodeName(), "orig_input_shape",
            std::to_string(inputDimNum).c_str(), "3 or 4");
        return GRAPH_FAILED;
    }

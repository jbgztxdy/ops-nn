...
namespace optiling {
    CONV_REGISTER_TILING_TEMPLATE(QuantConv3D, Conv3dBaseTilingV2, (int32_t)platform_ascendc::SocVersion::ASCEND910_95, 0);
 
    ge::graphStatus QuantConv3DTilingFunc(gert::TilingContext* context)
    {
    ...
    ge::graphStatus TilingPrepareForQuantConv3D(gert::TilingParseContext *context) {
        OP_TILING_CHECK(context == nullptr,
                        CUBE_INNER_ERR_REPORT("Conv3DV2", "context is null"),
        return ge::GRAPH_FAILED);
    ...
    .Tiling(QuantConv3DTilingFunc)
    .TilingParse<ConvTilingParseInfo>(TilingPrepareForQuantConv3D);
}
...
bool ConvTilingAlgorithmMmode::CheckMinL1Tiling() const
{
    ...
    uint64_t minKBL1 = tilingIns_->isC04Flag ?
                           AlignB(C04_CIN_SIZE * tilingIns_->shapeInfo.singlekH * tilingIns_->shapeInfo.singlekW,
...
bool ConvTilingAlgorithmMmode::CheckL1Buffer()
{
    ...
    uint64_t hoL1Load = min(
        (this->l1TilingRange.mAL1ValueRange.at(this->l1TilingIdx.mAL1Idx) / tilingIns_->shapeInfo.orgWo) + 2,
    ...
    uint64_t currentWeightL1Size = this->l1TilingRange.kBL1Range.at(this->l1TilingIdx.kBL1Idx) * this->dbValue.pbBL1 *
                                   this->l1TilingRange.nBL1ValueRange.at(this->l1TilingIdx.nBL1Idx) *
                                   this->weightDTypeSize;
    uint64_t currentBiasL1Size = tilingIns_->hasBias ? (this->l1TilingFlag.isBiasFullLoad ?
                                                            tilingIns_->shapeInfo.singleCo1 * tilingIns_->cubeInfo.n0 *
                                                                this->biasDTypeSize :
                                                            this->l0TilingParams.nL0 * this->biasDTypeSize) :
                                                       0;
    uint64_t currentFixpParamsSize = this->l1TilingFlag.isFixpFullLoad ?
                                         tilingIns_->shapeInfo.singleCo1 * tilingIns_->cubeInfo.n0 *
                                             tilingIns_->shapeInfo.channelWiseCoeff * FP16_DTYPE_SIZE :
                                         this->l0TilingParams.nL0 * tilingIns_->shapeInfo.channelWiseCoeff *
                                             FP16_DTYPE_SIZE;
...
int64_t ConvTilingAlgorithmMmode::InitCalcL1FullLoadParams()
{
    ...
    this->l1TilingCalc.c04KSizeAlign = AlignB(
        C04_CIN_SIZE * tilingIns_->shapeInfo.singlekH * tilingIns_->shapeInfo.singlekW, tilingIns_->cubeInfo.k0);
    this->l1TilingCalc.kBL1FullLoadSize = tilingIns_->isC04Flag ?
                                              this->l1TilingCalc.c04KSizeAlign :
                                              tilingIns_->shapeInfo.singleCi1 * this->l1TilingCalc.ci0HkWk;
    this->l1TilingCalc.kAL1FullLoadSize = tilingIns_->isC04Flag ?
                                              C04_CIN_SIZE :
    ...
    if ((tilingIns_->shapeInfo.singlekD * tilingIns_->shapeInfo.singleCi1 * hiL1FullLoad * tilingIns_->shapeInfo.orgWi *
         tilingIns_->cubeInfo.k0 * this->fMapDTypeSize) /
            this->fMapDTypeSize !=
        (tilingIns_->shapeInfo.singlekD * tilingIns_->shapeInfo.singleCi1 * hiL1FullLoad * tilingIns_->shapeInfo.orgWi *
         tilingIns_->cubeInfo.k0)) {
    ...
    if (!tilingIns_->isC04Flag) {
        this->l1TilingCalc.fmapFullLoadL1Size = tilingIns_->shapeInfo.singlekD * this->l1TilingCalc.kAL1FullLoadSize *
                                                hiL1FullLoad * tilingIns_->shapeInfo.orgWi * this->fMapDTypeSize *
                                                tilingIns_->innerBatch;
    } else if (tilingIns_->descInfo.fMapType.format == ConvFormat::NHWC &&
               (tilingIns_->shapeInfo.orgkH * tilingIns_->shapeInfo.orgkW == 1)) {
...
int64_t ConvTilingAlgorithmMmode::InitCalcL1Params()
{
    ...
    this->l1TilingCalc.fmapKL1FullLoadSize = static_cast<uint64_t>(tilingIns_->shapeInfo.singlekD) *
                                             this->l1TilingCalc.kAL1FullLoadSize * hiL1MinLoad *
                                             static_cast<uint64_t>(tilingIns_->shapeInfo.orgWi) *
                                             static_cast<uint64_t>(this->dbValue.pbAL1) * this->fMapDTypeSize *
                                             static_cast<uint64_t>(tilingIns_->innerBatch);
    // cal min/kfullload weight size in L1
    this->l1TilingCalc.weightMinLoadL1Size = max(
        kBL1MinLoad * this->l0TilingParams.nL0 * this->dbValue.pbBL1 * this->weightDTypeSize,
    ...
    this->l1TilingCalc.fixpMinLoadL1Size = this->l0TilingParams.nL0 * tilingIns_->shapeInfo.channelWiseCoeff *
                                           FP16_DTYPE_SIZE;
    return 0;
}
...
void ConvTilingAlgorithmMmode::GetL1TilingRange()
{
    ...
    auto effectiveCount = count_if(this->l1TilingRange.kAL1Range.begin(), this->l1TilingRange.kAL1Range.end(),
                                   [](uint64_t x) { return x <= MAX_16_BIT_NUM; });
    ...
    auto mL1EffectiveCount = count_if(this->l1TilingRange.mAL1ValueRange.begin(),
                                      this->l1TilingRange.mAL1ValueRange.end(),
                                      [](uint64_t x) { return x <= LOAD3D_M_START_POS_LIMIT; });
...
void ConvTilingAlgorithmMmode::InitL1TiLing()
{
    ...
    this->l1TilingInitMap[L1TilingMode::FULL_LOAD_BL1].SetIdx(0, this->l1TilingRange.kBL1Range.size() - 1, 0,
...
bool ConvTilingAlgorithmMmode::IsWeightFullLoadDominant()
{
    return (this->l1TilingCalc.fmapFullLoadL1Size <=
            this->l1TilingCalc.weightFullLoadL1Size * tilingIns_->platformInfo.abL1mte2BandWidthCof);
}
...
bool ConvTilingAlgorithmMmode::IsKAL1orKBL1FullLoad()
{
    ...
    uint64_t fmapSingleCoreL1Load = this->l1TilingCalc.kAL1FullLoadSize * hiL1SingleCore * tilingIns_->shapeInfo.orgWi *
...
bool ConvTilingAlgorithmMmode::IsKAL1andKBL1FullLoad()
{
    ...
    if (this->l1TilingFlag.kABL1CanFullLoadFlag) {
        ...
        uint64_t bothFullloadIterNMte2Size = fmapSingleCoreL1Load * tilingIns_->shapeInfo.singleDo +
                                             weightSingleCoreL1Load * tilingIns_->shapeInfo.singleDo *
                                                 tilingIns_->platformInfo.abL1mte2BandWidthCof *
                                                 CeilDiv(tilingIns_->shapeInfo.singleM,
                                                         this->l1TilingRange.mAL1ValueRange.at(tmpMAL1Idx));
        // iterM
        // fmap_mte2_size: dout*fmapSingleCoreL1Load*(loop n)
        // weight_mte2_size: weightSingleCoreL1Load * bwCof
        uint64_t bothFullloadIterMMte2Size = fmapSingleCoreL1Load * tilingIns_->shapeInfo.singleDo *
                                                 CeilDiv(tilingIns_->shapeInfo.singleCo,
                                                         this->l1TilingRange.nBL1ValueRange.at(tmpNBL1Idx)) +
                                             weightSingleCoreL1Load * tilingIns_->platformInfo.abL1mte2BandWidthCof;
...
void ConvTilingAlgorithmMmode::BiasL1TilingDecision()
{
    ...
    if (!this->l1TilingFlag.isFixpFullLoad) {
        ...
        bool exceedDataCopyLimits = tilingIns_->shapeInfo.singleCo1 * tilingIns_->cubeInfo.n0 * this->scaleDTypeSize >
                                    DATACOPYPARAMS_BURSTLEN_MAX;
        if (!CheckL1Buffer() || exceedDataCopyLimits) {
...
void ConvTilingAlgorithmMmode::SetL1TilingRes()
{
    ...
    if (this->l1TilingFlag.abL1Mode == L1TilingMode::FULL_LOAD_BL1) {
        ...
        this->l1TilingParams.mAL1Value = CeilDiv(tilingIns_->shapeInfo.singleM, tilingIns_->cubeInfo.m0) *
...
uint64_t ConvTilingAlgorithmMmode::CalcL1SizeForL0Tiling(uint64_t currmL0, uint64_t currnL0, uint64_t reserveWeightSize)
{
    ...
    uint64_t kBL1Min = tilingIns_->isC04Flag ?
                           AlignB(C04_CIN_SIZE * tilingIns_->shapeInfo.singlekH * tilingIns_->shapeInfo.singlekW,
                                  tilingIns_->cubeInfo.k0) :
                           tilingIns_->shapeInfo.singlekH * tilingIns_->shapeInfo.singlekW * tilingIns_->cubeInfo.k0;
...
void ConvTilingAlgorithmMmode::InitPingPong()
{
    ...
    if (CalcL1SizeForL0Tiling(tilingIns_->cubeInfo.m0, tilingIns_->cubeInfo.n0, l0BSize) <=
        tilingIns_->platformInfo.l1Size) {
        this->dbValue.pbAL1 = DOUBLE_BUFFER_NUM;
        this->dbValue.pbBL1 = DOUBLE_BUFFER_NUM;
    } else {
...
{
    if (tilingIns_->descInfo.fMapType.format == ConvFormat::NHWC &&
        (tilingIns_->shapeInfo.orgkH * tilingIns_->shapeInfo.orgkW == 1)) {
        return tilingIns_->platformInfo.l0ASize / (CONST_VALUE_2 * this->fMapDTypeSize * tilingIns_->shapeInfo.orgWi *
                                                   tilingIns_->shapeInfo.orgHi * l0TilingParams.kL0);
    } else {
        return tilingIns_->platformInfo.l0ASize /
               (CONST_VALUE_2 * this->fMapDTypeSize * l0TilingParams.mL0 * l0TilingParams.kL0);
    }
}
...
void ConvTilingAlgorithmMmode::CalFormulaicInnerBatch()
{
    ...
    uint64_t innerBatchLimit2 = tilingIns_->platformInfo.l0CSize /
                                (l0TilingParams.mL0 * l0TilingParams.nL0 *
    ...
    uint64_t currentWeightL1Size = this->weightDTypeSize * CONST_VALUE_2 * l0TilingParams.nL0 * l0TilingParams.kL0 *
                                   static_cast<uint64_t>(tilingIns_->shapeInfo.orgkH * tilingIns_->shapeInfo.orgkW);
    uint64_t currentFmapK;
    if (tilingIns_->isC04Flag) {
        currentFmapK = CONST_VALUE_2 *
                       AlignB(static_cast<uint64_t>(tilingIns_->shapeInfo.orgWi * tilingIns_->shapeInfo.orgHi) *
    ...
    uint64_t innerBatchLimit3 = isL1SizeOverFlow ?
                                    0 :
                                    (tilingIns_->platformInfo.l1Size - currentBiasL1Size - currentWeightL1Size) /
                                        currentFmapK;
    ...
    tilingIns_->innerBatch = static_cast<uint32_t>(
        min(min(innerBatchLimit1, innerBatchLimit2), min(innerBatchLimit3, innerBatchLimit4)));
    // Distribute batchs as evenly as possible across each core.
    tilingIns_->innerBatch = min(
        tilingIns_->innerBatch,
        static_cast<uint32_t>(
            CeilDiv(static_cast<uint64_t>(tilingIns_->shapeInfo.singleBatch),
...
void ConvTilingAlgorithmMmode::CheckL0CDoubleBuffer()
{
    ...
    bool kFullLoadFlag = l0TilingParams.kL0 ==
                         tilingIns_->shapeInfo.singlekD * tilingIns_->shapeInfo.singleCi1 * this->l1TilingCalc.ci0HkWk;
...
void ConvTilingAlgorithmMmode::GetUbTiling() { ScaleBiasUbTilingDecision(); }
} // namespace conv_tiling_algo_m
...
/*!
 * \file conv2d_base_tiling_repo.cpp
 * \brief
 */
#include "conv2d_base_tiling.h"
...
const bool Conv2dBaseTiling::GetTilingFromRepo()
{
...
const bool Conv2dBaseTiling::GetQuantConv2DTilingFromRepo()
{
    ...
    // RuntimeKb::PlatformInfo platformInfoAoe(opInfo_->aicoreNum, opInfo_tmp.socVersion);

    shared_ptr<void> inputArgs = nullptr;
    size_t inputArgsSize = 0;
    GetTilingInputArgs(inputArgs, inputArgsSize);
    uint32_t ret = Ops::NN::QueryBank(inputArgs.get(), inputArgsSize, "Conv2DV2", opInfo_tmp.socVersion,
...
const bool Conv2dBaseTiling::GetConv2DTilingFromRepo()
{
    shared_ptr<tuningtiling::TuningTilingDef> tuningTiling = nullptr;
    auto compileInfoPtr =

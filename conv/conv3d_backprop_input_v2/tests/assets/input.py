...
__input__ = {
    "kernel": {
        "conv3d_backprop_input_v2": "conv3d_backprop_input_v2_input"
    }
}
...
def conv3d_backprop_input_v2_input(input_size, filter, out_backprop,
                                    *,
                                    strides: list = None,
                                    pads: list = None,
                                    dilations: list = None,
                                    ...
                                    **kwargs):
    """
    Input function for conv3d_backprop_input_v2.
    All the parameters (names and order) follow @conv3d_backprop_input_v2_def.cpp without outputs.
    All the input Tensors are numpy.ndarray.

    Args:
        input_size: Output shape (const input, int32 tensor)
        filter: Filter tensor in FRACTAL_Z_3D format
    ...
    if filter_dtype_str == "float32" and enable_hf32:
    ...
    return [input_size_input, filter_input, out_backprop_input]
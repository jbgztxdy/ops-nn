...
template <class Intf>
__aicore__ inline void InitKDirectionBaseValue(Intf* self, uint64_t updateKAL1 = 0, uint64_t updateKBL1 = 0,
                                               uint64_t updateKL0 = 0)
{
    ...
    uint64_t alignCinKhKwKd = AlignB(self->ctx.singleCoreCin, GetInputkInOneC0Block<Intf>()) *
...
struct Init {
    static __aicore__ inline bool call(Intf* self, const void* __restrict tiling)
    {
        ...
        self->ctx.preloadAL1DbFlag = preloadAL1Flag == 1 && !self->ctx.kAL1fullload && !L0ASet2dFlag && !kAL1TailCase;
    ...
    static __aicore__ inline void InitBuffer(Intf* self)
    {
        ...
        uint64_t biasl1Spacesize = self->ctx.conv3dTiling->nL0 *
                                   sizeof(typename Intf::BiasT); // 非全载时bias伴随L0C切分
                                                          ...
                                                          uint64_t bl1Spacesize)
    {
        ...
        if constexpr (!Intf::bl1bypass) {
            if (!bl1db) {
                self->ctx.pipe.InitBuffer(self->ctx.queueBL1, 1,
                                          AlignB(bl1Spacesize * self->ctx.sizeOfWeight, C0_SIZE));
            } else {
                self->ctx.pipe.InitBuffer(self->ctx.queueBL1, 2,
                                          AlignB(bl1Spacesize * self->ctx.sizeOfWeight, C0_SIZE));
            }
        }
    }

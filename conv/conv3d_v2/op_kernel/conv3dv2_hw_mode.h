...
protected:
    __aicore__ inline bool InitSingleCoreDataWithHout()
    {
        ...
        bool isRealDim = this->CountIdxTail(1, this->conv3dRunInfo->mDim, this->conv3dRunInfo->hout,
                                            this->conv3dRunInfo->hout, mStruct);
        if (!isRealDim) [[unlikely]] {
            return false;
        }
                                      ...
                                      GM_ADDR workspace)
    {
        ...
        if constexpr (CONV_CFG::quantType == static_cast<int8_t>(QuantType::PER_CHANNEL_NO_OFFSET)) {
            uint64_t wsStartAddr = (this->blockIdx * 4) * this->conv3dApiTiling->mL0 *
                                   this->conv3dApiTiling->nL0; // 4表示workspace的份数
            this->workspaceGm.SetGlobalBuffer(reinterpret_cast<__gm__ typename conv::GetDstType<A_T>::Type*>(

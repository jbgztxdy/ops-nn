...
protected:
    __aicore__ inline bool InitSingleCoreDataWithBatch()
    {
        ...
        bool isRealDim = this->CountIdxTail(dataPerBatchDim, this->conv3dRunInfo->batchDim, this->conv3dRunInfo->batch,
                                            this->conv3dRunInfo->batch, batchStruct);
        if (!isRealDim) [[unlikely]] {
            return false;
        }
    ...
    {
        const uint64_t dataPerGroupOptDim = this->conv3dRunInfo->doDim * this->conv3dRunInfo->nDim *
                                            this->conv3dRunInfo->mDim;
        DataToFill groupOptStruct(singleCoreGroupOpt, groupOptIdxStart, isGroupOptDimTail);
        bool isRealDim = this->CountIdxTail(dataPerGroupOptDim, this->conv3dRunInfo->groupDim,
                                            this->conv3dApiTiling->groupOpt, this->conv3dApiTiling->groupOpt,
                                            groupOptStruct);
        if (!isRealDim) [[unlikely]] {
            return false;
        }
    ...
    __aicore__ inline bool InitSingleCoreDataWithDout()
    {
        ...
        bool isRealDim = this->CountIdxTail(dataPerDoDim, this->conv3dRunInfo->doDim, this->conv3dRunInfo->dout,
                                            this->conv3dRunInfo->dout, doStruct);
        if (!isRealDim) [[unlikely]] {
            return false;
        }
    ...
    __aicore__ inline bool InitSingleCoreDataWithCout()
    {
        ...
        bool isRealDim = this->CountIdxTail(this->conv3dRunInfo->mDim, this->conv3dRunInfo->nDim,
                                            AlignB(this->conv3dApiTiling->coutOpt, this->n0),
                                            this->conv3dApiTiling->coutOpt, nStruct);
        if (!isRealDim) [[unlikely]] {
            return false;
        }
    ...
    __aicore__ inline bool InitSingleCoreData()
    {
        ...
        if (preCoreSumCout >= this->conv3dRunInfo->cout) {
            ASC_OP_LOGD("[Conv3dv2WithGroups] No real data exists in groupopt. Skip the processing. groupOptIdxStart "
                        "%d, nIdxStart %d.",
                        groupOptIdxStart, this->nIdxStart);
            return false;
        }
        ...
        if (isGroupOptDimTail) {
            ...
            if (this->isNDimTail || preCoreSumCout + curCoreSumCout >= this->conv3dRunInfo->cout) {
                singleCoreCoutTail = (this->conv3dRunInfo->cout % this->conv3dApiTiling->coutOpt) %
                                     this->conv3dApiTiling->singleCoreCo;
            }
            return true;
        }
    ...
    {
        this->fmapOneBatchSize = this->conv3dRunInfo->din * this->c1In * this->conv3dRunInfo->hin *
                                 this->conv3dRunInfo->win * this->c0In;
        ...
        this->outputOneBatchSize = this->conv3dRunInfo->dout * this->c1Out * this->conv3dRunInfo->hout *
                                   this->conv3dRunInfo->wout * this->c0Out;
        ...
        int64_t hiIdxStart = (this->mIdxStart / this->conv3dRunInfo->wout) * this->conv3dRunInfo->strideH -
                             this->conv3dRunInfo->padTop;

        uint64_t fmStartAddr = this->batchIdxStart * this->fmapOneBatchSize +
                               this->Max(diIdxStart, 0) * this->c1In * this->conv3dRunInfo->hin *
                                   this->conv3dRunInfo->win * this->c0In +
                               groupOptIdxStart * fmapOneGroupOptSize +
    ...
    __aicore__ inline void Conv3DV2KernelImpl()
    {
        ...
        if (this->isDoDimTail || this->isNDimTail || this->isMDimTail || isGroupOptDimTail) [[unlikely]] {
            this->conv.SetSingleOutputShape(1, this->singleCoreN, this->singleCoreDout, this->singleCoreM,
                                            singleCoreGroupOpt);
        }
        ...
        ASC_OP_LOGD("[Conv3dv2WithGroups] doIdxStart %d mIdxStart %d diIdxStart %d hiIdxStart %d.\n", this->doIdxStart,
                    this->mIdxStart, diIdxStart, hiIdxStart);

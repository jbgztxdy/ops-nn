...
public:
    ...
    __aicore__ inline void LoadAL1()
    {
        ...
        if (unlikely(self_->ctx.mAL1UpdateFlag)) {
            CalcHiL1Pad();
            this->SetLoad3dFMatrix(self_->ctx.convTilingData->padLeft, self_->ctx.convTilingData->padRight, padTopL1,
                                   hiLoadL1, self_->ctx.convTilingData->orgWi);
        ...
        if (this->cin1LoadL1PadHead > 0) {
            this->Al1PadHeadSet2d(hiLoadL1, self_->ctx.convTilingData->orgWi);
        }
...
private:
    __aicore__ inline void CalcHiL1Pad()
    {
        ...
        hiLoadL1 = ((hoEndIdx - hoStartIdx) - 1) * self_->ctx.convTilingData->strideH + self_->ctx.dilatedKernelH;
        ...
        if (hiStartIdxWithPad < self_->ctx.convTilingData->padTop) {
            hiIdx = 0;
            padTopL1 = self_->ctx.convTilingData->padTop - hiStartIdxWithPad;
            hiLoadL1 -= padTopL1;
        }
    ...
    __aicore__ inline void SetDn2NzIntriParams(Dn2NzParams &intriParams)
    {
        ...
        intriParams.srcDValue = self_->ctx.convTilingData->orgDi * self_->ctx.convTilingData->orgHixWi;
        intriParams.srcDnMatrixStride = self_->ctx.convTilingData->orgHixWi * self_->ctx.convTilingData->dilationD;
    }
    ...
    __aicore__ inline void SetDn2NzIntriParams(Dn2NzParams &intriParams)
    {
        if (likely(wiLoadL1 == self_->ctx.convTilingData->orgWi)) {
            ...
            if constexpr (Intf::formatFmap == ConvFormat::NCDHW) {
                ...
                intriParams.srcDnMatrixStride = self_->ctx.convTilingData->orgWi * self_->ctx.convTilingData->orgCi;
            }
            intriParams.dstNzMatrixStride = wiLoadL1 * Intf::k0;
        }

        if constexpr (Intf::formatFmap == ConvFormat::NCDHW) {
            intriParams.srcDValue = self_->ctx.convTilingData->orgHixWi * self_->ctx.convTilingData->orgDi;
        } else {
            intriParams.srcDValue = self_->ctx.convTilingData->orgCi;
        }
        intriParams.dValue = this->cinLoadL1;

...
public:
    ...
    __aicore__ inline void LoadAL1Data()
    {
        uint64_t aL1GmPos = self_->ctx.batchIter * self_->ctx.fmapOneBatchSize+
                            this->cinIdx * self_->ctx.convTilingData->convApiTiling.orgDi * self_->ctx.convTilingData->convApiTiling.orgHixWi +
                            this->diIdx * self_->ctx.convTilingData->convApiTiling.orgHixWi +
    ...
    {
        uint64_t aL1GmPos = self_->ctx.batchIter * self_->ctx.fmapOneBatchSize +
                            this->diIdx * self_->ctx.convTilingData->convApiTiling.orgHixWi * self_->ctx.convTilingData->convApiTiling.orgCi +
    ...
    __aicore__ inline void LoadAL1()
    {
        ...
        if (unlikely(self_->ctx.mAL1UpdateFlag)) {
            CalcHiL1Pad();
            this->SetLoad3dFMatrix(self_->ctx.convTilingData->convApiTiling.padLeft, self_->ctx.convTilingData->convApiTiling.padRight, padTopL1,
                                   hiLoadL1, self_->ctx.convTilingData->convApiTiling.orgWi);
        ...
        if (this->cin1LoadL1PadHead > 0) {
            this->Al1PadHeadSet2d(hiLoadL1, self_->ctx.convTilingData->convApiTiling.orgWi);
        }
        ...
        if (this->cin1LoadL1PadTail > 0) {
            this->aL1Pos += this->dkLoadL1 * AlignB(this->cinLoadL1, Intf::k0) * hiLoadL1 * self_->ctx.convTilingData->convApiTiling.orgWi;
            this->Al1PadTailSet2d(hiLoadL1, self_->ctx.convTilingData->convApiTiling.orgWi);
        }
    }
...
private:
    __aicore__ inline void CalcHiL1Pad()
    {
        ...
        hiLoadL1 = ((hoEndIdx - hoStartIdx) - 1) * self_->ctx.convTilingData->convApiTiling.strideH + self_->ctx.dilatedKernelH;
        ...
        if (hiStartIdxWithPad < self_->ctx.convTilingData->convApiTiling.padTop) {
            hiIdx = 0;
            padTopL1 = self_->ctx.convTilingData->convApiTiling.padTop - hiStartIdxWithPad;
            hiLoadL1 -= padTopL1;
        }
    ...
    {
        uint64_t aL1Mi = hiLoadL1 * self_->ctx.convTilingData->convApiTiling.orgWi;
        ...
        intriParams.srcDValue = self_->ctx.convTilingData->convApiTiling.orgDi * self_->ctx.convTilingData->convApiTiling.orgHixWi;
        intriParams.srcDnMatrixStride = self_->ctx.convTilingData->convApiTiling.orgHixWi * self_->ctx.convTilingData->convApiTiling.dilationD;
    }
    ...
    __aicore__ inline void SetNd2NzIntriParams(Nd2NzParams& intriParams)
    {
        ...
        intriParams.srcNdMatrixStride = self_->ctx.convTilingData->convApiTiling.orgHixWi * self_->ctx.convTilingData->convApiTiling.dilationD *
...
public:
    ...
    {
        paddingWiAL1 = (self_->ctx.currentWoL1 - 1) * self_->ctx.convTilingData->convApiTiling.strideW + self_->ctx.dilatedKernelW;
        if (unlikely(self_->ctx.woL1SmallTail != 0 && woAL1Iter == self_->ctx.maxWoL1Iter)) {
            wiLeftPadIdx = ((woAL1Iter - 1) * self_->ctx.convTilingData->convApiTiling.woL1 + self_->ctx.woAL1Tail) *
                self_->ctx.convTilingData->convApiTiling.strideW - self_->ctx.convTilingData->convApiTiling.padLeft;
        } else {
            wiLeftPadIdx = woAL1Iter * self_->ctx.convTilingData->convApiTiling.woL1 * self_->ctx.convTilingData->convApiTiling.strideW -
        ...
        padRightL1 = wiRightPadIdx > self_->ctx.convTilingData->convApiTiling.orgWi ? (wiRightPadIdx - self_->ctx.convTilingData->convApiTiling.orgWi) : 0;
...
private:
    __aicore__ inline void LoadAl1Data()
    {
        ...
        if constexpr (Intf::formatFmap == ConvFormat::NCDHW) {
            aL1GmOffset = self_->ctx.convTilingData->convApiTiling.orgHixWi * self_->ctx.convTilingData->convApiTiling.dilationD;
            LoadAl1Data(realHiTopGmIdx, realWiTopGmIdx, al1Offset, aL1GmOffset);
        } else {
            aL1GmOffset = self_->ctx.convTilingData->convApiTiling.orgHixWi * self_->ctx.convTilingData->convApiTiling.dilationD * self_->ctx.convTilingData->convApiTiling.orgCi;
    ...
    __aicore__ inline void SetDn2NzIntriParams(Dn2NzParams &intriParams)
    {
        if (likely(wiLoadL1 == self_->ctx.convTilingData->convApiTiling.orgWi)) {
            intriParams.dnNum = 1;
            ...
            if constexpr (Intf::formatFmap == ConvFormat::NCDHW) {
                ...
                intriParams.srcDnMatrixStride = self_->ctx.convTilingData->convApiTiling.orgWi * self_->ctx.convTilingData->convApiTiling.orgCi;
            }
            intriParams.dstNzMatrixStride = wiLoadL1 * Intf::k0;
        }

        if constexpr (Intf::formatFmap == ConvFormat::NCDHW) {
            intriParams.srcDValue = self_->ctx.convTilingData->convApiTiling.orgHixWi * self_->ctx.convTilingData->convApiTiling.orgDi;
        } else {
            intriParams.srcDValue = self_->ctx.convTilingData->convApiTiling.orgCi;
        }
        intriParams.dValue = this->cinLoadL1;

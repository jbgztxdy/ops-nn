...
public:
    ...
    __aicore__ inline void Al1PadHeadSet2d(uint64_t hiLoadL1, uint64_t wiLoadL1) {
        ...
        if constexpr (Intf::isQuantScene || Intf::isDeQuantFlag) {
            uint16_t paddingValue = (static_cast<uint16_t>(self_->ctx.convTilingData->convApiTiling.offsetx)) << BIT_OFFSET_8 |
    ...
    __aicore__ inline void SetPadData()
    {
        if constexpr (Intf::isQuantScene || Intf::isDeQuantFlag) {
            ...
            InitConstValueParams<uint16_t> params(1, static_cast<uint16_t>(
                self_->ctx.convTilingData->convApiTiling.aL1SpaceSize / C0_SIZE), 0, padValue);
            buffAddr.dataLen = self_->ctx.convTilingData->convApiTiling.aL1SpaceSize;
            buffAddr.bufferAddr = self_->ctx.al1.GetPhyAddr();
            al1tmp.SetAddr(buffAddr);
    ...
    __aicore__ inline void CalcCiL1Pad()
    {
        ...
        uint64_t diStartWithPad = self_->ctx.diStartPos + self_->ctx.dOutIter * self_->ctx.convTilingData->convApiTiling.strideD +
                                  kdL1Idx * self_->ctx.convTilingData->convApiTiling.dilationD;
        diIdx = self_->ctx.diStartPos <= self_->ctx.convTilingData->convApiTiling.padHead ?
            diStartWithPad - self_->ctx.convTilingData->convApiTiling.padHead : diStartWithPad - self_->ctx.diStartPos;

        uint64_t diEndWithPad = diStartWithPad + (dkLoadL1 - 1) * self_->ctx.convTilingData->convApiTiling.dilationD + 1;

        cin1LoadL1PadHead = 0;
        cin1LoadL1PadTail = 0;

        if (diStartWithPad < self_->ctx.convTilingData->convApiTiling.padHead) {
            uint64_t kdTmp = CeilDiv((self_->ctx.convTilingData->convApiTiling.padHead - diStartWithPad),
                self_->ctx.convTilingData->convApiTiling.dilationD);
            kdTmp = kdTmp > dkLoadL1 ? dkLoadL1 : kdTmp;
            diIdx = self_->ctx.convTilingData->convApiTiling.dilationD == 1 ? 0 :
                kdTmp * self_->ctx.convTilingData->convApiTiling.dilationD - self_->ctx.convTilingData->convApiTiling.padHead + diStartWithPad;
            cin1LoadL1PadHead = kdTmp * AlignB(cinLoadL1, Intf::k0);
            dkLoadL1 -= kdTmp;
        }

        if (diEndWithPad > self_->ctx.convTilingData->convApiTiling.orgDi + self_->ctx.convTilingData->convApiTiling.padHead) {
            uint64_t kdTmp = CeilDiv(diEndWithPad - (self_->ctx.convTilingData->convApiTiling.orgDi + self_->ctx.convTilingData->convApiTiling.padHead),
                                     self_->ctx.convTilingData->convApiTiling.dilationD);
            kdTmp = kdTmp > dkLoadL1 ? dkLoadL1 : kdTmp;
...
public:
    ...
    __aicore__ inline void LoadBL1()
    {
        ...
        uint64_t dkIdx = 0;
        uint64_t cinIdx = 0;
        uint64_t coutIdx = self_->ctx.nBL1Iter * self_->ctx.convTilingData->convApiTiling.nBL1;
        ...
        uint64_t currentNBL1 = IsNBL1Tail() ? self_->ctx.nBL1Tail : self_->ctx.convTilingData->convApiTiling.nBL1;
        ...
        if constexpr (Intf::formatWeight == ConvFormat::NCDHW) {
            bL1GmPos = dkIdx * self_->ctx.convTilingData->convApiTiling.kernelHxkernelW + cinIdx *
                       self_->ctx.convTilingData->convApiTiling.kernelHxkernelWxkernelD + coutIdx * self_->ctx.singleCoreCi *
                       self_->ctx.convTilingData->convApiTiling.kernelHxkernelWxkernelD;
            bL1GmOffset = self_->ctx.convTilingData->convApiTiling.kernelHxkernelW;
        } else {
            bL1GmPos = dkIdx * self_->ctx.convTilingData->convApiTiling.kernelHxkernelW * self_->ctx.singleCoreCi *
                       self_->ctx.convTilingData->convApiTiling.orgCo + cinIdx * self_->ctx.convTilingData->convApiTiling.orgCo + coutIdx;
            bL1GmOffset = self_->ctx.convTilingData->convApiTiling.kernelHxkernelW * self_->ctx.singleCoreCi *
                       self_->ctx.convTilingData->convApiTiling.orgCo;
        }
        uint64_t bL1Pos = 0;
        uint64_t bL1Offset = AlignB(currentBL1Cin1, Intf::k0) * self_->ctx.convTilingData->convApiTiling.kernelHxkernelW *
...
private:
    __aicore__ inline void SetDn2NzIntriParams(Dn2NzParams &intriParams, const uint64_t currentNBL1)
    {
        ...
        intriParams.dstNzC0Stride = self_->ctx.convTilingData->convApiTiling.nBL1 * self_->ctx.convTilingData->convApiTiling.kernelHxkernelW;
        if constexpr (Intf::formatWeight == ConvFormat::NCDHW) {
            intriParams.srcDValue = self_->ctx.convTilingData->convApiTiling.kernelHxkernelWxkernelD;
            intriParams.dnNum = currentNBL1;
            intriParams.nValue = self_->ctx.convTilingData->convApiTiling.kernelHxkernelW;
            intriParams.srcDnMatrixStride = self_->ctx.singleCoreCi * self_->ctx.convTilingData->convApiTiling.kernelHxkernelWxkernelD;
            ...
            intriParams.srcDnMatrixStride = self_->ctx.singleCoreCi * self_->ctx.convTilingData->convApiTiling.orgCo;
            intriParams.srcDValue = self_->ctx.convTilingData->convApiTiling.orgCo;
            intriParams.dstNzNStride = 1;
            intriParams.dstNzMatrixStride = self_->ctx.convTilingData->convApiTiling.nBL1 * C0_SIZE / Intf::sizeOfWeight;
        }
    }

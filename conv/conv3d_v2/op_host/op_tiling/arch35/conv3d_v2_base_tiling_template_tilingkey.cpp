...
uint64_t Conv3dV2BaseTilingKey::GetFmpTilingValMMode(const bool kAL1FullloadFlag)
{
    ...
    mL1FullloadFlag = tilingData_.singleCoreHo <= tilingData_.hoL1;
    mL0FullloadFlag = tilingData_.hoL1 == tilingData_.hoL0;
...
uint64_t Conv3dV2BaseTilingKey::GetFmpTilingValHWMode(const bool kAL1FullloadFlag)
{
    ...
    woL0FullloadFlag = tilingData_.woL1 == tilingData_.woL0;
...
uint64_t Conv3dV2BaseTilingKey::GetWeightTilingVal()
{
    ...
    if (tilingData_.kBL1 == weightKSize) {
        kBL1FullloadFlag = true;
    }

    if (tilingData_.nBL1 == singleCoreNSize) {
...
{
    uint64_t l1PingPong = static_cast<uint64_t>(tilingData_.pBufferFlag & 0x18) >> L1_PB_OFFSET;
...
{
    return static_cast<uint64_t>(tilingData_.pBufferFlag & L0A_L0B_PB_FLAG_MASK);
}

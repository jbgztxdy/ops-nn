...
bool Conv3dTilingAlgorithmPointWise::CheckL1Buffer() const
{
    ...
    uint64_t currentFmL1Size = this->l1TilingRange.mAL1ValueRange.at(this->l1TilingIdx.mAL1Idx) *
                               this->doubleBufferValue.pbAL1 *
                               this->l1TilingRange.kAL1Range.at(this->l1TilingIdx.kAL1Idx) * this->fMapDTypeSize;
    ...
    uint64_t currentBiasL1Size = tilingIns_->hasBias ? biasSize * POINT_WISE_BIAS_CUBE_UNIT : INITIAL_SIZE;
...
uint64_t Conv3dTilingAlgorithmPointWise::CalcBTSize([[maybe_unused]] uint64_t currnL0) const { return INITIAL_SIZE; }

void Conv3dTilingAlgorithmPointWise::WeightBypassDecision() { return; }
...
int64_t Conv3dTilingAlgorithmPointWise::InitCalcL1Params()
{
    ...
    uint64_t fmapSizeInL1 = static_cast<uint64_t>(tilingIns_->shapeInfo.singlekD) * tilingIns_->shapeCalc.singleCi1 *
                            tilingIns_->shapeCalc.singleM1 *
                            static_cast<uint64_t>(tilingIns_->cubeInfo.m0 * tilingIns_->cubeInfo.k0);
    ...
    this->l1TilingCalc.fmapFullLoadL1Size = static_cast<uint64_t>(tilingIns_->shapeInfo.singlekD) *
                                            AlignUp(tilingIns_->shapeCalc.singleCi1 *
                                                        static_cast<uint64_t>(tilingIns_->cubeInfo.k0),
                                                    POINT_WISE_ALIGN_UNIT) *
                                            tilingIns_->shapeCalc.singleM1 *
                                            static_cast<uint64_t>(tilingIns_->cubeInfo.m0) * this->fMapDTypeSize;
    ...
    this->l1TilingCalc.fmapMinLoadL1Size = tilingIns_->cubeInfo.k0 * this->l0TilingParams.mL0 * this->fMapDTypeSize *
                                           this->doubleBufferValue.pbAL1 * multiK0;

    this->l1TilingCalc.fmapKL1FullLoadSize = static_cast<uint64_t>(tilingIns_->shapeInfo.singlekD) *
                                             tilingIns_->shapeCalc.singleCi1 *
                                             static_cast<uint64_t>(tilingIns_->cubeInfo.k0) * this->l0TilingParams.mL0 *
    ...
    this->l1TilingCalc.weightKL1FullLoadSize = static_cast<uint64_t>(tilingIns_->shapeInfo.singlekD) *
                                               ...
                                               static_cast<uint64_t>(this->doubleBufferValue.pbBL1) *
                                               this->weightDTypeSize;
    // cal bias size in L1
...
{
    uint64_t maxKAL12L0Loop = CeilDiv(this->l1TilingRange.kAL1Range.at(this->l1TilingIdx.kAL1Idx), k0);
    uint64_t maxKBL12L0Loop = CeilDiv(this->l1TilingRange.kBL1Range.at(this->l1TilingIdx.kBL1Idx), k0);
...
void Conv3dTilingAlgorithmPointWise::GetKL0TilingDecision()
{
    ...
    this->l0TilingIdx.kL0Idx = this->l0TilingIdx.kL0Idx == INITIAL_INDEX ?
                                   INITIAL_INDEX :
                                   static_cast<uint64_t>(this->l0TilingIdx.kL0Idx - 1);
...
{
    uint64_t usedL1Size = currmL0 * AlignUp(tilingIns_->cubeInfo.k0, POINT_WISE_ALIGN_UNIT) * fMapDTypeSize *
                          this->doubleBufferValue.pbAL1;
    usedL1Size += currnL0 * AlignUp(tilingIns_->cubeInfo.k0, POINT_WISE_ALIGN_UNIT) * weightDTypeSize *
                  this->doubleBufferValue.pbBL1;
...
{
    uint64_t fmapSingleCoreL1Load = AlignUp(static_cast<uint64_t>(tilingIns_->shapeInfo.singleM),
                                            static_cast<uint64_t>(tilingIns_->cubeInfo.m0)) *
                                    AlignUp(tilingIns_->shapeCalc.singleCi1 *
                                                static_cast<uint64_t>(tilingIns_->cubeInfo.k0),
                                            POINT_WISE_ALIGN_UNIT) *
...
bool Conv3dTilingAlgorithmPointWise::CoreL1TilingMinWeightBypass() const { return false; }

bool Conv3dTilingAlgorithmPointWise::NoneKABL1FullLoadWeightBypass() const { return false; }

...
public:
    void SetUp() { cout << "quant_convolution_test SetUp" << endl; }

    void TearDown() { cout << "quant_convolution_test TearDown" << endl; }
};
...
TEST_F(quant_convolution_test, ascend910B2_test_quantConv3D_bias)
{
    ...
    for (int i = 0; i < length; i++) {
        ...
        auto ut = OP_API_UT(
            aclnnQuantConvolution, // host api第二段接口名称
            INPUT(inp_desc, weight_desc, bias_desc, scale_desc, offset_desc, strides_desc, padding_desc, dilation_desc,
                  transposed, output_padding_desc, groups, offsetx, roundMode), // host api输入
            OUTPUT(output_desc));
...
TEST_F(quant_convolution_test, ascend_test_extend_conv2d_dtypes_group)
{
    std::vector<vector<aclDataType>> legalDtype = {
        ...
        {aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_FLOAT,
         aclDataType::ACL_FLOAT},
        {aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_FLOAT16,
         aclDataType::ACL_FLOAT},
        {aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_BF16, aclDataType::ACL_FLOAT},
        {aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_FLOAT8_E4M3FN, aclDataType::ACL_FLOAT8_E4M3FN,
         aclDataType::ACL_FLOAT}};
    const int64_t groups = 1;
    ...
    for (auto dtypes : legalDtype) {
        ...
        string roundMode = (dtypes[0] == aclDataType::ACL_INT8 || dtypes[0] == aclDataType::ACL_FLOAT8_E4M3FN) ?
                               "rint" :
                               "round";
        auto ut = OP_API_UT(
            aclnnQuantConvolution, // host api第二段接口名称
            INPUT(input_desc, weight_desc, bias_desc, scale_desc, offset_desc, strides_desc, padding_desc_1,
                  dilation_desc, transposed, output_padding_desc, groups, offsetx, roundMode.c_str()), // host api输入
            OUTPUT(output_desc));
        uint64_t workspace_size = 0;
        aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size); // check op graph
        // EXPECT_EQ(aclRet, ACL_SUCCESS);
    ...
    for (auto dtypes : legalDtype) {
        ...
        auto ut = OP_API_UT(
            aclnnQuantConvolution, // host api第二段接口名称
            INPUT(input_desc, weight_desc, nullptr, scale_desc, offset_desc, strides_desc, padding_desc_2,
                  dilation_desc, transposed, output_padding_desc, groups, offsetx, roundMode.c_str()), // host api输入
            OUTPUT(output_desc));
        uint64_t workspace_size = 0;
        aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size); // check op graph
...
TEST_F(quant_convolution_test, ascend_test_extend_conv2d_scaledtype_check)
{
    ...
    auto ut = OP_API_UT(
        aclnnQuantConvolution, // host api第二段接口名称
        INPUT(input_desc, weight_desc, bias_desc, scale_desc, offset_desc, strides_desc, padding_desc_2, dilation_desc,
              transposed, output_padding_desc, groups, offsetx, roundMode.c_str()), // host api输入
        OUTPUT(output_desc));
    uint64_t workspace_size = 0;
    aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size); // check op graph
    // EXPECT_EQ(aclRet, ACL_SUCCESS);
...
TEST_F(quant_convolution_test, ascend_test_extend_conv2d_inputdim_check)
{
    ...
    auto ut = OP_API_UT(
        aclnnQuantConvolution, // host api第二段接口名称
        INPUT(input_desc, weight_desc, bias_desc, scale_desc, offset_desc, strides_desc, padding_desc_1, dilation_desc,
              transposed, output_padding_desc, groups, offsetx, roundMode.c_str()), // host api输入
        OUTPUT(output_desc));
    uint64_t workspace_size = 0;
    aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size); // check op graph

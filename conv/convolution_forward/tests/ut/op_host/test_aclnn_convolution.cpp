...
public:
    void SetUp() { cout << "convolution_test SetUp" << endl; }

    void TearDown() { cout << "convolution_test TearDown" << endl; }
};
...
TEST_F(convolution_test, test_conv2D_input_FP16_weight_FP16_output_FP32)
{
    ...
    for (int i = 0; i < length; i++) {
        ...
        auto ut = OP_API_UT(aclnnConvolution, // host api第二段接口名称
                            INPUT(inp_desc, weight_desc, bias_desc, strides_desc, padding_desc, dilation_desc,
                                  transposed, output_padding_desc, groups), // host api输入
...
TEST_F(convolution_test, test_specialConv1D_FP16_DATA_ONES)
{
    ...
    auto
        inp_desc = TensorDesc(inp_dims, ACL_FLOAT16, ACL_FORMAT_NCL).ValueRange(-2, 2).Value(vector<float>(16 * 64, 1));
    auto weight_desc = TensorDesc(weight_dims, ACL_FLOAT16, ACL_FORMAT_NCL)
                           .ValueRange(-2, 2)
                           .Value(vector<float>(64 * 3, 1));
...
TEST_F(convolution_test, test_conv2D_FP16_no_contiguous)
{
    ...
    auto inp_desc = TensorDesc(inp_dims, ACL_FLOAT16, ACL_FORMAT_NCHW, {1, 1, 1, 16}, 0, {2, 2, 32, 16})
                        .ValueRange(0, 2);
    auto weight_desc = TensorDesc(weight_dims, ACL_FLOAT16, ACL_FORMAT_NCHW, {1, 1, 1, 3}, 0, {2, 2, 3, 3})
                           .ValueRange(0, 2);
    auto bias_desc = TensorDesc(bias_dims, ACL_FLOAT16, ACL_FORMAT_NCHW).ValueRange(0, 2);
    auto strides_desc = IntArrayDesc(strides_dims);
    auto padding_desc = IntArrayDesc(padding_dims);
...
TEST_F(convolution_test, ascend910B2_test_conv2D_C04_BFLOAT16)
{
    ...
    const int64_t groups = 1;
    bool transposed = false;
    vector<vector<int64_t>> inp_dims = {
        {32, 3, 1, 224}, {32, 3, 224, 224}, {32, 1, 256, 256}, {32, 3, 224, 224}, {16, 1, 20, 32768}};
    vector<vector<int64_t>> weight_dims = {
        {64, 3, 1, 32}, {64, 3, 32, 32}, {64, 1, 128, 128}, {64, 3, 32, 32}, {16, 1, 1, 32}};
    vector<int64_t> bias_dims = {64};
    vector<vector<int64_t>> output_dims = {
        {32, 64, 1, 193}, {32, 64, 193, 193}, {32, 64, 129, 129}, {32, 64, 193, 193}, {16, 16, 20, 32737}};
    vector<int64_t> strides_dims = {1, 1};
    vector<int64_t> padding_dims = {0, 0};
    vector<int64_t> dilation_dims = {1, 1};
    vector<int64_t> output_padding_dims = {0, 0};
...
TEST_F(convolution_test, ascend910B2_test_conv2D_with_asymmetric_pad)
{
    ...
    bool transposed = false;
    vector<vector<int64_t>> inp_dims = {{16, 40, 120, 120}, {16, 40, 120, 120}, {16, 64, 1, 120}, {4, 32, 16, 4200},
                                        {4, 32, 1, 4200},   {16, 32, 40, 40},   {16, 3, 224, 224}};
    vector<vector<int64_t>> weight_dims = {{31, 40, 10, 10}, {31, 40, 10, 10}, {31, 64, 1, 10}, {32, 32, 52, 52},
                                           {32, 32, 1, 52},  {16, 32, 4, 64},  {34, 3, 16, 16}};
    vector<vector<int64_t>> output_dims = {{16, 31, 30, 31}, {16, 31, 30, 30}, {16, 31, 1, 29}, {4, 32, 3, 72},
                                           {4, 32, 1, 65},   {16, 16, 15, 1},  {16, 34, 59, 61}};
    vector<vector<int64_t>> padding_dims = {{2, 4, 4, 6}, {3, 3},   {0, 2},          {100, 102, 192, 204},
                                            {0, 0, 0, 0}, {10, 12}, {10, 14, 14, 18}};
    ...
    for (int i = 0; i < length; i++) {
        ...
        auto ut = OP_API_UT(aclnnConvolution, // host api第二段接口名称
                            INPUT(inp_desc, weight_desc, bias_desc, strides_desc, padding_desc, dilation_desc,
                                  transposed, output_padding_desc, groups[i]), // host api输入
                            OUTPUT(output_desc), cubeMathType);
        uint64_t workspace_size = 0;
        aclnnStatus aclRet = ut.TestGetWorkspaceSize(&workspace_size); // check op graph
        EXPECT_EQ(aclRet, ACL_SUCCESS);
